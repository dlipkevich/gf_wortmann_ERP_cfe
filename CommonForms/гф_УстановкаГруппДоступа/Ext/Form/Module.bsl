...
Процедура КомандаЗаписатьНаСервере()
	...
	|		ПО гф_ГлобальныеЗначения.Значение = Организации.Родитель
	...
	|ГДЕ
	|	гф_ГлобальныеЗначения.Ключ = ""ГруппаДоступаДляКонтрагентов""
	|	И Организации.ЭтоГруппа 
	|	
	|;
	|    
	|////////////////////////////////////////////////////////////////////////////////
	...
	|ГДЕ
	|	ГруппыДоступа.Группа <> ЗНАЧЕНИЕ(Справочник.ГруппыДоступа.ПустаяСсылка)
	|;
	|	
	|ВЫБРАТЬ
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.СоздатьУдалить Тогда
				...
				СтрокаЗначенияДоступа.ВидДоступа		= ПредопределенноеЗначение("Справочник.Партнеры.ПустаяСсылка");
...
Процедура КомандаЗаписать(Команда)
	...
	Если ЗначениеЗаполнено(ГруппаДоступаПартнера) Тогда
	
		КомандаЗаписатьНаСервере();
		
	Иначе
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Группа доступа не заполнена. Запись не производится.");
...
Процедура КомандаОбновитьНаСервере()
	...
	СмещениеРезультатаОрганизацийГрупп	= 1;
	СмещениеРезультатаГруппДляВыбора	= 2;
	СмещениеРезультатаОрганизаций		= 3;
	
	ИндексРезультатаОрганизацийГрупп	= Результат.Количество() - СмещениеРезультатаОрганизацийГрупп;
	ИндексРезультатаГруппДляВыбора		= Результат.Количество() - СмещениеРезультатаГруппДляВыбора;
	ИндексРезультатаОрганизаций			= Результат.Количество() - СмещениеРезультатаОрганизаций;
