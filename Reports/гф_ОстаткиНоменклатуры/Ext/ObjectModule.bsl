...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если НЕ парамВидЦены.Использование Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Укажите Вид цены!");
		Возврат;
	КонецЕсли;
	...
	Для Каждого ЭлементОтбора Из Настройки.Отбор.Элементы Цикл
		Если ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТоварыВКоробах") Тогда
			КоробнойСклад = ЭлементОтбора.ПравоеЗначение;
		КонецЕсли;
	КонецЦикла;
	...
	Если парамПоказатьЦенуЗаказа.Значение = Неопределено Тогда
		парамПоказатьЦенуЗаказа.Значение = ЛОЖЬ;
	Иначе
		парамПоказатьЦенуЗаказа.Значение = ?(парамПоказатьЦенуЗаказа.Значение = Истина, Истина, Ложь);
	КонецЕсли;
	...
	Если НЕ парамВидЦены.Использование Тогда
		СхемаКомпоновкиДанных.НаборыДанных.НаборДанных1.Запрос = СтрЗаменить(
			СхемаКомпоновкиДанных.НаборыДанных.НаборДанных1.Запрос, "ВидЦены = &ВидЦены", "");
	КонецЕсли;
	...
	ЗаполнитьРисунки(ДокументРезультат, ДанныеРасшифровки);
...
// Формирует структуру с таблицей значений. Колонки: Склад, ТоварыВКоробах
//
...
Процедура ЗаполнитьРисунки(ДокументРезультат, ДанныеРасшифровки)
	...
	Для Кл = 1 По КоличествоКолонокТаблицы Цикл 		
		Для Ст = 1 По КоличествоСтрокТаблицы Цикл  			
			...
			Если ТипЗнч(ЗначениеРасшифровки) = Тип("СправочникСсылка.НоменклатураПрисоединенныеФайлы")
				...
				КонецЕсли;
			КонецЕсли;   // Добавлено  Галфинд \ Sakovich 15.03.2024
		КонецЦикла;  		
	КонецЦикла;
