...
Процедура ПриОткрытии(Отказ)
	...
	// #wortmann { 
	// Заполнение АдресаСервера и КлючаПодключения при открытии формы
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee5c410a8a706f
	// Галфинд_Домнышева 2023/09/27
	АдресСервера = _омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначение("гф_СерверСостояниеТоваровМаркировка");
...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	...
	Если ЭтоМобильныйКлиент Тогда
		РасположениеРесурсов = Отчет.КомпоновщикНастроек.Настройки.ПараметрыВывода.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("РасположениеРесурсов"));
		Если РасположениеРесурсов.Использование
			И РасположениеРесурсов.Значение = РасположениеРесурсовКомпоновкиДанных.Вертикально Тогда
			Элементы.ДекорацияИзменитьРасположениеРесурсов.Заголовок = "Горизонтально";
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииДоступностиОсновногоСервера(СтандартнаяОбработка)
	...
	Если ОсновнойСерверДоступен() = Ложь Тогда
		ОткрытьФорму("РегистрСведений.ОбновляемыеРезультатыОтчетов.Форма.ФормаПросмотраОтчетов");
		Закрыть();
	КонецЕсли;
...
Процедура ОтчетСкомпоноватьРезультат(Команда)
	Если Не ЗначениеЗаполнено(Отчет.Организация) Тогда
		Сообщить("Не заполнено значение Организации. Отчет не будет сформирован");
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьРасположениеРесурсов(Команда)
	...
	КомпоновкаДанныхКлиентСервер.УстановитьПараметрВывода(Отчет.КомпоновщикНастроек, "РасположениеРесурсов", РасположениеРесурсовКомпоновкиДанных[РасположениеРесурсов]);
	Элементы.ДекорацияИзменитьРасположениеРесурсов.Заголовок = ?(РасположениеРесурсов = "Вертикально", "Горизонтально", "Вертикально");
...
&НаКлиенте
Процедура СохранитьОфлайн(Команда)

	СохранитьОфлайнОтчет();

КонецПроцедуры

&НаКлиенте
Процедура СохранитьВсеОфлайн(Команда)
	
	ДлительнаяОперация = ОбновитьОфлайнОтчеты();
...
Процедура СписокОфлайнОтчетов(Команда)
	...
	ОткрытьФорму("РегистрСведений.ОбновляемыеРезультатыОтчетов.ФормаСписка",
				Новый Структура("Пользователь", ПользователиКлиент.ТекущийПользователь()));
...
Процедура ПослеОбновленияОфлайнОтчетов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ТекстСообщения = НСтр("ru = 'Произошла ошибка обновления офлайн-отчетов:';
								|en = 'An error occurred while updating the offline reports:'") + Символы.ПС + Результат.КраткоеПредставлениеОшибки;
	Иначе
		ТекстСообщения = НСтр("ru = 'Офлайн-отчеты пользователя актуализированы.';
								|en = 'User offline reports are updated.'")
	КонецЕсли;
...
Процедура НачатьЗамер()
	...
	// В связи с изменениями в БСП "рефиксЗамеров" в НастройкиОтчета больше не используется.
	// Код изменен согласно типовой функции "НачатьЗамер" из общей формы "ФормаОтчета".
...
Процедура ЗагрузитьРезультатФормированияОтчета()
	...
	//RS
	//ТабДокумент = Результат.ТабличныйДокумент;
	...
	Отчет.СравнениеТоваров.Очистить();
	
	Для Каждого ТекСтр Из ТабЗначений Цикл
		
		НовСтр = Отчет.СравнениеТоваров.Добавить();
		НовСтр.Номенклатура 		= ТекСтр.Номенклатура;
		НовСтр.Характеристика 		= ТекСтр.Характеристика;
		НовСтр.GTIN 				= ТекСтр.Штрихкод;
		НовСтр.Артикул 				= ТекСтр.Артикул;
		НовСтр.Сезон 				= ТекСтр.Номенклатура_Supplier_season_свойство_;
		
		НовСтр.Наименование_1С  	= ТекСтр.НаименованиеПолное;
		НовСтр.СтатусGTIN_НК_1С 	= ТекСтр.СостояниеВыгрузкиНоменклатуры;
		НовСтр.ТНВЭД_1С 			= ТекСтр.ТНВЭД;
		НовСтр.МВ_1С 				= ТекСтр.Номенклатура_MaterialSurface_свойство_;
		НовСтр.МН_1С 				= ТекСтр.Номенклатура_MaterialBottom_свойство_;
		НовСтр.МП_1С 				= ТекСтр.Номенклатура_MaterialLining_свойство_;
		
	    НовСтр.ЕстьОтличия 			= Истина;
		
	КонецЦикла;
	
	//сформируем переводы свойств
	ЗаполнитьПереводыСвойств();
	
	//запросим данные по АПИ от НацКаталога
	ЗаполнитьДанныеОтНациональногоКаталога();
	
	//выведем таб док
	ВывестиОтчетСравнение(ОтчетТабличныйДокумент);
...
Процедура ПерейтиКНастройкам(РасширенныйРежим = Неопределено)
	...
	Если ВыполнятьЗамеры Тогда
		МодульОценкаПроизводительностиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ОценкаПроизводительностиКлиент");
		// ++ Галфинд_ДомнышеваКР_11_10_2023
		// В связи с изменениями в БСП "ПрефиксЗамеров" в НастройкиОтчета больше не используется.
		// Код изменен согласно типовой функции "НачатьЗамер" из общей формы "ФормаОтчета".
		ИдентификаторЗамера = МодульОценкаПроизводительностиКлиент.ЗамерВремени(
		НастройкиОтчета.КлючЗамеров + ".Формирование",
		Ложь, Ложь);
...
&НаСервере
Процедура СохранитьОфлайнОтчет()
	
	Если НастройкиОтчета.СвойстваРезультата.КомпоновщикНастроек = Неопределено Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Отчет не сформирован!';
													|en = 'The report is not generated.'"));
		Возврат;
	КонецЕсли;
	
	РезультатОтчета = СкопироватьТабличныйДокумент(ОтчетТабличныйДокумент);
	
	РегистрыСведений.ОбновляемыеРезультатыОтчетов.СохранитьОфлайнОтчет(РезультатОтчета, НастройкиОтчета);
	
КонецПроцедуры

&НаСервере
Функция ОбновитьОфлайнОтчеты()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Обновление офлайн-отчетов пользователя';
															|en = 'Update user offline reports'");
...
Процедура ОпределитьПоведениеВМобильномКлиенте()
	...
	Элементы.КомандыИПоказатели.Заголовок = НСтр("ru = 'показатели';
												|en = 'indicators'");
	Элементы.Переместить(Элементы.КомандыИПоказатели, ЭтаФорма, Элементы.ПредопределенныеЭлементыНастроек);
...
Процедура ЗагрузитьНастройкиВКомпоновщик(ПараметрыЗагрузки)
	...
	ДобавлениеПараметровДанныхВНастройки();  // Галфинд_Домнышева 2023/05/26

КонецПроцедуры

// #wortmann { 
// Добавление параметров в настройки отчета
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8ea8b083fed1320811edf965bf211028
// Галфинд_Домнышева 2023/05/26
&НаСервере
Процедура ДобавлениеПараметровДанныхВНастройки()
	
	ОтчетОбъект = РеквизитФормыВЗначение("Отчет");

	Если СписокАртикулШтрихкод.Количество() = 0 Тогда
		ОтборПоСпискуGTIN = Ложь;
		ОтборПоСпискуАртикул = Ложь;
	Иначе 
		ОтборПоСпискуGTIN = ?(ВыборGTIN = "1", Истина, Ложь);
		ОтборПоСпискуАртикул = ?(ВыборGTIN = "2", Истина, Ложь);
	КонецЕсли;
	
	Настройки = ОтчетОбъект.КомпоновщикНастроек.ПолучитьНастройки();
    	
	ЗначениеПоиска = Новый ПараметрКомпоновкиДанных("Список");
	парамСписок = Настройки.ПараметрыДанных.НайтиЗначениеПараметра(ЗначениеПоиска);
	парамСписок.Значение = СписокАртикулШтрихкод;
	парамСписок.Использование = Истина; 
	
	ЗначениеПоиска = Новый ПараметрКомпоновкиДанных("ОтборПоСпискуGTIN");
	парамОтборПоСпискуGTIN = Настройки.ПараметрыДанных.НайтиЗначениеПараметра(ЗначениеПоиска);
	парамОтборПоСпискуGTIN.Значение = ОтборПоСпискуGTIN;
	парамОтборПоСпискуGTIN.Использование = Истина; 
	
	ЗначениеПоиска = Новый ПараметрКомпоновкиДанных("ОтборПоСпискуАртикул");
	парамОтборПоСпискуАртикул = Настройки.ПараметрыДанных.НайтиЗначениеПараметра(ЗначениеПоиска);
	парамОтборПоСпискуАртикул.Значение = ОтборПоСпискуАртикул;
	парамОтборПоСпискуАртикул.Использование = Истина; 
...
&НаСервере
Процедура ЗаполнитьПереводыСвойств()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	внешТЗ.МВ_1С КАК МВ_1С,
	|	внешТЗ.МН_1С КАК МН_1С,
	|	внешТЗ.МП_1С КАК МП_1С
	|ПОМЕСТИТЬ Т1
	|ИЗ
	|	&внешТЗ КАК внешТЗ
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	RC_Переводы.Объект КАК Материал,
	|	RC_Переводы.Значение КАК Перевод
	|ПОМЕСТИТЬ ВТ_ВсеПереводы
	|ИЗ
	|	РегистрСведений.гф_ПереводЗначенийРеквизитовИСвойств КАК RC_Переводы
	|ГДЕ
	|	RC_Переводы.Объект В
	|			(ВЫБРАТЬ
	|				ВТ_ВсеМатериалы.Материал
	|			ИЗ
	|				ВТ_ВсеМатериалы)
	|	И RC_Переводы.Язык = &ЯзыкНацКаталог
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_МВ.МВ_1С КАК МВ_1С,
	|	ЕСТЬNULL(ВТ_ВсеПереводы.Перевод, """") КАК Перевод
	|ИЗ
	|	ВТ_МВ КАК ВТ_МВ
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ВсеПереводы КАК ВТ_ВсеПереводы
	|		ПО ВТ_МВ.МВ_1С = ВТ_ВсеПереводы.Материал
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_МН.МН_1С КАК МН_1С,
	|	ЕСТЬNULL(ВТ_ВсеПереводы.Перевод, """") КАК Перевод
	|ИЗ
	|	ВТ_МН КАК ВТ_МН
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ВсеПереводы КАК ВТ_ВсеПереводы
	|		ПО ВТ_МН.МН_1С = ВТ_ВсеПереводы.Материал
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_МП.МП_1С КАК МП_1С,
	|	ЕСТЬNULL(ВТ_ВсеПереводы.Перевод, """") КАК Перевод
	|ИЗ
	|	ВТ_МП КАК ВТ_МП
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ВсеПереводы КАК ВТ_ВсеПереводы
	|		ПО ВТ_МП.МП_1С = ВТ_ВсеПереводы.Материал"; 
	
	Запрос.УстановитьПараметр("внешТЗ", Отчет.СравнениеТоваров.Выгрузить(,"МВ_1С, МН_1С, МП_1С"));
	Запрос.УстановитьПараметр("ЯзыкНацКаталог", Справочники.гф_ВидыЯзыков.НацКаталог);//НайтиПоНаименованию("Нац. каталог"));
	
	ПакетРезультатов = Запрос.ВыполнитьПакет();  
	
	ПереводыМВ = ПакетРезультатов[6].Выгрузить();
	ПереводыМН = ПакетРезультатов[7].Выгрузить();
	ПереводыМП = ПакетРезультатов[8].Выгрузить();
	
	СтруктураПоиска = Новый Структура;
	Для Каждого ТекСтрока Из ПереводыМВ Цикл
		
		Если ПустаяСтрока(ТекСтрока.Перевод) И ЗначениеЗаполнено(ТекСтрока.МВ_1С) Тогда
			Сообщить("Не нашли перевод для свойства материала верха: "+ ТекСтрока.МВ_1С);
			Продолжить;
		КонецЕсли;
		
		СтруктураПоиска.Вставить("МВ_1С", ТекСтрока.МВ_1С);
		НайденныеСтроки = Отчет.СравнениеТоваров.НайтиСтроки(СтруктураПоиска);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			НайденнаяСтрока.МВ_1С_Перевод = ТекСтрока.Перевод;
		КонецЦикла;	
		
	КонецЦикла;
	
	СтруктураПоиска.Очистить();
	Для Каждого ТекСтрока Из ПереводыМН Цикл
		
		Если ПустаяСтрока(ТекСтрока.Перевод) И ЗначениеЗаполнено(ТекСтрока.МН_1С) Тогда
			Сообщить("Не нашли перевод для свойства материала низа: "+ ТекСтрока.МН_1С);
			Продолжить;
		КонецЕсли;
		
		СтруктураПоиска.Вставить("МН_1С", ТекСтрока.МН_1С);
		НайденныеСтроки = Отчет.СравнениеТоваров.НайтиСтроки(СтруктураПоиска);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			НайденнаяСтрока.МН_1С_Перевод = ТекСтрока.Перевод;
		КонецЦикла;	
		
	КонецЦикла;
	
	СтруктураПоиска.Очистить();
	Для Каждого ТекСтрока Из ПереводыМП Цикл
		
		Если ПустаяСтрока(ТекСтрока.Перевод) И ЗначениеЗаполнено(ТекСтрока.МП_1С) Тогда
			Сообщить("Не нашли перевод для свойства материала подкладки: "+ ТекСтрока.МП_1С);
			Продолжить;
		КонецЕсли;
		
		СтруктураПоиска.Вставить("МП_1С", ТекСтрока.МП_1С);
		НайденныеСтроки = Отчет.СравнениеТоваров.НайтиСтроки(СтруктураПоиска);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			НайденнаяСтрока.МП_1С_Перевод = ТекСтрока.Перевод;
		КонецЦикла;	
...
&НаСервере
Процедура ЗаполнитьДанныеОтНациональногоКаталога()

	//пойдем запросим данные
	ПараметрыПодключения = гф_ИнтеграцияСервер.ПолучитьПараметрыПодключенияКHTTPСервису(Отчет.Организация);
	
	//ПараметрыПодключения.Адрес = "markirovka.crpt.ru";                 //рабочий
	//ПараметрыПодключения.Адрес = "markirovka.sandbox.crptech.ru";    //тестовый
	
	ПараметрыПодключения.Адрес = АдресСервера;
	...
	//соберем gtins по 25 шт
	МаксКолвоGTIN = 25;
	ТекКолво = 0;
	
	СтрокаGTINS = "";
	
	ПорядковыйНомерЗапроса = 1;
	
	ВсегоОжидаемЗапросов = Цел(Отчет.СравнениеТоваров.Количество() / 25);
	Если НЕ (ВсегоОжидаемЗапросов = Отчет.СравнениеТоваров.Количество() / 25) Тогда
		ВсегоОжидаемЗапросов = ВсегоОжидаемЗапросов + 1;
	КонецЕсли;
	
	Для Каждого СтрокаДанных Из Отчет.СравнениеТоваров Цикл
		
		
		СтрокаGTINS = СтрокаGTINS + ?(ПустаяСтрока(СтрокаGTINS),"",";") + СтрокаДанных.GTIN;
		
		ТекКолво = ТекКолво + 1;
		
		Если ТекКолво >= МаксКолвоGTIN Тогда
			

			ОбработатьПорциюДанных(СоединениеССервисом, СтрокаGTINS, Ключ);
			ТекКолво = 0;
			СтрокаGTINS = "";
			
			ПорядковыйНомерЗапроса = ПорядковыйНомерЗапроса +  1;
			
			гф_ИнтеграцияСервер.Пауза(3);
...
&НаСервере
Процедура ВывестиОтчетСравнение(ДокументРезультат)  
	
	ДокументРезультат.Очистить();
	
	ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
	Макет = ОтчетОбъект.ПолучитьМакет("ПФ_MXL_Сравнение");                
	
	ОбластьМакета = Макет.ПолучитьОбласть("ЗаголовокОтчета");
	ДокументРезультат.Вывести(ОбластьМакета);
	
	ОбластьМакета = Макет.ПолучитьОбласть("ЗаголовокТаблицы|ОсновнаяЧасть");
	ДокументРезультат.Вывести(ОбластьМакета);

	ОбластьМакета = Макет.ПолучитьОбласть("ЗаголовокТаблицы|ПрисоединяемаяЧасть");
	ОбластьМакета.Параметры.ИсточникСведений = "1С";
	ДокументРезультат.Присоединить(ОбластьМакета);
	
	ОбластьМакета.Параметры.ИсточникСведений = "Национальный каталог";
	ДокументРезультат.Присоединить(ОбластьМакета);
	
	КрасныйЦвет = WebЦвета.Красный;
	ЧерныйЦвет 	= WebЦвета.Черный;
	СинийЦвет	= WebЦвета.Синий;
	
	НПП = 1;
	
	Для Каждого ТекСтрока Из Отчет.СравнениеТоваров Цикл
		
		Если Отчет.ВариантВыводаДанных = 1 И НЕ ТекСтрока.ЕстьОтличия Тогда
			Продолжить;
		КонецЕсли;
		
		Наименование_Отличается 	= Ложь;
		СтатусGTIN_Отличается 		= Ложь;
		ТНВЭД_Отличается 			= Ложь;
		МВ_Отличается 				= 0;
		МН_Отличается 				= 0;
		МП_Отличается 				= 0;
		
		ОбластьМакета_Осн 		= Макет.ПолучитьОбласть("СтрокаТаблицы|ОсновнаяЧасть");
		ОбластьМакета_Присоед 	= Макет.ПолучитьОбласть("СтрокаТаблицы|ПрисоединяемаяЧасть");
		
		ОбластьМакета_Осн.Параметры.НПП 	= НПП;
		ОбластьМакета_Осн.Параметры.GTIN 	= ТекСтрока.GTIN;
		ОбластьМакета_Осн.Параметры.Артикул = ТекСтрока.Артикул;
		ОбластьМакета_Осн.Параметры.Сезон 	= ТекСтрока.Сезон;
		ОбластьМакета_Осн.Параметры.Характеристика 	= ТекСтрока.Характеристика; // Галфинд_Домнышева_26_05_2023
		
		ОбластьМакета_Осн.Параметры.Номенклатура = ТекСтрока.Номенклатура;
		
		ДокументРезультат.Вывести(ОбластьМакета_Осн);
		
		ОбластьМакета_Присоед.Параметры.Наименование 		= ТекСтрока.Наименование_1С;
		ОбластьМакета_Присоед.Параметры.СтатусGTIN 			= ТекСтрока.СтатусGTIN_НК_1С;
		ОбластьМакета_Присоед.Параметры.КодТНВЭД 			= ТекСтрока.ТНВЭД_1С;
		ОбластьМакета_Присоед.Параметры.МатериалВерха 		= ТекСтрока.МВ_1С_Перевод;
		ОбластьМакета_Присоед.Параметры.МатериалНиза 		= ТекСтрока.МН_1С_Перевод;
		ОбластьМакета_Присоед.Параметры.МатериалПодкладки 	= ТекСтрока.МП_1С_Перевод;
		
		ОбластьМакета_Присоед.Параметры.Номенклатура 		= ТекСтрока.Номенклатура;
		
		Если НРег(СокрЛП(ТекСтрока.Наименование_1С)) <> НРег(СокрЛП(ТекСтрока.Наименование_НК)) Тогда
			ОбластьМакета_Присоед.Область("ОбластьНаименование").ЦветТекста = КрасныйЦвет;
			Наименование_Отличается = Истина;
		Иначе
			ОбластьМакета_Присоед.Область("ОбластьНаименование").ЦветТекста = ЧерныйЦвет;
		КонецЕсли; 
		
		СтроковоеПредставлениеСтатусаВ1С = ПолучитьСтроковоеПредставлениеСтатусаGTIN_НК(ТекСтрока.СтатусGTIN_НК_1С);
	
		Если СтроковоеПредставлениеСтатусаВ1С <> СокрЛП(ТекСтрока.СтатусGTIN_НК_НК) Тогда
			ОбластьМакета_Присоед.Область("ОбластьСтатусGTIN").ЦветТекста = КрасныйЦвет;
			СтатусGTIN_Отличается = Истина;
		Иначе
			ОбластьМакета_Присоед.Область("ОбластьСтатусGTIN").ЦветТекста = ЧерныйЦвет;
		КонецЕсли;
		
		Если СокрЛП(ТекСтрока.ТНВЭД_1С) <> СокрЛП(ТекСтрока.ТНВЭД_НК) Тогда
			ОбластьМакета_Присоед.Область("ОбластьКодТНВЭД").ЦветТекста = КрасныйЦвет;
			ТНВЭД_Отличается = Истина;
		Иначе
			ОбластьМакета_Присоед.Область("ОбластьКодТНВЭД").ЦветТекста = ЧерныйЦвет;
		КонецЕсли;
		
		МВ_Отличается = СравнитьСвойствоСДаннымиНК(ТекСтрока.МВ_1С, ТекСтрока.МВ_1С_Перевод, ТекСтрока.МВ_НК);
		ОбластьМакета_Присоед.Область("ОбластьМВ").ЦветТекста = ?(МВ_Отличается = 2, КрасныйЦвет, ?(МВ_Отличается = 1, СинийЦвет, ЧерныйЦвет));
		
		МН_Отличается = СравнитьСвойствоСДаннымиНК(ТекСтрока.МН_1С, ТекСтрока.МН_1С_Перевод, ТекСтрока.МН_НК);
		ОбластьМакета_Присоед.Область("ОбластьМН").ЦветТекста = ?(МН_Отличается = 2, КрасныйЦвет, ?(МН_Отличается = 1, СинийЦвет, ЧерныйЦвет));
		
		МП_Отличается = СравнитьСвойствоСДаннымиНК(ТекСтрока.МП_1С, ТекСтрока.МП_1С_Перевод, ТекСтрока.МП_НК);
		ОбластьМакета_Присоед.Область("ОбластьМП").ЦветТекста = ?(МП_Отличается = 2, КрасныйЦвет, ?(МП_Отличается = 1, СинийЦвет, ЧерныйЦвет));
		
		ДокументРезультат.Присоединить(ОбластьМакета_Присоед);
		
		ОбластьМакета_Присоед.Параметры.Наименование 		= ТекСтрока.Наименование_НК;
		ОбластьМакета_Присоед.Параметры.СтатусGTIN 			= ТекСтрока.СтатусGTIN_НК_НК;
		ОбластьМакета_Присоед.Параметры.КодТНВЭД 			= ТекСтрока.ТНВЭД_НК;
		ОбластьМакета_Присоед.Параметры.МатериалВерха 		= ТекСтрока.МВ_НК;
		ОбластьМакета_Присоед.Параметры.МатериалНиза 		= ТекСтрока.МН_НК;
		ОбластьМакета_Присоед.Параметры.МатериалПодкладки 	= ТекСтрока.МП_НК;
		
		ОбластьМакета_Присоед.Параметры.Номенклатура 		= ТекСтрока.Номенклатура;
		
		ОбластьМакета_Присоед.Область("ОбластьНаименование").ЦветТекста = ?(Наименование_Отличается, КрасныйЦвет, ЧерныйЦвет);
		ОбластьМакета_Присоед.Область("ОбластьСтатусGTIN").ЦветТекста 	= ?(СтатусGTIN_Отличается, КрасныйЦвет, ЧерныйЦвет);
		ОбластьМакета_Присоед.Область("ОбластьКодТНВЭД").ЦветТекста 	= ?(ТНВЭД_Отличается, КрасныйЦвет, ЧерныйЦвет);
		ОбластьМакета_Присоед.Область("ОбластьМВ").ЦветТекста 			= ?(МВ_Отличается = 2, КрасныйЦвет, ?(МВ_Отличается = 1, СинийЦвет, ЧерныйЦвет));
		ОбластьМакета_Присоед.Область("ОбластьМН").ЦветТекста 			= ?(МН_Отличается = 2, КрасныйЦвет, ?(МН_Отличается = 1, СинийЦвет, ЧерныйЦвет));
		ОбластьМакета_Присоед.Область("ОбластьМП").ЦветТекста 			= ?(МП_Отличается = 2, КрасныйЦвет, ?(МП_Отличается = 1, СинийЦвет, ЧерныйЦвет));
...
&НаСервере
Процедура ОбработатьПорциюДанных(СоединениеССервисом, СтрокаGTINS, Ключ)
	
	Запрос = "/api/v3/true-api/nk/feed-product?"+Ключ + "&gtins=" + СтрокаGTINS;
	
	АдресЗапроса = Запрос;	
	HTTPЗапрос = Новый HTTPЗапрос(АдресЗапроса);
	
	ОтветСервиса = гф_ИнтеграцияСервер.ОтправитьЗапросЧерезСоединение(HTTPЗапрос, СоединениеССервисом, Ложь);
	
	Если ОтветСервиса.КодСостояния = 200 Тогда
		СтруктураДанных = гф_ИнтеграцияСервер.ДесериализоватьJSON(ОтветСервиса.ТекстОтвета);
		
		//Сообщить("Ответ: " + ОтветСервиса.ТекстОтвета);
		
		Если ТипЗнч(СтруктураДанных.result) = Тип("Массив") Тогда
			Для Каждого ОдинОтвет Из СтруктураДанных.result Цикл
				ОбработатьОдинОтвет(ОдинОтвет);
			КонецЦикла;
		ИначеЕсли ТипЗнч(СтруктураДанных.result) = Тип("Структура")	Тогда
			ОбработатьОдинОтвет(СтруктураДанных.result);
		КонецЕсли;	
	Иначе      
        Сообщить("По GTIN " + СтрокаGTINS + " был отправлен запрос: "+ Запрос);
		Сообщить("Получено состояние: "+ОтветСервиса.КодСостояния + " Ответ: " + ОтветСервиса.ТекстОтвета);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция СравнитьСвойствоСДаннымиНК(ЗначениеСвойства_Ссылка, ПереведеннаяСтрока, СтрокаИзНК)

	Рез = 0;
	
	Если НЕ ЗначениеЗаполнено(ЗначениеСвойства_Ссылка) Тогда
		НепереведеннаяСтрока_ДляСравнения = "";
	ИначеЕсли ТипЗнч(ЗначениеСвойства_Ссылка) = Тип("Строка") Тогда
		НепереведеннаяСтрока_ДляСравнения	= НРег(СокрЛП(ЗначениеСвойства_Ссылка));
	ИначеЕсли ТипЗнч(ЗначениеСвойства_Ссылка) = Тип("СправочникСсылка.ЗначенияСвойствОбъектов") Тогда
		НепереведеннаяСтрока_ДляСравнения	= НРег(СокрЛП(ЗначениеСвойства_Ссылка.Наименование));
	КонецЕсли;
	ПереведеннаяСтрока_ДляСравнения 	= НРег(СокрЛП(ПереведеннаяСтрока));
	СтрокаИзНК_ДляСравнения 			= НРег(СокрЛП(СтрокаИзНК));
	
	Если НепереведеннаяСтрока_ДляСравнения <> СтрокаИзНК_ДляСравнения
		И ПереведеннаяСтрока_ДляСравнения <> СтрокаИзНК_ДляСравнения Тогда
		Рез = 2;
	ИначеЕсли НепереведеннаяСтрока_ДляСравнения = СтрокаИзНК_ДляСравнения
		И ПереведеннаяСтрока_ДляСравнения <> СтрокаИзНК_ДляСравнения Тогда
		Рез = 1;
	КонецЕсли;
...
КонецФункции // СравнитьСвойствоСДаннымиНК()

&НаСервере
Функция ПолучитьСтроковоеПредставлениеСтатусаGTIN_НК(СсылкаНаСтатусGTIN)
	
	// ++ Галфинл_ДомнышеваКР_11_10_2023
	// в табличном документе по которому идет сравнение используются строковые данные
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee673a329ebe35
	//Рез = "draft";
	//Если СсылкаНаСтатусGTIN = Перечисления.Гф_СтатусыGTIN_В_НК.Опубликован Тогда
	//	Рез = "published";
	//ИначеЕсли СсылкаНаСтатусGTIN = Перечисления.гф_СтатусыGTIN_В_НК.НеПубликуется Тогда
	//	Рез = "archived";
	//КонецЕсли;
	
	МассивСтрокЧерновик = Новый Массив;//  Галфинл_ДомнышеваКР_19_12_2023
	МассивСтрокЧерновик.Добавить("Черновик");
	МассивСтрокЧерновик.Добавить("Требует доописания");
	МассивСтрокЧерновик.Добавить("Готов к модерации");
	МассивСтрокЧерновик.Добавить("На модерации");
	МассивСтрокЧерновик.Добавить("К публикации");
	
	Если СсылкаНаСтатусGTIN = "Опубликован" Тогда
		Рез = "published";
	ИначеЕсли СсылкаНаСтатусGTIN = "Не публикуется" Тогда
		Рез = "archived";
	ИначеЕсли МассивСтрокЧерновик.Найти(СсылкаНаСтатусGTIN) <> Неопределено Тогда // Галфинд_Домнышева_16_01_2024
		Рез = "draft";
	Иначе
		Рез = "пусто";
	КонецЕсли;
...
КонецФункции // ПолучитьСтроковоеПредставлениеСтатусаGTIN_НК()

&НаСервере
Процедура УстановитьПризнакЕстьОтличия(СтрокаТЧ)
	
	ЕстьОтличия = Ложь;
	Если СокрЛП(СтрокаТЧ.Наименование_1С) <> СокрЛП(СтрокаТЧ.Наименование_НК) Тогда
		ЕстьОтличия = Истина;
	КонецЕсли;
	
	СтроковоеПредставлениеСтатусаВ1С = ПолучитьСтроковоеПредставлениеСтатусаGTIN_НК(СтрокаТЧ.СтатусGTIN_НК_1С);
	
	Если СтроковоеПредставлениеСтатусаВ1С <> СокрЛП(СтрокаТЧ.СтатусGTIN_НК_НК) Тогда
		ЕстьОтличия = Истина;
	КонецЕсли;
	
	Если СокрЛП(СтрокаТЧ.ТНВЭД_1С) <> СокрЛП(СтрокаТЧ.ТНВЭД_НК) Тогда
		ЕстьОтличия = Истина;
	КонецЕсли;
	
	Если НРег(СокрЛП(СтрокаТЧ.МВ_1С_Перевод)) <> НРег(СокрЛП(СтрокаТЧ.МВ_НК)) Тогда
		ЕстьОтличия = Истина;
	КонецЕсли;
	
	Если НРег(СокрЛП(СтрокаТЧ.МН_1С_Перевод)) <> НРег(СокрЛП(СтрокаТЧ.МН_НК)) Тогда
		ЕстьОтличия = Истина;
	КонецЕсли;
	
	Если НРег(СокрЛП(СтрокаТЧ.МП_1С_Перевод)) <> НРег(СокрЛП(СтрокаТЧ.МП_НК)) Тогда
		ЕстьОтличия = Истина;
	КонецЕсли;
	
	
	Если НЕ ЕстьОтличия Тогда
		СтрокаТЧ.ЕстьОтличия = Ложь;
	КонецЕсли;	
КонецПроцедуры

&НаСервере
Процедура ОбработатьОдинОтвет(СтруктураСДанными) 
	
	ДанныеОтСервера = ПолучитьДанныеИзОтветаСервера(СтруктураСДанными);
	
	//проставим данные в ТЧ
	Структурапоиска = Новый Структура("GTIN", ДанныеОтСервера.GTIN);
	СтрокиПоиска = Отчет.СравнениеТоваров.НайтиСтроки(Структурапоиска);
	
	Для каждого СтрокаТЧ Из СтрокиПоиска Цикл
		
		ЗаполнитьЗначенияСвойств(СтрокаТЧ, ДанныеОтСервера,,"GTIN");
		
		УстановитьПризнакЕстьОтличия(СтрокаТЧ);
...
&НаСервере
Функция ПолучитьДанныеИзОтветаСервера(СтруктураСДанными)

	Рез = Новый Структура;
	
	Рез.Вставить("GTIN", 				СтруктураСДанными.identified_by[0].value);
	
	Рез.Вставить("СтатусGTIN_НК_НК", 	СтруктураСДанными.good_status);
	
	Для Каждого СвойствоТовара Из СтруктураСДанными.good_attrs Цикл
		
		Если ВРег(СвойствоТовара.attr_name) = "КОД ТНВЭД" Тогда
			Рез.Вставить("ТНВЭД_НК", 	СвойствоТовара.attr_value);	
		ИначеЕсли НРег(СвойствоТовара.attr_name) = "полное наименование товара" Тогда
			Рез.Вставить("Наименование_НК", 	СвойствоТовара.attr_value);	
		ИначеЕсли НРег(СвойствоТовара.attr_name) = "материал верха" Тогда
			Рез.Вставить("МВ_НК", 	СвойствоТовара.attr_value);				
		ИначеЕсли НРег(СвойствоТовара.attr_name) = "материал подкладки" Тогда
			Рез.Вставить("МП_НК", 	СвойствоТовара.attr_value);				
		ИначеЕсли НРег(СвойствоТовара.attr_name) = "материал низа / подошвы" Тогда
			Рез.Вставить("МН_НК", 	СвойствоТовара.attr_value);				
		КонецЕсли;	
		
	КонецЦикла;
	
	ПозицияЗапятойВНаименовании = Найти(Рез.Наименование_НК, ",");
	Если ПозицияЗапятойВНаименовании > 0 Тогда
		Рез.Вставить("Наименование_НК", Лев(Рез.Наименование_НК, ПозицияЗапятойВНаименовании - 1));
	КонецЕсли;

	Возврат Рез;
	
КонецФункции // ПолучитьДанныеИзОтветаСервера()


&НаКлиенте
Процедура ВариантВыводаДанныхПриИзменении(Элемент)
	ВывестиОтчетСравнение(ОтчетТабличныйДокумент);
КонецПроцедуры

// #wortmann { 
// Команда загрузки Артикулов и Штрихкодов из таблицы
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8ea8b083fed1320811edf965bf211028
// Галфинд_Домнышева 2023/05/26
&НаКлиенте
...
// #wortmann { 
// Заполнение СписокАртикулШтрихкод после закрытия формы "ФормаЗагрузки"
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8ea8b083fed1320811edf965bf211028
// Галфинд_Домнышева 2023/05/26
&НаКлиенте
Процедура ВыборВСписокЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено И
		...
		Результат.Действие = "ЗаполнениеСпискаОтбора" Тогда
			...
			СписокАртикулШтрихкод.Очистить();
			Для каждого Значение Из Результат.МассивДанных Цикл
				
				СписокАртикулШтрихкод.Добавить(Значение);
			КонецЦикла;
		    Элементы.СписокАртикулШтрихкод.Доступность = Истина;
	КонецЕсли;
	ДобавлениеПараметровДанныхВНастройки();
	
КонецПроцедуры// } #wortmann

// #wortmann { 
// Заполнение КлючаПодключения из данных НастройкиИнтеграции
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee5c410a8a706f
// Галфинд_Домнышева 2023/09/27
&НаСервере
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	гф_НастройкиИнтеграции.Ключ КАК Ключ
		|ИЗ
		|	РегистрСведений.гф_НастройкиИнтеграции КАК гф_НастройкиИнтеграции
		|ГДЕ
		|	гф_НастройкиИнтеграции.Организация = &Организация";
...
// #wortmann { 
// Действие команды ДобавитьCохранятьCтатусыНК
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee9508aea28555
// Галфинд_Домнышева 2023/12/19
&НаСервере
Процедура ДобавитьCохранятьCтатусыНКНаСервере()
	
	ОписаниеСтатусы = Новый ОписаниеТипов("ПеречислениеСсылка.гф_СтатусыGTIN_В_НК");
	ОписаниеБулево = Новый ОписаниеТипов("Булево");
	ОписаниеХарактеристика = Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры");
	
    Отбор = Новый Структура;
	Отбор.Вставить("ЕстьОтличия", Истина);

    ТаблицаКОтбору = Отчет.СравнениеТоваров.Выгрузить(Отбор);
	Если ТаблицаКОтбору.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Нет строк подходящих для обработки.");
		Возврат;
	КонецЕсли;	
	// так отличия могут быть разные, нам нужно сверить отличия в статусах
	МассивСтатусЧерновик = Новый Массив;
	МассивСтатусЧерновик.Добавить("Черновик");
	МассивСтатусЧерновик.Добавить("Требует доописания");
	МассивСтатусЧерновик.Добавить("Готов к модерации");
	МассивСтатусЧерновик.Добавить("На модерации");
	МассивСтатусЧерновик.Добавить("К публикации");

	СоответствиеЗначений = Новый Соответствие;
	СоответствиеЗначений.Вставить("published", "Опубликован");
	СоответствиеЗначений.Вставить("archived", "Не публикуется");
	СоответствиеЗначений.Вставить("draft", "Черновик"); 
	СоответствиеЗначений.Вставить("", "GTIN получен");
	СоответствиеЗначений.Вставить("пусто", "GTIN получен"); 
	
	СоответствиеПеречислений = Новый Соответствие;
	СоответствиеПеречислений.Вставить("published", Перечисления.гф_СтатусыGTIN_В_НК.Опубликован);
	СоответствиеПеречислений.Вставить("archived", Перечисления.гф_СтатусыGTIN_В_НК.НеПубликуется);
	СоответствиеПеречислений.Вставить("draft", Перечисления.гф_СтатусыGTIN_В_НК.Черновик);
	СоответствиеПеречислений.Вставить("", Перечисления.гф_СтатусыGTIN_В_НК.GTINПолучен);
	СоответствиеПеречислений.Вставить("пусто", Перечисления.гф_СтатусыGTIN_В_НК.GTINПолучен);

	ТаблицаКОтбору.Колонки.Добавить("СтатусGTIN_НК_1С_Перевод", ОписаниеСтатусы);
	ТаблицаКОтбору.Колонки.Добавить("ИдётВОбработку", ОписаниеБулево);
	ТаблицаКОтбору.Колонки.Добавить("ХарактеристикаН", ОписаниеХарактеристика);

	Для каждого Строка ИЗ ТаблицаКОтбору Цикл
		
		ЗначениеСтатусаНКПеревод = СоответствиеЗначений.Получить(Строка.СтатусGTIN_НК_НК);
		Если ЗначениеСтатусаНКПеревод = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если Строка.СтатусGTIN_НК_НК = "draft" 
				И МассивСтатусЧерновик.Найти(Строка.СтатусGTIN_НК_1С) <> Неопределено Тогда 
			Продолжить;
		КонецЕсли;		
				
		Если ЗначениеСтатусаНКПеревод <> Строка.СтатусGTIN_НК_1С Тогда			
			Строка.СтатусGTIN_НК_1С_Перевод = СоответствиеПеречислений.Получить(Строка.СтатусGTIN_НК_НК);
			Строка.ИдётВОбработку = Истина;
		КонецЕсли;
		
	КонецЦикла;
	
	ОтборСтрок = Новый Структура;
	ОтборСтрок.Вставить("ИдётВОбработку", Истина);
	ТаблицаДляОбработки = ТаблицаКОтбору.Скопировать(ОтборСтрок);
	Если ТаблицаДляОбработки.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Нет строк подходящих для изменения. Статусы соответствуют.");
		Возврат;
	КонецЕсли;
	ПодставитьНоменклатуру(ТаблицаДляОбработки);
	АктуализироватьСтатусы(ТаблицаДляОбработки);
	
КонецПроцедуры// } #wortmann

// #wortmann { 
// Команда добавлена сразу на форму
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee9508aea28555
// Галфинд_Домнышева 2023/12/19
&НаКлиенте
Процедура ДобавитьCохранятьCтатусыНК(Команда)
	// ++ Галфинд_ДомнышеваКР_19_01_2024
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeb5fdeca832e8
	//ДобавитьCохранятьCтатусыНКНаСервере();
	Если НЕ ЕстьДанныеПоНК() Тогда
		ОповещениеОПродолжении = Новый ОписаниеОповещения("ВопросПодтвердитьЗаписьГТИН",
		ЭтотОбъект);
		ПоказатьВопрос(ОповещениеОПродолжении, 
		"У выведенных элементов статус будет изменен на ""GTIN получен"". Продолжать?", РежимДиалогаВопрос.ДаНетОтмена, , 
		КодВозвратаДиалога.Нет, "Подтверждение действий пользователя");
	Иначе 
			ДобавитьCохранятьCтатусыНКНаСервере();	
	КонецЕсли;
    // ++ Галфинд_ДомнышеваКР_19_01_2024
...
// #wortmann { 
// Проверка заполненности ТЧ отчета по НК
// Галфинд_Домнышева 2024/01/19
Функция ЕстьДанныеПоНК() 
	
	ТаблицаНК = Отчет.СравнениеТоваров.Выгрузить(, "СтатусGTIN_НК_НК, ТНВЭД_НК, МВ_НК"); 
	ТаблицаНК.Свернуть("СтатусGTIN_НК_НК, ТНВЭД_НК, МВ_НК");
	Если ТаблицаНК.Количество() > 1 Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
// #wortmann { 
// Продолжение загрузки при выборе пользователя
// Галфинд_Домнышева 2024/01/19
&НаКлиенте
Процедура ВопросПодтвердитьЗаписьГТИН(Результат, ДополнительныеПараметры) Экспорт 
...
// #wortmann { 
// По отобранным данным происходит актуализация записей в РС гф_СтатусыGTIN и в ШтрихкодыНоменклатуры
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee9508aea28555
// Галфинд_Домнышева 2023/12/19
&НаСервере
Процедура АктуализироватьСтатусы(ТаблицаДляОбработки)
	...
	Для каждого Стр Из ТаблицаДляОбработки Цикл
		...
		Отбор.Вставить("Характеристика", стр.ХарактеристикаН);	
		
		ПоследниеДанные = РегистрыСведений.гф_СтатусыGTIN.ПолучитьПоследнее(ДатаСтатуса, Отбор);
		
		ЗаписьСтатус = РегистрыСведений.гф_СтатусыGTIN.СоздатьМенеджерЗаписи();
		ЗаписьСтатус.GTIN = Стр.GTIN;
		ЗаписьСтатус.Период = ДатаСтатуса;
		ЗаписьСтатус.Номенклатура = Стр.Номенклатура;
		ЗаписьСтатус.Характеристика = Стр.ХарактеристикаН;
		ЗаписьСтатус.Организация = Отчет.Организация;
		ЗаписьСтатус.Комментарий = "На основании отчетных данных из НК";
		ЗаписьСтатус.Статус = Стр.СтатусGTIN_НК_1С_Перевод;
		ЗаписьСтатус.Записать(); 
		
		ЗаписьШтрих = РегистрыСведений.ШтрихкодыНоменклатуры.СоздатьМенеджерЗаписи();
		ЗаписьШтрих.Штрихкод = Стр.GTIN;
		ЗаписьШтрих.Прочитать();
		
		ЗаписьШтрих.гф_СостояниеВыгрузкиНоменклатуры = Стр.СтатусGTIN_НК_1С_Перевод;
		ЗаписьШтрих.Записать(); 
...
// #wortmann { 
// По отобранным данным заполняются ссылочные значения Номенкалтуры и Характеристики
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee9508aea28555
// Галфинд_Домнышева 2023/12/19
&НаСервере
Процедура ПодставитьНоменклатуру(ТаблицаДляОбработки)
	...
	|	Т.Сезон КАК Сезон,
	|	Т.СтатусGTIN_НК_1С КАК СтатусGTIN_НК_1С,
	|	Т.СтатусGTIN_НК_НК КАК СтатусGTIN_НК_НК,
	|	Т.СтатусGTIN_НК_1С_Перевод КАК СтатусGTIN_НК_1С_Перевод,
	|	Т.ИдётВОбработку КАК ИдётВОбработку
	|ПОМЕСТИТЬ Исходная
	|ИЗ
	|	&ТаблицаДляОбработки КАК Т
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Штрихкоды.Характеристика КАК ХарактеристикаН,
	|	Исходная.Артикул КАК Артикул,
	|	Исходная.Сезон КАК Сезон,
	|	Исходная.СтатусGTIN_НК_1С КАК СтатусGTIN_НК_1С,
	|	Исходная.СтатусGTIN_НК_НК КАК СтатусGTIN_НК_НК,
	|	Исходная.СтатусGTIN_НК_1С_Перевод КАК СтатусGTIN_НК_1С_Перевод,
	|	Исходная.ИдётВОбработку КАК ИдётВОбработку
	|ИЗ
	|	Исходная КАК Исходная
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Штрихкоды КАК Штрихкоды
	|		ПО Исходная.GTIN = Штрихкоды.Штрихкод";
	
	Запрос.УстановитьПараметр("ТаблицаДляОбработки", ТаблицаДляОбработки);

	РезультатЗапроса = Запрос.Выполнить();
	
	НоваяТаб = РезультатЗапроса.Выгрузить();
	ТаблицаДляОбработки.Очистить();
	
	Для Каждого Стр Из НоваяТаб Цикл 
		
		ЗаполнитьЗначенияСвойств(ТаблицаДляОбработки.Добавить(), Стр);
