...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ТипЗнч(Данные.Значение)=Тип("Число")  тогда	
		Коллекция = ЭтаФорма.Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы;
		Настройки = ЭтаФорма.Отчет.КомпоновщикНастроек.Настройки;  	
		СтандартнаяОбработка=Ложь;  
		
		//ИДНастройкиПериод = Настройки.ПараметрыДанных.Элементы.Найти("Период").ИдентификаторПользовательскойНастройки;
		//КонецПериода = Коллекция.Найти(ИДНастройкиПериод).Значение.ДатаОкончания;
		ИДНастройкиКонецПериода = Настройки.ПараметрыДанных.Элементы.Найти("ДатаОкончания").ИдентификаторПользовательскойНастройки ;
		КонецПериода = Коллекция.Найти(ИДНастройкиКонецПериода).Значение ;
     	//НачалоПериода= Коллекция.Найти(ИДНастройкиПериод).Значение.ДатаНачала;
		ИДНастройкиСезон = Настройки.ПараметрыДанных.Элементы.Найти("Сезон").ИдентификаторПользовательскойНастройки;  
		Сезон=Коллекция.Найти(ИДНастройкиСезон).Значение;
		//Отбор = Новый Структура("Период", Новый СтандартныйПериод(НачалоПериода,КонецПериода));
		Отбор = Новый Структура("ДатаОкончания", КонецПериода );
	//	Если ЭлементОтбораорганизация.Использование Тогда
			Отбор.Вставить("Сезон", Сезон);
	//	КонецЕсли; 
		ПолеПартнер = Настройки.Отбор.ДоступныеПоляОтбора.Элементы.Найти("Партнер").Поле;
		ИДЭлмОтбораПартнер  = "";
		Для Каждого Элемент Из Настройки.Отбор.Элементы Цикл
			Если Элемент.ЛевоеЗначение = ПолеПартнер  Тогда ИДЭлмОтбораПартнер  = Элемент.ИдентификаторПользовательскойНастройки; Прервать; КонецЕсли;
		КонецЦикла;
		ЭлементОтбораПартнер  = Коллекция.Найти(ИДЭлмОтбораПартнер);
		Если ЭлементОтбораПартнер .Использование Тогда
			Отбор.Вставить("Партнер", ЭлементОтбораПартнер .ПравоеЗначение); 
		КонецЕсли;

		ПараметрыФормы = Новый Структура("СформироватьПриОткрытии, Отбор, КлючВарианта", Истина, Отбор, "РасшифровкаПоДокументам");
		
		//ОткрытьФорму("ВнешнийОтчет.гф_АнализСчетовнаАвансы.Форма", ПараметрыФормы, ЭтаФорма);
		ОткрытьФорму("Отчет.гф_АнализСчетовнаАвансы.Форма", ПараметрыФормы, ЭтаФорма);
...
Процедура ПерейтиКНастройкам(РасширенныйРежим = Неопределено)
	...
	//(н)Галфинд/Сефербеков 04.10.2023 ( 
	//ВыполнятьЗамеры = НастройкиОтчета.ВыполнятьЗамеры И ЗначениеЗаполнено(НастройкиОтчета.КлючЗамеров);
	ВыполнятьЗамеры = НастройкиОтчета.ВыполнятьЗамеры И ЗначениеЗаполнено(НастройкиОтчета.КлючЗамеров) И НастройкиОтчета.Свойство("ПрефиксЗамеров")  ;
	//(к)Галфинд/Сефербеков 04.10.2023 ) 
