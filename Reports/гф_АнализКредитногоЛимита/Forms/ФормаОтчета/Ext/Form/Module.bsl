...
Процедура ПриОткрытии(Отказ)
	...
	отчет.ДатаНачала = ДобавитьМесяц(отчет.ДатаОкончания, -12);
...
Процедура СохранитьВариант(Команда)
	...
	|en = 'Report option is saved'"),, Заголовок);
...
#КонецОбласти
#Область ДопФункцийОтчета  

&НаСервере
Процедура СформироватьНаСервере()
	Перем ТипыЛимитов;
	
	Панель1 = Элементы.Панель.ПодчиненныеЭлементы.Страницы;
	страниц = Панель1.ПодчиненныеЭлементы.Количество() - 1;    
	Для а=1 По страниц Цикл
		ЭтаФорма.Элементы.Удалить(ЭтаФорма.Элементы["Страница"+а]);
		#Область УдалениеРеквизитовФормы
		
		УдаляемыеРеквизиты = Новый Массив;
		УдаляемыеРеквизиты.Добавить("Поле"+а);
		ИзменитьРеквизиты( , УдаляемыеРеквизиты); 		
		#КонецОбласти
	КонецЦикла;
	
	Если отчет.ПоКонтрагентам Тогда
		Панель1.ОтображениеСтраниц = ОтображениеСтраницФормы.ЗакладкиСверху; 
	Иначе
		Панель1.ОтображениеСтраниц=ОтображениеСтраницФормы.Нет;
	КонецЕсли;
	
	Если отчет.ПоКонтрагентам Тогда 
		
		Лимит=Отчет.КомпоновщикНастроек.ПолучитьНастройки().ПараметрыДанных.Элементы.Найти("ТипыЛимитов");
		Если Лимит.Использование Тогда
			ТипыЛимитов = Отчет.КомпоновщикНастроек.ПолучитьНастройки().ПараметрыДанных.Элементы.Найти("ТипыЛимитов").Значение;
		Иначе
			ТипыЛимитов=Перечисления.гф_ТипыЛимитов.ПустаяСсылка();
		КонецЕсли;
		
		Контрагенты =отчет. КомпоновщикНастроек.ПолучитьНастройки().ПараметрыДанных.Элементы.Найти("Контрагент").Значение;   
		Сезон=отчет.КомпоновщикНастроек.ПолучитьНастройки().ПараметрыДанных.Элементы.Найти("Сезон").Значение;
		Данные = ПолучитьТаблицуПлатёжнойИстории(отчет.ДатаОкончания, Контрагенты);
		...
		поле = НастройкиСКД.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
		поле.Поле = новый ПолеКомпоновкиДанных("ПроцентKV");
		поле = НастройкиСКД.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
		поле.Поле = новый ПолеКомпоновкиДанных("ЗапрашиваемыйЛимитПроцент");
		поле = НастройкиСКД.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
		поле.Поле = новый ПолеКомпоновкиДанных("ДоговорКонтрагента");
		поле = НастройкиСКД.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
		поле.Поле = новый ПолеКомпоновкиДанных("Организация");
		//	
		...
		Если Типзнч(контрагенты)=тип("СписокЗначений") Тогда
			Для каждого строка Из Контрагенты Цикл
				...
				НовыйЭлемент1 = Элементы.Добавить("Страница"+ НомерСтраницы,Тип("ГруппаФормы"),Элементы.Страницы);
				НовыйЭлемент1.Вид = ВидГруппыФормы.Страница;
				НовыйЭлемент1.Заголовок = Строка(Контрагент);
				
				ДобавляемыеРеквизиты = Новый Массив;  
				НовыйРеквизит = Новый РеквизитФормы("Поле" + НомерСтраницы, Новый ОписаниеТипов("ТабличныйДокумент")); 
				ДобавляемыеРеквизиты.Добавить(НовыйРеквизит); 
				ЭтаФорма.ИзменитьРеквизиты(ДобавляемыеРеквизиты); 
				
				
				ДобавляемыеРеквизиты	= Новый Массив;  
				поле = Элементы.Добавить("Поле" + НомерСтраницы,Тип("ПолеФормы"), НовыйЭлемент1);   
				поле.Вид=ВидПоляФормы.ПолеТабличногоДокумента; 
				поле.ПутьКДанным = "Поле" + НомерСтраницы;
				поле.Видимость = Истина;
				поле.Доступность = Истина;
				поле.Высота = Элементы.ОтчетТабличныйДокумент.Высота;
				поле.Ширина = Элементы.ОтчетТабличныйДокумент.Ширина;
				ТабДок=ЭтаФорма["Поле" + НомерСтраницы] ;   
				поле.УстановитьДействие("ПриИзмененииСодержимогоОбласти", "ОтчетТабличныйДокументПриИзмененииСодержимогоОбласти"); 				
				нашли = таб.НайтиСтроки(новый Структура("НаименованиеПартнера", Строка(Контрагент)));
				Если нашли.Количество() > 0 Тогда
					нашли = нашли[0];
					
					нашли.ЗапрашиваемыйЛимитПроцент = Окр(нашли.ЗапрашиваемыйЛимитПроцент, 2);
					
					МакетПоКонтрагентам = РеквизитФормыВЗначение("Отчет").ПолучитьМакет("МакетПоКонтрагентам");
					макет = МакетПоКонтрагентам.ПолучитьОбласть("Область1");
					макет.Параметры.Заполнить(нашли);
					
					макет.Параметры.Сезон = Сезон;
					макет.Параметры.Организация = ПолучитьТоварныйЗнак(нашли.Организация);
					макет.Параметры.Контрагент= нашли.НаименованиеПартнера;
					макет.Параметры.КодКонтрагента=нашли.КодПартнера;
					
					Если нашли.СрокСотрудничества < 2 Тогда
						макет.Параметры.СрокСотрудничества = "<2";
					ИначеЕсли нашли.СрокСотрудничества < 3 Тогда
						макет.Параметры.СрокСотрудничества = "2-3";
					ИНаче
						макет.Параметры.СрокСотрудничества = "3>=";
					КонецЕсли;
					
					макет.Параметры.Организация0=ПолучитьТоварныйЗнак(нашли.Организация);
                    Если нашли.KV > 30 Тогда  
						макет.Параметры.ОценкаKV = 5;
					ИначеЕсли нашли.KV > 20 Тогда
						макет.Параметры.ОценкаKV = 4;
					ИначеЕсли нашли.KV > 0 Тогда
						макет.Параметры.ОценкаKV = 3;
					Иначе
						макет.Параметры.ОценкаKV = 0;
					КонецЕсли;
					
					Макет.Параметры.KV = нашли.KV;
					Макет.Область("R24C2").Значение=СуммаKV; 
					Макет.Область("R26C2").Значение=ДатаЗапросаKV; 
					Макет.Область("R27C2").Значение=ПричинаОтказаKV; 
					Макет.Область("ОтказыОтЗаказов").Значение=ОтказыОтЗаказов; 
					Если нашли.ЗапрашиваемоеКоличествоДнейОтсрочкиЗаСезон = 0 Тогда
						макет.Параметры.ОценкаПоСрокамОплаты = "введите, пожалуйста, значение показателя Payment terms";
					ИначеЕсли нашли.ЗапрашиваемоеКоличествоДнейОтсрочкиЗаСезон < 30 Тогда
						макет.Параметры.ОценкаПоСрокамОплаты = 2;
					ИначеЕсли нашли.ЗапрашиваемоеКоличествоДнейОтсрочкиЗаСезон <= 60 Тогда
						макет.Параметры.ОценкаПоСрокамОплаты = 1;
					Иначе
						макет.Параметры.ОценкаПоСрокамОплаты = 0;
					КонецЕсли;
					
					Если строка(нашли.Лояльность) = "Low" Тогда
						макет.Параметры.ОценкаЛояльности = 0;
					ИначеЕсли строка(нашли.Лояльность) = "Medium" Тогда
						макет.Параметры.ОценкаЛояльности = 1;
					ИначеЕсли строка(нашли.Лояльность) = "High" Тогда
						макет.Параметры.ОценкаЛояльности = 2;
					Иначе
						макет.Параметры.ОценкаЛояльности = "введите, пожалуйста, значение показателя Loyality";
					КонецЕсли;
					
					Если нашли.СрокСотрудничества < 0 Тогда
						макет.Параметры.ОценкаПоСроку = "<2";
					ИначеЕсли нашли.СрокСотрудничества < 3 Тогда
						макет.Параметры.ОценкаПоСроку = 1;
					Иначе
						макет.Параметры.ОценкаПоСроку = 2;
					КонецЕсли;
					
					ОплатаСредневзвешенное = ?(ЗначениеЗаполнено(нашли.ОплатаСредневзвешенное), нашли.ОплатаСредневзвешенное, 0);
					
					//Если не ЗначениеЗаполнено(нашли.ОплатаСредневзвешенное) или нашли.ОплатаСредневзвешенное < 0 Тогда
					Если ОплатаСредневзвешенное < 0 Тогда
						макет.Параметры.ОценкаПлатежнойДисциплиныСред = 0;
					ИначеЕсли ОплатаСредневзвешенное < 3 Тогда
						макет.Параметры.ОценкаПлатежнойДисциплиныСред = 3;
					ИначеЕсли ОплатаСредневзвешенное < 10 Тогда
						макет.Параметры.ОценкаПлатежнойДисциплиныСред = 4;
					ИНаче
						макет.Параметры.ОценкаПлатежнойДисциплиныСред = 5;
					КонецЕсли;
					
					ОплатаМин = ?(ЗначениеЗаполнено(нашли.ОплатаМин), -нашли.ОплатаМин, 0);
					
					//Если не ЗначениеЗаполнено(нашли.ОплатаМин) или нашли.ОплатаМин < 0 Тогда
					Если не ЗначениеЗаполнено(нашли.ОплатаМин) или ОплатаМин < 0 Тогда
						макет.Параметры.ОценкаПлатежнойДисциплиныМин = 0;
					ИначеЕсли нашли.ОплатаМин < 5 Тогда
						макет.Параметры.ОценкаПлатежнойДисциплиныМин = 4;
					ИНаче
						макет.Параметры.ОценкаПлатежнойДисциплиныМин = 5;
					КонецЕсли;
					
					попытка
						ОценкаКредитоспособности = макет.Параметры.ОценкаKV + макет.Параметры.ОценкаПоСрокамОплаты + макет.Параметры.ОценкаЛояльности + макет.Параметры.ОценкаПоСроку + макет.Параметры.ОценкаПлатежнойДисциплиныСред + макет.Параметры.ОценкаПлатежнойДисциплиныМин;
					исключение
						ОценкаКредитоспособности = "-";
					КонецПопытки;
					макет.Параметры.ОценкаКредитоспособности = ОценкаКредитоспособности;
					
					область = макет.Область("ОценкаKV");
					Если макет.Параметры.ОценкаKV = 0 Тогда
						область.ЦветФона = Новый Цвет(255, 140, 140);
					ИначеЕсли макет.Параметры.ОценкаKV = 3 Тогда
						область.ЦветФона = Новый Цвет(255, 255, 0);
					ИначеЕсли макет.Параметры.ОценкаKV = 4 Тогда
						область.ЦветФона = Новый Цвет(255, 255, 0);
					ИначеЕсли макет.Параметры.ОценкаKV = 5 Тогда
						область.ЦветФона = WebЦвета.НейтральноЗеленый;
					КонецЕсли;
					
					область = макет.Область("ОценкаПоСрокамОплаты");
					Если макет.Параметры.ОценкаПоСрокамОплаты = 0 Тогда
						область.ЦветФона = Новый Цвет(255, 140, 140);
					ИначеЕсли макет.Параметры.ОценкаПоСрокамОплаты = 1 Тогда
						область.ЦветФона = Новый Цвет(255, 255, 0);
					ИначеЕсли макет.Параметры.ОценкаПоСрокамОплаты = 2 Тогда
						область.ЦветФона = WebЦвета.НейтральноЗеленый;
					КонецЕсли;
					
					область = макет.Область("ОценкаЛояльности");
					Если макет.Параметры.ОценкаЛояльности = 0 Тогда
						область.ЦветФона = Новый Цвет(255, 140, 140);
					ИначеЕсли макет.Параметры.ОценкаЛояльности = 1 Тогда
						область.ЦветФона = Новый Цвет(255, 255, 0);
					ИначеЕсли макет.Параметры.ОценкаЛояльности = 2 Тогда
						область.ЦветФона = WebЦвета.НейтральноЗеленый;
					КонецЕсли;
					
					область = макет.Область("ОценкаПоСроку");
					Если макет.Параметры.ОценкаПоСроку = 0 Тогда
						область.ЦветФона = Новый Цвет(255, 140, 140);
					ИначеЕсли макет.Параметры.ОценкаПоСроку = 1 Тогда
						область.ЦветФона = Новый Цвет(255, 255, 0);
					ИначеЕсли макет.Параметры.ОценкаПоСроку = 2 Тогда
						область.ЦветФона = WebЦвета.НейтральноЗеленый;
					КонецЕсли;
					
					область = макет.Область("ОценкаПлатежнойДисциплиныСред");
					Если макет.Параметры.ОценкаПлатежнойДисциплиныСред = 0 Тогда
						область.ЦветФона = Новый Цвет(255, 140, 140);
					ИначеЕсли макет.Параметры.ОценкаПлатежнойДисциплиныСред = 3 Тогда
						область.ЦветФона = Новый Цвет(255, 255, 0);
					ИначеЕсли макет.Параметры.ОценкаПлатежнойДисциплиныСред = 4 Тогда
						область.ЦветФона = Новый Цвет(255, 255, 0);
					ИначеЕсли макет.Параметры.ОценкаПлатежнойДисциплиныСред = 5 Тогда
						область.ЦветФона = WebЦвета.НейтральноЗеленый;
					КонецЕсли;
					
					область = макет.Область("ОценкаПлатежнойДисциплиныМин");
					Если макет.Параметры.ОценкаПлатежнойДисциплиныМин = 0 Тогда
						область.ЦветФона = Новый Цвет(255, 140, 140);
					ИначеЕсли макет.Параметры.ОценкаПлатежнойДисциплиныМин = 4 Тогда
						область.ЦветФона = Новый Цвет(255, 255, 0);
					ИначеЕсли макет.Параметры.ОценкаПлатежнойДисциплиныМин = 5 Тогда
						область.ЦветФона = WebЦвета.НейтральноЗеленый;
					КонецЕсли;
					
					область = макет.Область("ОценкаКредитоспособности");
					Если макет.Параметры.ОценкаКредитоспособности = "-" Тогда
						// !!!!!
					ИначеЕсли макет.Параметры.ОценкаКредитоспособности = 0 Тогда
						область.ЦветФона = Новый Цвет(255, 140, 140);
					ИначеЕсли макет.Параметры.ОценкаКредитоспособности < 40 Тогда
						область.ЦветФона = Новый Цвет(255, 255, 0);
					ИначеЕсли макет.Параметры.ОценкаКредитоспособности = 40 Тогда
						область.ЦветФона = WebЦвета.НейтральноЗеленый;
					КонецЕсли;
					
					макет.Параметры.ОбоснованиеЗапросаЛимита = нашли.ОбоснованиеЗапросаЛимита;
					
					макет.Параметры.Аванс = ПолучитьПроцентАвансаПоКонтрагентуНаСезон(нашли.ДоговорКонтрагента);
					макет.Параметры.ИспользованиеАванса = нашли.ИспользованиеАванса;
					макет.Параметры.ОбщееПокрытиеЗаказа = нашли.ЗапрашиваемыйЛимитПроцент + нашли.ПроцентKV;
					
					//ВывестиОрганизацииВПравильномПорядке(макет, Компания);
					
					ТабДок.Вывести(макет);
					
					Макет = МакетПоКонтрагентам.ПолучитьОбласть("ЗаголовокДействующиеЛимиты");
					ТабДок.Вывести(Макет);
					
					макет = МакетПоКонтрагентам.ПолучитьОбласть("ДействующиеЛимиты");
					
					ИсторияЛимитов = ПолучитьИсториюЛимитов(Контрагент, ТипыЛимитов);
					
					ПоследняяДата = Неопределено;
					
					Пока ИсторияЛимитов.Следующий() Цикл
						макет.Параметры.Строка = Формат(ИсторияЛимитов.Период, "ДФ=dd.MM.yyyy"); //+ " по ... " 
						Макет.Параметры.ВидЛимита = ИсторияЛимитов.ВидЛимита;
						макет.Параметры.Сумма = ИсторияЛимитов.Сумма;
						ТабДок.Вывести(макет);
					КонецЦикла;
					
					макет = МакетПоКонтрагентам.ПолучитьОбласть("Область2");
					макет.Параметры.Заполнить(нашли);
					
					Если ОценкаКредитоспособности = "-" Тогда
						макет.Параметры.РекомендуемыйЛокальныйЛимитПроцент = "-";
						макет.Параметры.РекомендуемыйЛокальныйЛимит = "-";
						
					Иначе
						Если ОценкаКредитоспособности < 10 Тогда
							РекомендуемыйЛокальныйЛимитПроцент = 0;
						ИначеЕсли ОценкаКредитоспособности < 12 Тогда
							РекомендуемыйЛокальныйЛимитПроцент = 10;
						ИначеЕсли ОценкаКредитоспособности < 15 Тогда
							РекомендуемыйЛокальныйЛимитПроцент = 20;
						ИначеЕсли ОценкаКредитоспособности < 17 Тогда
							РекомендуемыйЛокальныйЛимитПроцент = 30;
						Иначе
							РекомендуемыйЛокальныйЛимитПроцент = 40;
						КонецЕсли;
						макет.Параметры.РекомендуемыйЛокальныйЛимитПроцент = РекомендуемыйЛокальныйЛимитПроцент;
						макет.Параметры.РекомендуемыйЛокальныйЛимит = нашли.ОбщаяСуммаЗаказаЗаСезон * РекомендуемыйЛокальныйЛимитПроцент / 100;
					КонецЕсли;
		...
		иначе 
			...
			Если нашли.Количество() > 0 Тогда
				...
				макет.Параметры.Сезон = Сезон;
				макет.Параметры.Организация = нашли.Организация;       
				макет.Параметры.Контрагент= нашли.НаименованиеПартнера;
				макет.Параметры.КодКонтрагента=нашли.КодПартнера;
	...
	КонецЕсли;
	//
	//Панель1.ТекущаяСтраница = Панель1.ПодчиненныеЭлементы.Страница[0];
КонецПроцедуры

&НаКлиенте
Процедура Сформировать1(Команда)   
	ЭтотОбъект.СкомпоноватьРезультат(РежимКомпоновкиРезультата.Фоновый);
	СформироватьНаСервере();
КонецПроцедуры
Функция ПолучитьИсториюЛимитов(Контрагент, ТипыЛимитов)
	
	запрос = новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&ДатаНачала КАК Период,
	|	а.ВидЛимита КАК ВидЛимита,
	|	а.Сумма КАК Сумма,
	|	а.Статус КАК Статус
	|ИЗ
	...
	|				И ВидЛимита.ТипЛимита В (&ТипыЛимитов)) КАК а
	|ГДЕ
	|	а.Статус.Активный
	|	И а.Сумма > 0
	|
	...
	|	а.Статус
	|ИЗ
	|	РегистрСведений.гф_ЛимитыКонтрагентов КАК а
	|ГДЕ
	|	а.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И а.Контрагент = &Контрагент
	|	И а.ВидЛимита.ТипЛимита В(&ТипыЛимитов)
	|	И а.Статус.Активный
	|	И а.Сумма > 0
	|
	|УПОРЯДОЧИТЬ ПО
	...
	запрос.УстановитьПараметр("ТипыЛимитов", ТипыЛимитов);
...
Функция ПолучитьТаблицуПлатёжнойИстории(ДатаОкончания, Контрагенты)
	...
	СхемаКомпоновкиИстории = РеквизитФормыВЗначение("Отчет").ПолучитьМакет("ПлатёжнаяИстория");
	...
	ПараметрыДанных.УстановитьЗначениеПараметра("ДатаНачала", отчет.ДатаНачала);
	ПараметрыДанных.УстановитьЗначениеПараметра("ДатаОкончания", КонецДня(отчет.ДатаОкончания));
	...
	ПараметрСезон = отчет.КомпоновщикНастроек.ПолучитьНастройки().ПараметрыДанных.Элементы.Найти("Сезон");    
	Если ТипЗнч(Контрагенты)=тип("СписокЗначений")Тогда
		Для каждого строка Из Контрагенты Цикл
...
Функция ПолучитьПредыдущиеСезоны(Сезон)
	...
	|	w_Сезоны.Год КАК Год,
	|	w_Сезоны.Номер КАК Номер
	|ИЗ
	|	Справочник.w_Сезоны КАК w_Сезоны
	|ГДЕ
	...
	пустое = Справочники.w_Сезоны.ПустаяСсылка();
...
Функция ПолучитьСезонПоНомеру(Номер)
	...
	|	Справочник.w_Сезоны КАК w_Сезоны
	|ГДЕ
	|	w_Сезоны.Номер = &Номер");
	запрос.УстановитьПараметр("Номер", Номер);
	выборка = запрос.Выполнить().Выбрать();
	Если выборка.Следующий() Тогда
		Возврат выборка.Ссылка;
	Иначе
		возврат Справочники.w_Сезоны.ПустаяСсылка();
	КонецЕсли;
	
КонецФункции // ПолучитьСезонПоНомеру()

Функция ПолучитьПроцентАвансаПоКонтрагентуНаСезон(ДоговорКонтрагента) 	
	запрос = новый Запрос(	"ВЫБРАТЬ
	|	ДоговорыКонтрагентовДополнительныеРеквизиты.Значение КАК СтраховойДепозит,
	...
	|ГДЕ
	|	ДоговорыКонтрагентовДополнительныеРеквизиты.Свойство.Наименование = ""Страховой депозит""
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(гф_ПроцентыПредоплаты.Процент,0) КАК Процент
	|ИЗ
	|	ВТ КАК ВТ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.гф_ПроцентыПредоплаты КАК гф_ПроцентыПредоплаты
	|		ПО ВТ.СтраховойДепозит = гф_ПроцентыПредоплаты.Номеклатура
	|ГДЕ
	|	ВТ.Ссылка = &ДоговорКонтрагента");
	
	запрос.УстановитьПараметр("ДоговорКонтрагента", ДоговорКонтрагента);
	выборка = запрос.Выполнить().Выбрать();
	Если выборка.Следующий() Тогда
		возврат строка(выборка.Процент) + "%";
	Иначе
		возврат "no advance / без аванса";
	КонецЕсли;
	
КонецФункции // ПолучитьПроцентАвансаПоКонтрагентуНаСезон()

Функция ПолучитьТоварныйЗнак(Организация)
	...
	Запрос.Текст = 
		...
		|	И ОрганизацииДополнительныеРеквизиты.Свойство.Наименование = ""Товарный знак""";
...
КонецФункции // ПолучитьТоварныйЗнак()

Функция ПолучитьЛокальныйЛимит(Контрагент, НомерСезона)
	
	запрос = новый Запрос(
	"ВЫБРАТЬ
	|	Сезоны.Ссылка,
	|	Сезоны.Номер,
	|	Договоры.Ссылка КАК ДоговорКонтрагентов,
	|	Значения.Значение
	|ИЗ
	|	Справочник.ДоговорыКонтрагентов КАК Договоры
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.w_Сезоны КАК Сезоны
	|		ПО Договоры.w_Сезон = Сезоны.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияСвойствОбъектов КАК Значения
	|		ПО Договоры.Ссылка = Значения.Объект
	|			И (Значения.Свойство = &Свойство)
	|ГДЕ
	|	Сезоны.Номер = &НомерСезона
	|	И Договоры.Владелец = &Контрагент");
	запрос.УстановитьПараметр("Контрагент", Контрагент);
	запрос.УстановитьПараметр("НомерСезона", НомерСезона);
	запрос.УстановитьПараметр("Свойство", Справочники.w_Настройки.СвойствоЛокальныйЛимит.Значение);
	выборка = запрос.Выполнить().Выбрать();
	Если выборка.Следующий() Тогда
...
КонецФункции // ПолучитьЛокальныйЛимит()

// Голубцов, правки, 04.10.2019
//
// Параметры:
//  Макет	 - 	ТабличныйДокумент. Табличный документ с данными. 
//  Компания - 	Строка. Организация из заголовка конфигурации.
Процедура ВывестиОрганизацииВПравильномПорядке(Макет, Компания)
	
	// Поищем компанию и определим ей порядок.
	Если Компания = "Tamaris" Тогда
		ПорядокКомпаний = "Tamaris,Caprice no business,Wendel no business,Jana no business,S. Oliver no business"; 	
	ИначеЕсли Компания = "Caprice" Тогда
		ПорядокКомпаний = "Caprice,Tamaris no business,Wendel no business,Jana no business,S. Oliver no business"; 	
	ИначеЕсли Компания = "Wendel" Тогда
		ПорядокКомпаний = "Wendel,Tamaris no business,Caprice no business,Jana no business,S. Oliver no business"; 	
	ИначеЕсли Компания = "Jana" Тогда
		ПорядокКомпаний = "Jana,Tamaris no business,Caprice no business,Wendel no business,S. Oliver no business"; 	
	ИначеЕсли Компания = "S. Oliver" Тогда
		ПорядокКомпаний = "S. Oliver,Tamaris no business,Caprice no business,Wendel no business,Jana no business";
	КонецЕсли;
	
	МассивКомпаний = СтрРазделить(ПорядокКомпаний, ",");
	
	//nobusiness = " no business";
	
	Для сч = 0 По МассивКомпаний.Количество() - 1 Цикл
		Макет.Параметры["Организация" + сч] = МассивКомпаний[сч]; // + nobusiness
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Процедура СохранитьВExelНаСервере()
КонецПроцедуры
...
Процедура СохранитьВExel(Команда)
	...
	Страниц = Элементы.Страницы.ПодчиненныеЭлементы.Количество()-1;
	Для а=1 По Страниц Цикл	
		ИмяФайла = файл.Каталог + "\" + Элементы.Страницы.ПодчиненныеЭлементы.Найти("Страница"+а).Заголовок + ".xls";
		ЭтаФорма["Поле"+а].Записать(ИмяФайла, ТипФайлаТабличногоДокумента.XLS);
	КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура ПоКонтрагентам(Команда)
	Отчет.ПоКонтрагентам= НЕ Отчет.ПоКонтрагентам;
	Элементы.ГруппаГлавное.ПодчиненныеЭлементы.ПоКонтрагентам.Пометка=Отчет.ПоКонтрагентам;
	Элементы.ГруппаГлавное.ПодчиненныеЭлементы.СохранитьВExel.Доступность=Отчет.ПоКонтрагентам; 
...
&НаКлиенте
Процедура ОтчетТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область, ДополнительныеПараметры)
	Если Область.Имя="R24C2" Тогда
		СуммаKV= Область.Значение;
	ИначеЕсли Область.Имя="R26C2" Тогда
		ДатаЗапросаKV=Область.Значение;
    ИначеЕсли Область.Имя="R27C2" Тогда
		ПричинаОтказаKV=Область.Значение;
	ИначеЕсли Область.Имя="ОтказыОтЗаказов" Тогда 
		ОтказыОтЗаказов=Область.Значение;
	КонецЕсли;
КонецПроцедуры
