...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	гф_Бюджеты.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.гф_Бюджеты КАК гф_Бюджеты
		|ГДЕ
		|	гф_Бюджеты.ПометкаУдаления = ЛОЖЬ";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Строка=Отчет.БюджетСписок.Добавить();   
		СтрокаБюджет=БюджетСписок.Добавить() ;
		Строка.Значение=ВыборкаДетальныеЗаписи.Ссылка; 
		СтрокаБюджет.Значение=ВыборкаДетальныеЗаписи.Ссылка;
	КонецЦикла;  
КонецПроцедуры
...
&НаКлиенте
Процедура БюджетСписок1НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ВыборБюджета", ЭтаФорма);
	
	Отмеченные = Новый СписокЗначений;
	Для каждого ЭлементБюджет Из Отчет.БюджетСписок Цикл
		Если ЭлементБюджет.Пометка Тогда
			Отмеченные.Добавить(ЭлементБюджет.Значение,, Истина);
		КонецЕсли;
	КонецЦикла;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ОграничиватьВыборУказаннымиЗначениями", Истина);
	ПараметрыФормы.Вставить("Представление", НСтр("ru = 'Отбор бюджета';
													|en = 'Company filter'"));
	ПараметрыФормы.Вставить("ЗначенияДляВыбора", Отчет.БюджетСписок);
	ПараметрыФормы.Вставить("ОписаниеТипов", БюджетСписок.ТипЗначения);
...
&НаКлиенте
Процедура БюджетСписок1ОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	ЭлементВыбранногоБюджета= Отчет.БюджетСписок.НайтиПоЗначению(ВыбранноеЗначение);
	Если ЭлементВыбранногоБюджета <> Неопределено Тогда
		Отчет.БюджетСписок.ЗаполнитьПометки(Ложь);
		ЭлементВыбранногоБюджета.Пометка = Истина;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура БюджетСписок1Очистка(Элемент, СтандартнаяОбработка)
	Отчет.БюджетСписок.ЗаполнитьПометки(Ложь);
КонецПроцедуры
&НаКлиенте
Процедура ВыборБюджета(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено И ТипЗнч(Результат) = Тип("СписокЗначений") Тогда
		Отчет.БюджетСписок = Результат;
		ОтборБюджетовПредставление= ОтборПредставление(Отчет.БюджетСписок);
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	ЗаполнитьБюджетСписокНаСервере();
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьБюджетСписокНаСервере()
	МассивБюджеты = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок
		(ОтборБюджетовПредставление, ", ");
	Для Каждого Эл Из МассивБюджеты Цикл
		БюджетСсылка = Справочники.гф_Бюджеты.НайтиПоНаименованию(Эл);
		Если ЗначениеЗаполнено(БюджетСсылка) Тогда
			ЭлСписка = БюджетСписок.НайтиПоЗначению(БюджетСсылка);
			Если ЭлСписка <> Неопределено Тогда
				ЭлСписка.Пометка = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Отчет.БюджетСписок = БюджетСписок;
КонецПроцедуры
// -- Галфинд СадомцевСА 09.11.2023
