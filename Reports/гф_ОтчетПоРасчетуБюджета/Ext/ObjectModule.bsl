...
Процедура ПолучитьТаблицуРассчёты(ДокументРезультат)
	...
	Для Каждого ЭлементБазовый Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		...
		КонецЕсли;
		// ++ Галфинд Волков 18.09.2023
		Если ЭлементБазовый.Представление = "БазоваяВалюта" Тогда
			ЭлементБазовый.ПравоеЗначение = Константы.БазоваяВалютаПоУмолчанию.Получить();
			ЭлементБазовый.Использование=Истина;
			Продолжить;
		КонецЕсли;
		Если ЭлементБазовый.Представление = "ВалютаУправленческогоУчета" Тогда
			ЭлементБазовый.ПравоеЗначение = Константы.БазоваяВалютаПоУмолчанию.Получить();
			ЭлементБазовый.Использование=Истина;
			Продолжить;
		КонецЕсли;
		Если ЭлементБазовый.Представление = "СтрокаДолгНеПросрочен" Тогда
			ЭлементБазовый.ПравоеЗначение = "Не просрочено";
			ЭлементБазовый.Использование=Истина;
			Продолжить;
		КонецЕсли;
		Если ЭлементБазовый.Представление = "ИспользуетсяОтборПоСегментуПартнеров" Тогда
			ЭлементБазовый.ПравоеЗначение = Ложь;
			ЭлементБазовый.Использование=Истина;
			Продолжить;
		КонецЕсли;
		Если ЭлементБазовый.Представление = "ВариантКлассификацииЗадолженности" Тогда
			ЭлементБазовый.ПравоеЗначение = Справочники.ВариантыКлассификацииЗадолженности.ПустаяСсылка();
			ЭлементБазовый.Использование=Истина;
			Продолжить;
		КонецЕсли;
		// -- Галфинд Волков 18.09.2023
	...
	Если Не ПараметрДатаОстатков = Неопределено И ТипЗнч(ПараметрДатаОстатков.Значение) = Тип("Дата") Тогда
		//ПараметрДатаОстатков.Значение = Новый Граница(КонецДня(ПараметрДатаОстатков.Значение), ВидГраницы.Включая);
		ПараметрДатаОстатков.Значение = КонецДня(ДатаОкончания);
	...
	Для Каждого ЭлементБазовый Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		...
		ИмяПоля = ЭлементБазовый.ЛевоеЗначение;
		Если Лев(ИмяПоля, 8) = "Договор." Тогда
			ЭлементБазовый.ПравоеЗначение 	= Справочники.КоллекцииНоменклатуры.ПустаяСсылка();
			ПолеДоговорСезон = Новый ПолеКомпоновкиДанных("Договор.гф_Сезон");
			ЭлементБазовый.ЛевоеЗначение 	= ПолеДоговорСезон;
			ЭлементБазовый.Использование 	= Истина;
			ЭлементБазовый.Представление 	= "Договор.гф_Сезон";
			ЭлементБазовый.ВидСравнения 	= ВидСравненияКомпоновкиДанных.НеРавно;
			Н = Истина;
		КонецЕсли;	
	КонецЦикла;
	Если Не Н Тогда
			Отборы = КомпоновщикНастроек.Настройки.Отбор.Элементы;
			НовыйЭлемент = Отборы.Добавить(тип("ЭлементОтбораКомпоновкиДанных"));
			НовыйЭлемент.ПравоеЗначение 	= Справочники.КоллекцииНоменклатуры.ПустаяСсылка();
			ПолеДоговорСезон = Новый ПолеКомпоновкиДанных("Договор.гф_Сезон");
			НовыйЭлемент.ЛевоеЗначение 		= ПолеДоговорСезон;
			НовыйЭлемент.Использование 		= Истина;
			НовыйЭлемент.Представление 		= "Договор.гф_Сезон";
	...
	Для каждого строкаК Из контрагенты Цикл
		...
		Для каждого строкаД Из доки Цикл
			...
			Для каждого строкаБ Из бюджеты Цикл
				...
				// ++ Галфинд СадомцевСА 02.04.2024 Исправил код в соответствии с кодом из УПП
...
Процедура ПечатьТаблицыРассчёты(ДокументРезультат)
	...
	// заголовок
	// .. ДокументРезультат.НачатьГруппуСтрок("", Ложь); // Галфинд СадомцевСА 09.11.2023 Добавил строку
	ЗаголовокТаблица2(ДокументРезультат, Макет, Суммы, Бюджеты);

	ДокументРезультат.ФиксацияСверху = ДокументРезультат.ВысотаТаблицы; // Галфинд СадомцевСА 02.04.2024 Добавил строку
	...
	// .. ДокументРезультат.ЗакончитьГруппуСтрок(); // Галфинд СадомцевСА 09.11.2023 Добавил строку
...
Процедура ЗаголовокТаблица2(ДокументРезультат, Макет, Суммы, Бюджеты)
	...
	//Если ДолгиПоБюджетам Тогда
		//..ДокументРезультат.НачатьГруппуКолонок("Интервал", Ложь); // Галфинд СадомцевСА 09.11.2023 Заремарил строку
		первый = Истина;
		Для каждого строкаБ Из бюджеты Цикл
			...
			КонецЕсли;
		КонецЦикла;
		//..ДокументРезультат.ЗакончитьГруппуКолонок(); // Галфинд СадомцевСА 09.11.2023 Заремарил строку
	//КонецЕсли;
...
КонецПроцедуры
 Процедура СтрокаКонтрагентыТаблица2( Контрагент, ДокументРезультат, Макет, Суммы, Бюджеты, ВыборкаДок)
	...
	//лимиты
	// ++ Галфинд СадомцевСА 10.11.2023
	//e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee7ed1c955470b
	ИтогиБюджетов = Итоги.Выгрузить();
	ИтогиБюджетов.Свернуть("Контрагент, Бюджет", "СуммаДолга, СуммаЛимита, Доступно, Использовано");
	// -- Галфинд СадомцевСА 10.11.2023
	...
	Для каждого строкаС Из суммы Цикл
		...
		Для каждого строкаБ Из бюджеты Цикл
			...
			//..суммыБюджет = Данные.Выгрузить(новый Структура("Контрагент, Бюджет", Контрагент, строкаБ.Бюджет));
			суммыБюджет = ИтогиБюджетов.НайтиСтроки(новый Структура("Контрагент, Бюджет", Контрагент, строкаБ.Бюджет));
			граница = суммыБюджет.Количество();
...
Функция ПолучитьТаблицуЗапланированоНаДатуОтчета(Бюджеты)
	...
	// Галфинд СадомцевСА 09.11.2023 Заменил строку
	//..Если Бюджеты.Количество() > 1 Тогда
	Если Бюджеты.Количество() > 0 Тогда
...
Функция ПолучитьТаблицуОборотов()
	...
	НастройкиКД = Компоновщик2.ПолучитьНастройки();
	
	Для Каждого ЭлементБазовый Из НастройкиКД.Отбор.Элементы Цикл
	
		Если ЭлементБазовый.Использование = Истина Тогда
			ИмяПоля = ЭлементБазовый.ЛевоеЗначение;
			Если Лев(ИмяПоля, 8) = "Договор." Тогда
				НастройкиКД.Отбор.Элементы.Удалить(ЭлементБазовый);
			КонецЕсли;
		КонецЕсли;
		
	КонецЦикла;
	Компоновщик2.ЗагрузитьНастройки(НастройкиКД);
...
Процедура ПечатьТаблицыИтоговыйРезультат(ДокументРезультат, ПланНаДату)
	...
	Для каждого строка Из таб Цикл
		// ++ Галфинд СадомцевСА 09.11.2023
		ЭлСписка = БюджетСписок.НайтиПоЗначению(строка.Бюджет);
		Если ЭлСписка <> Неопределено Тогда
			Если НЕ ЭлСписка.Пометка Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		// -- Галфинд СадомцевСА 09.11.2023
