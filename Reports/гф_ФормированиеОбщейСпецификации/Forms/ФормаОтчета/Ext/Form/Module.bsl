...
Процедура УстановитьВидимостьРеквизитов()
	...
	// ++ СадомцевСА 15.04.2024 Добавил отбор по "Складу" и флажку "Проведен документ" по кнопке "Получить документы"
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eefaf9f15eb3a5
	Элементы.Склад.Видимость = Ложь;
	Элементы.ПроведенДокумент.Видимость = Ложь;
	// -- СадомцевСА 15.04.2024
	...
	Если Отчет.Вид = "РеализацииПокупателей" Тогда
		...
		// СадомцевСА 15.04.2024 Добавил 2 строки
		Элементы.Склад.Видимость = Истина;
		Элементы.ПроведенДокумент.Видимость = Истина;
...
Процедура ЗаполнитьДокументы_РеализацииПокупателей()
	...
	// СадомцевСА 15.04.2024 Добавил отбор по Складу
	Если ЗначениеЗаполнено(Отчет.Склад) Тогда
		Запрос.УстановитьПараметр("Склад", Отчет.Склад);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "РеализацияТоваровУслуг.Склад = &Склад",  "ИСТИНА");
	КонецЕсли;
	// СадомцевСА 15.04.2024 Добавил отбор по флажку ПроведенДокумент
	Если Отчет.ПроведенДокумент = Ложь Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И РеализацияТоваровУслуг.Проведен",  "");
	КонецЕсли;
...
Процедура ЗаполнитьДокументы_ЗаказыКлиентов() 
	...
	// СадомцевСА 15.04.2024 Добавил 1 строку
	...
	// СадомцевСА 15.04.2024 Добавил отбор по флажку ПроведенДокумент
	Если Отчет.ПроведенДокумент = Ложь Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЗаказКлиента.Проведен = ИСТИНА",  "ИСТИНА");
	КонецЕсли;
