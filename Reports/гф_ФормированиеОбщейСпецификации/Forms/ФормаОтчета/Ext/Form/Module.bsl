...
//  мТовары - массив - массив номенклатур
// XYZ Юрий 2022/09/20
&НаСервере
...
// Возвращаемое значение:
//  выборка запроса - выбора из регистра "Дополнительные сведения" (Объект, Свойство, Значение)
// XYZ Юрий 2022/09/20
&НаСервере
Функция ИскатьПереводы(Номенклатура)
	...
	знчСвойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("имя", стрЗначение);
	Запрос.УстановитьПараметр("Свойство", знчСвойство);
	Выборка = Запрос.Выполнить().Выбрать();
