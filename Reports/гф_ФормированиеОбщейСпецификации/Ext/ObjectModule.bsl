...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если Вид = "РеализацииПокупателей"  и ФормироватьВПарах Тогда
		Для каждого ПолеВыбор Из Настройки.Выбор.Элементы Цикл
			Если СокрЛП(ПолеВыбор.Поле) = "УпаковочныйЛист" Тогда
				ПолеВыбор.Использование = ВыводитьNVE;
			ИначеЕсли СокрЛП(ПолеВыбор.Поле) = "НомерГТД" Тогда
				ПолеВыбор.Использование = ВыводитьNVE;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;	
	...
	// ++ ДомнышеваКР_22_04_2024
	Область = ДокументРезультат.НайтиТекст("Итого");
	Если Область <> Неопределено Тогда
		ДокументРезультат.Область(Область.Верх, Область.Лево, Область.Низ ,ШиринаТаблицы).Разъединить();
	КонецЕсли;
    // -- ДомнышеваКР_22_04_2024
	...
	Иначе
		ОбработатьВПарах(ДокументРезультат, ДанныеРасшифровки); // Добавлено  Галфинд \ Sakovich 15.03.2024
	КонецЕсли;	
КонецПроцедуры
...
Процедура УстановитьПараметрыОтчета(Настройки, СписокДокументов, ТипЦеныЗакупочная, ТипЦеныРозничная)
	...
	КонецЕсли;
	// -- Галфинд ВолковЕВ 18.04.2023
	Если Вид = "ПриобретениеТоваровИУслуг" Тогда
		Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("Контрагент", Контрагент);
		Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("Договор", ДоговорКонтрагента);
	ИначеЕсли Вид = "РеализацииПокупателей"  и ФормироватьВПарах Тогда
		Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("ВыводитьNVE", ВыводитьNVE);
	КонецЕсли;

КонецПроцедуры

// vvv Галфинд \ Sakovich 15.03.2024
Процедура ОбработатьВПарах(ДокументРезультат, ДанныеРасшифровки)
	ОбластьКартинка = ДокументРезультат.НайтиТекст("Картинка");
	КоличествоСтрокТаблицы = ДокументРезультат.ВысотаТаблицы;
	Для Ст = 1 По КоличествоСтрокТаблицы Цикл
		ТекОбласть = ДокументРезультат.Область(Ст, ОбластьКартинка.Лево);
		Если ТекОбласть.Расшифровка <> Неопределено Тогда
			...
			Если ПоляРасшифровки.Количество() > 0 Тогда
				ЗначениеРасшифровки =ПоляРасшифровки.Получить(0).Значение;
				Если ТипЗнч(ЗначениеРасшифровки) = Тип("СправочникСсылка.НоменклатураПрисоединенныеФайлы")
					И ЗначениеЗаполнено(ЗначениеРасшифровки) Тогда
					КартинкаДД = РаботаСФайлами.ДвоичныеДанныеФайла(ЗначениеРасшифровки, Ложь);
					Рисунок = Новый Картинка(КартинкаДД);
	...
	Если ОбластьКартинка <> неопределено Тогда  
		...
		Для НомерСтроки = ОбластьКартинка.Верх + 2 По ДокументРезультат.ВысотаТаблицы Цикл
			...
			Если ОбластьНомер <> неопределено Тогда
				...
				Если ОбластьНомерСтроки.Текст <> "Итого" Тогда
			...
			Если ТипЗнч(Расшифровка) = Тип("ИдентификаторРасшифровкиКомпоновкиДанных") Тогда
				...
				Если ФайлКартинки <> Неопределено 
					...
					КартинкаДД = РаботаСФайлами.ДвоичныеДанныеФайла(ФайлКартинки.Значение, Ложь);
