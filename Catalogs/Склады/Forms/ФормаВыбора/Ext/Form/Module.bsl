...
Процедура гф_ПриОткрытииПосле(Отказ)
	...
	Попытка     
		...
		// ++ Галфинд СадомцевСА 16.11.2022 Заменил кусок кода т.к. он влиял на выбор Склада в "других" окнах
		...
		//	КонецЕсли;
		//КонецЦикла;
		КоробнойСклад = гф_ЗаполнитьКоробнойСклад(ФормаВладелец);
...
// ++ Галфинд СадомцевСА 16.11.2022 Заменил кусок кода т.к. он влиял на выбор Склада в "других" окнах
// Параметры:
//  ФормаВладелец - Форма клиентского приложения
// 
// Возвращаемое значение:
//  Число 1 или 0 при наличии 
//  Неопределено
&НаКлиенте
Функция гф_ЗаполнитьКоробнойСклад(ФормаВладелец)
	Если ТипЗнч(ФормаВладелец) = Тип("ФормаКлиентскогоПриложения")
		И (ФормаВладелец.ИмяФормы = "ВнешнийОтчет.гф_ОстаткиНоменклатуры.Форма"
			ИЛИ ФормаВладелец.ИмяФормы = "Отчет.гф_ОстаткиНоменклатуры.Форма") Тогда
		Возврат ?(ФормаВладелец.гф_ТоварыВКоробах, 1, 0);
	КонецЕсли;
