...
Процедура гф_ПриОткрытииПосле(Отказ)
	...
	Попытка     
		...
		Если ТипЗнч(ФормаВладелец) = Тип("ФормаКлиентскогоПриложения")
			И (ФормаВладелец.ИмяФормы = "Отчет.гф_ОстаткиНоменклатуры.Форма.ФормаОтчета"
				ИЛИ ФормаВладелец.ИмяФормы = "ВнешнийОтчет.гф_ОстаткиНоменклатуры.Форма.ФормаОтчета") Тогда
			//Сообщить("" + ФормаВладелец.ИмяФормы);
