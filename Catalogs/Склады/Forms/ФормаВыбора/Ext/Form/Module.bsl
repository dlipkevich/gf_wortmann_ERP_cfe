...
Процедура гф_ПриОткрытииПосле(Отказ)
	...
	//Катютин 191222 { - вылетает с ошибкой
	...
	// ++ Галфинд СадомцевСА 16.11.2022 Заменил кусок кода т.к. он влияет на выбор Склада в "других" окнах
	СтруктураСклад = Новый Структура("гф_ТоварыВКоробах", Неопределено);
	
	Попытка     
		
		ЗаполнитьЗначенияСвойств(СтруктураСклад, ФормаВладелец);
		
	Исключение
	КонецПопытки;
	
	Если СтруктураСклад["гф_ТоварыВКоробах"] <> Неопределено Тогда
		КоробнойСклад = ?(СтруктураСклад["гф_ТоварыВКоробах"], 1, 0);
	КонецЕсли;
	// -- Галфинд СадомцевСА 16.11.2022
	...
	Исключение
		// ++ СадомцевСА 15.04.2024 Добавил отбор по Организации для формы отчета
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eefaf9f15eb3a5
		// Возврат;
		Попытка
			ОбъектФормыВладельца = ФормаВладелец.Отчет;
		Исключение
			Возврат;
		КонецПопытки;	
