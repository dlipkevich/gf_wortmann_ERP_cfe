
#Область ОбработчикиСобытийФормы

// #wortmann {
// #4.2.03
// Добавляет в обработчик события вызов создания полей контактного лица, устанавливает доступность групп доступа
// Журавлев Чугуев 2022/07/12
// 
// Параметры:
// Отказ				- Булево
// СтандартнаяОбработка	- Булево
&НаСервере
Процедура гф_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	
	гф_СоздатьРеквизитыКонтактногоЛица();

	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		Элементы.гф_ГруппыДоступа.Доступность = РольДоступна("гф_РедактированиеКонтактныхЛицПартнеров");
	КонецЕсли;
	
КонецПроцедуры// } #wortmann

// #wortmann {
// #4.2.03
// Добавляет в обработчик события вызовы создания и заполнения полей контактного лица
// Журавлев Чугуев 2022/07/12
// 
// Параметры:
// ТекущийОбъект - СправочникОбъект.КонтактныеЛица
&НаСервере
Процедура гф_ПриЧтенииНаСервереПосле(ТекущийОбъект)
	
	гф_СоздатьРеквизитыКонтактногоЛица();
	
КонецПроцедуры// } #wortmann

// #wortmann {
// #4.2.03
// Добавляет на форму поля контактного лица, если еще не созданы
// Журавлев Чугуев 2022/07/12
&НаСервере
Процедура гф_СоздатьРеквизитыКонтактногоЛица()
	
	Если Элементы.Найти("гф_ГруппыДоступа") = Неопределено Тогда
		
		ТипПолеФормы = Тип("ПолеФормы");
		ТипТаблицаФормы	= Тип("ТаблицаФормы");
		
		НовоеПоле = Элементы.Добавить("гф_ГруппыДоступа", ТипТаблицаФормы,
										Элементы.ГруппаОсновныеСведения);
		
		НовоеПоле.Видимость				= Истина;
		НовоеПоле.ПутьКДанным			= "Объект.гф_ГруппыДоступа";
		НовоеПоле.Отображение			= ОтображениеТаблицы.Список;
		НовоеПоле.ПоложениеЗаголовка	= ПоложениеЗаголовкаЭлементаФормы.Лево;
		НовоеПоле.УстановитьДействие("ОбработкаВыбора", "гф_ГруппыДоступаОбработкаВыбора");
		
		НовоеПоле = Элементы.Добавить("гф_ГруппыДоступа_ГруппаДоступа", ТипПолеФормы,
										Элементы.гф_ГруппыДоступа);
		
		НовоеПоле.Вид			= ВидПоляФормы.ПолеВвода;
		НовоеПоле.Видимость		= Истина;
		НовоеПоле.ПутьКДанным	= "Объект.гф_ГруппыДоступа.ГруппаДоступа";
		// #wortmann {
		// #4.2.03
		// Не нужно подбирать группы доступа из специального отфильтрованного списка
		// Журавлев Чугуев 2022/08/01
		//НовоеПоле.УстановитьДействие("НачалоВыбора", "гф_ГруппыДоступа_ГруппаДоступаНачалоВыбора");
		// } #wortmann
		
	КонецЕсли;
	
КонецПроцедуры// } #wortmann

// #wortmann {
// #4.2.03
// Обработчик события Начало выбора для групп доступа
// Журавлев Чугуев 2022/07/17
// 
// Параметры:
// Элемент				- Элемент формы
// ДанныеВыбора			- Справочник "Группы доступа"
// СтандартнаяОбработка	- Булево
&НаКлиенте
Процедура гф_ГруппыДоступа_ГруппаДоступаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка) 
	
	СтандартнаяОбработка = Ложь;
	
	ГруппаДоступа = _омОбщегоНазначенияКлиентСервер.ПолучитьГлобальноеЗначение("ГруппаДоступаДляКонтрагентов");
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ГруппаДоступа", ГруппаДоступа); 
	ПараметрыФормы.Вставить("РежимВыбора", Истина);
	
	ОткрытьФорму("Справочник.ГруппыДоступа.Форма.гф_ФормаВыбора", 
		ПараметрыФормы, Элементы["гф_ГруппыДоступа"], 
		ЭтаФорма.УникальныйИдентификатор, , ,
		Новый ОписаниеОповещения("ВыборЗавершение", ЭтотОбъект));
	
КонецПроцедуры// } #wortmann

// #wortmann {
// #4.2.03
// Обработчик оповещения завершения выбора для групп доступа
// Журавлев Чугуев 2022/07/17
// 
// Параметры:
// РезультатЗакрытия		- Справочник "Группы доступа"
// ДополнительныеПараметры	- Структура
&НаКлиенте
Процедура ВыборЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	
	Если ЗначениеЗаполнено(РезультатЗакрытия) Тогда
		
		ТекДанные = Элементы["гф_ГруппыДоступа"].ТекущиеДанные;
		
		Если ТекДанные <> Неопределено Тогда
			ТекДанные.ГруппаДоступа = РезультатЗакрытия;
		КонецЕсли;
		
	КонецЕсли;
	
КонецПроцедуры// } #wortmann

#КонецОбласти


