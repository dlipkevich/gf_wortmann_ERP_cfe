...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ЭлементСТакимЖеИменем	= НайтиТакойжеЭлемент(); 
	Если НЕ ЭлементСТакимЖеИменем = Объект.Ссылка И НЕ ЭлементСТакимЖеИменем = Неопределено И НЕ ЭлементСТакимЖеИменем.Пустая() Тогда
		Отказ = Истина;
		Предупреждение("Элемент справочника Последовательности проведения для интерфейса " + Объект.Наименование + " уже существует!");
	КонецЕсли;
