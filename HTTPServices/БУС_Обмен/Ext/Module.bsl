...
Функция GetAvailableStockInformationGet(Запрос)
	...
	|ИЗ
	|	РегистрНакопления.гф_ДвижениеКодовМаркировкиОрганизации.Остатки(, КМ.Номенклатура В (&МассивНоменклатуры)) КАК ДвижениеКодовМаркировкиОрганизацииОстатки
	|
	|СГРУППИРОВАТЬ ПО 
    ...
    |ГДЕ
    |	ВариантыКомплектацииНоменклатуры.Владелец В (&МассивНоменклатуры)
...
Функция СоздатьЗаказПокупателя(ДанныеЗаказа)
	...
	//уточнить где взять организацию
		...
		МассивНайденных = Тч.НайтиСтроки(Новый Структура("АдресДоставки, Склад", СтрокаШапка.АдресДоставки, СтрокаШапка.Склад));
		Если МассивНайденных.Количество() > 0 Тогда
			...
			СтрокаНайденных = МассивНайденных[0];
			ГруппаДоступаНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаНайденных["Номенклатура"], "ГруппаДоступа");
			ЗапросОрганизации.УстановитьПараметр("ГруппаДоступаНоменклатуры", ГруппаДоступаНоменклатуры);
			
			ВыборкаОрганизации = ЗапросОрганизации.Выполнить().Выбрать();
			Если ВыборкаОрганизации.Следующий() Тогда
				Заказ.Организация = ВыборкаОрганизации["Организация"];	
			КонецЕсли;
...
//1БЦ-- работы по задаче 32798
	...
	Результат.Вставить("ADDRESS_XML_ID", СокрЛП(Формат(АдресДоставкиGLN_номер, "ЧГ=0")));
