...
Процедура СоздатьКорректировкуНазначенияНеКоробногоСклада(Заказ)
	...
	|	СУММА(ТоварыНаСкладахОстатки.ВНаличииОстаток) КАК КоличествоУпаковок,
	|	&Склад КАК Склад,
	|	&Заказ КАК НовыйЗаказ
	|ИЗ
...
&НаСервере
Процедура НеИспользуется_ЗаполнитьТоварыЗаказа(Заказ)
