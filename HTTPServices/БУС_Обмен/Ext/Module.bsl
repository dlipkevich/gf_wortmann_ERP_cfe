...
Функция СоздатьЗаказПокупателя(ДанныеЗаказа)
	...
	//уточнить где взять организацию
		...
		Заказ.B2B_Сезон = Заказ.ДоговорКонтрагента.гф_Сезон;

		Заказ.гф_ВидЦены = Справочники.w_Настройки.ТипЦенОптовая.Значение;
		...
		Исключение
			ОписаниеОшибки = ОписаниеОшибки();
			ЗаписьЖурналаРегистрации("ОбменB2B.CheckOrCreateOrder", УровеньЖурналаРегистрации.Ошибка,Метаданные.Документы.ЗаказПокупателя,Заказ.Ссылка,"Проведение заказа при загрузке с сайта произошло с ошибкой:" + Символы.ПС + ОписаниеОшибки());
		КонецПопытки;
		//1БЦ--
...
Функция СформироватьВременныеТаблицыПоЗаказам(МассивДанных) Экспорт
	...
	|	ЗаказПокупателя.АдресДоставкиНовый КАК АдресДоставкиНовый
	|ПОМЕСТИТЬ Заказы
	|ИЗ
...
Функция ВыгрузитьЗаказ(МенеджерВременныхТаблиц, ТекЗаказ)
	...
	B2B_НомерЗаказа = ПолучитьЗначениеСвойства(ТекЗаказ.Ссылка, ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("B2B_НомерЗаказа"));
...
//1БЦ-- работы по задаче 32798
	...
	Результат.Вставить("PARTNER_XML_ID", СокрЛП(ТекЗаказ.Контрагент.RC_GLN_номер));
	Результат.Вставить("ADDRESS_XML_ID", СокрЛП(Формат(ТекЗаказ.АдресДоставкиНовый.GLNНомер, "ЧГ=0")));
	...
	Для каждого ТекНоменклатура из МассивТоваров Цикл
		...
		Всего = ?(ТекЗаказ.СуммаВключаетНДС,ТекНоменклатура.Сумма,ТекНоменклатура.Сумма + ТекНоменклатура.СуммаНДС);
		
		PRICE = ?(ТекНоменклатура.Количество = 0,Всего,Всего/ТекНоменклатура.Количество);
		BASE_PRICE = ?(ТекЗаказ.СуммаВключаетНДС,ТекНоменклатура.Цена ,ТекНоменклатура.Цена + ТекНоменклатура.Цена/100*УчетНДС.ПолучитьСтавкуНДС(ТекНоменклатура.СтавкаНДС)); 		
		DISCOUNT_PRICE = BASE_PRICE*ТекНоменклатура.Количество - Всего;
		VAT_RATE = УчетНДС.ПолучитьСтавкуНДС(ТекНоменклатура.СтавкаНДС)/100;
		VAT_INCLUDED  = ?(ТекЗаказ.СуммаВключаетНДС,1,0);
