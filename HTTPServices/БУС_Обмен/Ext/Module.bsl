...
Функция GetAvailableStockInformationGet(Запрос)
	...
	B2B_ОбщиеПроцедурыИФункции.УстановитьПараметрыЗапросаОстатков(Запрос, МассивНоменклатуры);
	...
	Пока Выборка.Следующий() Цикл
		...
		ИдТовара = B2B_ОбщиеПроцедурыИФункции.ПолучитьИдТовараИзОстатков(Выборка);
...
Функция СоздатьЗаказПокупателя(ДанныеЗаказа)
	...
	Для Каждого СтрокаШапка из Шапка Цикл
		...
		СкладКоробной = УправлениеСвойствами.ЗначениеСвойства(Заказ.Склад, "гф_СкладыТоварыВКоробах") = Истина;
		...
		Для каждого строка Из ПоискТЧ Цикл
			...
			новая.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.КОбеспечению;
			новая.Обособленно = Истина;
			...
			Если (НЕ новая.Номенклатура.Пустая()) и 
				...
				Если рез.Следующий() Тогда
					новая.Цена = рез.Цена;
					новая.гф_ЦенаСоСкидкой = новая.Цена;
			...
			Если Заказ.ЦенаВключаетНДС Тогда
				...
				Если ЗначениеЗаполнено(ЧисловаяСтавкаНДС) Тогда
					новая.СуммаНДС = новая.Сумма * ЧисловаяСтавкаНДС / (1 + ЧисловаяСтавкаНДС);
				КонецЕсли; 
			...
			Иначе
				...
				Если ЗначениеЗаполнено(ЧисловаяСтавкаНДС) Тогда
					новая.СуммаНДС = новая.Сумма * ЧисловаяСтавкаНДС;
				КонецЕсли; 
				
				новая.СуммаСНДС = новая.Сумма + новая.СуммаНДС;
				
			КонецЕсли;
			
			Если СкладКоробной Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации("ОбменB2B.CheckOrCreateOrder", УровеньЖурналаРегистрации.Предупреждение,,,"Документ НЕ записан " + ОписаниеОшибки);
		...
		Если ЗаказУспешноПровели И РезервироватьЗаказКлиентаПриЗагрузке Тогда
			...
			Иначе
				СоздатьКорректировкуНазначенияНеКоробногоСклада(Заказ.Ссылка);
			КонецЕсли;
...
Процедура СоздатьКорректировкуНазначения(Заказ)
	...
	|ГДЕ
	|	ШтрихкодыУпаковокТоваров.Номенклатура В (ВЫБРАТЬ Т.Номенклатура ИЗ ВтЗаказТоварыВКоробах КАК Т)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ГДЕ
	|	ВложенныеШтрихкоды.Штрихкод.Номенклатура В (ВЫБРАТЬ Т.Номенклатура ИЗ ВтЗаказТоварыВКоробах КАК Т)
	|;
	|
    |////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрНакопления.гф_ДвижениеКодовМаркировкиОрганизации.Остатки(, Склад = &Склад
	|		 И КМ В (ВЫБРАТЬ Т.КМ ИЗ ВтШтрихкоды КАК Т)) КАК ДвижениеКодовМаркировкиОрганизацииОстатки
	|
	|СГРУППИРОВАТЬ ПО 
    ...
    |	ДокументУпаковочныйЛист.гф_СостояниеКороба КАК СостояниеКороба,
    |	ДокументУпаковочныйЛист.гф_Агрегация КАК Агрегация,
    |	ДокументУпаковочныйЛист.гф_Комплектация КАК ВариантКомплектации,
	...
	|			ПО ДокументУпаковочныйЛист.гф_Заказ = ДокументЗаказКлиента.Ссылка
	|
	|ГДЕ
    |	ДокументУпаковочныйЛист.гф_Комплектация В
	|		(ВЫБРАТЬ ЗаказТоварыВКоробах.ВариантКомплектации ИЗ ВтЗаказТоварыВКоробах КАК ЗаказТоварыВКоробах)
    |	И ДокументУпаковочныйЛист.гф_СостояниеКороба В(&СостоянияКоробов)
	|	И ДокументЗаказКлиента.Ссылка ЕСТЬ NULL
	...
	Запрос.УстановитьПараметр("СостоянияКоробов", B2B_ОбщиеПроцедурыИФункции.СостоянияКоробовДляРезервирования());  
	Запрос.Выполнить();
	...
	Пока ВыборкаВариантов.Следующий() Цикл
		...
		КоличествоВариантов = Цел(ВыборкаВариантов["Количество"]);
		
		Если Не ЗначениеЗаполнено(КоличествоВариантов) Тогда
			Продолжить;
		КонецЕсли;
		...
		КоличествоВариантовСтрокой = Формат(КоличествоВариантов, "ЧГ=0");
	...
	Попытка
		...
		СоздатьДокументКорректировкиНазначения(Заказ, РеквизитыЗаказа, НазначениеЗаказ, РезультатЗапроса);
		...
		Для Каждого ЛистСсылка Из УпакЛисты Цикл
			...
			ЗаписьЖурналаРегистрации("ОбменB2B.CheckOrCreateOrder", УровеньЖурналаРегистрации.Информация ,Метаданные.Документы.УпаковочныйЛист,
				УЛДок.Ссылка, "Документ обновлен при загрузке с сайта!");
	...
	Исключение
		...
		ОписаниеОшибки = ОписаниеОшибки();
		ЗаписьЖурналаРегистрации("ОбменB2B.CheckOrCreateOrder", УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.КорректировкаНазначенияТоваров,,
...
Процедура СоздатьДокументКорректировкиНазначения(Заказ, РеквизитыЗаказа, НазначениеЗаказ, РезультатЗапроса)
	...
	ЗаписьЖурналаРегистрации("ОбменB2B.CheckOrCreateOrder", УровеньЖурналаРегистрации.Информация, Метаданные.Документы.КорректировкаНазначенияТоваров,
		ДокКН.Ссылка, "Документ проведен при загрузке с сайта!");
		
КонецПроцедуры

Процедура СоздатьКорректировкуНазначенияНеКоробногоСклада(Заказ)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВтЗаказКлиентаТовары.Номенклатура КАК Номенклатура,
	|	ВтЗаказКлиентаТовары.Характеристика КАК Характеристика,
	
	|	ВЫБОР
	|		КОГДА ВтТоварыНаСкладахОстатки.КоличествоОстаток < ВтЗаказКлиентаТовары.Количество
	|			ТОГДА ВтТоварыНаСкладахОстатки.КоличествоОстаток
	|		ИНАЧЕ ВтЗаказКлиентаТовары.Количество
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтТоварыНаСкладахОстатки КАК ВтТоварыНаСкладахОстатки
	|		ПО ВтЗаказКлиентаТовары.Номенклатура = ВтТоварыНаСкладахОстатки.Номенклатура
	|			И ВтЗаказКлиентаТовары.Характеристика = ВтТоварыНаСкладахОстатки.Характеристика";
...
Функция ОбработатьЗагруженныеДанные(ДанныеЗаказа)
	...
	Для каждого строка Из ДанныеЗаказа.Получить("PRODUCTS") Цикл
		...
		Если Не ЗначениеЗаполнено(Номенклатура) Тогда
			ТекстСообщения = "Не найдена номенклатура по XML_ID: %1 (CML2_ARTICLE: %2, ORDER_ID: %3)";
		...
		Если Не ЗначениеЗаполнено(Характеристика) Тогда
			ТекстСообщения = "Не найдена характеристика по XML_ID: %1 (CML2_ARTICLE: %2, ORDER_ID: %3)";
			ТекстСообщения = СтрШаблон(ТекстСообщения, XML_ID, строка["CML2_ARTICLE"], ДанныеЗаказа.Получить("ORDER_ID"));  
			ЗаписьЖурналаРегистрации("ОбменB2B.CheckOrCreateOrder", УровеньЖурналаРегистрации.Предупреждение,,, ТекстСообщения);
		КонецЕсли;
...
Функция CheckSiteUpdateStatusGet(Запрос)
	...
	Попытка
		...
		ИмяПортала = ИмяПорталаИзЗапроса(Запрос.ПараметрыЗапроса);
		Результат.Вставить("ИмяПортала", ИмяПортала);
		
		Задание = РегламентноеЗаданиеУзла(ИмяПортала, "ОбменB2B.CheckSiteUpdateStatus");
		
		Если Задание <> Неопределено И Задание.Метаданные.Имя = "ОбменССайтом" Тогда
		...
		Если Найдено > 1 Тогда
			Результат.Вставить("Состояние", "Ошибка");
			Результат.Вставить("Включено",  "Ошибка");
			Результат.Вставить("Причина", "Найдено более одного задания!");
		КонецЕсли;	
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("ОбменB2B.CheckSiteUpdateStatus", УровеньЖурналаРегистрации.Ошибка,,, "Ошибка: " + ОписаниеОшибки());
...
Функция РегламентноеЗаданиеУзла(ИмяПортала, СобытиеЖурналаРегистрации)
	
	Если Не ЗначениеЗаполнено(ИмяПортала) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации, УровеньЖурналаРегистрации.Информация,,, "ИмяПортала = " + ИмяПортала);
	
	ПорталСсылка = Справочники.B2B_Портал[ИмяПортала];
	
	УзелСсылка = ПланыОбмена.ОбменССайтом.НайтиПоРеквизиту("B2B_Портал", ПорталСсылка);
	Если Не ЗначениеЗаполнено(УзелСсылка) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ИдентификаторЗаданияСтрокой = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелСсылка, "ИдентификаторРегламентногоЗадания");
	
	Если Не ЗначениеЗаполнено(ИдентификаторЗаданияСтрокой) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ИдентификаторЗадания = Новый УникальныйИдентификатор(ИдентификаторЗаданияСтрокой);
	
	Возврат РегламентныеЗадания.НайтиПоУникальномуИдентификатору(ИдентификаторЗадания);
	
КонецФункции

Функция ИмяПорталаИзЗапроса(ПараметрыЗапроса)
	
	ИмяПортала = ПараметрыЗапроса.Получить("Portal");
	
	ИмяПортала = СтрЗаменить(ИмяПортала, " ", "");
...
Функция StartSiteUpdateGet(Запрос)
	...
	Результат = Новый Структура;
	Результат.Вставить("Результат", "Ошибка");
	...
	Попытка
		...
		Задание = РегламентноеЗаданиеУзла(ИмяПортала, "ОбменB2B.StartSiteUpdate"); 
		
		Если Задание <> Неопределено И Задание.Метаданные.Имя = "ОбменССайтом" Тогда
			
			Если Задание.Использование Тогда
				
				ФоновыеЗадания.Выполнить(Задание.Метаданные.ИмяМетода, Задание.Параметры, Задание.Ключ, Задание);
				
				ЗаписьЖурналаРегистрации("ОбменB2B.StartSiteUpdate", УровеньЖурналаРегистрации.Информация,,, "Задание запущено");
				Результат.Вставить("Результат","Успешно"); 
				
			Иначе
				
				ЗаписьЖурналаРегистрации("ОбменB2B.StartSiteUpdate", УровеньЖурналаРегистрации.Предупреждение,,, "Запуск запрещен");
				Результат.Вставить("Результат", "Запрещено");
				
			КонецЕсли;
			
		Иначе
			
			Результат.Вставить("Результат", "Задание не найдено");
			
		КонецЕсли;
		
	Исключение
		
		ЗаписьЖурналаРегистрации("ОбменB2B.StartSiteUpdate", УровеньЖурналаРегистрации.Ошибка,,,
			"При запуске задания произошла ошибка: " + ОписаниеОшибки());
		Результат.Вставить("Результат", "Ошибка в процессе запуска фонового задания");
...
Функция StartUpdateStockGoodsGet(Запрос)
	...
	Попытка
		...
		ИмяПортала = ИмяПорталаИзЗапроса(Запрос.ПараметрыЗапроса);
		Портал = Справочники.B2B_Портал[ИмяПортала];
		
	Исключение
		ЗаписьЖурналаРегистрации("ОбменB2B.UpdateStockGoods", УровеньЖурналаРегистрации.Ошибка,,, ОписаниеОшибки());
	КонецПопытки;
	...
	Если ЗначениеЗаполнено(START_UNLOADING) Тогда
		...
		ЗаписьЖурналаРегистрации("ОбменB2B.UpdateStockGoods", УровеньЖурналаРегистрации.Информация,,,
			"Получен запрос на получение остатков со стороны портала " + Портал + " с ответом сразу");
		
		Попытка
			
			ТекстОтвета = B2B_РегламентныеПроцедуры.B2B_ТекстОтветаОстатков(Портал);
	...
	Иначе 
		...
		ЗаписьЖурналаРегистрации("ОбменB2B.UpdateStockGoods", УровеньЖурналаРегистрации.Информация,,,
			"Получен запрос на получение остатков со стороны портала " + Портал + " с ответом отдельным вызовом");
		
		Попытка
			
			ПараметрыМетода = Новый Массив;
			ПараметрыМетода.Добавить(Портал);
			
			ФоновыеЗадания.Выполнить("B2B_РегламентныеПроцедуры.B2B_ВыгрузитьОстаткиНаПортал",
				ПараметрыМетода, Новый УникальныйИдентификатор, "B2B_ВыгрузитьОстаткиНаПортал");
