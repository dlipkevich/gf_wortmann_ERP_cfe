...
Процедура гф_ПодготовитьДокументыЗаказНаЭмиссиюКМкПередачеИСМП()
	Попытка
		...
		,СтрокаСообщения);
		// СадомцевСА 07.02.2024
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eec0dcf39c8d9c
		ЗаказНаЭмиссиюКМИзменитьИдентификаторПроизводственногоЗаказа();
...
Функция ПодготовитьМассивДокументовЗаказНаЭмиссиюКМ()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			   ...
			   // СадомцевСА 20.12.2023 Реализовал "обработку" Заказов на эмиссию КМ с учетом указанных приоритетов
			   // Добавил поле Порядок
	           |	ВЫБОР
	           |		КОГДА Статусы.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.Черновик)
	           |				И Статусы.ДальнейшееДействие1 = ЗНАЧЕНИЕ(Перечисление.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеКодыМаркировки)
	           |			ТОГДА 1
	           |		КОГДА Статусы.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.СУЗПринятКОбработке)
	           |				И Статусы.ДальнейшееДействие1 = ЗНАЧЕНИЕ(Перечисление.ДальнейшиеДействияПоВзаимодействиюИСМП.ОжидайтеЗавершенияОбработкиДанныхИСМП)
	           |			ТОГДА 2
	           |		КОГДА Статусы.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.КПередачеВИСМП)
	           |				И Статусы.ДальнейшееДействие1 = ЗНАЧЕНИЕ(Перечисление.ДальнейшиеДействияПоВзаимодействиюИСМП.ОжидайтеПередачуДанныхРегламентнымЗаданием)
	           |			ТОГДА 3
	           |		КОГДА Статусы.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.Ошибка)
	           |				И Статусы.ДальнейшееДействие1 = ЗНАЧЕНИЕ(Перечисление.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеКодыМаркировки)
	           |			ТОГДА 4
	           |		ИНАЧЕ 100
	           |	КОНЕЦ КАК Порядок,
			   ...
			   // Галфинд СадомцевСА 13.12.2023 Добавил в очередь сообщений ИСМП Заказы на эмиссию со статусом "Обрабатывается в СУЗ (генерация кодов)"
			   // e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee98c979cb7f4b
			   //|	И Статусы.ДальнейшееДействие1 = ЗНАЧЕНИЕ(Перечисление.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеКодыМаркировки)
			   |	И (Статусы.ДальнейшееДействие1 = ЗНАЧЕНИЕ(Перечисление.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеКодыМаркировки)
	           |		ИЛИ Статусы.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.СУЗПринятКОбработке)
	           |		ИЛИ Статусы.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.КПередачеВИСМП))
               ...
               |	Статусы.Статус КАК СтатусИСМП,
               |	Статусы.ДальнейшееДействие1 КАК ДальнейшееДействиеИСМП1,
			   // СадомцевСА 20.12.2023 Добавил поле Порядок
	           |	ВЫБОР
	           |		КОГДА Статусы.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.Черновик)
	           |				И Статусы.ДальнейшееДействие1 = ЗНАЧЕНИЕ(Перечисление.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеКодыМаркировки)
	           |			ТОГДА 1
	...
	// СадомцевСА 13.03.2024 Добавил Дату в сортировку Заказов на эмиссию КМ для последующего добавления в очередб сообщений
	ТЗ.Сортировать("Порядок, Дата");
...
Процедура гф_МаркировкаТоваровВводВОборот()
	Попытка
		...
		Если ТЗДокументов.Количество() > 0 Тогда
			...
			ОбработатьТЗДокументов(ТЗДокументов);
			...
			//		И СтруктураВозврата["МаркировкаТоваровИСМП"].Проведен
			...
			//	// для события "РегламентированноеЗаданиеМаркировкаТоваровВводВОборот"
			//	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeb5ffa09cbb5f
			//	ИначеЕсли ЗначениеЗаполнено(СтруктураВозврата.ОписаниеОшибки) Тогда
			//		ТекстОшибки = "" + СтруктураВозврата.ОписаниеОшибки;
			//		Если СтруктураВозврата.Свойство("НеОпубликованыТовары") Тогда
			//			ТекстОшибки = ТекстОшибки + Символы.ПС + "Не все товары опубликованы! " + 
			//				" Документ:" + СтруктураВозврата["МаркировкаТоваровИСМП"];
			//			// Для Каждого СтрокаТЗ Из СтруктураВозврата["НеОпубликованыТовары"] Цикл
			//			//	ТекстОшибки = ТекстОшибки + Символы.ПС + СтрокаТЗ.Номенклатура + "/" + СтрокаТЗ.Характеристика;
			//			// КонецЦикла;
			//		КонецЕсли;
			//		СтрокаСообщения = "Ошибка при создании Маркировки товаров ИС МП: " + ТекстОшибки;
			//		ЗаписьЖурналаРегистрации("РегламентированноеЗаданиеМаркировкаТоваровВводВОборот",
			//			УровеньЖурналаРегистрации.Ошибка,
			//			Метаданные.РегламентныеЗадания.гф_МаркировкаТоваровВводВОборот,
			//			,СтрокаСообщения);
			//	КонецЕсли;
			//КонецЦикла;
			// Галфинд СадомцевСА 19.03.2024 Отменил "передачу в ЧЗ" документов Маркировка товаров
			// регл. заданием "Маркировка товаров (ввод в оборот)" т.к. "передача в ЧЗ" выполняется
			// другим регл. заданием "Подготовить документы Маркировка товаров к передаче ИСМП" (Парамонов Р.)
		...
		// vvv Галфинд \ Sakovich 03.04.2024
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eef18868ba6b22
		ТЗДокументовПБЗ = ПодготовитьТЗДокументовПБЗ();
		Если ТЗДокументовПБЗ.Количество() > 0 Тогда
			Для Каждого СтрокаТЗ Из ТЗДокументовПБЗ Цикл
				СтруктураВозврата = гф_МаркировкаТоваровИСМПВызовСервера.гф_СоздатьМаркировкаТоваровИСМП_ОснованиеПБЗ(СтрокаТЗ);
				Если Не (ЗначениеЗаполнено(СтруктураВозврата["МаркировкаТоваровИСМП"])
					И СтруктураВозврата["МаркировкаТоваровИСМП"].Проведен
					И СтруктураВозврата["ЕстьОшибкиЗаполнения"] = Ложь)
					И ЗначениеЗаполнено(СтруктураВозврата.ОписаниеОшибки) Тогда
					ТекстОшибки = "" + СтруктураВозврата.ОписаниеОшибки;
					Если СтруктураВозврата.Свойство("НеОпубликованыТовары") Тогда
						ТекстОшибки = ТекстОшибки + Символы.ПС + "Не все товары опубликованы! " + 
						?(ЗначениеЗаполнено(СтруктураВозврата["МаркировкаТоваровИСМП"]),
						" Документ:" + СтруктураВозврата["МаркировкаТоваровИСМП"], "");
					КонецЕсли;
					СтрокаСообщения = "Ошибка при создании Маркировки товаров ИС МП: " + ТекстОшибки;
					ЗаписьЖурналаРегистрации("РегламентированноеЗаданиеМаркировкаТоваровВводВОборот",
						УровеньЖурналаРегистрации.Ошибка,
						Метаданные.РегламентныеЗадания.гф_МаркировкаТоваровВводВОборот, , СтрокаСообщения);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		// ^^^ Галфинд \ Sakovich 03.04.2024
...
Процедура ОбработатьТЗДокументов(ТЗДокументов)
	МассивМаркировки = Новый Массив;
	Для Каждого СтрокаТЗ Из ТЗДокументов Цикл
		СтруктураВозврата = гф_МаркировкаТоваровИСМПВызовСервера.гф_СоздатьМаркировкаТоваровИСМП(СтрокаТЗ);
		Если ЗначениеЗаполнено(СтруктураВозврата["МаркировкаТоваровИСМП"])
...
Функция ПодготовитьТЗДокументовПТУ()
	...
	|	И  ТаможеннаяДекларацияИмпортТовары.ДокументПоступления.гф_ЗаказатьКМ
	// -- Галфинд_ДомнышеваКР_12_02_2024
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	// ++ Галфинд СадомцевСА 18.03.2024 Регл. задание "Маркировка товаров (ввод в оборот)"
	// Добавил фильтр по "Особенность учета номенклатуры" при поиске ПТУ для создания Маркировки ИСМП
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eee519bc4c8d69
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	Документ.ПриобретениеТоваровУслуг.Товары КАК ТаблицаТовары
	|	ПО ТаможеннаяДекларацияИмпорт.ДокументПоступления = ТаблицаТовары.Ссылка
...
Процедура гф_ВыгрузитьФайлыВWMS()
	...
	Попытка
		ЗаписьЖурналаРегистрации("РегламентнаяВыгрузкаФайловВWMS.СтартОбработки", УровеньЖурналаРегистрации.Информация,
			Метаданные.Обработки.гф_ОбменСWMS);
		ОбработкаЗапуска = Обработки.гф_ОбменСWMS.Создать();
		ОбработкаЗапуска.ВыгрузитьДанные();
		ЗаписьЖурналаРегистрации("РегламентнаяВыгрузкаФайловВWMS.ЗавершениеОбработки", УровеньЖурналаРегистрации.Информация,
			Метаданные.Обработки.гф_ОбменСWMS);
	Исключение
		ТекстОшибки = ОписаниеОшибки();
		СтрокаСообщения = "Ошибка при запуске обработки: " + ТекстОшибки;
		ЗаписьЖурналаРегистрации("РегламентнаяВыгрузкаФайловВWMS.ОшибкаПриЗапуске", УровеньЖурналаРегистрации.Информация,
			Метаданные.Обработки.гф_ОбменСWMS,,СтрокаСообщения);
	КонецПопытки;
...
// #wortmann {
// Метод регламентного задания "Создать документы Вывода КМ для РТУ в ЕАЭС"
// Галфинд Домнышева 2023/12/29
&Вместо("СоздатьДокументыВыводаКМдляРТУвЕАЭС")
Процедура гф_СоздатьДокументыВыводаКМдляРТУвЕАЭС()
	...
	Попытка
		...
		гф_ТрансграничныеПродажиВызовСервера.СозданиеДокументовВыводаКМ();
...
// #wortmann {
// Метод регламентного задания "Подготовить документы Вывода КМ к передаче ИСМП"
// Галфинд Домнышева 2023/12/29
&Вместо("ПодготовитьДокументыВыводаКМКПередачеИСМП")
Процедура гф_ПодготовитьДокументыВыводаКМКПередачеИСМП()
	
	Попытка
		СтрокаСообщения = "Запуск регл. задания.";
		ЗаписьЖурналаРегистрации("РегламентированноеЗаданиеПодготовитьДокументыВыводаКМКПередачеИСМП",
		УровеньЖурналаРегистрации.Информация,
		Метаданные.РегламентныеЗадания.гф_ПодготовитьДокументыВыводаКМКПередачеИСМП,
		,СтрокаСообщения);
		
		гф_ТрансграничныеПродажиВызовСервера.ПодготовитьДокументыВыводаКМКПередачеИСМП();
...
Процедура ЗаказНаЭмиссиюКМИзменитьИдентификаторПроизводственногоЗаказа()
	// СадомцевСА 07.02.2024 Регламентное задание "Подготовить документы Заказ на эмиссию КМ к передаче ИСМП"
	// Внес изменения: перед выполнение "основного" алгоритма для Заказов на эмиссию КМ
	// 1) изменяем реквизит документа ИдентификаторПроизводственногоЗаказа (поле Идентификатор заказа:)
	// 2) изменяем Статус документа в регистре СтатусыДокументовИСМП
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eec0dcf39c8d9c
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|Таблица.Ссылка КАК Ссылка
	...
	|И НЕ Таблица.ПометкаУдаления
	|И Статусы.ДальнейшееДействие1 = ЗНАЧЕНИЕ(Перечисление.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеКодыМаркировки)
	|И Статусы.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.Ошибка)
	|И ОчередьСообщенийИСМП.Сообщение ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	Пока Выборка.Следующий() Цикл
		ДокОбъект = Выборка.Ссылка.ПолучитьОбъект();
		ДокОбъект.ИдентификаторПроизводственногоЗаказа = "";
		...
		Если НаборЗаписей.Количество() = 1 Тогда
			...
			Запись.Статус = Перечисления.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.Черновик;
			Запись.ДальнейшееДействие1 = Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеКодыМаркировки;
			Запись.ДальнейшееДействие2 = Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.НеТребуется;
			Запись.ДальнейшееДействие3 = Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.НеТребуется;
		КонецЕсли;
...
// #wortmann {
// Метод регламентного задания "Загрузка картинок номенклатуры "
// Галфинд Sakovich 2024/03/22
&Вместо("ЗагрузкаКартинокНоменклатуры")
Процедура гф_ЗагрузкаКартинокНоменклатуры()
	Попытка
		СтрокаСообщения = "Запуск регл. задания.";
		ЗаписьЖурналаРегистрации("РегламентноеЗадание.ЗагрузкаКартинокНоменклатуры",
		УровеньЖурналаРегистрации.Информация,
		Метаданные.РегламентныеЗадания.гф_ЗагрузкаКартинокНоменклатуры, ,СтрокаСообщения);
		
		ОбработкаЗапуска = Обработки.гф_ЗагрузкаКартинок.Создать();
		ОбработкаЗапуска.ЗагрузкаКартинокНоменклатурыFTPСервера();
...
Функция ПодготовитьТЗДокументовПБЗ()
	Запрос = Новый Запрос;
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	Статусы.Основание КАК Основание,
	|	Статусы.СтатусОформления КАК СтатусОформления,
	|	Статусы.Архивный КАК Архивный,
	|	Статусы.Контрагент КАК Контрагент,
	|	Статусы.Ответственный КАК Ответственный,
	|	Статусы.Дата КАК Дата,
	|	Статусы.Номер КАК Номер,
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА МаркировкаТоваровИСМП.Ссылка ЕСТЬ NULL
	|				ТОГДА ЛОЖЬ
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ) КАК ЕстьМаркировка
	|ИЗ
	...
	|		ПО (Статусы.Основание = МаркировкаТоваровИСМП.ДокументОснование)
	|			И (НЕ МаркировкаТоваровИСМП.ПометкаУдаления)
	|ГДЕ
	|	Статусы.Основание ССЫЛКА Документ.ПроизводствоБезЗаказа
	|	И Статусы.Основание.Проведен
	|	И Статусы.СтатусОформления = ЗНАЧЕНИЕ(Перечисление.СтатусыОформленияДокументовГосИС.НеОформлено)
	|	И Статусы.Документ = ЗНАЧЕНИЕ(Документ.МаркировкаТоваровИСМП.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	Статусы.Документ,
	|	Статусы.Основание,
	|	Статусы.СтатусОформления,
	|	Статусы.Архивный,
	|	Статусы.Контрагент,
	|	Статусы.Ответственный,
	|	Статусы.Дата,
	|	Статусы.Номер
	|
	|ИМЕЮЩИЕ
...
&Вместо("бф_ЗаполнениеВГХ")
Процедура гф_бф_ЗаполнениеВГХ()
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|	бф_СредниеВГХпоВидамТоваров.Вес КАК Вес,
		|	бф_СредниеВГХпоВидамТоваров.ДлинаКороба КАК ДлинаКороба,
		|	бф_СредниеВГХпоВидамТоваров.ШиринаКороба КАК ШиринаКороба,
		|	бф_СредниеВГХпоВидамТоваров.ВысотаКороба КАК ВысотаКороба
		|ИЗ
		|	Справочник.ВариантыКомплектацииНоменклатуры КАК ВариантыКомплектацииНоменклатуры
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.бф_СредниеВГХпоВидамТоваров КАК бф_СредниеВГХпоВидамТоваров
		|		ПО ВариантыКомплектацииНоменклатуры.Владелец.НаименованиеПолное = бф_СредниеВГХпоВидамТоваров.ВидТовара
		|			И ВариантыКомплектацииНоменклатуры.КоличествоУпаковок = бф_СредниеВГХпоВидамТоваров.КоличествоПар
		|			И (ВариантыКомплектацииНоменклатуры.гф_ВесКороба = 0
		|				ИЛИ ВариантыКомплектацииНоменклатуры.гф_ОбъемКороба = 0
		|				ИЛИ ВариантыКомплектацииНоменклатуры.гф_ДлинаКороба = 0
		|				ИЛИ ВариантыКомплектацииНоменклатуры.гф_ШиринаКороба = 0
		|				ИЛИ ВариантыКомплектацииНоменклатуры.гф_ВысотаКороба = 0)";
	...
	СоответствиеПолей.Вставить("гф_ВесКороба",		"Вес");
	СоответствиеПолей.Вставить("гф_ДлинаКороба",	"ДлинаКороба");
	СоответствиеПолей.Вставить("гф_ШиринаКороба",	"ШиринаКороба");
	СоответствиеПолей.Вставить("гф_ВысотаКороба",	"ВысотаКороба"); 
	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		СпрОбъект = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		Для каждого Поле Из СоответствиеПолей Цикл
			Если СпрОбъект[Поле.Ключ] = 0 Тогда
				СпрОбъект[Поле.Ключ] = ВыборкаДетальныеЗаписи[Поле.Значение];
			КонецЕсли;
		КонецЦикла;
		СпрОбъект.гф_ОбъемКороба = СпрОбъект.гф_ДлинаКороба * СпрОбъект.гф_ШиринаКороба * СпрОбъект.гф_ВысотаКороба * pow(10, -9);
