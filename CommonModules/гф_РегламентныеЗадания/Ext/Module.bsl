...
Функция ПодготовитьМассивДокументовЗаказНаЭмиссиюКМ()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
               ...
               |ГДЕ
			   // Галфинд СадомцевСА 07.08.2023 Исправил "ошибки" СонарКуба: перенес "условие с ИЛИ" вниз текста запроса
			   //|	(Таблица.ДокументОснование.Проведен
			   //|	 ИЛИ Таблица.ДокументОснование ССЫЛКА Документ.ПересортицаТоваров)
			   ...
			   |	Статусы.ДальнейшееДействие1 = ЗНАЧЕНИЕ(Перечисление.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеКодыМаркировки)
               |	И ОчередьСообщенийИСМП.Сообщение ЕСТЬ NULL
			   |	И (Таблица.ДокументОснование.Проведен
			   |	 ИЛИ Таблица.ДокументОснование ССЫЛКА Документ.ПересортицаТоваров)";
	//           |	И Таблица.Номер = &Номер";
	//Запрос.УстановитьПараметр("Номер", "0100-001083");
	//Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	//|	Таблица.Ссылка КАК Ссылка,
	//|	Таблица.ПометкаУдаления КАК ПометкаУдаления,
	//|	Таблица.Номер КАК Номер,
...
Функция ПодготовитьТЗДокументовПТУ()
	...
	//ТЗ.Свернуть("ДокументПоступления");
	//Возврат ТЗ.ВыгрузитьКолонку("ДокументПоступления");
	Возврат ТЗ;
КонецФункции
...
Процедура гф_ЗагрузитьФайлыИзWMS()
	...
	Попытка
		ЗаписьЖурналаРегистрации("РегламентнаяЗагрузкаФайловИзWMS.СтартОбработки", УровеньЖурналаРегистрации.Информация, Метаданные.Обработки.гф_ОбменСWMS);
		ОбработкаЗапуска = Обработки.гф_ОбменСWMS.Создать();
		ОбработкаЗапуска.ЗагрузитьДанные();
		ЗаписьЖурналаРегистрации("РегламентнаяЗагрузкаФайловИзWMS.ЗавершениеОбработки", УровеньЖурналаРегистрации.Информация, Метаданные.Обработки.гф_ОбменСWMS);
	Исключение
		ТекстОшибки = ОписаниеОшибки();
		СтрокаСообщения = "Ошибка при запуске обработки: " + ТекстОшибки;
		ЗаписьЖурналаРегистрации("РегламентнаяЗагрузкаФайловИзWMS.ОшибкаПриЗапуске", УровеньЖурналаРегистрации.Информация, Метаданные.Обработки.гф_ОбменСWMS,,СтрокаСообщения);
	КонецПопытки;
