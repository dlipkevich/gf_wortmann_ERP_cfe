...
Функция СоздатьОбратнуюКорректировкуНазначения(Заказ)
	...
	СкладКоробной = УправлениеСвойствами.ЗначениеСвойства(РеквизитыЗаказа.Склад, "гф_СкладыТоварыВКоробах") = Истина; 
	...
	Иначе
		...
		Запрос.УстановитьПараметр("НазначениеЗаказ", НазначениеЗаказ); 
		...
		|						Т.Ссылка = &Заказ)
		|				И Склад = &Склад
		|				И Назначение = &НазначениеЗаказ) КАК ТоварыНаСкладахОстатки
		|
		|СГРУППИРОВАТЬ ПО
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			Если СкладКоробной И УпакЛисты.Найти(Выборка["гф_IDкороба"]) = Неопределено Тогда
...
Функция B2B_ТекстОтветаОстатков(Портал) Экспорт 
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураОстатки.Вставить("InStock", Выборка.КоличествоОстаток);
