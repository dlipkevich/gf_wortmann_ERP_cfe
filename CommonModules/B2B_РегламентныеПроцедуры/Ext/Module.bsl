...
Процедура гф_B2B_СнятиеРезерва(Портал)
	...
	|		
	|ГДЕ
	|	СтатусыЗаказов.СтатусЗаказа = &СтатусЗаказаНовый
	...
	|	МИНИМУМ(Календарь.Дата) КАК ДатаПроверки
	|ИЗ
	...
	|          	И СтатусыЗаказовСрезПоследних.СтатусЗаказа = &СтатусЗаказаНовый
	|	
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	...
	|			ПО НовыеЗаказы.Период <= Календарь.Дата
	...
	|	
	|ИМЕЮЩИЕ
	|	МИНИМУМ(Календарь.Дата) МЕЖДУ &НачалоПериода И &КонецПериода";
	
	Запрос.УстановитьПараметр("Портал", Портал);
	Запрос.УстановитьПараметр("СтатусЗаказаНовый", Справочники.гф_В2ВСтатусЗаказаКлиента.Новый);
	...
	Запрос.УстановитьПараметр("КонецПериода",  ДатаЗадания);
	Запрос.УстановитьПараметр("НачалоПериода", ДатаЗадания - НастройкаГлубинаАнализаДней * 86400);
	...
	|		
	|ГДЕ
	|	СтатусыЗаказовСрезПоследних.СтатусЗаказа = &СтатусЗаказаНовый
	...
	|	НовыеЗаказы.ЗаказПокупателя КАК ЗаказПокупателя
	|ИЗ
	|	РегистрНакопления.ЗаказыКлиентов.Обороты(&НачалоПериода, , Регистратор, ) КАК ЗаказыКлиентовОбороты
