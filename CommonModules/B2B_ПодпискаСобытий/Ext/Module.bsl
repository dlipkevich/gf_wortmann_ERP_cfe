...
Процедура B2B_ПередЗаписьюЗаказПокупателя(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт 
	...
	Если Источник.ДополнительныеСвойства.Свойство("CheckOrCreateOrder") Тогда
		Возврат;	
	КонецЕсли; 
	
	Если 16 = 32 Тогда // отключена автоматическая отправка изменений заказа на сайт, отправка из обработки ВыгрузкаБУС
	...
	Попытка		
		...
		Если НомерЗаказаССайта <> Неопределено Тогда 
			...
			Портал = B2B_ОбщиеПроцедурыИФункции.ПорталЗаказа(Источник.Ссылка);
		...
		КонецЕсли;				
	Исключение
		Сообщение="Ошибка отправки на сайт изменений заказа № " + СокрЛП(Источник.Номер) + ": " + ОписаниеОшибки();				
