...
Функция гф_СоздатьМаркировкаТоваровИСМП_ОснованиеПБЗ(СтрокаТЗ, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СсылкаПБЗ = СтрокаТЗ["Основание"];
	Если ТипЗнч(СсылкаПБЗ) <> Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
		СтруктураВозврата["ОписаниеОшибки"] = "Не корректно! Тип значения документа производства:"
			+ ТипЗнч(СсылкаПБЗ);
		Возврат СтруктураВозврата;
	КонецЕсли;
	
	МаркировкаТоваровИСМП = Документы.МаркировкаТоваровИСМП.СоздатьДокумент();
	МаркировкаТоваровИСМП.Заполнить(СсылкаПБЗ);
	МаркировкаТоваровИСМП.Дата = ТекущаяДатаСеанса();
	МаркировкаТоваровИСМП.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотПроизводствоРФ;
	МаркировкаТоваровИСМП.Комментарий = "Создан автоматически регл. заданием Маркировка товаров (ввод в оборот) "
		+ ТекущаяДатаСеанса();
	МаркировкаТоваровИСМП.ДатаПроизводства = СтрокаТЗ["Дата"];
	МаркировкаТоваровИСМП.Ответственный = "";

	МаркировкаТоваровИСМП.ШтрихкодыУпаковок.Очистить();
	Запрос = Новый Запрос("
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ШК.Штрихкод КАК ШтрихкодУпаковки
	|ИЗ
	|	Документ.ПроизводствоБезЗаказа.гф_ПродукцияВКоробах КАК ПБЗ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УпаковочныйЛист КАК УЛ
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ШтрихкодыУпаковокТоваров.ВложенныеШтрихкоды КАК ШК
	|			ПО (УЛ.гф_Агрегация = ШК.Ссылка)
	|		ПО (ПБЗ.IDКороба = УЛ.Ссылка)
	|ГДЕ
	|	ПБЗ.Ссылка = &Ссылка
	|	И ШК.Штрихкод ЕСТЬ НЕ NULL");
	Запрос.УстановитьПараметр("Ссылка", СсылкаПБЗ);
	Результат = Запрос.Выполнить();
	Если Результат.Пустой() Тогда
		СтруктураВозврата["ОписаниеОшибки"] = "Не найдены штрихкоды для создания документа маркировки "
			+ "по документу: " + СсылкаПБЗ;
		Возврат СтруктураВозврата;
	КонецЕсли;
	МаркировкаТоваровИСМП.ШтрихкодыУпаковок.Загрузить(Результат.Выгрузить());
	гф_ПроверитьЗаполнениеЗаписать(МаркировкаТоваровИСМП, СтруктураВозврата);
	
	Возврат СтруктураВозврата;

КонецФункции // ^^^ Галфинд \ Sakovich 03.04.2024
...
Процедура ЗаполнитьМаркировку(МаркировкаТоваровИСМП, ДокументПоступления)
	...
	// Шук 15.02.2024 БФ Заполнение Ввода в оборот на основании поступления
	//{
	...
	ТЗ = Документы.ПриобретениеТоваровУслуг.ИспользуемыеКодыМаркировки(ДокументПоступления);  
	//}
...
Процедура гф_ПреобразоватьШтрихкодыТСДВBase64(ШтрихкодыТСД)
	...
	Для Каждого ЭлементМассива Из ШтрихкодыТСД Цикл
		...
		гф_ПреобразоватьШтрихкодыТСДВBase64_Продолжение(ЭлементМассива, ЕстьСвойствоФорматBase64, ФорматBase64);
		// СадомцевСА 27.04.2024 Исправил ошибки СонарКуба
...
Процедура гф_ПреобразоватьШтрихкодыТСДВBase64_Продолжение(ЭлементМассива, ЕстьСвойствоФорматBase64, ФорматBase64)
	Если Не ФорматBase64 Тогда
		Если ЗначениеЗаполнено(ЭлементМассива.Штрихкод) Тогда
			ЭлементМассива.Штрихкод = ШтрихкодированиеИСКлиентСервер.ШтрихкодВBase64(ЭлементМассива.Штрихкод);
			Если ЕстьСвойствоФорматBase64 Тогда
...
Процедура гф_ПроверитьЗаполнениеЗаписать(МаркировкаТоваровИСМП, СтруктураВозврата)
	...
	Иначе
		...
		// Галфинд СадомцевСА 27.04.2024 Исправил "ошибки" СонарКуба
		ЗаполнитьСтрокиТЧ(МаркировкаТоваровИСМП, МассивУдалитьСтроки, РезультатПроверки);
...
//Процедура ДобавитьКомментарий(МаркировкаТоваровИСМП, ДокументПоступления, НайденнаяСтрока)
//	// СадомцевСА 16.02.2024 Реализовал добавление комментария: КМ <> относится к другой поставке <номер поставки>
//	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eecb3ac16153d5
//	Если ТипЗнч(НайденнаяСтрока.Пересортица) <> Тип("ДокументСсылка.ПересортицаТоваров") Тогда
//		Возврат;
//	КонецЕсли;
//	Если Не ЗначениеЗаполнено(НайденнаяСтрока.Пересортица.гф_IDКороба) Тогда
//		Возврат;
//	КонецЕсли;
//	Если НайденнаяСтрока.Пересортица.гф_IDКороба.гф_Поставка <> ДокументПоступления Тогда
//		МаркировкаТоваровИСМП.Комментарий = МаркировкаТоваровИСМП.Комментарий + Символы.ПС +
//			"КМ:" + НайденнаяСтрока.ЗначениеШтрихкода + " относится к другой поставке:" +
//			НайденнаяСтрока.Пересортица.гф_IDКороба.гф_Поставка;
//	КонецЕсли;
//КонецПроцедуры

Процедура ЗаполнитьСтрокиТЧ(МаркировкаТоваровИСМП, МассивУдалитьСтроки, РезультатПроверки)
	Для Каждого СтрокаСРасхожнением Из РезультатПроверки.СтрокиСРасхождением Цикл
		Если СтрокаСРасхожнением.НомерСтроки > МаркировкаТоваровИСМП.Товары.Количество() Тогда
			Продолжить;
		КонецЕсли;
