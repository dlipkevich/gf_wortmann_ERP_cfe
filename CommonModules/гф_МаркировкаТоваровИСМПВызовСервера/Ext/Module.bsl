...
Процедура ЗаполнитьМаркировку(МаркировкаТоваровИСМП, ДокументПоступления)
	...
	Для Каждого Пересортица Из МассивПересортица Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			Индекс = Индекс + 1;
			// СадомцевСА 16.02.2024 Регламентное задание "Маркировка товаров (ввод в оборот)"
			// Реализовал добавление комментария: КМ <> относится к другой поставке <номер поставки>
			// при создании нового документа Маркировка товаров ИС МП
			// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eecb3ac16153d5
			ДобавитьКомментарий(МаркировкаТоваровИСМП, ДокументПоступления, НайденнаяСтрока);
		КонецЦикла;
	КонецЦикла;
