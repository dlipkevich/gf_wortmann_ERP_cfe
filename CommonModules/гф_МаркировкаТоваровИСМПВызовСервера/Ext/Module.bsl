Функция гф_СоздатьМаркировкаТоваровИСМП(СтрокаТЗ, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(СтрокаТЗ.ДокументПоступления) <> Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
		СтруктураВозврата["ОписаниеОшибки"] = "Не корректно! Тип значения документа поступления:" + ТипЗнч(СтрокаТЗ.ДокументПоступления);
		Возврат СтруктураВозврата;
	КонецЕсли;
	...
	//Реализовал новый алгорит заполнения нового документа Макрировка товаров ИС МП регл заданием "Маркировка товаров (ввод в оборот)"
...
Процедура ЗаполнитьМаркировку(МаркировкаТоваровИСМП, ДокументПоступления)
	//1) Необходимо по каждому упаковочному листу в ПТиУ - смотреть КОд упаковки в справочнике "Штрихкоды товаров и упаковок"
	...
	//смотреть регистратор - пересортица - искать эту пересортицу в РС "Пул кодов маркировки" и подставлять за место старого.
	...
	ПараметрыСканирования = ШтрихкодированиеИС.ПараметрыСканирования(МаркировкаТоваровИСМП, Неопределено, МаркировкаТоваровИСМП.ВидПродукции); 
	...
	Результат = ГрупповаяОбработкаШтрихкодовИС.ОбработатьПолученныеДанныеТСДВДокументе(СтруктураДокумента, Штрихкоды, ПараметрыСканирования);

	ПараметрыУказанияСерий = ИнтеграцияИС.ПараметрыУказанияСерийФормыОбъекта(МаркировкаТоваровИСМП, Документы.МаркировкаТоваровИСМП);
	
	гф_ОбработатьДобавленныеИзмененныеСтроки(Результат.ДобавленныеСтроки, Результат.ИзмененныеСтроки, СтруктураДокумента, ПараметрыУказанияСерий);
...
Процедура гф_ОбработатьДобавленныеИзмененныеСтроки(ДобавленныеСтроки, ИзмененныеСтроки, СтруктураДокумента, ПараметрыУказанияСерий)
...
Процедура гф_ПодключитьОбработкуКодовМаркировки(Форма, ЕстьТабличнаяЧастьШтрихкодыУпаковок=Истина, ДополнительныеКлючиШтрихкодовУпаковок = "")
...
Функция ПодготовитьСтруктуруМаркировкаТоваровИСМП(МаркировкаТоваровИСМП)
	...
	СтруктураРеквизитов.Вставить("Объект", СтруктураРеквизитов);
