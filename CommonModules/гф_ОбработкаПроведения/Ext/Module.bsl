...
// #wortmann {
// #Русская зима. Формирование заказов поставщикам в коробах.
// Галфинд Sakovich 2023/04/05
// Формирование движений типовых документов по рН гф_ВариантыКомплектацииКЗаказуПоставщикам
// см. ПодпискиНаСобытия.гф_ОбработкаПроведенияВариантыКомплектацииКЗаказуПоставщикам
// Параметры:
// Источник - ДокументОбъект - регистраторы регистра
// Отказ - Булево - отказ от записи документа
// РежимПроведения - ДокументОбъект.РежимПроведенияДокумента 
Процедура гф_ОбработкаПроведенияВариантыКомплектацииКЗаказуПоставщикам(Источник, Отказ, РежимПроведения) Экспорт

	Движения = Источник.Движения.гф_ВариантыКомплектацииКЗаказуПоставщикам;
	Движения.Записать();
	
	Движения.Записывать = Ложь;
	
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаказКлиента") Тогда
		ВариантыКомплектацииКЗаказуПоставщикамЗаказКлиента(Источник, Движения);
	КонецЕсли;
	
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаказПоставщику") Тогда
		ВариантыКомплектацииКЗаказуПоставщикамЗаказПоставщику(Источник, Движения);
	КонецЕсли;
...
Процедура ВариантыКомплектацииКЗаказуПоставщикамЗаказКлиента(Источник, Движения)
	
	Если Источник.гф_ТоварыВКоробах.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	|	Шапка.Ссылка КАК ДокументУчетаПотребности,
	|	СУММА(ВЫБОР
...
Процедура ВариантыКомплектацииКЗаказуПоставщикамЗаказПоставщику(Источник, Движения)
	
	Если Источник.гф_ПродукцияВКоробах.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	Шапка.Организация КАК Организация,
	|	Шапка.Контрагент КАК Поставщик,
	|	Шапка.Склад КАК Склад,
	|	ПродукцияВКоробах.ВариантКомплектации КАК ВариантКомплектации,
	|	Шапка.Ссылка КАК ДокументУчетаПотребности,
	|	СУММА(ВЫБОР
