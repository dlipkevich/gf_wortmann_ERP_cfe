...
Процедура гф_АгрегироватьШтрихкодыУпаковокНоменклатуры(Ссылка) Экспорт
	...
	Для каждого стрТЗ Из вспТЗ Цикл
		...
		КонецЕсли;
	КонецЦикла;
	вспТЗ = ТЗ.Скопировать(, "НайденныйАгрегат");
	вспТЗ.Свернуть("НайденныйАгрегат");
	Для каждого стрТЗ Из вспТЗ Цикл
		строкиДляЗаполнения = ТЗ.НайтиСтроки(Новый Структура("НайденныйАгрегат", стрТЗ["НайденныйАгрегат"]));
...
Функция гф_ПолучитьСкладИзДокументаОснования(Ссылка) Экспорт
	...
	Если ИмяМетаданных = "ВходящаяТранспортнаяОперацияВЕТИС" 
		...
		ТорговыйОбъект = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ТорговыйОбъект");
		Если ТипЗнч(ТорговыйОбъект) = Тип("СправочникСсылка.Склады") Тогда
