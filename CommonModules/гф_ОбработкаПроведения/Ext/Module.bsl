...
Процедура ДвиженияКодовМаркировкиРеализацияТоваровУслуг(Источник, Движения, Отказ)
	...
	|	ДвиженияДокумента.КМ КАК КМ,
	|	ДвиженияДокумента.ВидДвижения КАК ВидДвижения,
	|	ВЫБОР
	...
	|	ДанныеДляДвижений.Склад КАК Склад,
	|	ДанныеДляДвижений.КМ КАК КМ,
	|	ЕСТЬNULL(Остатки.КоличествоОстаток, 0) - ДанныеДляДвижений.Количество + ЕСТЬNULL(ДвиженияДокумента.Количество, 0) КАК Нехватка
	|ИЗ
	|	ДанныеДляДвижений КАК ДанныеДляДвижений
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.гф_ДвижениеКодовМаркировкиОрганизации.Остатки(
	|				&Момент,
	...
	|		ПО ДанныеДляДвижений.Организация = ДвиженияДокумента.Организация
	|			И ДанныеДляДвижений.Склад = ДвиженияДокумента.Склад
	|			И ДанныеДляДвижений.КМ = ДвиженияДокумента.КМ
	|ГДЕ
	|	ЕСТЬNULL(Остатки.КоличествоОстаток, 0) - ДанныеДляДвижений.Количество + ЕСТЬNULL(ДвиженияДокумента.Количество, 0) < 0";
	
	тзДвижения = Движения.Выгрузить();
	Запрос.УстановитьПараметр("ДвиженияДокумента", тзДвижения);
	Запрос.УстановитьПараметр("Ссылка", Источник.Ссылка);
	Запрос.УстановитьПараметр("Момент", Источник.МоментВремени());
	ПакетРезультатов = Запрос.ВыполнитьПакетСПромежуточнымиДанными();
	
	РезультатКонтроль = ПакетРезультатов[4];
	Если Не РезультатКонтроль.Пустой() Тогда
		Шаблон = "По организации %1, по складу %2 - нет остатков по списываемому коду маркировки: %3";
		ВыборкаКонтроль = РезультатКонтроль.Выбрать();
		Пока ВыборкаКонтроль.Следующий() Цикл
	...
	РезультатДвижения = ПакетРезультатов[3];
	Таблица = РезультатДвижения.Выгрузить();
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Таблица, тзДвижения);
	тзДвижения.ЗаполнитьЗначения(Истина, "Активность");
	Движения.Загрузить(тзДвижения);
