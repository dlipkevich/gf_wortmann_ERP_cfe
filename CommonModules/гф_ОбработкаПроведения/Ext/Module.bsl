...
Процедура ДвиженияКодовМаркировкиМаркировкаТоваровИСМП(Источник, Движения)
	...
	|	ВЫБОР
	|		КОГДА &СкладКоробной
	|			ТОГДА ЕСТЬNULL(ВложенныеШтрихкоды.Ссылка, ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка))
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)
	|	КОНЕЦ КАК Короб,
	|	ДокТч.КМ КАК КМ,
	|	1 КАК Количество,
...
Процедура ДвиженияКодовМаркировкиПересортицаТоваров(Источник, Движения, Отказ)
	...
	//|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)
	//|	КОНЕЦ КАК Короб,
	|	ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка) КАК Короб,
	...
	//|	ВЫБОР
	//|		КОГДА &СкладКоробной
	//|			ТОГДА ЕСТЬNULL(тчВложенныеШтрихкоды.Ссылка, ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка))
	//|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)
	//|	КОНЕЦ,
...
Процедура ДвиженияКодовМаркировкиРеализацияТоваровУслуг(Источник, Движения, Отказ)
	...
	|	ВЫБОР
	|		КОГДА &СкладКоробной
	|			ТОГДА Штрихкоды.ШтрихкодУпаковки
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)
	|	КОНЕЦ КАК Короб,
	|	ЕСТЬNULL(ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Штрихкод, ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)) КАК КМ,
	|	1 КАК Количество
	|ПОМЕСТИТЬ Данные
	|ИЗ
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ШтрихкодыУпаковокТоваров.ВложенныеШтрихкоды КАК ШтрихкодыУпаковокТоваровВложенныеШтрихкоды
	|			ПО (Штрихкоды.ШтрихкодУпаковки = ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Ссылка)
	|		ПО (Штрихкоды.Ссылка = Реализация.Ссылка)
	|ГДЕ
	|	Реализация.Ссылка = &Ссылка
	|	И Штрихкоды.ШтрихкодУпаковки.ТипУпаковки = ЗНАЧЕНИЕ(Перечисление.ТипыУпаковок.МультитоварнаяУпаковка)
	|	И ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Штрихкод ЕСТЬ НЕ NULL 
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА &СкладКоробной
	|			ТОГДА ЕСТЬNULL(ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Ссылка, ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка))
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)
	|	КОНЕЦ,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.ШтрихкодыУпаковок КАК Штрихкоды
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ШтрихкодыУпаковокТоваров.ВложенныеШтрихкоды КАК ШтрихкодыУпаковокТоваровВложенныеШтрихкоды
	|			ПО Штрихкоды.ШтрихкодУпаковки = ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Штрихкод
	|		ПО (Штрихкоды.Ссылка = Реализация.Ссылка)
	|ГДЕ
	|	Реализация.Ссылка = &Ссылка
	|	И Штрихкоды.ШтрихкодУпаковки.ТипУпаковки = ЗНАЧЕНИЕ(Перечисление.ТипыУпаковок.МаркированныйТовар)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ДанныеДляДвижений.Короб КАК Короб,
	...
	// ++ Галфинд ВолковЕВ 05.06.2023 Ошибка при заполнении движений КМ
	//ПакетРезультатов = Запрос.ВыполнитьПакет();
	ПакетРезультатов = Запрос.ВыполнитьПакетСПромежуточнымиДанными();
...
Процедура ДвиженияКодовМаркировкиПеремещениеТоваров(Источник, Движения, Отказ)
	...
	|			ТОГДА Штрихкоды.ШтрихкодУпаковки
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)
	|	КОНЕЦ КАК КоробОтправитель,
	|	ВЫБОР
	|		КОГДА &СкладПолучательВКоробах
	|			ТОГДА Штрихкоды.ШтрихкодУпаковки
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)
	|	КОНЕЦ КАК КоробПолучатель,
	|	ЕСТЬNULL(ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Штрихкод, ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)) КАК КМ,
	|	1 КАК Количество
	|ПОМЕСТИТЬ Данные
	|ИЗ
	...
	|	Данные.Регистратор КАК Регистратор,
	|	Данные.Организация КАК Организация,
	|	Данные.СкладОтправитель КАК СкладОтправитель,
	|	Данные.СкладПолучатель КАК СкладПолучатель,
	|	Данные.КоробОтправитель КАК КоробОтправитель,
	|	Данные.КоробПолучатель КАК КоробПолучатель,
	|	Данные.КМ КАК КМ,
	|	Данные.Количество КАК Количество
	|ПОМЕСТИТЬ ДанныеДляДвижений
	|ИЗ
	...
	|	ДанныеДляДвижений.Регистратор КАК Регистратор,
	|	ДанныеДляДвижений.Организация КАК Организация,
	|	ДанныеДляДвижений.СкладОтправитель КАК Склад,
	|	ДанныеДляДвижений.КоробОтправитель КАК Короб,
	|	ДанныеДляДвижений.КМ КАК КМ,
	|	ДанныеДляДвижений.Количество КАК Количество
	|ПОМЕСТИТЬ Движения
	|ИЗ
	...
	|	ДанныеДляДвижений.Регистратор,
	|	ДанныеДляДвижений.Организация,
	|	ДанныеДляДвижений.СкладПолучатель,
	|	ДанныеДляДвижений.КоробПолучатель,
	|	ДанныеДляДвижений.КМ,
	|	ДанныеДляДвижений.Количество
	|ИЗ
	|	ДанныеДляДвижений КАК ДанныеДляДвижений
	...
	|							ДанныеДляДвижений КАК т2)) КАК Остатки
	|		ПО ДанныеДляДвижений.Организация = Остатки.Организация
	|			И ДанныеДляДвижений.СкладОтправитель = Остатки.Склад
	|			И ДанныеДляДвижений.КМ = Остатки.КМ
	|ГДЕ
	|	ЕСТЬNULL(Остатки.КоличествоОстаток, 0) - ДанныеДляДвижений.Количество < 0");
