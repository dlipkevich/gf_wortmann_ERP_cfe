...
Процедура гф_ОбработкаПроведенияДвижениеКодовМаркировкиОрганизации(Источник, Отказ, РежимПроведения) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.ПередачаТоваровХранителю") Тогда
		Движения.Записать();
		Движения.Записывать = Ложь;
		ДвиженияКодовМаркировкиПередачаТоваровХранителю(Источник, Движения, Отказ);
	КонецЕсли;
...
Процедура ДвиженияКодовМаркировкиПеремещениеТоваров(Источник, Движения, Отказ)
	...
	Если Не ПеремещениеДляОбработкиУдаления Тогда
		...
		|ГДЕ
		|	НЕ т.КМ В (ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка), NULL)
		|	
		|	Объединить Все 
		|	
		| ВЫБРАТЬ 
...
// ($)
Процедура ДвиженияКодовМаркировкиПередачаТоваровХранителю(Источник, Движения, Отказ)
	...
	Если ПроводитьСРазукомплектовкой Тогда
		...
		Запрос.Текст = "
		|ВЫБРАТЬ
		|	докПередача.Дата КАК Период,
		|	докПередача.Ссылка КАК Регистратор,
		|	докПередача.Организация КАК Организация,
		|	докПередача.Склад КАК Склад,
		|	докПередача.Контрагент КАК Контрагент,
		|	ПередачаТоваровХранителюгф_ТоварыВКоробах.УпаковочныйЛист КАК УпаковочныйЛист,
		|	докУпаковочныйЛист.гф_Агрегация КАК КМАгрегат,
		|	1 КАК Количество
		|ПОМЕСТИТЬ ДанныеКороба
		|ИЗ
		|	Документ.УпаковочныйЛист КАК докУпаковочныйЛист
		|		ПРАВОЕ СОЕДИНЕНИЕ Документ.ПередачаТоваровХранителю.гф_ТоварыВКоробах КАК ПередачаТоваровХранителюгф_ТоварыВКоробах
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПередачаТоваровХранителю КАК докПередача
		|			ПО (ПередачаТоваровХранителюгф_ТоварыВКоробах.Ссылка = докПередача.Ссылка)
		|		ПО (ПередачаТоваровХранителюгф_ТоварыВКоробах.УпаковочныйЛист = докУпаковочныйЛист.Ссылка)
		|ГДЕ
		|	докПередача.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Иначе
		...
		Запрос.Текст = "
		|ВЫБРАТЬ
		|	Передача.Дата КАК Период,
		|	Передача.Ссылка КАК Регистратор,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПередачаТоваровХранителю.гф_ШтрихкодыУпаковок КАК Штрихкоды
		|		ПО (Штрихкоды.Ссылка = Передача.Ссылка)
		|ГДЕ
		|	Реализация.Ссылка = &Ссылка
		|;
		|
