...
// Параметры:
//  НастройкиFTPСоединения - Структура - Структура параметров FTPСоединения
//
// Возвращаемое значение:
...
Функция гф_ПолучитьСоединение(НастройкиFTPСоединения) Экспорт
	...
	Если ЗначениеЗаполнено(НастройкиFTPСоединения.ПользовательПрокси) Тогда
		...
		ПроксиСервер.Установить(СокрЛП(НастройкиFTPСоединения.ПротоколПрокси), СокрЛП(НастройкиFTPСоединения.АдресПрокси), 
								?(НастройкиFTPСоединения.ПортПрокси = 0, 8080, НастройкиFTPСоединения.ПортПрокси));
	КонецЕсли;
	
	Сервер = СокрЛП(НастройкиFTPСоединения.Сервер);
	Порт = ?(НастройкиFTPСоединения.Порт = 0, 21, НастройкиFTPСоединения.Порт);
	ИмяПользователя = СокрЛП(НастройкиFTPСоединения.Пользователь);
	ПарольПользователя = СокрЛП(НастройкиFTPСоединения.Пароль);
