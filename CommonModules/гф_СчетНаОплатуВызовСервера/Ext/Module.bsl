...
Функция ПолучитьЭтапыОплаты(ПараметрКоманды) Экспорт 
	// ++ Галфинд_ДомнышеваКР_16_04_2024
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eefb42168576fe
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА ЗаказКлиентаЭтапыГрафикаОплаты.СуммаЗалогаЗаТару > 0
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ЭтоЗалогЗаТару,
		|	РеализацияТоваровУслуг.СуммаДокумента КАК СуммаПлатежа
		|ИЗ
		|	Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.ЭтапыГрафикаОплаты КАК ЗаказКлиентаЭтапыГрафикаОплаты
		|		ПО РеализацияТоваровУслуг.ЗаказКлиента = ЗаказКлиентаЭтапыГрафикаОплаты.Ссылка
		|ГДЕ
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЭтапыГрафикаОплаты  = РезультатЗапроса.Выгрузить();
		ЭтапыОплатыХЗ = Новый ХранилищеЗначения(ЭтапыГрафикаОплаты);
		Возврат ЭтапыОплатыХЗ;
	Иначе
		ЭтапыГрафикаОплаты  = ПараметрКоманды.ЗаказКлиента.ЭтапыГрафикаОплаты.Выгрузить();
		ЭтапыОплатыХЗ = Новый ХранилищеЗначения(ЭтапыГрафикаОплаты);
		Возврат ЭтапыОплатыХЗ;
	КонецЕсли;
	// -- Галфинд_ДомнышеваКР_16_04_2024	  
