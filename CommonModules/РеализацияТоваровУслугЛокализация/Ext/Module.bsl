...
Процедура гф_ДобавитьКомандыПечати(КомандыПечати)
	...
	Если НЕ ЭтоПартнер Тогда
		// Универсальный передаточный документ (УПД)(Черновик)
		КомандаПечати = КомандыПечати.Добавить();	// СтрокаТаблицыЗначений
		КомандаПечати.МенеджерПечати = "Обработка.гф_УПДЧерновик.СформироватьПечатнуюФормуУПДЧерновик";
		...
		// Универсальный передаточный документ (УПД)(Свернутый)
		КомандаПечати = КомандыПечати.Добавить();	// СтрокаТаблицыЗначений
		КомандаПечати.МенеджерПечати = "Обработка.гф_УПДЧерновик.СформироватьПечатнуюФормуУПДСвернутый";
		КомандаПечати.Идентификатор = "УПД(Свернутый)";
		КомандаПечати.Представление = НСтр("ru = 'Универсальный передаточный документ (УПД)(Свернутый)';
		|en = 'Universal transfer document (UTD)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ДополнительныеПараметры.Вставить("ВыводитьУслуги", Истина);
		...
		КомандаПечати = КомандыПечати.Добавить();	// СтрокаТаблицыЗначений
		КомандаПечати.МенеджерПечати = "Обработка.гф_УПДЧерновик.СформироватьПечатнуюФормуЗаявкиНаОтгрузкуСНГ";
		КомандаПечати.Идентификатор = "Заявка на отправку груза в СНГ";
		КомандаПечати.Представление = НСтр("ru = 'Заявка на отправку груза в СНГ(Вортманн)';
		|en = 'Application for cargo shipment to the CIS(Wortnann)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ДополнительныеПараметры.Вставить("ВыводитьУслуги", Истина);
		
		// Заявка на экспорт
		КомандаПечати = КомандыПечати.Добавить();	// СтрокаТаблицыЗначений
		КомандаПечати.МенеджерПечати = "Обработка.гф_УПДЧерновик.СформироватьПечатнуюФормуЗаявкаНаЭкспрот";
		КомандаПечати.Идентификатор = "ЗаявкаНаЭкспорт";
		КомандаПечати.Представление = НСтр("ru = 'Заявка на экспорт';
		|en = 'Application for export'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ДополнительныеПараметры.Вставить("ВыводитьУслуги", Истина);
		
		// Заявка на расход реализация Склад
		КомандаПечати = КомандыПечати.Добавить();	// СтрокаТаблицыЗначений
		КомандаПечати.МенеджерПечати = "Обработка.гф_УПДЧерновик.СформироватьПечатнуюФормуЗаявкаНаРасходРеализацииСклад";
		КомандаПечати.Идентификатор = "ЗаявкаНаРеализациюСклад";
		КомандаПечати.Представление = НСтр("ru = 'Заявка на расход реализация Склад';
