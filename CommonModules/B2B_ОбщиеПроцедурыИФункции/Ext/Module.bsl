...
Функция СформироватьВременныеТаблицыПоЗаказам(МассивДанных) Экспорт
	...
	|
	|	Строка(ЗаказПокупателяТовары.Номенклатура.Артикул)
	| 	+
	|	ВЫБОР
...
Функция ПолучитьИдТовараДляСайта(Номенклатура, Характеристика = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		Если ЗначениеЗаполнено(АртикулНоменклатуры) Тогда
			...
			Если ЗначениеЗаполнено(СтарыйЭлемент) Тогда
				ИдНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтарыйЭлемент, "GUID");
			КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ИдНоменклатуры) Тогда
	...
	Если ЗначениеЗаполнено(СтараяХарактеристика) Тогда
		ИдХарактеристики = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтараяХарактеристика, "GUID");
	КонецЕсли;
		
	Если Не ЗначениеЗаполнено(ИдХарактеристики) И ЗначениеЗаполнено(Характеристика) Тогда
		ИдХарактеристики = Строка(Характеристика.УникальныйИдентификатор());
	КонецЕсли; 
	
	Возврат ИдТовараПоИдНоменклатурыИХарактеристики(ИдНоменклатуры, ИдХарактеристики);
	
КонецФункции

Функция НовыйИдТовараДляСайта(Номенклатура, Характеристика = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		ИдНоменклатуры = Строка(Номенклатура.УникальныйИдентификатор());
	Иначе
		ИдНоменклатуры = "";
	КонецЕсли;
...
Функция ПолучитьИдТовараИзОстатков(Выборка) Экспорт
	
	ИдТовара = "";
	
	ИдНоменклатуры = Выборка["ИдентификаторНоменклатуры"]; 
	Если Не ЗначениеЗаполнено(ИдНоменклатуры) Тогда
		Возврат НовыйИдТовараДляСайта(Выборка["Номенклатура"], Выборка["Характеристика"]);
	КонецЕсли;
	
	ИдХарактеристики = Выборка["ИдентификаторХарактеристики"]; 
	Если Не ЗначениеЗаполнено(ИдХарактеристики) И ЗначениеЗаполнено(Выборка["Характеристика"]) Тогда
		ИдХарактеристики = Строка(Выборка["Характеристика"].УникальныйИдентификатор());
	КонецЕсли;
...
Функция ИдТовараПоИдНоменклатурыИХарактеристики(ИдНоменклатуры, ИдХарактеристики)
	
	Если ЗначениеЗаполнено(ИдХарактеристики) Тогда
		ИдТовара = ИдНоменклатуры + "#" + ИдХарактеристики;
	Иначе
		ИдТовара = ИдНоменклатуры + "#" + ИдНоменклатуры;
	КонецЕсли;
...
Функция ТекстЗапросаОстатковПоСпискуНоменклатуры() Экспорт
	...
	|			,
	|			КМ В (ВЫБРАТЬ Т.КМ ИЗ ВтШтрихкоды КАК Т)
	|				И Склад В
	|					(ВЫБРАТЬ
	|						СкладыОстатков.Склад
	|					ИЗ
	|						РегистрСведений.B2B_СоответствиеСкладВидНоменклатуры КАК СкладыОстатков)) КАК ДвижениеКодовМаркировкиОрганизацииОстатки
	|
	|СГРУППИРОВАТЬ ПО
	|	ДвижениеКодовМаркировкиОрганизацииОстатки.Склад,
	|	ДвижениеКодовМаркировкиОрганизацииОстатки.КМ,
	|	ДвижениеКодовМаркировкиОрганизацииОстатки.КМ.Номенклатура,
	|	ДвижениеКодовМаркировкиОрганизацииОстатки.КМ.Характеристика
	|
	|ИМЕЮЩИЕ
	...
	|ПОМЕСТИТЬ ВтОстаткиБезКоробов
	|ИЗ
	|	РегистрНакопления.ТоварыНаСкладах.Остатки(, &ВыбратьОстаткиБезКоробов  
	...
	|	СправочникВариантов.Владелец.Артикул КАК АртикулНоменклатуры,
	|	
	...
	|	
	|	ЕСТЬNULL(Остатки.КоличествоКоробов, 0) КАК КоличествоОстаток
	|
	|ПОМЕСТИТЬ ВтПолныеОстатки
	|ИЗ
	...
	|
	|ВЫБРАТЬ
	|	Характеристики.Номенклатура КАК Номенклатура,
	|	Характеристики.Номенклатура.Артикул КАК АртикулНоменклатуры,
	|	
	|	Характеристики.Характеристика КАК Характеристика,
	|	
	|	Характеристики.Номенклатура.Артикул  + ВЫБОР
	|		КОГДА Характеристики.Характеристика <> ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|			ТОГДА ""/"" + Характеристики.Характеристика.Наименование
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК ПолныйАртикул,
	|	
	|	ЕСТЬNULL(ОстаткиБезКоробов.КоличествоОстаток, 0) КАК КоличествоОстаток
	|
	|ИЗ
	|	ВтХарактеристикиНоменклатуры КАК Характеристики
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтОстатки КАК Остатки
	|		ПО Характеристики.Номенклатура   = Остатки.Номенклатура
	|   	 И Характеристики.Характеристика = Остатки.Характеристика
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтОстаткиБезКоробов КАК ОстаткиБезКоробов
	|		ПО Характеристики.Номенклатура   = ОстаткиБезКоробов.Номенклатура
	|   	 И Характеристики.Характеристика = ОстаткиБезКоробов.Характеристика
	|
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	B2B_СЭ_ТипНоменклатуры.Артикул КАК Артикул,
	|	МАКСИМУМ(B2B_СЭ_ТипНоменклатуры.GUID) КАК Идентификатор
	|ПОМЕСТИТЬ ВтТипыНоменклатуры
	|ИЗ
	|	Справочник.B2B_СЭ_ТипНоменклатуры КАК B2B_СЭ_ТипНоменклатуры
	|ГДЕ
	|	B2B_СЭ_ТипНоменклатуры.Артикул В (ВЫБРАТЬ Т.АртикулНоменклатуры ИЗ ВтПолныеОстатки КАК Т)
	|СГРУППИРОВАТЬ ПО
	|	B2B_СЭ_ТипНоменклатуры.Артикул
	|; 
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	B2B_СправочникНоменклатураУПП.Артикул КАК Артикул,
	|	МАКСИМУМ(B2B_СправочникНоменклатураУПП.GUID) КАК Идентификатор
	|ПОМЕСТИТЬ ВтНоменклатураУПП
	|ИЗ
	|	Справочник.B2B_СправочникНоменклатураУПП КАК B2B_СправочникНоменклатураУПП
	|ГДЕ
	|	B2B_СправочникНоменклатураУПП.Артикул В (ВЫБРАТЬ Т.ПолныйАртикул ИЗ ВтПолныеОстатки КАК Т)
	|СГРУППИРОВАТЬ ПО
	|	B2B_СправочникНоменклатураУПП.Артикул
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Остатки.ПолныйАртикул,
	|	
	|	ЕСТЬNULL(ТипыНоменклатуры.Идентификатор, """") КАК ИдентификаторНоменклатуры,
	|	ЕСТЬNULL(НоменклатураУПП.Идентификатор, """")  КАК ИдентификаторХарактеристики,
	|	
	|	Остатки.КоличествоОстаток КАК КоличествоОстаток
	|ИЗ
	|	ВтПолныеОстатки КАК Остатки
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтТипыНоменклатуры КАК ТипыНоменклатуры
	|			ПО Остатки.АртикулНоменклатуры = ТипыНоменклатуры.Артикул
	|		 	 И Остатки.АртикулНоменклатуры <> """" 
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтНоменклатураУПП КАК НоменклатураУПП
	|			ПО Остатки.ПолныйАртикул = НоменклатураУПП.Артикул
	|		 	 И Остатки.ПолныйАртикул <> """"";
...
Процедура ВыгрузитьОстатки(НастройкиПодключения, МассивНоменклатуры) Экспорт
	...
	УстановитьПараметрыЗапросаОстатков(Запрос, МассивНоменклатуры);
	...
	Пока Выборка.Следующий() Цикл
		...
		ИдТовара = ПолучитьИдТовараИзОстатков(Выборка);
		СтруктураОстатки.Вставить("XML_ID", ИдТовара);
		
		СтруктураОстатки.Вставить("QUANTITY", Выборка.КоличествоОстаток);
...
Функция СкладыБезКоробов(МассивСкладов = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(МассивСкладов) Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РегистрСоответствия.Склад
		|ИЗ
		|	РегистрСведений.B2B_СоответствиеСкладВидНоменклатуры КАК РегистрСоответствия";
	...
	|	ЕСТЬNULL(СкладыДополнительныеРеквизиты.Значение, ЛОЖЬ) КАК ТоварыВКоробах
	|ИЗ
	...
	|			ПО СправочникСклады.Ссылка = СкладыДополнительныеРеквизиты.Ссылка
	|			И (СкладыДополнительныеРеквизиты.Свойство = &ДопРеквизитТоварыВКоробах)
	|
	|ГДЕ
	|  	СправочникСклады.Ссылка В (&МассивСкладов)
	|	И ЕСТЬNULL(СкладыДополнительныеРеквизиты.Значение, ЛОЖЬ) = ЛОЖЬ";
	Запрос.УстановитьПараметр("МассивСкладов", МассивСкладов);	
	Запрос.УстановитьПараметр("ДопРеквизитТоварыВКоробах", ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту(
		"ИдентификаторДляФормул", "гф_СкладыТоварыВКоробах"));	
	
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Склад");
	
КонецФункции

Функция СостоянияКоробовДляРезервирования() Экспорт
	
	Результат = Новый Массив;
	
	Результат.Добавить(Справочники.гф_СостянияКоробов.ПолныйКомплект);	
	Результат.Добавить(Справочники.гф_СостянияКоробов.НеПолный);	
	Результат.Добавить(Справочники.гф_СостянияКоробов.Пересорт);	
	Результат.Добавить(Справочники.гф_СостянияКоробов.НеПолныйИПересорт);
	
	Возврат Результат;
	
КонецФункции

Процедура УстановитьПараметрыЗапросаОстатков(Запрос, МассивНоменклатуры) Экспорт
	
	Запрос.УстановитьПараметр("МассивНоменклатуры", МассивНоменклатуры);
	Запрос.УстановитьПараметр("СкладыБезКоробов", СкладыБезКоробов());
	Запрос.УстановитьПараметр("ВыбратьОстаткиБезКоробов", Истина);
	Запрос.УстановитьПараметр("СостоянияКоробов", СостоянияКоробовДляРезервирования());
