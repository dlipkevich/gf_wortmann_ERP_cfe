								...
								Пока ТаблицаНоменклатуры.Количество() > 0 Цикл
									...
									Если ДокументОбъект.ТоварыПоРаспоряжениям.Количество() > 0 Тогда    
										...
										Если Параметры.Свойство("гф_УпаковочныйЛист") Тогда
											...
											Для Каждого УПТаблицы Из Параметры.гф_УпаковочныйЛист Цикл
												Для Каждого УП Из УПТаблицы Цикл
													...
													// ДокументОбъект.гф_СуммаНоменклатуры = ДокументОбъект.гф_СуммаНоменклатуры +
													// (УП.ЦенаУпаковочногоЛиста * УП.Скидка / 100 - УП.ЦенаУпаковочногоЛиста) * (-1);
													// ДокументОбъект.гф_СуммаНоменклатуры = ДокументОбъект.гф_СуммаНоменклатуры +
													// (УП.ЦенаУпаковочногоЛиста / 100 - УП.ЦенаУпаковочногоЛиста) * (-1);
													ДокументОбъект.гф_СуммаНоменклатуры = ДокументОбъект.гф_СуммаНоменклатуры + УП.ЦенаУпаковочногоЛиста;
												КонецЦикла;
											КонецЦикла;
											...
											Если Не Отладка Тогда
												// Учитываются скидки рассчитанные в момент отгрузки на основании данных регистра и введенных пользователем
												// в диалоговом окне "Установить скидки и наценки".
												Если Параметры.Свойство("гф_СуммаДляОрдера") Тогда
													
													СуммаНоменклатуры = 0;
													
													Для Каждого СуммаЗаказа Из Параметры.гф_СуммаДляОрдера Цикл
														// ++ Галфинд ВолковЕВ 2024/04/25 Распределение сумм с учетом адреса в заказе и расходном ордере
														// СуммаНоменклатуры = СуммаНоменклатуры + СуммаЗаказа.Сумма;
														// Распределение сумм в соответсвии с номером адреса из заказа при отгрузке на разные адреса по одному клиенту
														Если Параметры.Свойство("гф_ЗаказАдрес")
															И СуммаЗаказа.Свойство("Адрес")
															И Параметры.гф_ЗаказАдрес = СуммаЗаказа.Адрес Тогда
															СуммаНоменклатуры = СуммаНоменклатуры + СуммаЗаказа.Сумма;
														Иначе
															СуммаНоменклатуры = СуммаНоменклатуры + СуммаЗаказа.Сумма;
														КонецЕсли;
													...
													Если СуммаНоменклатуры > 0 Тогда
														ДокументОбъект.гф_СуммаНоменклатуры = СуммаНоменклатуры;
													КонецЕсли;
										...
										ИначеЕсли Параметры.Свойство("гф_Товары") Тогда
											...
											Для Каждого ТаблицаТоваров Из Параметры.гф_Товары Цикл
												Для Каждого Товар Из ТаблицаТоваров Цикл
													ДокументОбъект.гф_СуммаНоменклатуры = ДокументОбъект.гф_СуммаНоменклатуры +
														...
														// * (1 + Товар.СтавкаНДС.Ставка/100);
														* (1 + Товар.СтавкаНДС / 100);
												КонецЦикла;
											КонецЦикла;
										...
										Если ДокументОбъект.Проведен И Не ЕстьОшибка Тогда
											...
											Если Параметры.Свойство("гф_УпаковочныйЛист") Тогда
												...
												Для Каждого ТаблицаТоваров Из Параметры.гф_УпаковочныйЛист Цикл
													Для Каждого УП Из ТаблицаТоваров Цикл
											...
											КонецЕсли;	
										Иначе
											// Типовой механизм по очереди добавляет в ордер номенклатуру из каждого заказа и пробует провести, и если по одному заказу
											// нет остатков то проведение ордера выдает ошибку и убирает из ордера номенклатуру из ордера по такому заказу
											// Для отслеживания таких ошибок в обработке по отгрузке проверяется появление такой ошибке
											Если ЕстьОшибка Тогда
												Параметры.гф_ЕстьОшибкаОстаткаНоменклатуры = Истина;
											КонецЕсли;
										КонецЕсли;
										// } #wortmann
										#КонецВставки
