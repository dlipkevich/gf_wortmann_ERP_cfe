...
&ИзменениеИКонтроль("ОбъединитьОрдераПоТекущуюДату")
Процедура гф_ОбъединитьОрдераПоТекущуюДату(Параметры, ТекущаяДата, ОформленныеОрдера)
					...
					Пока Выборка.СледующийПоЗначениюПоля("Помещение") Цикл
						...
						Если Выборка.Следующий() Тогда
							...
							Попытка
								...
								ТекущийОрдерОбъект = Выборка.Ссылка.ПолучитьОбъект(); // ДокументОбъект.РасходныйОрдерНаТовары
								...
								Если ТекущийОрдерОбъект <> Неопределено Тогда
									...
									Пока Выборка.Следующий() Цикл
										...
										РасходныйОрдерОбъект  = Выборка.Ссылка.ПолучитьОбъект(); // ДокументОбъект -
									...
									//	
									//	ТекущийОрдерОбъект.ДополнительныеСвойства.Вставить("гф_УпаковочныйЛист", Параметры.гф_УпаковочныйЛист);
									//	
									//КонецЕсли;
									//// } #wortmann
									#КонецВставки
								...
								//Если Параметры.Свойство("гф_УпаковочныйЛист") Тогда
								//	
								//	гф_ТекущийОрдерОбъект = Выборка.Ссылка.ПолучитьОбъект();
								//	
								//	Для Каждого УП Из Параметры.гф_УпаковочныйЛист Цикл
								//		гф_УпаковочныйЛистОбъект = УП.Значение.ПолучитьОбъект();
