...
Процедура ИзменитьДанныеУпаковочныйЛистИШтрихкодУпаковки(пNVE, Эмиссия, Пересортица)
	...
	НомерГТД = Агрегат.гф_НомерГТД;// Галфинд_ДомнышеваКР_31_01_2024
	...
	Для каждого стрУбрать Из тзУбрать Цикл
		...
		Для каждого Эл Из мСтрокУЛ Цикл
			...
			КонецЕсли;
		КонецЦикла;
		// ++ Галфинд_ДомнышеваКР_31_01_2024
		Если НомерГТД = Справочники.НомераГТД.ПустаяСсылка() Тогда
			НомерГТД = стрУбрать["спрШтрихкод"].гф_НомерГТД;
		КонецЕсли;
		// -- Галфинд_ДомнышеваКР_31_01_2024
		
		
		// vvv Галфинд \ Sakovich 19.02.2024
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eec9bc4eeab8f7
		// перенесено в конец процедуры
	...
	Для каждого стрДобавить Из тзДобавить Цикл
		...
		Если Не ЗначениеЗаполнено(КМ) 
			...
			// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee224d0352ee32 
			// Галфинд_Домнышева 2023/07/18
			обНовыйШтрихКод["гф_ДокументОснование"] = стрДобавить["гф_ДокументОснование"];
			обНовыйШтрихКод["гф_ПолныйКодМаркировки"] = стрДобавить["гф_ПолныйКодМаркировки"];
			// } #wortmann 
			обНовыйШтрихКод.гф_НомерГТД = НомерГТД; // ++ Галфинд_ДомнышеваКР_31_01_2024
			...
			обНовыйШтрихКод["гф_Статус"] = Перечисления.гф_СтатусыКМ_в_ШК.Новый;
			...
			гф_ЭмиссияКодовМаркировкиВызовСервера.ЗаписатьИсторияСтатусовКМ(
				Период, 
				КМ, 
				Перечисления.гф_СтатусыКМ_в_ШК.Новый,
				"Рабочее место кладовщика");
			// ^^^ Галфинд \ Sakovich 20.07.2023
		// ++ Галфинд_ДомнышеваКР_09_02_2024
		// Если КМ есть, заполняем у него НомерГТД
		Иначе
			ОбъектКМ = КМ.ПолучитьОбъект();
			ОбъектКМ.гф_НомерГТД = НомерГТД; 
			ОбъектКМ.Записать(); 
		// ++ Галфинд_ДомнышеваКР_09_02_2024
		КонецЕсли;
...
Процедура ПроверитьУЛПоВариантуКомплектацииИПроставитьСтатусКороба(обУЛ) Экспорт
	...
	|	ВЫБОР
    // СадомцевСА 25.07.2023 Исправил алгоритм определения Состояния короба в документе Упаковочный лист
	//e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee2a2d552fe312
//1. Если общее количество в УЛ < количества Варианта комплектации, при этом по всем позициям ТЧ Номенклатура+Характеристика количество в УЛ < либо = количества по этой позиции в Варианте комплектации. , то установить Состояние УЛ "Не полный"
	//|		КОГДА Данные.КоличествоУЛ < Данные.КоличествоВК
	|		КОГДА Данные.КоличествоУЛ <= Данные.КоличествоВК
	|				И ДанныеИтоги.ВсегоКоличествоУЛ < ДанныеИтоги.ВсегоКоличествоВК
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК НеПолный,
	|	ВЫБОР
//2. Если количество по какой-то позиции Номенклатура+Характеристика в УЛ > количества по этой позиции в Варианте комплектации (в т.ч. есть Номенклатуры, которых не было в Варианте комплектации), то сверяем общее количество пар в УЛ и Варианте комплектации:
	|		КОГДА Данные.КоличествоУЛ > Данные.КоличествоВК
//2.1. количество в УЛ = Варианту комплектации, то Состояние УЛ = "Пересорт"
	|				И ДанныеИтоги.ВсегоКоличествоУЛ = ДанныеИтоги.ВсегоКоличествоВК
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Пересорт,
	|	ВЫБОР
	|		КОГДА Данные.КоличествоУЛ > Данные.КоличествоВК
//2.2. количество в УЛ НЕ РАВНО Варианту комплектации, то Состояние УЛ = "Не полный и пересорт".	
	|				И НЕ(ДанныеИтоги.ВсегоКоличествоУЛ = ДанныеИтоги.ВсегоКоличествоВК)
	|			ТОГДА ИСТИНА
	//|			ТОГДА ВЫБОР
	...
	|	МАКСИМУМ(ДанныеСвернуто.НеПолныйИПересорт) КАК НеПолныйИПересорт,
	//|	МАКСИМУМ(ДанныеСвернуто.НеПолный) КАК Пересорт,
	|	МАКСИМУМ(ДанныеСвернуто.Пересорт) КАК Пересорт,
...
// Параметр: ПТУ = Неопределено Добавлено Галфинд_ДомнышеваКР_28_11_2023 для вызова из ПТУ
Процедура ОбработкаПересортицы(ПТУ = Неопределено) Экспорт
	// vvv Галфинд \ Sakovich 19.02.2024
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eec9bc4eeab8f7
	ЕстьПраваНаИзменениеОбъектовОбработки = ПроверитьПраваНаОбъектыОбработкаПересортицы();
	Если Не ЕстьПраваНаИзменениеОбъектовОбработки Тогда
		Возврат;
	КонецЕсли;
	// ^^^ Галфинд \ Sakovich 19.02.2024
	...
	// ++ Галфинд_ДомнышеваКР_28_11_2023
	// Для отработки из самого документа ПТУ
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee88366af8f0c9
	Если ПТУ <> Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ВЫРАЗИТЬ(УпаковочныйЛист.гф_Поставка КАК Документ.ПриобретениеТоваровУслуг).Проведен",
		"И ВЫРАЗИТЬ(УпаковочныйЛист.гф_Поставка КАК Документ.ПриобретениеТоваровУслуг) = &ПТУ");
	...
	Пока Выборка.Следующий() Цикл
		...
		// ++ Галфинд СадомцевСА 31.10.2023 Реализовал:
		// После того, как обработка находит пересортицу - и привязанный к нему "Заказ на эмиссию".
		// Нужно, чтобы этот заказ на эмиссию перезаписался.
		// А потом выполнялись все нужные процедуры, зашитые в РЗ
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee77004f47a781
		гф_ЗаписатьСвязанныйЗаказНаЭмиссию(Выборка["Пересортица"]);
		// -- Галфинд СадомцевСА 31.10.2023
		ОбПересортица = Выборка["Пересортица"].ПолучитьОбъект();
		Если ОбПересортица <> Неопределено Тогда 
			
			// vvv Галфинд \ Sakovich 05.02.2024
			//ОбПересортица.Дата = ТекущаяДатаСеанса(); // Галфинд_Домнышева_02_02_2024
			Если ЗначениеЗаполнено(Выборка["ГТД"]) 
				И Выборка["ГТД"].Дата >= ОбПересортица["Дата"] Тогда
					ОбПересортица["Дата"] = Выборка["ГТД"].Дата + 1;
			КонецЕсли;
			// ^^^ Галфинд \ Sakovich 05.02.2024
			
			ЗаполнитьТЧПоГТДПересортица(ОбПересортица);// Галфинд_Домнышева_19_10_2023
			//Шук 21.02.2024 Транзакционность обработки пересортиц
			//{ 
			...
			Попытка
				...
				Если ЗначениеЗаполнено(Выборка["Эмиссия"]) Тогда   
					ИзменитьДанныеУпаковочныйЛистИШтрихкодУпаковки(
					Выборка["ЗначениеШтрихкодаNVE"], 
					Выборка["Эмиссия"], 
					Выборка["Пересортица"]); 
			...
			Исключение
				ОтменитьТранзакцию();
				ТекстОшибки = ОписаниеОшибки();
				ЗаписьЖурналаРегистрации("Обработка пересортицы. Ошибка Проведения, обработки УЛ/Агрегата ", 
										УровеньЖурналаРегистрации.Предупреждение, 
										Метаданные.Документы.ПересортицаТоваров, 
										Выборка["Пересортица"], ТекстОшибки);
				Продолжить;	
			КонецПопытки; 		
			//}
...
Процедура ОбработкаУдаления() Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫРАЗИТЬ(СтатусОтгрузки.Документ КАК Документ.ПриобретениеТоваровУслуг) КАК ПТУ,
	|	МАКСИМУМ(СтатусОтгрузки.Период) КАК Период
	|ПОМЕСТИТЬ ПериодыКрай
	|ИЗ
	|	РегистрСведений.гф_СтатусОтгрузки КАК СтатусОтгрузки
	|
	|СГРУППИРОВАТЬ ПО
	|	ВЫРАЗИТЬ(СтатусОтгрузки.Документ КАК Документ.ПриобретениеТоваровУслуг)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПериодыКрай КАК ПериодыКрай
	|		ПО гф_СтатусОтгрузкиСрезПоследних.Документ = ПериодыКрай.ПТУ
	|			И гф_СтатусОтгрузкиСрезПоследних.Период = ПериодыКрай.Период
	|ГДЕ
	...
	ВыборкаПТУ = Результат[6].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	Пока ВыборкаПТУ.Следующий() Цикл
		...
		Пока ВыборкаУЛ.Следующий() Цикл
			...
			ДокПТУОтработанПолностью = Истина;
			...
			// ++ Галфинд_ДомнышеваКР_15_11_2023
			обПеремещение.Комментарий = "Создано автоматически рег.заданием ""Обработка удаления""";
			обПеремещение.Ответственный = Пользователи.АвторизованныйПользователь();
			// --Галфинд_ДомнышеваКР_15_11_2023
			...
			обПеремещение.ДополнительныеСвойства.Вставить("ИзРЗ_гф_ОбработкаУдаления",Истина);
			// Галфинд \ Shtak  2024/03/11 <=
			...
			// vvv Галфинд \ Sakovich 02.04.2024
				...
				Если ЗначениеЗаполнено(ДопРеквизит) Тогда
					СтрокаДопРеквизита = обПеремещение.ДополнительныеРеквизиты.Найти(ДопРеквизит, "Свойство");
					Если СтрокаДопРеквизита = Неопределено Тогда
						СтрокаДопРеквизита = обПеремещение.ДополнительныеРеквизиты.Добавить();
						СтрокаДопРеквизита.Свойство = ДопРеквизит;
					КонецЕсли;
			...
			Исключение
				ДокПТУОтработанПолностью = Ложь;
				
				Инфо = "Не удалось создать документ ""Перемещение товаров"" на основании документа " + ДокПТУ;
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				Если НЕ СокрЛП(ПодробноеПредставлениеОшибки) = "Непредвиденная ошибка" Тогда
					Инфо = Инфо + "; " + ПодробноеПредставлениеОшибки;
				КонецЕсли;
			...
			Если НужноСоздаватьПриходныйОрдер Тогда
				...
				обПрОрдер.Комментарий = "Создано автоматически рег.заданием ""Обработка удаления"""; // Галфинд_ДомнышеваКР_15_11_2023
			...
			Если НужноСоздаватьРасходныйОрдер Тогда
				...
				обРасхОрдер.Комментарий = "Создано автоматически рег.заданием ""Обработка удаления""";  // Галфинд_ДомнышеваКР_15_11_2023
			...
			Если НужноСоздаватьРасходныйОрдер И ПерезаписыватьРОТ Тогда
				обРасхОрдер_Ссылка = обРасхОрдер.Ссылка;
				обРасхОрдер = обРасхОрдер_Ссылка.ПолучитьОбъект();
			...
			Если НужноСоздаватьПриходныйОрдер И ПерезаписыватьПОТ Тогда
				обПрОрдер_Ссылка = обПрОрдер.Ссылка;
				обПрОрдер = обПрОрдер_Ссылка.ПолучитьОбъект();
			...
			обПеремещение_Ссылка = обПеремещение.Ссылка;
			обПеремещение = обПеремещение_Ссылка.ПолучитьОбъект();
			обПеремещение.ДополнительныеСвойства.Вставить("ИзРЗ_гф_ОбработкаУдаления",Истина);
		...
		КонецЦикла;
		#КонецОбласти
		Если ДокПТУОтработанПолностью Тогда
			ПеревестиСтатусОтработанПТУ(ДокПТУ); // Галфинд_ДомнышеваКР_15_11_2023
		КонецЕсли;
...
// #wortmann { 
// Заполняет пустые поля номера ГТД в ТЧ Товары в док. Пересортица
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee6db7b8301dde
// Галфинд_Домнышева 2032/10/19
//
// Параметры:
//	ОбПересортица - ДокументОбъект.ПересортицаТоваров 
Процедура ЗаполнитьТЧПоГТДПересортица(ОбПересортица)
	...
	Запрос.Текст = 
		...
		|	Документ.ПриобретениеТоваровУслуг.гф_ПродукцияВКоробах КАК ПриобретениеТоваровУслуггф_ПродукцияВКоробах
		|ГДЕ
		|	ПриобретениеТоваровУслуггф_ПродукцияВКоробах.IDКороба = &IDКороба
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТаможеннаяДекларацияИмпортТовары.Ссылка КАК ТДИТ,
		...
		|	ПТУ КАК ПТУ
		|ГДЕ
		|	ТаможеннаяДекларацияИмпортТовары.ДокументПоступления В
		|			(ВЫБРАТЬ
		|				ПТУ.Ссылка
		|			ИЗ
		|				ПТУ КАК ПТУ)
		|	И (ТаможеннаяДекларацияИмпортТовары.Номенклатура, ТаможеннаяДекларацияИмпортТовары.Характеристика) В
		|			(ВЫБРАТЬ
		|				Товары.Номенклатура,
	...
	Запрос.УстановитьПараметр("IDКороба", ОбПересортица.гф_IDКороба);
	Запрос.УстановитьПараметр("Ссылка", ОбПересортица.Ссылка);
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Для каждого Строка Из ОбПересортица.Товары Цикл
...
// ++ Галфинд СадомцевСА 31.10.2023
Процедура гф_ЗаписатьСвязанныйЗаказНаЭмиссию(Пересортица)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	СвязанныеДокументы.Ссылка КАК Ссылка
	|ИЗ
	|	КритерийОтбора.СвязанныеДокументы(&ЗначениеОтбора) КАК СвязанныеДокументы";
	Запрос.УстановитьПараметр("ЗначениеОтбора", Пересортица);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	Пока Выборка.Следующий() Цикл
		Если ТипЗнч(Выборка.Ссылка) = Тип("ДокументСсылка.ЗаказНаЭмиссиюКодовМаркировкиСУЗ") Тогда
...
// #wortmann { 
// Переводим ПТУ в РС гф_СтатусОтгрузки в Отработан 
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee82cd6f02bc42
// Галфинд_Домнышева 2023/11/15
//
// Параметры:
//	ДокПТУ - ДокументСсылка.ПриобретениеТоваровУслуг
Процедура ПеревестиСтатусОтработанПТУ(ДокПТУ)
	
	ДатаС = ТекущаяДатаСеанса();
	Отбор = Новый Структура("Документ", ДокПТУ);
	ТЗСтатусы = РегистрыСведений.гф_СтатусОтгрузки.СрезПоследних(ДатаС, Отбор);
	
	Набор = РегистрыСведений.гф_СтатусОтгрузки.СоздатьНаборЗаписей();
	Набор.Отбор.Период.Установить(ТЗСтатусы[0].Период);
	Набор.Отбор.Документ.Установить(ДокПТУ);
	Набор.Отбор.Отгрузил.Установить(ТЗСтатусы[0].Отгрузил);
	...
	Иначе
		Возврат;
	КонецЕсли;
	СтрокаНабора.Отработан = Истина;
	Набор.Записать();    	
	
КонецПроцедуры// } #wortmann

// vvv Галфинд \ Sakovich 19.02.2024
Функция ПроверитьПраваНаОбъектыОбработкаПересортицы()
	ТекущийПользователь = Пользователи.АвторизованныйПользователь();
	ЕстьПраваНаОбъекты = Истина;
	
	Если ЗначениеЗаполнено(ТекущийПользователь)
		И Не Пользователи.ЭтоПолноправныйПользователь(ТекущийПользователь) Тогда
		
		Шаблон = "У пользователя не хватает прав для изменения объекта ""%1"". Пересортицы обработаны не будут.";
		
		Если Не  ПравоДоступа("Изменение", Метаданные.Документы.ЗаказНаЭмиссиюКодовМаркировкиСУЗ) Тогда
			СтрокаСообщения = СтрШаблон(Шаблон, "Документ.ЗаказНаЭмиссиюКодовМаркировкиСУЗ");
			...
			СтрокаСообщения);
			ЕстьПраваНаОбъекты = Ложь;
		КонецЕсли;
		
		Если Не ПравоДоступа("Изменение", Метаданные.Документы.ПересортицаТоваров) Тогда
			СтрокаСообщения = СтрШаблон(Шаблон, "Документ.ПересортицаТоваров");
			ЗаписьЖурналаРегистрации("РегламентированноеЗаданиеОбработкаПересортицы",
			УровеньЖурналаРегистрации.Информация,
			Метаданные.РегламентныеЗадания.гф_ОбработкаПересортицы, ,
			СтрокаСообщения);
		...
		Если Не ПравоДоступа("Изменение", Метаданные.Документы.УпаковочныйЛист) Тогда
			СтрокаСообщения = СтрШаблон(Шаблон, "Документ.УпаковочныйЛист");
			ЗаписьЖурналаРегистрации("РегламентированноеЗаданиеОбработкаПересортицы",
			УровеньЖурналаРегистрации.Информация,
			Метаданные.РегламентныеЗадания.гф_ОбработкаПересортицы, ,
			СтрокаСообщения);
		...
		Если Не ПравоДоступа("Изменение", Метаданные.РегистрыСведений.гф_ПересортицаУдаляемыеШК) Тогда
			СтрокаСообщения = СтрШаблон(Шаблон, "РегистрСведений.гф_ПересортицаУдаляемыеШК");
			ЗаписьЖурналаРегистрации("РегламентированноеЗаданиеОбработкаПересортицы",
			УровеньЖурналаРегистрации.Информация,
			Метаданные.РегламентныеЗадания.гф_ОбработкаПересортицы, ,
			СтрокаСообщения);
		...
		Если Не ПравоДоступа("Изменение", Метаданные.РегистрыСведений.ПулКодовМаркировкиСУЗ) Тогда
			СтрокаСообщения = СтрШаблон(Шаблон, "РегистрСведений.ПулКодовМаркировкиСУЗ");
			ЗаписьЖурналаРегистрации("РегламентированноеЗаданиеОбработкаПересортицы",
			УровеньЖурналаРегистрации.Информация,
			Метаданные.РегламентныеЗадания.гф_ОбработкаПересортицы, ,
			СтрокаСообщения);
		...
		Если Не ПравоДоступа("Изменение", Метаданные.РегистрыСведений.гф_ИсторияСтатусовКМ) Тогда
			СтрокаСообщения = СтрШаблон(Шаблон, "РегистрСведений.гф_ИсторияСтатусовКМ");
			ЗаписьЖурналаРегистрации("РегламентированноеЗаданиеОбработкаПересортицы",
			УровеньЖурналаРегистрации.Информация,
			Метаданные.РегламентныеЗадания.гф_ОбработкаПересортицы, ,
			СтрокаСообщения);
		...
		Если Не ПравоДоступа("Изменение", Метаданные.Справочники.ШтрихкодыУпаковокТоваров)
			Или Не ПравоДоступа("Добавление", Метаданные.Справочники.ШтрихкодыУпаковокТоваров) Тогда
			СтрокаСообщения = СтрШаблон(Шаблон, "Справочник.ШтрихкодыУпаковокТоваров");
			ЗаписьЖурналаРегистрации("РегламентированноеЗаданиеОбработкаПересортицы",
			УровеньЖурналаРегистрации.Информация,
			Метаданные.РегламентныеЗадания.гф_ОбработкаПересортицы, ,
			СтрокаСообщения);
	...
	Возврат ЕстьПраваНаОбъекты;
КонецФункции // ^^^ Галфинд \ Sakovich 19.02.2024
