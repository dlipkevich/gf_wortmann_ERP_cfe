...
Процедура ОбработатьИсключенияРМКладовщика() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Основание = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка["Ссылка"], "ДокументОснование");
		...
		Если СозданаЗаписьСтатусовИсключений  = Истина Тогда
			ИзменитьДанныеУпаковочныйЛистИШтрихкодУпаковки(пNVE);
			УстановитьПризнакОбработан_ПересортицаУдаляемыеШтрихкоды(Основание);
		КонецЕсли;
	КонецЦикла;
...
Процедура ИзменитьДанныеУпаковочныйЛистИШтрихкодУпаковки(пNVE) Экспорт
	...
	|ГДЕ
	|	УдаляемыеШК.Пересортица.гф_IDКороба = &УЛ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ПересортицаТоваров.гф_IDКороба = &УЛ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Для каждого стрДобавить Из тзДобавить Цикл
		...
		ПараметрыОтбора = Новый Структура("Номенклатура, Характеристика", 
		стрДобавить["Номенклатура"], 
		стрДобавить["Характеристика"]);
		мСтрокУЛ = обУЛ.Товары.НайтиСтроки(ПараметрыОтбора);
		Если мСтрокУЛ.Количество() > 0 Тогда
			мСтрокУЛ[0]["КоличествоУпаковок"] = 
			?(мСтрокУЛ[0]["КоличествоУпаковок"] > 0, мСтрокУЛ[0]["КоличествоУпаковок"] + 1, 0);
			
			мСтрокУЛ[0]["Количество"] = 
			?(мСтрокУЛ[0]["Количество"] > 0, мСтрокУЛ[0]["Количество"] + 1, 0);
		Иначе
...
Процедура УстановитьПризнакОбработан_ПересортицаУдаляемыеШтрихкоды(Пересортица)
