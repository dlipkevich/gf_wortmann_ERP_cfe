...
Процедура гф_ПроверитьКорректностьВозвращаемыхТоваров(Знач ДокументПродажи, ИмяТаблицы, Отказ, ТаблицаПроверяемыеТовары)
	...
	// Галфинд Sakovich 2022/11/29
		...
		СлужебноеНазначениеДляВозврата = _омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначение("СлужебноеНазначениеВозвратТоваров");
		Если СлужебноеНазначениеДляВозврата <> Неопределено Тогда
				Для каждого СтрТЗ Из ТаблицаПроверяемыеТовары Цикл
					Если СтрТЗ["Назначение"]  = СлужебноеНазначениеДляВозврата Тогда
						СтрТЗ["Назначение"]  = "";
					КонецЕсли;
				КонецЦикла;
		КонецЕсли;
