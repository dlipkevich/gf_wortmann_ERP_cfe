...
Процедура гф_РасходныйОрдерОтгрузкаРеализацией(Источник, Отказ, РежимПроведения)
	...
	Если ТипЗнч(ЗаказКлиента) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		Иначе
			...
			// Что бы Автор в новом документе не перезаписывался типовым механизмом процедуры "УстановитьАвтораОбъектаПередЗаписью"
			// в общем модуле "ЗаполнениеОбъектовПоСтатистике", устанавливаем загрузку "Истина" для возврата из процедуры
			ДокОбъект.ОбменДанными.Загрузка = Истина;
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(ДокОбъект, ЗаказКлиента,,"Номер, Проведен, Автор");
		ДокОбъект.Дата = ТекущаяДатаСеанса();
