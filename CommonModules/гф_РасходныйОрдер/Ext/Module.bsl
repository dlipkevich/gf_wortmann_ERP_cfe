...
Процедура гф_РасходныйОрдерОтгрузкаПеремещением(Источник, Отказ, РежимПроведения)
	...
	Для Каждого Строка Из ДанныеДокументаРасходныйОрдер Цикл
		...
		Если Строка.ОтметкаОбработки = Ложь Тогда
			...
			// Передаем параметры для обработки проведения, если отгрузка перемещением, то не очищать таблицу со штрихкодами
			ДокОбъект.ДополнительныеСвойства.Свойство("гф_ОтгрузкаПеремещением", НайденноеЗначение);
			
			Если НайденноеЗначение = Неопределено Тогда
				ДокОбъект.ДополнительныеСвойства.Вставить("гф_ОтгрузкаПеремещением");
				ДокОбъект.ДополнительныеСвойства.гф_ОтгрузкаПеремещением = Истина;
			КонецЕсли;
