...
Процедура гф_РасходныйОрдерОтгрузкаРеализацией(Источник, Отказ, РежимПроведения)
	...
	Если ТипЗнч(ЗаказКлиента) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		ДокументОснование = ПолучитьДанныеПоДокументуРеализацияТоваровИУслуг(Источник);
		...
		Если ДокументОснование <> Неопределено Тогда
			...
			Если Источник.Статус = Перечисления.СтатусыРасходныхОрдеров.Отгружен Тогда
				...
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Перепроведен документ ""%1"" с актуальной датой отгрузки.'"), ДокументОснование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				Возврат;
			КонецЕсли;
		...
		Если ЗаписатьДокумент Тогда
			...
			Если Источник.Статус = Перечисления.СтатусыРасходныхОрдеров.Проверен Тогда
				...
				// -- Галфинд Волков 27.04.2023
					ДокументЗаблокирован = Ложь;
					ДокументЗаблокирован = ЗаблокироватьДокумент(ДокОбъект);
				Иначе
...
Процедура гф_РасходныйОрдерОтгрузкаПеремещением(Источник, Отказ, РежимПроведения)
	...
	Для Каждого Строка Из ДанныеДокументаРасходныйОрдер Цикл
		...
		ДокументОснование = ПолучитьДокументОснованиеПоЗаказуКлиента(Источник.Ссылка, Строка.Распоряжение);
		...
		Если ДокументОснование <> Неопределено Тогда
			...
			// ++ Волков Было ранее, просто не перезаполняем, а проводим с актуальной датой
			//Если Источник.Статус = Перечисления.СтатусыРасходныхОрдеров.Отгружен Тогда
			//	ДокОбъект.Дата = ТекущаяДатаСеанса();
			//	ДокОбъект.Записать(РежимЗаписиДокумента.Проведение);
			//	
		...
		Если Строка.ОтметкаОбработки = Ложь Тогда
			...
			//ДокОбъект.ПеремещениеПоЗаказам 	= Истина;
			...
			Если ДокументЗаблокирован Тогда
				...
				Если ПровестиДокумент Тогда
					...
					ЗаписатьПровестиДокумент(ДокОбъект, РежимЗаписиДокумента.Проведение);
				Иначе
					//ДокументКорректировка.Записать(РежимЗаписиДокумента.Запись);
					ЗаписатьПровестиДокумент(ДокОбъект, РежимЗаписиДокумента.Запись);
				КонецЕсли;	
		...
		Если Строка.ОтметкаОбработкиКорректировка = Ложь Тогда
			...
			Иначе
				ДокументКорректировка = Документы.КорректировкаНазначенияТоваров.СоздатьДокумент();
				ДокументНовый = Истина;
				ДокументКорректировка.ДополнительныеСвойства.Вставить("ЭтоНовый", Истина);
			КонецЕсли;
			...
			Иначе
				...
				Если Не ДокументНовый Тогда
					ДокументЗаблокирован = ЗаблокироватьДокумент(ДокументКорректировка);
				Иначе
					ДокументЗаблокирован = Истина;
				КонецЕсли;
				...
				Если ДокументЗаблокирован Тогда
					...
					Если ПровестиДокумент Тогда
					    //ДокументКорректировка.Записать(РежимЗаписиДокумента.Проведение);	
						ЗаписатьПровестиДокумент(ДокументКорректировка, РежимЗаписиДокумента.Проведение);
					Иначе
						//ДокументКорректировка.Записать(РежимЗаписиДокумента.Запись);
						ЗаписатьПровестиДокумент(ДокументКорректировка, РежимЗаписиДокумента.Запись);
					КонецЕсли;	
...
Процедура гф_РасходныйОрдерОтгрузкаНаВнутренниеНужды(Источник, Отказ, РежимПроведения)
	...
	Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЗаказНаВнутреннееПотребление")
		...
		ДокументОснование = ПолучитьДокументРаспоряжениеПоЗаказНаВнутреннееПотребление(Распоряжение);
...
Функция ЗаблокироватьДокумент(ДокОбъект)
