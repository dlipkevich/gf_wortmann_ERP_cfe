	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.гф_Агрегация <> Неопределено
			...
			Исключение
				...
				ВнестиТекстЗаписиВЖурнал(НСтр("ru = 'Блокировка перед записью элемента справочника ""Штрих-коды упаковок товаров"" не выполнена'"));
			...
			Исключение
				...
				ВнестиТекстЗаписиВЖурнал(НСтр("ru = 'Запись в элемент справочника ""Штрих-коды упаковок товаров"" не выполнена'"));	
		...
		КонецЕсли;
			...
			Исключение
				...
				ВнестиТекстЗаписиВЖурнал(НСтр("ru = 'Блокировка документа не выполнена'"));
			...
			Исключение
				...
				ВнестиТекстЗаписиВЖурнал(НСтр("ru = 'Запись в документ не выполнена'"));
...
Процедура гф_РасходныйОрдерИзменениеСтатусаОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	...
	// ++ Галфинд_ДомнышеваКР_13_05_2024
	Если Источник.Статус = Перечисления.СтатусыРасходныхОрдеров.КОтгрузке 
		И НЕ ПолучитьЗначениеРеквизитаСкладаТоварыВКоробках(Источник.Склад)
		И Обувь(Источник.Ссылка) Тогда
		
        гф_ПеререзервированиеПодЗаказРасходныйОрдер(Источник, Отказ);
	КонецЕсли;	
	// ++ Галфинд_ДомнышеваКР_13_05_2024
	...
	Если Источник.СкладскаяОперация = Перечисления.СкладскиеОперации.ОтгрузкаКлиенту
		...
		Если Не Отказ
			...
			ВыполнитьКорректировкуЗаказаНеОтгружатьПоРасходномуОрдеру(Источник, Отказ);
		КонецЕсли;
		// -- Галфинд ВолковЕВ 2024/03/19
...
Процедура гф_РасходныйОрдерОтгрузкаРеализацией(Источник, Отказ, РежимПроведения)
	...
	Если ТипДокЗаказКлиента Тогда
		...
		// Добавление комментария в заказ из любой первой строки любого первого заказа (Комментарий РТУ в каждой строке одинаковый)
		Если ДокОбъект.Товары.Количество() > 0 И ЗначениеЗаполнено(ДокОбъект.Товары[0].ЗаказКлиента) Тогда
			...
			|	И ЗаказКлиентаТовары.ДатаОтгрузки = &ДатаОтгрузки
			|	И ЗаказКлиентаТовары.Количество = &Количество
			...
			Запрос.УстановитьПараметр("Номенклатура", ДокОбъект.Товары[0].Номенклатура);
			Запрос.УстановитьПараметр("Характеристика", ДокОбъект.Товары[0].Характеристика);
			Запрос.УстановитьПараметр("Количество", ДокОбъект.Товары[0].Количество);
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				ДокОбъект.Комментарий = ВыборкаДетальныеЗаписи.гф_КомментарийРТУ;
			КонецЦикла;
...
Процедура ОтправитьПисьмо(ДокОбъект, ТекстПисьма, ПроведениеДокумента, Уведомление = Ложь)
	...
	Если Не Уведомление Тогда // Галфинд_ДомнышеваКР_02_05_2024
		Запрос.УстановитьПараметр("Ссылка", ДокОбъект.Автор);
	// -- Галфинд ЕсиповАВ 04.05.2023
	// ++ Галфинд_ДомнышеваКР_02_05_2024	
	Иначе
		Запрос.УстановитьПараметр("Ссылка", ДокОбъект.Ответственный);
	КонецЕсли;
	// ++ Галфинд_ДомнышеваКР_02_05_2024
	...
	// ++ Галфинд_ДомнышеваКР_02_05_2024	
	ИначеЕсли Уведомление Тогда
		СообщениеПочты.Тема = "Расходный ордер " + ДокОбъект.Номер + " от: " + ДокОбъект.Дата 
		+ " не перевелся в статус ""К отгрузке"".";
	// -- Галфинд_ДомнышеваКР_02_05_2024	
	Иначе 
...
Процедура СоздатьПечатнуюФорму(МассивОбъектов, ТекстПисьма, СообщениеПочты, ОбъектыПечати, Представление, ЧастьИмениФайла, ИмяМетода)
	...
	Попытка
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации("РО_ОтправкаСообщений", УровеньЖурналаРегистрации.Предупреждение, , , ОписаниеОшибки());
		КонецЕсли;
		
	Исключение
		
		ТекстПисьма = ТекстПисьма + "Ошибка при создании отчета! " + ОписаниеОшибки() + Символы.ПС;
		ЗаписьЖурналаРегистрации("РО_ОтправкаСообщений", УровеньЖурналаРегистрации.Предупреждение, , , 
								"Ошибка при создании отчета! " + ОписаниеОшибки());
	КонецПопытки;
	
	ПутьКФайлу = КаталогВременныхФайлов(); 
	// ++ Галфинд_ДомнышеваКР_19_04_2024
	// ИмяФайла = ЧастьИмениФайла + Лев(Строка(ТекущаяДатаСеанса()), 10) + ".xls";
	ТекДата = ТекущаяДатаСеанса();
	ТекДатаСтрокой = Формат(ТекДата, "ДФ=dd.MM.yyyy");
	ИмяФайла = ЧастьИмениФайла + ТекДатаСтрокой + ".xls";
	// -- Галфинд_ДомнышеваКР_19_04_2024
	...
	Если ИмяМетода = "УПДЧерновик" Тогда
		...
		Если ТипЗнч(ДокСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			ДатаДокументаСтрока = Формат(ДокСсылка.Дата, "ДФ=dd.MM.yyyy");// Галфинд_ДомнышеваКР_19_04_2024
			ИмяФайла = ЧастьИмениФайла + СокрЛП(ДокСсылка.Контрагент.Наименование)
				+ " " + СокрЛП(ДокСсылка.Номер) + " " + ДатаДокументаСтрока + ".xls";
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("РО_ОтправкаСообщений", УровеньЖурналаРегистрации.Предупреждение, , , 
								"Не удалось добавить вложение! " + ОписаниеОшибки());
	КонецПопытки;
...
Процедура ВыгрузитьТДвФайл(Таб, ПолноеИмяФайла, ТекстПисьма)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("РО_ОтправкаСообщений", УровеньЖурналаРегистрации.Предупреждение, , , 
								"Не удалось создать файл! " + ОписаниеОшибки());
	КонецПопытки;
...
Процедура АгрегацияПерезаполнить(УпаковочныйЛист, ОтгрузкаКодовМаркировкиПарами, ШтрихкодыУпаковок, Статус
	...
	Пока ВыборкаДетальныеЗаписиАгрегация.Следующий() Цикл
		...
		// СадомцевСА 27.04.2024 Заменил строку
		...
		КонецЕсли;
			...
			Если флОтгрузкаКодовМаркировкиПарами Тогда
				...
				АгрегацияПерезаполнить_ОтгрузкаКМ_Пары(ВыборкаДетальныеЗаписиАгрегация, ШтрихкодыУпаковок);
				//
			...
			Иначе
				...
				Если Статус = Перечисления.СтатусыРасходныхОрдеров.КОтгрузке Тогда
					...
					// СадомцевСА 27.04.2024 Исправил ошибки СонарКуба. Перенес код в процедуру.
					АгрегацияПерезаполнить_ОтгрузкаКМ_Короба(ВыборкаДетальныеЗаписиАгрегация, ШтрихкодыУпаковок);
					//Если ВыборкаДетальныеЗаписиАгрегация.гф_Агрегация <> Неопределено
					//	И ЗначениеЗаполнено(ВыборкаДетальныеЗаписиАгрегация.гф_Агрегация) Тогда
					//
					//Попытка
				...
				КонецЕсли; // Если Статус = Перечисления.СтатусыРасходныхОрдеров.КОтгрузке Тогда
				
			КонецЕсли; // Если флОтгрузкаКодовМаркировкиПарами Тогда
...
Процедура ЗаписатьПровестиОтменитьПроведениеДокумента(ДокОбъект, РежимЗаписиДокумента)
	...
	Исключение
		...
		ВнестиТекстЗаписиВЖурнал(НСтр("ru = 'Проведение документа не выполнено'"));
...
Процедура ЗаполнитьДанныеПоНоменклатуреРеализация(Источник, ДокОбъект, МассивЗаказов)
	...
	// ++ Галфинд ВолковЕВ 2024/05/03 Получение скидки для реализации из отгруженных строк заказа
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Ссылка", ДокОбъект.Товары.Выгрузить().ВыгрузитьКолонку("ЗаказКлиента"));
	...
	Для Каждого Стр Из ДокОбъект.Товары Цикл
		...
		Отбор.Вставить("Ссылка", Стр.ЗаказКлиента);
		...
		Отбор.Вставить("гф_ДатаДляРТУ", НачалоДня(ДокОбъект.Дата));
		
		СписокУпаковочныйЛистИлиНоменклатура = ВыборкаДетальныеЗаписи.НайтиСтроки(Отбор);
		
		Если СписокУпаковочныйЛистИлиНоменклатура.Количество() > 0 Тогда
			СкидкаПоКодуСтрокиИзЗаказа = СписокУпаковочныйЛистИлиНоменклатура[0].гф_СкидкаДляРТУ;
			
			Если ЗначениеЗаполнено(СкидкаПоКодуСтрокиИзЗаказа) И СкидкаПоКодуСтрокиИзЗаказа > 0 Тогда
				СкидкаКоэффициент = (ЗначениеСто - СкидкаПоКодуСтрокиИзЗаказа) / ЗначениеСто;
				
				Стр.Цена = Стр.Цена * СкидкаКоэффициент;
...
Процедура ВыполнитьКорректировкуЗаказаНеОтгружатьПоРасходномуОрдеру(Источник, Отказ = Ложь)
	...
	ТЗ_Заказы.Колонки.Добавить("ОтказОтЗаписиПоискКомплектации");
	...
	Для Каждого СтрокаЗаказы Из ТЗ_Заказы Цикл
		...
		СтрокаЗаказы.ОтказОтЗаписиПоискКомплектации 	= Ложь;
		...
		Если Не Отказ Тогда
			...
			Для Каждого СтрокаУпаковочныйЛистНоменклатура Из СписокУпаковочныйЛистИлиНоменклатура Цикл
				...
				Если ТоварыВКоробках Тогда
					...
					// что бы при работе с табличной частью заказа добавить это количество
				...
				Если ТоварыВКоробках Тогда
					...
					Для Каждого СтрокаТоварыВКоробах Из ТЗ_Заказ_ТоварыВКоробах Цикл
						...
						Если СтрокаУпаковочныйЛистНоменклатура.ВариантКомплектации = СтрокаТоварыВКоробах.ВариантКомплектации
							...
							// ++ Галфинд ВолковЕВ 2024/04/26 Упаковочные листы в заказах добавляются по причине и их также необходимо
							// корректировать при полной отмене отгрузки по ордеру
							// И Не СтрокаТоварыВКоробах.Добавлено Тогда
							Тогда
							// -- Галфинд ВолковЕВ 2024/04/26
						...
						ИначеЕсли СтрокаУпаковочныйЛистНоменклатура.ВариантКомплектации = СтрокаТоварыВКоробах.ВариантКомплектации
							...
							ТекстСообщения = СтрШаблон(НСтр("ru = 'Вкладка ""Товары в коробах"" Строка %1 заказа %2 добавлена по причине ""%3""'"),
							СтрокаТоварыВКоробах.НомерСтроки, СтрокаЗаказы.Заказ, СтрокаТоварыВКоробах.ПричинаДобавления);
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,,Отказ);
							Возврат;
					...
					Если ОтказЗаписьУпаковочногоЛиста Тогда
						...
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
						СтрокаЗаказы.ОтказОтЗаписиПоискКомплектации = Истина;
						СтрокаЗаказы.ОтказОтЗаписи = Истина;
						Прервать;
					КонецЕсли;
				...
				Иначе
					...
					ОбработкаЗаполненияТоваров(Отказ, ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных,
						ОбъектЗаказ, Источник.ДатаОтгрузки);
	...
	Если ТЗ_Заказы.Количество() > 0 Тогда
		...
		НаличиеЗаказаСПерепроведением = ТЗ_Заказы.Найти(Ложь, "ОтказОтЗаписиПоискКомплектации");
		
		Если НаличиеЗаказаСПерепроведением = Неопределено Тогда
			
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Корректировка заказа(ов) не выполнена, не была сопоставлена комплектация в заказе по ""%1"".'"),
			Источник.Ссылка);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		...
		Для Каждого СтрокаЗаказы Из ТЗ_Заказы Цикл
			...
			Попытка
				...
				СтрокаЗаказы.ОшибкаПроведенияЗаказа = Ложь;
		...
		Иначе
			...
			Если НаличиеЗаказаСОшибкойЗаписи.Количество() > 0 Тогда
				ТекстСообщения = НСтр("ru = 'Выполение перепроведения скорректированного заказа завершилось неудачно.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Иначе	
				ТекстСообщения = НСтр("ru = 'Повторная корректировка заказа(ов) не требуется.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
...
Процедура УдалитьНоменклатуру(Отказ, ТабКомплектация, ТЗ_Заказ_Товары, СтрокаТоварыВКоробах, СостояниеКороба)
	...
	Для Каждого СтрокаКомплектацияРазбиение Из ТабКомплектация Цикл
		...
		ИначеЕсли МассивСтрок.Количество() > 1 Тогда
			...
			МассивУдалитьСтрокиНоменклатуры.Добавить(МассивСтрок[0]); НомНайденаВМассиве = Истина;
		Иначе
			
			Если МассивСтрок.Количество() = 0 Тогда
				
				// Новый отбор без указания точного количества, в найденных строках
				// количество будет вычитаться (отменяться) частями, сколько возможно.
				// Если полностью не удастся списать частями все отменяемое количество то отмена отгрузки будет прервана.
				Отбор = Новый Структура();
				Отбор.Вставить("ВариантОбеспечения", Отгрузить);
				Отбор.Вставить("Номенклатура", Номенклатура);
				Отбор.Вставить("Характеристика", Характеристика);
				...
				Если МассивСтрок.Количество() > 0 Тогда
					...
					// Общее число отменяемых строк номенклатуры в ордере и заказе может быть одинаковым и общее количество тоже,
					// а количество указанное в строке может не совпадать.
					// Требуется подсчет оставшегося количества для отмены в следующей строке после обработки предыдущей.
					...
					Для Каждого СтрокаМассива Из МассивСтрок Цикл
						...
						Если СтрокаМассива.Количество > КоличествоОставшееся Тогда
							...
							СтрокаМассива.Количество = СтрокаМассива.Количество - КоличествоОставшееся;
							СтрокаМассива.КоличествоУпаковок = СтрокаМассива.КоличествоУпаковок - КоличествоОставшееся;
							КоличествоОставшееся = 0;
				...
				Иначе
					...
					ТекстСообщения = СтрШаблон(НСтр("ru = 'Исключительная ситуация, код ошибки для разработчика ПП454. Проблема с определением ""%1"" по ""%2"" на вкладке ""Товары"". Артикул ""%3"".'"),
					СтрокаКомплектацияРазбиение.Номенклатура, СтрокаКомплектацияРазбиение.Характеристика, СтрокаКомплектацияРазбиение.Номенклатура.Артикул);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
					Отказ = Истина;
...
Процедура ОбработкаЗаполненияТоваров(Отказ, ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура,
	СтруктураПереданныхДанных, ОбъектЗаказ, ДатаОтгрузки)
	...
	ПозицияНоменклатурыСопоставленаПоДате = Ложь;
	...
	Если МассивСтрок.Количество() = 1 Тогда
		...
		МассивСтрок[0].гф_СкидкаДляРТУ 		= ЗначениеНеопределено;
		МассивСтрок[0].гф_ДатаДляРТУ 		= ЗначениеНеопределено;
		МассивСтрок[0].гф_КомментарийРТУ 	= ЗначениеНеопределено;
	...
	ИначеЕсли МассивСтрок.Количество() > 1 Тогда
		...
		Для Каждого Стр Из МассивСтрок Цикл
			...
			Если НачалоДня(Стр.гф_ДатаДляРТУ) = НачалоДня(ДатаОтгрузки) Тогда
				Стр.ВариантОбеспечения 	= Перечисления.ВариантыОбеспечения.КОбеспечению;
				Стр.гф_СкидкаДляРТУ 	= ЗначениеНеопределено;
				Стр.гф_ДатаДляРТУ 		= ЗначениеНеопределено;
				Стр.гф_КомментарийРТУ 	= ЗначениеНеопределено;
				
				ПозицияНоменклатурыСопоставленаПоДате = Истина;
			КонецЕсли;
			
		КонецЦикла;
		
		Если Не ПозицияНоменклатурыСопоставленаПоДате Тогда
			
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Исключительная ситуация, код ошибки для разработчика КЕ333. Проблема с определением ""%1"" по ""%2"" на вкладке ""Товары"".'"),
			СтрокаКомплектацияРазбиение.Номенклатура, СтрокаКомплектацияРазбиение.Характеристика);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Отказ = Истина;
			...
			Если МассивСтрок.Количество() > 0 Тогда
				...
				Для Каждого СтрокаМассива Из МассивСтрок Цикл
					...
					Если СтрокаМассива.Количество > КоличествоОставшееся Тогда
						...
						УдаляемоеКоличество = СтрокаКомплектацияРазбиение.Количество;
						КоличествоОставшееся = КоличествоОставшееся - УдаляемоеКоличество;
						...
						Количество = СтрокаУпаковочныйЛистНоменклатура.Количество;
...
Процедура ДобавитьСтрокуРазбиением(ПараметрыОбработки, НоваяТаблица, ТекущаяСтрока, Товары, СписокКодовСтроки,
	...
	Если ТекущаяСтрока[ПараметрыРазбиенияСтроки.ИмяПоляКоличество] > Количество Тогда
		...
		НоваяСтрока.ДатаОтгрузки 			= Неопределено;
		НоваяСтрока.гф_СкидкаДляРТУ 		= Неопределено;
		НоваяСтрока.гф_ДатаДляРТУ 			= Неопределено;
		НоваяСтрока.гф_КомментарийРТУ 		= Неопределено;
	...
	ИначеЕсли ТекущаяСтрока[ПараметрыРазбиенияСтроки.ИмяПоляКоличество] = Количество Тогда
		...
		ТекущаяСтрока.ДатаОтгрузки 			= Неопределено;
		ТекущаяСтрока.гф_СкидкаДляРТУ 		= Неопределено;
		ТекущаяСтрока.гф_ДатаДляРТУ 		= Неопределено;
		ТекущаяСтрока.гф_КомментарийРТУ 	= Неопределено;
...
Процедура ВнестиТекстЗаписиВЖурнал(ТекстЗаписиВЖурнал)
...
// #wortmann {
// Для Парного склада при проведении РО в статсу "КОтгрузке" по Номенклатура = Обувь
// проверка остатков КМ и создание док. гф_ПеререзервированиеПодЗаказ
// Галфинд_Домнышева 2024/05/13
//
// При изменении статуса в документе "Расходный ордер", выполняется содание и запись\проведение документа "РТиУ"
// Параметры:
...
//  Отказ - Булево - Если в теле процедуры-обработчика установить данному параметру значение Истина,
//                   то будет выполнен отказ от продолжения работы после выполнения проверки заполнения.
Процедура гф_ПеререзервированиеПодЗаказРасходныйОрдер(Источник, Отказ)
	
	ТаблицаНазначений =  Источник.ОтгружаемыеТовары.ВыгрузитьКолонки();
	ТаблицаНазначений.Колонки.Добавить("ШтрихкодУпаковки", Новый ОписаниеТипов("СправочникСсылка.ШтрихкодыУпаковокТоваров"));
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("Действие", Перечисления.ДействияСоСтрокамиОрдеровНаОтгрузку.Отгрузить); 
	ОтгружаемыеТовары = Источник.ОтгружаемыеТовары.НайтиСтроки(ПараметрыОтбора);
	...
	Для каждого Строка Из ОтгружаемыеТовары Цикл
		...
		Иначе
			Для Стр = 1 по Строка.Количество Цикл
				НоваяСтрока = ТаблицаНазначений.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
				НоваяСтрока.Количество = 1;
				НоваяСтрока.КоличествоУпаковок = 1;
	...
	ТаблицаНазначений.Сортировать("Назначение Возр");
	
	ТаблицаПолученныхКМ = ПолучитьТаблицуКМ(Источник.Ссылка);
	...
	Для каждого Строка Из ТаблицаНазначений Цикл
		...
		ПараметрыОтбора.Вставить("Занято", Ложь);
		
		МассивСтрок = ТаблицаПолученныхКМ.НайтиСтроки(ПараметрыОтбора);
		
		Если МассивСтрок <> Неопределено И МассивСтрок.Количество() > 0 Тогда
			 МассивСтрок[0].Назначение = Строка.Назначение;
			 МассивСтрок[0].Занято = Истина;
		КонецЕсли;
		
	КонецЦикла;
	
	ПроверитьОстатокНаСкладах(Источник.Ссылка, ТаблицаПолученныхКМ, Отказ);
		
КонецПроцедуры// } #wortmann 

// #wortmann {
// Формирует таблицу по данным ТЧ ШтрихкодыУпаковок
// Галфинд_Домнышева 2024/05/13
//
// Параметры:
//	Источник - ДокументСсылка.РасходныйОрдерНаТовары - Документ расходный ордер.
//
// Возвращаемое значение:
//	ТаблицаЗначений - сформированная ТЗ по результатам Запроса 
Функция ПолучитьТаблицуКМ(Источник)
...
// Параметры:
//	Источник - ДокументСсылка.РасходныйОрдерНаТовары - Документ расходный ордер.
//	ТаблицаПолученныхКМ - ТаблицаЗначений
//  Отказ - Булево - Если в теле процедуры-обработчика установить данному параметру значение Истина,
//                   то будет выполнен отказ от продолжения работы после выполнения проверки заполнения.
Процедура ПроверитьОстатокНаСкладах(Источник, ТаблицаПолученныхКМ, Отказ)
	
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьТекстЗапросаОстатковНаСкладе();
	
	МассивДок = Новый Массив;
	ДокСсылка = ПроверитьНаличиеСоздатьНовый(Источник);
	Если ДокСсылка <> Неопределено Тогда
		МассивДок.Добавить(ДокСсылка); 
	КонецЕсли;
    ДокСсылкаОснование = ПолучитьДанныеПоДокументуРеализацияТоваровИУслуг(Источник);
	Если ДокСсылкаОснование <> Неопределено Тогда
	   МассивДок.Добавить(ДокСсылкаОснование); 
	КонецЕсли;
	Запрос.УстановитьПараметр("ТаблицаПолученныхКМ", ТаблицаПолученныхКМ);
	Запрос.УстановитьПараметр("Дата", ТекущаяДатаСеанса());
	Запрос.УстановитьПараметр("Орг", Источник.Склад.гф_Организация);
	Запрос.УстановитьПараметр("Ссылка", Источник);
	...
	РезультатСовсемНет = Результат[7];
	
	Если НЕ РезультатСовсемНет.Пустой() Тогда 
		Отказ = Истина;
		
		ВыборкаБез = РезультатСовсемНет.Выбрать();
		ТекстУведомления = "Недостаточно товара: ";
		Пока ВыборкаБез.Следующий() Цикл
			ТекстУведомления = ТекстУведомления + "<p> Артикул: " + ВыборкаБез.Артикул + " характеристика: "
			+ ВыборкаБез.Характеристика + " с номером ГТД: " + ВыборкаБез.НомерГТД + "</p>"; 
		КонецЦикла;
		ОтправитьПисьмо(Источник, ТекстУведомления, Истина, Истина);
		ТекстУведомления = СтрЗаменить(ТекстУведомления, "</p>", Символы.ПС);
	...
	Если ДокСсылка = Неопределено Тогда
		ДокОбъект = Документы.гф_ПеререзервированиеПодЗаказ.СоздатьДокумент();
	Иначе
		ДокОбъект = ДокСсылка.ПолучитьОбъект();
	КонецЕсли;
	ДокОбъект.РасходныйОрдерОснование = Источник;
	ДокОбъект.Склад = Источник.Склад;
	
	ДокОбъект.Дата = ?(ЗначениеЗаполнено(ДокОбъект.Дата), ДокОбъект.Дата, ТекущаяДатаСеанса());

	Товары = ДокОбъект.Товары;
	Товары.Очистить();
	
	РезультатИтог = Результат[5];
	
	ВыборкаНоменклатура = РезультатИтог.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	                                                                                                   
	Пока ВыборкаНоменклатура.Следующий() Цикл 
		ВыборкаХарактеристика = ВыборкаНоменклатура.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);	
		Пока ВыборкаХарактеристика.Следующий() Цикл 	
			ВыборкаГТД = ВыборкаХарактеристика.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаГТД.Следующий() Цикл 
				КоличествоОстаток = ВыборкаГТД.КоличествоОстаток;
				
				ВыборкаДетали = ВыборкаГТД.Выбрать();                         
				
				КоличествоОсталось = Неопределено;
				
				Пока ВыборкаДетали.Следующий() И (КоличествоОсталось = Неопределено ИЛИ КоличествоОсталось > 0) Цикл
					
					КоличествоДок = ВыборкаДетали.КоличествоДок;
					КоличествоОсталось = ?(КоличествоОсталось = Неопределено, КоличествоДок, КоличествоОсталось);
					
					СтрокаТЧ = Товары.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТЧ, ВыборкаДетали);
					КоличествоВСтроке = ?(КоличествоОсталось <= ВыборкаДетали.КоличествоОстаток, КоличествоОсталось, ВыборкаДетали.КоличествоОстаток); 
					СтрокаТЧ.Количество = КоличествоВСтроке; 
					СтрокаТЧ.ГТД = ВыборкаДетали.НомерГТД;
					СтрокаТЧ.НазначениеФинальное = ВыборкаДетали.Назначение;
					КоличествоОсталось = КоличествоОсталось - КоличествоВСтроке;
				КонецЦикла;
	...
	Исключение
		ДокОбъект.Записать(РежимЗаписиДокумента.Запись);
		ЗаписьЖурналаРегистрации("ПеререзервированиеПодЗаказ", УровеньЖурналаРегистрации.Ошибка, ДокОбъект, , ОписаниеОшибки());
	КонецПопытки;
	
КонецПроцедуры// } #wortmann

// #wortmann {
// Проверяет наличие документа гф_ПеререзервированиеПодЗаказ 
// Галфинд_Домнышева 2024/05/13
//
// Параметры:
//	Источник - ДокументСсылка.РасходныйОрдерНаТовары - Документ расходный ордер.
//
// Возвращаемое значение:
//	Документ - ДокументСсылка.гф_ПеререзервированиеПодЗаказ ИЛИ Неопределено 
Функция ПроверитьНаличиеСоздатьНовый(Источник)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	гф_ПеререзервированиеПодЗаказ.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.гф_ПеререзервированиеПодЗаказ КАК гф_ПеререзервированиеПодЗаказ
	|ГДЕ
	|	гф_ПеререзервированиеПодЗаказ.РасходныйОрдерОснование = &РасходныйОрдерОснование
	|	И гф_ПеререзервированиеПодЗаказ.Проведен";
	
	Запрос.УстановитьПараметр("РасходныйОрдерОснование", Источник);
...
// #wortmann {
// Получает текст для запроса по остаткам.
// Галфинд_Домнышева 2024/05/13
//
// Параметры:
//	Источник - ДокументСсылка.РасходныйОрдерНаТовары - Документ расходный ордер.
...
// Возвращаемое значение:
//	Текст - Строка 
Функция ПолучитьТекстЗапросаОстатковНаСкладе() 
	
	Текст = "ВЫБРАТЬ
		|	ТаблицаПолученныхКМ.ШтрихкодУпаковки КАК ШтрихкодУпаковки,
		|	ТаблицаПолученныхКМ.Номенклатура КАК Номенклатура,
		|	ТаблицаПолученныхКМ.Характеристика КАК Характеристика,
		|	ТаблицаПолученныхКМ.Назначение КАК Назначение,
		|	ТаблицаПолученныхКМ.Количество КАК Количество,
		|	ТаблицаПолученныхКМ.Занято КАК Занято
		|ПОМЕСТИТЬ ВТ_КМ
		|ИЗ
		|	&ТаблицаПолученныхКМ КАК ТаблицаПолученныхКМ
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 1
		|ВЫБРАТЬ
		|	ВТ_КМ.Номенклатура КАК Номенклатура,
		|	ВТ_КМ.Характеристика КАК Характеристика,
		|	ВТ_КМ.ШтрихкодУпаковки.гф_НомерГТД КАК НомерГТД,
		|	ВТ_КМ.Назначение КАК Назначение,
		|	СУММА(ВТ_КМ.Количество) КАК Количество
		...
		|СГРУППИРОВАТЬ ПО
		|	ВТ_КМ.Номенклатура,
		|	ВТ_КМ.Характеристика,
		|	ВТ_КМ.ШтрихкодУпаковки.гф_НомерГТД,
		|	ВТ_КМ.Назначение
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 2
		 |ВЫБРАТЬ
        |	ВТ_Назначения.Номенклатура КАК Номенклатура,
        |	ВТ_Назначения.Характеристика КАК Характеристика,
        |	ВТ_Назначения.Количество КАК КоличествоДок,
        |	ВТ_Назначения.Назначение КАК НазначениеИсходное,
        ...
        |				ТОГДА 1
        |			ИНАЧЕ -1
        |		КОНЕЦ * ТоварыОрганизаций.Количество) КАК КоличествоОстаток,
        |	ВТ_Назначения.НомерГТД КАК НомерГТД
        |ПОМЕСТИТЬ ВТ_Остатки
        |ИЗ
        |	ВТ_Назначения КАК ВТ_Назначения
        |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыОрганизаций КАК ТоварыОрганизаций
        |		ПО ВТ_Назначения.Номенклатура = ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Номенклатура
        |			И ВТ_Назначения.Характеристика = ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Характеристика
        |			И ВТ_Назначения.НомерГТД = ТоварыОрганизаций.НомерГТД
        |			И ВТ_Назначения.Назначение = ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Назначение
        |ГДЕ
        |	НЕ ТоварыОрганизаций.Регистратор В (&МассивДок)
        |	И ТоварыОрганизаций.Организация = &Орг
        |	И (ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Номенклатура, ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Характеристика, ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Назначение, ТоварыОрганизаций.АналитикаУчетаНоменклатуры.МестоХранения, ТоварыОрганизаций.НомерГТД) В
        |			(ВЫБРАТЬ
        |				Т.Номенклатура,
        |				Т.Характеристика,
        |				Т.Назначение,
        ...
        |
        |СГРУППИРОВАТЬ ПО
        |	ВТ_Назначения.Номенклатура,
        |	ВТ_Назначения.Характеристика,
        |	ВТ_Назначения.Количество,
        |	ВТ_Назначения.Назначение,
        |	ТоварыОрганизаций.АналитикаУчетаНоменклатуры,
        |	ВТ_Назначения.НомерГТД
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////  3
        |ВЫБРАТЬ
        ...
        |	ВТ_Остатки.НазначениеИсходное КАК НазначениеИсходное,
        |	ВТ_Остатки.НомерГТД КАК НомерГТД,
        |	ВТ_Остатки.КоличествоДок - ВТ_Остатки.КоличествоОстаток КАК КоличествоДок
        |ПОМЕСТИТЬ ВТ_НЕдостаточно
        |ИЗ
        |	ВТ_Остатки КАК ВТ_Остатки
        |ГДЕ
        |	ВТ_Остатки.КоличествоДок > ВТ_Остатки.КоличествоОстаток
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////  4
        |ВЫБРАТЬ
        |	ВТ_НЕдостаточно.Номенклатура КАК Номенклатура,
        |	ВТ_НЕдостаточно.Характеристика КАК Характеристика,
        |	ВТ_НЕдостаточно.КоличествоДок КАК КоличествоДок,
        |	ВТ_НЕдостаточно.НазначениеИсходное КАК НазначениеИсходное,
        |	ТоварыОрганизаций.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
        ...
        |			ИНАЧЕ -1
        |		КОНЕЦ * ТоварыОрганизаций.Количество) КАК КоличествоОстаток,
        |	ВТ_НЕдостаточно.НомерГТД КАК НомерГТД
        |ПОМЕСТИТЬ ВТ_ОстаткиБезУчета
        |ИЗ
        |	ВТ_НЕдостаточно КАК ВТ_НЕдостаточно
        |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыОрганизаций КАК ТоварыОрганизаций
        |		ПО ВТ_НЕдостаточно.Номенклатура = ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Номенклатура
        |			И ВТ_НЕдостаточно.Характеристика = ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Характеристика
        |			И ВТ_НЕдостаточно.НомерГТД = ТоварыОрганизаций.НомерГТД
        |ГДЕ
        |	НЕ ТоварыОрганизаций.Регистратор В (&МассивДок)
        |	И ТоварыОрганизаций.Организация = &Орг
        |	И (ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Номенклатура, ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Характеристика, ТоварыОрганизаций.АналитикаУчетаНоменклатуры.МестоХранения, ТоварыОрганизаций.НомерГТД) В
        |			(ВЫБРАТЬ
        |				Т.Номенклатура,
        |				Т.Характеристика,
        |				&Склад,
        ...
        |
        |СГРУППИРОВАТЬ ПО
        |	ВТ_НЕдостаточно.Номенклатура,
        |	ВТ_НЕдостаточно.Характеристика,
        |	ВТ_НЕдостаточно.КоличествоДок,
        |	ВТ_НЕдостаточно.НазначениеИсходное,
        |	ТоварыОрганизаций.АналитикаУчетаНоменклатуры,
        |	ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Назначение,
        |	ВТ_НЕдостаточно.НомерГТД
        |;
		|
		|//////////////////////////////////////////////////////////////////////////////// 5
		|ВЫБРАТЬ
		|	ВТ_ОстаткиБезУчета.Номенклатура КАК Номенклатура,
		|	ВТ_ОстаткиБезУчета.Характеристика КАК Характеристика,
		|	ВТ_ОстаткиБезУчета.КоличествоДок КАК КоличествоДок,
		|	ВТ_ОстаткиБезУчета.НазначениеИсходное КАК НазначениеИсходное,
		|	ВТ_ОстаткиБезУчета.Назначение КАК Назначение,
		|	ВТ_ОстаткиБезУчета.КоличествоОстаток КАК КоличествоОстаток,
		|	ВТ_ОстаткиБезУчета.НомерГТД КАК НомерГТД
		|ИЗ
		|	ВТ_ОстаткиБезУчета КАК ВТ_ОстаткиБезУчета
		|
		|УПОРЯДОЧИТЬ ПО
		|	Назначение
		|ИТОГИ
		...
		|	ВТ_ОстаткиБезУчета.Характеристика КАК Характеристика,
		|	ВТ_ОстаткиБезУчета.НомерГТД КАК НомерГТД,
		|	МАКСИМУМ(ВТ_ОстаткиБезУчета.КоличествоДок) КАК КоличествоДок,
		|	СУММА(ВТ_ОстаткиБезУчета.КоличествоОстаток) КАК КоличествоОстаток
		|ПОМЕСТИТЬ ОстаткиГруп
		|ИЗ
		|	ВТ_ОстаткиБезУчета КАК ВТ_ОстаткиБезУчета
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ОстаткиБезУчета.Номенклатура,
		|	ВТ_ОстаткиБезУчета.Характеристика,
		|	ВТ_ОстаткиБезУчета.НомерГТД
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////  7
		|ВЫБРАТЬ
		|	ОстаткиГруп.Номенклатура КАК Номенклатура,
		|	ОстаткиГруп.Номенклатура.Артикул КАК Артикул,
		|	ОстаткиГруп.Характеристика КАК Характеристика,
		|	ОстаткиГруп.НомерГТД КАК НомерГТД,
		|	ОстаткиГруп.КоличествоДок КАК КоличествоДок,
		|	ОстаткиГруп.КоличествоОстаток КАК КоличествоОстаток
		|ИЗ
		|	ОстаткиГруп КАК ОстаткиГруп
		|ГДЕ
		|	ОстаткиГруп.КоличествоДок > ОстаткиГруп.КоличествоОстаток";

	Возврат Текст;
	
КонецФункции// } #wortmann

Процедура АгрегацияПерезаполнить_ОтгрузкаКМ_Короба(ВыборкаДетальныеЗаписиАгрегация, ШтрихкодыУпаковок)
	Если ВыборкаДетальныеЗаписиАгрегация.гф_Агрегация <> Неопределено
		И ЗначениеЗаполнено(ВыборкаДетальныеЗаписиАгрегация.гф_Агрегация) Тогда
...
Процедура АгрегацияПерезаполнить_ОтгрузкаКМ_Пары(ВыборкаДетальныеЗаписиАгрегация, ШтрихкодыУпаковок)
