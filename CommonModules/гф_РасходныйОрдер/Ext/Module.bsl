...
Процедура гф_РасходныйОрдерОтгрузкаРеализацией(Источник, Отказ, РежимПроведения)
	...
	Если ТипЗнч(ЗаказКлиента) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		Если ДокСсылкаОснование <> Неопределено Тогда
			...
			//ДокОбъект = ПолучитьДокументПоДокументуОснованиеИлиРаспоряжению(ДокументОснование);
			
			// Волков
			ДокОбъект = ДокСсылкаОснование.Получитьобъект();
			ЗаблокироватьДокумент(ДокОбъект);
			ПроверитьПометкуУдаленияИПроведение(ДокОбъект);
			// Волков
		...
		Если ЕстьУпаковочныйЛист Тогда
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				ДокОбъект, КлючСвязи, КодСтроки, ДокОбъект.ШтрихкодыУпаковок, ОбщаяСуммаПоУпаковочногоЛиста, СкладскаяОперация, Заказ, Источник);
		...
		Если ЗаписатьДокумент Тогда
			...
			Если Источник.Статус = Перечисления.СтатусыРасходныхОрдеров.Проверен Тогда
				...
				// ++ Галфинд Волков 27.04.2023 Выше добалено значение потерянной переменной
				//// vvv Галфинд \ Sakovich 25.04.2023
				////Если Не ДокументНовый Тогда
				//Если Не ДокументНовый  = Истина Тогда
				//// ^^^ Галфинд \ Sakovich 25.04.2023
				Если Не ДокументНовый Тогда
				...
				Если ДокументЗаблокирован Тогда
					//ДокОбъект.Записать(РежимЗаписиДокумента.Запись);
				    ЗаписатьПровестиДокумент(ДокОбъект, РежимЗаписиДокумента.Запись);
				КонецЕсли;
			...
			ИначеЕсли Источник.Статус = Перечисления.СтатусыРасходныхОрдеров.КОтгрузке
				...
				Если ДокументЗаблокирован Тогда
					//ДокОбъект.Записать(РежимЗаписиДокумента.Проведение);
					ЗаписатьПровестиДокумент(ДокОбъект, РежимЗаписиДокумента.Проведение);
				КонецЕсли;
...
Процедура гф_РасходныйОрдерОтгрузкаПеремещением(Источник, Отказ, РежимПроведения)
	...
	// Пока несколько раз в цикле приходится перезаписывать перемещение, по каждой позиции проверки, так как создаем по каждой корректировку
	Для Каждого Строка Из ДанныеДокументаРасходныйОрдер Цикл
		
		// ПеремещениеТоваров
		
		ДокСсылкаОснование = ПолучитьДокументОснованиеПоЗаказуКлиента(Источник.Ссылка, Строка.Распоряжение);
		
		КодСтроки = 1;
		
		//ДокументНовый = Ложь;
		ДокументЗаблокированПриПолучении = Ложь;
		Если ДокСсылкаОснование <> Неопределено Тогда
		...
		Если Не Строка.ОтметкаОбработки Тогда
			...
			Если ЕстьУпаковочныйЛист Тогда
				...
				ЗаполнитьДанныеПоУпаковочнымЛистам(Источник, ДокОбъект, УпаковочныеЛисты, Источник.СкладскаяОперация, Источник.Статус);
			...
			Иначе
				...
				Для Каждого СтрокаТПР Из ТоварыПоРаспоряжениям Цикл
					
					Если СтрокаТПР.ОтметкаОбработки = Ложь И Строка.Распоряжение = СтрокаТПР.Распоряжение и Не Строка.НомерСтроки = СтрокаТПР.НомерСтроки Тогда
						НоваяСтрока = ДокОбъект.Товары.Добавить();
						...
						СтрокаТПР.ОтметкаОбработки = Истина;
			...
			Строка.ОтметкаОбработки = Истина;
			ДокОбъект.ДополнительныеСвойства.Вставить("ДокументОснование", Истина);
		...
		Если Не Строка.ОтметкаОбработкиКорректировка Тогда
			...
			Если ДокСсылкаОснованиеКорректировка <> Неопределено Тогда
				...
				//ДокументКорректировка = ПолучитьДокументПоДокументуОснованиеИлиРаспоряжению(ДокументОснованиеКорректировка);
				//ДокОбъектКорректировка = ПолучитьДокументПоДокументуОснование(ДокСсылкаОснованиеКорректировка);
				//ЗаблокироватьДокумент(ДокОбъектКорректировка);
				
				ДокОбъектКорректировка = ДокСсылкаОснованиеКорректировка.Получитьобъект();
				ЗаблокироватьДокумент(ДокОбъектКорректировка);
				Если Не БылоОбращениеКДокументу Тогда
					ПроверитьПометкуУдаленияИПроведение(ДокОбъектКорректировка);
					ОбращениеКДокументу = Истина;
				КонецЕсли;
			...
			// ++ Волков отладка Пока подставляем ложь
			//ЕстьУпаковочныйЛист = Ложь;
			// -- Волков отладка
			...
			Иначе
				...
				// ++ Волков отладка, пока подставляем Истина, ранее подставляли временно Ложь
				//ЕстьУпаковочныйЛист = Истина;
				// -- Волков отладка
				...
				Строка.ОтметкаОбработкиКорректировка = Истина;
				ДокОбъектКорректировка.ДополнительныеСвойства.Вставить("ДокументОснование", Истина);
				
				// ++ Волков отладка
				//ДокОбъектКорректировка.Записать(РежимЗаписиДокумента.Проведение);
				
				//ДокументЗаблокирован = Ложь;
				//
				//Если Не ДокументНовый Тогда
				//	ДокументЗаблокирован = ЗаблокироватьДокументСПроверкойЭтоНовый(ДокОбъектКорректировка);
				//Иначе
				//	ДокументЗаблокирован = Истина;
				//КонецЕсли;
				...
				Если ДокОбъект.ДополнительныеСвойства.Свойство("ЭтоНовый") Или ДокОбъектКорректировка.Заблокирован() Тогда
					
					ПровестиДокумент = Истина;
					Если ПровестиДокумент Тогда
					    ЗаписатьПровестиДокумент(ДокОбъектКорректировка, РежимЗаписиДокумента.Проведение);
					Иначе
						ЗаписатьПровестиДокумент(ДокОбъектКорректировка, РежимЗаписиДокумента.Запись);
					КонецЕсли;	
...
Процедура гф_ДобавитьТоварыПоУпаковочномуЛисту(УпаковочныйЛист, ТоварыВКоробах, ВидыЗапасов, ДокОбъект,
	КлючСвязи, КодСтроки, ШтрихкодыУпаковок, ОбщаяСуммаПоУпаковочногоЛиста, СкладскаяОперация, Заказ, Источник)      
	...
	Если Источник.ДополнительныеСвойства.Свойство("гф_УпаковочныйЛист") Тогда
		...
		// Типизируем колонки таблицы значений для раюботы с запросом
...
Процедура ОтправитьПисьмо(ДокОбъект, ТекстПисьма, ПроведениеДокумента)
	...
	Иначе 
		СообщениеПочты.Тема = "Создана «Планируемая отгрузка».";
...
Процедура ЗаполнитьДанныеПоУпаковочнымЛистам(Источник, ДокОбъект, УпаковочныеЛисты, Знач СкладскаяОперация, Знач Статус)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Иначе
			...
			// Для документа "Перемещение" табличная часть имеет другое название чем у документа "Ренализация"
		...
		гф_ДобавитьТоварыПоУпаковочномуЛисту(НоваяСтрока.УпаковочныйЛист, ДокОбъект.Товары, ДокОбъект.ВидыЗапасов,
		ДокОбъект, КлючСвязи, КодСтроки, ШтрихкодыУпаковок, ОбщаяСуммаПоУпаковочногоЛиста, СкладскаяОперация, Заказ, Источник);
		...
		Пока ВыборкаДетальныеЗаписиАгрегация.Следующий() Цикл
			...
			Если ВыборкаДетальныеЗаписиАгрегация.гф_Агрегация <> Неопределено Тогда
				...
				Иначе
					...
					// Ранее использовался Источник.Статус
...
Процедура гф_РасходныйОрдерОтгрузкаНаВнутренниеНужды(Источник, Отказ, РежимПроведения)
	...
	Если СтруктураЗаполнения <> Неопределено Тогда
		...
		гф_ЗаполнитьПоЗаказу(СтруктураЗаполнения, Распоряжение, ДокОбъект, ДокументНовый);
	КонецЕсли;
	
КонецПроцедуры

Процедура гф_ЗаполнитьПоЗаказу(СтруктураЗаполнения, Ссылка, ДокОбъект, ДокументНовый)
...
Процедура ЗаписатьПровестиДокумент(ДокОбъект, РежимЗаписиДокумента)
...
Функция ПолучитьДокументПоДокументуОснованиеИлиРаспоряжению(Документ)
	...
	Если ДокОбъект.Проведен Тогда
		...
		Если ДокументЗаблокирован Тогда
			...
			ЗаписатьПровестиДокумент(ДокОбъект, РежимЗаписиДокумента.ОтменаПроведения);
...
// Используется при работе с документами: Перемещение и
Процедура ПроверитьПометкуУдаленияИПроведение(ДокОбъект)
...
Процедура ЗаблокироватьДокумент(ДокОбъект)
