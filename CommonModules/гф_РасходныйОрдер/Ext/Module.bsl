...
Процедура гф_РасходныйОрдерИзменениеСтатусаОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	...
	Если Источник.СкладскаяОперация = Перечисления.СкладскиеОперации.ОтгрузкаКлиенту
		...
		// ++ Галфинд ВолковЕВ 2024/03/19
		//Если Источник.гф_КорректироватьЗаказ Тогда // Убрать перед внесением в хранилище
		Если Не Отказ
		...
		Если Не Отказ Тогда
			...
			Для Каждого СтрокаУпаковочныйЛистНоменклатура Из СписокУпаковочныйЛистИлиНоменклатура Цикл
				...
				Если ТоварыВКоробках Тогда
					...
					Для Каждого СтрокаТоварыВКоробах Из ТЗ_Заказ_ТоварыВКоробах Цикл
						...
						Если СтрокаУпаковочныйЛистНоменклатура.ВариантКомплектации = СтрокаТоварыВКоробах.ВариантКомплектации
							...
							Если СтрокаТоварыВКоробах.Количество = 1 Тогда
								...
								//ОчиститьКодСтрокиВУпаковочномЛисте(СтрокаУпаковочныйЛистНоменклатура.УпаковочныйЛистРодитель);	
								//
								//ИзменитьАгрегациюУпаковочногоЛиста(СтрокаУпаковочныйЛистНоменклатура.УпаковочныйЛистРодитель);
					...
					//СтруктураПересчетаСуммы = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВТЧ(ОбъектЗаказ);
					//
					//КэшированныеЗначения = Неопределено;
					//
					//СтруктураДействий = Новый Структура;
					//СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
					//СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
					//ОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТЗ_Заказ_ТоварыВКоробах, СтруктураДействий, КэшированныеЗначения);
	...
	Если ТЗ_Заказы.Количество() > 0 Тогда
		...
		Для Каждого СтрокаЗаказы Из ТЗ_Заказы Цикл
			...
			Если СтрокаЗаказы.Перепроведение Тогда
				...
				Если ТранзакцияАктивна() Тогда
					...
					Если НаборКурсов.Количество() = 0 Тогда
						...
						//НовыйКурс.УпаковочныйЛист 		= Документы.УпаковочныйЛист.ПустаяСсылка();
						//НовыйКурс.Номенклатура 			= Справочники.Номенклатура.ПустаяСсылка(); 
						//НовыйКурс.Характеристика 		= Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка(); 
						НовыйКурс.Выполнено 			= Истина;
						//НовыйКурс.Количество 			= 1;
			...
			Иначе
				...
				//ТекстСообщения = "Сообщить код ситуации администратору ""456""";
				//ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				Продолжить; 
			КонецЕсли;
