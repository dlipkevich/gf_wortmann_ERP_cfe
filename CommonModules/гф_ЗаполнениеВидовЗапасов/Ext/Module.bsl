...
Процедура гф_ПерезаполнитьВидыЗапасовПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.ПередачаТоваровХранителю") Тогда
		ПередачаТоваровХранителюСформироватьВидыЗапасов(Источник);
	КонецЕсли;
...
Процедура РеализацияТоваровУслугСформироватьВидыЗапасов(Источник)
	...
	ВсегоСуммаРучнойСкидки = Товары.Итог("СуммаРучнойСкидки");
	ВсегоСуммаАвтоматическойСкидки = Товары.Итог("СуммаАвтоматическойСкидки");
	...
	ВЗ_ВсегоСуммаРучнойСкидки = тзВидыЗапасовНовая.Итог("СуммаРучнойСкидки");
	ВЗ_ВсегоСуммаАвтоматическойСкидки = тзВидыЗапасовНовая.Итог("СуммаАвтоматическойСкидки");
	...
	РазницаСуммаРучнойСкидки = ВсегоСуммаРучнойСкидки - ВЗ_ВсегоСуммаРучнойСкидки;
	РазницаСуммаАвтоматическойСкдики = ВсегоСуммаАвтоматическойСкидки - ВЗ_ВсегоСуммаАвтоматическойСкидки;
	
	Если НЕ (РазницаСуммаНДС = 0 И РазницаСуммаСНДС = 0  И РазницаСуммаВзаиморасчетов = 0
		И РазницаСуммаРучнойСкидки = 0 И РазницаСуммаАвтоматическойСкдики = 0) Тогда
		...
		Если НЕ ВсегоСтрок = 0 Тогда
			...
			ПоследняяСтрока.СуммаРучнойСкидки = ПоследняяСтрока.СуммаРучнойСкидки + РазницаСуммаРучнойСкидки;
			ПоследняяСтрока.СуммаАвтоматическойСкидки = 
			ПоследняяСтрока.СуммаАвтоматическойСкидки + РазницаСуммаАвтоматическойСкдики;
			ВидыЗапасовИзменены = Истина;
		КонецЕсли;
	КонецЕсли;
	// Галфинд \ Shtak  2024/04/12 <=
...
Процедура ПередачаТоваровХранителюСформироватьВидыЗапасов(Источник)
	...
	ТаблицаАналитикаТоваровПоГТД = ПолучитьТаблицуГТДПоНоменклатуреКоробов(Источник);
	...
	Для каждого стрВидыЗапасов Из тзВидыЗапасов Цикл
		стрПоиска = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(стрВидыЗапасов["АналитикаУчетаНоменклатуры"],
		...
		Для каждого Эл Из мСтрокТоваровПоГТД Цикл
			...
			Если Не ЗначениеЗаполнено(нс["ИдентификаторСтроки"]) Тогда
				нс["ИдентификаторСтроки"] = XMLСтрока(Новый УникальныйИдентификатор());
			КонецЕсли;
...
Функция ПолучитьТаблицуАналитикиТоваровПоГТДРеализация(Источник)
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(втДанные.Количество, 0) < вт_Товары.КоличествоУпаковок
