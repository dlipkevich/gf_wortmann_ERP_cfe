...
Процедура гф_ПерезаполнитьВидыЗапасовПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.ПеремещениеТоваров") Тогда
		...
		// $$$===========================vvv ОТЛАДКА vvv======================29.12.2023 13:41:32=============| SBB
		// заглушка ниже - отменена
		
		//// vvv Галфинд \ Sakovich 28.12.2023
		//// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eea5739c1beca5
		//// замену ГТД не производим по Перемещениям на основании поставки по импорту
		//Если ЗначениеЗаполнено(Источник.ДокументОснование)
		//	И ТипЗнч(Источник.ДокументОснование) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
		//	ХозОперацияОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Источник.ДокументОснование, "ХозяйственнаяОперация");
		//	Если ХозОперацияОснования = Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпорту Тогда
		//		Возврат;
		//	КонецЕсли;
		//КонецЕсли;
		//// ^^^ Галфинд \ Sakovich 28.12.2023
	
		// ===========================^^^ КОНЕЦ ОТЛАДКИ ^^^===================29.12.2023 13:41:32=============| SBB
...
Процедура ИзменитьГТДВВидахЗапасов(Источник)
	...
	ТаблицаАналитикаТоваровПоГТД = ПолучитьТаблицуАналитикиТоваровПоГТД(тчТовары, ТаблицаГТДПоНоменклатуре, Источник);
	...
	Для каждого стрВидыЗапасов Из тзВидыЗапасов Цикл
		Если ТипЗнч(Источник) = Тип("ДокументОбъект.РеализацияТоваровУслуг") Тогда
			стрПоиска = Новый Структура("АналитикаУчетаНоменклатуры, ЗаказКлиента, Количество", 
			стрВидыЗапасов["АналитикаУчетаНоменклатуры"],  стрВидыЗапасов["ЗаказКлиента"], стрВидыЗапасов["Количество"]);
		Иначе
			стрПоиска = Новый Структура("АналитикаУчетаНоменклатуры, Количество", 
			стрВидыЗапасов["АналитикаУчетаНоменклатуры"], стрВидыЗапасов["Количество"]);
		КонецЕсли;
...
// Параметры: Объект - ДокументОбъект.ПеремещениеТоваров
Функция ПолучитьТаблицуГТДПоНоменклатуреКоробов(Объект)
	тз_Короба = Объект.гф_ТоварыВКоробах.Выгрузить();
	Запрос = Новый Запрос();
	Запрос.Текст = "
	|ВЫБРАТЬ
...
Функция ПолучитьТаблицуАналитикиТоваровПоГТД(тчТовары, ТаблицаГТДПоНоменклатуре, Источник)
