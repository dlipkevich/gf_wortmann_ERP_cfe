...
Функция ПолучитьДеталиОперацииОтслеживания(КодОперации, КодАтрибута = Неопределено) Экспорт  

	КодОперацииСтрока = Строка(КодОперации);
	КодАтрибутаСтрока = Строка(КодАтрибута);
	...
	Операция = СоответствиеКодовОпераций[КодОперацииСтрока];     
	...
	Если ЗначениеЗаполнено(Операция) Тогда
		...
		Атрибут = СоответствиеАтрибутов[КодАтрибутаСтрока];
