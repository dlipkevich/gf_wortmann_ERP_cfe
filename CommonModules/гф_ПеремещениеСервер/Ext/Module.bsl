...
Функция ДанныеЗаполненияПриходныйОрдерНаТовары(докПеремещение, ЗаполнениеПоКоробам = Истина, Назначение = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА &УстанавливатьНазначение
	|			ТОГДА &Назначение
	|		ИНАЧЕ ПеремещениеТоваровТовары.Назначение
	...
	|		КОГДА &УстанавливатьНазначение
	|			ТОГДА &Назначение
	|		ИНАЧЕ УпаковочныйЛистТовары.Назначение
	|	КОНЕЦ КАК Назначение,
	...
	УстанавливатьНазначение = ?(Назначение = Неопределено, Ложь, Истина);
	Запрос.УстановитьПараметр("УстанавливатьНазначение", УстанавливатьНазначение);
...
Функция ДанныеЗаполненияРасходныОрдерйНаТовары(
	докПеремещение, 
	Действие, 
	ЗаполнениеПоКоробам = Истина,
	Назначение = Неопределено) Экспорт
