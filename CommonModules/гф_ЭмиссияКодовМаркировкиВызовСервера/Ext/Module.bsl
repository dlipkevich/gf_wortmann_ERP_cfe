...
Функция гф_СоздатьЗаказНаЭмиссиюКодовМаркировки(Основание, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		ДокументОснование = Основание; // Добавлено  Галфинд \ Sakovich 11.03.2024
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПересортицаТоваров") Тогда
		...
		// vvv Галфинд \ Sakovich 11.03.2024
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eedfb23e76a9aa
		ЗаказНаМаркировку.СпособВводаВОборот = Перечисления.СпособыВводаВОборотСУЗ.Импорт;
		// ^^^ Галфинд \ Sakovich 11.03.2024
	...
	// vvv Галфинд \ Sakovich 12.01.2024
	ПринудительноЗаполнятьОтветственного = Ложь;
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		Если ДополнительныеПараметры.Свойство("ПринудительноЗаполнятьОтветственного") Тогда
			ДополнительныеПараметры.Свойство("ПринудительноЗаполнятьОтветственного", ПринудительноЗаполнятьОтветственного);
		КонецЕсли;
	КонецЕсли;	
	// ^^^ Галфинд \ Sakovich 12.01.2024
	...
	ЗаказНаЭмиссиюКодовМаркировкиОтветственныйЗаполнить(
		СтруктураВозврата["ЗаказНаЭмиссиюКодовМаркировкиСУЗ"],
		ПринудительноЗаполнятьОтветственного);   //Добавил фактический параметр  Галфинд \ Sakovich 12.01.2024
	// ++ Галфинд СадомцевСА 23.08.2023
...
Процедура гф_ПодготовитьКПередачеИСМП(МассивВходящихДанных, ЗаказКодовВозможен = Ложь,
	ОграничитьРазмерОчередиСообщений = Истина) Экспорт
	// #wortmann {
	// Параметры:
	...
	//  ОграничитьРазмерОчередиСообщений - булево значение по-умолчанию Истина (СадомцевСА 15.03.2024)
	...
	Для каждого Эл Из МассивВходящихДанных Цикл
		// ++ СадомцевСА 20.12.2023
		Если ОграничитьРазмерОчередиСообщений Тогда
		...
		ОбработатьЭлМассиваВходящихДанных(ВходящиеДанные, Эл, ЗаказКодовВозможен);
...
Процедура ОбработатьЭлМассиваВходящихДанных(ВходящиеДанные, Эл, ЗаказКодовВозможен)
	СтруктураПроверки = ПолучитьСтруктуруПроверки();
	ЗаполнитьДальнейшиеДействияЭмиссии(Эл, СтруктураПроверки);
	// ++ СадомцевСА 23.05.2023
	Если СтруктураПроверки["ЗаказКодовВозможен"] = Неопределено Тогда
...
Функция ЭлементШкСуществует(ЗначениеШтрихкода) Экспорт
	ШкСуществует = Ложь;
	...
	Если Не Результат.Пустой() Тогда
		ШкСуществует = Истина;
	КонецЕсли;
	Возврат ШкСуществует;
КонецФункции 

Процедура ПроверитьСоздатьЭлементыМаркированныйТовар(ЗаказНаЭмиссию = Неопределено) Экспорт
	...
	// Шук 29.02.2024 Отбор по заказу на эмиссию в случае эмиссии кодов по заказу, созданного на основе пересортицы
	//{
	|	&ДОЗаказНаЭмиссию
	//}
	|ИТОГИ ПО
	...
	Если ЗначениеЗаполнено(ЗаказНаЭмиссию) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДОЗаказНаЭмиссию", "И ПулКодовМаркировкиСУЗ.ЗаказНаЭмиссию = &ЗаказНаЭмиссию");
		Запрос.УстановитьПараметр("ЗаказНаЭмиссию", ЗаказНаЭмиссию);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДОЗаказНаЭмиссию", "");
	КонецЕсли;
	//}
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока ВыборкаЗаказы.Следующий() Цикл
			...
			Пока ВыборкаШтрихКод.Следующий() Цикл
				...
				Если ЭлементШкСуществует(ВыборкаШтрихКод.ЗначениеШтрихкода) Тогда
					Продолжить;
				КонецЕсли;
	...
	// vvv Галфинд \ Sakovich 13.02.2024
	// необходимо перепровести документы заказов на эмиссию для создания остатков по регистру "Движение кодов маркировки организаций"
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ПулКодовМаркировкиСУЗ.ДокументОснование КАК Документ.ПриобретениеТоваровУслуг) КАК Поставка,
	|	ПулКодовМаркировкиСУЗ.ЗаказНаЭмиссию КАК ЗаказНаЭмиссию,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПулКодовМаркировкиСУЗ.КодМаркировки) КАК КоличествоКМ
	|ПОМЕСТИТЬ вт_ДокументыЭмиссии
	|ИЗ
	|	РегистрСведений.ПулКодовМаркировкиСУЗ КАК ПулКодовМаркировкиСУЗ
	|ГДЕ
	|	ВЫРАЗИТЬ(ПулКодовМаркировкиСУЗ.ДокументОснование КАК Документ.ПриобретениеТоваровУслуг).гф_ЗаказатьКМ
	|	И НЕ ПулКодовМаркировкиСУЗ.ЗаказНаЭмиссию.ПометкаУдаления
	|
	|СГРУППИРОВАТЬ ПО
	|	ВЫРАЗИТЬ(ПулКодовМаркировкиСУЗ.ДокументОснование КАК Документ.ПриобретениеТоваровУслуг),
	|	ПулКодовМаркировкиСУЗ.ЗаказНаЭмиссию
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	вт_ДокументыЭмиссии.ЗаказНаЭмиссию КАК ЗаказНаЭмиссию
	|ИЗ
	|	вт_ДокументыЭмиссии КАК вт_ДокументыЭмиссии
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.гф_ДвижениеКодовМаркировкиОрганизации КАК гф_ДвижениеКодовМаркировкиОрганизации
	|		ПО вт_ДокументыЭмиссии.ЗаказНаЭмиссию = гф_ДвижениеКодовМаркировкиОрганизации.Регистратор
	|ГДЕ
	|	гф_ДвижениеКодовМаркировкиОрганизации.Регистратор ЕСТЬ NULL
	|	И вт_ДокументыЭмиссии.ЗаказНаЭмиссию.Проведен
	// Шук 29.02.2024 Отбор по заказу на эмиссию в случае эмиссии кодов по заказу, созданного на основе пересортицы
	//{
	|	&ДОЗаказНаЭмиссию";
	//}	
	...
	//{
	Если ЗначениеЗаполнено(ЗаказНаЭмиссию) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДОЗаказНаЭмиссию", "И вт_ДокументыЭмиссии.ЗаказНаЭмиссию = &ЗаказНаЭмиссию");
		Запрос.УстановитьПараметр("ЗаказНаЭмиссию", ЗаказНаЭмиссию);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДОЗаказНаЭмиссию", "");
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		обЗаказНаЭмиссию = Выборка["ЗаказНаЭмиссию"].ПолучитьОбъект();
		Если обЗаказНаЭмиссию <> Неопределено Тогда
			Попытка
				обЗаказНаЭмиссию.Записать(РежимЗаписиДокумента.Проведение);
			Исключение
				ТекстОшибки = ОписаниеОшибки();
				СтрокаСообщения = "Не удалось провести документ "+ Выборка["ЗаказНаЭмиссию"] +
				" Ошибка: " + ТекстОшибки;
				ЗаписьЖурналаРегистрации("Документ.ЗаказНаЭмиссиюКодовМаркировкиСУЗ.ОшибкаЗаписи", 
				УровеньЖурналаРегистрации.Информация, , , СтрокаСообщения);
			КонецПопытки;
...
Процедура гф_СтатусыДокументовИСМППриЗаписиПоПереоценкеПриЗаписи(Источник, Отказ, Замещение) Экспорт
	...
	Для каждого Запись Из Источник Цикл
		Если ТипЗнч(Запись.Документ) = Тип("ДокументСсылка.ЗаказНаЭмиссиюКодовМаркировкиСУЗ")
			...
			гф_ЭмиссияКодовМаркировкиВызовСервера.ПроверитьСоздатьЭлементыМаркированныйТовар(Запись.Документ);
			//}
			Прервать;	
		КонецЕсли;
	КонецЦикла; 
...
Процедура ЗаказНаЭмиссиюКодовМаркировкиОтветственныйЗаполнить(ЗаказНаЭмиссиюКМ, ПринудительноЗаполнятьОтветственного = Ложь)
	  // Галфинд \ Sakovich 12.01.2024 - добавил параметр "ПринудительноЗаполнятьОтветственного"
	  // Галфинд СадомцевСА 23.08.2023
	// Реализовал заполнение Ответственного в новом документе Заказ на эмиссию КМ при выполнении регл. задания
	// (при необходимости)
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee41a172d18e5a
	
	Если ЗначениеЗаполнено(ЗаказНаЭмиссиюКМ.Ответственный)
		// vvv Галфинд \ Sakovich 12.01.2024
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeb064fff34737
		// в некоторых случаях хотят принудительно устанавливать ответственного из сертификата
		И Не (ПринудительноЗаполнятьОтветственного = Истина)
		// ^^^ Галфинд \ Sakovich 12.01.2024
		Тогда
...
Процедура гф_ЗаменитьСкладДвижениеКодовМаркировкиОрганизации(ЗаказНаЭмиссию)
	...
	// vvv Галфинд \ Sakovich 20.04.2024
	//СкладЭмиссииРЗ = _омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначение("гф_СкладЭмиссииРЗ");
	СкладЭмиссииРЗ = Неопределено;
	ДокументОснование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаказНаЭмиссию, "ДокументОснование");
