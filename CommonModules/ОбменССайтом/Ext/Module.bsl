...
Функция ПрямаяВыгрузкаВJSONНаСайт(Параметры, СтрокаТаблицы, ТаблицаИнформации)
	...
	Для сч = 0 по ТипыЦен.Количество()-1 Цикл
		...
		Запрос.УстановитьПараметр("Объект",Параметры.УзелОбмена.ВладелецКаталога);
		Запрос.УстановитьПараметр("Свойство",Справочники.B2B_w_Настройки.СвойствоGUID_УПП.Значение);
...
Процедура ЗаполнитьМассивOffersПредложения(Параметры, МассивOffersПредложения, СтруктураOffersПредложения, СписокСвойствХарактеристик, ТаблицаКартинок, НомерПакета)
	...
	для каждого стр из Параметры.Товары цикл
		...
		Для каждого стрХарактеристика из стр.Характеристики цикл
			...
			ПолучитьМассивСвойств(МассивСвойств, стр.Номенклатура, СписокСвойствХарактеристик, Параметры.тзСвойств,Строка(стрХарактеристика.Характеристика));
		...
		ИначеЕсли (Структ.ТипЗначений <> "Справочник" и стр.ВыгружатьВХИ) Тогда
			...
			Пока ВыборкаЗначенийСвойств.Следующий() Цикл     
				...
				Исключение                                                                   
					...
					СтруктЗначений.Значение		= СокрЛП(Лев(ВыборкаЗначенийСвойств.Значение, 1000));
				Конецпопытки;
				МассивЗначений.Добавить(СтруктЗначений);   
			КонецЦикла;	
			Структ.ВариантыЗначений = МассивЗначений;
		ИначеЕсли Структ.ТипЗначений = "Справочник" Тогда
			...
			Если ТипЗначенияСправочник = "ЗначенияСвойствОбъектов" тогда
				...
				Пока ВыборкаЗначенийСвойств.Следующий() Цикл
					...
					СтруктЗначений.Значение	  = Лев(ВыборкаЗначенийСвойств.Значение.Наименование, 1000);
					МассивЗначений.Добавить(СтруктЗначений);
				КонецЦикла;
			Иначе
				...
				Если Ложь Тогда
					...
					Пока ВыборкаЗначенийСвойств.Следующий() Цикл
						Если ЗначениеЗаполнено(ВыборкаЗначенийСвойств.Объект) тогда  
							...
							СтруктЗначений.Значение	  = Лев(ВыборкаЗначенийСвойств.Наименование, 1000);
							МассивЗначений.Добавить(СтруктЗначений);
						КонецЕсли;	
					КонецЦикла;
				КонецЕсли;
...
Процедура ПолучитьМассивСвойств(МассивСвойств, Номенклатура, Свойства, тзСвойств, ЗаменитьРазмерХарактеристики = "") 
	...
	Пока рез.Следующий() цикл        
		...
		Иначе
			Структ = Новый Структура("Ид,Значение",Ид,Строка(рез.Значение));   
		КонецЕсли;
