...
Процедура ЗаполнитьМассивPricesПакетПредложенийПредложения(Параметры, МассивPricesПакетПредложенийПредложения, СтруктураPricesПакетПредложенийПредложения, НомерПакета)
	...
	|ИЗ
	|	РегистрНакопления.гф_ДвижениеКодовМаркировкиОрганизации.Остатки(, КМ В (ВЫБРАТЬ Т.КМ ИЗ ВтШтрихкоды КАК Т)) КАК ДвижениеКодовМаркировкиОрганизацииОстатки
	|
	|СГРУППИРОВАТЬ ПО 
	...
	для каждого стр из Параметры.Товары цикл  
		...
		|
		|ГДЕ
        |	УпаковочныйЛист.гф_Комплектация.Владелец = &Владелец
