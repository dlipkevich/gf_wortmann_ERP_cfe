...
Процедура B2B_ВыполнитьОбменССайтомЧерезJSON(Параметры, РезультатОбмена, ТаблицаИнформации) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаКаталогов Цикл
		...
		Если Параметры.УзелОбмена.B2B_ОбменJSON Тогда
			...
			Успешно = ПрямаяВыгрузкаВJSONНаСайт(Параметры, СтрокаТаблицы, ТаблицаИнформации, РезультатОбмена);
	...
	Иначе
		...
		Если Успешно Тогда
			...
			ТоварыКУдалениюРегистрации = Новый Массив;
			Для Каждого ЭлементМассива Из Параметры.СтруктураИзменений.Товары Цикл
				
				Если РезультатОбмена.Свойство("ВыгруженныеТовары")
					И РезультатОбмена["ВыгруженныеТовары"].Найти(ЭлементМассива) = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				ТоварыКУдалениюРегистрации.Добавить(ЭлементМассива);
				
			КонецЦикла;
			
			Если ЗначениеЗаполнено(ТоварыКУдалениюРегистрации) Тогда
				ПланыОбмена.УдалитьРегистрациюИзменений(Параметры.УзелОбмена, ТоварыКУдалениюРегистрации);
			КонецЕсли;
...
Функция ПрямаяВыгрузкаВJSONНаСайт(Параметры, СтрокаТаблицы, ТаблицаИнформации, РезультатОбмена)
	...
	ОтборСкладыДляОстатков = Неопределено;
	...
	Для каждого ЭлементОтбора из ОтборКаталога Цикл
		...
		ИначеЕсли Строка(ЭлементОтбора.ЛевоеЗначение) = "Номенклатура" И ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке Тогда
			ОтборНоменклатуры = ЭлементОтбора.ПравоеЗначение;
			
		ИначеЕсли Строка(ЭлементОтбора.ЛевоеЗначение) = "Склад" Тогда
			ОтборСкладыДляОстатков = ЭлементОтбора.ПравоеЗначение; 
			
		КонецЕсли;
		
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(ОтборСкладыДляОстатков) Тогда
		
		ЗапросСкладов = Новый Запрос;
		ЗапросСкладов.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СкладыОстатков.Склад КАК Склад
		|ИЗ
		|	РегистрСведений.B2B_СоответствиеСкладВидНоменклатуры КАК СкладыОстатков";
		ОтборСкладыДляОстатков = ЗапросСкладов.Выполнить().Выгрузить().ВыгрузитьКолонку("Склад"); 
		
	КонецЕсли;
	 	
	РезультатОбмена.Вставить("ВыгруженныеТовары", Новый Массив);;
	...
	Для Каждого стр из Параметры.СтруктураИзменений.Товары Цикл   
		...
		Если ЗначениеЗаполнено(ОтборНоменклатуры) И ОтборНоменклатуры.НайтиПоЗначению(стр) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ГруппаНаСайтеОсновная = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(стр, "B2B_ГруппаНаСайтеОсновная"); 
		Если Не ЗначениеЗаполнено(ГруппаНаСайтеОсновная) Тогда
			Продолжить;
		КонецЕсли;
		
		ИдентификаторРазделаНаСайте = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаНаСайтеОсновная, "ИдентификаторРаздела");
		Если Не ЗначениеЗаполнено(ИдентификаторРазделаНаСайте) Тогда
			Продолжить;
		КонецЕсли;
		
		РезультатОбмена["ВыгруженныеТовары"].Добавить(стр);
		...
		АртикулНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НовСтр.Номенклатура, "Артикул");
		ЭлементИсторическогоСправочника = Справочники.B2B_СЭ_ТипНоменклатуры.НайтиПоРеквизиту("Артикул", АртикулНоменклатуры);
		
		Если ЗначениеЗаполнено(ЭлементИсторическогоСправочника) Тогда
			НовСтр.ИД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлементИсторическогоСправочника, "GUID");    
	...
	тзТиповЦен.Колонки.Добавить("ИД", ОбщегоНазначения.ОписаниеТипаСтрока(50));
	тзТиповЦен.Колонки.Добавить("Наименование", ОбщегоНазначения.ОписаниеТипаСтрока(128));
	тзТиповЦен.Колонки.Добавить("ЦенаВключаетНДС");
	тзТиповЦен.Колонки.Добавить("ВалютаЦеныНаименование", ОбщегоНазначения.ОписаниеТипаСтрока(16));
	...
	|	СправочникВидыЦен.ВалютаЦены.Наименование КАК ВалютаЦеныНаименование,
	|	ЕСТЬNULL(РегистрСоответствия.Идентификатор, УНИКАЛЬНЫЙИДЕНТИФИКАТОР(СправочникВидыЦен.Ссылка)) КАК ИД,
	|	ЕСТЬNULL(РегистрСоответствия.Наименование, СправочникВидыЦен.Наименование) КАК Наименование
	|ИЗ
	|	Справочник.ВидыЦен КАК СправочникВидыЦен
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.B2B_ИдентификаторыТиповЦен КАК РегистрСоответствия
	|		ПО СправочникВидыЦен.Ссылка = РегистрСоответствия.ВидЦен
	|			И (РегистрСоответствия.Организация = &Организация)
	|ГДЕ
	|	СправочникВидыЦен.Ссылка В(&ТипыЦен)";
	...
	Пока ВыборкаВидовЦен.Следующий() Цикл
		...
		СтрокаТиповЦен = тзТиповЦен.Добавить();
		
		ЗаполнитьЗначенияСвойств(СтрокаТиповЦен, ВыборкаВидовЦен);
		
		Если Не ЗначениеЗаполнено(СтрокаТиповЦен["Наименование"]) Тогда
			СтрокаТиповЦен["Наименование"] = ВыборкаВидовЦен["ВидЦеныНаименование"];	
		КонецЕсли;
	...
	СписокСвойствВыгружаемыхКакСписки = Новый СписокЗначений;
	//Список взят из старой базы, по хорошему надо в реквизит вытащить и настраивать в пользовательском режиме
	СписокСвойствВыгружаемыхКакСписки.Добавить(ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("B2B_Size"));
	СписокСвойствВыгружаемыхКакСписки.Добавить(Справочники.B2B_w_Настройки.Свойство_B2B_РазмерныйРяд.Значение);
	//СписокСвойствВыгружаемыхКакСписки.Добавить(ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("MainColorCode")); 
	...
	НепереводимыеСвойства = Новый Массив;
	НепереводимыеСвойства.Добавить(ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("B2B_Предзаказ"));
	НепереводимыеСвойства.Добавить(ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("B2B_Каталог"));
	НепереводимыеСвойства.Добавить(ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("ИМ_СтранаПроизводства"));
	НепереводимыеСвойства.Добавить(ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("B2B_Size"));
	...
	тзСвойств.Колонки.Добавить("ТипЗначений");
	
	ЗапросИдентификаторов = Новый Запрос;
	ЗапросИдентификаторов.Текст =
	"ВЫБРАТЬ
	|	B2B_РИ_ИдентификаторыСвойствНоменклатурыБитрикс.Объект КАК Объект,
	|	B2B_РИ_ИдентификаторыСвойствНоменклатурыБитрикс.Идентификатор КАК Идентификатор,
	|	B2B_РИ_ИдентификаторыСвойствНоменклатурыБитрикс.ТипЗначений КАК ТипЗначений,
	...
	ТаблицаИдентификаторов = ЗапросИдентификаторов.Выполнить().Выгрузить();
	...
	Для Сч = 0 по СписокСвойствДляВыгрузки.Количество()-1 Цикл
		...
		ВыгружатьКакСписок = СписокСвойствВыгружаемыхКакСписки.НайтиПоЗначению(СписокСвойствДляВыгрузки[сч].Значение);
		НоваяСтрока.ВыгружатьВХИ = ВыгружатьКакСписок <> Неопределено;
		
		ВыгружатьКакСправочник = СписокСвойствВыгружаемыхВHLСправочники.НайтиПоЗначению(СписокСвойствДляВыгрузки[сч].Значение);
		НоваяСтрока.ВлияетНаЦену = ВыгружатьКакСправочник <> Неопределено;           
		
		МассивНайденных = ТаблицаИдентификаторов.НайтиСтроки(Новый Структура("Объект", НоваяСтрока.Свойство));
		Если МассивНайденных.Количество() = 1 Тогда
			СтрокаИдентификаторов = МассивНайденных[0];
			НоваяСтрока.ИД           = СтрокаИдентификаторов["Идентификатор"];
			НоваяСтрока.Наименование = СтрокаИдентификаторов["Наименование"];
			НоваяСтрока.ТипЗначений  = СтрокаИдентификаторов["ТипЗначений"];
		Иначе
			НоваяСтрока.ИД = Строка(НоваяСтрока.Свойство.УникальныйИдентификатор());
			НоваяСтрока.Наименование = НоваяСтрока.свойство.Наименование;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(НоваяСтрока["ТипЗначений"]) Тогда
			
			Если ВыгружатьКакСписок <> Неопределено Тогда
				НоваяСтрока["ТипЗначений"] = "Список";
				
			ИначеЕсли ВыгружатьКакСправочник <> Неопределено Тогда
				НоваяСтрока["ТипЗначений"] = "Справочник";
	...
	тзСвойствПредложений.Колонки.Добавить("ТипЗначений");
	...
	Для Каждого Свойство из СвойстваДляПредложений Цикл
		...
		ВыгружатьКакСписок = СписокСвойствВыгружаемыхКакСписки.НайтиПоЗначению(НоваяСтрока.Свойство);
		НоваяСтрока.ВыгружатьВХИ = ВыгружатьКакСписок <> Неопределено;
		
		ВыгружатьКакСправочник = СписокСвойствВыгружаемыхВHLСправочники.НайтиПоЗначению(НоваяСтрока.Свойство);
		НоваяСтрока.ВлияетНаЦену = ВыгружатьКакСправочник <> Неопределено;           
	...
	ПараметрыДляJSONОбмена.Вставить("НепереводимыеСвойства", НепереводимыеСвойства);
	
	ПараметрыДляJSONОбмена.Вставить("КоллекцияНоменклатурыСвойство", ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Supplier_season"));
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	КоллекцииНоменклатуры.Код КАК Код,
	|	КоллекцииНоменклатуры.Наименование КАК Наименование
	|ИЗ
	|	Справочник.КоллекцииНоменклатуры КАК КоллекцииНоменклатуры";
	Выборка = Запрос.Выполнить().Выбрать();
	КоллекцииНоменклатуры = Новый Соответствие;
	Пока Выборка.Следующий() Цикл
		КоллекцииНоменклатуры.Вставить(СокрЛП(Выборка["Код"]), СокрЛП(Выборка["Наименование"]));
	КонецЦикла;
	ПараметрыДляJSONОбмена.Вставить("КоллекцииНоменклатуры", КоллекцииНоменклатуры);
	
	ПараметрыДляJSONОбмена.Вставить("ОтборСкладыДляОстатков", ОтборСкладыДляОстатков);
...
Процедура ВыгрузитьКартинки(Параметры, ТаблицаКартинок, НомерПакета)
	...
	для каждого стр из Параметры.Товары Цикл
		...
		РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(стр.Номенклатура, "Артикул, ФайлКартинки");
		
		УжеВыгруженнаяКартинка = ТаблВыгруженныхКартинок.Найти(РеквизитыНоменклатуры.Артикул);
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(РеквизитыНоменклатуры.ФайлКартинки) Тогда
				Продолжить;
			КонецЕсли;
			
			Попытка
			
				АдресКартинки = ВыгрузитьКартинку(ИмяКаталога, стр.Номенклатура, РеквизитыНоменклатуры.ФайлКартинки);
				
				Если ЗначениеЗаполнено(АдресКартинки) Тогда
					НовСтр = ТаблВыгруженныхКартинок.Добавить();
					НовСтр.Артикул = РеквизитыНоменклатуры.Артикул;
			...
			Исключение
				Сообщить("Не удалось выгрузить картинку номенклатуры " + стр.Номенклатура + ", ошибка: " + ОписаниеОшибки());
				Продолжить;	
			КонецПопытки;
...
Процедура ЗаполнитьМассивOffersПредложения(Параметры, МассивOffersПредложения, СтруктураOffersПредложения, СписокСвойствХарактеристик, ТаблицаКартинок, НомерПакета)
	...
	для каждого стр из Параметры.Товары цикл
		...
		Для каждого стрХарактеристика из стр.Характеристики цикл
			...
			ПолучитьМассивСвойств(Параметры, МассивСвойств, стр.Номенклатура, СписокСвойствХарактеристик, Параметры.СвойстваДляПредложений,
				Строка(стрХарактеристика.Характеристика), стрХарактеристика.ВариантКомплектации);
...
Процедура ЗаполнитьМассивImportСвойства(Параметры, МассивImportСвойства,СтруктураImportСвойства, СписокСвойств, ФильтрТаблицаСвойств = Неопределено)
	...
	СвойствоB2B_Size = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("B2B_Size");
	...
	для каждого стр из тзСвойств Цикл
		...
		Структ.ТипЗначений = стр.ТипЗначений; 
		
		//Структ.Внешний = стр.ВыгружатьВХИ Или стр.ВлияетНаЦену;
		Структ.Внешний = Ложь; 
		
		лУчитываетсяВЦене			= ?(ЗначениеЗаполнено(стр.ВлияетНаЦену), стр.ВлияетНаЦену, Ложь);
		Структ.Информационное = НЕ лУчитываетсяВЦене;     
		
		Если Не ЗначениеЗаполнено(Структ.ТипЗначений) Тогда
		...
		ИначеЕсли Структ.ТипЗначений = "Дата" Тогда
			...
			Структ.Удалить("ВариантыЗначений");
			Структ.Внешний = Истина;
			
		ИначеЕсли Структ.ТипЗначений = "Булево" Тогда
			
			Структ.ТипЗначений = "Список";
			Структ.Внешний = Ложь;
			
			Если Структ.Свойство("ВариантыЗначений") Тогда
		...
		//ИначеЕсли Структ.ТипЗначений = "Список" Или Структ.ТипЗначений = "Справочник" Тогда 
		//	Структ.Внешний = (Структ.ТипЗначений = "Справочник");
		
		ИначеЕсли Структ.ТипЗначений = "Список" Тогда 
			Структ.Внешний = Ложь;
			
			Если Не Структ.Свойство("ВариантыЗначений") Тогда
				
				Структ.Вставить("ВариантыЗначений", Новый Массив);
				
			Иначе
			
				МассивЗначений = Новый Массив;
				
				Если стр.Свойство = Справочники.B2B_w_Настройки.Свойство_B2B_РазмерныйРяд.Значение Тогда
					
					РазмерныеРяды = РазличныеРазмерныеРяды(Параметры.B2B_Портал);
					Для Каждого РазмерныйРядСтрокой Из РазмерныеРяды Цикл
						
						СтруктЗначений = ЗначениеИзСтрокиВнутр(ЗначениеВСтрокуВнутр(Структ.ВариантыЗначений));
						
						СтруктЗначений.ИдЗначения = РазмерныйРядСтрокой;
						СтруктЗначений.Значение   = РазмерныйРядСтрокой;
				...
				Иначе
					...
					|	И ТИПЗНАЧЕНИЯ(ЗначенияСвойствОбъектов.Объект) = ТИП(Справочник.Номенклатура)
					|	И НЕ ВЫРАЗИТЬ(ЗначенияСвойствОбъектов.Объект КАК Справочник.Номенклатура).ПометкаУдаления
					|	И ВЫРАЗИТЬ(ЗначенияСвойствОбъектов.Объект КАК Справочник.Номенклатура).B2B_Портал  = &НужныйB2BПортал
					|
					|ОБЪЕДИНИТЬ ВСЕ
					|
					|ВЫБРАТЬ РАЗЛИЧНЫЕ
					|	СправочникХарактеристики.Наименование КАК Значение
					|ИЗ
					|	Справочник.ХарактеристикиНоменклатуры КАК СправочникХарактеристики
					|ГДЕ
					|	&B2B_Size	
					|	И НЕ СправочникХарактеристики.ПометкаУдаления
					|	И ТИПЗНАЧЕНИЯ(СправочникХарактеристики.Владелец) <> ТИП(Справочник.Номенклатура)
					|;
					|
					|////////////////////////////////////////////////////////////////////////////////
					|ВЫБРАТЬ РАЗЛИЧНЫЕ
					...
					Запрос.УстановитьПараметр("B2B_Size", стр.Свойство = СвойствоB2B_Size);
					
					ПереводитьСвойство = Истина;
					Если Параметры.Свойство("НепереводимыеСвойства")
						И Параметры["НепереводимыеСвойства"].Найти(стр.Свойство) <> Неопределено Тогда
						ПереводитьСвойство = Ложь;
					КонецЕсли; 
					
					ЭтоСвойствоКоллекцияНоменклатуры = Ложь;
					Если Параметры.Свойство("КоллекцияНоменклатурыСвойство") И Параметры["КоллекцияНоменклатурыСвойство"] = стр.Свойство Тогда
						ЭтоСвойствоКоллекцияНоменклатуры = Истина;
					КонецЕсли; 
					...
					Пока ВыборкаЗначенийСвойств.Следующий() Цикл     
						...
						//	СтруктЗначений.Значение		= Число(Лев(ВыборкаЗначенийСвойств.Значение, 64));
						//Исключение                                                                   
						...
						ЗначениеСвойства = СокрЛП(ВыборкаЗначенийСвойств.Значение); 
						Если ЭтоСвойствоКоллекцияНоменклатуры И ЗначениеЗаполнено(ЗначениеСвойства) Тогда
							
							ЗначениеСвойстваНаименование = Параметры["КоллекцииНоменклатуры"].Получить(ЗначениеСвойства);
							
							Если ЗначениеЗаполнено(ЗначениеСвойстваНаименование) Тогда
								ЗначениеСвойства = ЗначениеСвойстваНаименование;	
							КонецЕсли;
							
						Иначе
							
							Попытка
								ЗначениеСвойстваЧисловое = Число(ЗначениеСвойства);
								ПереводитьСвойство = Ложь;
							Исключение                                                                   
								ЗначениеСвойства = СокрЛП(ЗначениеСвойства);
							Конецпопытки;
							
							Если ПереводитьСвойство Тогда
								ЗначениеСвойства = ПереводСвойства(ЗначениеСвойства);
							КонецЕсли;
							
						КонецЕсли;
						
						СтруктЗначений.Значение	= ЗначениеСвойства; 
...
Функция РазличныеРазмерныеРяды(B2B_Портал) 
	
	РазмерныеРяды = Новый Массив;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВариантыКомплектацииНоменклатурыТовары.Ссылка.Владелец КАК Владелец,
	|	ВариантыКомплектацииНоменклатурыТовары.Характеристика.Наименование КАК Характеристика,
	|	СУММА(ВариантыКомплектацииНоменклатурыТовары.Количество) КАК Количество
	|ИЗ
	|	Справочник.ВариантыКомплектацииНоменклатуры.Товары КАК ВариантыКомплектацииНоменклатурыТовары
	|ГДЕ
	|	ВариантыКомплектацииНоменклатурыТовары.Ссылка.Владелец.B2B_Портал = &B2B_Портал
	|	И НЕ ВариантыКомплектацииНоменклатурыТовары.Ссылка.ПометкаУдаления
	|	И НЕ ВариантыКомплектацииНоменклатурыТовары.Характеристика.ПометкаУдаления
	|
	|СГРУППИРОВАТЬ ПО
	|	ВариантыКомплектацииНоменклатурыТовары.Ссылка.Владелец,
	|	ВариантыКомплектацииНоменклатурыТовары.Характеристика.Наименование
	|
	|УПОРЯДОЧИТЬ ПО
	|	Владелец,
	|	Характеристика
	...
	Пока ВыборкаВладелец.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			РазмерныйРядСтрокой = РазмерныйРядСтрокой + Выборка.Характеристика + "-" + Строка(Выборка.Количество)+"|";
		КонецЦикла;
		
		Если РазмерныеРяды.Найти(РазмерныйРядСтрокой) = Неопределено Тогда
			РазмерныеРяды.Добавить(РазмерныйРядСтрокой);	
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат РазмерныеРяды;
	
КонецФункции

Функция РазмерныйРядВариантаКомплектации(ВариантКомплектации) 
...
Процедура ЗаполнитьМассивImportТовары(Параметры, МассивImportТовары, СтруктураImportТовары, СписокСвойств, ТаблицаКартинок, НомерПакета)
	...
	для каждого стр из Параметры.Товары цикл    
		...
		МассивСвойств = Новый Массив;                          
		ПолучитьМассивСвойств(Параметры, МассивСвойств, стр.Номенклатура, СписокСвойств, Параметры.тзСвойств);
		Структ.ЗначенияСвойств = МассивСвойств;
...
Процедура ПолучитьМассивГрупп(МассивГрупп, Номенклатура, СЭ_ТипНоменклатуры) 
	...
	ГруппаНаСайтеОсновная = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "B2B_ГруппаНаСайтеОсновная"); 
	Если ЗначениеЗаполнено(ГруппаНаСайтеОсновная) Тогда
		ИдентификаторРазделаНаСайте = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаНаСайтеОсновная, "ИдентификаторРаздела");
		Если ЗначениеЗаполнено(ИдентификаторРазделаНаСайте) И МассивГрупп.Найти(ИдентификаторРазделаНаСайте) = Неопределено Тогда
			МассивГрупп.Добавить(ИдентификаторРазделаНаСайте);	
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолучитьМассивСвойств(Параметры, МассивСвойств, Номенклатура, Свойства, тзСвойств, ЗаменитьРазмерХарактеристики = "", ВариантКомплектации = "") 
	...
	Пока рез.Следующий() цикл 
		...
		Если рез.Свойство = Справочники.B2B_w_Настройки.Свойство_B2B_РазмерныйРяд.Значение Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ЗаменитьРазмерХарактеристики <> "" И Ид = "B2B_Size" Тогда   
			Продолжить;
		КонецЕсли;
		
		ПереводитьСвойство = Истина;
		Если Параметры.Свойство("НепереводимыеСвойства")
			И Параметры["НепереводимыеСвойства"].Найти(рез.Свойство) <> Неопределено Тогда
			ПереводитьСвойство = Ложь;
		КонецЕсли; 
		
		ЭтоСвойствоКоллекцияНоменклатуры = Ложь;
		Если Параметры.Свойство("КоллекцияНоменклатурыСвойство") И Параметры["КоллекцияНоменклатурыСвойство"] = рез.Свойство Тогда
			ЭтоСвойствоКоллекцияНоменклатуры = Истина;
		КонецЕсли; 
		
		Структ = Новый Структура;
		Структ.Вставить("Ид", Ид);
		
		ЗначениеСвойства = СокрЛП(рез.Значение);
		...
		Структ.Вставить("Значение", ЗначениеСвойства);
		
		МассивСвойств.Добавить(Структ);
		
	КонецЦикла;
	
	Если ЗаменитьРазмерХарактеристики <> "" Тогда   
		Структ = Новый Структура("Ид, Значение", "B2B_Size", Строка(ЗаменитьРазмерХарактеристики));
		МассивСвойств.Добавить(Структ);
	КонецЕсли;
	...
	Если ВариантКомплектации <> "" Тогда
		...
		//КонецЦикла;
		РазмерныйРядСтрокой = РазмерныйРядВариантаКомплектации(ВариантКомплектации);	
		
		Структ = Новый Структура("Ид,Значение", тзСвойств.Найти(Справочники.B2B_w_Настройки.Свойство_B2B_РазмерныйРяд.Значение).ИД, РазмерныйРядСтрокой);
		МассивСвойств.Добавить(Структ); 
...
Процедура ЗаполнитьМассивImportКлассификаторТипыЦен(Параметры, МассивImportКлассификаторТипыЦен,СтруктураImportКлассификаторТипыЦен)	
	...
	для каждого стрТЦ из Параметры.ТипыЦен цикл
		...
		Структ.Ид = стрТЦ.ИД;
		Структ.Наименование = стрТЦ.Наименование;
		Структ.Валюта = стрТЦ.ВалютаЦеныНаименование;
		
		Структ.Налог.Наименование = "НДС";
		Структ.Налог.УчтеноВСумме = стрТЦ.ЦенаВключаетНДС;
...
Процедура ЗаполнитьМассивOffersСвойства(Параметры, МассивOffersСвойства,СтруктураOffersСвойства, СписокСвойствХарактеристик)
	...
	НоваяКолонка = ФильтрТаблицаСвойств.Колонки.Добавить("ТипЗначений"); 
	
	Для каждого стр из Параметры.СвойстваДляПредложений Цикл
		
		НоваяСтрока = ФильтрТаблицаСвойств.Добавить(); 
		ЗаполнитьЗначенияСвойств(НоваяСтрока,стр,"Свойство,ИД,Наименование,ТипЗначений");
...
Процедура ЗаполнитьМассивPricesПакетПредложенийПредложения(Параметры, МассивPricesПакетПредложенийПредложения, СтруктураPricesПакетПредложенийПредложения, НомерПакета)
	...
	МассивНоменклатуры = Новый Массив;
	Для Каждого СтрокаТоваров Из Параметры.Товары Цикл
	...
	|		И &ОтборСкладыДляОстатков) КАК ДвижениеКодовМаркировкиОрганизацииОстатки
	|
	|СГРУППИРОВАТЬ ПО 
	...
	Если ЗначениеЗаполнено(Параметры["ОтборСкладыДляОстатков"]) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборСкладыДляОстатков", "Склад В ИЕРАРХИИ(&ОтборСкладыДляОстатков)");
		Запрос.УстановитьПараметр("ОтборСкладыДляОстатков", Параметры["ОтборСкладыДляОстатков"]);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборСкладыДляОстатков", "ИСТИНА");	
	КонецЕсли;
	
	Запрос.Выполнить();
	
	// ОстаткиБезКоробов
	ВыбратьОстаткиБезКоробов = Истина;
	Если ВыбратьОстаткиБезКоробов Тогда
		ЗапросОстатковБезКоробов = Новый Запрос;
		ЗапросОстатковБезКоробов.Текст =
		"ВЫБРАТЬ
		...
		ЗапросОстатковБезКоробов.УстановитьПараметр("МассивНоменклатуры", МассивНоменклатуры); 
		
		СкладыБезКоробов = B2B_ОбщиеПроцедурыИФункции.СкладыБезКоробов(Параметры["ОтборСкладыДляОстатков"]);
		ЗапросОстатковБезКоробов.УстановитьПараметр("СкладыБезКоробов", СкладыБезКоробов);
		
		тзОстатковБезКоробов = ЗапросОстатковБезКоробов.Выполнить().Выгрузить();
	КонецЕсли;
	
	// ТаблицаЦен
	МассивВидовЦен = Новый Массив;
	Для Каждого СтрокаТиповЦен из Параметры.ТипыЦен Цикл
		МассивВидовЦен.Добавить(СтрокаТиповЦен.ВидЦены);	
	КонецЦикла;
	...
	ЗапросЦен.УстановитьПараметр("МассивВидовЦен", МассивВидовЦен);
	ЗапросЦен.Текст =
	"ВЫБРАТЬ
	|	ЦеныНоменклатуры25СрезПоследних.Номенклатура КАК Номенклатура,
	|	ЦеныНоменклатуры25СрезПоследних.ВидЦены КАК ВидЦены,
	|	ЦеныНоменклатуры25СрезПоследних.ВидЦены.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	МАКСИМУМ(ЦеныНоменклатуры25СрезПоследних.Цена) КАК Цена
	|ИЗ
	|	РегистрСведений.ЦеныНоменклатуры25.СрезПоследних(
	|			,
	|			ВидЦены В (&МассивВидовЦен)
	|				И Номенклатура В (&МассивНоменклатуры)) КАК ЦеныНоменклатуры25СрезПоследних
	|
	|СГРУППИРОВАТЬ ПО
	...
	для каждого стр из Параметры.Товары цикл  
		...
		Если Не ЗначениеЗаполнено(тзХарактеристик) Тогда
			Запрос.Текст =
			"ВЫБРАТЬ
			...
			|		ВЫБОР
			|			КОГДА НЕ СправочникХарактеристики.Ссылка ЕСТЬ NULL
			|			ТОГДА ""/"" + СправочникХарактеристики.Наименование	
			|			ИНАЧЕ """"
			...
			|			ПО СправочникНоменклатура.Ссылка = СправочникХарактеристики.Владелец 
			|ГДЕ
			|	СправочникНоменклатура.Ссылка = &Владелец";
			тзХарактеристик = Запрос.Выполнить().Выгрузить();
		КонецЕсли;
		...
		//|	СУММА(ЕСТЬNULL(гф_ДвижениеКодовМаркировкиОрганизацииОстатки.КоличествоОстаток, ВТ.Остаток)) КАК Остаток,
		//|	МАКСИМУМ(ВТ.Цена) КАК Цена,
		//|	СУММА(ВТ.Количество) КАК КоличествоПарВНаборе,
		//|	ВТ.НаименованиеВарианта КАК НаименованиеВарианта,
		//|	ВТ.ВариантКомплектации КАК ВариантКомплектации
		//|ИЗ
		//|	ВТ КАК ВТ
		//|		ЛЕВОЕ СОЕДИНЕНИЕ ВтДвижениеКодовМаркировкиОстатки КАК гф_ДвижениеКодовМаркировкиОрганизацииОстатки
		//|		ПО ВТ.Номенклатура = гф_ДвижениеКодовМаркировкиОрганизацииОстатки.Номенклатура
		//|			И ВТ.Характеристика = гф_ДвижениеКодовМаркировкиОрганизацииОстатки.Характеристика
		//|
		//|СГРУППИРОВАТЬ ПО
		//|	ВТ.Номенклатура,
		//|	ВТ.Характеристика,
		//|	ВТ.НаименованиеВарианта,
		//|	ВТ.ВариантКомплектации
		//|;
		//|
		//|////////////////////////////////////////////////////////////////////////////////
		//|УНИЧТОЖИТЬ ВТ";
		...
		ЗапросСвободныхОстатков.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
		ЗапросСвободныхОстатков.УстановитьПараметр("Владелец",стр.Номенклатура); 
		ЗапросСвободныхОстатков.УстановитьПараметр("СостоянияКоробов", B2B_ОбщиеПроцедурыИФункции.СостоянияКоробовДляРезервирования());  
		...
		|
		|ГДЕ
        |	ДокументУпаковочныйЛист.гф_Комплектация.Владелец = &Владелец
		...
		Для каждого стрТЦ из Параметры.ТипыЦен Цикл 
			...
			НайденныеСтрокиЦен = ТаблицаЦен.НайтиСтроки(Новый Структура("ВидЦены, Номенклатура", стрТЦ.ВидЦены, стр.Номенклатура));
			Если НайденныеСтрокиЦен.Количество() > 0 Тогда
				СтрокаЦен = НайденныеСтрокиЦен[0];
				Цена = СтрокаЦен["Цена"];
				ЦенаВключаетНДС = СтрокаЦен["ЦенаВключаетНДС"];
			Иначе
				Цена = 0;
			...
			Если ЗначениеЗаполнено(Цена) Тогда
				...
				СтруктЦена.Вставить("Представление", Строка(Цена)+" руб."); 
				СтруктЦена.Вставить("ЦенаЗаЕдиницу", Цена); 
				
				СтруктЦена.Вставить("Валюта","руб");
				
				СтруктНалог = Новый Структура("Наименование, УчтеноВСумме", "НДС", ЦенаВключаетНДС);
		...
		ВыгруженныеИдентификаторы = Новый Массив;
		Для Каждого стрХарактеристика из стр.Характеристики Цикл  
			
			//130623
			Отб = Новый Структура("Номенклатура,Характеристика",стр.Номенклатура,стрХарактеристика.Характеристика);
			СтрокаСвободныхОстатков = тзСвободныхОстатков.НайтиСтроки(Отб);
			
			Если СтрокаСвободныхОстатков.Количество() > 0 Тогда
				
				стрХарактеристика.Остаток = СтрокаСвободныхОстатков[0].КоличествоКоробов;
				
			ИначеЕсли ВыбратьОстаткиБезКоробов Тогда 
				
				СтрокиОстатковБезКоробов = тзОстатковБезКоробов.НайтиСтроки(Отб);
				Если СтрокиОстатковБезКоробов.Количество() > 0 Тогда
					стрХарактеристика.Остаток = СтрокиОстатковБезКоробов[0].КоличествоОстаток;
				Иначе
					стрХарактеристика.Остаток = 0;
				КонецЕсли;
			...
			стрХарактеристика.ИД = B2B_ОбщиеПроцедурыИФункции.ПолучитьИдТовараДляСайта(стр.Номенклатура, стрХарактеристика.Характеристика);
			
			Если ВыгруженныеИдентификаторы.Найти(стрХарактеристика.ИД) = Неопределено Тогда
				ВыгруженныеИдентификаторы.Добавить(стрХарактеристика.ИД);
			Иначе
				Продолжить;	
			КонецЕсли;
