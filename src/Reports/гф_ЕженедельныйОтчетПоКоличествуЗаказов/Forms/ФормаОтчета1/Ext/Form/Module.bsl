...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если Данные.Поле="ВсегоЗаказов"   или Данные.Поле="ВсегоКлиентов" тогда	
	...
	Настройки = ЭтаФорма.Отчет.КомпоновщикНастроек.Настройки;  	
		СтандартнаяОбработка=Ложь;  
		ПолеОрганизация = Настройки.Отбор.ДоступныеПоляОтбора.Элементы.Найти("Организация").Поле;
		ИДЭлмОтбораОрганиация = "";
		Для Каждого Элемент Из Настройки.Отбор.Элементы Цикл
			Если Элемент.ЛевоеЗначение = ПолеОрганизация Тогда ИДЭлмОтбораОрганиация = Элемент.ИдентификаторПользовательскойНастройки; Прервать; КонецЕсли;
		КонецЦикла;
		ЭлементОтбораОрганизация = Коллекция.Найти(ИДЭлмОтбораОрганиация);
		...
		Если ЭлементОтбораорганизация.Использование Тогда
			Отбор.Вставить("Организация", ЭлементОтбораОрганизация.ПравоеЗначение);
        КонецЕсли;
		ПараметрыФормы = Новый Структура("СформироватьПриОткрытии, Отбор, КлючВарианта", Истина, Отбор, "РасшифровкаРегистратор");
		
		ОткрытьФорму("Отчет.гф_ЕженедельныйОтчетПоКоличествуЗаказов.Форма", ПараметрыФормы, ЭтаФорма);
		
	ИначеЕсли  Данные.Поле="ВсегоПодтвержденныхЗаказов" или Данные.Поле="ВсегоПодтвержденныхКлиентов" Тогда
			Коллекция = ЭтаФорма.Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы;
	Настройки = ЭтаФорма.Отчет.КомпоновщикНастроек.Настройки;  
		...
		Если ЭлементОтбораорганизация.Использование Тогда
			Отбор.Вставить("Организация", ЭлементОтбораОрганизация.ПравоеЗначение); 
		КонецЕсли;
		Отбор.Вставить("Статус",	"Подтвержден");
		
		ПараметрыФормы = Новый Структура("СформироватьПриОткрытии, Отбор, КлючВарианта", Истина, Отбор, "РасшифровкаРегистратор");
		
		ОткрытьФорму("Отчет.гф_ЕженедельныйОтчетПоКоличествуЗаказов.Форма", ПараметрыФормы, ЭтаФорма);
		
	ИначеЕсли Данные.Поле="ВсегоПисем" Или  Данные.Поле="ВсегоОтправленныхПисем" Тогда
		Коллекция = ЭтаФорма.Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы;
	Настройки = ЭтаФорма.Отчет.КомпоновщикНастроек.Настройки;  
		...
		ПараметрыФормы = Новый Структура("СформироватьПриОткрытии, Отбор, КлючВарианта", Истина, Отбор, "РасшифрокаРегистраторПисьма");
		
		ОткрытьФорму("ВнешнийОтчет.гф_ЕженедельныйОтчетПоКоличествуЗаказов.Форма", ПараметрыФормы, ЭтаФорма);     
