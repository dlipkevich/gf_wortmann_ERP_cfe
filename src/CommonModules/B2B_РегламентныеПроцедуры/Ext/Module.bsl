...
Функция ОбработатьДату(ДатаПроверки)
	...
	|	И РегламентированныйПроизводственныйКалендарь.ВидДня В(&РабДни)";
...
&После("B2B_СнятиеРезерва")  //1БЦ++ 31929
Процедура B2B_B2B_СнятиеРезерва() Экспорт
...
&Вместо("B2B_ВыгрузкаОстаткаНаСайт")  //1БЦ++ 32627
Функция B2B_B2B_ВыгрузкаОстаткаНаСайт(ЗапросGet = Ложь) Экспорт
...
//1БЦ по задаче 32798
	...
	НастройкиПодключения = B2B_ОбщиеПроцедурыИФункции.ВосстановитьНастройкиПодключенияПортал();
	БЦ_ГлубинаАнализаРеализацийДляОтправкиСтатусаЗаказов = Неопределено;
	Если ТипЗнч(НастройкиПодключения) = Тип("Соответствие") Тогда
		БЦ_ГлубинаАнализаРеализацийДляОтправкиСтатусаЗаказов = НастройкиПодключения.Получить("БЦ_ГлубинаАнализаРеализацийДляОтправкиСтатусаЗаказов");
	КонецЕсли;
	Если БЦ_ГлубинаАнализаРеализацийДляОтправкиСтатусаЗаказов = Неопределено или БЦ_ГлубинаАнализаРеализацийДляОтправкиСтатусаЗаказов = 0 Тогда 
		БЦ_ГлубинаАнализаРеализацийДляОтправкиСтатусаЗаказов=7;
		ЗаписьЖурналаРегистрации("B2B регламентное задание выгрузки статусов", УровеньЖурналаРегистрации.Информация, , , "Проверьте настройки значения глубины анализа");
	КонецЕсли;                          
	ДатаЗадания = ТекущаяДата();
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	РеализацияТоваровУслуг.CRM_Основание КАК CRM_Основание,
	|	Сведения.Значение
	|ИЗ
	|	Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ЗначенияСвойствОбъектов КАК Сведения
	|		ПО Сведения.Объект = РеализацияТоваровУслуг.CRM_Основание
	|		И Свойство = &Свойство
	|ГДЕ
	|	РеализацияТоваровУслуг.Дата >= &Дата
	|	И РеализацияТоваровУслуг.CRM_Основание ССЫЛКА Документ.ЗаказПокупателя
	|	И Сведения.Значение = Истина";
	Запрос.УстановитьПараметр("Свойство",ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("B2B_Загружен"));
	Запрос.УстановитьПараметр("Дата",НачалоДня(ТекущаяДата())-60*60*24*БЦ_ГлубинаАнализаРеализацийДляОтправкиСтатусаЗаказов);
	Результат = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("CRM_Основание");
	
	B2B_ОбщиеПроцедурыИФункции.ВыгрузитьСтатусы(НастройкиПодключения,Результат);
