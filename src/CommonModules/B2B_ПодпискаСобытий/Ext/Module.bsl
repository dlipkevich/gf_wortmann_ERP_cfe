...
Процедура B2B_ПередЗаписьюЗаказПокупателя(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт 
	...
	Попытка		
		...
		Если РежимЗаписи <> РежимЗаписиДокумента.Проведение Тогда  
			Возврат;
		КонецЕсли;		
		НомерЗаказаССайта = B2B_ОбщиеПроцедурыИФункции.ПолучитьЗначениеСвойства(Источник.Ссылка,ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("B2B_НомерЗаказа"));		
		Если НомерЗаказаССайта<>Неопределено Тогда 
			...
			ЗаписьЖурналаРегистрации("Данные.Изменение", УровеньЖурналаРегистрации.Информация,,,"Начало отправки изменений на сайт");
			Если ТипЗнч(НастройкиПодключения) = Тип("Соответствие") Тогда				
				...
				ЗаписьЖурналаРегистрации("Данные.Изменение", УровеньЖурналаРегистрации.Информация,,,"Окончание отправки изменений на сайт");
			КонецЕсли;			
		КонецЕсли;				
	Исключение
		Сообщение="Ошибка отправки на сайт изменений заказа №"+СокрЛП(Источник.Ссылка.Номер);				
		ЗаписьЖурналаРегистрации("Данные.Изменение", УровеньЖурналаРегистрации.Информация, , , Сообщение);						
	КонецПопытки;
	//1БЦ-- МГ 31839
