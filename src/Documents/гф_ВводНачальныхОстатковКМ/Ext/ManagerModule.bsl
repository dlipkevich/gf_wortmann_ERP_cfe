...
Процедура НайтиДобавитьСтрануПроисхожденияНомерГТД(строкаТЧ, строкаЗагрузки)
	...
	Если НомераГТД.Пустой() Тогда
		ОбщегоНазначения.СообщитьПользователю("Не найден элемент спр. ""Номера ГТД"" по коду " + НомерГТД_строкой);
	КонецЕсли;
	Если СтраныМира.Пустой() Тогда
		ОбщегоНазначения.СообщитьПользователю("Не найден элемент спр. ""Страны мира"" по наименованию " + СтранаПроисхождения_строкой);
	КонецЕсли;
	Если Декларация.Пустой() Тогда
		Если Не СтраныМира.Пустой() Тогда
			...
			Попытка
				...
				ОбщегоНазначения.СообщитьПользователю("Записан новый элемент спр. ""Номера ГТД"" по коду " + 
				НомерГТД_строкой + " и стране происхождения " + СтранаПроисхождения_строкой);
			Исключение
				ОбщегоНазначения.СообщитьПользователю("Не удалось записать элемент спр. ""Номера ГТД"" по коду " + 
				НомерГТД_строкой + " и стране происхождения " + СтранаПроисхождения_строкой);
			КонецПопытки;
		КонецЕсли;
	Иначе
