...
Процедура НайтиДобавитьСтрануПроисхожденияНомерГТД(строкаТЧ, строкаЗагрузки)
	...
	// В поиск страны добавили поиск по НаименованиеПолное 
	// Галфинд_Домнышева 2023/09/06
	//|	СтраныМира.Наименование = &СтранаПроисхождения_строкой
	|	(СтраныМира.Наименование = &СтранаПроисхождения_строкой
	|	ИЛИ СтраныМира.НаименованиеПолное = &СтранаПроисхождения_строкой)
    // } #wortmann
	...
	Если НомераГТД.Пустой() Тогда
		...
		+ " в строке " + строкаЗагрузки.Идентификатор + " ."); // Галфинд_Домнышева 2023/09/06
	КонецЕсли;
	...
	Если Декларация.Пустой() Тогда
		Если Не СтраныМира.Пустой() И Не НомерГТД_строкой = "" Тогда
			...
			Исключение
				...
				НомерГТД_строкой + " и стране происхождения " + СтранаПроисхождения_строкой
				+ " по строке " + строкаЗагрузки.Идентификатор + " ."); // Галфинд_Домнышева 2023/09/06
			КонецПопытки;
		КонецЕсли;
	Иначе
