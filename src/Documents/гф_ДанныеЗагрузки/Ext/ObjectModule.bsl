...
// BSLLS:LatinAndCyrillicSymbolInWord-on	
	...
	ИначеЕсли НовыйДокумент.гф_СтатусРаботыСЗаказомИ5 = Справочники.гф_СтатусРаботыСЗаказомИ5.ИзменениеИзИ5 И НЕ Отказ Тогда
		...
		КонецЦикла;
		ЗаполнитьИзмененияЗначений(ТаблицаРасхождений, НовыйДокумент, ТекущаяДата);
	ИначеЕсли НЕ ФлагЗаписиДокумента И НовыйДокумент <> Неопределено И НЕ Отказ Тогда
...
Функция ИщемИСоздаемНовыйДокумент(НомерДокумента, ДатаДокумента, ТекущаяДата, СтрокаТЧ, 
	...
	Если Результат.Следующий() Тогда
		...
		Если ФлагЗаписиДокумента Тогда
		ИзменяемСтатусНайденногоДокумента(НовыйДокумент, ФлагЗаписиДокумента);
		...
		Если СокрЛП(ФункцияДокумента) = "27" Тогда
			...
			НовыйДокумент.ОбменДанными.Загрузка = Истина;
			НовыйДокумент.Комментарий = НовыйДокумент.Комментарий + "Документ помечен на удаление согласно " + ЭтотОбъект + "."; 
			НовыйДокумент.ДополнительныеСвойства.Вставить("гф_ОбработкаОтгрузкаПоЗаказам", Истина);
			НовыйДокумент.Записать(РежимЗаписиДокумента.Запись);
			НовыйДокумент.ОбменДанными.Загрузка = Ложь;
	...
	Иначе
		...
		НовыйДокумент.гф_ДатаОбновленияИзИ5 = Дата(НайтиСтрокуДереваПоТэгу("Document_date", Отказ)); // Галфинд_ДомнышеваКР_24_03_2023
...
Процедура ПроверкаОтгруженныхТоваров(НовыйДокумент, ФлагЗаписиДокумента) 
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		"Изменения из I5 не внесены, в табличной части Товары содержатся строки с Отменой.";
		НовыйДокумент.ДополнительныеСвойства.Вставить("гф_ОбработкаОтгрузкаПоЗаказам", Истина);
...
//	НовыйДокумент - ДокументОбъект.ЗаказКлиента - Создаваемый документ
//  ФлагЗаписиДокумента - Булево
Процедура ИзменяемСтатусНайденногоДокумента(НовыйДокумент, ФлагЗаписиДокумента)
...
Процедура РассчитатьСкидкуВКоробах(НовыйДокумент, ТаблицаЗначенийЦенИзИ5, ТаблицаРасхождений)
	...
	Для каждого Строка Из НовыйДокумент.гф_ТоварыВКоробах Цикл
		...
		Строка.СуммаНДС =  Строка.Сумма * (СтавкаНДС.Ставка / СтоПроцентов);		
		Строка.СуммаСНДС = Строка.СуммаНДС + Строка.Сумма;
		
		ОпределениеРасхожденийЦен(ТаблицаРасхождений, Строка, СтокаЦен);		
	КонецЦикла;
...
Процедура ОпределениеРасхожденийЦен(ТаблицаРасхождений, Строка, СтокаЦен)
...
// #wortmann { 
// Код частично заимствован из модуля формы док.ЗаказКлиента проц. ЗаполнитьТоварыНаСервере()
// Процедура пересчитывает ТЧ Товары и устанавливает Цену в зависимости от наличия скидки
// Галфинд_Домнышева 2023/03/01
//
// Параметры:
//	НовыйДокумент - ДокументОбъект.ЗаказКлиента - Создаваемый документ
//	ТаблицаЗначенийЦенИзИ5 - ТаблицаЗначений - содержит колонки "ВариантКомплектации", "ЦенаКоробаИзИ5". 
Процедура ПересчетТЧТовары(НовыйДокумент, ТаблицаЗначенийЦенИзИ5)
	...
	ОписаниеВарианта = Новый ОписаниеТипов("СправочникСсылка.ВариантыКомплектацииНоменклатуры");
	ОписаниеЧисло = Новый ОписаниеТипов("Число");
	ВременнаяТаблицаТЧТовары = ПодготовитьВременнуюТаблицуТЧТовары();
	ВременнаяТаблицаТЧТовары.Колонки.Добавить("ВариантКомплектации", ОписаниеВарианта);
	ВременнаяТаблицаТЧТовары.Колонки.Добавить("ЦенаКороба", ОписаниеЧисло);
	
	Для Каждого СтрокаТоварыВКоробах Из НовыйДокумент.гф_ТоварыВКоробах Цикл
	...
	ВременнаяТаблицаТЧТовары.Свернуть("ВариантКомплектации,Номенклатура,Характеристика,
	...
	Для Каждого Строка Из ВременнаяТаблицаТЧТовары Цикл
		...
		Иначе
			НоваяСтрока.Цена = Окр(Строка.ЦенаКороба / Строка.ВариантКомплектации.КоличествоУпаковок, 2);           
		КонецЕсли;
		...
		//СтрокаДатыОтгрузки = ТаблицаЗначенийЦенИзИ5.Найти(Строка.ВариантКомплектации, "ВариантКомплектации");
		//НоваяСтрока.ДатаОтгрузки = СтрокаДатыОтгрузки.ДатаДоставки; 
		// -- Галфинд_Домнышева_КР_08_09_2023
	КонецЦикла;	  
	...
	ЗаполнитьСтавкуНДС = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(НовыйДокумент, Истина);
	...
	тчТовары.Свернуть("ДатаОтгрузки, Номенклатура,Характеристика, Цена, СтавкаНДС, ПроцентРучнойСкидки,
	|ВариантОбеспечения, ВидЦены, Обособленно, гф_ДатаДляРТУ, гф_ЦенаСоСкидкой,
	|ХарактеристикиИспользуются", "КоличествоУпаковок, Количество, Сумма, СуммаНДС, СуммаСНДС, СуммаРучнойСкидки");
	...
	СкладГруппа = Справочники.Склады.ЭтоГруппаИСкладыИспользуютсяВТЧДокументовПродажи(НовыйДокумент.Склад);
	СкладыСервер.ЗаполнитьСкладыВТабличнойЧасти(НовыйДокумент.Склад, СкладГруппа, НовыйДокумент.Товары, Ложь);
	НовыйДокумент.ДополнительныеСвойства.Вставить("гф_ОбработкаОтгрузкаПоЗаказам", Истина);
...
Процедура НайдемКонтрагента(СтрокаТЧ, НовыйДокумент, Отказ) 
	...
	Если ЗначениеЗаполнено(НовыйДокумент.Договор) Тогда
		НовыйДокумент.Контрагент = НовыйДокумент.Договор.Контрагент;
		НовыйДокумент.Партнер = НовыйДокумент.Договор.Партнер;
		
		Если  ЗначениеЗаполнено(НовыйДокумент.Договор.СтавкаНДС) Тогда
			СтавкаНДС = НовыйДокумент.Договор.СтавкаНДС;
		КонецЕсли;
		НовыйДокумент.НалогообложениеНДС = НовыйДокумент.Договор.НалогообложениеНДС;
		Если НовыйДокумент.Договор.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт Тогда
	...
	Иначе
		...
		+ " и годом " + гф_Год + " и организациии " + ЭтотОбъект.Организация + " с наличием контрагента с GLN "  
...
Процедура СделатьЗаписьУстановкиЦен(пар_Номенклатура, пар_ТипЦен, пар_ДокУстановкиЦен, СтрокаТЧ, ВалютаДокумента, Отказ)
	...
	Если ЕстьЗаписьСТакимиПараметрамиВЦенах(СтруктураВходПар) Тогда
		...
		Если пар_Номенклатура.Артикул <> пар_НоменклатураАртикулПрошлый Тогда
			СообщениеОбОшибке = "Для Артикула " + пар_Номенклатура.Артикул + " в периоде " + Формат(пар_ДокУстановкиЦен.Дата, "ДЛФ=Д") 
...
Функция СозданиеВариантаКомплектации(Вариант, Артикул, НаименованиеВарианта, Характеристика, СтрокаТЧ, Отказ)
	...
	Если Вариант <> Неопределено Тогда
		...
		Иначе
			...
			СообщениеОбОшибке = "Важная информация. При проведении документа " + ЭтотОбъект 
	...
	Если Номенклатура <> Неопределено Тогда 
		Если НаличиеШтрихкодовВБазе(Артикул, СтрокаТЧ, Отказ) Тогда
...
Функция ВариантКомплектацииСОшибками(Вариант, СтрокаТЧ, Отказ)	
	...
	Если Вариант.Владелец.ИспользованиеХарактеристик = Перечисления.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеДляВидаНоменклатуры 
		// #wortmann { 
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee4573acf7342c
		// Галфинд_Домнышева 2023/08/28
...
Функция НаличиеШтрихкодовВБазе(Артикул, СтрокаТЧ, Отказ)
...
Процедура ФормированиеВидаНоменклатурыПолСтавки(НовыйЭлемент, СтрокаН, СтрокаТЧ) 
	...
	Если НЕ ЗначениеЗаполнено(НовыйЭлемент.ГруппаФинансовогоУчета) Тогда
		НовыйЭлемент.ГруппаФинансовогоУчета = НовыйЭлемент.ВидНоменклатуры.ГруппаФинансовогоУчета;
	КонецЕсли;
	// -- Галфинд_Домнышева_16_03_2023
...
Функция ПолучитьЗначениеИзРСПереводЗначений(Марка)
	...
	Запрос.УстановитьПараметр("Язык", Справочники.гф_ВидыЯзыков.НайтиПоНаименованию("Нац. каталог"));
...
Процедура ДоговорДляКонтрагента(НовыйДокумент, СтрокаТЧ, GLNНомер)	
	...
	//КонецЕсли;
	// } #wortmann
	Если НовыйДокумент.Договор <> Неопределено И НовыйДокумент.Договор <> Договор Тогда
...
Функция СравнитьДокументы(НовыйДокумент)
	...
	Если ТЧДокументаДоСтатусаИзменениеИзИ5.Количество() <> НовыйДокумент.гф_ТоварыВКоробах.Количество() Тогда
		...
		СтрокаДляЗаписи.ЗначениеТекущее = НовыйДокумент.гф_ТоварыВКоробах.Количество();
		СтрокаДляЗаписи.ЗначениеИзИ5 = ТЧДокументаДоСтатусаИзменениеИзИ5.Количество();			
	КонецЕсли;
...
Процедура ЗаписатьОшибку(ТекстыОшибок)
    // Галфинд_Домнышева_16.03.2023
	// внесены изменения в процедуру в связи с неправильной загрузкой данных в этот РС.
	...
	Попытка	
		...
		НаборЗаписей.Отбор.ПутьФайла.Установить(ЭтотОбъект.ПолноеИмяФайла);
		НаборЗаписей.Отбор.Организация.Установить(ЭтотОбъект.Организация);
		НаборЗаписей.Отбор.Интерфейс.Установить(ЭтотОбъект.Интерфейс);
