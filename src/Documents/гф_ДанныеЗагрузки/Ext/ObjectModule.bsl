...
Процедура ОпределяемШапкуЗаказаКлиента(СтрокаТЧ, НовыйДокумент, ФлагЗаписиДокумента, ТекущаяДата, Отказ)
	...
	Если ЗначениеЗаполнено(НомерДокумента) И ЗначениеЗаполнено(ДатаДокумента) Тогда
		...
		ФункцияДокумента, ТекущаяДата, СтрокаТЧ, ФлагЗаписиДокумента, Отказ);
	...
	НовыйДокумент.гф_ВидЦены = _омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначение("гф_ГлобальныеЗначенияОптоваяЗакупочнаяЦена");
...
//	ДатаДокумента - Дата
//	ФункцияДокумента - Строка - значение ТЭГ "Document_function" 
//	ТекущаяДата - Дата - единая дата для записей в РС
//	СтрокаТЧ - строка табличной части из РегистрСведений.гф_СтрокиДокументаДанныеЗагрузки
//	ФлагЗаписиДокумента - Булево
//  Отказ - параметр системы
...
Функция ИщемИСоздаемНовыйДокумент(НомерДокумента, ДатаДокумента, ФункцияДокумента, ТекущаяДата, СтрокаТЧ, 
	...
	Иначе
		...
		НовыйДокумент.Организация =  ОрганизацияПоПапке(ЭтотОбъект.ПолноеИмяФайла);
		...
		ПроверимФункциюДокументаДляУдаления(ФункцияДокумента, НовыйДокумент, СтрокаТЧ);
...
//	СтрокаТЧ - строка табличной части из РегистрСведений.гф_СтрокиДокументаДанныеЗагрузки
//
Процедура ПроверимФункциюДокументаДляУдаления(ФункцияДокумента, НовыйДокумент, СтрокаТЧ)
...
Процедура ЗаполнитьРеквизитыДокумента(СтрокаТЧ, НовыйДокумент, НовыйЭлементТоварыВКоробах,
	...
	Если ИмяРеквизита = "Контрагент" Тогда   // Тэг - GLN_customer
		...
		НайдемКонтрагента(СтрокаТЧ, НовыйДокумент, ФлагЗаписиДокумента); 
...
Процедура ОбработкаВариантаКомплектацииИРасхождений(НовыйДокумент, НовыйЭлементТоварыВКоробах,
	...
	Если ЗначениеЗаполнено(НайденныйВариантКомплектации) Тогда
		...
		НовыйЭлементТоварыВКоробах.ВидЦены = _омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначение("гф_ГлобальныеЗначенияОптоваяЗакупочнаяЦена");	
		
		ЗагрузитьТЧТовары(НайденныйВариантКомплектации, НовыйЭлементТоварыВКоробах, НовыйДокумент, ДатаДоставки);
		
		НовыйЭлементТоварыВКоробах.ЦенаКоробаСоСкидкой = НовыйЭлементТоварыВКоробах.ЦенаКороба * (1 - НовыйЭлементТоварыВКоробах.Скидка/100);
		НовыйЭлементТоварыВКоробах.Сумма = НовыйЭлементТоварыВКоробах.Количество * НовыйЭлементТоварыВКоробах.ЦенаКоробаСоСкидкой;
		НовыйЭлементТоварыВКоробах.СтавкаНДС = СтавкаНДС;
		НовыйЭлементТоварыВКоробах.СуммаНДС =  НовыйЭлементТоварыВКоробах.Сумма * (СтавкаНДС.Ставка/100);
...
//	НовыйДокумент - ДокументОбъект.ЗаказКлиента - Создаваемый документ
//	ФлагЗаписиДокумента - Булево
Процедура НайдемКонтрагента(СтрокаТЧ, НовыйДокумент, ФлагЗаписиДокумента) 
	...
	Если ЗначениеЗаполнено(НайденныйКонтрагент) Тогда
		...
		НовыйДокумент.Партнер =  НайтиЗначениеПоРеквизитуСправочника ("Партнеры", "Наименование", НайденныйКонтрагент.Наименование);
...
Процедура ОбработкаСтрокДокументаПоСозданиюНоменклатуры(ТабличнаяЧасть, ТаблицаРегистраСоответствияСвойств,
	...
	Для Каждого СтрокаТЧ Из ТабличнаяЧасть Цикл 
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.ПолеЗагрузки) И СтрокаТЧ.НомерСтрокиРодителя <> НомерПредыдущегоРодителя Тогда
			...
			Если ТипСправочника = "Номенклатура" Тогда
				...
				Если НайденноеЗначение.Количество() > 0 Тогда
					...
					НовыйЭлемент = ПолучитьНовуюНоменклатуруСАртикулом(НоваяНоменклатура, Артикул, СтрокаТЧ, ТипСправочника, Отказ);
				...
				Иначе
					...
					НовыйЭлемент.ВидНоменклатуры = НайтиЗначениеПоРеквизитуСправочника("ВидыНоменклатуры", "Наименование", "Обувь импортная");
...
//	СтрокаТЧ - строка табличной части из РегистрСведений.гф_СтрокиДокументаДанныеЗагрузки 
//	ТипСправочника - Строка - название справочника
//	Отказ - параметр системы
//
// Возвращаемое значение:
//  НовыйЭлемент - СправочникОбъект.Номенклатура
Функция ПолучитьНовуюНоменклатуруСАртикулом(НоваяНоменклатура, Артикул, СтрокаТЧ, ТипСправочника, Отказ) 
...
Процедура СозданиеПроведениеУстановкиЦен(ТабличнаяЧасть, СтруктураЦен, Отказ) 
	...
	Для Каждого СтрокаТЧ Из ТабличнаяЧасть Цикл 
		...
		Если УсловиеСоздания  Тогда
			...
			Характеристики = ПолучитьХарактеристики(ЭлементНоменклатуры);
			...
			Для каждого ХН из Характеристики Цикл
				
				ПараметрыОтбора = Новый Структура;
				ПараметрыОтбора.Вставить("Номенклатура", ЭлементНоменклатуры);
				ПараметрыОтбора.Вставить("ВидЦены", ВидЦеныСтроки);
				ПараметрыОтбора.Вставить("ХарактеристикаЦО", ХН.Ссылка);
				
				СтрокаУникальна = СтрокаТовары2_5Уникальная(ДокументУстановкаЦен.Товары2_5, ПараметрыОтбора);
			//пар_ТипЦен = СоответствиеЦен.Получить(СтрокаТЧ.ПолеЗагрузки.Наименование);
			
			Если СтрокаУникальна Тогда
				СделатьЗаписьУстановкиЦен(ПараметрыОтбора, ДокументУстановкаЦен, СтрокаТЧ, ВалютаДокумента, Отказ);
			КонецЕсли;
			КонецЦикла;
...
Функция СтрокаТовары2_5Уникальная(Товары, ПараметрыОтбора)
	//ПараметрыОтбора = Новый Структура;
	//ПараметрыОтбора.Вставить("Номенклатура", Номенклатура);
	//ПараметрыОтбора.Вставить("ВидЦены", ВидЦены);
	//ПараметрыОтбора.Вставить("ХарактеристикаЦО", Характеристика);
...
Процедура СделатьЗаписьУстановкиЦен(ПараметрыОтбора, пар_ДокУстановкиЦен, СтрокаТЧ, ВалютаДокумента, Отказ)
	...
	СтруктураВходПар.Вставить("Номенклатура", 		ПараметрыОтбора.Номенклатура);
	СтруктураВходПар.Вставить("ТипЦен", 				ПараметрыОтбора.ВидЦены);
	СтруктураВходПар.Вставить("ХарактеристикаЦО", 				ПараметрыОтбора.ХарактеристикаЦО);
	
	Если ЕстьЗаписьСТакимиПараметрамиВЦенах(СтруктураВходПар) Тогда
		СообщениеОбОшибке = "Для Артикула " + ПараметрыОтбора.Номенклатура.Артикул + " в периоде " + Формат(ЭтотОбъект.Дата, "ДЛФ=Д") 
	...
	//ХарактеристикиНоменклатуры = ПолучитьХарактеристики(пар_Номенклатура.Ссылка);
	
	//Для каждого ХН из ХарактеристикиНоменклатуры Цикл
	НоваяСтрока 			= пар_ДокУстановкиЦен.Товары2_5.Добавить();
	НоваяСтрока.Номенклатура 		= ПараметрыОтбора.Номенклатура;
	НоваяСтрока.ВидЦены 	= ПараметрыОтбора.ВидЦены;
	НоваяСтрока.Валюта 	= ВалютаДокумента;
	НоваяСтрока.ХарактеристикаЦО = ПараметрыОтбора.ХарактеристикаЦО;
...
Функция ПолучитьХарактеристики(Ссылка) 
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ХарактеристикиНоменклатурыДляЦенообразования.Ссылка КАК Ссылка
		|ИЗ
		|	РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатурыДляЦенообразования КАК ХарактеристикиНоменклатурыДляЦенообразования
		|		ПО ШтрихкодыНоменклатуры.Характеристика.Наименование = ХарактеристикиНоменклатурыДляЦенообразования.Наименование
		|ГДЕ
...
Функция ЕстьЗаписьСТакимиПараметрамиВЦенах(СтруктураВходПар)
	...
	Запрос.УстановитьПараметр("ХарактеристикаЦО", СтруктураВходПар.ХарактеристикаЦО);
...
// BSLLS:LatinAndCyrillicSymbolInWord-on
	...
	Если СокрЛП(Group_type) = "10E" Тогда								// Это Pricat Sort - производим проверку
		...
		МассивНеНайденнойНоменклатуры = ПолучитьМассивНеНайденнойНоменклатуры(СтрокиШтрихКоды);
		...
		Если МассивНеНайденнойНоменклатуры.Количество() > 0 Тогда
			...
			Для каждого ЭлементМассиваEAN Из МассивНеНайденнойНоменклатуры Цикл      
				...
				СтрокаПереченьНеНайденнойНоменклатуры + "; ") + Символы.ПС + Строка(ЭлементМассиваEAN);			
			КонецЦикла; 
			СообщениеОбОшибке = "Ошибка при проведении документа " + ЭтотОбъект 
...
Функция ПолучитьМассивНеНайденнойНоменклатуры(СтрокиШтрихКоды)
	...
	Для каждого СтрокаШтрихкод Из СтрокиШтрихКоды Цикл
		...
		Если (НайденныйЭлемент = Неопределено ИЛИ НайденныйЭлемент.Пустая()) 
			И МассивНеНайденнойНоменклатуры.Найти(EAN) = Неопределено Тогда
...
Процедура ЗаполнитьЗначенияСвойствНовогоЭлемента(НовыйЭлемент, ДополнительныеРеквизиты, СтрокаТЧ, ЗначенияСвойств)
	...
	Для каждого Реквизит Из ДополнительныеРеквизиты Цикл 
		...
		ИначеЕсли Реквизит.Тэг = "Supplier_season" Тогда												
			Свойство_Сезон = НайтиДополнительныйРеквизитПоНаименованию("Supplier_season");
			Значение = Новый Структура("Свойство, Значение", Свойство_Сезон, Реквизит.Значение);
			ЗначенияСвойств.Добавить(Значение);
		Иначе
			Продолжить;
		КонецЕсли;
...
Процедура ЗаписатьПолНоменклатуры(СтрокаТЧ, НовыйЭлемент, ПолНоменклатуры)
	...
	Попытка
		...
		Для Каждого Строка Из ЗначенияСвойств Цикл
			...
			Если Запись = Неопределено Тогда
				...
				Запись.Свойство = НайтиДополнительныйРеквизитПоНаименованию(Строка.Свойство.Наименование);
...
Процедура ПоискИЗаписьКартинки(РеквизитЗначение, СтрокаТЧ, НовыйЭлемент)
	...
	Иначе	
		...
		//ПозицияСимвола = СтрНайти(ЗначениеКартинки, "."); 
...
Процедура ЗагрузитьТЧТовары(НайденныйВариантКомплектации, НовыйЭлементТоварыВКоробах, НовыйДокумент, ДатаОтгрузки)
	...
	ВременнаяТаблицаТЧТовары.Колонки.Добавить("ХарактеристикаЦО");
	...
	Для каждого СтрокаСостава Из НайденныйВариантКомплектации.Товары Цикл
		...
		НоваяСтрока.ХарактеристикаЦО = Справочники.ХарактеристикиНоменклатурыДляЦенообразования.НайтиПоНаименованию(СтрокаСостава.Характеристика.Наименование);
		
	КонецЦикла;	
	
	ВременнаяТаблицаТЧТовары.Свернуть("Номенклатура, Характеристика, ХарактеристикаЦО", "КоличествоУпаковок");
	...
	Для каждого Строка Из ВременнаяТаблицаТЧТовары Цикл
		...
		Отбор.Вставить("ХарактеристикаЦО", Строка.ХарактеристикаЦО);
		
		Цена = РегистрыСведений.ЦеныНоменклатуры25.ПолучитьПоследнее(НовыйДокумент.Дата+1, Отбор).Цена;
...
Функция ПолучитьПоследнююЦену(Отбор)
