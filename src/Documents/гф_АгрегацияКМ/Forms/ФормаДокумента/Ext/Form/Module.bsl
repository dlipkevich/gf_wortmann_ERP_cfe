...
&НаКлиенте
Асинх Процедура гф_ЗаполнитьВидДвиженияВместо(Команда)
	Результат = Ждать ВвестиЗначениеАсинх(
	ПредопределенноеЗначение("Перечисление.гф_ВидДвиженияАгрегацииКМ.Приход"),
	"Выберите вид агрегации (для очистки - закройте окно без выбора)");
	Для каждого стрТч Из Объект.Товары Цикл
		стрТч["Движение"] = Результат;
	КонецЦикла;
КонецПроцедуры
