...
Процедура гф_СоздатьРеквизитыКонтрагента()
	...
	РеквизитФормы_гф_ГЛН_номер       = Новый РеквизитФормы("гф_GLN_номер",      
	ОписаниеТиповСтрока13, , "GLN_номер", Истина);
...
Процедура гф_ДобавитьПоляНаФорму()
	...
	гф_ДобавитьПоляНаФормуПродолжение();
	...
	// Галфинд_Домнышева_КР_24_07_2023 перенесла из гф_ДобавитьПоляНаФормуПродолжение(), из-за ограничения внутри
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee29eaf874ab92
	// #wortmann {
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=811dbcee7bda45d711ed130ce78add5e
...
// Галфинд Окунев 2022/07/05.
&НаСервере
Процедура гф_ДобавитьПоляНаФормуПродолжение()
...
Процедура гф_ДобавитьАдрес(Команда)
	...
	ЭтотОбъект, УникальныйИдентификатор, , , ДобавитьАдресЗавершение);
...
Процедура гф_ЗаполнитьРеквизитыКонтрагента(Партнер)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	B2B_КонтактныеЛицаПоОрганизациям.Организация КАК B2B_КонтактныеЛицаПоОрганизациям_Организация,
	|	B2B_КонтактныеЛицаПоОрганизациям.ОсновноеКонтактноеЛицо КАК B2B_КонтактныеЛицаПоОрганизациям_ОсновноеКонтактноеЛицо
	|ИЗ
