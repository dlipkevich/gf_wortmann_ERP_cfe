...
Процедура ЗаполнитьОтборы()
	...
	СтруктураРезервов = ПолучитьИзВременногоХранилища(Параметры["СтруктураРезервов"]);
	ЗаполнитьОтборВариантКомплектации(СтруктураРезервов, флРезервыВКоробах);
	ЗаполнитьОтборНоменклатураХарактеристика(СтруктураРезервов, флРезервыВКоробах);
...
Процедура ЗаполнитьТекстЗапросаВтЗаказы(Запрос)
	...
	Иначе
		Запрос.Текст = ТекстЗапросаВтЗаказыТовары(флПеренестиНовоеНазначение, отборНоменклатураХарактеристика);
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьОтборВариантКомплектации(СтруктураРезервов, флРезервыВКоробах)
	отборВариантКомплектации.Очистить();
	Если Не флРезервыВКоробах Тогда
		Возврат;
	КонецЕсли;
	Если СтруктураРезервов.Свойство("РезервыВКоробах") Тогда
		отборВариантКомплектации.Загрузить(СтруктураРезервов["РезервыВКоробах"]);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьОтборНоменклатураХарактеристика(СтруктураРезервов, флРезервыВКоробах)
	отборНоменклатураХарактеристика.Очистить();
	Если флРезервыВКоробах Тогда
		Возврат;
	КонецЕсли;
	Если СтруктураРезервов.Свойство("Резервы") Тогда
		отборНоменклатураХарактеристика.Загрузить(СтруктураРезервов["Резервы"]);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьСписокНазначенийВКоробах(Запрос)
	...
	Если флПеренестиНовоеНазначение Тогда
		...
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee51757e267b8e
		Для каждого стрКоллекции Из СписокНазначений Цикл
			// для оптимизации впоследствии нужно вычислять в запросе
			стрКоллекции["КоличествоУпаковокЗаказа"]= стрКоллекции.Заказ.гф_ТоварыВКоробах.Итог("Количество");
		КонецЦикла;
	...
	|	втЗаказы.Количество - ЕСТЬNULL(втРезервыКоробов.Резерв, 0) - втЗаказы.Отгружается КАК НеХватает,
	|	ВЫБОР
	|		КОГДА втЗаказы.Количество > ЕСТЬNULL(втРезервыКоробов.Резерв, 0) + втЗаказы.Отгружается
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК НеОбеспечен
	...
	|	втЗаказы.Количество КАК Количество,
	|	втЗаказы.Отгружается КАК Отгружается,
	|	втЗаказы.Резерв КАК Резерв,
	|	втЗаказы.НеХватает КАК НеХватает
	|ИЗ
	|	втВсеЗаказы КАК втЗаказы";
	Результат = Запрос.Выполнить();
	ВсеЗаказыВКоробах.Загрузить(Результат.Выгрузить());
	
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	т.Заказ КАК Заказ,
	               |	МИНИМУМ(т.НеОбеспечен) КАК НеОбеспечен
...
Процедура ЗаполнитьСписокНазначений()
	...
	Запрос.УстановитьПараметр("ВариантКомплектации", отборВариантКомплектации.Выгрузить().ВыгрузитьКолонку("ВариантКомплектации"));
	Запрос.УстановитьПараметр("Номенклатура", отборНоменклатураХарактеристика.Выгрузить().ВыгрузитьКолонку("Номенклатура"));
	Запрос.УстановитьПараметр("Характеристика", отборНоменклатураХарактеристика.Выгрузить().ВыгрузитьКолонку("Характеристика"));
	...
	Если флПеренестиНовоеНазначение Тогда
		...
		Для каждого стрКоллекции Из СписокНазначений Цикл
			// для оптимизации впоследствии нужно вычислять в запросе
			стрКоллекции["КоличествоУпаковокЗаказа"]= стрКоллекции.Заказ.Товары.Итог("КоличествоУпаковок");
		КонецЦикла;
		// ^^^ Галфинд \ Sakovich 13.09.2023
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	И ЗаказКлиентаТовары.Характеристика В(&Характеристика)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ЗаказКлиентаТовары.Количество - ЕСТЬNULL(втОстатки.Доступно, 0) - ЕСТЬNULL(втОстатки.ВНаличии, 0) КАК НеХватает,
	               |	ВЫБОР
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ЗаказКлиентаТовары.Характеристика КАК Характеристика,
	               |	ЗаказКлиентаТовары.Количество КАК Количество,
	               |	ЗаказКлиентаТовары.Доступно КАК Доступно,
	               |	ЗаказКлиентаТовары.ВНаличии КАК ВНаличии,
	               |	ЗаказКлиентаТовары.НеХватает КАК НеХватает,
	               |	ЗаказКлиентаТовары.НеОбеспечен КАК НеОбеспечен
	               |ИЗ
	               |	втВсеЗаказы КАК ЗаказКлиентаТовары";
	Результат = Запрос.Выполнить();
	ВсеЗаказы.Загрузить(Результат.Выгрузить());
...
Процедура ЗаполнитьСписокНоменклатуры()
	...
	Если флРезервыВКоробах Тогда
		Для Каждого ВариантКомплектации Из отборВариантКомплектации Цикл
			Для Каждого СтрокаКомплектации Из ВариантКомплектации.Значение.Товары Цикл
				СписокНоменклатуры.Добавить(СтрокаКомплектации.Номенклатура);
			КонецЦикла;
		КонецЦикла;
	Иначе
		Для Каждого СтрокаТЗ Из отборНоменклатураХарактеристика Цикл
...
Процедура Выбрать(Команда)
	...
	Если ТД <> Неопределено Тогда
		АдресХранилища = ПолучитьДанныеЗаказа(ТД.Назначение);
...
Функция ТекстЗапросаВтЗаказыТоварыВКоробах(флПеренестиНовоеНазначение, отборВариантКомплектации)
	...
	Иначе
		Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		        ...
		        |ГДЕ
		        |	ЗаказКлиентаТоварыВКоробах.ВариантКомплектации В(&ВариантКомплектации)
...
&НаСервереБезКонтекста
Функция ТекстЗапросаВтЗаказыТовары(флПеренестиНовоеНазначение, отборНоменклатураХарактеристика)
	Если флПеренестиНовоеНазначение Тогда
		Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ РАЗРЕШЕННЫЕ
		               |	ЗаказКлиента.Ссылка КАК Заказ
		               |ПОМЕСТИТЬ втЗаказы
...
&НаСервере
Функция ПолучитьДанныеЗаказа(Назначение)
	СтруктураВозврата = Новый Структура("Назначение", Назначение);
	Если флРезервыВКоробах Тогда
		ТЗВсеЗаказы = РеквизитФормыВЗначение("ВсеЗаказыВКоробах");
	Иначе
		ТЗВсеЗаказы = РеквизитФормыВЗначение("ВсеЗаказы");
	КонецЕсли;
	НайденныеСтроки = ТЗВсеЗаказы.НайтиСтроки(Новый Структура("Заказ", Назначение.Заказ));
	СтруктураВозврата.Вставить("СтрокиЗаказа", НайденныеСтроки);
...
Процедура ЗаполнитьРезервыВКоробахНаСервере(Запрос)
	...
	МассивВК = отборВариантКомплектации.Выгрузить().ВыгрузитьКолонку("ВариантКомплектации");
