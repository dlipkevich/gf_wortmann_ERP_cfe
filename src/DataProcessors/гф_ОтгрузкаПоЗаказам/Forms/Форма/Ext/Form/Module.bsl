...
Процедура УдалитьСтрокиБезУпаковочногоЛиста(ДеревоЗначений)
	...
	СчетчикДерево = ДеревоЗначений.Строки.Количество() - 1;
	
	Пока СчетчикДерево >= 0 Цикл
		
		Счетчик = ДеревоЗначений.Строки[СчетчикДерево].Строки.Количество() - 1;
			
		Пока Счетчик >= 0 Цикл
			
			Если ДеревоЗначений.Строки[СчетчикДерево].Строки[Счетчик].Строки.Количество() = 0 Тогда
				
				 ДеревоЗначений.Строки[СчетчикДерево].Строки.Удалить(ДеревоЗначений.Строки[СчетчикДерево].Строки[Счетчик]);
		...
		Если ДеревоЗначений.Строки[СчетчикДерево].Строки.Количество() = 0 Тогда
				
			ДеревоЗначений.Строки.Удалить(ДеревоЗначений.Строки[СчетчикДерево]);
				 
		КонецЕсли;
		
		СчетчикДерево = СчетчикДерево - 1;
...
// Не используется
Процедура УдалитьСтрокиБезУпаковочногоЛистаСтарыйАлгоритм(ДеревоЗначений)
	...
	Пока Счетчик >= 0 Цикл
		...
		Для Каждого СтрокаСпЗнСтрока_Н Из СпЗнСтрока_Н Цикл
			...
			Если ДеревоЗначений.Строки[Счетчик].Клиент = СтрокаСпЗнСтрока_Н.Клиент
				И ДеревоЗначений.Строки[Счетчик].АдресДоставки = СтрокаСпЗнСтрока_Н.АдресДоставки
				// -- Галфинд ВолковЕВ 18.07.2023
				// Тогда
				И ДеревоЗначений.Строки[Счетчик].УИ = СтрокаСпЗнСтрока_Н.УИ Тогда
				
				// ++ Галфинд ВолковЕВ 18.07.2023
				//ДеревоЗначений.Строки.Удалить(СтрокаСпЗнСтрока_Н.ИндексСтроки);
				ДеревоЗначений.Строки.Удалить(ДеревоЗначений.Строки[Счетчик]);
				// -- Галфинд ВолковЕВ 18.07.2023
