	...
	Для Каждого СтрокаЗаказы Из ТЗ_Заказы Цикл
		...
		Для Каждого СтрокаУпаковочныйЛистНоменклатура Из СтрокаЗаказы.СписокУпаковочныйЛистИлиНоменклатура Цикл
			...
			Если Объект.ТоварыВКоробках Тогда
				...
				Отказ = Ложь;
				ОбработкаЗаполненияТоваровВКоробах(Отказ, ВозвратПродолжить, ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных, ОтказЗаписьУпаковочногоЛиста, ОбъектЗаказ,
					ТЗ_Заказ_ТоварыВКоробах,ТЗ_Новая_Заказ_ТоварыВКоробах,  ТЗ_ПричиныИзмененияТоварыВКоробах, ТЗ_Новая_Заказ_ПричиныИзмененияТоваровВКоробах, ТабКомплектация, СписокКодовСтрокиПоУпаковочнымЛистам);
				...
				КонецЕсли;
			Иначе
				ОбработкаЗаполненияТоваров(ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных, ОтказЗаписьУпаковочногоЛиста, ОбъектЗаказ)
			КонецЕсли;
...
Процедура ДобавитьСтрокуРазбиениемПересорт(ПараметрыОбработки, НоваяТаблица, ТекущаяСтрока, ТаблицаИсточник, ПроверкаОтказ, СписокКодовСтроки, УпаковочныйЛист, СтруктураПереданныхДанных)
...
Процедура ДобавитьСтрокуРазбиениемНеПолный(ПараметрыОбработки, НоваяТаблица, ТекущаяСтрока, ТаблицаИсточник, ПроверкаОтказ, СписокКодовСтроки, УпаковочныйЛист, СтруктураПереданныхДанных)
...
Процедура ОбработкаЗаполненияТоваров(ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных, ОтказЗаписьУпаковочногоЛиста, ОбъектЗаказ)
...
Процедура ОбработкаЗаполненияТоваровВКоробах(Отказ, ВозвратПродолжить, ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных, ОтказЗаписьУпаковочногоЛиста, ОбъектЗаказ,
	ТЗ_Заказ_ТоварыВКоробах, ТЗ_Новая_Заказ_ТоварыВКоробах, ТЗ_ПричиныИзмененияТоварыВКоробах, ТЗ_Новая_Заказ_ПричиныИзмененияТоваровВКоробах, ТабКомплектация, СписокКодовСтрокиПоУпаковочнымЛистам)
	...
	// Найденную строку пробуем отгрузить. В дополнение добавляем строки по истории работы с вариантами комплектаций, какое количество было отгружено и какое осталось
	...
	Если СостояниеКороба = Справочники.гф_СостянияКоробов.Пересорт Тогда
		...
		ОбработкаЗаполненияТоваровВКоробахПересорт(Отказ, ВозвратПродолжить, ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных, ОтказЗаписьУпаковочногоЛиста, ОбъектЗаказ,
			ТЗ_Заказ_ТоварыВКоробах, ТЗ_Новая_Заказ_ТоварыВКоробах, ТЗ_ПричиныИзмененияТоварыВКоробах, ТЗ_Новая_Заказ_ПричиныИзмененияТоваровВКоробах, ТабКомплектация, СписокКодовСтрокиПоУпаковочнымЛистам, СкидкаВКоробах, Количество)
			
	//НеПолный
	ИначеЕсли СостояниеКороба = Справочники.гф_СостянияКоробов.НеПолный Тогда
		
		 ОбработкаЗаполненияТоваровВКоробахНеПолный(Отказ, ВозвратПродолжить, ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных, ОтказЗаписьУпаковочногоЛиста, ОбъектЗаказ,
			ТЗ_Заказ_ТоварыВКоробах, ТЗ_Новая_Заказ_ТоварыВКоробах, ТЗ_ПричиныИзмененияТоварыВКоробах, ТЗ_Новая_Заказ_ПричиныИзмененияТоваровВКоробах, ТабКомплектация, СписокКодовСтрокиПоУпаковочнымЛистам, СкидкаВКоробах, Количество)
			
	//НеполныйИПересорт
	ИначеЕсли СостояниеКороба = Справочники.гф_СостянияКоробов.НеПолныйИПересорт Тогда
		
		ОбработкаЗаполненияТоваровВКоробахНеполныйИПересорт(Отказ, ВозвратПродолжить, ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных, ОтказЗаписьУпаковочногоЛиста, ОбъектЗаказ,
			ТЗ_Заказ_ТоварыВКоробах, ТЗ_Новая_Заказ_ТоварыВКоробах, ТЗ_ПричиныИзмененияТоварыВКоробах, ТЗ_Новая_Заказ_ПричиныИзмененияТоваровВКоробах, ТабКомплектация, СписокКодовСтрокиПоУпаковочнымЛистам, СкидкаВКоробах, Количество)
				...
				Для Каждого СтрокаКомплектацияРазбиение Из ТабКомплектация Цикл
					...
					Если СтрокаКомплектацияРазбиение.Номенклатура = СтрокаТовары.Номенклатура
						...
						ИначеЕсли СтрокаТовары.КоличествоУпаковок = Количество Тогда
							...
							ПолучитьКомментарийИСкидкиДляРТУ(СтрокаТовары.гф_КомментарийРТУ, СтрокаТовары.гф_СкидкаДляРТУ, СтрокаТовары.гф_ДатаДляРТУ, СтрокаТовары.ПроцентРучнойСкидки, СтруктураПереданныхДанных);
...
Процедура ОбработкаЗаполненияТоваровВКоробахПересорт(Отказ, ВозвратПродолжить, ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных, ОтказЗаписьУпаковочногоЛиста, ОбъектЗаказ,
	ТЗ_Заказ_ТоварыВКоробах, ТЗ_Новая_Заказ_ТоварыВКоробах, ТЗ_ПричиныИзмененияТоварыВКоробах, ТЗ_Новая_Заказ_ПричиныИзмененияТоваровВКоробах, ТабКомплектация, СписокКодовСтрокиПоУпаковочнымЛистам, СкидкаВКоробах, Количество)
	...
	Для Каждого СтрокаРазницаВНоменклатуре Из НайтиСтрокиРазницаТаблиц Цикл
		...
		НайтиСтрокиЭталон = ТЗ_УпаковочныйЛистЭталонКомплектация.НайтиСтроки(Новый Структура("Номенклатура, Характеристика", СтрокаРазницаВНоменклатуре.Номенклатура, СтрокаРазницаВНоменклатуре.Характеристика));
	...
	Для Каждого СтрокаРазница Из ТЗСНоменклатуройИКоличествомДляКорректировки Цикл
		...
		Если Не НайтиСтроки_ВариантОбеспечения = Неопределено И НайтиСтроки_ВариантОбеспечения.Количество() > 0 Тогда
			...
			Если СтрокаРазница.ДобавитьКоличество > 0 Тогда
				...
				//ПараметрыРазбиенияСтроки = ПараметрыРазбиенияСтрокиПересорт();
				
				//Количество = СтрокаРазница.ДобавитьКоличество;
				
				//ПараметрыОбработки = ПолучитьПараметрыОбработкиПересорт(ДополнительныеПараметры, Количество);
				
				//ДобавитьСтрокуРазбиениемДобавитьКоличествоПересорт(ПараметрыОбработки, СтрокаТовары, ТЗ_Заказ_Товары);
			...
			ИначеЕсли СтрокаРазница.УбратьКоличество > 0 Тогда
				...
				//ДобавитьСтрокуРазбиениемУбратьКоличествоПересорт(ПараметрыОбработки, СтрокаТовары, ТЗ_Заказ_Товары, ОтказЗаписьУпаковочногоЛиста);
				...
				Если СтрокаТовары.КоличествоУпаковок > Количество Тогда
					...
					НоваяСтрока.КоличествоУпаковок 	= СтрокаРазница.УбратьКоличество;
					НоваяСтрока.Отменено 			= Истина;
					// ++ Галфинд ВолковЕВ 31.05.2023
					НоваяСтрока.ПричинаОтмены 		= Справочники.ПричиныОтменыЗаказовКлиентов.гф_ФактическоеРасхождениеНаСкладе;
					// -- Галфинд ВолковЕВ 31.05.2023
					...
					//ДобавитьСтрокуДляПересчетаСуммы(ДополнительныеПараметры.СтруктураПересчетаСуммы, СтрокаТовары);
					//ДобавитьСтрокуДляПересчетаСуммы(ДополнительныеПараметры.СтруктураПересчетаСуммы, НоваяСтрока);
				...
				ИначеЕсли СтрокаТовары.КоличествоУпаковок = Количество Тогда
					...
					// Проверить, так как здесь не создается новая строка
					//ДобавитьСтрокуДляПересчетаСуммы(ДополнительныеПараметры.СтруктураПересчетаСуммы, СтрокаТовары);
					//ДобавитьСтрокуДляПересчетаСуммы(ДополнительныеПараметры.СтруктураПересчетаСуммы, НоваяСтрока);
					//
					//ПересчитатьСуммы(ДополнительныеПараметры.СтруктураПересчетаСуммы);
	...
	Для Каждого СтрокаКомплектацияРазбиение Из ТабКомплектация Цикл
		...
		//ДобавитьСтрокуРазбиениемПересорт(ПараметрыОбработки, ТЗ_Новая_Заказ_Товары, СтрокаТовары, ТЗ_Заказ_Товары, ОтказЗаписьУпаковочногоЛиста,
		...
		// Бывает что СтрокаТовары.ВариантОбеспечения это массив
		Если Не ТипЗнч(СтрокаТовары) = Тип("Массив") Тогда
			...
			Если СтрокаТовары.КоличествоУпаковок > Количество Тогда
				...
				НоваяСтрока.КоличествоУпаковок 	= СтрокаКомплектацияРазбиение.Количество;
				НоваяСтрока.ВариантОбеспечения 	= Перечисления.ВариантыОбеспечения.Отгрузить;
				НоваяСтрока.ДатаОтгрузки 		= ТекущаяДата();
			...
			ИначеЕсли СтрокаТовары.КоличествоУпаковок = Количество Тогда
				...
				// Пересчет сумм по каждой строке
				//ДобавитьСтрокуЗавершениеПересорт(НоваяСтрока, ДополнительныеПараметры, СтрокаТовары);
				//ДобавитьСтрокуЗавершениеПересорт(СтрокаТовары, ДополнительныеПараметры);
...
Процедура ОбработкаЗаполненияТоваровВКоробахНеПолный(Отказ, ВозвратПродолжить, ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных, ОтказЗаписьУпаковочногоЛиста, ОбъектЗаказ,
	ТЗ_Заказ_ТоварыВКоробах, ТЗ_Новая_Заказ_ТоварыВКоробах, ТЗ_ПричиныИзмененияТоварыВКоробах, ТЗ_Новая_Заказ_ПричиныИзмененияТоваровВКоробах, ТабКомплектация, СписокКодовСтрокиПоУпаковочнымЛистам, СкидкаВКоробах, Количество)
...
Процедура ОбработкаЗаполненияТоваровВКоробахНеполныйИПересорт(Отказ, ВозвратПродолжить, ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, СтрокаУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных, ОтказЗаписьУпаковочногоЛиста, ОбъектЗаказ,
	ТЗ_Заказ_ТоварыВКоробах, ТЗ_Новая_Заказ_ТоварыВКоробах, ТЗ_ПричиныИзмененияТоварыВКоробах, ТЗ_Новая_Заказ_ПричиныИзмененияТоваровВКоробах, ТабКомплектация, СписокКодовСтрокиПоУпаковочнымЛистам, СкидкаВКоробах, Количество)
	...
	Для Каждого СтрокаРазницаВНоменклатуре Из НайтиСтрокиРазницаТаблиц2 Цикл
		...
		НайтиСтрокиЭталон = ТЗ_УпаковочныйЛист.НайтиСтроки(Новый Структура("Номенклатура, Характеристика", СтрокаРазницаВНоменклатуре.Номенклатура, СтрокаРазницаВНоменклатуре.Характеристика));
