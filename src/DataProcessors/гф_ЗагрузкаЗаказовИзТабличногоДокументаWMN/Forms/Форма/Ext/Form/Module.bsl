...
&НаСервереБезКонтекста
Функция ПолучитьАдресДоставкиПоНомеру(НомерАдреса, Контрагент)
...
Функция ПрочитатьТабличныеДанные()
	...
	Пока Истина Цикл
		...
		ВариантКомплектации = НайтиВариантКомплектации(новая.ВариантКомплектации);
		Если ВариантКомплектации.Пустая() Тогда
			ДобавитьСтрокуТЗПротокол("Ошибка:" + новая.ВариантКомплектации, "Не найден вариант комплектации", НомерСтроки)
		КонецЕсли;
		...
		АдресДоставки = ПолучитьАдресДоставкиПоНомеру(новая.НомерАдресаДоставки, Контрагент);
		Если АдресДоставки.Пустая() Тогда
			ДобавитьСтрокуТЗПротокол("Ошибка:" + новая.НомерАдресаДоставки + " контрагент:" + Контрагент, "Не найден адрес доставки", НомерСтроки)
		КонецЕсли;
