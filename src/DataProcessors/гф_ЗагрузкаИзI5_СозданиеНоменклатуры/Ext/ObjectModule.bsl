...
Функция ВыполнитьОбменДанными(МассивЗагружаемыхФайлов) Экспорт
	...
	// #wortmann { 
	// Установка даты документа из файла  
	// Галфинд Волков 2022/12/09
	ТЗ = Новый ТаблицаЗначений;
	ТЗ.Колонки.Добавить("Ссылка");
	ТЗ.Колонки.Добавить("Артикул");
	ТЗ.Колонки.Добавить("Suppliers_article_number");
	ТЗ.Колонки.Добавить("Colour_code_supplier");
	ТЗ.Колонки.Добавить("EAN");
	ТЗ.Колонки.Добавить("НомерСтрокиРодителя");
	ТЗ.Колонки.Добавить("Найден");
	
	Для Каждого СозданныйДокумент Из СозданныеДокументы Цикл
		
		ДокументДанные = СозданныйДокумент.ПолучитьОбъект();
		Если ДокументДанные.Интерфейс = Перечисления.гф_Интерфейсы.PRICAT_SORT Тогда
			
			Запрос = Новый Запрос;
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	гф_СтрокиДокументаДанныеЗагрузки.Тэг КАК Тэг,
			|	гф_СтрокиДокументаДанныеЗагрузки.Значение КАК Значение,
			|	гф_СтрокиДокументаДанныеЗагрузки.НомерСтрокиРодителя КАК НомерСтрокиРодителя
			|ПОМЕСТИТЬ ВременнаяТаблица
			|ИЗ
			|	РегистрСведений.гф_СтрокиДокументаДанныеЗагрузки КАК гф_СтрокиДокументаДанныеЗагрузки
			|ГДЕ
			|	гф_СтрокиДокументаДанныеЗагрузки.Документ = &Документ
			|	И гф_СтрокиДокументаДанныеЗагрузки.Тэг = &Тэг
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|ГДЕ
			|	гф_СтрокиДокументаДанныеЗагрузки.Документ = &Документ
			|	И гф_СтрокиДокументаДанныеЗагрузки.Тэг = &Тэг1
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|ГДЕ
			|	гф_СтрокиДокументаДанныеЗагрузки.Документ = &Документ
			|	И гф_СтрокиДокументаДанныеЗагрузки.Тэг = &Тэг2
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ВременнаяТаблица.Значение КАК Suppliers_article_number,
			|	ВременнаяТаблица.НомерСтрокиРодителя КАК НомерСтрокиРодителя,
			|	ВременнаяТаблица1.Значение КАК Colour_code_supplier,
			|	ВременнаяТаблица2.Значение КАК EAN
			|ИЗ
			|	ВременнаяТаблица КАК ВременнаяТаблица
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблица1 КАК ВременнаяТаблица1
			|		ПО ВременнаяТаблица.НомерСтрокиРодителя = ВременнаяТаблица1.НомерСтрокиРодителя
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблица2 КАК ВременнаяТаблица2
			|		ПО ВременнаяТаблица.НомерСтрокиРодителя = ВременнаяТаблица2.НомерСтрокиРодителя";
			
			Запрос.УстановитьПараметр("Документ", ДокументДанные.Ссылка);
			Запрос.УстановитьПараметр("Тэг", "Suppliers_article_number");
			Запрос.УстановитьПараметр("Тэг1", "Colour_code_supplier");
			Запрос.УстановитьПараметр("Тэг2", "EAN");
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Suppliers_article_number = ВыборкаДетальныеЗаписи.Suppliers_article_number;
				Colour_code_supplier = ВыборкаДетальныеЗаписи.Colour_code_supplier;
				...
				Стр.Ссылка = ДокументДанные.Ссылка;
				Стр.Suppliers_article_number = Suppliers_article_number;
				Стр.Colour_code_supplier = Colour_code_supplier;
				Стр.EAN = ВыборкаДетальныеЗаписи.EAN;
				Стр.Артикул = Артикул;
				Стр.НомерСтрокиРодителя = ВыборкаДетальныеЗаписи.НомерСтрокиРодителя;
				Стр.Найден = Ложь;
				
			КонецЦикла;
			
			МассивАртикулов = ТЗ.ВыгрузитьКолонку("Артикул");
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				НайденнаяСтрока = ТЗ.Найти(ВыборкаДетальныеЗаписи.Артикул, "Артикул");
			...
			Для Каждого Строка Из ТЗ Цикл
				...
				Если Не Строка.Найден Тогда
					...
					ЭлементСправочника = Справочники.Номенклатура.НайтиПоРеквизиту("Артикул", Строка.Артикул);
					
					Если Не ЗначениеЗаполнено(ЭлементСправочника.Ссылка) Тогда
						
						НовыйЭлементСправочника = Справочники.Номенклатура.СоздатьЭлемент();
						
						НовыйЭлементСправочника.Наименование = Строка.EAN;
						НовыйЭлементСправочника.НаименованиеПолное = Строка.EAN;
						НовыйЭлементСправочника.Артикул =  Строка.Артикул;
						НовыйЭлементСправочника.ВидНоменклатуры = Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Обувь импортная");
						НовыйЭлементСправочника.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар;
						НовыйЭлементСправочника.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ОбувнаяПродукция;
						НовыйЭлементСправочника.ИспользованиеХарактеристик = Перечисления.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеДляВидаНоменклатуры;
						НовыйЭлементСправочника.ЕдиницаИзмерения = Справочники.УпаковкиЕдиницыИзмерения.НайтиПоНаименованию("пар");
						НовыйЭлементСправочника.СтавкаНДС = Справочники.СтавкиНДС.НайтиПоРеквизиту("Ставка", 20);
						НовыйЭлементСправочника.ВариантОформленияПродажи = Перечисления.ВариантыОформленияПродажи.РеализацияТоваровУслуг;
						НовыйЭлементСправочника.ГруппаДоступа = Справочники.ГруппыДоступаНоменклатуры.НайтиПоНаименованию("ВОРТМАНН");
						НовыйЭлементСправочника.Качество = Перечисления.ГрадацииКачества.Новый;
						Попытка
							НовыйЭлементСправочника.Записать();
						Исключение
							ПодробноеПредставление = НСтр("ru = 'Не удалось создать номенклатуру с артикулом '") +  Строка.Артикул;
...
Процедура ЗагрузитьВетку(Файл, Ветка, ДокОбъект, НаборЗаписей, НомерСтроки)
	...
	Пока Файл.Прочитать() Цикл
		// #wortmann { 
		// Установка даты документа из файла  
		// Галфинд Волков 2022/11/09
		Если Тэг = "Document_date" И Не Файл.Имя = Тэг Тогда
			ДокОбъект.Дата = Дата(Файл.Значение);
		КонецЕсли;
		// } #wortmann
		// BSLLS:Typo-off
