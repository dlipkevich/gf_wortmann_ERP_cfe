...
Перем ФайлОбмена2 Экспорт;            // Последовательно записываемый/читаемый файл обмена для статистики.
Перем НомерСтрокиФайла Экспорт;
...
// MoscowSoft - Начало
Перем ДошлиДоПрошлогоПВД;
Перем КлиентКролика;

Перем ВестиРасширенныйПротоколОшибок Экспорт;
Перем ВыводитьДатуВремяСобытияВПротокол Экспорт;
Перем РазбиватьПротоколНаЧастиРазмераМб Экспорт;
Перем ПредыдущийВыгружаемыйОбъект Экспорт;
Перем МассивИменФайловПротокола Экспорт;
Перем ВариантОтправкиПротоколаВыполнения Экспорт;
Перем РазмерБайтФайлаПротокола Экспорт;
Перем ТекстыОшибок Экспорт;
Перем НазваниеПроектаПереноса Экспорт;
Перем АдресЭлектроннойПочтыДляОтправки Экспорт;
Перем ВерсияПереносаДанных Экспорт;
// MoscowSoft - конец
...
Функция НайтиОбъектПоСвойству(Менеджер, Свойство, Значение,
	...
	// СорокинСС - 27.12.2021 - начало
	// С таким кодом не может найти по реквизиту "Имя" значение пвх ДополнительныеРеквизитыИСведения
	//Если Свойство = "Имя" Тогда
	Если Свойство = "Имя" И СтрНайти(СокрЛП(Менеджер), "Перечисление") Тогда
		
		Возврат Менеджер[Значение];
		
	ИначеЕсли Свойство = "Код" Тогда
	// СорокинСС - 27.12.2021 - конец
...
Процедура ЗаписатьВФайл(Узел) Экспорт
	...
	Если НепосредственноеЧтениеВИБПриемнике Тогда
		...
		КонецЕсли;
	// СорокинСС - 20.10.2020 - начало
	ИначеЕсли мсВыгрузкаRabbitMQ Тогда
		ОтправитьСообщениеВRabbitMQ(ИнформацияДляЗаписиВФайл);
	// СорокинСС - 20.10.2020 - конец
	Иначе
...
Функция ПолучитьСтруктуруЗаписиПротокола(КодСообщения = "", СтрокаОшибки = "") Экспорт
	...
	СтруктураОшибки.Вставить("ПредыдущийВыгружаемыйОбъект");
...
Функция ЗаписатьИнформациюОбОшибкеОбработчикиПКС(КодСообщения, СтрокаОшибки, ПКО, ПКС, Источник = "", 
	...
	// СорокинСС - 12.01.2022 - начало
	Если ЗначениеЗаполнено(ПредыдущийВыгружаемыйОбъект) Тогда
		ЗП.ПредыдущийВыгружаемыйОбъект = "" + ПредыдущийВыгружаемыйОбъект 
									+ "(тип: " + СокрЛП(ТипЗнч(ПредыдущийВыгружаемыйОбъект)) + ")";
		ПредыдущийВыгружаемыйОбъект = Неопределено; // вдруг у следующей ошибки не будет заполнен, а мы ошибочно выведем предыдущий объект
		// от другой выгрузки
	КонецЕсли;
	// СорокинСС - 12.01.2022 - конец
...
Функция НайтиПравило(Объект, ИмяПравила="") Экспорт
	...
	Если Не ПустаяСтрока(ИмяПравила) Тогда
		// СорокинСС - 05.02.2022 - начало
		//Правило = Правила[ИмяПравила];
		Если Правила.Свойство(ИмяПравила) Тогда
			Правило = Правила[ИмяПравила];
		Иначе
			...
			//
			//ЭтоДокумент = Ложь;
			//ЭтоСправочник = Справочники.ТипВсеСсылки().СодержитТип(ЗП.ТипОбъекта);
			//Если Не ЭтоСправочник Тогда
			//	ЭтоДокумент = Документы.ТипВсеСсылки().СодержитТип(ЗП.ТипОбъекта);
			//КонецЕсли;
			//Если ЭтоСправочник Тогда
			//	ЗП.ТипОбъекта = "Справочник """ + СокрЛП(ЗП.ТипОбъекта) + """";
			//КонецЕсли;
			//Если ЭтоДокумент Тогда
			//	ЗП.ТипОбъекта = "Документ """ + СокрЛП(ЗП.ТипОбъекта) + """";
			//КонецЕсли;
			//
			//ЗП.ОписаниеОшибки = "Не найдено правило конвертации """ + ИмяПравила
			//				+ """ при выгрузке объекта """
			//				+ ЗП.Объект + """ типа """ + ЗП.ТипОбъекта + """";
			//ЗаписатьВПротоколВыполнения(45, ЗП, Истина);
			
			Возврат "";
		КонецЕсли;
		// СорокинСС - 05.02.2022 - конец
	Иначе
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаНазванияПКО = " ПКО: " + СокрЛП(ИмяПКО) + "  (" 
							+ ?(ЗначениеЗаполнено(ПКО), СокрЛП(ПКО.Наименование), "Ошибка! ПКО по имени""" + ИмяПКО + """ не найдено!") + ")";
...
Процедура ПередатьОдинПараметрВПриемник(Имя, ИсходноеЗначениеПараметра, ПравилоКонвертации = "") Экспорт
	...
	Иначе
		...
		Если Пусто Тогда
			...
			Если ЗначениеЗаполнено(ПКОСвойств) Тогда
...
Процедура ИнициализироватьВедениеПротоколаОбмена() Экспорт
	...
	Исключение
		Сообщение = "Не получилось создать файл протокола по адресу: " + ИмяФайлаПротоколаОбмена + Символы.ПС + "Текст ошибки: " + ОписаниеОшибки();
		Сообщить(Сообщение);
...
Процедура ЗавершитьВедениеПротоколаОбмена() Экспорт 
	...
	Если Не ЗначениеЗаполнено(ИмяФайлаПротоколаОбмена) Тогда
		Возврат;
	КонецЕсли;
...
Функция ЗаписатьВПротоколВыполнения(Код="", СтруктураЗаписи=Неопределено, ВзвестиФлагОшибок=Истина, 
	...
	// СорокинСС - 13.02.2022 - начало
	// Анализ ошибок в тексте протокола
	Если (ВариантОтправкиПротоколаВыполнения = 3 Или ВариантОтправкиПротоколаВыполнения = 4)
		И (СтрНайти(ИтоговаяСтрокаДляЗаписи, "Ошибка") Или СтрНайти(ИтоговаяСтрокаДляЗаписи, "Не найдено соответствие"))
		И СтрНайти(ТекстыОшибок, ИтоговаяСтрокаДляЗаписи) = 0 Тогда
		
		// надо добавлять обезличенную информацию (без объекта, который выгружается)
		// и искать потом тоже обезличенную. Иначе будет эта переменная пухнуть с каждым воспроизведением ошибки
		ТекстыОшибок = ТекстыОшибок + ИтоговаяСтрокаДляЗаписи;
		
		ТекстПисьма = "Добрый день." + Символы.ПС
						+ "При выполнении переноса обнаружена ошибка с текстом: " 
						+  ИтоговаяСтрокаДляЗаписи;
		ИмяКоманды = "sendEmail";
		СтруктураНастроек = Новый Структура("ТемаПисьма, ТекстПисьма, АдресПолучателяПисьма", 
											НазваниеПроектаПереноса + ". Обнаружена ошибка",
											ТекстПисьма,
											АдресЭлектроннойПочтыДляОтправки);
		Сообщение = ВыполнитьКомандуВебсервисаMoscowSoft(ИмяКоманды, СтруктураНастроек);
		
	КонецЕсли;
	// СорокинСС - 13.02.2022 - конец
...
Функция НайтиЭлементЗапросом(СтруктураСвойств, СвойстваПоиска, ТипОбъекта = Неопределено, 
	МенеджерТипа = Неопределено, КоличествоРеальныхСвойствДляПоиска = Неопределено, ИмяТипаОбъекта = Неопределено)
	...
	Для каждого Свойство Из СвойстваПоиска Цикл
		...
		СтрокаПоиска = СтрокаПоиска + ?(ЗначениеЗаполнено(СтрокаПоиска), ", ", "") + "ИмяПараметра: " + ИмяПараметра + ", ЗначениеПараметра: " + ЗначениеПараметра;
	...
	Исключение
		ТекстОшибки = "Тип объекта: " + ТипОбъекта + ", менеджер типа: " + МенеджерТипа + ", Имя типа объекта: " + ИмяТипаОбъекта
						+ ", Строка поиска: " + СтрокаПоиска
...
Функция НайтиЭлементПоСвойствамПоиска(ТипОбъекта, ИмяТипаОбъекта, СвойстваПоиска, 
	...
	Если ИскатьЗапросом Тогда
		...
		СсылкаНаОбъект = НайтиЭлементЗапросом(СтруктураСвойств, ВременныеСвойстваПоиска, ТипОбъекта, , КоличествоРеальныхСвойствДляПоиска, ИмяТипаОбъекта);
...
Процедура ВыгрузитьСвойства(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, КоллекцияПКС, УзелКоллекцииСвойств = Неопределено, 
	Знач ОбъектКоллекции = Неопределено, ИмяПредопределенногоЭлемента = Неопределено, Знач ВыгрузитьТолькоСсылку = Ложь, 
	СписокВременныхФайлов = Неопределено)
	...
	Для каждого ПКС Из КоллекцияПКС Цикл
		...
		Если Не ПустаяСтрока(ИмяПКО) И ЗначениеЗаполнено(ИмяПКО) Тогда
			
			// СорокинСС - 05.02.2022 - начало
			// ПКОСвойств = Правила[ИмяПКО];
			Если Правила.Свойство(ИмяПКО) Тогда
				ПКОСвойств = Правила[ИмяПКО];
			Иначе
				ПКОСвойств = Неопределено;
				Сообщение = "Не найдено в правилах конвертации правило с именем " + ИмяПКО
							+ " в ПКС " + ПКС.Наименование
							+ " в ПКО " + ПКО.Наименование
							+ " при выгрузке объекта """ + ПредставлениеИсточника(Источник)
							+ """ типа """
							+ СокрЛП(ТипЗнч(Источник)) + """";
				ЗаписатьИнформациюОбОшибкеОбработчикиПКС(55, Сообщение, ПКО, ПКС, Источник, 
						"ПередВыгрузкойСвойства", Значение);
...
Процедура ВыгрузкаОбъектаВыборки(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, ВыборкаДляВыгрузкиДанных = Неопределено)
	...
	Если ВестиРасширенныйПротоколОшибок Тогда
		//ПредыдущийВыгружаемыйОбъект = Объект;
	КонецЕсли;
...
Процедура ОткрытьФайлЗагрузки(ТолькоПрочитатьШапку=Ложь, ДанныеФайлаОбмена = "") Экспорт
	...
	ФайлОбмена2 = Новый ЧтениеXML();

	Попытка
		Если НЕ ПустаяСтрока(ДанныеФайлаОбмена) Тогда
			ФайлОбмена2.УстановитьСтроку(ДанныеФайлаОбмена);
		Иначе
			ФайлОбмена2.ОткрытьФайл(ИмяФайлаЗагрузкиДанных);
		КонецЕсли;
	Исключение
	КонецПопытки;
	
	ФайлОбмена2.Прочитать();
	...
	Если ИмяУзла = "ПравилаОбмена" Тогда
		Если БезопаснаяЗагрузка И ЗначениеЗаполнено(ИмяФайлаПравилОбмена) 
			И (мсВариантЗагрузкиДанных = 0 Или Не мБылиПрочитаныПравилаОбменаПриЗагрузке) Тогда
			
			ЗагрузитьПравилаОбмена(ИмяФайлаПравилОбмена, "XMLФайл");
			ФайлОбмена.Пропустить();
			НомерСтрокиФайла = НомерСтрокиФайла + 1;
		ИначеЕсли мсВариантЗагрузкиДанных = 1 И мБылиПрочитаныПравилаОбменаПриЗагрузке Тогда
			ФайлОбмена.Пропустить();
			НомерСтрокиФайла = НомерСтрокиФайла + 1;
		Иначе
...
Функция СтрокаОсталосьВремя(ВремяНачалаЗагрузки, НомерСтрокиФайла, СтрокВФайле)
	
	ОсталосьСекунд = (СтрокВФайле / НомерСтрокиФайла - 1) * (ТекущаяДатаСеанса() - ВремяНачалаЗагрузки);
	
	Часов = Цел(ОсталосьСекунд/3600);
	Минут = Цел((ОсталосьСекунд-Часов*3600)/60);
	Секунд = ОсталосьСекунд - Часов*3600 - Минут*60;
	
	СтрокаВремениВыполнения = "Осталось примерно: " + ?(Часов=0, "0", СокрЛП(Часов)) + ":" + ?(Минут = 0, "00", Формат(Минут, "ЧЦ=2; ЧДЦ=0; ЧВН=")) + ":" 
								+ ?(Секунд = 0, "00", Формат(Секунд, "ЧЦ=2; ЧН=00; ЧВН="));
...
Процедура ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
	...
	Попытка
	    СтрокВФайле = 0;
		Пока ФайлОбмена2.Прочитать() Цикл
		...
		Пока ФайлОбмена.Прочитать() Цикл
			НомерСтрокиФайла = НомерСтрокиФайла + 1;
			ИмяУзла = ФайлОбмена.ЛокальноеИмя;
			// ЗаписьЖурналаРегистрации("ЗагрузкаДанных.ИмяУзла", , , , "ИмяУзла: " + ИмяУзла);
			
			ПроцентВыполнения = Мин(100, Окр(100 * НомерСтрокиФайла / СтрокВФайле, 0, РежимОкругления.Окр15как20));
			Если ПроцентВыполнения > ПредыдущийПроцент Тогда
				
				СтрокаОсталосьВремя = СтрокаОсталосьВремя(ВремяНачалаЗагрузки, НомерСтрокиФайла, СтрокВФайле);
				
				ДлительныеОперации.СообщитьПрогресс(ПроцентВыполнения, СтрШаблон("Запись: %1 из %2. %3", 
													НомерСтрокиФайла, СтрокВФайле, СтрокаОсталосьВремя));
			КонецЕсли;
			ПредыдущийПроцент = ПроцентВыполнения;
			
			Если ИмяУзла = "Объект" Тогда
				
				ПроцентВыполнения = Окр(100 * НомерСтрокиФайла / СтрокВФайле, 2);
				
				Сообщение = "... " + ТекущаяДатаСеанса() + " - " + СокрЛП(ПроцентВыполнения) + " %";
				// ЗаписьЖурналаРегистрации("МС.ЗагрузкаДанных", УровеньЖурналаРегистрации.Информация, , , Сообщение);
...
Функция ВыполнитьДействияПередЧтениемДанных(СтрокаДанных = "") Экспорт
	...
	Если мсВариантЗагрузкиДанных = 0 Тогда
		Правила.Очистить();
	КонецЕсли;
	...
	Если ПустаяСтрока(СтрокаДанных) И мсВариантЗагрузкиДанных = 0 Тогда
...
Процедура ВыполнитьЗагрузку() Экспорт
	...
	Попытка
		Если Метаданные.Имя <> "BASERP" Тогда
			Выполнить("УстановитьОтключениеБезопасногоРежима(Истина);");
		КонецЕсли;
	Исключение
	КонецПопытки;
	
	Если мсВариантЗагрузкиДанных = 1 Тогда
		// Если включен режим защиты от опасных действий,
		// то в момент инициализации выскочит модальное окно.
	...
	ЗаписьЖурналаРегистрации("ЗагрузкаДанных.РаботаВозможна", , , , "РаботаВозможна = " + РаботаВозможна);
	
	Если НЕ РаботаВозможна Тогда
		Возврат;
	КонецЕсли;	

	// СорокинСС - 18.07.2020 - начало
	// ПроизвестиЧтениеДанных();
	Если мсВариантЗагрузкиДанных = 0 Тогда	
		ПроизвестиЧтениеДанных();
	Иначе
		ПрочитатьСообщенияRabbitMQ();
	КонецЕсли;
	// СорокинСС - 18.07.2020 - конец
...
Процедура ПрочитатьСообщенияRabbitMQ() Экспорт
	...
	КлиентКролика.Connect(мсСерверAMQP, 5672, ПользовательИнформационнойБазыДляПодключения, ПарольИнформационнойБазыДляПодключения, ПользовательИнформационнойБазыДляПодключения);
	
	Попытка
        Потребитель = КлиентКролика.BasicConsume(мсИмяОчереди, "", Истина, Ложь, 0);
        Пока КлиентКролика.BasicConsumeMessage("", ОтветноеСообщение, ТегСообщения, 5000) Цикл
            
			Попытка
				ЛеваяЧасть = Лев(ОтветноеСообщение, 9);
				Если СтрНайти(ЛеваяЧасть, "###") > 0 Тогда
					// сообщение разбито на части. Будем собирать из частей
					Если МассивСообщений = Неопределено Тогда
						МассивСообщений = Новый Массив();
					КонецЕсли;
					ЧастьСообщения = Новый Структура("НомерЧасти, Данные, ПолноеИмяФайла");
					ЧастьСообщения.НомерЧасти = СтрЗаменить(ЛеваяЧасть, "#", "");
					ЧастьСообщения.Данные = Прав(ОтветноеСообщение, СтрДлина(ОтветноеСообщение)-9);
					МассивСообщений.Добавить(ЧастьСообщения);
					
					КлиентКролика.BasicAck(ТегСообщения);
				Иначе
					Если МассивСообщений <> Неопределено Тогда
						ЗагрузитьМассивСообщений(МассивСообщений);
						МассивСообщений = Неопределено;
					Иначе
						ЗагрузитьСообщениеВБазуДанных(ОтветноеСообщение);
				...
				КонецЕсли;
			Исключение
				КлиентКролика.BasicReject(тегСообщения);
			КонецПопытки;			
        ...
        КлиентКролика.BasicCancel("");
    Исключение
        СообщитьПользователю("Ошибка получения сообщений: " + КлиентКролика.GetLastError() + ". Текст из исключения: " + ОписаниеОшибки());
	КонецПопытки;
...
Процедура ЗагрузитьМассивСообщений(МассивСообщений)
	
	ИменаЧастей = Новый Массив();
	Для Каждого ЧастьСообщения Из МассивСообщений Цикл
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла("zip") + ЧастьСообщения.НомерЧасти;
		
		ДД = Base64Значение(ЧастьСообщения.Данные);
	...
	Попытка
		...
		// извлекаем из архива на диск
		ПапкаНазначения = КаталогВременныхФайлов();
		ЭлементАрхива = ЧтениеZipФайла.Элементы[0];
		ЧтениеZipФайла.Извлечь(ЭлементАрхива, ПапкаНазначения, РежимВосстановленияПутейФайловZIP.НеВосстанавливать, );
		
		ИмяФайлаСообщения = КаталогВременныхФайлов() + ЭлементАрхива.ПолноеИмя;
	Исключение
		ИмяФайлаСообщения = ИмяВременногоФайла;
	КонецПопытки;
	
	ТекстовыйФайл = Новый ТекстовыйДокумент();
	ТекстовыйФайл.Прочитать(ИмяФайлаСообщения);
	ТекстСообщения = ТекстовыйФайл.ПолучитьТекст();
	
	ЗагрузитьТекстСообщения(ТекстСообщения);
	
КонецПроцедуры

Функция ТекстСообщенияИзДвоичныхДанных(ДвоичныеДанные)
	
	ТекстСообщения = "";
	
	Попытка
		ДД = Base64Значение(ДвоичныеДанные);
		
		БуферДвоичныхДанных = ПолучитьБуферДвоичныхДанныхИзДвоичныхДанных(ДД);
	...
	Исключение
		// не всегда приходят данные в архиве. Иногда просто строка, если выгружали без сжатия
		ТекстСообщения = ДвоичныеДанные;
	КонецПопытки;
...
Процедура ЗагрузитьСообщениеВБазуДанных(ДвоичныеДанные)
	
	ТекстСообщения = ТекстСообщенияИзДвоичныхДанных(ДвоичныеДанные);
	
	ЗагрузитьТекстСообщения(ТекстСообщения);
	
КонецПроцедуры

Процедура ЗагрузитьТекстСообщения(ТекстСообщения)
	
	// ФайлОбмена - должен быть файл
	Макет = ПолучитьМакет("НачалоФайлаВыгрузки");
	ТекстСообщения = Макет.ПолучитьТекст() + Символы.ПС + СокрЛП(ТекстСообщения) + Символы.ПС + "</ФайлОбмена>";
	
	ОткрытьФайлЗагрузки(Ложь, ТекстСообщения);
	
	ВыполнитьДействияПередЧтениемДанных();
	
	ПроизвестиЧтениеДанных();
	
КонецПроцедуры

Функция ВыполнитьГлобальныйОбработчикПередЗагрузкойДанных()
...
//
// Параметры:
//  ВыгружатьЧастями - если "истина", значит выполняется выгрузка частями и нужно номер части в имя файла вставить
//  НомерЧасти - номер текущей выгружаемой части данных выгрузки
//  СтрВключенныеПравила - строка с информацией о том, какое ПВД (или группа ПВД) в данный момент выгружается. Добавляется в имя файла выгрузки
//  СтруктураНастроек - дополнительные настройки выгрузки. Если заполнены, нужно инициализировать этими значениями настройки модуля объекта
//
Процедура ВыполнитьВыгрузку(ВыгружатьЧастями = Ложь, НомерЧасти = 0, СтрВключенныеПравила = "", 
							СтруктураНастроек=Неопределено) Экспорт
	
	// Сорокин - 06.01.2022 - начало
	Если ВыгружатьЧастями Тогда
		
		НаЧтоЗаменять = "_" + ЗаменитьЗапрещенныеСимволы(СтрВключенныеПравила) + "_" + СтрЗаменить(СокрЛП(НомерЧасти), Символы.НПП, "");
		ИмяФайлаОбмена = СтрЗаменить(ИмяФайлаОбмена, ".xml", НаЧтоЗаменять + ".xml");
		ИмяФайлаОбмена = СтрЗаменить(ИмяФайлаОбмена, ".zip", НаЧтоЗаменять + ".zip");
	КонецЕсли;
	// Сорокин - 06.01.2022 - конец
	
	Если ЗначениеЗаполнено(СтруктураНастроек) Тогда
		ВестиРасширенныйПротоколОшибок     = СтруктураНастроек.ВестиРасширенныйПротоколОшибок;
		ВыводитьДатуВремяСобытияВПротокол  = СтруктураНастроек.ВыводитьДатуВремяСобытияВПротокол;
		РазбиватьПротоколНаЧастиРазмераМб  = СтруктураНастроек.РазбиватьПротоколНаЧастиРазмераМб;
		ВариантОтправкиПротоколаВыполнения = СтруктураНастроек.ВариантОтправкиПротоколаВыполнения;
		НазваниеПроектаПереноса            = СтруктураНастроек.НазваниеПроектаПереноса;
		АдресЭлектроннойПочтыДляОтправки   = СтруктураНастроек.АдресЭлектроннойПочтыДляОтправки;
		ВерсияПереносаДанных               = СтруктураНастроек.ВерсияПереносаДанных;
	КонецЕсли;
	...
	Если НЕ НепосредственноеЧтениеВИБПриемнике Тогда
		...
		Если ПустаяСтрока(ИмяФайлаОбмена) И Не мсВыгрузкаRabbitMQ Тогда
	...
	Попытка
		...
		Если мсВыгрузкаRabbitMQ Тогда
			ОтправитьСообщениеВRabbitMQ(мXMLПравила);
		Иначе
...
Функция СведенияОВнешнейОбработке() Экспорт
    ...
    ПараметрыРегистрации.Вставить("Наименование", "Универсальный обмен от MoscowSoft");
    ПараметрыРегистрации.Вставить("Версия", "1.0");
    ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
    ПараметрыРегистрации.Вставить("Информация", "Универсальный обмен от MoscowSoft");
    ПараметрыРегистрации.Вставить("ВерсияБСП", "2.0.7.6");
    ТаблицаКоманд = ПолучитьТаблицуКоманд();
    ДобавитьКоманду(ТаблицаКоманд,
    "Универсальный обмен от MoscowSoft",
    "1",
    "ОткрытиеФормы",
    Истина,""
...
Процедура ОтправитьСообщениеВRabbitMQ(Сообщение)
	
	
	//// выполнить тест отправки и получения сообщения через RabbitMQ
	//АдресСервера    = "rattlesnake.rmq.cloudamqp.com";
	//ИмяПользователя = "lsleqgcw";
	//Пароль          = "17TnDZ91BXM1OIfmwjBxTzSydmb7pQWp";
	//Порт            = 5672;
	//Хост            = "lsleqgcw";
	//ИмяОчереди      = "test2";
	...
	Если КлиентКролика <> Неопределено Тогда
		Попытка
			КлиентКролика.Connect(мсСерверAMQP, мсПорт, ПользовательИнформационнойБазыДляПодключения, 
							      ПарольИнформационнойБазыДляПодключения, мсХостAMQP, , , 60);
			//Клиент.DeclareQueue(ИмяОчереди, Ложь, Ложь, Ложь, Ложь);
		    КлиентКролика.BasicPublish("", мсИмяОчереди, Сообщение, 0, Ложь);
			Сообщение = "";
			КлиентКролика = Неопределено;
		Исключение
			Сообщение = ОписаниеОшибки();
			СообщитьПользователю(сообщение);
			ОшибкаИзКомпоненты = КлиентКролика.GetLastError();
			СообщитьПользователю("Ошибка из компоненты: " + ОшибкаИзКомпоненты);
			ЗаписьЖурналаРегистрации("ОтправитьСообщениеВRabbitMQ.Ошибка", УровеньЖурналаРегистрации.Ошибка, , , Сообщение);
			ЗаписьЖурналаРегистрации("ОтправитьСообщениеВRabbitMQ.Ошибка", УровеньЖурналаРегистрации.Ошибка, , , ОшибкаИзКомпоненты);
		КонецПопытки;
    КонецЕсли;
...
Процедура УстановитьКомпонентуРозовогоКролика()
	...
	Если ОшибкаСуществованияКомпоненты Тогда
		Попытка
			...
			ДвоичныеДанные = ПолучитьМакет("PinkRabbitMQWin64");
...
Процедура ДлительнаяОперация(СтруктураПараметров, АдресРезультата) Экспорт
	...
	Для Каждого ЭлементСтруктурыРеквизитОбработки Из СтруктураПараметров.РеквизитыОбработки Цикл
		Выполнить(ЭлементСтруктурыРеквизитОбработки.Ключ + " =  ЭлементСтруктурыРеквизитОбработки.Значение;");
	КонецЦикла;
...
Функция ПутьВебСервиса() Экспорт
	
	СтрАдрес = "";
	Попытка
		Соединение = Новый HTTPСоединение("moscowsoft.com");
		
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла();
		
		Запрос = Новый HTTPЗапрос("/webservice_name.txt");
		
		Результат = Соединение.Получить(Запрос);
		
		СтрАдрес = Результат.ПолучитьТелоКакСтроку();
		
		// Сорокин - 02.03.2019 - начало
		// Ответ слишком длинный, скорее всего сработал редирект.
		// Попробуем получить через https
		Если СтрДлина(СтрАдрес) > 30 Тогда
			Соединение = Новый HTTPСоединение("moscowsoft.com", ,,,,, Новый ЗащищенноеСоединениеOpenSSL());
			...
			СтрАдрес = Результат.ПолучитьТелоКакСтроку();
		КонецЕсли;
		// Сорокин - 02.03.2019 - конец
	Исключение
		Возврат "moscowsoft.ddns.net";
	КонецПопытки;
...
Функция ВыполнитьКомандуВебсервисаMoscowSoft(ИмяКоманды, СтруктураПараметров=Неопределено) Экспорт
	
	ИмяСервера = "";
	
	Попытка
		ИмяСервера = ПутьВебСервиса();
		
		Возврат ВыполнитьКомандуВебсервисаПоОдномуИмениСервера(ИмяСервера, ИмяКоманды, СтруктураПараметров);
	Исключение
		
		ОписаниеОшибкиЗапросаПоПервомуИмени = ОписаниеОшибки();
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации("УниверсальныйОбменДаннымиXML.ВыполнитьКомандуВебсервисаMoscowSoft", УровеньЖурналаРегистрации.Ошибка,
									, , "При попытке подключения к веб-сервису обновлений произошла ошибка: " + ТекстОшибки
										+ Символы.ПС + "Рекомендуем обратиться к разработчику по контактам, по которым приобретали разраотку");
			Возврат "Не удалось подключиться к сервису обновлений. ТекстОшибки: " + ТекстОшибки + " Проверьте подключение к интернету, далее обратитесь к разработчику по адресу, по которому приобретали разработку";
		КонецПопытки;
	КонецПопытки;
	
КонецФункции

Функция ДатаПравилКонвертации()
	
	Если Не ЗначениеЗаполнено(ИмяФайлаПравилОбмена) Тогда
		Возврат Дата(2000,1,1);
	Иначе
		ДатаВерсии = ВерсияПравилОбмена;
...
Функция ВыполнитьКомандуВебсервисаПоОдномуИмениСервера(ИмяСервера, ИмяКоманды, СтруктураПараметров)
	
	ИмяПользователя = "Вебсервис"; 
	Пароль = Неопределено;
	strРезультат = "";
	
	Определения = Новый WSОпределения("http://" + ИмяСервера + "/moscowsoft/ws/msConvertationWork.1cws?wsdl", 
	...
	ДатаПравилКонвертации = ДатаПравилКонвертации();
	// strРезультат = Прокси.getStatus(Объект.мсНомерЗаказа, Объект.мсАдресЭлектроннойПочты, ДатаПравилКонвертации);
	
	Если ИмяКоманды <> "sendEmail" Тогда
		Выполнить("strРезультат = Прокси." + ИмяКоманды + "(мсНомерЗаказа, мсАдресЭлектроннойПочты, ДатаПравилКонвертации);");
	Иначе
		ТемаПисьма = СтруктураПараметров.ТемаПисьма;
		ТекстПисьма = СтруктураПараметров.ТекстПисьма;
		АдресПолучателяПисьма = СтруктураПараметров.АдресПолучателяПисьма;
		
		ХранилищеФайловПротокола = ХранилищеФайловПротокола();
		
		Выполнить("strРезультат = Прокси.sendEmail(мсНомерЗаказа, мсАдресЭлектроннойПочты, ТемаПисьма, "
				+ "ТекстПисьма, АдресПолучателяПисьма, ХранилищеФайловПротокола);");
	КонецЕсли;
...
Функция ХранилищеФайловПротокола()
	// создаем во временной папке архив-зип.
	// В него добавляем все файлы протокола.
	// сохраняем архив
	// помещаем данные файла архива в хранилище значения
	// передаем как параметр функции веб-сервиса
	
	Если ВариантОтправкиПротоколаВыполнения <> 1 Тогда
	...
	НовыйАрхив = Новый ЗаписьZipФайла(
			        ИмяАрхива,
			        , // пароль на архив
			        "Если есть вопросы, пишите на support@moscowsoft.com", // комментарий к архиву
			        МетодСжатияZIP.Сжатие, // сжатие или копирование
			        УровеньСжатияZIP.Максимальный,
			        МетодШифрованияZIP.Zip20
			    );
				
	Для Каждого Элемент Из МассивИменФайловПротокола Цикл				
		Попытка
			НовыйАрхив.Добавить(Элемент.Значение);
		Исключение
		КонецПопытки;
	КонецЦикла;
	
	Попытка
		НовыйАрхив.Добавить(ИмяФайлаПротоколаОбмена);
	Исключение
	КонецПопытки;
	
	НовыйАрхив.Записать();
	
	Файл = Новый ДвоичныеДанные(ИмяАрхива);
...
Функция ПолучитьПравилаНаСервере(ИмяСервера) Экспорт
	...
	Если ВерсияПереносаДанных = 0 Тогда
		ФайлПравил = Прокси.getConvertationRules(мсНомерЗаказа, мсАдресЭлектроннойПочты);
	Иначе
		ФайлПравил = Прокси.getConvertationRulesAdditional(мсНомерЗаказа, мсАдресЭлектроннойПочты, ВерсияПереносаДанных);
	КонецЕсли;
	
	Адрес = ПоместитьВоВременноеХранилище(ФайлПравил.binaryData, Новый УникальныйИдентификатор());
...
Функция ПредставлениеИсточника(Источник)
	...
	Если ТипЗначенияИсточника = Тип("ВыборкаДанных") 
		Или ТипЗначенияИсточника = Тип("ВыборкаИзРезультатаЗапроса") Тогда
		
		Возврат СокрЛП(ТипЗначенияИсточника);
	Иначе
		Попытка
			Возврат СокрЛП(Источник);
		Исключение
			Возврат "Ошибка представления у значения типа " + ТипЗначенияИсточника;
		КонецПопытки;
	КонецЕсли;

КонецФункции

Функция ОтправитьСмс(ТекстСообщения, НомерТелефона)

	АдресСервера = "sms.ru";
	ТелефонОтправка = НомерТелефона; // "+79263764403";
	API_ID = ТокенСмсРу;
	
	HTTPСоединение = Новый HTTPСоединение(АдресСервера);
	HTTPЗапрос = Новый HTTPЗапрос("https://sms.ru/sms/send?api_id="+API_ID+"&to="+
	       ТелефонОтправка+"&msg="+ТекстСообщения+"&json=0");
...
Функция ОтправитьВсемСмс() Экспорт
	
	Если Не ОтправлятьУведомленияОбОкончанииОбменаПоСмс Или Не ЗначениеЗаполнено(ТелефоныДляУведомлений)
		Или Не ЗначениеЗаполнено(ТокенСмсРу) Тогда
		Возврат "";
	КонецЕсли;
	
	ТекстСообщения = "Zavershen obmen dannymi";
	
	МассивТелефонов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ТелефоныДляУведомлений, ",", Истина);
	ТекстОтвета = "";
	Для Каждого Телефон Из МассивТелефонов Цикл
		ТекстОтвета = ТекстОтвета + ОтправитьСмс(ТекстСообщения, Телефон);	
	КонецЦикла;
...
ДошлиДоПрошлогоПВД = Ложь;
КлиентКролика      = Неопределено;

РазмерБайтФайлаПротокола = 0;
МассивИменФайловПротокола = Новый Массив();
ТекстыОшибок = "";
