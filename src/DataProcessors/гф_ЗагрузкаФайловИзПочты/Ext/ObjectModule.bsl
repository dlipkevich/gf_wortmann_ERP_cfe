...
Процедура ЗагрузитьФайлыИзПочтовыхЯщиков(ИнформацияОЗагруженныхФайлах, ПочтовыеЯщики = Неопределено) Экспорт
	...
	Для Каждого ТекЯщик Из ПочтовыеЯщики Цикл
		...
		ИнформацияОПочтовомЯщике.Вставить("EMail", ТекЯщик.EMail);
...
Функция ЗагрузитьФайлыИзПочтовогоЯщика(ПочтовыйЯщик, ЗагруженныеФайлы)
	...
	ПротоколПодключения = ?(ПочтовыйЯщик.IMAP, ПротоколИнтернетПочты.IMAP, ПротоколИнтернетПочты.POP3);
	...
	Если Заголовки.Количество() = 0 Тогда
		...
		ТекстОшибки = ОписаниеОшибки();
		СтрокаСообщения = "В почтовом ящике """ + ПочтовыйЯщик.EMail + """ не найдено входящих сообщений.  " + ТекстОшибки;
	...
	Иначе 
		...
		СтрокаСообщения = "В почтовом ящике """ + ПочтовыйЯщик.EMail + """ найдено входящих сообщений: " + Формат(Заголовки.Количество(), "ЧДЦ=0");
	...
	Для Каждого ТекПисьмо Из Письма Цикл
		ЗагруженоФайлов = ЗагрузитьФайлыИзПисьма(ТекПисьмо, ЗагруженныеФайлы, ПочтовыйЯщик.EMail, ТекКаталог);
