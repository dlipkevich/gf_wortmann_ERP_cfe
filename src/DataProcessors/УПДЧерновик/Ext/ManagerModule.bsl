...
Функция СформироватьПечатнуюФормуЗаявкаНаЭкспрот(СтруктураТипов, ОбъектыПечати, ПараметрыПечати = Неопределено) Экспорт
	...
	Для Каждого Стр Из Таб[0].гф_ТоварыВКоробах Цикл
		...
		Для Каждого Тов Из Таб[0].Товары Цикл
			...
			ОбластьСтрока.Параметры.КодТНВЭД            = Тов.КодТНВЭД.Код;
...
Процедура ЗаполнитьТабличныйДокументУПД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, СтруктураТипов) Экспорт
	...
	МакетУПД = УправлениеПечатью.МакетПечатнойФормы("Обработка.УПДЧерновик.ПФ_MXL_УПД_Свернутый");
	МакетУПД_625 = УправлениеПечатью.МакетПечатнойФормы("Обработка.УПДЧерновик.ПФ_MXL_УПД_Свернутый");
	МакетУПД_981 = УправлениеПечатью.МакетПечатнойФормы("Обработка.УПДЧерновик.ПФ_MXL_УПД_Свернутый");
	МакетУПД_534 = УправлениеПечатью.МакетПечатнойФормы(
						"Обработка.УПДЧерновик.ПФ_MXL_УПД_Свернутый");
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ПервичныеДокументыВКоробах Тогда 
			...
			ОбластьИтого.Параметры.Заполнить(СтрутураИтого);
