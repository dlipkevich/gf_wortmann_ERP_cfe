...
// BSLLS:LatinAndCyrillicSymbolInWord-on		
	...
	Исключение 
		ОтменитьТранзакцию();
		Сообщение = Формат(ТекущаяДатаСеанса(), "ДЛФ=T") + " - Возникла ошибка загрузки номенклатуры";
...
Функция НайтиЗначениеНастройкиЗагружаемыхДанных(Значение)
	...
	Запрос.УстановитьПараметр("Интерфейс", Перечисления.гф_Интерфейсы.ORDRSP);
...
Процедура ФормированиеВидаНоменклатурыПолСтавки(НовыйЭлемент, СтрокаН, СтрокаТЧ) 
	...
	КонецЕсли;
	// ++ Галфинд_Домнышева_06_02_2024
	Если НЕ НовыйЭлемент.ИспользованиеХарактеристик = ВидНоменклатуры.ИспользованиеХарактеристик Тогда
		 НовыйЭлемент.ИспользованиеХарактеристик = ВидНоменклатуры.ИспользованиеХарактеристик;
	КонецЕсли;
	// -- Галфинд_Домнышева_06_02_2024
...
Процедура ЗаписатьОшибку(ТекстыОшибок)
	...
	Попытка	
		...
		// ++ Галфинд_ДомнышеваКР_10_11_2023
		// Если файл был загружен ранее, но у док. статус ЗагруженосОшибками или просто Загружен, то
		// в РС гф_ОшибкиЗагрузки уже есть аналогичные записи на другую дату, по этому отбираем еще раз и по дате.
