...
Процедура НайдемКонтрагента(СтрокаТЧ, НовыйДокумент, Отказ) 
	...
	Если ЗначениеЗаполнено(НайденныйКонтрагент) Тогда
		...
		ДоговорДляКонтрагента(НовыйДокумент, СтрокаТЧ);
...
Функция УстановитьСезон(СтрокаТЧ, Отказ, Сезон = Неопределено)
	...
	Запрос.УстановитьПараметр("Сезон", Лев(Сезон, 6));
	РезультатЗапроса = Запрос.Выполнить();
...
//
Процедура ДоговорДляКонтрагента(НовыйДокумент, СтрокаТЧ)	
	
	Договор = ПолучитьДоговорПоСезону(НовыйДокумент, СтрокаТЧ);
	...
	Если НовыйДокумент.Договор <> Договор Тогда
...
Функция ПолучитьДоговорПоСезону(НовыйДокумент, СтрокаТЧ)
