...
Процедура СозданиеПроведениеУстановкиЦен(ТабличнаяЧасть, СтруктураЦен, Отказ) 
	...
	Для Каждого СтрокаТЧ Из ТабличнаяЧасть Цикл 
		...
		Если УсловиеСоздания  Тогда
			...
			СозданиеСтрокПоХН(СтрокаТЧ, ВидЦеныСтроки, ЭлементНоменклатуры, ДокументУстановкаЦен, ВалютаДокумента, Отказ);
...
// #wortmann { 
// Вызывает процедуру записи в табличную часть Товары2_5 документа УстановкаЦенНоменклатуры
// по каждой характеристики для ЭлементНоменклатуры
// Галфинд_Домнышева 2023/01/25
//
// Параметры:
//	СтрокаТЧ - Тип.СтрокаТЧ - текущая строка табличной части НаборЗаписейТэгов 
//	ВидЦеныСтроки - СправочникСсылка.ВидыЦен - загружаемый ВидЦен в ТЧ 
//	ЭлементНоменклатуры - СправочникСсылка.Номенклатура - загружаемая номенклатура в ТЧ
//	ДокументУстановкаЦен - ДокументСсылка.УстановкаЦенаНоменклатуры - создаваемый документ
//	ВалютаДокумента - СправочникСсылка.Валюты
//	Отказ - системный параметр
Процедура СозданиеСтрокПоХН(СтрокаТЧ, ВидЦеныСтроки, ЭлементНоменклатуры, ДокументУстановкаЦен, ВалютаДокумента, Отказ)
...
//	ПараметрыОтбора - Структура - содержит "Номенклатура", "ВидЦены", "ХарактеристикаЦО" 
// 
// Возвращаемое значение:
...
Процедура СделатьЗаписьУстановкиЦен(ПараметрыОтбора, пар_ДокУстановкиЦен, СтрокаТЧ, ВалютаДокумента, Отказ)
	...
	Если ЕстьЗаписьСТакимиПараметрамиВЦенах(СтруктураВходПар) Тогда
		СообщениеОбОшибке = "Для Артикула " + ПараметрыОтбора.Номенклатура.Артикул + " в периоде " 
							+ Формат(ЭтотОбъект.Дата, "ДЛФ=Д") + " цена уже записана. Не обрабатывается";
		ДобавитьСтрокуВТекстыОшибки(СообщениеОбОшибке);
		ЗаписьЖурналаРегистрации("Ошибка", УровеньЖурналаРегистрации.Ошибка, ЭтотОбъект, ЭтотОбъект, СообщениеОбОшибке);
		Возврат;
	КонецЕсли;	
...
Процедура ЗагрузитьТЧТовары(НайденныйВариантКомплектации, НовыйЭлементТоварыВКоробах, НовыйДокумент, ДатаОтгрузки)
	...
	Для каждого СтрокаСостава Из НайденныйВариантКомплектации.Товары Цикл
		...
		НаименованиеХН = СтрокаСостава.Характеристика.Наименование; 
		НоваяСтрока.ХарактеристикаЦО = 
										Справочники.ХарактеристикиНоменклатурыДляЦенообразования.НайтиПоНаименованию(НаименованиеХН);
