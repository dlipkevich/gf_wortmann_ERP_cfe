...
Процедура гф_ПриЗаписи(Отказ)
	...
	Если ЭтотОбъект.СтатусДокумента = Перечисления.гф_СтатусыДокументовЗагрузки.ЗагруженоСОшибками Тогда
		ЗаписьЖурналаРегистрации("ЗагрузкаИзI5", УровеньЖурналаРегистрации.Ошибка, ЭтотОбъект, ЭтотОбъект, 
	...
	Если Интерфейс <> Перечисления.гф_Интерфейсы.PRICAT Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации("ЗагрузкаИзI5", УровеньЖурналаРегистрации.Ошибка, ЭтотОбъект, ЭтотОбъект, СообщениеОбОшибке);
	...
	Если Результат.Следующий() Тогда
		...
		Если ФлагЗаписиДокумента Тогда
			...
			Если СокрЛП(ФункцияДокумента) = "27" Тогда
				...
				ЗаписьЖурналаРегистрации("ЗагрузкаИзI5", УровеньЖурналаРегистрации.Информация, ЭтотОбъект, ЭтотОбъект, 
				СообщениеОбОшибке);
...
Процедура ОбработкаВариантаКомплектацииИРасхождений(НовыйДокумент, НовыйЭлементТоварыВКоробах,
	...
	Если ЗначениеЗаполнено(НайденныйВариантКомплектации) Тогда
		...
		Иначе 
			...
			ДобавитьСтрокуВТекстыОшибки(СообщениеОбОшибке);
			ЗаписьЖурналаРегистрации("ЗагрузкаИзI5", УровеньЖурналаРегистрации.Предупреждение, , ЭтотОбъект, СообщениеОбОшибке);
	...
	Если НайденноеЗначение.Количество() > 0 Тогда
		...
		Иначе
			...
			Иначе
				...
				ДобавитьСтрокуВТекстыОшибки(СообщениеОбОшибке);
			КонецЕсли;
			ЗаписьЖурналаРегистрации("ЗагрузкаИзI5", УровеньЖурналаРегистрации.Ошибка, Ссылка, Ссылка, СообщениеОбОшибке);
			Возврат Неопределено;
		КонецЕсли;
	Иначе
