...
Процедура РассчитатьСкидкуВКоробах(НовыйДокумент, ТаблицаЗначенийЦенИзИ5, ТаблицаРасхождений)
	...
	СтавкаНДСДляПередачи = ПолучитьСтавкуНДСДоговораПередачи(НовыйДокумент.Договор); // Галфинд_ДомнышеваКР_03_05_2024
	...
	Для каждого Строка Из НовыйДокумент.гф_ТоварыВКоробах Цикл
		...
		Строка.СтавкаНДС = СтавкаНДСПоВариантуКомплектации(Строка.ВариантКомплектации, ПараметрыДляСтавкиНДС, СтавкаНДСДляПередачи);
...
// #wortmann { 
// Для договоров с Типом "Передача на комиссию" необходимо использовать СтавкуНДС договора
// Галфинд_Домнышева 03_05_2024
//
// Параметры:
//	Договор - СправочникСсылка.ДоговораыКонтрагентов
//
// Возвращаемое значение:
//	СправочникСсылка.СтавкиНДС - значение ставки договора или Неопределено
Функция ПолучитьСтавкуНДСДоговораПередачи(Договор)

	Если Договор.ТипДоговора = Перечисления.ТипыДоговоров.СКомиссионером Тогда
...
Функция СтавкаНДСПоВариантуКомплектации(ВариантКомплектации, ПараметрыДляСтавкиНДС, СтавкаНДСДляПередачи)
	
	Результат = Справочники.СтавкиНДС.ПустаяСсылка();
	// ++ Галфинд_ДомнышеваКР_03_05_2024
	Если ЗначениеЗаполнено(СтавкаНДСДляПередачи) Тогда
		 Возврат СтавкаНДСДляПередачи;
	КонецЕсли;
	// -- Галфинд_ДомнышеваКР_03_05_2024
...
Процедура НайдемКонтрагента(СтрокаТЧ, НовыйДокумент, Отказ) 
	...
	ДоговорДляКонтрагента(НовыйДокумент, СтрокаТЧ, GLN_номер, Отказ);
...
//	Отказ - параметр системы
//
Процедура ДоговорДляКонтрагента(НовыйДокумент, СтрокаТЧ, GLNНомер, Отказ)	
	...
	Если НЕ ЗначениеЗаполнено(Договор) Тогда
		СообщениеОбОшибке = "По документу " + НовыйДокумент + " Договор найден в количестве > 1 или не найден. "
		+ Символы.ПС + ЭтотОбъект + " в строке № " + СтрокаТЧ.ПорядковыйНомерСтроки;
		ДобавитьСтрокуВТекстыОшибки(СообщениеОбОшибке);
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации, УровеньЖурналаРегистрации.Ошибка, , ЭтотОбъект, СообщениеОбОшибке);
		Отказ = Истина;
	КонецЕсли;	
	...
	Иначе
		...
		Если Договор.ТипДоговора = Перечисления.ТипыДоговоров.СКомиссионером Тогда
			...
			НовыйДокумент.гф_ВидЦены =
			_омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначение("гф_ГлобальныеЗначенияРозничнаяЦена");
		КонецЕсли;
		// -- Галфинд_ДомнышеваКР_03_05_2024
	КонецЕсли;
КонецПроцедуры// } #wortmann
...
Функция ПолучитьДоговорПоСезону(НовыйДокумент, СтрокаТЧ, GLNНомер)
	...
	Если Не РезультатЗапроса.Пустой() И НЕ Выборка.Количество() > 1 Тогда
