...
Процедура ПроходимЦиклПоТЭГ(НовыйДокумент, ТабличнаяЧасть, ТекущаяДата, ТаблицаЗначенийЦенИзИ5,
	...
	Для Каждого СтрокаТЧ Из ТабличнаяЧасть Цикл // Цикл по тегам XML
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.ПолеЗагрузки) Тогда
			...
			// остальные тэги	
			Иначе 
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТЧ.ПолеЗагрузки, "ТипОбъекта")
					И СтрокаТЧ.ПолеЗагрузки.ТипОбъекта = "Заказ клиента" И НовыйДокумент <> Неопределено Тогда
...
//
Процедура НеИспользуется_ЗаписатьОшибку(ТекстыОшибок)
