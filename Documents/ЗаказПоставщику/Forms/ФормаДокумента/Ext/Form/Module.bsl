...
Процедура гф_ПродукцияВКоробахIDКоробаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка) 
	...
	Если ТекущиеДанные <> Неопределено Тогда   //Добавлено  Галфинд \ Sakovich 07.11.2023
...
// #wortmann {
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee530b90402f34
// Галфинд Sakovich 2023/09/14
&НаКлиенте
...
//+++ БФ Бобнев К.С. 26.03.24
&НаКлиенте
Процедура бф_ЗагрузитьЦеныИзТабДока(Команда)
	ОписаниеОповещения = Новый ОписаниеОповещения("бф_ПослеЗакрытияТабДока", ЭтотОбъект);
	ОткрытьФорму("Документ.ЗаказПоставщику.Форма.бф_ФормаЗагрузкиИзТабличногоДокумента",, ЭтотОбъект,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
//--- БФ Бобнев К.С. 26.03.24
#КонецОбласти
...
Процедура гф_СоздатьНовыеРеквизиты()
	...
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee78c2bb2ea7c9 (п.5)
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		Элементы["гф_IDКоробов"].ИзменятьСоставСтрок = Ложь;
		Элементы["гф_IDКоробов"].ИзменятьПорядокСтрок = Ложь;
		Элементы["IDКоробовIDКороба"].ТолькоПросмотр = Истина;
		Элементы["IDКоробовВариантКомплектации"].ТолькоПросмотр = Истина;
	КонецЕсли;
	...
	//+++ БФ Бобнев К.С. 26.03.24
	НоваяКоманда = Команды.Добавить("бф_ЗагрузитьЦеныИзТабДока");
	НоваяКоманда.Заголовок = "Загрузить цены";
	НоваяКоманда.Действие = "бф_ЗагрузитьЦеныИзТабДока";
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина; 
	
	НоваяКнопка = Элементы.Добавить("бф_ЗагрузитьЦеныИзТабДока", ТипКнопкаФормы, 
		Элементы.гф_ПродукцияВКоробах.КоманднаяПанель);
	НоваяКнопка.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
	НоваяКнопка.ИмяКоманды = "бф_ЗагрузитьЦеныИзТабДока";
	//--- БФ Бобнев К.С. 26.03.24
КонецПроцедуры	
...
Процедура гф_ДоступностьЭлементовГруппыЭмиссияКМ()
	...
	// Галфинд \ Sakovich 03.11.2023
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee78c2bb2ea7c9 (п.2)
	Элементы.гф_ЗаказатьКМ.Доступность = Объект.Статус = Перечисления.СтатусыЗаказовПоставщикам.Подтвержден;
...
Процедура ОбработатьУпаковочныеЛистыНаСервере()
	...
	Менеджер= Документы.ЗаказПоставщику;   //Добавлено  Галфинд \ Sakovich 07.11.2023
	...
	Для каждого стрТз Из тзПродукцияВКоробах Цикл
		...
		Если ЗначениеЗаполнено(стрТз["ВариантКомплектации"]) И ЗначениеЗаполнено(стрТз["КоличествоКоробов"]) Тогда
			Если тзIDКоробов.Количество()>0 И 
				Менеджер.УпаковочныеЛистыСозданыВсе(Объект, стрТз["ВариантКомплектации"], стрТз["КоличествоКоробов"]) Тогда
	...
	Для каждого Эл Из мСтрокДляРазмножения Цикл
		...
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8ea8b083fed1320811ee0a1be12f8135
		ГраницаСчета = Менеджер.ОпределитьГраницуСчета(Объект, Эл);
	...
	Для каждого строкаУпЛист Из мСтрокДляСозданияУпЛистов Цикл
		...
		//ДокУпЛист.гф_Поставка = Объект.Ссылка; //Изменено  Галфинд \ Sakovich 31.10.2023 10:30:48   old:<>
		...
		Для каждого стрТч Из тчВариантКомплектации Цикл
			...
			ЗаполнитьЗначенияСвойств(нс, стрТч, "Номенклатура, Характеристика, Упаковка, КоличествоУпаковок, Количество"); //Изменено  Галфинд \ Sakovich 31.10.2023 10:31:14   old:<>
		...
		Иначе
			ДокУпЛист.ДополнительныеСвойства.Вставить("ПараметрСклад", Объект.Склад);
		КонецЕсли;
		ДокУпЛист.ДополнительныеСвойства.Вставить("ЗаказПоставщику", Объект.Ссылка);
		...
		//		// --Галфинд_ДомнышеваКР_16_06_2023
		//		ДокУпЛист.гф_ДатаАгрегации = Дата(1,1,1); // Галфинд_ДомнышеваКР_27_10_2023
		//		Попытка
		...
		Попытка
			// режим Проведение (хотя тут нужен режим Запись) - для того, чтобы сагрерировать Штриходы
			ДокУпЛист.Записать(РежимЗаписиДокумента.Проведение); 
			строкаУпЛист["IDКороба"] = ДокУпЛист.Ссылка;
		Исключение
...
//Изменено  Галфинд \ Sakovich 08.11.2023 Перенесено в модуль менеджера
//&НаСервере
...
//Изменено  Галфинд \ Sakovich 07.11.2023 Перенесено в модуль менеджера
//// #wortmann {
//// e1cib/data/Задача.ЗадачаИсполнителя?ref=8ea8b083fed1320811ee01247e7e0c5a
//// Галфинд_ДомнышеваКР_07_06_2023
//&НаСервере
...
Процедура бф_ПослеЗакрытияТабДока(Результат, ДополнительныеПараметры) Экспорт
    ...
    ОбластьЯчеек = Результат.Область(1, 1, Результат.ВысотаТаблицы, Результат.ШиринаТаблицы);
	...
	ТаблицаЦен.Колонки.Добавить("ЦенаЧисло", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
	ТаблицаЦен.ЗагрузитьКолонку(ТаблицаЦен.ВыгрузитьКолонку("Цена"),"ЦенаЧисло");
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаЦен.Артикул КАК Артикул,
		|	ТаблицаЦен.ЦенаЧисло КАК Цена
		|ПОМЕСТИТЬ ВТ_Цены
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЗаказПоставщикугф_ПродукцияВКоробах.ВариантКомплектации КАК ВариантКомплектации,
		|	ЗаказПоставщикугф_ПродукцияВКоробах.ВариантКомплектации.Владелец.Артикул КАК Артикул,
		|	ЕСТЬNULL(ВТ_Цены.Цена, 0) КАК Цена,
		|	ЕСТЬNULL(ЗаказПоставщикугф_ПродукцияВКоробах.ВариантКомплектации.Владелец.СтавкаНДС.Ставка, 0) КАК СтавкаНДС,
		|	ЗаказПоставщикугф_ПродукцияВКоробах.ВариантКомплектации.КоличествоУпаковок КАК Количество,
		|	ЕСТЬNULL(ВТ_Цены.Цена, 0) * ЗаказПоставщикугф_ПродукцияВКоробах.ВариантКомплектации.КоличествоУпаковок КАК ЦенаКороба
		|ИЗ
		|	Документ.ЗаказПоставщику.гф_ПродукцияВКоробах КАК ЗаказПоставщикугф_ПродукцияВКоробах
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Цены КАК ВТ_Цены
		|		ПО ЗаказПоставщикугф_ПродукцияВКоробах.ВариантКомплектации.Владелец.Артикул = ВТ_Цены.Артикул
		|ГДЕ
		|	ЗаказПоставщикугф_ПродукцияВКоробах.Ссылка = &Ссылка";
	...
	Для каждого СтрокаТЧ Из Объект.гф_ПродукцияВКоробах Цикл
		НайденнаяСтрока = РезультатТаблица.Найти(СтрокаТЧ.ВариантКомплектации, "ВариантКомплектации");
		Если НайденнаяСтрока = Неопределено Тогда
			ОбщегоНазначения.СообщитьПользователю("Для строки " + СтрокаТЧ.НомерСтроки + " не указана новая цена");
			Продолжить;
		ИначеЕсли НайденнаяСтрока.Цена = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю("Для строки " + СтрокаТЧ.НомерСтроки + " не указана новая цена");
			Продолжить;
		КонецЕсли;     
		
		СтрокаТЧ.СтоимостьКороба = НайденнаяСтрока.ЦенаКороба;
		СтрокаТЧ.Сумма = СтрокаТЧ.КоличествоКоробов * СтрокаТЧ.СтоимостьКороба;
		СтрокаТЧ.НДС = СтрокаТЧ.Сумма * НайденнаяСтрока.СтавкаНДС / (100 + НайденнаяСтрока.СтавкаНДС);
