...
Процедура гф_ЗаполнитьТоварыПоТоварамКПоступлению(ВидЗаполнения, ДатаПоступления)
	...
	Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
		...
		|	УпаковочныйЛистТовары.КоличествоУпаковок * ПроизводствоБезЗаказагф_ПродукцияВКоробах.КоличествоКоробов КАК КоличествоУпаковок,
		|	УпаковочныйЛистТовары.КоличествоУпаковок * ПроизводствоБезЗаказагф_ПродукцияВКоробах.КоличествоКоробов КАК Количество
		|ИЗ
	...
	Пока ВыборкаУпЛист.Следующий() Цикл
		...
		Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
			// примечание: Галфинд \ Sakovich 18.01.2023	
			// для других документов впоследствии сделать так же, как и для ПроизводствоБезЗаказа
			// (необходимо переработать запросы)
			стрУпЛист.КоличествоУпаковок = ВыборкаУпЛист.КоличествоКоробов;
			стрУпЛист.Количество = ВыборкаУпЛист.КоличествоКоробов;
		Иначе
