...
Процедура гф_ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	И ЭтоНовый() // Галфинд_ДомнышеваКР_07_03_2024
	Тогда
		гф_ЗаполнитьТовары();	
	КонецЕсли;
...
Процедура гф_ОбработкаПроведения(Отказ, РежимПроведения)
	// Галфинд СадомцевСА 04.12.2023 При проведении: если Статус "Принят" то в Перемещении устанавливаю статус "Принято"
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(Распоряжение) <> Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
		Возврат;
	КонецЕсли;
	Если Распоряжение.Статус <> Перечисления.СтатусыПеремещенийТоваров.Принято Тогда
		ДокПеремещение = Распоряжение.ПолучитьОбъект();
