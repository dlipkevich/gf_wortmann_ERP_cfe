...
Процедура гф_ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ПроведениеДокументаОсновная(Отказ, РежимПроведения);
	
	МенеджерВременныхТаблиц = ПроведениеДокументов.МенеджерВременныхТаблицКонтроля(ЭтотОбъект);
	ПроведениеДокументов.УстановитьДопСвойстваРегистра(Движения.ТоварыОрганизаций, ЭтотОбъект, МенеджерВременныхТаблиц, Истина); 
	
	Движения.Записать();
	
	ВыполнитьКонтрольРезультатовПроведения(ЭтотОбъект, МенеджерВременныхТаблиц, Отказ);
...
Процедура ПроведениеДокументаОсновная(Отказ, РежимПроведения)
...
Процедура ВыполнитьКонтрольРезультатовПроведения(Документ, МенеджерВременныхТаблиц, Отказ)
	...
	Документ.ДополнительныеСвойства.Вставить("ПараметрыЗаполненияВидовЗапасов", ПараметрыЗаполнения);
	
	Запрос        = Новый Запрос;
	ТекстыЗапроса = Новый СписокЗначений;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	
	ЗапасыСервер.ИнициализироватьДанныеКонтроляИзменений(Запрос, ТекстыЗапроса, Документ);
	...
	ЗапасыСервер.СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ);
	
	Если Отказ Тогда
		
		Если ПроведениеДокументов.СвойстваДокумента(Документ).РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
