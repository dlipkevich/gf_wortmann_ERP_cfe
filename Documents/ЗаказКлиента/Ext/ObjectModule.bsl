...
Процедура гф_ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// #wortmann { 
	// Галфинд_Домнышева 2024/04/24
	// Перед удалением док. необходимо проверять наличие обособленных остатков
	Если Не Ссылка.ПометкаУдаления И ПометкаУдаления И ПроверитьПолучитьОбособленныеОстатки() Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = '%1 . Под заказ зарезервирован товар. Чтобы удалить заказ, необходимо снять резерв'"), ЭтотОбъект.Ссылка);;
	...
	// #wortmann {
	// #При записи или проведении заполняем типовой реквизит АдресДставки по значению адреса элемента
	// указанного в реквизите гф_АдресДоставки
	// Галфинд Volkov 2023/06/19
	АдресДоставкиИзСправочника = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(гф_АдресДоставки, "Адрес");	
	Если Не ЗначениеЗаполнено(АдресДоставки) Или АдресДоставки <> АдресДоставкиИзСправочника Тогда
		АдресДоставки = АдресДоставкиИзСправочника;
	КонецЕсли;
	// } #wortmann
	
	// ++ Галфинд СадомцевСА 28.12.2023 Реализовал проверку заполнения Адреса доставки при проведении документа
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eea4c450d64a34
	Если Не ЗначениеЗаполнено(АдресДоставки)
		// СадомцевСА 05.02.2024 Убрал условие "Проведение = Истина" при проверке Адреса доставки
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eebe86ae5e0112
		//И РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = '%1 необходимо заполнить реквизит ""Адрес доставки для печати"".'"), ЭтотОбъект.Ссылка);;
		ОбщегоНазначения.СообщитьПользователю(
			ТекстОшибки,
			ЭтотОбъект,
			"АдресДоставки", 
	...
	Если Отказ Тогда
		Возврат;
	КонецЕсли;	
	// -- Галфинд СадомцевСА 28.12.2023
	
	Если Не ДополнительныеСвойства.Свойство("гф_ОбработкаОтгрузкаПоЗаказам") Тогда
		...
		Если ЭтоТоварыВКоробах(Склад) Тогда
			...
			Документы.ЗаказКлиента.ЗаполнитьТоварыНаСервере(ЭтотОбъект);
			// -- Галфинд Volkov 2024/01/30
		КонецЕсли;
		// -- Галфинд СадомцевСА 18.10.2022
	КонецЕсли;
	
	// Проверка и заполнеие идентификатора строки, идентификатор требуется для записи истории
	// в отдельную таблицу "гф_ПричиныИзмененияТоваровВКоробах", история по изменению количества варианта
	// комплектации в процессе частичных отгрузок упаковочных листов соответствующих этому варианту
	// СадомцевСА 27.04.2024 Исправил ошибки СонарКуба. Перенес код в процедуру.
	ЗаполнитьПричиныИзмененияТоваровВКоробах();
	...
	//	КонецЕсли;
	//	
	//	// Заполнение первой строки истории по варианту комплектации в количестве отгруженных упаковочных листов
	//	// соответствующих этому варианту комплектации
...
Процедура ЗаполнитьПричиныИзмененияТоваровВКоробах()
	Для Каждого СтрокаТоварыВКоробах Из ЭтотОбъект.гф_ТоварыВКоробах Цикл
...
Процедура ЗаполнитьТоварыНаСервере() Экспорт
	...
	Для Каждого СтрокаТоварыВКоробах Из гф_ТоварыВКоробах Цикл
		...
		// ++ Галфинд СадомцевСА 04.10.2023 Реализовал заполнение Товаров в Заказе по Коробам с учетом "добавленных/отмененных" коробов
		...
		Если СтрокаТоварыВКоробах["ВариантОбеспечения"] = Перечисления.ВариантыОбеспечения.Отгрузить
			Или СтрокаТоварыВКоробах.ПричинаОтмены = Справочники.ПричиныОтменыЗаказовКлиентов.гф_Отгружено
	...
	ЗапросЦен.УстановитьПараметр("Период", ?(ЗначениеЗаполнено(гф_ДатаОбновленияИзИ5), гф_ДатаОбновленияИзИ5, Дата));
...
Функция РассчитатьЦенуПары(ВариантКомплектации, ЦенаКороба)
	...
	Возврат ЦенаПары;
КонецФункции // -- Галфинд СадомцевСА 30.11.2022
...
// #wortmann { 
// Функция определяет наличие обособленных Остатков по док. ЗаказКлиента
// Галфинд_Домнышева 2024/04/24
//
// Возвращаемое значение:
//	Булево - Наличие или Отсутствие остатков 
Функция ПроверитьПолучитьОбособленныеОстатки()
	...
	Запрос.Текст = 
		...
		|			&ДатаУдаления,
		|			(Номенклатура, Характеристика, Назначение.Заказ) В
		|					(ВЫБРАТЬ
		|						Т.Номенклатура,
		|						Т.Характеристика,
		|						Т.Заказ
		|					ИЗ
		|						ВТ_ТоварыЗаказа КАК Т)
		|				И Склад = &Склад) КАК ТоварыНаСкладахОстатки
		|ГДЕ
		|	ТоварыНаСкладахОстатки.ВНаличииОстаток > 0";
	
	Запрос.УстановитьПараметр("ДатаУдаления", ТекущаяДатаСеанса());
