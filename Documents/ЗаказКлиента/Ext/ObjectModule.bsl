...
Процедура ЗаполнитьТоварыНаСервере()
	...
	Если Не ЗначениеЗаполнено(гф_ВидЦены) Тогда
	...
	ВременнаяТаблицаТЧТовары.Колонки.Добавить("ВариантКомплектации", Новый ОписаниеТипов("СправочникСсылка.ВариантыКомплектацииНоменклатуры"));
...
Процедура ЗаполнитьТоварыНаСервереПустойВидЦены()
	...
	ВременнаяТаблицаТЧТовары.Свернуть("Номенклатура,Характеристика,гф_ДобавленоПоПричине,гф_ПричинаДобавления,Отменено,ПричинаОтмены,
		|ВариантОбеспечения,ВариантКомплектации,ЦенаКороба,ПроцентРучнойСкидки", "КоличествоУпаковок");
	
	тчТовары = ПодготовитьВременнуюТаблицуТЧТовары();
	тчТовары.Колонки.Добавить("ВариантКомплектации", Новый ОписаниеТипов("СправочникСсылка.ВариантыКомплектацииНоменклатуры"));
	тчТовары.Колонки.Добавить("ЦенаКороба", Новый ОписаниеТипов("Число"));
	НомерСтроки = 0;
	...
	Для каждого Строка из ВременнаяТаблицаТЧТовары Цикл
		...
		НоваяСтрока.Количество = Строка.КоличествоУпаковок;
		//НоваяСтрока.Цена = РегистрыСведений.ЦеныНоменклатуры25.ПолучитьПоследнее(Дата, Отбор).Цена;
		НоваяСтрока.Цена = РассчитатьЦенуПары(НоваяСтрока.ВариантКомплектации, НоваяСтрока.ЦенаКороба);
		НоваяСтрока.Обособленно = Истина;		
		НоваяСтрока.гф_ЦенаСоСкидкой = НоваяСтрока.Цена * (1 - НоваяСтрока.ПроцентРучнойСкидки/100);
	КонецЦикла;	  
