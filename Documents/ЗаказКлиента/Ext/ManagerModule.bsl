...
Процедура ЗаполнитьТоварыНаСервере(ДокОбъект, ПереходВПроцедуруИзФормы = Ложь) Экспорт
	...
	ЗапросЦен.УстановитьПараметр("Период", ДокОбъект.Дата);
	// Волк1
	ЗапросЦен.УстановитьПараметр("Номенклатура", ВременнаяТаблицаТЧТовары.ВыгрузитьКолонку("Номенклатура"));
	ЗапросЦен.УстановитьПараметр("ВидЦены",      ВременнаяТаблицаТЧТовары.ВыгрузитьКолонку("ВидЦены"));
	...
	Если Не Отладка Тогда
		СтруктураПересчетаСуммы = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВТЧ(ДокОбъект);
