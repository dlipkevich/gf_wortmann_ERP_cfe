...
Процедура гф_РезервироватьПоНазначениюТоварыВКоробах(Команда)   
	...
	ИнфоРезервированиеНеВыполнено = Неопределено;
	гф_ВыполнитьРезервированиеПоНазначениюТоварыВКоробах(ИнфоРезервированиеНеВыполнено);
	Если ИнфоРезервированиеНеВыполнено <> Неопределено Тогда
		ПоказатьПредупреждение(, ИнфоРезервированиеНеВыполнено, 10, "Резервирование не выполнено!");
	Иначе
...
&НаСервере
Процедура гф_ВыполнитьРезервированиеПоНазначениюТоварыВКоробах(Инфо = Неопределено);
	...
	Если тУЛ.Количество() = 0 Тогда
		Инфо = "На складе отгрузки отсутствуют короба по вариантам комплектации (из заказа) с пустым заказом клиента.
	...
	Если Не ТребуетсяРезервированиеОбщее Тогда
		Инфо = "Резервирование не требуется, т.к. все заказанные товары уже в резерве.";
		Возврат;
	КонецЕсли;
	...
	Для каждого стрВарианты Из ТаблицаПотребностейПоВариантам Цикл
		...
		Для каждого стрУЛ Из мСтрокУЛ Цикл
	...
	Иначе
		Инфо = "На складе отгрузки ни для одного короба, подобранного по вариантам комплектации (из заказа) 
