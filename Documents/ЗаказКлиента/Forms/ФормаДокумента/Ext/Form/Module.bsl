...
Процедура гф_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	гф_ЭтоТоварыВКоробах = ЭтоТоварыВКоробах(Объект.Склад);
	//Элементы.Товары.ТолькоПросмотр = гф_ЭтоТоварыВКоробах;
	Элементы.гф_ЗаполнитьТовары.Доступность = гф_ЭтоТоварыВКоробах;
	// -- Галфинд СадомцевСА 18.10.2022
...
Процедура гф_АдресДоставкиОчистка(Элемент, СтандартнаяОбработка)
	...
	#Вставка
	Объект["гф_" + ИмяРеквизитаАдресаДоставки] = Неопределено;
	#КонецВставки
...
Процедура гф_СоздатьНовыеРеквизиты()
	...
	НовоеПоле.Заголовок = "Комментари РТУ";
	НовоеПоле.Вид = ВидПоляФормы.ПолеВвода;
...
Процедура РассчитатьСтрокуНаСервере(ИдентификаторСтроки)
	...
	Строка.ЦенаКоробаСоСкидкой = Строка.ЦенаКороба * (1 - Строка.Скидка/100);
	...
	КонецЕсли;
	Строка.СуммаНДС = Строка.Сумма * (Ставка/100);
...
Процедура ЗаполнитьПодборНаСервере(Результат) 
	...
	Для каждого Вариант Из Результат Цикл
		...
		НоваяСтрока.Скидка = РассчитатьСкидку(Объект.Организация, Объект.Контрагент, Объект.Договор, Объект.Дата, Объект.гф_ДатаОбновленияИзИ5);
...
Процедура гф_ТоварыВКоробахПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		Если Элемент.ТекущиеДанные <> Неопределено Тогда
			...
			Элемент.ТекущиеДанные.Скидка = РассчитатьСкидку(Объект.Организация, Объект.Контрагент, Объект.Договор, Объект.Дата, Объект.гф_ДатаОбновленияИзИ5);
...
Процедура гф_ТоварыВКоробахВариантКомплектацииПриИзменении(Элемент)
	...
	ТД.ЦенаКоробаСоСкидкой = ТД.ЦенаКороба * (1 - ТД.Скидка/100);
...
Процедура гф_ЗаполнитьДействие(ВариантОбеспечения)
	Для Каждого Идентификатор Из Элементы.гф_ТаблицаТоварыВКоробах.ВыделенныеСтроки Цикл
		...
		КонецЕсли;
	КонецЦикла;
	//Для Каждого СтрокаТЧ Из Объект.гф_ТоварыВКоробах Цикл
	//	СтрокаТЧ.ВариантОбеспечения = ВариантОбеспечения;
	//КонецЦикла;
...
&НаКлиенте
Процедура гф_ТоварыПересчитатьЦенуСоСкидкой()
...
&НаКлиенте
Процедура гф_ТоварыПересчитатьСуммуСНДС()
	ТД = Элементы.Товары.ТекущиеДанные;
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура гф_ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		// Цена со скидкой
		ТекущаяСтрока.гф_ЦенаСоСкидкой = ТекущаяСтрока.Цена * (1 - ТекущаяСтрока.ПроцентРучнойСкидки/100);
		// -- Галфинд СадомцевСА 18.10.2022
		#КонецВставки
...
Процедура гф_ТоварыПриНачалеРедактированияПосле(Элемент, НоваяСтрока, Копирование)
	Если НоваяСтрока Тогда
		Если Элемент.ТекущиеДанные <> Неопределено Тогда
			Элемент.ТекущиеДанные.ВидЦены = Объект.гф_ВидЦены;
			Элемент.ТекущиеДанные.ПроцентРучнойСкидки = РассчитатьСкидку(Объект.Организация, Объект.Контрагент, Объект.Договор, Объект.Дата, Объект.гф_ДатаОбновленияИзИ5);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
// -- Галфинд СадомцевСА 18.10.2022
