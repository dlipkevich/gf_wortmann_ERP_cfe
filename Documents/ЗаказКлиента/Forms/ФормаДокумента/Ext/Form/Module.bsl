...
Процедура гф_ТоварыВКоробахИДКоробаПредставлениеНачалоВыбора(Элемент)
	...
	ПараметрыФормы = новый Структура;
	ПараметрыФормы.Вставить("ИдентификаторСтроки", ТекущиеДанные.ИдентификаторСтроки);
	
	ОткрытьФорму("Документ.ЗаказКлиента.Форма.гф_ФормаПросмотраУЛ", ПараметрыФормы, ЭтотОбъект);
...
Процедура гф_СформироватьПредставлениеИДКороба(ИдентификаторСтроки = неопределено)
	...
	Если ИдентификаторСтроки <> неопределено Тогда
		...
		Отбор = новый Структура;
		Отбор.Вставить("ИдентификаторСтроки", ТекущиеДанныеСтроки.ИдентификаторСтроки);
		...
		Для каждого НайденаяСтрока из НайденныеСтроки Цикл
			...
			Если Индекс = 0 Тогда
				 Представление =  Строка(НайденаяСтрока.УпаковочныйЛист);
				 Индекс = Индекс + 1;
				 Продолжить;
			КонецЕсли;
			 
			Представление = Представление + " , " + Строка(НайденаяСтрока.УпаковочныйЛист);
