...
Процедура гф_РезервироватьПоНазначениюТоварыВКоробах(Команда)   
	Если Не Объект.Проведен Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("Перед выполнением операции документ необходимо провести.");
		Возврат;
	КонецЕсли;
	
	// vvv Галфинд \ Sakovich 03.03.2023
	//гф_ВыполнитьРезервированиеПоНазначениюТоварыВКоробах();
		...
		Для каждого стрУЛ Из мСтрокУЛ Цикл
			Если ПотребностьУЛПоВариантам > 0 Тогда
				...
				Для каждого стрТоварыУЛ Из мСтрокТоварыУЛ Цикл
					...
					мСтрокТаблицыОстатков = вспСвободныеОстатки.НайтиСтроки(ОтборСтрокТоварыУЛ);
					Для каждого СтрокаОстатков Из мСтрокТаблицыОстатков Цикл
						ОстатковДостаточно = 
						(СтрокаОстатков["Запас"] - 
						СтрокаОстатков["Резерв"] - 
						СтрокаОстатков["Потребность"] -
						стрТоварыУЛ["КоличествоУпаковок"]) >= 0;
