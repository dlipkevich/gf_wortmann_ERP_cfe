...
Процедура гф_ТоварыВКоробахКоличествоЦенаПриИзменении(Элемент)
	...
	РасчитатьСтрокуНаКлиенте(ТекущаяСтрока);	
...
Процедура гф_СоздатьНовыеРеквизиты()
	...
	Команда.Отображение = ОтображениеКнопки.Картинка;//.КартинкаИТекст;
...
&НаКлиенте
Процедура РасчитатьСтрокуНаКлиенте(Строка)
...
Процедура гф_КомандаПодборКомплектации(Команда)
	...
	ОткрытьФорму("Справочник.ВариантыКомплектацииНоменклатуры.Форма.гф_ФормаПодбора", ПараметрыФормы, ЭтаФорма, , , ,
...
Процедура гф_РазбитьСтрокуЗавершение(НоваяСтрока, ДополнительныеПараметры) Экспорт 
	 ...
	 Если НоваяСтрока <> Неопределено Тогда
		РасчитатьСтрокуНаКлиенте(НоваяСтрока);	 
	 КонецЕсли;	 
...
Процедура ЗаполнитьПодборНаСервере(Результат) 
	...
	Для каждого Вариант Из Результат Цикл
		...
		НоваяСтрока.ЦенаКороба = РегистрыСведений.ЦеныНоменклатуры25.ПолучитьПоследнее(ТекущаяДатаСеанса(), Отбор).Цена;
		НоваяСтрока.Сумма = НоваяСтрока.Количество * НоваяСтрока.ЦенаКороба; 
	КонецЦикла;  
	
	ЗаполитьТоварыНаСервере();
...
&НаСервере
Процедура ЗаполитьТоварыНаСервере()
	...
	Для каждого СтрокаТоварыВКоробах из Объект.гф_ТоварыВКоробах Цикл
		...
		Для каждого СтрокаСостава из СтрокаТоварыВКоробах.ВариантКомплектации.Товары Цикл
			...
			НоваяСтрока.КоличествоУпаковок = СтрокаТоварыВКоробах.Количество * СтрокаСостава.Количество;
			
			Отбор = новый Структура;
			Отбор.Вставить("Номенклатура", СтрокаСостава.Номенклатура);			
			Отбор.Вставить("ВидЦены", ТипЦеныЗакупочная);
