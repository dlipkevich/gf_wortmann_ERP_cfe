...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	// Проверка наличия документа Маркировка
	// Код закоментирован временно для дальнейшей отладки без доступа к ЧестномуЗнаку
	// Галфинд ДомнышеваКР 2023/05/23
	//ЕстьМаркировка = ПроверитьНаличиеДокументаМаркировки(Ссылка);
	//Если Не ЕстьМаркировка Тогда
	//	ПоказатьПредупреждение(, "Сначала введите в оборот КМ!", 10);
	//	Возврат;
	//КонецЕсли;
	// } #wortmann
...
// Галфинд ДомнышеваКР 2023/05/23
&НаСервере
Функция ПроверитьНаличиеДокументаМаркировки(Ссылка)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СтатусыОформленияДокументовИСМП.Основание КАК Основание
		|ИЗ
		|	РегистрСведений.СтатусыОформленияДокументовИСМП КАК СтатусыОформленияДокументовИСМП
		|ГДЕ
		|	СтатусыОформленияДокументовИСМП.Основание = &Основание
		|	И СтатусыОформленияДокументовИСМП.СтатусОформления = ЗНАЧЕНИЕ(Перечисление.СтатусыОформленияДокументовГосИС.Оформлено)
		|	И ТИПЗНАЧЕНИЯ(СтатусыОформленияДокументовИСМП.Документ) = ТИП(Документ.МаркировкаТоваровИСМП)";
...
// #wortmann {
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8ea8b083fed1320811edf49fcd05bbaf
// Требуется смена статуса и перепроведение ПриходногоОрдера
// Галфинд ДомнышеваКР 2023/05/23
&НаСервере
Процедура ПроверитьИПерезаписатьСтатусПриходного(ДокПТУ)
	...
	Запрос.Текст = 
		...
		|	И ПриходныйОрдерНаТовары.Склад В
		|			(ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Ключ", "гф_ГлобальныеЗначенияТаможенныйСклад");
	Запрос.УстановитьПараметр("Распоряжение", ДокПТУ);
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		Выборка.Следующий();
		ПрихОрдер = Выборка.Ссылка;
	Иначе
		Возврат;
	КонецЕсли;
	
	ПрихОрдерОбъект = ПрихОрдер.ПолучитьОбъект();
	ПрихОрдерОбъект.Статус = Перечисления.СтатусыПриходныхОрдеров.Принят;
	Попытка
		ПрихОрдерОбъект.Записать(РежимЗаписиДокумента.Проведение);
		ОбщегоНазначения.СообщитьПользователю("У документа " + ПрихОрдерОбъект.Ссылка + " обновлен статус на ""Принят"""); 
	Исключение
		ОбщегоНазначения.СообщитьПользователю("Не удалось изменить статус у документа " + ПрихОрдерОбъект.Ссылка + 
		" по таможенному складу ");
		ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки());
		Возврат;
	КонецПопытки;
КонецПроцедуры// } #wortmann

Функция ПолучитьИзменениеСоставаТоваровИзПересортицы(мУЛ)
	Запрос = Новый Запрос("
	|ВЫБРАТЬ
	...
	|		ПО ПересортицаТоваров.Ссылка = ПересортицаТоваровТовары.Ссылка
	|ГДЕ
	|	ПересортицаТоваров.Проведен
	|	И ПересортицаТоваров.гф_IDКороба В (&мУЛ)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТоварыПересортица.Номенклатура КАК Номенклатура, 
	|	ТоварыПересортица.Характеристика КАК Характеристика,
	|	ТоварыПересортица.Назначение КАК Назначение,
	|	СУММА(ТоварыПересортица.Количество) КАК Количество,
	|	СУММА(ТоварыПересортица.Количество) КАК КоличествоУпаковок
	|ИЗ
	|	ТоварыПересортица КАК ТоварыПересортица
	|
	|СГРУППИРОВАТЬ ПО
	|	ТоварыПересортица.Номенклатура, 
	|	ТоварыПересортица.Характеристика,
	|	ТоварыПересортица.Назначение"); 
...
Процедура ПереопределитьСоставТоваровСУчетомПересортицы(обПеремещение, ТаблицаТоваровДляИзменения);
	
	Товары = обПеремещение.Товары.Выгрузить();  
	стрПоиска = Новый Структура("Номенклатура, Характеристика, Назначение");
	мУдаляемыхСтрок = Новый Массив;
	Для каждого стрИзм Из ТаблицаТоваровДляИзменения Цикл
		ЗаполнитьЗначенияСвойств(стрПоиска, стрИзм);
		мСтр = Товары.НайтиСтроки(стрПоиска); 
		Если стрИзм.Количество > 0 Тогда
			Если мСтр.Количество() = 0 Тогда
				нс = Товары.Добавить();
				ЗаполнитьЗначенияСвойств(нс, стрИзм);  
				нс.Упаковка = Товары[0].Упаковка;  
				нс.НазначениеОтправителя = нс.Назначение;
			Иначе
				мСтр[0].Количество = мСтр[0].Количество + стрИзм.Количество;
				мСтр[0].КоличествоУпаковок = мСтр[0].КоличествоУпаковок + стрИзм.КоличествоУпаковок;
			КонецЕсли;
		КонецЕсли;   
		
		Если стрИзм.Количество < 0 Тогда
			Если мСтр.Количество() > 0 Тогда
				НужноУбрать = -стрИзм.Количество;
				Для каждого стрТовары Из мСтр Цикл
					Убрали = Мин(стрТовары.КоличествоУпаковок, НужноУбрать);
					стрТовары.Количество = стрТовары.Количество - Убрали;
					стрТовары.КоличествоУпаковок = стрТовары.КоличествоУпаковок - Убрали;
					Если стрТовары.КоличествоУпаковок = 0 Тогда
						мУдаляемыхСтрок.Добавить(стрТовары); 
					КонецЕсли;
					НужноУбрать = НужноУбрать - Убрали;
					Если НужноУбрать = 0 Тогда
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
	...
	Для каждого Эл Из мУдаляемыхСтрок Цикл
		Товары.Удалить(Эл);
	КонецЦикла; 
	обПеремещение.Товары.Загрузить(Товары); 
