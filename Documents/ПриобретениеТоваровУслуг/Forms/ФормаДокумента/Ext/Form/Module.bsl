...
// #wortmann {
// Галфинд ДомнышеваКР 2023/05/17
&НаСервере
...
Процедура гф_СоздатьНовыеРеквизиты()
	...
	НоваяКоманда = Команды.Добавить("гф_ОбработатьПересортицу");
	НоваяКоманда.Заголовок = "Обработать пересортицу";
	НоваяКоманда.Подсказка = "Найти не проведенные пересортицы и обработать.";
	НоваяКоманда.Действие = "гф_ОбработатьПересортицу";
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	
	НовыйЭлемент = Элементы.Вставить("гф_ОбработатьПересортицу",
		ТипКнопкаФормы, Элементы.ФормаКоманднаяПанель, Элементы.ФормаГруппаКомандыФормы);
	НовыйЭлемент.Заголовок = "Обработать пересортицу"; 
	НовыйЭлемент.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
	НовыйЭлемент.ИмяКоманды = "гф_ОбработатьПересортицу";
	//} #wortmann 
...
Процедура гф_УстановитьВидимостьИДоступность()
	...
	// #wortmann {
	// Галфинд ДомнышеваКР 2023/11/28
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee88366af8f0c9
	гф_ДоступностьКомандыОбработкаПересортицы();
	// } #wortmann 
	
	// #wortmann {
	// Галфинд ДомнышеваКР 2023/06/20
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8133bcee7bda45d711edef103d0e4914
	Если НЕ (ПроверкаРоли("гф_УстановкаNVE_назначен") ИЛИ ПроверкаРоли("ПолныеПрава")) Тогда 
		Элементы.гф_NVE_назначен.Доступность = Ложь;
	КонецЕсли;
	// } #wortmann
КонецПроцедуры
...
Процедура гф_ПересчитатьТовары(Команда)
	...
	Если ПоставкаЗакрыта() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Поставка закрыта. Изменение ТЧ ""Товары"" невозможно");
		Возврат;
	КонецЕсли;
	// ++ Галфинд_ДомнышеваКР_15_11_2023
...
// #wortmann { 
// Проверка закрытой поставки 
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee82ce5cd4c238
// Галфинд_Домнышева 2023/11/15
//
// Возвращаемое значение:
//	Истина - если поставка закрыта, Ложь если не закрыта
&НаСервере
Функция ПоставкаЗакрыта()
...
// #wortmann { 
// Команда "Обработка Пересортиц" должна быть Доступна Только для операции Импорт и Таможенного склада 
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee88366af8f0c9
// Галфинд_Домнышева 2023/11/28
&НаСервере
Процедура гф_ДоступностьКомандыОбработкаПересортицы()
	
	ТаможенныеСклады = _омОбщегоНазначенияКлиентСервер.ПолучитьГлобальноеЗначениеМассив("гф_ГлобальныеЗначенияТаможенныйСклад");
	
	ДоступностьОбработкаПересортицы = (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпорту)
										И ТаможенныеСклады.Найти(Объект.Склад) <> Неопределено;  
	
	Элементы.гф_ОбработатьПересортицу.Доступность = ДоступностьОбработкаПересортицы;
		
КонецПроцедуры// } #wortmann

// #wortmann { 
// Вызывает обработку Пересортицу по текущему ПТУ 
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee88366af8f0c9
// Галфинд_Домнышева 2023/11/28
&НаКлиенте
Процедура гф_ОбработатьПересортицу(Команда)
	
	Если ВозможноОбработатьВсеПересортицы() Тогда
		гф_ОбработкаИсключенийПоКоробамВызовСервера.ОбработкаПересортицы(Объект.Ссылка);
	КонецЕсли;
	
КонецПроцедуры// } #wortmann

// #wortmann { 
// Проверяет возможность вызова обработки Пересортицы, в случае не возможность выдает ошибки 
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee88366af8f0c9
// Галфинд_Домнышева 2023/11/28
//
// Возвращаемое значение:
//	Истина - если все этикетки по УЛ из Пересортиц наклеены и ГТД проведен, иначе Ложь
&НаСервере
Функция ВозможноОбработатьВсеПересортицы()
	
	ВозможноОбработать = Истина;
	...
	Запрос.Текст = 
		...
		|	УЛ_ПТУ.Ссылка КАК ПТУ,
		|	УЛ_ПТУ.IDКороба КАК УЛ,
		...
		|		ПО УЛ_ПТУ.IDКороба = ПересортицаТоваров.гф_IDКороба
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УпаковочныйЛист КАК УпаковочныйЛист
		|		ПО УЛ_ПТУ.IDКороба = УпаковочныйЛист.Ссылка
		|ГДЕ
		|	НЕ ПересортицаТоваров.ПометкаУдаления
		|	И НЕ ПересортицаТоваров.Проведен
		|	И ПересортицаТоваров.гф_IDКороба В
		|			(ВЫБРАТЬ
		|				Т.IDКороба
		|			ИЗ
		|				УЛ_ПТУ КАК Т)
		...
		|	ВЫБОР
		|		КОГДА гф_ОбработкаИсключенийСрезПоследних.Статус = ЗНАЧЕНИЕ(Перечисление.гф_СтатусыИсключений.ЭтикеткиНаклеены)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ЭтикеткиНаклеены
		...
		|		ПО ПересортицаУЛ.NVE = гф_ОбработкаИсключенийСрезПоследних.NVE
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ЕстьNULL(ТаможеннаяДекларацияИмпортТовары.Ссылка, ЗНАЧЕНИЕ(Документ.ТаможеннаяДекларацияИмпорт.ПустаяСсылка)) КАК ГТД,
		|	ПересортицаПТУ.УЛ КАК УЛ,
		|	ПересортицаПТУ.ЭтикеткиНаклеены КАК ЭтикеткиНаклеены
		|ИЗ
	...
	ВыборкаГТД = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Пока ВыборкаГТД.Следующий() Цикл
		
		Если ВыборкаГТД.ГТД = Документы.ТаможеннаяДекларацияИмпорт.ПустаяСсылка() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(" Не найден ГТД или он не проведён!");
			ВозможноОбработать = Ложь;
		КонецЕсли;
		
		Выборка = ВыборкаГТД.Выбрать();
	
		Пока Выборка.Следующий() Цикл
			Если НЕ Выборка.ЭтикеткиНаклеены Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не наклеены этикетки для УЛ " + Выборка.УЛ);
				ВозможноОбработать = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
    Возврат ВозможноОбработать;
