...
Процедура гф_СоздатьНовыеРеквизиты()
	...
	НоваяКоманда.Заголовок = "Обработать упаковочные листи";
	...
	НовыйЭлемент = Элементы.Вставить("гф_ОбработатьУпаковочныеЛисты",
		ТипКнопкаФормы, Элементы.гф_ПродукцияВКоробах.КоманднаяПанель);
	НовыйЭлемент.Заголовок = "Обработать упаковочные листи";
...
Процедура ОбработатьУпаковочныеЛистыНаСервере()
	...
	СлужебноеНазначение = _омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначение("СлужебноеНазначениеДляТоваров");
