...
Функция СформироватьПечатнуюФормуИмпорт70(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого СтруктураОбъектов Из МассивОбъектов Цикл
		...
		|	НоменклатураРеквизиты.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ВременнаяТаблица_МатериалыПодошвы
		|ИЗ
		|	НоменклатураРеквизиты КАК НоменклатураРеквизиты
		...
		|	ДополнительныеРеквизитыИСведения.Имя КАК Имя,
		|	НоменклатураРеквизиты.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ВременнаяТаблица_МатериалыВерха
		|ИЗ
		|	НоменклатураРеквизиты КАК НоменклатураРеквизиты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
		|			ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения КАК ДополнительныеРеквизитыИСведения
		...
		|	ДополнительныеРеквизитыИСведения.Имя КАК Имя,
		|	НоменклатураРеквизиты.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ВременнаяТаблица_ТипыОбуви
		|ИЗ
		|	НоменклатураРеквизиты КАК НоменклатураРеквизиты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
		|			ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения КАК ДополнительныеРеквизитыИСведения
		...
		|ВЫБРАТЬ
		|	НоменклатураРеквизиты.Ссылка КАК Ссылка,
		|	ВременнаяТаблица_МатериалыВерха.Значение КАК МатериалыВерха,
		|	ВременнаяТаблица_МатериалыПодошвы.Значение КАК МатериалыПодошвы,
		|	ВременнаяТаблица_ТипыОбуви.Значение КАК ТипыОбуви
		|ПОМЕСТИТЬ ВременнаяТаблица
		|ИЗ
		|	НоменклатураРеквизиты КАК НоменклатураРеквизиты
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблица_МатериалыПодошвы КАК ВременнаяТаблица_МатериалыПодошвы
		|		ПО НоменклатураРеквизиты.Ссылка = ВременнаяТаблица_МатериалыПодошвы.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблица_МатериалыВерха КАК ВременнаяТаблица_МатериалыВерха
		|		ПО НоменклатураРеквизиты.Ссылка = ВременнаяТаблица_МатериалыВерха.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблица_ТипыОбуви КАК ВременнаяТаблица_ТипыОбуви
		|		ПО НоменклатураРеквизиты.Ссылка = ВременнаяТаблица_ТипыОбуви.Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблица1.КоличествоВПарах КАК КоличествоВПарах,
		|	ВременнаяТаблица1.Артикул КАК Артикул,
		|	ВременнаяТаблица1.КоличествоКоробов КАК КоличествоКоробов,
		|	ВременнаяТаблица1.Владелец КАК Владелец,
		|	ВременнаяТаблица1.КоличествоВКоробах КАК КоличествоВКоробах,
		|	ВременнаяТаблица1.КодТНВЭД КАК КодТНВЭД,
		|	ВременнаяТаблица1.Марка КАК Марка,
		|	ВременнаяТаблица1.Номенклатура КАК Номенклатура,
		|	ВременнаяТаблица1.СтранаПроисхождения КАК СтранаПроисхождения,
		|	ВременнаяТаблица1.ПроизводительИмпортерКонтрагент КАК ПроизводительИмпортерКонтрагент,
		|	ВременнаяТаблица1.НаименованиеПолное КАК НаименованиеПолное,
		|	ВременнаяТаблица1.СертификатНоменклатуры КАК СертификатНоменклатуры,
		|	ВременнаяТаблица1.ДатаНачалаСрокаДействия КАК ДатаНачалаСрокаДействия,
		|	ВременнаяТаблица1.ДатаОкончанияСрокаДействия КАК ДатаОкончанияСрокаДействия,
		|	ВременнаяТаблица.Ссылка КАК Ссылка,
		|	ВременнаяТаблица.МатериалыВерха КАК МатериалыВерха,
		|	ВременнаяТаблица.МатериалыПодошвы КАК МатериалыПодошвы,
		|	ВременнаяТаблица.ТипыОбуви КАК ТипыОбуви
		|ИЗ
		|	ВременнаяТаблица1 КАК ВременнаяТаблица1
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблица КАК ВременнаяТаблица
		|		ПО ВременнаяТаблица1.Номенклатура = ВременнаяТаблица.Ссылка";
		
		//Свойство_Material_bottom  = ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Material bottom");
		//Свойство_Material_surface = ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Material surface");
		////СвойствоРазмер            = ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Size");
		//СвойствоПол               = ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Пол");
