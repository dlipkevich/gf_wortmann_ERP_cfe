...
Процедура гф_ДобавитьКомандыПечати(КомандыПечати)
	...
	// Шук А.И. 15.02.2024
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПФ_Импорт40_Новая";
	КомандаПечати.Представление = НСтр("ru = 'Импорт 40 новая';
										|en = 'Import 40 new'");
	//КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	КомандаПечати.МенеджерПечати = "Документ.ПриобретениеТоваровУслуг";
...
Процедура гф_Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	// Шук А.И. 15.02.2024
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_Импорт40_Новая") Тогда	
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
		КоллекцияПечатныхФорм,
		"ПФ_Импорт40_Новая",
		НСтр("ru = 'Импорт 40 новая'"),
		СформироватьПечатнуюФормуИмпорт40Новая(МассивОбъектов, ОбъектыПечати, ПараметрыПечати));
	КонецЕсли;
...
// Шук А.И. 15.02.2024
Функция СформироватьПечатнуюФормуИмпорт40Новая(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого СтруктураОбъектов Из МассивОбъектов Цикл
		...
		Запрос = Новый Запрос;
		Запрос.Текст = ТекстЗапросаИспользуемыеДокументомКМ();
		Запрос.УстановитьПараметр("Ссылка", СтруктураОбъектов.Ссылка);		
		ТабличныйДокумент.Вывести(ОбластьЗаголовокТаблицы);
		
		РезультатЗапроса = Запрос.ВыполнитьПакет();
		ВыборкаИтоги = РезультатЗапроса[2].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);    
		ВыборкаЭмитированныеКоды = РезультатЗапроса[3].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		СтруктураПоиска = Новый Структура("ШтрихкодУпаковки", "");
		МассивИспользованныхКМ = Новый Массив;  
		...
		Пока ВыборкаИтоги.Следующий() Цикл
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл	
				...
				Если ВыборкаДетальныеЗаписи.КодУдален Тогда 
					ВыборкаЭмитированныеКоды.Сбросить();
					СтруктураПоиска.ШтрихкодУпаковки = ВыборкаДетальныеЗаписи.КМ;
					
					Если ВыборкаЭмитированныеКоды.НайтиСледующий(СтруктураПоиска) Тогда
						ВыборкаДетали = ВыборкаЭмитированныеКоды.Выбрать();
						
						Пока ВыборкаДетали.Следующий() Цикл
							
							Если МассивИспользованныхКМ.Найти(ВыборкаДетали.ЭмитированныйКод) = Неопределено 
								 И ВыборкаДетальныеЗаписи.IDКороба = ВыборкаДетали.Короб Тогда
								МассивИспользованныхКМ.Добавить(ВыборкаДетали.ЭмитированныйКод);
								ОбластьСтрока.Параметры.КМ = ВыборкаДетали.ЭмитированныйКод;
								Прервать;
							КонецЕсли;							
						КонецЦикла;						
					КонецЕсли;									
				КонецЕсли;
				ОбластьСтрока.Параметры.КМ = УдалитьСлужебныеСимволыИзКода(ОбластьСтрока.Параметры.КМ);
...
// Шук А.И. 15.02.2024
Функция УдалитьСлужебныеСимволыИзКода(Знач КМ)   
	
	Если ТипЗнч(КМ) = Тип("Строка") И СтрДлина(КМ) <> 0 Тогда
		Часть1 = Сред(КМ, 2,2);		 
		Часть2 = Сред(КМ, 5, 14);  
		Часть3 = Сред(КМ, 20, 2);    
		Часть4 = Сред(КМ, 23);    
		ОбработанныйКМ = Часть1 + Часть2 + Часть3 + Часть4;  
		Возврат ОбработанныйКМ;
	Иначе
		Возврат КМ;
	КонецЕсли;
...
// Шук А.И. 15.02.2024
Функция ТекстЗапросаИспользуемыеДокументомКМ()
	...
	Текст = "ВЫБРАТЬ
		...
		|			ТОГДА гф_ПересортицаУдаляемыеШК.ШтрихкодУпаковки
		|		ИНАЧЕ гф_ПересортицаУдаляемыеШК.ШтрихкодУпаковки.ЗначениеШтрихкода
		|	КОНЕЦ КАК ШтрихкодУпаковки,
		|	ПулКодовМаркировкиСУЗ.КодМаркировки КАК ЭмитированныйКод
		|ПОМЕСТИТЬ УдаляемыеШК
		|ИЗ
		...
		|		ПО гф_ПересортицаУдаляемыеШК.Пересортица = ПулКодовМаркировкиСУЗ.ДокументОснование
		|ГДЕ
		...
		|		КОНЕЦ В
		|			(ВЫБРАТЬ
		|				ДанныеПоставки.КМ КАК КМ
		|			ИЗ
		|				ДанныеПоставки КАК ДанныеПоставки)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДанныеПоставки.Ссылка КАК Ссылка,
		|	ДанныеПоставки.НомерПП КАК НомерПП,
		|	ДанныеПоставки.СтранаПроисхождения КАК СтранаПроисхождения,
		|	ДанныеПоставки.КодТНВЭД КАК КодТНВЭД,
		|	ДанныеПоставки.Артикул КАК Артикул,
		|	ДанныеПоставки.КоличествоВКоробах КАК КоличествоВКоробах,
		|	ДанныеПоставки.КоличествоВПарах КАК КоличествоВПарах,
		|	ДанныеПоставки.ЕдиноеИзмерение КАК ЕдиноеИзмерение,
		|	ДанныеПоставки.КМ КАК КМ,
		|	ДанныеПоставки.IDКороба КАК IDКороба,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА УдаляемыеШК.ШтрихкодУпаковки ЕСТЬ NULL
		|				ТОГДА ЛОЖЬ
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ) КАК КодУдален
		|ИЗ
		|	ДанныеПоставки КАК ДанныеПоставки
		|		ЛЕВОЕ СОЕДИНЕНИЕ УдаляемыеШК КАК УдаляемыеШК
		|		ПО ДанныеПоставки.КМ = УдаляемыеШК.ШтрихкодУпаковки
		|
		|СГРУППИРОВАТЬ ПО
		|	ДанныеПоставки.Ссылка,
		|	ДанныеПоставки.НомерПП,
		|	ДанныеПоставки.СтранаПроисхождения,
		|	ДанныеПоставки.КодТНВЭД,
		|	ДанныеПоставки.Артикул,
		|	ДанныеПоставки.КоличествоВКоробах,
		|	ДанныеПоставки.КоличествоВПарах,
		|	ДанныеПоставки.ЕдиноеИзмерение,
		|	ДанныеПоставки.КМ,
		|	ДанныеПоставки.IDКороба
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерПП,
		|	КМ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	УдаляемыеШК.ШтрихкодУпаковки КАК ШтрихкодУпаковки,
		|	УдаляемыеШК.ЭмитированныйКод КАК ЭмитированныйКод,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(УдаляемыеШК.Пересортица) = ТИП(Документ.ПересортицаТоваров)
		|			ТОГДА ВЫРАЗИТЬ(УдаляемыеШК.Пересортица КАК Документ.ПересортицаТоваров).гф_IDКороба
		|		ИНАЧЕ УдаляемыеШК.Пересортица
		|	КОНЕЦ КАК Короб
		|ИЗ
		|	УдаляемыеШК КАК УдаляемыеШК
		|
		|УПОРЯДОЧИТЬ ПО
		|	ЭмитированныйКод     
		|
...
Функция СформироватьПечатнуюФормуИмпорт40(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого СтруктураОбъектов Из МассивОбъектов Цикл
		...
		// Запрос должен подтягивать старые КМ, иначе дальнейшая обработка ниже теряет смысл,
		// В РС гф_ПересортицаУдаляемыеШК не будет ничего найдено и новый КМ из Пула не подберется.
		//|ГДЕ
		...
		Пока ВыборкаИтоги.Следующий() Цикл
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл	
				...
				// необходимо исключить пересортицы помеченные на удаление
				|ГДЕ
				|	НЕ гф_ПересортицаУдаляемыеШК.Пересортица.ПометкаУдаления
				// -- Галфинд_ДомнышеваКР_12_01_2024
				|";
				Таб = Запрос.Выполнить().Выгрузить();
				Если Не ТипЗнч(ВыборкаДетальныеЗаписи.КМ) = Тип("Null") Тогда
					Индекс = 0;
					Кол = Таб.Количество();
					Для Каждого Стр Из Таб Цикл
						Результат = СтрСравнить(Стр.ШтрихкодУпаковки, ВыборкаДетальныеЗаписи.КМ);
						Если Результат = 0 Тогда
						//Если Стр.ШтрихкодУпаковки = ВыборкаДетальныеЗаписи.КМ Тогда
							Запрос = Новый Запрос;
							Запрос.Текст =
							"ВЫБРАТЬ
							...
							|ГДЕ
							|	ПулКодовМаркировкиСУЗ.ДокументОснование.Ссылка = &Ссылка
							// ++ Галфинд_ДомнышеваКР_12_01_2024
							// в пуле могут не содержаться данные из-за действий пользователя
							|	И ПулКодовМаркировкиСУЗ.ЗаказНаЭмиссию <> Значение(Документ.ЗаказНаЭмиссиюКодовМаркировкиСУЗ.ПустаяССылка)
							// -- Галфинд_ДомнышеваКР_12_01_2024
							|";
							Запрос.УстановитьПараметр("Ссылка",Стр.Пересортица.Ссылка);
							ТабДок = Запрос.Выполнить().Выгрузить();
							КолПер = ТабДок.Количество();
							Если Не КолПер = 0 Тогда
								ОбластьСтрока.Параметры.КМ = ТабДок[0].КодМаркировки;
							КонецЕсли;
						КонецЕсли;
						Индекс = Индекс + 1;
						Если Индекс = Кол Тогда
							Прервать;
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
...
// Шук А.И. 15.02.2024
Функция ИспользуемыеКодыМаркировки(ПриобретениеТоваров) Экспорт
	
	ТаблицаКодов = Новый ТаблицаЗначений();
	ТаблицаКодов.Колонки.Добавить("ЗначениеШтрихкода", Новый ОписаниеТипов("Строка",,,, Новый КвалификаторыСтроки(100))); 
	
	Запрос = Новый Запрос;	
	Запрос.Текст = ТекстЗапросаИспользуемыеДокументомКМ();
	Запрос.УстановитьПараметр("Ссылка", ПриобретениеТоваров);
	РезультатЗапроса = Запрос.ВыполнитьПакет(); 
	...
	Пока ВыборкаИтоги.Следующий() Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл				
			...
			СтрокаКодов.ЗначениеШтрихкода = ВыборкаДетальныеЗаписи.КМ;
				...
				Если ВыборкаЭмитированныеКоды.НайтиСледующий(СтруктураПоиска) Тогда
					...
					Пока ВыборкаДетали.Следующий() Цикл
						...
						Если МассивИспользованныхКМ.Найти(ВыборкаДетали.ЭмитированныйКод) = Неопределено 
							И ВыборкаДетальныеЗаписи.IDКороба = ВыборкаДетали.Короб Тогда
							МассивИспользованныхКМ.Добавить(ВыборкаДетали.ЭмитированныйКод);
							СтрокаКодов.ЗначениеШтрихкода = ВыборкаДетали.ЭмитированныйКод;
							Прервать;
						КонецЕсли;							
					КонецЦикла;						
				КонецЕсли;									
	...
	ТаблицаКодов.Свернуть("ЗначениеШтрихкода");
...
Функция гф_ПолучитьТекстЗапросаКодыМаркировки() Экспорт
		Возврат "
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПТУКороба.IDКороба КАК IDКороба
		|ПОМЕСТИТЬ КоробаПоставки
