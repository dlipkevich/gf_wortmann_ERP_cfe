...
Процедура гф_ПриЗаписи(Отказ)
	...
	Если ЗначениеЗаполнено(ЗаявкаНаВозвратТоваровОтКлиента)
		...
		// документ Возврат создается когда заявка в статусе гф_КМПроверены или гф_ТоварПеремаркирован (добавлено домнышева_24_11_2023)
		И (ЗаявкаНаВозвратТоваровОтКлиента.Статус = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.гф_КМПроверены
		ИЛИ ЗаявкаНаВозвратТоваровОтКлиента.Статус = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.гф_ТоварПеремаркирован) Тогда 
...
Функция ПолучитьСчетФактуруПоСвязаннымДокумента(ДокументРеализации)
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ ВТ_товары
		|ИЗ
		|	&ТоварыПоВозврату КАК ШтрихкодыУпаковок
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	Документ.КорректировкаРеализации.ШтрихкодыУпаковок КАК КорректировкаРеализации
		|ГДЕ
		|	КорректировкаРеализации.Ссылка.ДокументОснование = &ДокументОснование
		|	И КорректировкаРеализации.ШтрихкодУпаковки В
		|			(ВЫБРАТЬ
		|				ВТ_товары.ШтрихкодУпаковки
		|			ИЗ
	...
	ТоварыПоВозврату = ЭтотОбъект.ШтрихкодыУпаковок.Выгрузить();
	ТоварыПоВозврату.Свернуть("ШтрихкодУпаковки"); 
