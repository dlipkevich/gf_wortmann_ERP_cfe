...
// Галфинд Sakovich 2024/01/22
&После("ПередЗаписью")
Процедура гф_ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	гф_ПодобратьОбъектРасчетаВРасшифровкуПлатежа();
...
// #wortmann {
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeb5086b16cfb7
// при создании документа необходимо подбирать объект расчета в расшифровку платежа
// исходя из основного договора контрагента по организации из документа
// Галфинд Sakovich 2024/01/22
Процедура гф_ПодобратьОбъектРасчетаВРасшифровкуПлатежа()

	Если ЭтоНовый() Тогда
		
		лТекстЗапроса = "
		|ВЫБРАТЬ
		|	Договоры.ОсновнойДоговор КАК ОсновнойДоговор,  
		|	естьNULL(ОбъектыРасчетов.Ссылка, Значение(Справочник.ОбъектыРасчетов.ПустаяСсылка)) КАК ОбъектРасчетов
		|ИЗ
		|	Справочник.Контрагенты.гф_ОсновнойДоговорКонтрагента КАК Договоры
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыРасчетов КАК ОбъектыРасчетов
		|		ПО (Договоры.ОсновнойДоговор = ОбъектыРасчетов.Объект)
		|ГДЕ
		|	Договоры.Ссылка = &Контрагент
		...
		лВыборка.Следующий();
		Если Не ЗначениеЗаполнено(лВыборка["ОбъектРасчетов"]) Тогда
			Возврат;
		КонецЕсли;
		ПодобранныйОбъектРасчетов = лВыборка["ОбъектРасчетов"];
		
		Для каждого лРасшифровка Из РасшифровкаПлатежа Цикл
			
			лОбъектРасчетов = лРасшифровка["ОбъектРасчетов"];
			
			Если ЗначениеЗаполнено(лОбъектРасчетов) Тогда
				лОбъектОбъектаРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				лОбъектРасчетов, "Объект");
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(лОбъектРасчетов) 
				ИЛИ ТипЗнч(лОбъектОбъектаРасчетов) = Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств") Тогда
				лРасшифровка["ОбъектРасчетов"] = ПодобранныйОбъектРасчетов;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
