...
Процедура гф_СоздатьНовыеЭлементы()
	...
	НоваяКнопка = Элементы.Добавить("гф_КомандаЗаполнитьПоТоварамВКоробах", Тип("КнопкаФормы"), Элементы.ГруппаЗаполнить);
...
Процедура гф_УстановитьДоступностьТоваровВКоробах()
	ДоступностьТоварыВКоробах = УправлениеСвойствами.ЗначениеСвойства(Объект.Склад, "гф_СкладыТоварыВКоробах") = Истина; 
...
Процедура гф_КомандаПодбор(Команда)     
	...
	ПараметрыОткрытия.Вставить("ПередачаТоваров", Объект.Ссылка);
	ПараметрыОткрытия.Вставить("Склад", Объект.Склад);
	ОткрытьФорму("Документ.ПередачаТоваровХранителю.Форма.гф_ФормаПодбораУпаковочныхЛистов", 
	...
	Если ТекущиеДанные <> Неопределено Тогда   
		Если ТекущиеДанные.УпаковочныйЛист <> ЭтотОбъект["гф_ТекущийУпаковочныйЛист"] Тогда
			...
			Если СтрокиДублей.Количество() > 1 Тогда     
				...
				ТекущиеДанные.УпаковочныйЛист + " уже есть в документе");
