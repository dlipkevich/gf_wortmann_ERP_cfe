...
Процедура гф_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Элементы.ВыходныеИзделияАртикул.Видимость = Истина;
	// } #wortmann
КонецПроцедуры
...
Процедура гф_ДобавитьЭлементыФормы()
	...
	НоваяКоманда = Команды.Добавить("гф_ПересчитатьПродукцию");
	НоваяКоманда.Заголовок = "Пересчитать продукцию";
	НоваяКоманда.Подсказка = "Пересчитать данные ТЧ ""Продукция"" по данным ТЧ ""Продукция в коробах""";
	НоваяКоманда.Действие = "гф_ПересчитатьПродукцию";
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	
	НовыйЭлемент = Элементы.Вставить("гф_ПересчитатьПродукцию",
		ТипКнопкаФормы, 
		Элементы.ВыходныеИзделияГруппаЗаполнить, 
		Элементы.ВыходныеИзделияПодобратьТовары);
	НовыйЭлемент.Заголовок = "Пересчитать продукцию";
	НовыйЭлемент.ИмяКоманды = "гф_ПересчитатьПродукцию";
	
	НоваяКоманда = Команды.Добавить("гф_СвернутьСтроки");
	НоваяКоманда.Заголовок = "Свернуть строки";
	НоваяКоманда.Подсказка = "Свернуть строки по номенклатуре и характеристике";
	НоваяКоманда.Действие = "гф_СвернутьСтроки";
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	
	НовыйЭлемент = Элементы.Вставить("гф_СвернутьСтроки",
		ТипКнопкаФормы, 
		Элементы.ВыходныеИзделияГруппаЗаполнить, 
		Элементы.ВыходныеИзделияПодобратьТовары);
	НовыйЭлемент.Заголовок = "Свернуть строки";
	НовыйЭлемент.ИмяКоманды = "гф_СвернутьСтроки";

	НовыйЭлемент = Элементы.Вставить("гф_ГруппаПродукцияВКоробах", ТипГруппаФормы,
		Элементы.ГруппаСтраницы, Элементы.ГруппаВыходныеИзделия);
	...
	НоваяКоманда = Команды.Добавить("гф_ПересчитатьПродукциюЗавершениеЭкспорт");
	НоваяКоманда.Заголовок = "";
	НоваяКоманда.Подсказка = "";
	НоваяКоманда.Действие = "гф_ПересчитатьПродукциюЗавершениеЭкспорт";
	
	НоваяКоманда = Команды.Добавить("гф_МатериалыИРаботыДополнитьПоСпецификацииЭкспорт");
	НоваяКоманда.Заголовок = "";
	НоваяКоманда.Подсказка = "";
	НоваяКоманда.Действие = "гф_МатериалыИРаботыДополнитьПоСпецификацииЭкспорт";
	
	ДобавляемыеРеквизиты = Новый Массив;
	Реквизит = Новый РеквизитФормы("гф_ДополнитьВыходныеИзделия", Новый ОписаниеТипов("Неопределено"));
	ДобавляемыеРеквизиты.Добавить(Реквизит);
	Реквизит = Новый РеквизитФормы("гф_ДополнитьПоСпецификации", Новый ОписаниеТипов("Неопределено"));
	ДобавляемыеРеквизиты.Добавить(Реквизит);
	ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	
	Параметры.Свойство("гф_ДополнитьВыходныеИзделия", ЭтаФорма.гф_ДополнитьВыходныеИзделия);
	Параметры.Свойство("гф_ДополнитьПоСпецификации", ЭтаФорма.гф_ДополнитьПоСпецификации);
	// -- СадомцевСА 04.12.2023
...
&НаКлиенте
Процедура гф_ВыходныеИзделияПередОкончаниемРедактированияПосле(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	Если Не Отказ И Не ОтменаРедактирования Тогда
		ТекущиеДанные = Элемент.ТекущиеДанные;
		гф_ЗаполнитьНазначениеПолучательПоУмолчанию(ТекущиеДанные);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура гф_ЗаполнитьНазначениеПолучательПоУмолчанию(ВыходныеИзделияСтрокаКоллекции)
	
	Если Не ЗначениеЗаполнено(ВыходныеИзделияСтрокаКоллекции["Получатель"]) Тогда
		гф_ГлобальныеЗначенияСкладРЗ = 
		_омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначение("гф_ГлобальныеЗначенияСкладРЗ");
		ВыходныеИзделияСтрокаКоллекции["Получатель"] = гф_ГлобальныеЗначенияСкладРЗ;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ВыходныеИзделияСтрокаКоллекции["Назначение"]) Тогда
		ВыходныеИзделияСтрокаКоллекции["Назначение"] = ПредопределенноеЗначение("Справочник.Назначения.гф_Техническое");
	КонецЕсли;	
...
&НаКлиенте
Процедура гф_ПересчитатьПродукцию(Команда) Экспорт
	
	Если Объект.ВыходныеИзделия.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru='Табличная часть ""Продукция"" будет очищена. Продолжить?'");
		ДопПараметры = Новый Структура() ;
		Оповещение = Новый ОписаниеОповещения("гф_ВопросПересчитатьПродукциюЗавершение", ЭтотОбъект, ДопПараметры);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
	Иначе
		гф_ПересчитатьПродукциюЗавершение();
	КонецЕсли;		
	
КонецПроцедуры

&НаКлиенте
Процедура гф_СвернутьСтроки(Команда) Экспорт
	Если Объект.ВыходныеИзделия.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	гф_СвернутьСтрокиНаСервере();
...
&НаСервере
Процедура гф_СвернутьСтрокиНаСервере()
	Таблица = Объект.ВыходныеИзделия.Выгрузить();
	Таблица.Свернуть("Номенклатура, Упаковка, Характеристика", "КоличествоУпаковок, Количество");
	Объект.ВыходныеИзделия.Загрузить(Таблица);
КонецПроцедуры
...
&НаКлиенте
Процедура гф_ПересчитатьПродукциюЗавершение()
...
&НаКлиенте 
Процедура гф_ВопросПересчитатьПродукциюЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура гф_ТаблицаКоличествоУпаковокПриИзменении(ИмяТЧ)
	...
	Для каждого СтрокаВыходныеИзделия Из Объект.ВыходныеИзделия Цикл
		...
		Если Не ЗначениеЗаполнено(ТекущаяСтрока.Назначение) Тогда	
			ТекущаяСтрока.Назначение = ПредопределенноеЗначение("Справочник.Назначения.гф_Техническое");
		КонецЕсли;
	КонецЦикла;
...
Процедура гф_ЗагрузитьВыходныеИзделияИзФайлаНаСервере(АдресЗагруженныхДанных)
	...
	Для каждого СтрокаТаблицы Из ЗагруженныеДанные Цикл 
		...
		НоваяСтрокаТовары.Количество = СтрокаТаблицы.КоличествоУпаковок;
		НоваяСтрокаТовары.Характеристика = СтрокаТаблицы.Характеристика;
		НоваяСтрокаТовары.Назначение = Справочники.Назначения.гф_Техническое;
...
&НаКлиенте
&После("ПолучитьСтрокиИзБуфераОбмена")
Процедура гф_ПолучитьСтрокиИзБуфераОбмена(ИмяТЧ)
	Если ИмяТЧ = "ВыходныеИзделия" Тогда
...
// ++ СадомцевСА 04.12.2023
&НаКлиенте
Процедура гф_ПересчитатьПродукциюЗавершениеЭкспорт(Команда) Экспорт
	гф_ПересчитатьПродукциюЗавершение();
	Для Каждого стрКоллекции Из Объект.ВыходныеИзделия Цикл
		стрКоллекции["ДоляСтоимости"]		= стрКоллекции["КоличествоУпаковок"];
		стрКоллекции["Получатель"]			= ЭтаФорма.гф_ДополнитьВыходныеИзделия.Получатель;
		стрКоллекции["Назначение"]			= ЭтаФорма.гф_ДополнитьВыходныеИзделия.Назначение;
		стрКоллекции["НаправлениеВыпуска"]	= ЭтаФорма.гф_ДополнитьВыходныеИзделия.НаправлениеВыпуска;
		стрКоллекции["Спецификация"]		= ЭтаФорма.гф_ДополнитьВыходныеИзделия.Спецификация;
	КонецЦикла;
	ОбщегоНазначенияУТКлиент.Записать(ЭтаФорма);
КонецПроцедуры

&НаКлиенте
Процедура гф_МатериалыИРаботыДополнитьПоСпецификацииЭкспорт(Команда) Экспорт
	ДополнитьПоСпецификацииНаСервере("МатериалыИРаботы");;
	Для Каждого стрКоллекции Из Объект.МатериалыИРаботы Цикл
		Если стрКоллекции["ТипНоменклатуры"] = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар") Тогда
			стрКоллекции["Склад"] = ЭтаФорма.гф_ДополнитьПоСпецификации.Склад;
		КонецЕсли;
		стрКоллекции["Подразделение"] = ЭтаФорма.гф_ДополнитьПоСпецификации.Подразделение;
	КонецЦикла;
	ОбщегоНазначенияУТКлиент.Записать(ЭтаФорма);
КонецПроцедуры
// -- СадомцевСА 04.12.2023
