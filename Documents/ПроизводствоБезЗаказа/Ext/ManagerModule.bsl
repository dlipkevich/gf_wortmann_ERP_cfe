	...
	Для каждого стрТч Из ПродукцияВКоробах Цикл
		Если ЗначениеЗаполнено(стрТч["IDКороба"]) Тогда
			...
			Если Не ЗначениеЗаполнено(Поставка) Тогда
				...
				Если обУЛ <> Неопределено Тогда
					обУЛ["гф_Поставка"] = Ссылка;
					Если ЗначениеЗаполнено(обУЛ["гф_Агрегация"]) Тогда
						Если Не ЗначениеЗаполнено(обУЛ["гф_ДатаАгрегации"]) Тогда
							обУЛ["гф_ДатаАгрегации"] = ТекущаяДатаСеанса();
						КонецЕсли;
						обУЛ.ДополнительныеСвойства.Вставить("ПровестиСДатойАгрегации");
					КонецЕсли;
					РежимЗаписи = РежимЗаписиДокумента.Проведение;
					обУЛ.Записать(РежимЗаписи);
				КонецЕсли;	
