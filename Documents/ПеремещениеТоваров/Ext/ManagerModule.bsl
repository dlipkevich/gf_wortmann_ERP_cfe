...
Процедура гф_ДобавитьКомандыПечати(КомандыПечати)
	...
	//(н)Галфинд/Сефербеков 10.03.2024 ( 
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "гф_ПечатьЭтикетокWortman";
	КомандаПечати.Представление = НСтр("ru = 'Печать этикеток Wortman'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	КомандаПечати.Порядок = 10;
	КомандаПечати.Обработчик = "гф_ФормированиеПечатныхФормКлиентСервер.гф_СформироватьПечатнуюФормуПечатьЭтикетокWortman";
	//(к)Галфинд/Сефербеков 10.03.2024 ) 
