...
Процедура гф_ПодготовитьАгрегациюКМ(Отказ)
	...
	Если ДополнительныеСвойства.Свойство("РусскаяЗима") 
		И ДополнительныеСвойства["РусскаяЗима"]
		И ДополнительныеСвойства.Свойство("ЗаказПоставщику")
		И ЗначениеЗаполнено(ДополнительныеСвойства["ЗаказПоставщику"]) Тогда
		ПараметрПоставка = ДополнительныеСвойства["ЗаказПоставщику"];
	Иначе
		ПараметрПоставка = гф_Поставка;
	КонецЕсли;
	...
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeec18a6b213f4
	//Запрос.УстановитьПараметр("Поставка", гф_Поставка);
	Запрос.УстановитьПараметр("Поставка", ПараметрПоставка);
	// ^^^ Галфинд \ Sakovich 27.03.2024
