...
Функция ДанныеДляПечатиСпецификации(СсылкаНаДокумент, ПараметрыСпецификацииПоНовойЦене = Неопределено)
	...
	Для Каждого СвойствоДоговора Из МассивСвойствДоговора Цикл
		...
		Если СвойствоДоговора.Имя = "гф_ДоговорыКонтрагентовПервичныеДокументыВКоробах" Тогда 
			
			Запрос.Параметры.Вставить("СвойствоДоговоровПервичныеДокументыВКоробах", СвойствоДоговора.Ссылка);
	...
	ДанныеШапки.Вставить("ДатаДоговораСтрока", Формат(ДанныеШапки, "ДФ=dd.MM.yyyy"));
...
Функция ТекстЗапроса_ВТШапка()
	...
	|	ISNULL(ДоговорыКонтрагентовПервичныеДокументыВКоробах.Значение,ЛОЖЬ) КАК ПервичныеДокументыВКоробах
	|ПОМЕСТИТЬ ВТ_Шапка
	|ИЗ
	...
	|			ПРАВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов.ДополнительныеРеквизиты КАК ДоговорыКонтрагентовПервичныеДокументыВКоробах
	|			ПО (ДоговорыКонтрагентовПервичныеДокументыВКоробах.Ссылка = ДоговорыКонтрагентов.Ссылка)
	|				И (ДоговорыКонтрагентовПервичныеДокументыВКоробах.Свойство = &СвойствоДоговоровПервичныеДокументыВКоробах)
