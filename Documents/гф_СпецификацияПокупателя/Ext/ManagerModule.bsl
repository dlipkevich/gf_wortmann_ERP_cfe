...
Функция ДанныеДляПечатиСпецификации(СсылкаНаДокумент, ПараметрыСпецификацииПоНовойЦене = Неопределено)
	...
	Для Каждого СтрокаТовара Из ДанныеТабличнойЧасти Цикл
		...
		Если СтрокаТовара.Номенклатура = Null Тогда
			...
			"ДФ=dd.MM.yyyy");
			НомерСтроки = ДанныеТабличнойЧасти.Индекс(СтрокаТовара) + 1; // Галфинд_ДомнышеваКР_23_04_2024
			Если НомерСтроки <= (ДанныеТабличнойЧасти.Количество() - 1) Тогда
				СтрокаТовара.ИмяЗаказа = ДанныеТабличнойЧасти[НомерСтроки].ИмяЗаказа; // Галфинд_ДомнышеваКР_18_04_2024
			КонецЕсли;
			Продолжить;
...
Функция ТекстЗапросаДанныхПечатнойФормыСпецификации(ПервичныеДокументыВКоробах, НаОстатокПоНовойЦене)
	...
	// ++ Галфинд_ДомнышеваКР_18_04_2024
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eefd637fd9348e
	// изменено 23.04.2024 - не было учтено наличие разных заказов в спецификации
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.гф_СпецификацияПокупателя.ЗаказыКлиентов КАК Заказы
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.Товары КАК ЗаказКлиентаТовары
	|			ПО Заказы.ЗаказКлиента = ЗаказКлиентаТовары.Ссылка
	|		ПО ВТ_Шапка.Спецификация = Заказы.Ссылка
	|ГДЕ
	|	НЕ Заказы.ЗаказКлиента ЕСТЬ NULL
	|;
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫБОР
	|		КОГДА ПОДСТРОКА(ВТ_АРТИКУЛЫ.Артикул, 1, 1) = ""1""
	|			ТОГДА "" Tamaris""
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ПОДСТРОКА(ВТ_АРТИКУЛЫ.Артикул, 1, 1) = ""5""
	|					ТОГДА "" S. Oliver""
	|				ИНАЧЕ ВЫБОР
	|						КОГДА ПОДСТРОКА(ВТ_АРТИКУЛЫ.Артикул, 1, 1) = ""8""
	|							ТОГДА "" Tamaris Comfort, Jana""
	|						ИНАЧЕ """"
	|					КОНЕЦ
	|			КОНЕЦ
	|	КОНЕЦ КАК Текст,
	|	ВТ_АРТИКУЛЫ.Заказ КАК Заказ
	|ПОМЕСТИТЬ ВТ_ДобавкаИмени
	|ИЗ
	...
	|	ЕСТЬNULL(СУММА(ВТ_НоменклатураСуммы.ОбщееКоличество), 0) КАК ОбщееКоличество, // СадомцевСА 16.04.2024
	...
	|	ВТ_Итоги.ОбщееКоличество КАК ОбщееКоличество, // СадомцевСА 16.04.2024
	...
	|	ВТ_Итоги.ОбщееКоличество, // СадомцевСА 16.04.2024
	|	ВТ_Итоги.Сумма,
	|	ВТ_Итоги.СуммаНДС,
	|	ВТ_Итоги.СуммаСНДС
	|;
	...
	|	ЗаказКлиента.гф_ИмяЗаказа + ВТ_ДобавкаИмени.Текст  КАК ИмяЗаказа,
    ...
    // ++ Галфинд_ДомнышеваКР_18_04_2024
	// изм. 23.04 так как не было учтено наличие разл. заказов в специфик.
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДобавкаИмени КАК ВТ_ДобавкаИмени
	|		ПО (ВТ_ДобавкаИмени.Заказ = ЗаказКлиента.Ссылка)
    // -- Галфинд_ДомнышеваКР_18_04_2024
	...
	|	СУММА(ОбщееКоличество), // СадомцевСА 16.04.2024
	|	СУММА(Сумма),
	|	СУММА(СуммаНДС),
	|	СУММА(СуммаСНДС)
	|ПО
