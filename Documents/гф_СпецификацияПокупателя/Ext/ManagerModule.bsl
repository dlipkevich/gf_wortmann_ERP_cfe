...
Функция ТекстЗапросаДанныхПечатнойФормыСпецификации(ПервичныеДокументыВКоробах, НаОстатокПоНовойЦене)
	...
	|
	|	СГРУППИРОВАТЬ ПО
	|		ЗаказКлиентагф_ТоварыВКоробах.ВариантКомплектации) КАК Подзапрос
	...
	|	МАКСИМУМ(ЕСТЬNULL(РозничныеЦены.Цена, 0)) КАК РозничнаяЦена,
	|	МАКСИМУМ(ЕСТЬNULL(НовыеЦены.Цена, 0)) КАК НоваяЦена
	|ПОМЕСТИТЬ ВТ_Цены
	|ИЗ
	...
	|							Т.Номенклатура
	|						ИЗ
	|							ВТ_Номенклатура КАК Т)) КАК РозничныеЦены
	|		ПО ВТ_Номенклатура.Номенклатура = РозничныеЦены.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры25.СрезПоследних(
	|				&ДатаЦен,
	|				ВидЦены = &ПрайсЛист
	|					И Номенклатура В
	...
	|							Т.Номенклатура
	|						ИЗ
	|							ВТ_Номенклатура КАК Т)) КАК НовыеЦены
	|		ПО (ВТ_Номенклатура.Номенклатура = РозничныеЦены.Номенклатура)
	...
	|			ТОГДА ВЫРАЗИТЬ(Цены.НоваяЦена * (1 - &СкидкаКлиента / 100) КАК ЧИСЛО(15, 2))
	...
	|	КОНЕЦ КАК Цена,
	|	Цены.РозничнаяЦена КАК РекомендуемаяРозничнаяЦена,
	...
	|		ВТ_Заказы КАК ВТ_Заказы
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.Товары КАК ЗаказКлиентаТовары
	|			ПО (ЗаказКлиентаТовары.Ссылка = ВТ_Заказы.Заказ)) КАК Подзапрос
	...
	|	ВЫБОР
	|		КОГДА &НаОстатокПоНовойЦене = ИСТИНА
	|			ТОГДА ВЫРАЗИТЬ(Цены.НоваяЦена * ВТ_ВариантыКомплектации.Количество * (1 - &СкидкаКлиента / 100) КАК ЧИСЛО(15, 2))
	...
	|			ПО (ТоварыВКоробах.Ссылка = ВТ_Заказы.Заказ)) КАК Подзапрос
