...
Функция ДанныеДляПечатиСпецификации(СсылкаНаДокумент, ПараметрыСпецификацииПоНовойЦене = Неопределено)
	...
	Для Каждого СтрокаТовара Из ДанныеТабличнойЧасти Цикл
		...
		Если СтрокаТовара.Номенклатура = Null Тогда
			...
			"ДФ=dd.MM.yyyy");
			СтрокаТовара.ИмяЗаказа = ДанныеТабличнойЧасти[1].ИмяЗаказа; // Галфинд_ДомнышеваКР_18_04_2024
...
Функция ТекстЗапросаДанныхПечатнойФормыСпецификации(ПервичныеДокументыВКоробах, НаОстатокПоНовойЦене)
	...
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВЫБОР
	|		КОГДА ПОДСТРОКА(ВТ_Номенклатура.Номенклатура.Артикул, 1, 1) = ""1""
	|			ТОГДА "" Tamaris""
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ПОДСТРОКА(ВТ_Номенклатура.Номенклатура.Артикул, 1, 1) = ""5""
	|					ТОГДА "" S. Oliver""
	|				ИНАЧЕ ВЫБОР
	|						КОГДА ПОДСТРОКА(ВТ_Номенклатура.Номенклатура.Артикул, 1, 1) = ""8""
    ...
    // ++ Галфинд_ДомнышеваКР_18_04_2024
	|	ВТ_ДобавкаИмени КАК ВТ_ДобавкаИмени
    // -- Галфинд_ДомнышеваКР_18_04_2024
