...
Процедура ЗаказыКлиентовЗаполнитьСуммы(ЗаказКлиента = Неопределено)
	...
	// ++ Галфинд ВолковЕВ 22.08.2023 Сумма получалась сразу из двух табличных частей
	...
	Запрос.Текст = "ВЫБРАТЬ
	|	Заказы.Ссылка КАК Ссылка,
	|	СУММА(ЗаказКлиентагф_ТоварыВКоробах.СуммаСНДС) КАК Сумма,
	|	СУММА(ЗаказКлиентагф_ТоварыВКоробах.СуммаНДС) КАК СуммаНДС
	//|	ВЫРАЗИТЬ(СУММА(ЗаказКлиентагф_ТоварыВКоробах.Сумма) / (100 + МАКСИМУМ(СтавкиНДС.Ставка)) * МАКСИМУМ(СтавкиНДС.Ставка) КАК ЧИСЛО(15, 2)) КАК СуммаНДС
	// ++ Галфинд_ДомнышеваКР_24_04_2024
	|ПОМЕСТИТЬ СуммаВКоробах
    // -- Галфинд_ДомнышеваКР_24_04_2024
	|ИЗ
	|	Документ.ЗаказКлиента КАК Заказы
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.гф_ТоварыВКоробах КАК ЗаказКлиентагф_ТоварыВКоробах
	//|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтавкиНДС КАК СтавкиНДС
	//|			ПО ЗаказКлиентагф_ТоварыВКоробах.СтавкаНДС = СтавкиНДС.Ссылка
	|		ПО Заказы.Ссылка = ЗаказКлиентагф_ТоварыВКоробах.Ссылка
	|ГДЕ
	|	Заказы.Ссылка В(&Заказы)
	|
	|СГРУППИРОВАТЬ ПО
	|	Заказы.Ссылка
	// -- Галфинд ВолковЕВ 22.08.2023
	// ++ Галфинд_ДомнышеваКР_24_04_2024
	// По некоробным Складам ТЧ ТоварыВКоробах не заполняются, соответственно Суммы = 0
	// доработано заполнение Сумм
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СуммаВКоробах.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА СуммаВКоробах.Сумма <> 0
	|			ТОГДА СуммаВКоробах.Сумма
	|		ИНАЧЕ СуммаВПарах.Сумма
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА СуммаВКоробах.СуммаНДС <> 0
	|			ТОГДА СуммаВКоробах.СуммаНДС
	|		ИНАЧЕ СуммаВПарах.СуммаНДС
	|	КОНЕЦ КАК СуммаНДС
	|ИЗ
	|	СуммаВКоробах КАК СуммаВКоробах
	|		ЛЕВОЕ СОЕДИНЕНИЕ СуммаВПарах КАК СуммаВПарах
	|		ПО СуммаВКоробах.Ссылка = СуммаВПарах.Ссылка";
    // -- Галфинд_ДомнышеваКР_24_04_2024
