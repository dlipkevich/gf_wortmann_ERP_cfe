...
Функция СформироватьПечатнуюФормуРасходнаяНакладнаяКороб(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Стр Из Таб Цикл
		...
		ОбластьТЧ.Параметры.Товар      = СтрокаТовараВКоробах.УпаковочныйЛист.гф_Комплектация.Владелец.Наименование + "
		...
		ОбластьТЧ.Параметры.Количество = СтрокаТовараВКоробах.Коэффициент;
		ОбластьТЧ.Параметры.Пар        = СтрокаТовараВКоробах.КоличествоПар;
		ОбластьТЧ.Параметры.Цена       = СтрокаТовараВКоробах.ЦенаКороба;
		ОбластьТЧ.Параметры.Сумма      = Таб[0].СуммаДокумента; 
	...
	ОбластьТекст.Параметры.Всего = "Всего наименований " + Сч + ", на сумму " + ИтогоСум + " руб.";
	ОбластьТекст.Параметры.ВсегоТекст = ЧислоПрописью(ИтогоСум);
