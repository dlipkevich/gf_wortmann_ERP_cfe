...
Функция ПолучитьНомерДокументаСУчетомРасходногоОрдера(Знач Номер, НомерОрдера) Экспорт
	...
	Если Лев(Номер, ДлинаПрефиксаЭтогоНомера + ДлинаНомераОрдераБезПрефикса) <> ПрефиксЭтогоНомера + НомерОрдераБезПрефикса Тогда 
		...
		// временное решение, не учитывает выданные, но не записанные номера
		ПозицияПрефикса = СтрНайти(Номер, "_");
		Если ПозицияПрефикса > 0 Тогда
			Префикс = Лев(Номер, ПозицияПрефикса);	
			ВыданныйНомерЧислом = Число(СтрЗаменить(Номер, Префикс, ""));
			Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
			|	РеализацияТоваровУслуг.Номер КАК Номер
			|ИЗ
			|	Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
			|ГДЕ
			|	РеализацияТоваровУслуг.Номер ПОДОБНО &Префикс СПЕЦСИМВОЛ ""~""
			|
			|УПОРЯДОЧИТЬ ПО
			|	Номер УБЫВ");
			Запрос.УстановитьПараметр("Префикс", СтрЗаменить(Префикс, "_", "~_") + "%");
			
			Результат = Запрос.Выполнить();
			Если Не Результат.Пустой() Тогда
				Выборка = Результат.Выбрать();
				Выборка.Следующий();
				НомерДокументаВБазе = Выборка.Номер;
				НомерБезПрефиксаВБазе = СтрЗаменить(НомерДокументаВБазе, Префикс, "");
				НомерБезПрефиксаЧисломВБазе = Число(НомерБезПрефиксаВБазе);
				Если ВыданныйНомерЧислом <= НомерБезПрефиксаЧисломВБазе  Тогда
					НомерБезПрефикса = Формат(НомерБезПрефиксаЧисломВБазе + 1, "ЧГ=0");
					НомерБезПрефикса = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(НомерБезПрефикса, ОсталасьДлинаЭтогоНомера, "0", "Слева");
...
Процедура гф_ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры)
	
	// ++ Галфинд_Домнышева 2024/04/08
	Если ПравоДоступа("Добавление", Метаданные.Документы.СчетНаОплатуКлиенту) Тогда
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Обработчик = "СозданиеНаОснованииУТКлиент.гф_СоздатьСчетНаОплатуРТУсТабЧ";
		КомандаСоздатьНаОсновании.Идентификатор = "СоздатьСчетНаОплатуСТабличнойЧастью";
		КомандаСоздатьНаОсновании.Представление = НСтр("ru = 'Счет на оплату с ТЧ';
														|en = 'Commercial invoice with TS'");
