...
Процедура гф_ЗаполнитьВидыЗапасов(Отказ)
	...
	Если (Статус <> Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате
		...
		#Вставка
		// vvv Галфинд \ Sakovich 10.04.2024
		гф_ПереопределитьПараметрыЗаполненияВидовЗапасовПоУсловиям(ПараметрыЗаполнения);
		#КонецВставки
...
// vvv Галфинд \ Sakovich 10.04.2024
Процедура гф_ПереопределитьПараметрыЗаполненияВидовЗапасовПоУсловиям(ПараметрыЗаполнения)
	НеКонтролироватьОтрицательныеОстатки = ЗапасыСервер.гф_НеКонтролироватьОтрицательныеОстатки(ЭтотОбъект);
	Если НеКонтролироватьОтрицательныеОстатки Тогда
		ПараметрыЗаполнения["ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию"] = Истина;
	КонецЕсли;
КонецПроцедуры // ^^^ Галфинд \ Sakovich 10.04.2024
