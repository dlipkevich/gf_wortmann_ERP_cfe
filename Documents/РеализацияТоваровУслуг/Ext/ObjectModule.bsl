...
// vvv Галфинд \ Sakovich 23.04.2024
// реализация меанизма перенесена в &Перед("ПередЗаписью")
...
Процедура гф_ПередЗаписью1(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Склад, "гф_АгентскийСклад") Тогда
	...
	Если ЭтоНовый() И НЕ ЗначениеЗаполнено(Номер) Тогда
		Если ЗначениеЗаполнено(гф_РасходныйОрдер) Тогда
			...
			НомерОрдера = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(гф_РасходныйОрдер, "Номер");
			// vvv Галфинд \ Sakovich 27.04.2024
			ПозицияДефиса = СтрНайти(НомерОрдера, "-");
			Если ПозицияДефиса > 0 Тогда
				НомерОрдера = Сред(НомерОрдера, ПозицияДефиса + 1);
			КонецЕсли; 
			// ^^^ Галфинд \ Sakovich 27.04.2024
		Иначе
			НомерОрдера = "0000000";
		КонецЕсли;
		Префикс = НомерОрдера + "_";
...
&ИзменениеИКонтроль("ПриУстановкеНовогоНомера")
Процедура гф_ПриУстановкеНовогоНомера(СтандартнаяОбработка, Префикс)

	#Вставка
	//Добавлено  Галфинд \ Sakovich 23.04.2024
	гф_ВходящийПрефикс = Префикс;
	#КонецВставки
	Префикс = ОбщегоНазначенияУТПовтИсп.ДополнительныйПрефиксНумератораДокументыРеализацииТоваров();
	#Вставка
	//Добавлено  Галфинд \ Sakovich 23.04.2024
	Если Префикс = "" Тогда
		Префикс = гф_ВходящийПрефикс;
	Иначе
		Префикс = гф_ВходящийПрефикс + Префикс;
	КонецЕсли;
	#КонецВставки
