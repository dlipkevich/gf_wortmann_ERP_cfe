...
Процедура гф_ПоместитьТаблицуТоваровТорговыхДокументов(МассивТорговыхДокументов, МенеджерВременныхТаблиц, ИмяВременнойТаблицы, ДополнительныеПараметры)
	...
	Для Каждого Основание Из МассивТорговыхДокументов Цикл
		...
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаРеализации") 
			...
			// ++ Галфинд_Домнышева 2024/02/29
			// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eea56c59774a78
			ТаблицаДляДвижений = ПолучитьТаблицу(Основание, ДополнительныеПараметры);
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаДляДвижений, ТаблицаТоваров);
			// ++ Галфинд_Домнышева 2024/02/29
			Продолжить;
		КонецЕсли;
		// -- Галфинд_Домнышева 2023/04/10
...
// #wortmann { 
// Функция формирует ТЗ по товарам корректировки 
// Частично код заимствован из УчетНДСЛокализация.ОтразитьРеализациюКлиенту
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eea56c59774a78
// Галфинд_Домнышева 2024/02/29
Функция ПолучитьТаблицу(ДокументСсылка, ДополнительныеПараметры)
	
	Запрос = Новый Запрос;
		
	ЗаполнитьПараметрыИнициализацииКорректировки(Запрос, ДокументСсылка);
    ...
    Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА ВидыЗапасов.Ссылка.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
		|			ТОГДА ВидыЗапасов.КодТНВЭД
		...
		|					ТОГДА ВидыЗапасов.Упаковка.ЕдиницаИзмерения
		|				ИНАЧЕ ВидыЗапасов.Упаковка
		|			КОНЕЦ
		|	КОНЕЦ КАК ЕдиницаИзмерения,
		...
		|	СУММА(ВидыЗапасов.КоличествоПоРНПТ * ВЫБОР
		|			КОГДА ВидыЗапасов.КоличествоПоРНПТ > 0
		|				ТОГДА -1
		|			ИНАЧЕ 1
		...
		|		ПО ВидыЗапасов.Ссылка = СуммыДокументов.Ссылка
		|			И ВидыЗапасов.ИдентификаторСтроки = СуммыДокументов.ИдентификаторСтроки
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК СчетФактураВыданныйДокументыОснования
		|		ПО ВидыЗапасов.Ссылка.ДокументОснование = СчетФактураВыданныйДокументыОснования.ДокументОснование
		...
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|	И ВидыЗапасов.Ссылка.Организация <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
		|	И НЕ ЛОЖЬ
		|	И (НЕ ЛОЖЬ
		|			ИЛИ ЛОЖЬ)
		|	И ВидыЗапасов.Ссылка.Дата >= &ДатаОтраженияВозвратовКорректировочнымиСФ
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ВидыЗапасов.АналитикаУчетаНаборов.НоменклатураНабора,
		|	ВидыЗапасов.АналитикаУчетаНаборов.ХарактеристикаНабора,
...
// #wortmann { 
// Функция формирует параметры запроса 
// Взято из ММ. КорректировкаРеализации.ЗаполнитьПараметрыИнициализации
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eea56c59774a78
// Галфинд_Домнышева 2024/02/29
Процедура  ЗаполнитьПараметрыИнициализацииКорректировки(Запрос, ДокументСсылка)
