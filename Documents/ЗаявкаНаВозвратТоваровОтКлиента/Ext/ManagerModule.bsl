...
Процедура СопоставитьЗагружаемыеДанные(
	...
	// ++ Галфинд_Домнышева_20_02_2024
	|	ВЫБОР
	|		КОГДА СТРНАЙТИ(ВЫРАЗИТЬ(ДанныеДляСопоставления.гф_КМСтрокой КАК СТРОКА(200)), "" "") <> НЕОПРЕДЕЛЕНО
	|			ТОГДА СТРЗАМЕНИТЬ(СОКРЛП(ВЫРАЗИТЬ(ДанныеДляСопоставления.гф_КМСтрокой КАК СТРОКА(200))), "" "", """")
	|		ИНАЧЕ СОКРЛП(ВЫРАЗИТЬ(ДанныеДляСопоставления.гф_КМСтрокой КАК СТРОКА(200)))
	|	КОНЕЦ КАК КМ,
	//|	СОКРЛП(ВЫРАЗИТЬ(ДанныеДляСопоставления.гф_КМСтрокой КАК СТРОКА(200))) КАК КМ,
	|	ВЫБОР
	|		КОГДА СТРНАЙТИ(ВЫРАЗИТЬ(ДанныеДляСопоставления.гф_КМДляЗагрузки КАК СТРОКА(200)), "" "") <> НЕОПРЕДЕЛЕНО
	|			ТОГДА СТРЗАМЕНИТЬ(СОКРЛП(ВЫРАЗИТЬ(ДанныеДляСопоставления.гф_КМДляЗагрузки КАК СТРОКА(200))), "" "", """")
	|		ИНАЧЕ СОКРЛП(ВЫРАЗИТЬ(ДанныеДляСопоставления.гф_КМДляЗагрузки КАК СТРОКА(200)))
	|	КОНЕЦ КАК КМДляЗагрузки,
	...
	|ПОМЕСТИТЬ ДанныеДляСопоставления1
	// -- Галфинд_ДомнышеваКР_08_02_2024
	|ИЗ
	|	&ДанныеДляСопоставления КАК ДанныеДляСопоставления
	|;
	// ++ Галфинд_ДомнышеваКР_08_02_2024
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eec665564a4b45
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеДляСопоставления.КМ КАК КМ,
	|	ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Ссылка КАК Агрегат
	|ПОМЕСТИТЬ Агрегация
	|ИЗ
	|	ДанныеДляСопоставления1 КАК ДанныеДляСопоставления
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ШтрихкодыУпаковокТоваров.ВложенныеШтрихкоды КАК ШтрихкодыУпаковокТоваровВложенныеШтрихкоды
	|		ПО ДанныеДляСопоставления.КМ = ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Штрихкод.ЗначениеШтрихкода
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеДляСопоставления.GTIN КАК GTIN,
	|	ДанныеДляСопоставления.GTIN_1_14 КАК GTIN_1_14,
	|	ДанныеДляСопоставления.GTIN_2_13 КАК GTIN_2_13,
	|	ДанныеДляСопоставления.КМ КАК КМ,
	|	ВЫБОР
	|		КОГДА НЕ Агрегация.Агрегат ЕСТЬ NULL
	|			ТОГДА ДанныеДляСопоставления.КМ
	|		ИНАЧЕ ДанныеДляСопоставления.КМДляЗагрузки
	|	КОНЕЦ КАК КМДляЗагрузки,
	|	ДанныеДляСопоставления.Артикул КАК Артикул,
	|	ДанныеДляСопоставления.НаименованиеНоменклатуры КАК НаименованиеНоменклатуры,
	...
	|ИЗ
	|	ДанныеДляСопоставления1 КАК ДанныеДляСопоставления
	|		ЛЕВОЕ СОЕДИНЕНИЕ Агрегация КАК Агрегация
	|		ПО ДанныеДляСопоставления.КМ = Агрегация.КМ
	|;
	// -- Галфинд_ДомнышеваКР_08_02_2024
	|
	|////////////////////////////////////////////////////////////////////////////////1
	...
	ТаблицаНоменклатура = РезультатыЗапросов[6].Выгрузить(); // ТаблицаЗначений
	ТаблицаДанные = РезультатыЗапросов[7].Выгрузить(); // ТаблицаЗначений
...
Процедура гф_СоздатьПриходныйОрдерНаТовары(ОбъектИлиСсылка, ЗагружаемыеТовары = Неопределено, Статус = Неопределено) Экспорт
	...
	Если Статус = Неопределено Тогда // Галфинд_ДомнышеваКР_21_11_2023
	...
	Попытка
		...
		ЗаписатьДокументПриходныйОрдерВТабличнуюЧасть(обПрихОрдер, ОбъектИлиСсылка, Статус);// Галфинд_Домнышева 2023/04/24
	Исключение
...
Процедура ЗаписатьДокументПриходныйОрдерВТабличнуюЧасть(ПриходныйОрдер, ЗаявкаНаВозврат, Статус)
	...
	Для каждого Строка Из ПриходныйОрдер.Товары Цикл
		...
		СтруОтбора.Вставить("гф_ПринятСкладом", Ложь); // Добавлено Галфинд_ДомнышеваКР_13_12_2023 
	...
	Заявка.Статус = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОбеспечению;
	КонецЕсли;
	// -- Галфинд_ДомнышеваКР_21_11_2023
