...
Процедура гф_ДобавитьЭлементыФормы()
	...
	Команда.Заголовок = "5. Создать документ ""Заказ на эмиссию кодов маркировки""";
	...
	Команда.Заголовок = "6. Создать документ ""Возврат товаров от клиента""";
...
Процедура гф_ЗагрузитьТоварыИзФайлаНаСервере(АдресЗагруженныхДанных)
	...
	Для каждого СтрокаТаблицы Из ЗагруженныеДанные Цикл 
		...
		// #wortmann {
		// ++ Галфинд_Домнышева_19_04_2023
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8131bcee7bda45d711edd201ed470fa8
		//Проверка СтрокаТаблицы.гф_КМСтрокой		
		Если    Лев(СтрокаТаблицы.гф_КМСтрокой, 1) <> "("  Тогда
			Если Сред(СтрокаТаблицы.гф_КМСтрокой, 4, 1) <> ")"  Тогда
				КМСтрокой = "(" + Лев(СтрокаТаблицы.гф_КМСтрокой, 2) + ")" + Сред(СтрокаТаблицы.гф_КМСтрокой, 3);
			Иначе
				КМСтрокой = "(" + СтрокаТаблицы.гф_КМСтрокой;
			КонецЕсли;
		ИначеЕсли Сред(СтрокаТаблицы.гф_КМСтрокой, 4, 1) <> ")"  Тогда
			КМСтрокой = Лев(СтрокаТаблицы.гф_КМСтрокой, 3) + ")" + Сред(СтрокаТаблицы.гф_КМСтрокой, 4);
		Иначе
			КМСтрокой = СтрокаТаблицы.гф_КМСтрокой;
		КонецЕсли;
		// нс.гф_КМСтрокой = СтрокаТаблицы.гф_КМСтроко;
		нс.гф_КМСтрокой = КМСтрокой;
		// } #wortmann
...
Процедура гф_СоздатьДокументВозврат(Команда)
	...
	Если НЕ гф_ПустыеВозвратыИзТаблицыСервере() Тогда
...
Процедура гф_СоздатьДокументВозвратНаСервере(Реализация, ТаблицаВозвращаемыхТоваров)
	...
	Попытка
		...
		Для Каждого Строка Из Объект.ВозвращаемыеТовары Цикл
			...
			КонецЕсли;
		КонецЦикла;
		ЗаполнитьНомераГТДПоРеализации(ДокументВозвратТоваров, Реализация);
		Модифицированность = Истина;
...
Процедура СоздатьПеремаркировкиТовараНаСервере()
	...
	ВозвращаемыеТовары = Объект.ВозвращаемыеТовары.Выгрузить();
	вспТЗ = ВозвращаемыеТовары.Скопировать(СтруктураОтбора, "гф_ДокументВозврат, гф_ПеремаркировкаТовара"); 
	вспТЗ.Свернуть("гф_ДокументВозврат, гф_ПеремаркировкаТовара");
	Для каждого стрТЗ Из вспТЗ Цикл
		...
		Если ЗначениеЗаполнено(стрТЗ["гф_ДокументВозврат"]) Тогда 
			...
			Если мСтрокТовары.Количество() > 0 Тогда
				...
				СоздатьПеремаркировкуТовараНаСервере(стрТЗ["гф_ДокументВозврат"], ВозвращаемыеТоварыДляПеремаркировки, СоздатьПеремещения);
			Иначе
...
// Галфинд_Домнышева 2023/04/10
&НаСервере
Процедура СоздатьПеремаркировкуТовараНаСервере(Основание, ТоварыДляПеремаркировки, СоздатьПеремещения)
	...
	Попытка
		ПеремаркировкаОбъект.Записать(РежимЗаписиДокумента.Проведение);
...
Процедура СоздатьДокументыПеремещенияИОрдера()
	...
	Исключение
		...
		"со склада " + Объект.Склад + " на склад " + ВиртуальныйСклад);
	КонецПопытки; 
