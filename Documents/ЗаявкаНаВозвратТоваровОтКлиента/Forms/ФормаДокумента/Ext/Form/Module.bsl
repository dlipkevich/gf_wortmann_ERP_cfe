...
Процедура ЗагрузитьРТУ(УПП, суидДок, ЕстьИзменения)
	...
	Для НомерСтроки = 0 По ВсегоСтрок Цикл
		...
		нс.Номенклатура = НайтиНоменклатуру(стр.Номенклатура);
...
Процедура ЗагрузитьКорректировки(УПП, ссРТУ, ЕстьИзменения)
	...
	Для НомерСтроки = 0 По ВсегоСтрок -1 Цикл
		...
		обДок.Товары.Очистить();
		Для каждого стч Из уппДок.Товары Цикл
			ОбъектКоллекции = стч;
			
			нс = обДок.Товары.Добавить();		
			
			нс.Количество = стч.Количество;
		    Если УПП.ЗначениеЗаполнено(ОбъектКоллекции.Номенклатура)
				И УПП.ЗначениеЗаполнено(ОбъектКоллекции.Номенклатура.ЕдиницаХраненияОстатков)
				И ОбъектКоллекции.Номенклатура.ЕдиницаХраненияОстатков <> ОбъектКоллекции.ЕдиницаИзмерения
				И ОбъектКоллекции.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент Тогда
				
				нс.Количество = ОбъектКоллекции.Количество * ОбъектКоллекции.Коэффициент 
					/ ОбъектКоллекции.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент;
			КонецЕсли;
            нс.Номенклатура = НайтиНоменклатуру(стч.Номенклатура);
			нс.СтавкаНДС = НайтиСтавкуНДС(стч.СтавкаНДС, УПП);
			нс.Склад = НайтиСклад(стч.Склад);
			нс.Сумма = стч.Сумма;
			нс.СуммаНДС = стч.СуммаНДС;
			нс.Цена = стч.Цена;
			
			нс.ВариантОтражения = Перечисления.ВариантыОтраженияКорректировокРеализаций.СписатьНаРасходы;
			нс.КоличествоУпаковок = стч.Количество; 
			// нс.серия ????
			нс.СуммаСНДС = ОбъектКоллекции.Сумма + ?( уппДок.СуммаВключаетНДС, 0, ОбъектКоллекции.СуммаНДС);
			
			нс.Упаковка = НайтиЕИ(ОбъектКоллекции.ЕдиницаИзмерения);
...
&НаСервере
Функция НайтиНоменклатуру(Источник)
...
Функция ВыгрузитьТЧТоварыРТУ(УПП, Источник)
	...
	КоллекцияОбъектов.Свернуть("Ссылка, Номенклатура, Склад, СтавкаНДС, Цена, ВидЦены, ЗаказКлиента, ИдентификаторСтроки, СерияНоменклатуры, СрокПоставки, Упаковка, ПроцентАвтоматическихСкидок, ПроцентСкидкиНаценки, ИмяТабличнойЧасти, Коэффициент, КлючСвязи, КодСтроки, Назначение","Количество, КоличествоУпаковок, Сумма, СуммаНДС, СуммаСНДС");
	КоллекцияОбъектов.Колонки.Добавить("Характеристика");
...
Процедура вппУпаковочныйЛист(УПП, д, РТУ)
	...
	Если НЕ ЗначениеЗаполнено(ссгф_Агрегация) Тогда
		...
		Для каждого стз Из д.гф_Агрегация.ВложенныеШтрихкоды Цикл
		    ...
		    Если НЕ ЗначениеЗаполнено(ссШКПары) Тогда
			    ...
			    обШКПары.Номенклатура = НайтиНоменклатуру(шк.Номенклатура);
	...
	Если НЕ ЗначениеЗаполнено(ссгф_Комплектация) Тогда
		...
		обВК.Владелец = НайтиНоменклатуру(д.гф_Комплектация.Владелец);
		...
		Для каждого стз Из д.гф_Комплектация.Товары Цикл
			...
			нс.Номенклатура = НайтиНоменклатуру(стз.Номенклатура);
