...
Процедура СоздатьКорректировкуРеализации_возвратWMN(докРеализации, ТаблицаВозвращаемыхТоваров)
	...
	Попытка
		...
		ПроверкаСозданиеСчетФактурыВыданной(обКорректировка, докРеализации); // Галфинд_Домнышева_09_06_2023
...
&НаСервере
Процедура ПроверкаСозданиеСчетФактурыВыданной(обКорректировка, докРеализации)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		КонецПопытки;
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("В документах отгрузки клиенту нет оформленной сч-фактуры, 
		|проведение сч-фактуры на возврат невозможно. Для документа " + обКорректировка.Ссылка + " сч-фактура не создана.");
	КонецЕсли;
