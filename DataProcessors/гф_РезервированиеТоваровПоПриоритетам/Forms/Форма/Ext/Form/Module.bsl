...
Процедура ЗаполнитьПриоритетыНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				И &УсловиеПоСкладу
				   |				И НЕ ПриоритетНеДействует) КАК Приоритеты
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Приоритеты.Приоритет,
	               |	Приоритеты.Контрагент.Наименование";   //Добавлено  Галфинд \ Sakovich 09.12.2023 16:35:27
	...
	// vvv Галфинд \ Sakovich 09.12.2023
    //Запрос.УстановитьПараметр("Дата", Объект.Дата);
	Запрос.УстановитьПараметр("Дата", ТекущаяДатаСеанса());
    
	// ^^^ Галфинд \ Sakovich 09.12.2023
...
Процедура СохранитьНаСервере()
	...
	Для каждого стрТз Из тзКЗаписи Цикл
		СтруктураИзмерений = Новый Структура("Контрагент, Организация, Склад, Заказ");
		ЗаполнитьЗначенияСвойств(СтруктураИзмерений, стрТз);
				
		Набор = РегистрыСведений.гф_ПриоритетыРезервов.СоздатьНаборЗаписей();
		Набор.Отбор["Контрагент"].Установить(СтруктураИзмерений["Контрагент"]);
		Набор.Отбор["Организация"].Установить(СтруктураИзмерений["Организация"]);
		Набор.Отбор["Склад"].Установить(СтруктураИзмерений["Склад"]);
		Набор.Отбор["Заказ"].Установить(СтруктураИзмерений["Заказ"]);
		Набор.Прочитать();
		
		Запись = Набор.Добавить();
		ЗаполнитьЗначенияСвойств(Запись, СтруктураИзмерений); 
		Запись.Период = ДатаЗаписи;
		Запись.Приоритет = стрТз["НовыйПриоритет"];
		
		Для каждого ЗаписьНабора Из Набор Цикл
			ЗаписьНабора["ПриоритетНеДействует"] = Ложь;
		КонецЦикла;
		Набор.Записать();
		
		мСтрокТекущиеПриоритеты = тзТекущиеПриоритеты.НайтиСтроки(СтруктураИзмерений);
		Если мСтрокТекущиеПриоритеты.Количество() Тогда
			мСтрокТекущиеПриоритеты[0]["ТекущийПриоритет"]= стрТз["НовыйПриоритет"];
		КонецЕсли;
	КонецЦикла;
	Объект.ТекущиеПриоритеты.Загрузить(тзТекущиеПриоритеты);
...
Функция ТекстЗапросРаспределениеНоменклатурыПоЗаказамПоПриоритетам()
	...
	|		ПО УпаковочныйЛистТовары.Ссылка = УЛ_безЗаказа.УпаковочныйЛист
	|ГДЕ
	|	УпаковочныйЛистТовары.Назначение В(&Назначения)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	МАКСИМУМ(Заказ_ТоварыВКоробах.Количество) КАК КоличествоКоробов,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СвободныеЗапасы КАК СвободныеЗапасы
	|		ПО (СвободныеЗапасы.Номенклатура = УЛ_безЗаказа_безНазначения.Номенклатура)
	|ГДЕ
	|	ИСТИНА
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	Заказ_ТоварыВКоробах.Ссылка,
	|	Заказ_ТоварыВКоробах.ВариантКомплектации,
	|	УЛ_безЗаказа_безНазначения.УпаковочныйЛист,
	|	Заказ_ТоварыВКоробах.ВариантКомплектации.Владелец,
	|	Заказ_ТоварыВКоробах.ВариантКомплектации.Владелец.Артикул,
	|	Заказ_ТоварыВКоробах.Ссылка.Контрагент,
	|	Заказ_ТоварыВКоробах.Ссылка.Склад,
	|	Заказ_ТоварыВКоробах.Ссылка.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрСведений.гф_ПриоритетыРезервов.СрезПоследних(&Дата,  НЕ ПриоритетНеДействует) КАК гф_ПриоритетыРезервовСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|		ГДЕ
	|			НЕ УпаковочныйЛист.ПометкаУдаления) КАК ВложенныйЗапрос
	|		ПО (ИСТИНА)
	...
	|
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(вт_ПриоритетыПоЗаказам.КоличествоКоробов) - КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВложенныйЗапрос.Ссылка) > 0";
...
Процедура ЗаполнитьНераспределеннымиОстаткамиНаСервере() // 5.2.08 (пункт 1.1.2.6)
	...
	Если ИсходноеНазначение.Количество() = 0 Тогда
		...
		Запрос.УстановитьПараметр("Склад", Объект.Склад);
		Запрос.УстановитьПараметр("Назначения", Справочники.Назначения.ПустаяСсылка());
	...
	Для каждого сП Из ТаблицаПриоритетов Цикл
		...
		Если мСтрокТаблицыКоличестваКоробов.Количество() > 0 Тогда
			...
			Если строкаКоличестваКоробов["ЕщеЕстьСвободныхКоробов"] > 0 Тогда
				строкаКоличестваКоробов["ЕщеЕстьСвободныхКоробов"] = строкаКоличестваКоробов["ЕщеЕстьСвободныхКоробов"] - 1;
				строкаКоличестваКоробов["РастраченоКоробов"] = строкаКоличестваКоробов["РастраченоКоробов"] + 1;
			Иначе
				Продолжить;
			КонецЕсли;
		Иначе
	...
	Для каждого ЭлНазначение Из мНазначенийСвернуто Цикл
		...
		Для каждого элКороб Из мКоробовСвернуто Цикл
			...
			Для каждого стрТчУпЛист Из тчТоварыУпЛист Цикл
				...
				// vvv Галфинд \ Sakovich 26.12.2023  
				// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eea3458657e74a  
				// в упаковочных листах иногда бывает незаполнено одно из полей "Количество", "КоличествоУпаковок"
				ОсобенностьУчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(нс["Номенклатура"], "ОсобенностьУчета");
				ЭтоОбувнаяПродукция =  ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ОбувнаяПродукция;
				Если ЭтоОбувнаяПродукция Тогда   
					Если нс["Количество"] = 0 Тогда
						нс["Количество"] = нс["КоличествоУпаковок"];
					КонецЕсли;
					Если нс["КоличествоУпаковок"] = 0 Тогда
						нс["КоличествоУпаковок"] = нс["Количество"];
					КонецЕсли;
				КонецЕсли;
				// ^^^ Галфинд \ Sakovich 26.12.2023
...
&НаСервереБезКонтекста
Процедура ТекущиеПриоритетыПередУдалениемНаСервере(СтруктураИзмерений)
	Набор = РегистрыСведений.гф_ПриоритетыРезервов.СоздатьНаборЗаписей();
	Отбор = Набор.Отбор;
	Отбор["Контрагент"].Установить(СтруктураИзмерений["Контрагент"]);
	Отбор["Организация"].Установить(СтруктураИзмерений["Организация"]);
	Отбор["Склад"].Установить(СтруктураИзмерений["Склад"]);
	Отбор["Заказ"].Установить(СтруктураИзмерений["Заказ"]);
	
	Набор.Прочитать();
	Для каждого Запись Из Набор Цикл
		Запись.ПриоритетНеДействует = Истина;
	КонецЦикла;
...
&НаКлиенте
Процедура ТекущиеПриоритетыПередУдалением(Элемент, Отказ)
	ТекДанные = Элементы.ТекущиеПриоритеты.ТекущиеДанные;
	СтруктураИзмерений = Новый Структура("Контрагент, Организация, Склад, Заказ");
	ЗаполнитьЗначенияСвойств(СтруктураИзмерений, ТекДанные);
	ТекущиеПриоритетыПередУдалениемНаСервере(СтруктураИзмерений);
КонецПроцедуры
