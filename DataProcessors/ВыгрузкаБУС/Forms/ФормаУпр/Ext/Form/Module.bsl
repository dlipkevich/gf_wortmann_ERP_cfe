...
Функция ПолучитьЗапросПоЗаказам()
	...
	|	Заказ.Контрагент.Партнер.ГруппаДоступа КАК ГруппаДоступаКонтрагента,
...
Процедура ОтправитьЗаказыНаСервере()   
	...
	НастройкиПодключения = B2B_ОбщиеПроцедурыИФункции.ВосстановитьНастройкиПодключенияПортал(Объект.Портал);
...
Процедура КонтрагентыОтправитьНаСервере()     
	...
	Если не ЗначениеЗаполнено(Объект.БЦ_Организация) Тогда	
		Сообщить("Выберите организацию для определения конт.лиц");
		Возврат;
	КонецЕсли;
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Для каждого Ссылка Из МассивСсылок Цикл
				...
				B2B_ОбщиеПроцедурыИФункции.ЗаписатьЗначениеСвойства(Ссылка.Партнер, СтруктураСвойства); 
			...
			Попытка
				Для каждого Строка Из ОбъектРезультат["PARTNER"] Цикл 
					...
					КонтактЛицо = Справочники.КонтактныеЛицаПартнеров.ПолучитьСсылку(Новый УникальныйИдентификатор(СокрЛП(Гуид)));
					...
					B2B_ОбщиеПроцедурыИФункции.ЗаписатьЗначениеСвойства(КонтактЛицо, СтруктураСвойства);
...
Функция ПолучитьМассивКонтрагентов(МассивСсылок)
	...
	|	МАКСИМУМ(ЕСТЬNULL(КонтрагентыКонтактныеЛица.ОсновноеКонтактноеЛицо, ЗНАЧЕНИЕ(Справочник.КонтактныеЛицаПартнеров.ПустаяСсылка))) КАК КонтактноеЛицо,
	|	Контрагенты.B2B_ОсновнойБанковскийСчет.НомерСчета КАК ОсновнойБанковскийСчетНомерСчета,
	|	Контрагенты.Партнер.БизнесРегион.Наименование КАК РегионНаименование
	|ИЗ
	|	Справочник.Контрагенты КАК Контрагенты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.B2B_КонтактныеЛицаПоОрганизациям КАК КонтрагентыКонтактныеЛица
	|		ПО Контрагенты.Ссылка = КонтрагентыКонтактныеЛица.Ссылка
	|			И (КонтрагентыКонтактныеЛица.Организация = &Организация)
	|			И (КонтрагентыКонтактныеЛица.ОсновноеКонтактноеЛицо <> ЗНАЧЕНИЕ(Справочник.КонтактныеЛицаПартнеров.ПустаяСсылка))
	|ГДЕ
	...
	Запрос.УстановитьПараметр("Организация", Объект.БЦ_Организация); 
	...
	Пока Рез.Следующий() Цикл
		...
		СтруктураКонтрагент.Вставить("XML_ID", СокрЛП(Рез.гф_GLN_номер));
		СтруктураКонтрагент.Вставить("CODE", Прав(СокрЛП(Рез.Код), 5));
		...
		Если ЗначениеЗаполнено(Рез.РегионНаименование) Тогда
			СтруктураКонтрагент.Вставить("AREA_NAME", СокрЛП(Рез.РегионНаименование));
		КонецЕсли;
...
//		|КодПоОКПО, КодОКВЭД, ИНН, КПП, Телефоны, ЭлектроннаяПочта, ЮридическийАдрес, ФактическийАдрес, Банк, БИК, КоррСчет, НомерСчета";
		...
		СкидкиКонтрагента = БЦ_ПолучитьСкидкиКонтрагента(Рез.Ссылка);
		
		Контакты = ПолучитьКонтактныеЛица(Рез.Ссылка, Истина, СкидкиКонтрагента);
		Если Контакты.Количество()>0 Тогда
			СтруктураКонтрагент.Вставить("REPRESENTATIV", Контакты[0]);
		КонецЕсли;
		
		Контакты = ПолучитьКонтактныеЛица(Рез.Ссылка, Ложь, СкидкиКонтрагента);
...
Функция ПолучитьКонтактныеЛица(Контрагент, Основной = Истина, СкидкиКонтрагента = неопределено)
	...
	|			И (ЗначенияСвойствОбъектов.Свойство = &СвойствоИДКонтактногоЛица)
	|ГДЕ
	|	КЛКонтрагентов.Владелец = &Владелец
	|	И КЛКонтрагентов.Ссылка В (&КонтактныеЛица)
	|	И НЕ КЛКонтрагентов.ПометкаУдаления
	|	И ВЫБОР
	|			КОГДА &Основной
	|				ТОГДА КЛКонтрагентов.Ссылка = &ОсновноеКонтактноеЛицо 
	|			ИНАЧЕ КЛКонтрагентов.Ссылка <> &ОсновноеКонтактноеЛицо 
	|		КОНЕЦ";
	
	Запрос.УстановитьПараметр("Владелец", Контрагент.Партнер);
	
	КонтактныеЛица = Новый Массив;
	ОсновноеКонтактноеЛицо = Справочники.КонтактныеЛицаПартнеров.ПустаяСсылка();
	Для каждого стр из Контрагент.B2B_КонтактныеЛицаПоОрганизациям Цикл
		
		Если стр.Организация = Объект.БЦ_Организация И ЗначениеЗаполнено(стр.ОсновноеКонтактноеЛицо) Тогда
			
			КонтактныеЛица.Добавить(стр.ОсновноеКонтактноеЛицо);
			
			Если Не ЗначениеЗаполнено(ОсновноеКонтактноеЛицо) Тогда
				ОсновноеКонтактноеЛицо = стр.ОсновноеКонтактноеЛицо;
			КонецЕсли;
	...
	Запрос.УстановитьПараметр("СвойствоИДКонтактногоЛица", ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("B2B_ИД_КонтактногоЛица"));
	...
	Пока Рез.Следующий() Цикл                     		
		...
		ФамилияИмяОтчество = РегламентированнаяОтчетность.РазложитьФИО(Рез.ФИО);
		
		СтруктураПользователя.Вставить("LAST_NAME",   ФамилияИмяОтчество.Фамилия);
		СтруктураПользователя.Вставить("NAME",        ФамилияИмяОтчество.Имя);
		СтруктураПользователя.Вставить("SECOND_NAME", ФамилияИмяОтчество.Отчество);
...
Функция БЦ_ПолучитьСкидкиКонтрагента(Контрагент)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ИсторияИзмененияСкидокСрезПоследних.Скидка КАК Скидка
	|ИЗ
	|	РегистрСведений.гф_ИсторияИзмененияСкидок.СрезПоследних(
	|			&ТекДата,
	|			Организация = &Организация
	|				И Контрагент = &Контрагент
	|				И ВидСкидки = &ВидСкидки) КАК ИсторияИзмененияСкидокСрезПоследних
	|ГДЕ
	|	&ОтборПоСезону";
	
	Если ЗначениеЗаполнено(Объект.БЦ_Сезон) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПоСезону", "ИсторияИзмененияСкидокСрезПоследних.Сезон = &Сезон");
		Запрос.УстановитьПараметр("Сезон", Объект.БЦ_Сезон); 
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПоСезону", "ИСТИНА");
	КонецЕсли; 
	...
	Запрос.УстановитьПараметр("ВидСкидки", Объект.БЦ_ТипСкидкиДляОбуви);
	Рез = Запрос.Выполнить().Выбрать();
	Если Рез.Следующий() Тогда
		Скидки.Вставить("PERSONAL_DISCOUNT_SHOES", Рез.Скидка);
	Иначе
		Скидки.Вставить("PERSONAL_DISCOUNT_SHOES", 0);
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ВидСкидки", Объект.БЦ_ТипСкидкиДляАксессуаров);
	Рез = Запрос.Выполнить().Выбрать();
	Если Рез.Следующий() Тогда
		Скидки.Вставить("PERSONAL_DISCOUNT_ACCESSORIES", Рез.Скидка);
	Иначе
		Скидки.Вставить("PERSONAL_DISCOUNT_ACCESSORIES", 0);
	КонецЕсли;
...
Функция ПолучитьМассивАдресов(Контрагент)
	...
	Пока Рез.Следующий() Цикл
		...
		Попытка
			...
			СтруктураАдрес.REGION = Рез.Ссылка_.РегионНаименование;
			
			//чтобы перенести город и улицу надо както преобразовать строку Рез.Ссылка_.Адрес в структуру, но АдресныйКлассификаторСлужебный только из xml или json разбирает,
			//убрал пока
...
Процедура НастройкиПодключенияСохранияитьНаСервере()
	...
	B2B_ОбщиеПроцедурыИФункции.СохранитьНастройкиПодключенияПортал(НастройкиПодключения, Объект.Портал);
	...
	Для Каждого СтрокаУр1 Из Уровень1 Цикл
		...
		Для Каждого СтрокаУр2 Из ПоискУр2 Цикл         
			...
			Для Каждого СтрокаУр3 Из ПоискУр3 Цикл
				...
				НоменклатураОбъект = СтрокаУр3.Ссылка.ПолучитьОбъект();
				НоменклатураОбъект.B2B_ГруппаНаСайтеОсновная = СпрОУр2.Ссылка;
				НоменклатураОбъект.Записать();   
				Сообщить(Строка(НоменклатураОбъект)+" включена в "+Строка(СпрОУр2.Ссылка));
...
Процедура ФормаированиеКаталогаTamarisNewНаСервере(СписокНоменклатуры = Неопределено)
	...
	Для Каждого Строка Из Рез Цикл    
		...
		Для Каждого СтрокаПоиска Из ПоискТоваров Цикл
			...
			НоменклатураОбъект = СтрокаПоиска.Ссылка.ПолучитьОбъект();
			НоменклатураОбъект.B2B_ГруппаНаСайтеОсновная = СпрО.Ссылка;
			НоменклатураОбъект.Записать();   
			Сообщить(Строка(НоменклатураОбъект)+" включена в "+Строка(СпрО.Ссылка));
...
Процедура СоздатьРеглЗаданиеАвторезеревированияНаСервере() 
	...
	ОбъектЗадания = ЗаданиеАвторезервирования(Объект.Портал);
	
	Если ОбъектЗадания = Неопределено Тогда
	
		ОбъектЗадания = РегламентныеЗадания.СоздатьРегламентноеЗадание("B2B_СнятиеРезерва");
		ОбъектЗадания.Наименование = "B2B Снятие резерва " + Объект.Портал;
		ОбъектЗадания.Использование = Истина;
		ОбъектЗадания.Ключ = "B2B_СнятиеРезерва" + Объект.Портал.УникальныйИдентификатор();
		ОбъектЗадания.Параметры.Добавить(Объект.Портал);
			
		ОбъектЗадания.Записать();

	КонецЕсли;
	
	РасписаниеАвторезервирования = ОбъектЗадания.Расписание;
...
Процедура СоздатьРеглЗаданиеАвторезеревирования(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Портал) Тогда
		Сообщить("Не выбран Портал!");
		Возврат;
	КонецЕсли;
	
	СоздатьРеглЗаданиеАвторезеревированияНаСервере();
	
	РасписаниеРеглЗаданияАвторезервирования(Неопределено);
...
Процедура РасписаниеРеглЗаданияАвторезервирования(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("ПослеИзмененияРасписанияАвторезервирования", ЭтотОбъект);
	ДиалогРасписания = Новый ДиалогРасписанияРегламентногоЗадания(РасписаниеАвторезервирования);
	ДиалогРасписания.Показать(Обработчик);

КонецПроцедуры

&НаКлиенте
Процедура ПослеИзмененияРасписанияАвторезервирования(РасписаниеРезультат, ДополнительныеПараметры) Экспорт
	
	Если РасписаниеРезультат <> Неопределено Тогда
		
		Модифицированность = Истина;
		РасписаниеАвторезервирования = РасписаниеРезультат;
		
		ПослеИзмененияРасписанияАвторезервированияНаСервере();
		
		УстановитьТекстРасписанияАвторезервирования();
...
&НаСервере
Процедура ПослеИзмененияРасписанияАвторезервированияНаСервере()
	
	ОбъектЗадания = ЗаданиеАвторезервирования(Объект.Портал);
	Если ОбъектЗадания <> Неопределено Тогда
		ОбъектЗадания.Расписание = РасписаниеАвторезервирования;
		ОбъектЗадания.Записать();
	КонецЕсли;
...
Функция ЗаданиеАвторезервирования(Портал)
	
	УстановитьПривилегированныйРежим(Истина);
	
	КлючЗадания = "B2B_СнятиеРезерва" + Портал.УникальныйИдентификатор();
...
&НаСервере
Процедура УстановитьТекстРасписанияАвторезервирования()
	
	Перем ТекстРасписания, РасписаниеАктивно;
	
	Задание = ЗаданиеАвторезервирования(Объект.Портал);
	
	Если Задание <> Неопределено Тогда
		
		УстановитьПривилегированныйРежим(Истина);
		
		РасписаниеАвторезервирования = Задание.Расписание;
		
		ИнтеграцияИС.ПолучитьТекстЗаголовкаИРасписанияРегламентнойНастройки(Задание, ТекстРасписания, РасписаниеАктивно);
		
		Элементы.РасписаниеРеглЗаданияАвторезервирования.Заголовок = ТекстРасписания;
		
	Иначе
		
		РасписаниеАвторезервирования = Неопределено;
		
		Элементы.РасписаниеРеглЗаданияАвторезервирования.Заголовок = "Задание не найдено";
		
	КонецЕсли;
	
	Элементы.СоздатьРеглЗаданиеАвторезеревирования.Видимость = РасписаниеАвторезервирования = Неопределено;
	Элементы.РасписаниеРеглЗаданияАвторезервирования.Доступность = РасписаниеАвторезервирования <> Неопределено;
...
Процедура НоменклатураСохранитьНаСервере()   
	...
	Для каждого Строка Из Объект.ТаблицаНоменклатуры Цикл
		...
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "B2B_Портал") <> Объект.Портал Тогда   
			//ЗаписатьЗначениеСвойства(Номенклатура,Новый Структура("Свойство,Значение",Справочники.B2B_w_Настройки.Свойство_B2B_Портал.Значение,Объект.Портал));
...
Функция ЗаписатьЗначениеСвойства(Объект, СтруктураСвойства) Экспорт
	...
	Возврат B2B_ОбщиеПроцедурыИФункции.ЗаписатьЗначениеСвойства(Объект, СтруктураСвойства); 
...
Процедура НастройкиСкидокСохранитьНаСервере()
	...
	Свойство_МаксимальнаяСкидка = Справочники.B2B_w_Настройки.B2B_РазмерМаксимальнойСкидки.Значение;  
	
	Если Справочники.B2B_w_Настройки.B2B_РазмерМаксимальнойСкидки.Значение = Неопределено или
		Справочники.B2B_w_Настройки.B2B_РазмерМаксимальнойСкидки.Значение.Пустая() 
		Тогда
		Сообщить("Необходимо задать в справочнике настроек (Обмен с сайтом - Дополнительные функции - Настройка свойств) свойство B2B_РазмерМаксимальнойСкидки");
		Сообщить("Сохранение прервано");
	    Возврат;
	КонецЕсли;
	
	запись = РегистрыСведений.ДополнительныеСведения.СоздатьМенеджерЗаписи();
	запись.Объект = Объект.БЦ_Организация;
	запись.Свойство = Свойство_МаксимальнаяСкидка;
...
&НаСервере
Процедура НоменклатураЗагрузитьСправочникНоменклатураНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура НоменклатураЗагрузитьСправочникНоменклатура(Команда)
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда		
		...
		СоответствиеКолонок = Новый Структура;
		ТЗ_Данные = ПрочитатьФайлНаСервере2(Файл, СоответствиеКолонок);
...
Функция ПрочитатьФайл(Адрес)
	...
	ТаблицаРезультат = ПолучитьСтруктуруФайла();
	
	Для ТекущаяСтрока = НачальнаяСтрока По КонечнаяСтрока Цикл
		НоваяСтрока = ТаблицаРезультат.Добавить();
		
		Для ТекущаяКолонка = 1 ПО 4 Цикл
			Область = ТабличныйДокумент.ПолучитьОбласть("R" + СтрЗаменить(ТекущаяСтрока, Символы.НПП, "") + "C" + Формат(ТекущаяКолонка, "ЧЦ=2; ЧДЦ=0; ЧГ="));
			ТекущаяОбласть = Область.ТекущаяОбласть;
			ЗначениеЯчейки = СокрЛП(ТекущаяОбласть.Текст);
			НоваяСтрока["Колонка_" + ТекущаяКолонка] = ЗначениеЯчейки;
			НоваяСтрока.НомерСтроки = ТекущаяСтрока;
		КонецЦикла;
	КонецЦикла;
...
Функция ПолучитьСтруктуруФайла()
	...
	ТаблицаРезультат.Колонки.Добавить("Колонка_1");				//Артикул
	ТаблицаРезультат.Колонки.Добавить("Колонка_2");				//Наименование
	ТаблицаРезультат.Колонки.Добавить("Колонка_3");				//Группа Битрикс
	ТаблицаРезультат.Колонки.Добавить("Колонка_4");				//Описание

	ТаблицаРезультат.Колонки.Добавить("НомерСтроки", новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(10, 0)));			//НомерСтроки
...
Функция ПрочитатьФайлНаСервере2(мФайл, СоответствиеКолонок, Предзаказ = Ложь)
	
	ТаблицаРезультат = ПрочитатьФайл(мФайл.АдресВоВремХранилище);
	ЗагрузитьТипыНоменклатуры(ТаблицаРезультат);
		
КонецФункции  


Процедура ЗагрузитьТипыНоменклатуры(ТаблицаРезультат)
	сч = 0;  
	текстС = "";
	Для Каждого ТекущаяСтрока Из ТаблицаРезультат Цикл 
		Артикул = ТекущаяСтрока.Колонка_1;
		Наименование = ТекущаяСтрока.Колонка_2;
		Описание = ТекущаяСтрока.Колонка_4; 
		ГруппаБитрикса = ТекущаяСтрока.Колонка_3; 
		
		Если НайтиПоАртикулу(Артикул) = Неопределено Тогда
			НовыйТип = Справочники.Номенклатура.СоздатьЭлемент();
		Иначе 
			НовыйТип = НайтиПоАртикулу(Артикул).ПолучитьОбъект();
		КонецЕсли;
		
		НовыйТип.Родитель = Родитель;
		НовыйТип.ВидНоменклатуры = ВидНоменклатуры;  
		НовыйТип.СтавкаНДС = Справочники.СтавкиНДС.НайтиПоНаименованию("20%");  
		НовыйТип.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар;
		Справочники.Номенклатура.ЗаполнитьРеквизитыПоВидуНоменклатуры(НовыйТип,
																	Ложь,
																	Ложь,
		...
		НовыйТип.Артикул = Артикул;
		НовыйТип.Описание = Описание;
		
		НовыйТип.B2B_ГруппаНаСайтеОсновная = Справочники.B2B_Б_ПользовательскиеГруппыТоваров.НайтиПоНаименованию(ГруппаБитрикса);
		
		НовыйТип.Записать();            
		сч = сч + 1;
		ТекстС = ТекстС + Строка(сч)+". Записана номенклатура "+Строка(Наименование);
	КонецЦикла;                                                                      
	Если сч > 0 Тогда
...
//	//               |	НЕ СЭ_ТипНоменклатуры.ПометкаУдаления
//	//               |	И СЭ_ТипНоменклатуры.ЭтоГруппа
//	//               |	И СЭ_ТипНоменклатуры.Наименование = &Наименование";
//	//Запрос.УстановитьПараметр("Наименование", ГруппаБитрикса);
//	//Выборка = Запрос.Выполнить().Выбрать();
//	//
//	//Если Выборка.Следующий() Тогда 
//	//	Возврат Выборка.Ссылка;
//	//Иначе 
//	//	Возврат Справочники.СЭ_ТипНоменклатуры.ПустаяСсылка();
//	//КонецЕсли;
//	
//КонецФункции
...
Функция НайтиПоАртикулу(Артикул)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	СЭ_ТипНоменклатуры.Ссылка КАК Ссылка
	               |ИЗ
	               |	Справочник.Номенклатура КАК СЭ_ТипНоменклатуры
	               |ГДЕ
	               |	СЭ_ТипНоменклатуры.Артикул = &Артикул
	               |	И НЕ СЭ_ТипНоменклатуры.ПометкаУдаления";
	Запрос.УстановитьПараметр("Артикул", Артикул);
	Выборка = Запрос.Выполнить().Выбрать();
...
&НаСервере
Функция ЗагрузитьСвойстваНаСервере(мФайл, СоответствиеКолонок, Предзаказ = Ложь)
	
	ТаблицаРезультат = ПрочитатьФайлСвойства(мФайл.АдресВоВремХранилище);
	ЗаполнитьСвойствоНоменклатуры(ТаблицаРезультат);

КонецФункции

&НаКлиенте
Процедура ЗагрузитьСвойства(Команда)
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда		
		...
		СоответствиеКолонок = Новый Структура;
		ТЗ_Данные = ЗагрузитьСвойстваНаСервере(Файл, СоответствиеКолонок);
...
Функция ПрочитатьФайлСвойства(Адрес)
	...
	ТаблицаРезультат = ПолучитьСтруктуруФайлаСвойства();
	
	Для ТекущаяСтрока = НачальнаяСтрока По КонечнаяСтрока Цикл
		НоваяСтрока = ТаблицаРезультат.Добавить();
		
		Для ТекущаяКолонка = 1 ПО 2 Цикл
			Область = ТабличныйДокумент.ПолучитьОбласть("R" + СтрЗаменить(ТекущаяСтрока, Символы.НПП, "") + "C" + Формат(ТекущаяКолонка, "ЧЦ=2; ЧДЦ=0; ЧГ="));
			ТекущаяОбласть = Область.ТекущаяОбласть;
			ЗначениеЯчейки = СокрЛП(ТекущаяОбласть.Текст);
			НоваяСтрока["Колонка_" + ТекущаяКолонка] = ЗначениеЯчейки;
...
Функция ПолучитьСтруктуруФайлаСвойства()
	
	ТаблицаРезультат = Новый ТаблицаЗначений;
	ТаблицаРезультат.Колонки.Добавить("Колонка_1");				//Штрихкод
	ТаблицаРезультат.Колонки.Добавить("Колонка_2");				//Значение
...
Процедура ЗаполнитьСвойствоНоменклатуры(ТаблицаРезультат)

	текстСообщения = "";   
	сч = 0;
	Для Каждого ТекущаяСтрока Из ТаблицаРезультат Цикл 
		Номенклатура = ПолучитьВладельцаПоШтрихкоду(ТекущаяСтрока.Колонка_1);
		//Номенклатура = Справочники.Номенклатура.НайтиПоКоду(ТекущаяСтрока.Колонка_1);
		Если Не ЗначениеЗаполнено(Номенклатура) Тогда 
			Сообщить("Товар не найден по коду: " + ТекущаяСтрока.Колонка_1);
			Продолжить;
		КонецЕсли;
		...
		Если Свойство.ТипЗначения = Тип("СправочникСсылка.ЗначенияСвойствОбъектов") Тогда
			МенеджерЗаписи.Значение = ПолучитьЗначение(ТекущаяСтрока.Колонка_2);
		Иначе
			МенеджерЗаписи.Значение = ТекущаяСтрока.Колонка_2;
		КонецЕсли;
		МенеджерЗаписи.Записать(Истина); 
		сч = сч + 1;
		текстСообщения = текстСообщения + Строка(сч)+". Записано допсведение в "+Строка(Номенклатура)+"
		|";
	КонецЦикла;
	Если сч > 0 Тогда
		сообщ = новый СообщениеПользователю;
...
Функция ПолучитьЗначение(ЗначениеСтрока)
	
	ЗначениеСсылка = Справочники.ЗначенияСвойствОбъектов.ПустаяСсылка();
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	И ЗначенияСвойствОбъектов.Наименование = &ЗначениеСтрока";
...
Функция ПолучитьВладельцаПоШтрихкоду(Штрихкод) Экспорт
...
&НаКлиенте
Процедура ЗагрузитьГруппыНаСайте(Команда)
	
	Если Не ЗначениеЗаполнено(РодительПользовательскихГрупп) Тогда
		Сообщить("Не выбран Родитель пользовательских групп!");
		Возврат;
	КонецЕсли;
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда		
		...
		ЗаполнитьГруппыНаСайте(Файл, РодительПользовательскихГрупп);
...
&НаСервереБезКонтекста
Процедура ЗаполнитьГруппыНаСайте(мФайл, РодительПользовательскихГрупп);
	...
	КоличествоСтрок = ДанныеФайлаТабличныйДокумент.ВысотаТаблицы;
	КоличествоКолонок = 3; //ДанныеФайлаТабличныйДокумент.ШиринаТаблицы;
	...
	Для СчетчикКоличестваСтрок = 2 по КоличествоСтрок Цикл   
		Новстрока = ТЗ_Данные.Добавить();
		Для СчетчикНомерПоля = 1 По КоличествоКолонок Цикл 
			знчЯчейки = ДанныеФайлаТабличныйДокумент.Область(СчетчикКоличестваСтрок, СчетчикНомерПоля, СчетчикКоличестваСтрок, СчетчикНомерПоля).Текст;
			ИмяКолонки = "f"+Сокрлп(СчетчикНомерПоля);
	...
	ЗапросГруппНаСайте = Новый Запрос;
	ЗапросГруппНаСайте.Текст =
	"ВЫБРАТЬ
	|	ПользовательскиеГруппыТоваров.Ссылка КАК Ссылка,
	|	ПользовательскиеГруппыТоваров.Наименование КАК Наименование
	|ИЗ
	|	Справочник.B2B_Б_ПользовательскиеГруппыТоваров КАК ПользовательскиеГруппыТоваров
	|ГДЕ
	|	ПользовательскиеГруппыТоваров.Ссылка В ИЕРАРХИИ(&РодительПользовательскихГрупп)
	|	И НЕ ПользовательскиеГруппыТоваров.ПометкаУдаления";
	ЗапросГруппНаСайте.УстановитьПараметр("РодительПользовательскихГрупп", РодительПользовательскихГрупп);
	ТаблицаГруппНаСайте = ЗапросГруппНаСайте.Выполнить().Выгрузить();
	...
	Для Каждого СтрокаДанных Из ТЗ_Данные Цикл
		...
		Артикул = СтрокаДанных["f1"]; 
		Если Не ЗначениеЗаполнено(Артикул) Тогда
			Сообщить("Не загружена строка: " + НомерСтрокиФайла + ": пустая колонка #1 (артикул)");
			Продолжить;
		КонецЕсли; 
		
		ИмяГрупыНаСайте = СтрокаДанных["f2"]; 
		Если Не ЗначениеЗаполнено(ИмяГрупыНаСайте) Тогда
			Сообщить("Не загружена строка: " + НомерСтрокиФайла + ": пустая колонка #2 (Группа на сайте (основная))");
			Продолжить;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(НайденнаяНоменклатура) Тогда
			Сообщить("Не загружена строка: " + НомерСтрокиФайла + ": для артикула: " + Артикул + " не найдена номенклатура");
			Продолжить;
		КонецЕсли;
		
		СтрокаГруппНаСайте = ТаблицаГруппНаСайте.Найти(ИмяГрупыНаСайте, "Наименование");
		Если СтрокаГруппНаСайте = Неопределено Тогда
			
			Сообщить("Не загружена строка: " + НомерСтрокиФайла + ": по наименованию: " + ИмяГрупыНаСайте +
				" не найден элемент в справочнике (Битрикс) Пользовательские группы товаров");
			Продолжить; 
			
		КонецЕсли;
		
		НоменклатураОбъект = НайденнаяНоменклатура.ПолучитьОбъект(); 
		НоменклатураОбъект.B2B_ГруппаНаСайтеОсновная = СтрокаГруппНаСайте["Ссылка"];
		
		Если ЗначениеЗаполнено(СтрокаДанных["f3"]) Тогда
			НоменклатураОбъект.B2B_НазваниеWeb = СтрокаДанных["f3"];
		Иначе
			Сообщить("Внимание, в строке " + НомерСтрокиФайла + ": пустая колонка #3 (B2 b название web (для сайта))");
		КонецЕсли;
	...
	Сообщить("Загружено " + ЗагруженоСтрок + " из " + Строка(НомерСтрокиФайла - 1));
