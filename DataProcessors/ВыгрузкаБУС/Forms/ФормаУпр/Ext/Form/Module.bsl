...
Процедура ЗаполнитьГруппыНаСайте(мФайл, РодительПользовательскихГрупп);
	...
	КоличествоСтрок = ДанныеФайлаТабличныйДокумент.ВысотаТаблицы;
	КоличествоКолонок = 2; //ДанныеФайлаТабличныйДокумент.ШиринаТаблицы;
	...
	НеНайденныеИменаГруппНаСайте = Новый Массив;
	Для Каждого СтрокаДанных Из ТЗ_Данные Цикл
		...
		Если Не ЗначениеЗаполнено(НайденнаяНоменклатура) Тогда
			Сообщить("Для артикула: " + Артикул + " не найдена номенклатура!");
			Продолжить;
		КонецЕсли;
		...
		Если СтрокаГруппНаСайте = Неопределено Тогда
			...
			Если НеНайденныеИменаГруппНаСайте.Найти(ИмяГрупыНаСайте) = Неопределено Тогда
				НеНайденныеИменаГруппНаСайте.Добавить(ИмяГрупыНаСайте);
				Сообщить("По наименованию: " + ИмяГрупыНаСайте + " не найден элемент в справочнике (Битрикс) Пользовательские группы товаров");
			КонецЕсли;
		...
		//Сообщить("У номенклатуры " + Артикул + " " + НайденнаяНоменклатура + " установена Группа на сайте (основная) в значение " + СтрокаГруппНаСайте["Ссылка"]);
