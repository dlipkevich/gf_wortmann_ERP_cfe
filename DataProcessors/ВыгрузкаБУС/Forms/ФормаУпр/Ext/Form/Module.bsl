...
Функция БЦ_ПолучитьСкидкиКонтрагента(Контрагент)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	w_ТипыСкидок.Ссылка КАК ТипСкидки,
	|	ЕСТЬNULL(Скидки.Скидка, 0) КАК Скидка
	|ИЗ
	|	Справочник.B2B_w_ТипыСкидок КАК w_ТипыСкидок
...
Процедура СоздатьСвойства() 
	...
	СоответствиеСвойств.Вставить(ЭтаОбработка.ИдентификаторСвойства_B2B_КонтрагентВыгружен(), ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию(ЭтаОбработка.ИдентификаторСвойства_B2B_КонтрагентВыгружен()));
