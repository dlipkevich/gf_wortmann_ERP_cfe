...
Функция ЗагрузитьФайлыИзПочтовогоЯщика(ПочтовыйЯщик, ЗагруженныеФайлы)
	...
	Если Заголовки.Количество() = 0 Тогда
		...
		Возврат 0;
	Иначе 
		ЗаписьЖурналаРегистрации("ОбработкаЗакгрузкаФайловИзПочты.ДанныеВЯщике", УровеньЖурналаРегистрации.Информация, Метаданные.Обработки.гф_ЗагрузкаФайловИзПочты);
		СтрокаСообщения = "В почтовом ящике """ + ПочтовыйЯщик.Профиль + """ найдено входящих сообщений: " + Формат(Заголовки.Количество(), "ЧДЦ=0");
		ЗаписьЖурналаРегистрации("ОбработкаЗакгрузкаФайловИзПочты.ДанныеВЯщике", УровеньЖурналаРегистрации.Информация, Метаданные.Обработки.гф_ЗагрузкаФайловИзПочты,,СтрокаСообщения);
	...
	//Соединение.ОчиститьУдаленныеСообщения(); // подходит только для IMAP
...
Функция ПрочитатьФайлОбщий(ИмяФайла = "", ДанныеФайла, ВложенияПисьма, ИнформацияОПисьме, ТекКаталог)
	...
	Если СтрНачинаетсяС(ИмяИРасширениеФайла.ИмяБезРасширения, "InvoiceAusgehend") 
		И НЕ СтрНачинаетсяС(ИмяИРасширениеФайла.ИмяБезРасширения, "InvoiceAusgehend_V") Тогда
		Возврат 0;
	КонецЕсли;
	// -- Галфинд_ДомнышеваКР_09_02_2024
