	...
	Пока ВыборкаОрганизация.Следующий() Цикл
		...
		Пока ВыборкаПоставщик.Следующий() Цикл
			...
			Пока ВыборкаВариант.Следующий()  Цикл
				...
				Протокол.ДобавитьСтроку("		По варианту комплектации: " + ВыборкаВариант["ВариантКомплектации"]);
				ВыборкаДетали = ВыборкаВариант.Выбрать();
				Протокол.ДобавитьСтроку("			Потребность: " + ВыборкаВариант["Количество"]);
				Пока ВыборкаДетали.Следующий() Цикл
					Протокол.ДобавитьСтроку("				" + ВыборкаДетали["ЗаказКлиента"] + " количество: " + ВыборкаДетали["Количество"]);
				КонецЦикла;
				струкПоиска = Новый Структура("Организация, Контрагент, ВариантКомплектации",
				ВыборкаВариант["Организация"], ВыборкаВариант["Поставщик"], ВыборкаВариант["ВариантКомплектации"]);
				мСтрокЗаказано = ТаблицаОстатковЗаказано.НайтиСтроки(струкПоиска);
				Если мСтрокЗаказано.Количество() > 0 Тогда
					Для каждого стрЗаказано Из мСтрокЗаказано Цикл
						ЗаказаноПоВарианту = ЗаказаноПоВарианту + стрЗаказано["КоличествоКоробов"];
					КонецЦикла;
					Протокол.ДобавитьСтроку("			Заказано: " + ЗаказаноПоВарианту);
					Для каждого стрЗаказано Из мСтрокЗаказано Цикл
						Протокол.ДобавитьСтроку("				" + стрЗаказано["Ссылка"] + ", статус: " + 
						стрЗаказано["Статус"] + ", количество коробов: " + стрЗаказано["КоличествоКоробов"]);
					КонецЦикла;
				КонецЕсли;
				Если ВыборкаВариант["Количество"] > ЗаказаноПоВарианту Тогда
					НеобходимоЗаказать = ВыборкаВариант["Количество"] - ЗаказаноПоВарианту;
					Протокол.ДобавитьСтроку("			НЕОБХОДИМО ЗАКАЗАТЬ: " + НеобходимоЗаказать);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
