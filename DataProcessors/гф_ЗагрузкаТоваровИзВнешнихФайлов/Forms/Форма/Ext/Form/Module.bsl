...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьПараметрыВыбораНоменклатурыПриСозданииНаСервере();
...
&НаСервере
Процедура УстановитьПараметрыВыбораНоменклатурыПриСозданииНаСервере()
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ПроверитьЗаполнениеПриЗагрузкеУпаковочныхЛистов(Отказ, ПроверяемыеРеквизиты);
	...
	Если СоздатьНомераГТД Или ПроверятьЗаполнениеХарактеристик Тогда
		Для ТекИндекс = 0 По Объект.Товары.Количество() - 1 Цикл
			...
			Если ТекущаяСтрока.СоздатьНомерГТД И ТекущаяСтрока.ВедетсяУчетПоГТД 
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					...
					,
					"Объект." + ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", 
																	ТекущаяСтрока.НомерСтроки, "СтранаПроисхождения"),
					,
					Отказ);
			...
			Если ТекущаяСтрока.ХарактеристикиИспользуются И Не ЗначениеЗаполнено(ТекущаяСтрока.Характеристика) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					...
					"Объект." + ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", 
																		ТекущаяСтрока.НомерСтроки, "Характеристика"), ,
					Отказ);
...
&НаСервере
Процедура ПроверитьЗаполнениеПриЗагрузкеУпаковочныхЛистов(Отказ, ПроверяемыеРеквизиты)
		...
		Для ТекИндекс = 0 По Объект.Товары.Количество() - 1 Цикл
			...
			Если ЗначениеЗаполнено(ТекущаяСтрока.УпаковочныйЛист) Тогда
				...
				Иначе
					...
					Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Товары", 
																	ТекущаяСтрока.НомерСтроки, "УпаковочныйЛист");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , Поле, , Отказ);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	СтруктураПараметровСопоставленияНоменклатуры = Новый Структура;
	СтруктураПараметровСопоставленияНоменклатуры.Вставить("Ссылка",                             Объект);
	СтруктураПараметровСопоставленияНоменклатуры.Вставить("Партнер",                            Объект.Партнер);
	СтруктураПараметровСопоставленияНоменклатуры.Вставить("НеВыполнятьПроверкуДляПользователя", Ложь);
	...
	Если СопоставлятьПоНоменклатуреПоставщиков Тогда
		...
		СтруктураДействий.Вставить(
			...
			СтруктураПараметровСопоставленияНоменклатуры);
	КонецЕсли;
...
Процедура ТоварыПроцентРучнойСкидкиПриИзменении(Элемент)
	...
	СтруктураДействий.Вставить("ПересчитатьСуммуСУчетомРучнойСкидки", 
								Новый Структура("ПересчитыватьСуммуРучнойСкидки", Ложь));
	СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
	СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
...
Процедура ТоварыНоменклатураПартнераНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОповещениеОЗакрытие = Новый ОписаниеОповещения("ОбработатьРезультатВыбораНоменклатурыПартнера", 
							ЭтотОбъект, ДополнительныеПараметры);

	НоменклатураПартнеровКлиент.ОткрытьФормуВыбораНоменклатурыПартнера(ЭтотОбъект, Партнер, ТекущаяСтрока, 
							Элемент.Заголовок, ОповещениеОЗакрытие);
...
Процедура ДобавитьПодобраннуюНоменклатуру(КодСтроки, НайденнаяНоменклатура, НайденнаяХарактеристика = Неопределено, 
											НайденнаяНоменклатураПартнера = Неопределено)
...
&НаСервере
Функция ПоискПоСвязаннойТаблице(МассивНайденныхРанее, СвязаннаяТаблица, Связи, ОтборПоСвязаннойТаблице, 
									ПоляНайденныхРанееДляРезультата, ПоляСвязаннойТаблицыДляРезультата)
	...
	ОбработатьМассивНайденныхРанее(МассивНайденныхРанее, СвязаннаяТаблица, Связи, ОтборПоСвязаннойТаблице, 
									ПоляНайденныхРанееДляРезультата, ПоляСвязаннойТаблицыДляРезультата, Результат);
		
	Возврат Результат;
КонецФункции	

&НаСервере
Процедура ОбработатьМассивНайденныхРанее(МассивНайденныхРанее, СвязаннаяТаблица, Связи, ОтборПоСвязаннойТаблице, 
									ПоляНайденныхРанееДляРезультата, ПоляСвязаннойТаблицыДляРезультата, Результат)
	...
	Если МассивНайденныхРанее.Количество() > 0 Тогда
		Для каждого СтрокаПоКлючу Из МассивНайденныхРанее Цикл
			...
			ОбработатьСтрокиСвязаннойТаблицы(НайденныеПоОтбору, ПоляНайденныхРанееДляРезультата,
										ПоляСвязаннойТаблицыДляРезультата, СтрокаПоКлючу, Результат);
...
&НаСервере
Процедура ОбработатьСтрокиСвязаннойТаблицы(НайденныеПоОтбору, ПоляНайденныхРанееДляРезультата,
										ПоляСвязаннойТаблицыДляРезультата, СтрокаПоКлючу, Результат)    
...
Процедура СопоставитьДанныеТабличногоДокументаСДаннымиИБ()
	...
	ДополнитьНомераКолонокПоНастройкам(НомераКолонок, НомерКолонки); 
		
	ВысотаШапки = 1;
	НомерСтроки = ВысотаШапки + 1 ;
	
	СтроковыйНомер = Формат(НомерСтроки, "ЧН=0; ЧГ=0");
	ЗаполненыНоменклатураКодАртикулШтрихкод = ЗначениеЗаполнено(ТабличныйДокумент.Область("R" + СтроковыйНомер + 
																	НомераКолонок.Получить("КолонкаАртикул")).Текст)
		Или ЗначениеЗаполнено(ТабличныйДокумент.Область("R" + СтроковыйНомер + 
															НомераКолонок.Получить("КолонкаНоменклатура")).Текст)
		Или ЗначениеЗаполнено(ТабличныйДокумент.Область("R" + СтроковыйНомер + 
															НомераКолонок.Получить("КолонкаШтрихкод")).Текст)
	...
	Пока ЗаполненыНоменклатураКодАртикулШтрихкод Цикл
		...
		ЗаполнитьДанныеНоменклатуры(НоваяСтрока, НомераКолонок, СтроковыйНомер, ВысотаШапки, НомерСтроки);
		
		ЗагрузитьЗапасы(НоваяСтрока, НомераКолонок, СтроковыйНомер, ВысотаШапки, НомерСтроки);
		
		ЗагрузитьЦеныСуммы(НоваяСтрока, НомераКолонок, СтроковыйНомер, ВысотаШапки, НомерСтроки);
		
		ЗагрузитьГТД(НоваяСтрока, НомераКолонок, СтроковыйНомер, ВысотаШапки, НомерСтроки);
		...
		Связи = Новый Структура("ЗначениеДляОтбораХарактеристик", "НаименованиеХарактеристикиПоиск");
		...
		НайденнаяНоменклатураПоКлючу = ТаблицаНоменклатурыПоставщиков.НайтиСтроки(
											Новый Структура("АртикулПоставщикаПоиск", СтрокаПоискаАртикул));
				
		Для каждого НайденнаяСтрока Из НайденнаяНоменклатураПоКлючу Цикл			
			ДобавитьПодобраннуюНоменклатуру(НомерСтроки, НайденнаяСтрока.Номенклатура, 
											НайденнаяСтрока.Характеристика, НайденнаяСтрока.НоменклатураПартнера);
		КонецЦикла;	
				
		МассивПодобраннойНоменклатуры = ПодобраннаяНоменклатура.НайтиСтроки(Новый Структура("КодСтроки", НомерСтроки));
		
		ОбработатьПодобраннуюНоменклатуру(МассивПодобраннойНоменклатуры, НоваяСтрока, НомерСтроки,  ТипЦеныЗакупочная, ВысотаШапки);
		...
		ДополнитьСтруктуруДействий(СтруктураДействий, НоваяСтрока);		
		...
		Попытка
			ЗаполненыНоменклатураКодАртикулШтрихкод = ЗначениеЗаполнено(ТабличныйДокумент.Область("R" + СтроковыйНомер + 
				...
				Или ЗначениеЗаполнено(ТабличныйДокумент.Область("R" + СтроковыйНомер + 
																		НомераКолонок.Получить("КолонкаШтрихкод")).Текст)
				Или ЗначениеЗаполнено(ТабличныйДокумент.Область("R" + СтроковыйНомер + 
																		НомераКолонок.Получить("КолонкаКод")).Текст);
		Исключение
	...
	ДополнитьТабличнуюЧастьДокумента();
...
&НаСервере
Процедура ДополнитьНомераКолонокПоНастройкам(НомераКолонок, НомерКолонки)
...
&НаСервере
Процедура ЗаполнитьДанныеНоменклатуры(НоваяСтрока, НомераКолонок, СтроковыйНомер, ВысотаШапки, НомерСтроки)
	
	Попытка
		НоваяСтрока.Штрихкод = ТабличныйДокумент.Область("R" + СтроковыйНомер + 
		НомераКолонок.Получить("КолонкаШтрихкод")).Текст;
		НоваяСтрока.Код = ТабличныйДокумент.Область("R" + СтроковыйНомер + НомераКолонок.Получить("КолонкаКод")).Текст;
		НоваяСтрока.Артикул = ТабличныйДокумент.Область("R" + СтроковыйНомер + 
		НомераКолонок.Получить("КолонкаАртикул")).Текст;
		НоваяСтрока.НоменклатураНаименование = ТабличныйДокумент.Область("R" + СтроковыйНомер + 
		НомераКолонок.Получить("КолонкаНоменклатура")).Текст;
		Если ИспользоватьХарактеристикиНоменклатуры Тогда
			НоваяСтрока.ХарактеристикаНаименование = ТабличныйДокумент.Область("R" + СтроковыйНомер + 
			НомераКолонок.Получить("КолонкаХарактеристика")).Текст;
		КонецЕсли;
		Если ЗагружатьКоличество Тогда
...
&НаСервере
Процедура ЗагрузитьЗапасы(НоваяСтрока, НомераКолонок, СтроковыйНомер, ВысотаШапки, НомерСтроки)
	
	Если ЗагружатьЗапасы Тогда
		
		МинимальныйОстаток = 0;
		Попытка
			МинимальныйОстаток = ТабличныйДокумент.Область("R" + СтроковыйНомер + 
			НомераКолонок.Получить("КолонкаМинимальныйОстаток")).Текст;
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		
		ПрисвоитьЧисловоеЗначениеПриЗаполненииТЧ(НоваяСтрока.МинимальныйОстаток, 
		МинимальныйОстаток, НомерСтроки, "МинимальныйОстаток", ВысотаШапки);
		
		МаксимальныйОстаток = 0;
		Попытка
			МаксимальныйОстаток = ТабличныйДокумент.Область("R" + СтроковыйНомер + 
			НомераКолонок.Получить("КолонкаМаксимальныйОстаток")).Текст;
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		
		ПрисвоитьЧисловоеЗначениеПриЗаполненииТЧ(НоваяСтрока.МаксимальныйОстаток, 
		МаксимальныйОстаток, НомерСтроки, "МаксимальныйОстаток", ВысотаШапки);
...
&НаСервере
Процедура ЗагрузитьЦеныСуммы(НоваяСтрока, НомераКолонок, СтроковыйНомер, ВысотаШапки, НомерСтроки)
	...
	Если ЗагружатьСуммы Тогда
		...
		Попытка
			Если ИспользоватьРучныеСкидкиВЗакупках И ЗагружатьСкидки Тогда
				ПроцентРучнойСкидки = ТабличныйДокумент.Область("R" + СтроковыйНомер + 
				НомераКолонок.Получить("КолонкаПроцентРучнойСкидки")).Текст;
				СуммаРучнойСкидки = ТабличныйДокумент.Область("R" + СтроковыйНомер + 
				НомераКолонок.Получить("КолонкаСуммаРучнойСкидки")).Текст;
		...
		ПрисвоитьЧисловоеЗначениеПриЗаполненииТЧ(НоваяСтрока.ПроцентРучнойСкидки, ПроцентРучнойСкидки, 
		НомерСтроки, "ПроцентРучнойСкидки", ВысотаШапки);
		ПрисвоитьЧисловоеЗначениеПриЗаполненииТЧ(НоваяСтрока.СуммаРучнойСкидки, СуммаРучнойСкидки, 
		НомерСтроки, "СуммаРучнойСкидки", ВысотаШапки);
...
&НаСервере
Процедура ЗагрузитьГТД(НоваяСтрока, НомераКолонок, СтроковыйНомер, ВысотаШапки, НомерСтроки)
	
	Если ЗагружатьГТД Тогда
		Попытка
			НомерГТД            = ТабличныйДокумент.Область("R" + СтроковыйНомер + 
						НомераКолонок.Получить("КолонкаНомерГТД")).Текст;
			СтранаПроисхождения = ТабличныйДокумент.Область("R" + СтроковыйНомер + 
						НомераКолонок.Получить("КолонкаСтранаПроисхождения")).Текст;
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
&НаСервере
Процедура ОбработатьПодобраннуюНоменклатуру(МассивПодобраннойНоменклатуры, НоваяСтрока, 
										НомерСтроки, ТипЦеныЗакупочная, ВысотаШапки)
	
	Если МассивПодобраннойНоменклатуры.Количество() <> 0 Тогда 
		
		Для каждого СтрокаПодобраннойНоменклатуры Из МассивПодобраннойНоменклатуры Цикл
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрокаТовары, СтрокаПодобраннойНоменклатуры);
			
			Отбор = Новый Структура;
			Отбор.Вставить("Номенклатура", НоваяСтрокаТовары.Номенклатура);	
			Отбор.Вставить("ВидЦены", ТипЦеныЗакупочная);
			
			НоваяСтрокаТовары.Цена = РегистрыСведений.ЦеныНоменклатуры25.ПолучитьПоследнее(ТекущаяДатаСеанса(), Отбор).Цена;
...
&НаСервере
Процедура ДополнитьСтруктуруДействий(СтруктураДействий, НоваяСтрока)
	...
	Если ПересчитыватьСуммы Тогда
		...
		ДобавитьДополнительныеДействия(СтруктураДействий, НоваяСтрока);		
...
&НаСервере
Процедура ДобавитьДополнительныеДействия(СтруктураДействий, НоваяСтрока)
...
&НаСервере
Процедура ДополнитьТабличнуюЧастьДокумента()
...
Функция ПодобратьНоменклатуруПоШтрихкоду(Штрихкод)
	...
	Если ЗначениеЗаполнено(ОтборПоНоменклатуре) Тогда
		...
		Если ОтборПоНоменклатуре.Свойство("ТипНоменклатуры")
			...
			ОбработатьОграничения(ОтборПоНоменклатуре, ТекстУсловияОтбораПоНоменклатуре);
...
&НаСервере
Процедура ОбработатьОграничения(ОтборПоНоменклатуре, ТекстУсловияОтбораПоНоменклатуре)
	
	ОграничиватьНаборыПоУслугам = 
		ОтборПоНоменклатуре.ТипНоменклатуры.Найти(Перечисления.ТипыНоменклатуры.Услуга) <> Неопределено
	...
	ОграничиватьНаборыПоТоварам = 
		ОтборПоНоменклатуре.ТипНоменклатуры.Найти(Перечисления.ТипыНоменклатуры.МногооборотнаяТара) <> Неопределено
...
Функция ПолучитьДанныеИБПоНоменклатуреИХарактеристикам()
	...
	Если ЗначениеЗаполнено(ОтборПоНоменклатуре) Тогда 		
		...
		УстановитьОтборПоТипуНоменклатуры(ТекстУсловияОтбораПоНоменклатуре);
	...
	ДобавитьИнформациюПоХарактеристикам(Результат, РезультатЗапроса);
			
	РезультатЗапроса = МассивРезультатов[4]; // РезультатЗапроса
	
	ДобавитьИнформациюПоНоменклатуре(Результат, РезультатЗапроса);
	...
	Если СопоставлятьПоНоменклатуреПоставщиков Тогда
		...
		Пока ВыборкаТаблицыНоменклатурыПоставщиков.Следующий() Цикл
			...
			НоваяСтрокаТаблицыНоменклатурыПоставщиков.НоменклатураПартнераПоиск = 
							ВРег(СтрЗаменить(НоваяСтрокаТаблицыНоменклатурыПоставщиков.НоменклатураПартнера, " ", ""));
			НоваяСтрокаТаблицыНоменклатурыПоставщиков.АртикулПоставщикаПоиск = 
							ВРег(СтрЗаменить(НоваяСтрокаТаблицыНоменклатурыПоставщиков.АртикулПоставщика, " ", ""));
		КонецЦикла;
	КонецЕсли;	
...
&НаСервере
Процедура УстановитьОтборПоТипуНоменклатуры(ТекстУсловияОтбораПоНоменклатуре)
...
&НаСервере
Процедура ДобавитьИнформациюПоХарактеристикам(Результат, РезультатЗапроса)
	...
	Пока ВыборкаТаблицыХарактеристик.Следующий() Цикл
		...
		НоваяСтрокаТаблицыХарактеристик.НаименованиеХарактеристикиПоиск = 
							ВРег(СтрЗаменить(НоваяСтрокаТаблицыХарактеристик.НаименованиеХарактеристики, " ", ""));
		НоваяСтрокаТаблицыХарактеристик.НаименованиеХарактеристикиПолноеПоиск = 
							ВРег(СтрЗаменить(НоваяСтрокаТаблицыХарактеристик.НаименованиеХарактеристикиПолное, " ", ""));
	КонецЦикла;
	
КонецПроцедуры	

&НаСервере
Процедура ДобавитьИнформациюПоНоменклатуре(Результат, РезультатЗапроса)
	...
	Пока ВыборкаТаблицыНоменклатуры.Следующий() Цикл
		...
		НоваяСтрокаТаблицыНоменклатуры.НаименованиеПоиск = 
							ВРег(СтрЗаменить(НоваяСтрокаТаблицыНоменклатуры.Наименование, " ", ""));
		НоваяСтрокаТаблицыНоменклатуры.НаименованиеПолноеПоиск = 
							ВРег(СтрЗаменить(НоваяСтрокаТаблицыНоменклатуры.НаименованиеПолное, " ", ""));
	КонецЦикла;
...
Процедура ДополнитьТоварыНомерамиГТД()
	...
	Для каждого УточняемаяСтрока Из ТаблицаГТД Цикл
		...
		Если ЗначениеЗаполнено(УточняемаяСтрока.НомерГТД) Тогда
			...
			ЗаполняемыеСтрока = Неопределено;
			ОбработатьНайденыеСтрокиГТД(УточняемаяСтрока, НайденныеСтроки, ЗаполняемыеСтрока);			
...
&НаСервере
Процедура ОбработатьНайденыеСтрокиГТД(УточняемаяСтрока, НайденныеСтроки, ЗаполняемыеСтрока)

	Если НайденныеСтроки.Количество() = 1 Тогда
		ЗаполняемыеСтрокиТаблицыТовары = Объект.Товары.НайтиСтроки(
									Новый Структура("КодСтроки", УточняемаяСтрока.КодСтроки));
...
Процедура ИнициализироватьТабличныйДокумент()
	...
	МакетЗаполненияПоВнешнемуИсточнику = Обработки.гф_ЗагрузкаТоваровИзВнешнихФайлов.ПолучитьМакет("МакетЗагрузкиДанных");
	...
	ЗагрузитьСуммы(МакетЗаполненияПоВнешнемуИсточнику);
...
&НаСервере
Процедура ЗагрузитьСуммы(МакетЗаполненияПоВнешнемуИсточнику)
...
Процедура ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействий, 
																ЦенаВключаетНДС, ПересчитыватьСуммы)
...
Функция ПоместитьТоварыВХранилище()
	...
	ТоварыДляПереноса.Колонки.Добавить("ХарактеристикаНабора", 
...
Процедура СоздатьНомераГТД()
	...
	ТаблицаСоздаваемыхНомеровГТД = Объект.Товары.Выгрузить(
							Новый Структура("СоздатьНомерГТД, ВедетсяУчетПоГТД", Истина, Истина),
							"ЗагружаемыйНомерГТД, СтранаПроисхождения");
	Запрос = Новый Запрос("
	|ВЫБРАТЬ
...
Процедура Далее(Команда)
	...
	ОчиститьСообщения();
	Состояние(НСтр(
			"ru = 'Осуществляется сопоставление введенных данных с данными информационной базы. Пожалуйста подождите...';
...
Процедура ПеренестиТовары(Команда)
	...
	Если Объект.Товары.НайтиСтроки(
				Новый Структура("СоздатьНомерГТД, ВедетсяУчетПоГТД", Истина, Истина)).Количество() > 0 Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	//                                                                         
	УстановитьОформлениеУчетаНДС();
	
    СкрытьКоличествоУпаковокЗапасов();                                         	
	//                                                                         
	...
	//
    УстановитьУсловноеОформлениеПоГТД();
	...
	ДополнитьУсловноеФормлениеПоГТД();
	...
	//
    УстановитьОформлениеДляУпаковочногоЛиста();
			
КонецПроцедуры

&НаСервере
Процедура СкрытьКоличествоУпаковокЗапасов()
...
&НаСервере
Процедура УстановитьУсловноеОформлениеПоГТД()
...
&НаСервере
Процедура ДополнитьУсловноеФормлениеПоГТД()
...
&НаСервере
Процедура УстановитьОформлениеДляУпаковочногоЛиста()
...
&НаСервере
Процедура УстановитьОформлениеУчетаНДС()
...
Процедура ЗаполнитьУпаковочныеЛисты()
	...
	Для Каждого Стр Из РезультатЗапроса[3].Выгрузить() Цикл
		Объект.Товары[Стр.КодСтроки - ОбратныйИндекс].УпаковочныйЛист = Стр.УпаковочныйЛист;
		Объект.Товары[Стр.КодСтроки - ОбратныйИндекс].Количество = 1;
		Объект.Товары[Стр.КодСтроки - ОбратныйИндекс].КоличествоУпаковок = 1;
	КонецЦикла;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ПрисвоитьЧисловоеЗначениеПриЗаполненииТЧ(Приемник, СтрокаИсточникЧисловогоЗначения, 
										АбсолютныйНомерСтроки, НазваниеКолонки, ВысотаШапки = 0)
	Успех = Истина;
	...
	Если ЗначениеЗаполнено(ЗначениеДляПриведения) Тогда
		...
		Иначе
			Успех = Ложь;
			ТекстОшибки = НСтр(
			 "ru = 'В строке %НомерСтроки%, колонке ""%Колонка%"" введено значение ""%Значение%"", не являющееся числом.';
