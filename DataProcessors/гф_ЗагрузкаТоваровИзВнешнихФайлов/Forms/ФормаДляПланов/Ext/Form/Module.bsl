...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.Периоды.Загрузить(ТаблицаПериоды);
	...
	ПараметрыВидаПлана = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
						ВидПлана, "ТипПлана, Владелец, ЗаполнятьПартнераВТЧ, ЗаполнятьСоглашениеВТЧ, ЗаполнятьСкладВТЧ");
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ПараметрыВидаПлана);
	
	ПараметрыСценария = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				ПараметрыВидаПлана.Владелец, "ИспользоватьРасчетПоСкоростиПродаж, ИспользоватьДляПланированияМатериалов");
	...
	УстановитьНачалоПериода(АктивныеПериоды);
	...
	Если ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
		...
		УправлениеДаннымиОбИзделияхКлиентСервер.УстановитьПараметрыВыбораСпецификаций(
															МассивЭлементов, ПараметрыВыбораСпецификаций);
		
	КонецЕсли;
	// -- НЕ УТ
	ОбработатьКроссТаблицу(КроссТаблица, ТаблицаПериоды, ПараметрыВидаПлана, КоличествоПериодов);
	...
	Элементы.ТоварыСоглашениеСПоставщиком.ОграничениеТипа = 
	...
	Если ЭтоАдресВременногоХранилища(Параметры.АдресТаблицыТоваров) Тогда
		...
		Для каждого СтруктураТовар Из МассивСтруктурТоваров Цикл
			...
			ДополнитьСтруктуруДаннымиНоменклатуры(СтруктураТовар);
						  
			ДополнитьСтруктуруДаннымиПартнера(СтруктураТовар);
						
			ДополнитьСтруктуруСкладскимДанными(СтруктураТовар);
						
			ДополнитьСтруктуруТовараПоПлануПродаж(СтруктураТовар);
...
Процедура УстановитьЗаголовок()
	...
	Если ЭтоПланПродажПоКатегориям Тогда
		...
		Элементы.Шаг1Надпись1.Заголовок = НСтр(
		"ru = 'Необходимо скопировать колонки в таблицу из внешнего файла (Excel. Word и пр.) через буфер обмена. Обязательно должна быть заполнена колонка: ""Товарная категория"".';
		|en = 'Copy columns to table from an external file (Excel. Word, etc.) via clipboard. The ""Product category"" column is required.'");
	
	Иначе
	
		Элементы.Шаг1Надпись1.Заголовок = НСтр(
		"ru = 'Необходимо скопировать колонки в таблицу из внешнего файла (Excel. Word и пр.) через буфер обмена. Обязательно должна быть заполнена одна из колонок: ""Штрихкод"", ""Код"", ""Артикул"" или ""Номенклатура"".';
...
&Насервере
Процедура УстановитьНачалоПериода(АктивныеПериоды)
...
&НаСервере
Процедура ОбработатьКроссТаблицу(КроссТаблица, ТаблицаПериоды, ПараметрыВидаПлана, КоличествоПериодов)
...
&НаСервере
Процедура ДополнитьСтруктуруДаннымиНоменклатуры(СтруктураТовар)
	...
	Если СтруктураТовар.Свойство("ИдентификаторУпаковка") 
		...
		СтруктураТовар.Вставить("Упаковка",       ?(СтрНайти(Строка(Упаковка), 
										НСтр("ru = '<Объект не найден>';
										|en = '<Object is not found>'")) > 0, 
							Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(), Упаковка));
	КонецЕсли; 
														
КонецПроцедуры

&НаСервере
Процедура ДополнитьСтруктуруДаннымиПартнера(СтруктураТовар)
	
	Если СтруктураТовар.Свойство("ИдентификаторПартнер") 
		И ЗначениеЗаполнено(СтруктураТовар.ИдентификаторПартнер) Тогда
		СтруктураТовар.Вставить("Партнер",        Справочники.Партнеры.ПолучитьСсылку(
			Новый УникальныйИдентификатор(СтруктураТовар.ИдентификаторПартнер)));
	КонецЕсли; 
	Если СтруктураТовар.Свойство("ИдентификаторСоглашение") 
		И ЗначениеЗаполнено(СтруктураТовар.ИдентификаторСоглашение) Тогда
		СтруктураТовар.Вставить("Соглашение",     Справочники.СоглашенияСПоставщиками.ПолучитьСсылку(
			Новый УникальныйИдентификатор(СтруктураТовар.ИдентификаторСоглашение)));
	КонецЕсли;	
	
КонецПроцедуры

&НаСервере
Процедура ДополнитьСтруктуруСкладскимДанными(СтруктураТовар)
	...
	Если СтруктураТовар.Свойство("ИдентификаторВариантКомплектации") 
		...
		СтруктураТовар.Вставить("ВариантКомплектации", Справочники.ВариантыКомплектацииНоменклатуры.ПолучитьСсылку(
		Новый УникальныйИдентификатор(СтруктураТовар.ИдентификаторВариантКомплектации)));
	КонецЕсли; 
	// ++ НЕ УТ
...
&НаСервере
Процедура ДополнитьСтруктуруТовараПоПлануПродаж(СтруктураТовар)
...
Процедура Далее(Команда)
	...
	Если КроссТаблица Тогда
		СтрокаНачалаДанных = 3;
	Иначе
		СтрокаНачалаДанных = 2;
	КонецЕсли; 
	
	Для каждого ОбязательноеПоле Из ОбязательныеПоля Цикл
		Область = "R" + СтрокаНачалаДанных + "C" + Формат(ОбязательноеПоле.Значение, "ЧН=0; ЧГ=0");
	...
	Состояние(
		НСтр("ru = 'Осуществляется сопоставление введенных данных с данными информационной базы. Пожалуйста подождите...';
			|en = 'Entered data is being mapped with the infobase data. Please wait...'") , , , 
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ПроверитьСопоставлениеДанныхСтрокиЗагрузки(ТекущаяСтрока, ЭтотОбъект);
	НастроитьВариантыОтображенияСтрок(ЭтотОбъект);
...
Процедура ТоварыЦенаПриИзменении(Элемент)
	...
	Если КроссТаблица Тогда
		...
		ПриИзмененииКоличестваСуммыСтроки(Объект, ЭтотОбъект, ТекущаяСтрока);
...
Процедура Подключаемый_ПриИзмененииСуммы(Элемент)
	...
	ПриИзмененииКоличестваСуммыСтроки(Объект, ЭтотОбъект, СтрокаТоваров);
...
Процедура ПриИзмененииКоличестваСуммыСтроки(Объект, Форма, СтрокаТоваров)
	...
	Для каждого СтрокаПериода Из Форма.Периоды Цикл
		...
		Если Форма.ПланироватьПоСумме Тогда
			СтрокаТоваров["Сумма_" + СтрокаПериода.ИмяКолонки] = 
					СтрокаТоваров["Количество_" + СтрокаПериода.ИмяКолонки] * СтрокаТоваров["Цена_" + СтрокаПериода.ИмяКолонки];
		Иначе
...
Процедура РассчитатьИтоговыеПоказатели(Форма, РассчитыватьСтроки = Истина)
	...
	ОбработатьТЧТовары(Товары, Форма, РассчитыватьСтроки);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ОбработатьТЧТовары(Товары, Форма, РассчитыватьСтроки)
	...
	Для каждого СтрокаТЧ Из Товары Цикл
		...
		ОбработатьСтрокуТЧТовары(СтрокаТЧ, Форма, РассчитыватьСтроки, Цена, КолонкиЦенДляОбновления);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбработатьСтрокуТЧТовары(СтрокаТЧ, Форма, РассчитыватьСтроки, Цена, КолонкиЦенДляОбновления)
	...
	Для каждого СтрокаПериода Из Форма.Периоды Цикл
		...
		Если РассчитыватьСтроки Тогда
			...
			СтрокаТЧ["Сумма_" + СтрокаПериода.ИмяКолонки] = 
			СтрокаТЧ["Количество_" + СтрокаПериода.ИмяКолонки] * СтрокаТЧ["Цена_" + СтрокаПериода.ИмяКолонки];
			СтрокаТЧ.Сумма = СтрокаТЧ.Сумма + СтрокаТЧ["Сумма_" + СтрокаПериода.ИмяКолонки];
		КонецЕсли;
		
		Форма["Объект_" + "Товары" + "Количество_" + СтрокаПериода.ИмяКолонки] = 
		Форма["Объект_" + "Товары" + "Количество_" + СтрокаПериода.ИмяКолонки] + 
		СтрокаТЧ["Количество_" + СтрокаПериода.ИмяКолонки];
		Форма["Объект_" + "Товары" + "Сумма_" + СтрокаПериода.ИмяКолонки] = 
		Форма["Объект_" + "Товары" + "Сумма_" + СтрокаПериода.ИмяКолонки] + 
		СтрокаТЧ["Сумма_" + СтрокаПериода.ИмяКолонки];
...
Процедура СопоставитьДанныеТабличногоДокументаСДаннымиИБ()
	...
	ПроверитьЗаполненостьОбязательныхПолей(ЗаполненыОбязательноеПоле, СтроковыйНомер);
	...
	Пока ЗаполненыОбязательноеПоле Цикл
		...
		ДополнитьДанныеСтроки(НоваяСтрока, НомерСтроки);
		...
		ЗаполнитьСтруктуруДействий(СтруктураДействий, НоваяСтрокаТовары, НоваяСтрока);
...
&НаСервере
Процедура ДополнитьДанныеСтроки(НоваяСтрока, НомерСтроки)
	...
	Для каждого КлючЗначение Из СтруктураПоиска Цикл
		...
		Если КлючЗначение.Значение > 0 Тогда
			...
			СтроковыйНомер = Формат(НомерСтроки, "ЧН=0; ЧГ=0");
			Значение = ТабличныйДокумент.Область("R" + СтроковыйНомер + "C" + 
			Формат(КлючЗначение.Значение, "ЧН=0; ЧГ=0")).Текст;
			
			УстановитьЗначение(Значение, КлючЗначение);											
...
&НаСервере
Процедура ПроверитьЗаполненостьОбязательныхПолей(ЗаполненыОбязательноеПоле, СтроковыйНомер)

	Для каждого ОбязательноеПоле Из ОбязательныеПоля Цикл
		
		ЗаполненыОбязательноеПоле = ЗаполненыОбязательноеПоле 
					ИЛИ ЗначениеЗаполнено(ТабличныйДокумент.Область("R" + СтроковыйНомер + "C" + 
								Формат(ОбязательноеПоле.Значение, "ЧН=0; ЧГ=0")).Текст);
...
&НаСервере
Процедура УстановитьЗначение(Значение, КлючЗначение)

	Если ТипЗнч(Значение) = Тип("Строка") 
		И (СтрНайти(КлючЗначение.Ключ, "Количество") > 0
...
&НаСервере
Процедура ЗаполнитьСтруктуруДействий(СтруктураДействий, НоваяСтрокаТовары, НоваяСтрока)
...
Процедура ИнициализироватьСтруктуруПоиска()
	...
	ДополнитьСтруктуруПоискаПоПлануПродаж(СтруктураПоиска, ОбязательныеПоля, НомерКолонки);
			
	ДополнитьСтруктуруПартнеромСкладскимиДанными(СтруктураПоиска, ОбязательныеПоля, НомерКолонки);
		
	Если КроссТаблица Тогда
		
		ДополнитьСтруктуруЕслиКроссТаблица(СтруктураПоиска, ОбязательныеПоля, НомерКолонки);
						 
	Иначе
		
		ДополнитьСтруктуруДатами(СтруктураПоиска, ОбязательныеПоля, НомерКолонки);
...
&НаСервере
Процедура ДополнитьСтруктуруПоискаПоПлануПродаж(СтруктураПоиска, ОбязательныеПоля, НомерКолонки)
...
&НаСервере
Процедура ДополнитьСтруктуруПартнеромСкладскимиДанными(СтруктураПоиска, ОбязательныеПоля, НомерКолонки)
...
&НаСервере 
Процедура ДополнитьСтруктуруЕслиКроссТаблица(СтруктураПоиска, ОбязательныеПоля, НомерКолонки)
...
&НаСервере
Процедура ДополнитьСтруктуруДатами(СтруктураПоиска, ОбязательныеПоля, НомерКолонки)
...
Процедура СопоставитьЗагружаемыеДанныеСДаннымиИБ()
	...
	Для каждого СтрокаТЧ Из Объект.Товары Цикл
		...
		ЗаполнитьСтруктуруДействийПриСопоставлении(СтруктураДействий, СтрокаТЧ);
...
&НаСервере
Процедура ЗаполнитьСтруктуруДействийПриСопоставлении(СтруктураДействий, СтрокаТЧ)
...
Процедура НайтиНоменклатуруПоЗагружаемымДанным(СтрокаТЧ)
	...
	Если ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемыйШтрихкод) Тогда
		...
		НайтиСтрокиПоШтрихкоду(СтрокаТЧ);
		
	ИначеЕсли ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемыйКод) И ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяХарактеристика) Тогда
		
		НайтиСтрокиПоКодуИХарактеристике(НомерСтроки, СтрокаТЧ, СтрокаПоискаКод, СтрокаПоискаХарактеристика);
			
	ИначеЕсли ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемыйАртикул) 
			И ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяНоменклатура) 
			И ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяХарактеристика) Тогда
			
		НайтиСтрокиПоАртикулНоменклатураХарактеристика(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул, СтрокаПоискаНоменклатура,
	...
	ИначеЕсли ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемыйКод) И НЕ ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяХарактеристика) Тогда
		...
		НайтиСтрокиПоКодИНаименование(НомерСтроки, СтрокаТЧ, СтрокаПоискаКод, СтрокаПоискаНоменклатура);
		
	Иначе
		ПродолжитьПоискПоКлючевымПолям(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул, СтрокаПоискаНоменклатура, 
												СтрокаПоискаХарактеристика);
...
&НаСервере
Процедура ПродолжитьПоискПоКлючевымПолям(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул, СтрокаПоискаНоменклатура,
												СтрокаПоискаХарактеристика)
	
Если ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемыйАртикул) 
	И ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяНоменклатура) 
	И НЕ ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяХарактеристика) Тогда
	
	НайтиСтрокиПоАртикулИНаименование(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул, СтрокаПоискаНоменклатура);
	
ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемыйАртикул) 
	И ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяНоменклатура) 
	И ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяХарактеристика) Тогда
	
	НайтиСтрокиПоХарактеристикаИНаименование(НомерСтроки, СтрокаТЧ, СтрокаПоискаХарактеристика, 
	СтрокаПоискаНоменклатура);		
	
ИначеЕсли ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемыйАртикул) 
	И НЕ ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяНоменклатура) 
	И НЕ ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяХарактеристика) Тогда
	
	НайтиСтрокиПоАртикул(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул);
	
ИначеЕсли ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемыйАртикул) 
	И НЕ ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяНоменклатура) 
	И ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяХарактеристика) Тогда
	
	НайтиСтрокиПоАртикулИХарактеристика(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул, СтрокаПоискаХарактеристика);
	
ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемыйАртикул) 
	И ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяНоменклатура) 
	И НЕ ЗначениеЗаполнено(СтрокаТЧ.ЗагружаемаяХарактеристика) Тогда
	
	НайтиСтрокиПоНаименованиюНомеклатуры(НомерСтроки, СтрокаТЧ, СтрокаПоискаНоменклатура);
...
&НаСервере
Процедура НайтиСтрокиПоШтрихкоду(СтрокаТЧ)
...
&НаСервере
Процедура НайтиСтрокиПоКодуИХарактеристике(НомерСтроки, СтрокаТЧ, СтрокаПоискаКод, СтрокаПоискаХарактеристика)
...
&НаСервере
Процедура НайтиСтрокиПоАртикулНоменклатураХарактеристика(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул, 
									СтрокаПоискаНоменклатура, СтрокаПоискаХарактеристика)

	НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(
			Новый Структура("АртикулПоиск, НаименованиеПоиск, НаименованиеХарактеристикиПоиск", 
				СтрокаПоискаАртикул, 
				СтрокаПоискаНоменклатура,
				СтрокаПоискаХарактеристика));
		Если НайденныеСтроки.Количество() = 0 Тогда
			НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(
				Новый Структура("АртикулПоиск, НаименованиеПолноеПоиск, НаименованиеХарактеристикиПоиск", 
					СтрокаПоискаАртикул, 
					СтрокаПоискаНоменклатура,
					СтрокаПоискаХарактеристика)); 
					
			ПродолжитьПоискПоКлючам(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул, 
									СтрокаПоискаНоменклатура, СтрокаПоискаХарактеристика, НайденныеСтроки);
			
		КонецЕсли;
		
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			ПодобранныеРанееТовары = ПодобраннаяНоменклатура.НайтиСтроки(
										Новый Структура("Номенклатура, Характеристика, КодСтроки",
												НайденнаяСтрока.Номенклатура, НайденнаяСтрока.Характеристика, НомерСтроки));
...
&НаСервере
Процедура ПродолжитьПоискПоКлючам(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул, 
									СтрокаПоискаНоменклатура, СтрокаПоискаХарактеристика, НайденныеСтроки)
		
	Если НайденныеСтроки.Количество() = 0 Тогда
		НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(
		Новый Структура("АртикулПоиск, НаименованиеПоиск, НаименованиеХарактеристикиПолноеПоиск", 
		СтрокаПоискаАртикул, 
		СтрокаПоискаНоменклатура,
		СтрокаПоискаХарактеристика));
		Если НайденныеСтроки.Количество() = 0 Тогда
			НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(
			Новый Структура("АртикулПоиск, НаименованиеПолноеПоиск, НаименованиеХарактеристикиПолноеПоиск", 
			СтрокаПоискаАртикул, 
			СтрокаПоискаНоменклатура,
			СтрокаПоискаХарактеристика));
			Если НайденныеСтроки.Количество() = 0 Тогда
				НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(
				Новый Структура("АртикулПоиск, НаименованиеПоиск", 
				СтрокаПоискаАртикул, 
				СтрокаПоискаНоменклатура));
				Если НайденныеСтроки.Количество() = 0 Тогда
					НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(
					Новый Структура("АртикулПоиск, НаименованиеПолноеПоиск", 
					СтрокаПоискаАртикул, 
					СтрокаПоискаНоменклатура));
				КонецЕсли;
			КонецЕсли;
...
&НаСервере
Процедура НайтиСтрокиПоКодИНаименование(НомерСтроки, СтрокаТЧ, СтрокаПоискаКод, СтрокаПоискаНоменклатура)
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		ПодобранныеРанееТовары = ПодобраннаяНоменклатура.НайтиСтроки(
		Новый Структура("Номенклатура, КодСтроки", НайденнаяСтрока.Номенклатура, НомерСтроки));
		Если ПодобранныеРанееТовары.Количество() = 0 Тогда
			ПодобраннаяСтрока = ПодобраннаяНоменклатура.Добавить();
			ПодобраннаяСтрока.КодСтроки = НомерСтроки;
...
&НаСервере
Процедура НайтиСтрокиПоАртикулИНаименование(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул, СтрокаПоискаНоменклатура)
...
&НаСервере 
Процедура НайтиСтрокиПоХарактеристикаИНаименование(НомерСтроки, СтрокаТЧ, СтрокаПоискаХарактеристика, 
																		СтрокаПоискаНоменклатура)
		
	НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(
	Новый Структура("НаименованиеХарактеристикиПоиск, НаименованиеПоиск",
	СтрокаПоискаХарактеристика,
	СтрокаПоискаНоменклатура));
	
	Если НайденныеСтроки.Количество() = 0 Тогда
		НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(
		Новый Структура("НаименованиеХарактеристикиПоиск, НаименованиеПолноеПоиск",
		СтрокаПоискаХарактеристика,
		СтрокаПоискаНоменклатура));
		Если НайденныеСтроки.Количество() = 0 Тогда
			НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(
			Новый Структура("НаименованиеХарактеристикиПолноеПоиск, НаименованиеПоиск",
			СтрокаПоискаХарактеристика,
			СтрокаПоискаНоменклатура));
			Если НайденныеСтроки.Количество() = 0 Тогда
				НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(
				Новый Структура("НаименованиеХарактеристикиПолноеПоиск, НаименованиеПолноеПоиск",
				СтрокаПоискаХарактеристика,
				СтрокаПоискаНоменклатура));
				
				ПродолжитьПоискПоКлючамХаракткристикиНаименованию(НомерСтроки, СтрокаТЧ, СтрокаПоискаХарактеристика, 
																		СтрокаПоискаНоменклатура, НайденныеСтроки);
...
&НаСервере
Процедура ПродолжитьПоискПоКлючамХаракткристикиНаименованию(НомерСтроки, СтрокаТЧ, СтрокаПоискаХарактеристика, 
																		СтрокаПоискаНоменклатура, НайденныеСтроки)
...
&НаСервере
Процедура НайтиСтрокиПоАртикул(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул)
...
&НаСервере
Процедура НайтиСтрокиПоАртикулИХарактеристика(НомерСтроки, СтрокаТЧ, СтрокаПоискаАртикул, СтрокаПоискаХарактеристика)
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(
		Новый Структура("АртикулПоиск, НаименованиеХарактеристикиПолноеПоиск",
		СтрокаПоискаАртикул,
		СтрокаПоискаХарактеристика));
	КонецЕсли;
...
&НаСервере
Процедура НайтиСтрокиПоНаименованиюНомеклатуры(НомерСтроки, СтрокаТЧ, СтрокаПоискаНоменклатура)
...
Процедура СопоставитьТоварныеКатегорииРейтинги()
	...
	|		ПО (ТоварныеКатегории.Наименование = ДанныеДляСопоставления.ЗагружаемаяТоварнаяКатегория)
	...
	|		ПО (РейтингиПродажНоменклатуры.Наименование = ДанныеДляСопоставления.ЗагружаемыйРейтингПродаж)
	|ГДЕ
	...
	Для каждого СтрокаТаблицы Из Объект.Товары Цикл 
		...
		СопоставитьКатегорию(СтрокаТаблицы, ТаблицаТоварныеКатегории, СтрокаТоварнаяКатегория);
				
		СтрокаРейтингПродаж = Неопределено;
		
		СопоставитьРейтинг(СтрокаТаблицы, СтрокаРейтингПродаж, ТаблицаРейтингиПродаж);
				
		Если СтрокаТоварнаяКатегория <> Неопределено ИЛИ СтрокаРейтингПродаж <> Неопределено Тогда
			ПроверитьСопоставлениеДанныхСтрокиЗагрузки(СтрокаТаблицы, ЭтотОбъект);
		КонецЕсли; 
...
&НаСервере
Процедура СопоставитьКатегорию(СтрокаТаблицы, ТаблицаТоварныеКатегории, СтрокаТоварнаяКатегория)
...
&НаСервере
Процедура СопоставитьРейтинг(СтрокаТаблицы, СтрокаРейтингПродаж, ТаблицаРейтингиПродаж)
...
Процедура СопоставитьПартнераИСоглашение()
	...
	|		ПО (Партнеры.Наименование = ДанныеДляСопоставления.ЗагружаемыйПартнер)
	|ГДЕ
	...
	|			И (Соглашения.Наименование = ДанныеДляСопоставления.ЗагружаемоеСоглашение)
	|ГДЕ
	...
	Для каждого СтрокаТаблицы Из Объект.Товары Цикл 
		...
		СопоставитьПартнера(СтрокаТаблицы, ТаблицаПартнеры, СтрокаПартнер);
						
		СтрокаСоглашение = Неопределено; 
		СопоставитьСоглашение(СтрокаТаблицы, ТаблицаСоглашения, СтрокаСоглашение);
...
&НаСервере
Процедура СопоставитьПартнера(СтрокаТаблицы, ТаблицаПартнеры, СтрокаПартнер)
...
&НаСервере 
Процедура СопоставитьСоглашение(СтрокаТаблицы, ТаблицаСоглашения, СтрокаСоглашение)
...
Процедура СопоставитьУпаковки()
	...
	|			И (УпаковкиНоменклатуры.Наименование = ДанныеДляСопоставления.ЗагружаемаяУпаковка)
	|ГДЕ
...
Процедура СопоставитьСклады()
	...
	|		ПО (Склады.Наименование = ДанныеДляСопоставления.ЗагружаемыйСклад)
...
Процедура СопоставитьВариантыКомплектаций()
	...
	|			И (ВариантыКомплектацииНоменклатуры.Наименование = ДанныеДляСопоставления.ЗагружаемыйВариантКомплектации)
	|ГДЕ
...
Процедура СопоставитьСпецификации()
	...
	|			И (РесурсныеСпецификации.Ссылка.Наименование = ДанныеДляСопоставления.ЗагружаемаяСпецификация
	|				ИЛИ ДанныеДляСопоставления.ЗагружаемаяСпецификация = """")
	|ГДЕ
...
Процедура ИнициализироватьТабличныйДокумент()
	...
	СуффиксМакета = ?(КроссТаблица, "Кросс", "");
	...
	ИнициализироватьТабличныйДокументПоНоменклатуре(МакетЗаполненияПоВнешнемуИсточнику, ТабличныйДокумент);
		
	ИнициализироватьТабличныйДокументПоПартнеру(МакетЗаполненияПоВнешнемуИсточнику, ТабличныйДокумент);
		
	ИнициализироватьТабличныйДокументПоСкладуРейтингу(МакетЗаполненияПоВнешнемуИсточнику, ТабличныйДокумент);
		
	Если КроссТаблица Тогда
		
		ИнициализироватьТабличныйДокументКроссТаблица(МакетЗаполненияПоВнешнемуИсточнику, ТабличныйДокумент);
	...
	Иначе
		...
		ИнициализироватьТабличныйДокументУпаковкаСумма(МакетЗаполненияПоВнешнемуИсточнику, ТабличныйДокумент);
...
&НаСервере
Процедура ИнициализироватьТабличныйДокументПоНоменклатуре(МакетЗаполненияПоВнешнемуИсточнику, ТабличныйДокумент)
...
&НаСервере
Процедура ИнициализироватьТабличныйДокументПоПартнеру(МакетЗаполненияПоВнешнемуИсточнику, ТабличныйДокумент)
...
&НаСервере
Процедура ИнициализироватьТабличныйДокументПоСкладуРейтингу(МакетЗаполненияПоВнешнемуИсточнику, ТабличныйДокумент)
...
&НаСервере
Процедура ИнициализироватьТабличныйДокументКроссТаблица(МакетЗаполненияПоВнешнемуИсточнику, ТабличныйДокумент)
	...
	ЭтаФормаПериоды = ЭтотОбъект.Периоды; // см. Планирование.ТаблицаПериоды
...
&НаСервере
Процедура ИнициализироватьТабличныйДокументУпаковкаСумма(МакетЗаполненияПоВнешнемуИсточнику, ТабличныйДокумент)
...
Процедура ПроверитьСопоставлениеДанныхСтрокиЗагрузки(СтрокаЗагрузки, Форма)
	...
	ПроверитьСопоставлениеДанныхПоНоменклатуре(СтрокаЗагрузки, Форма);
     
	ПроверитьСопоставлениеДанныхПоПартнеру(СтрокаЗагрузки, Форма);
		
	ПроверитьСопоставлениеДанныхПоСкладамРейтингам(СтрокаЗагрузки, Форма);
			
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ПроверитьСопоставлениеДанныхПоНоменклатуре(СтрокаЗагрузки, Форма)
...
&НаКлиентеНаСервереБезКонтекста
Процедура ПроверитьСопоставлениеДанныхПоПартнеру(СтрокаЗагрузки, Форма)
...
&НаКлиентеНаСервереБезКонтекста
Процедура ПроверитьСопоставлениеДанныхПоСкладамРейтингам(СтрокаЗагрузки, Форма)
	...
	ПроверитьСопоставлениеДанныхПоРейтингам(СтрокаЗагрузки, Форма);
			
КонецПроцедуры	

&НаКлиентеНаСервереБезКонтекста
Процедура ПроверитьСопоставлениеДанныхПоРейтингам(СтрокаЗагрузки, Форма)
...
Процедура ЗаполнитьСписокВыбораНоменклатурыХарактеристик(КодТекущейСтроки, СписокХарактеристик = Ложь)
	...
	Для каждого ЭлементМассива Из МассивПодобраннойНоменклатуры Цикл
		Если СписокХарактеристик Тогда
			Если СписокВыбораХарактеристика.НайтиПоЗначению(ЭлементМассива.Характеристика) = Неопределено Тогда
				СписокВыбораХарактеристика.Добавить(ЭлементМассива.Характеристика);
			КонецЕсли;
		Иначе
...
Процедура НастроитьВариантыОтображенияСтрок(Форма)
	...
	СтрокиЕстьНесопоставленныеДанные = Форма.Объект.Товары.НайтиСтроки(
										Новый Структура("ЕстьНесопоставленныеДанные", Истина));
	Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура УстановитьДоступностьКолонок()
	...
	Элементы.ТоварыСоглашениеСПоставщиком.Видимость = ТипПлана = Перечисления.ТипыПланов.ПланЗакупок 
																			И ЗаполнятьСоглашениеВТЧ;
	...
	//++ НЕ УТ
	Элементы.ТоварыГруппаСпецификация.Видимость = (ТипПлана = Перечисления.ТипыПланов.ПланПроизводства) 
																				И ИспользоватьДляПланированияМатериалов;
	//-- НЕ УТ
