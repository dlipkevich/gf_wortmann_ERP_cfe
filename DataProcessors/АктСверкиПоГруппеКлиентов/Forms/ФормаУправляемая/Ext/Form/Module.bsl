...
Процедура ЗаполнитьПоДаннымБухгалтерскогоУчета(ФильтрСписокСчетов = неопределено) Экспорт
	...
	//(н)Галфинд/Сефербеков 16.04.2024 ( 	
	|ВЫБОР
	|		КОГДА ХозрасчетныйОбороты.Субконто2 = НЕОПРЕДЕЛЕНО
	|				ИЛИ ХозрасчетныйОбороты.Субконто2 ЕСТЬ NULL
	|				ИЛИ ХозрасчетныйОбороты.Субконто2 = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ДоговорЗаполнен,
	//(к)Галфинд/Сефербеков 16.04.2024 ) 
	...
	|	ВременнаяТаблицаДанных.ДокументРегистратор
	//(н)Галфинд/Сефербеков 16.04.2024 ( 
	|	,ВременнаяТаблицаДанных.ДоговорЗаполнен
	//(к)Галфинд/Сефербеков 16.04.2024 ) 
	|
	|ИМЕЮЩИЕ
	...
	//(н)Галфинд/Сефербеков 16.04.2024 ( 
	|И ВЫБОР
	|			КОГДА ВременнаяТаблицаДанных.КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетныеСчета))
	|					И НЕ ВременнаяТаблицаДанных.ДоговорЗаполнен
	|				ТОГДА ЛОЖЬ
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	//(к)Галфинд/Сефербеков 16.04.2024 ) 
	...
	Если ЗначениеЗаполнено(Объект.ДатаНачала) Тогда
		...
		// ++ Галфинд СадомцевСА 13.02.2024 Исправил заполнение "Остатка на начало"
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eec98c7f9941a9
		Объект.ОстатокНаНачало = ОстатокНаНачало;
		// -- Галфинд СадомцевСА 13.02.2024
