Процедура ВыполнитьЗагрузкуДанныхОтчетаИзФайла(АдресВХ, Настройки) Экспорт
	...
	Для сч=НомерСтрокиНачалаДанных По ТабДок.ВысотаТаблицы Цикл
		...
		//<< Перминов А.С. 09.10.2023
		Попытка
			СуммаВознаграждения = Число(СОКРЛП(ТабДок.Область(АдресЯчейки).Значение)); 
		Исключение
			СуммаВознаграждения = Число(СтрЗаменить(СтрЗаменить(СОКРЛП(ТабДок.Область(АдресЯчейки).Текст),Символы.НПП,""),".",",")); 
		КонецПопытки;
		//<< Перминов А.С. 09.10.2023
