...
Функция ПеренестиВНовыйОрдерСервер(ЗаданиеНаПеревозку, СтрокиДляПереноса, Документ)
	...
	НовыйОрдерОбъект.Статус = Перечисления.СтатусыРасходныхОрдеров.КОтбору;
...
Процедура ИзменитьСтатусыПеремещений(ДокументыПеремещения) 
	...
	Для каждого Перемещение Из ДокументыПеремещения Цикл
		...
		Исключение 
			...
			ЗаписьЖурналаРегистрации("ОбработкаРасходныхОрдеровПослеСклада", УровеньЖурналаРегистрации.Информация, ЭтотОбъект, ЭтотОбъект, Сообщение);
		КонецПопытки;
...
Процедура СоздатьПеремещения(ОбработанныеЗаявки)
	...
	Для каждого Документ Из  ОбработанныеЗаявки Цикл
		...
		ПеремещениеОбъект.Статус = Перечисления.СтатусыПеремещенийТоваров.Принято;// изменено Галфинд_ДомнышеваКР_29_08_2023
		ПеремещениеОбъект.гф_ЗаявкаНаВозврат = Документ.Ссылка; 
		
		// ++Галфинд_ДомнышеваКР_29_08_2023  
		// необходимо заполнять ТЧ ШтрихкодыУпаковок
		Для каждого Строка Из ТоварыДляЗагрузки Цикл
			нс = ПеремещениеОбъект["гф_ШтрихкодыУпаковок"].Добавить();
		...
		ПеремещениеОбъект.ДополнительныеСвойства.Вставить("гф_ОтгрузкаПеремещением", Истина);// ++ Галфинд_ДомнышеваКР_29_08_2023 
		Попытка
