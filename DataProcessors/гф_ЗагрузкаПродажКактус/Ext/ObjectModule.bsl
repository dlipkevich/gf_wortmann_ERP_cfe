							...
							Если элМассива["num"] = НовСтр.НомерСтрокиВЗаказе И элМассива["barcode"] = НовСтр.Штрихкод Тогда
								...
								Для Каждого элУчетныеДанные из мУчетныеДанные Цикл
									...
									Если элУчетныеДанные["code"] = "chestnyznak" Тогда
										ЗаполнитьСтуктуруСтрокиКМ(СтруктураСтрокиКМ, элУчетныеДанные["stringValue"]); 
...
Процедура ЗаполнитьСтуктуруСтрокиКМ(Структура, Значение)
	
	Структура.КодМаркировкиСтрока = Значение;
	Структура.КодМаркировки = ПреобразоватьКМвФормат1С(Структура.КодМаркировкиСтрока);
	Структура.КМBASE64 = ШтрихкодированиеИСКлиентСервер.ШтрихкодВBase64(Структура.КодМаркировки);
