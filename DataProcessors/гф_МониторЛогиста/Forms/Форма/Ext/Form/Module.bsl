...
Процедура ТаблицаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = ИмяТаблицы + "ТК" Тогда
...
Функция ПолучитьТекстЗапросаПоставка()
	...
	Результат = "ВЫБРАТЬ
	            ...
	            |	ВЫБОР
	            |		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СтатусыДокументовИСМП.Статус) < 2
	            |			ТОГДА МАКСИМУМ(СтатусыДокументовИСМП.Статус)
	            |		ИНАЧЕ ""Другое""
	            |	КОНЕЦ КАК СтатусКМ,
	            ...
	            |	ЕСТЬNULL(гф_ИсторияСтатусовПриходныхОрдеровСрезПоследних.Загружен, ЛОЖЬ) КАК ПОЗагружен,
	            |	ВЫБОР
	            |		КОГДА ПриходныйОрдерНаТовары.Ссылка ЕСТЬ NULL
	            |			ТОГДА """"
	            |		КОГДА ЕСТЬNULL(гф_ИсторияСтатусовПриходныхОрдеровСрезПоследних.Загружен, ЛОЖЬ)
	            |			ТОГДА ""Обмен с WMS""
	            |		ИНАЧЕ ""Оприходовал""
	            |	КОНЕЦ КАК Оприходовал
	            |ИЗ
...
Процедура УстановитьДоступностьКолонокТаблицы(ИмяТаблицы)
	...
	Для Каждого Колонка Из Элементы[ИмяТаблицы].ПодчиненныеЭлементы Цикл
		...
		Если Колонка.Имя = ИмяТаблицы + "ГруппаТК" Тогда
			
			Для Каждого ЭлементГруппыТК Из Колонка.ПодчиненныеЭлементы Цикл
				
				Если ЭлементГруппыТК.Имя <> ИмяТаблицы + "ТК" Тогда
					
					ЭлементГруппыТК.ТолькоПросмотр = Истина;
