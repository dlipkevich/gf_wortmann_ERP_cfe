...
Процедура ИзменитьСтатусыРО(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ИзменитьСтатусыРОПродолжение", ЭтотОбъект);
...
&НаКлиенте
Процедура ИзменитьСтатусыРОПродолжение(Статус, ДополнительныеПараметры) Экспорт 
...
Процедура ОбновитьТаблицу(ИмяТаблицы)
	...
	Для Каждого ОтмеченнаяСтрока Из ОтмеченныеСтроки Цикл
		...
		Иначе
			...
			Если ИмяТаблицы = "Отгрузка" Тогда
	...
	Если МассивОтмеченных.Количество() Тогда
		...
		Для Каждого СтрокаТаблицы Из Объект[ИмяТаблицы] Цикл
			...
			Иначе	
				...
				Если ИмяТаблицы = "Отгрузка" И МассивОтмеченных.Найти(СтрокаТаблицы.РасходныйОрдер) <> Неопределено Тогда
