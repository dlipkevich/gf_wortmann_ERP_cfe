...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ++ Галфинд СадомцевСА 16.01.2024 Ограничил доступ для пользователя при наличии роли "Монитор логиста ограничения (гф)"
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeb125b67e066b
	Если РольДоступна("гф_МониторЛогистаОграничения") Тогда
		ОграничитьФункционал();
	КонецЕсли;
	// -- Галфинд СадомцевСА 16.01.2024
...
Процедура ИзменитьСтатусыРОПродолжение(Статус, ОтмеченныеСтроки) Экспорт
	...
	Если ЗначениеЗаполнено(Статус) Тогда
		...
		Если МассивОрдеров.Количество() Тогда
			...
			// vvv Галфинд \ Sakovich 01.04.2024
			// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeed9ba6fc6a20
			ОповещениеОЗавершении = Новый ОписаниеОповещения("ИзменениеСтатусовРООкончание", ЭтотОбъект, МассивОрдеров);
			Кнопки = Новый СписокЗначений();
			Кнопки.Добавить(1,"Да");
			Кнопки.Добавить(2,"Нет");
			Кнопки.Добавить(3,"Отмена");
			ПоказатьВопрос(ОповещениеОЗавершении, "Выполнено изменение статусов РО. Провести документы РО?", 
			Кнопки, 30, 2, "Проведение расходных ордеров", 2);
			// ^^^ Галфинд \ Sakovich 01.04.2024
...
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeed9ba6fc6a20
&НаКлиенте
Процедура ИзменениеСтатусовРООкончание(Результат, МассивОрдеров) Экспорт
	Если Результат = 1 Тогда
		ВыполнитьПроведениеРОНаСервере(МассивОрдеров);
	КонецЕсли;
КонецПроцедуры // ^^^ Галфинд \ Sakovich 01.04.2024

// vvv Галфинд \ Sakovich 01.04.2024
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeed9ba6fc6a20
&НаСервере
Процедура ВыполнитьПроведениеРОНаСервере(МассивОрдеров)
	Для каждого РО Из МассивОрдеров Цикл
		обРО = РО.ПолучитьОбъект();
		Попытка
			обРО.Записать(РежимЗаписиДокумента.Проведение);
		Исключение
			ОписаниеОшибки = ОписаниеОшибки();
			ОбщегоНазначения.СообщитьПользователю("Не удалось провести " + РО + " по причине: " + ОписаниеОшибки);
		КонецПопытки;
	КонецЦикла;
КонецПроцедуры // ^^^ Галфинд \ Sakovich 01.04.2024
...
Функция ПолучитьТекстЗапросаПоставка()
	...
	|		Подзапрос.Склад КАК Склад,
	|		Подзапрос.Контрагент КАК Контрагент,
	|		КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПриобретениеТоваровУслуггф_ПродукцияВКоробах.IDКороба) КАК КоличествоКоробов,
	|		Подзапрос.КоличествоПар КАК КоличествоПар,
	|		Подзапрос.Партнер КАК Партнер,
	|		Подзапрос.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|		Подзапрос.ДатаПоступления КАК ДатаПоступления,
...
Функция ПолучитьТекстЗапросаОтгрузка()
	...
	|	ОрдерТоварыПоРаспоряжениям.Распоряжение КАК Распоряжение,
	|	ОрдерТоварыПоРаспоряжениям.Ссылка КАК Ордер,
	|	СУММА(ОрдерТоварыПоРаспоряжениям.Количество) КАК Количество
	|ПОМЕСТИТЬ РасходныеОрдераВнутреннееПотребление
	|ИЗ
	|	Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК ОрдерТоварыПоРаспоряжениям
	|ГДЕ
	|	ОрдерТоварыПоРаспоряжениям.Распоряжение ССЫЛКА Документ.ЗаказНаВнутреннееПотребление
	|
	|СГРУППИРОВАТЬ ПО
	|	ОрдерТоварыПоРаспоряжениям.Распоряжение,
	|	ОрдерТоварыПоРаспоряжениям.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|			ИЛИ НЕ РеализацияТоваровУслуг.гф_РасходныйОрдер = ЗНАЧЕНИЕ(Документ.РасходныйОрдерНаТовары.ПустаяСсылка))
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВнутреннееПотребление.Дата,
	|	ВнутреннееПотребление.Номер,
	|	ТИПЗНАЧЕНИЯ(ВнутреннееПотребление.Ссылка),
	...
	|	Контрагенты.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ЮрЛицоНеРезидент),
	|	Партнеры.Код,
	|	ВнутреннееПотребление.Склад,
	|	РасходныеОрдераВнутреннееПотребление.Количество,
	|	Заказ.Ссылка,
	|	РасходныеОрдераВнутреннееПотребление.Ордер,
	|	гф_АдресаДоставкиСправочник.Ссылка,
	|	гф_АдресаДоставкиСправочник.НомерАдреса,
	|	гф_АдресаДоставкиСправочник.РегионКод,
	|	гф_АдресаДоставкиСправочник.РегионНаименование,
	|	гф_АдресаДоставкиСправочник.ТипПрайсЛистаТК,
	|	гф_АдресаДоставкиСправочник.ТК,
	|	ЗНАЧЕНИЕ(Справочник.гф_ПочтаРоссииПочтовыеОтправления.ПустаяСсылка)
	|ИЗ
	|	Документ.ВнутреннееПотребление КАК ВнутреннееПотребление
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
	|		ПО ВнутреннееПотребление.гф_Контрагент = Контрагенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Партнеры КАК Партнеры
	|		ПО ВнутреннееПотребление.гф_Партнер = Партнеры.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВнутреннееПотребление.Товары КАК ВнутреннееПотреблениеТовары
	|		ПО ВнутреннееПотребление.Ссылка = ВнутреннееПотреблениеТовары.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаВнутреннееПотребление КАК Заказ
	|			ЛЕВОЕ СОЕДИНЕНИЕ РасходныеОрдераВнутреннееПотребление КАК РасходныеОрдераВнутреннееПотребление
	|			ПО Заказ.Ссылка = РасходныеОрдераВнутреннееПотребление.Распоряжение
	|		ПО ВнутреннееПотребление.ЗаказНаВнутреннееПотребление = Заказ.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.гф_АдресаДоставки КАК гф_АдресаДоставкиСправочник
	|		ПО ВнутреннееПотребление.гф_АдресДоставки = гф_АдресаДоставкиСправочник.Ссылка
	|ГДЕ
	|	(ВнутреннееПотребление.Проведен
	|			ИЛИ НЕ РасходныеОрдераВнутреннееПотребление.Ордер В (NULL, ЗНАЧЕНИЕ(Документ.РасходныйОрдерНаТовары.ПустаяСсылка)))
	|	И ВнутреннееПотребление.Дата >= &ДатаНачала
	...
	|	ВЫБОР
	|		КОГДА ВТ_Подзапрос1.ЗаказКлиента ССЫЛКА Документ.ЗаказКлиента
	|			ТОГДА ВЫБОР
	...
	|					ИНАЧЕ 0
	|				КОНЕЦ
	|		КОГДА ВТ_Подзапрос1.ЗаказКлиента ССЫЛКА Документ.ЗаказНаВнутреннееПотребление
	|			ТОГДА ВЫБОР
	|					КОГДА МАКСИМУМ(ЗаказНаВнутреннееПотреблениеТовары.ДатаОтгрузки) < НАЧАЛОПЕРИОДА(ВТ_Подзапрос1.Дата, ДЕНЬ)
	|						ТОГДА РАЗНОСТЬДАТ(МАКСИМУМ(ЗаказНаВнутреннееПотреблениеТовары.ДатаОтгрузки), ВТ_Подзапрос1.Дата, ДЕНЬ)
	|					ИНАЧЕ 0
	|				КОНЕЦ
	|		ИНАЧЕ 0
	...
	|	ВЫБОР
	|		КОГДА ВТ_Подзапрос1.ЗаказКлиента ССЫЛКА Документ.ЗаказКлиента
	|			ТОГДА МАКСИМУМ(ЗаказКлиентаТовары.ДатаОтгрузки)
	|		КОГДА ВТ_Подзапрос1.ЗаказКлиента ССЫЛКА Документ.ЗаказНаВнутреннееПотребление
	|			ТОГДА МАКСИМУМ(ЗаказНаВнутреннееПотреблениеТовары.ДатаОтгрузки)
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ КАК ДатаПлановойОтгрузки
	...
	|		ПО ВТ_Подзапрос1.ЗаказКлиента = ЗаказНаВнутреннееПотреблениеТовары.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ВЫБОР
	|		КОГДА ВТ_Подзапрос2.ЗаказКлиента ССЫЛКА Документ.ЗаказКлиента
	|			ТОГДА КОЛИЧЕСТВО(ТоварыВКоробах.НомерСтроки)
	|		КОГДА ВТ_Подзапрос2.ЗаказКлиента ССЫЛКА Документ.ЗаказНаВнутреннееПотребление
	|			ТОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ УпаковочныйЛистВнутреннееПотребление.Ссылка)
	|		ИНАЧЕ 0
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УпаковочныйЛист КАК УпаковочныйЛистВнутреннееПотребление
	|		ПО (ВТ_Подзапрос2.ЗаказКлиента ССЫЛКА Документ.ЗаказНаВнутреннееПотребление)
	|			И ВТ_Подзапрос2.ЗаказКлиента = УпаковочныйЛистВнутреннееПотребление.гф_Заказ
	|			И (НЕ УпаковочныйЛистВнутреннееПотребление.ПометкаУдаления)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК КоличествоПар,
	|	Склады.гф_Организация КАК гф_Организация,
	|	СУММА(ВЫБОР
	|			КОГДА РасходныйОрдерНаТоварыОтгружаемыеТовары.ЭтоУпаковочныйЛист
	|				ТОГДА РасходныйОрдерНаТоварыОтгружаемыеТовары.Количество
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК КоличествоКоробов,
	|	ВЫБОР
	|		КОГДА ЗаданияНаПеревозку.Ссылка ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		КОГДА ЗаданияНаПеревозку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаданийНаПеревозку.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		КОГДА ЗаданияНаПеревозку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаданийНаПеревозку.Формируется)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК СозданаЗаявкаВ_ТК,
	...
	|		ПО РасходныйОрдерНаТовары.ЗаданиеНаПеревозку = ЗаданияНаПеревозку.Ссылка
	|			И РасходныйОрдерНаТовары.ЗаданиеНаПеревозку = ЗаданияНаПеревозку.Ссылка
	|ГДЕ
	|	РасходныйОрдерНаТовары.Проведен
	|	И РасходныйОрдерНаТовары.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И РасходныйОрдерНаТовары.СкладскаяОперация В (ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ОтгрузкаКлиенту), ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ОтгрузкаНаВнутренниеНужды))
	...
	|ГДЕ
	|	НЕ Подзапрос.РТУ В (ЗНАЧЕНИЕ(Документ.РеализацияТоваровУслуг.ПустаяСсылка), ЗНАЧЕНИЕ(Документ.ВнутреннееПотребление.ПустаяСсылка))
	...
	|	ВЫБОР
	|		КОГДА гф_ИсторияСтатусовРасходныхОрдеровСрезПоследних.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРасходныхОрдеров.Отгружен)
	|			ТОГДА гф_ИсторияСтатусовРасходныхОрдеровСрезПоследних.Период
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ КАК ДатаОтгрузки,
	|	гф_ИсторияСтатусовРасходныхОрдеровСрезПоследних.СтатусИзменил КАК РОСтатусИзменил
	|ПОМЕСТИТЬ ВТ_ИсторияСтатусов
	|ИЗ
	...
	|	ЕСТЬNULL(ВТ_РТУ.КоличествоКоробов, ВТ_РО.КоличествоКоробов) КАК КоличествоКоробов,
	...
	|	ЕСТЬNULL(ВТ_ИсторияСтатусов.ДатаОтгрузки, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаОтгрузки,
	|	ВТ_РО.СозданаЗаявкаВ_ТК КАК СозданаЗаявкаВ_ТК
	|ИЗ
	...
	|		И ВТ_Документы.РасходныйОрдер  = ВТ_РТУ.РасходныйОрдер  
	...
	|ГДЕ
	|	НЕ ЕСТЬNULL(ВТ_РО.РасходныйОрдер, ЗНАЧЕНИЕ(Документ.РасходныйОрдерНаТовары.ПустаяСсылка)) = ЗНАЧЕНИЕ(Документ.РасходныйОрдерНаТовары.ПустаяСсылка)";
	Возврат Результат;
...
Функция ПолучитьТекстЗапросаПеремещение()
	...
	|ПОМЕСТИТЬ ВТ_ПеремещениеТоваровТовары
	|ИЗ
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПеремещениеТоваров.Ссылка КАК Ссылка,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПеремещениеТоваровТоварыВКоробах.УпаковочныйЛист) КАК КоличествоКоробов
	|ПОМЕСТИТЬ ВТ_ПеремещениеТоваровТоварыВКоробах
	|ИЗ
	|	Документ.ПеремещениеТоваров КАК ПеремещениеТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПеремещениеТоваров.гф_ТоварыВКоробах КАК ПеремещениеТоваровТоварыВКоробах
	|		ПО ПеремещениеТоваров.Ссылка = ПеремещениеТоваровТоварыВКоробах.Ссылка
	|ГДЕ
	|	ПеремещениеТоваров.Проведен
	|	И ПеремещениеТоваров.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И (ПеремещениеТоваров.Организация = &Организация
	...
	|	
	|	ПеремещениеТоваровТовары.КоличествоПар КАК КоличествоПар,
	|	ПеремещениеТоваровТоварыВКоробах.КоличествоКоробов КАК КоличествоКоробов,
	|	
	...
	|ИЗ
	|	Документ.ПеремещениеТоваров КАК ПеремещениеТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПеремещениеТоваровТовары КАК ПеремещениеТоваровТовары
	|		ПО ПеремещениеТоваров.Ссылка = ПеремещениеТоваровТовары.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПеремещениеТоваровТоварыВКоробах КАК ПеремещениеТоваровТоварыВКоробах
	|		ПО ПеремещениеТоваров.Ссылка = ПеремещениеТоваровТоварыВКоробах.Ссылка
	|ГДЕ
	|	ПеремещениеТоваров.Проведен
	|	И ПеремещениеТоваров.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	// ++ Галфинд_ДомнышеваКР_12_02_2024
	// Не подтягивались перемещения по которым только ПО без РО
	|	ВЫБОР
	|		КОГДА Подзапрос.Документ <> ЗНАЧЕНИЕ(Документ.ПеремещениеТоваров.ПустаяСсылка)
	|			ТОГДА Подзапрос.Документ
	|		ИНАЧЕ ВТ_ПО.Распоряжение
	|	КОНЕЦ КАК Документ,
	//|	Подзапрос.Документ КАК Документ,
	// -- Галфинд_ДомнышеваКР_12_02_2024
	|	Подзапрос.РасходныйОрдер КАК РасходныйОрдер,
...
Процедура ЗаполнитьСписокВыбораТиповДокументов()
	...
	// vvv Галфинд \ Sakovich 01.04.2024
	ВнутреннееПотребление = Метаданные.Документы.ВнутреннееПотребление;
	Элементы.ТипДокумента.СписокВыбора.Добавить(ВнутреннееПотребление.Имя,  ВнутреннееПотребление.Синоним);
	// ^^^ Галфинд \ Sakovich 01.04.2024
...
Процедура ДобавитьОтмеченныеСтрокиВМассивДокументов(МассивДокументов, ИмяРаздела, ПрО)
	...
	Для Каждого ОтмеченнаяСтрока Из МассивОтмеченныхСтрок Цикл
		...
		Если ПрО = Истина Тогда
			...
			Иначе
				Если ИмяРаздела <> "Перемещение" Тогда // Галфинд_ДомнышеваКР_15_02_2024
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
&НаСервере
Процедура ВыгрузитьРеестрОтгрузкиНаСервере(ТабДок, Сформирован)
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	Макет = ОбработкаОбъект.ПолучитьМакет("Макет");
	тз_Отгрузка = Объект.Отгрузка.Выгрузить();
	мОтмеченныхСтрок = тз_Отгрузка.НайтиСтроки(Новый Структура("Флаг", Истина));
	Если мОтмеченныхСтрок.ВГраница() = -1 Тогда
		ОбщегоНазначения.СообщитьПользователю("Нет данных для выгрузки!");
		Возврат;
	КонецЕсли;
	...
	НПП = 0;
	тзОтмеченных = тз_Отгрузка.Скопировать(мОтмеченныхСтрок);
	мСкладов = тзОтмеченных.ВыгрузитьКолонку("Склад");
	мОрдеров = тзОтмеченных.ВыгрузитьКолонку("РасходныйОрдер");
	
	Запрос = Новый Запрос();
	
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	тчСклдыДопРекв.Значение КАК СкладКодКлиента,
	|	ВЫРАЗИТЬ(тчОрдерТоварыПоРасп.Распоряжение КАК Документ.ЗаказКлиента).гф_АдресДоставки КАК АдресДоставки,
	|	ВЫРАЗИТЬ(тчОрдерТоварыПоРасп.Распоряжение КАК Документ.ЗаказКлиента).гф_АдресДоставки.ТК КАК ТК,
	|	тчОрдерТоварыПоРасп.Ссылка КАК РасходныйОрдер,
	|	тчОрдерТоварыПоРасп.Ссылка.Получатель КАК Контрагент
	// Галфинд_ДомнышеваКР_11_04_2024_сверн. во врем таб
	|ПОМЕСТИТЬ ТЧЗаказы

	|ИЗ
	|	Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК тчОрдерТоварыПоРасп
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады.ДополнительныеРеквизиты КАК тчСклдыДопРекв
	|		ПО тчОрдерТоварыПоРасп.Ссылка.Склад = тчСклдыДопРекв.Ссылка
	|			И (тчСклдыДопРекв.Свойство.Имя = ""гф_СкладыКодКлиентаСклад"")
	|ГДЕ
	|	тчОрдерТоварыПоРасп.Ссылка В(&РасходныйОрдер)
	|	И тчОрдерТоварыПоРасп.НомерСтроки = 1
	// ++ Галфинд_ДомнышеваКР_11_04_2024
	|
    |ОБЪЕДИНИТЬ ВСЕ
    |
    |ВЫБРАТЬ
    |	тчСклдыДопРекв.Значение,
    |	ВЫРАЗИТЬ(тчОрдерТоварыПоРасп.Распоряжение КАК Документ.ЗаказНаВнутреннееПотребление).гф_АдресДоставки,
    |	ВЫРАЗИТЬ(тчОрдерТоварыПоРасп.Распоряжение КАК Документ.ЗаказНаВнутреннееПотребление).гф_АдресДоставки.ТК,
    |	тчОрдерТоварыПоРасп.Ссылка,
    |	тчОрдерТоварыПоРасп.Ссылка.Получатель
    |ИЗ
    |	Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК тчОрдерТоварыПоРасп
    |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады.ДополнительныеРеквизиты КАК тчСклдыДопРекв
    |		ПО тчОрдерТоварыПоРасп.Ссылка.Склад = тчСклдыДопРекв.Ссылка
    ...
    |////////////////////////////////////////////////////////////////////////////////
    |ВЫБРАТЬ
    |	ТЧЗаказы.СкладКодКлиента КАК СкладКодКлиента,
    |	ТЧЗаказы.АдресДоставки КАК АдресДоставки,
    |	ТЧЗаказы.ТК КАК ТК,
    |	ТЧЗаказы.РасходныйОрдер КАК РасходныйОрдер,
    |	ТЧЗаказы.Контрагент КАК Контрагент
    |ИЗ
    |	ТЧЗаказы КАК ТЧЗаказы
    |ГДЕ
    |	НЕ ТЧЗаказы.АдресДоставки ЕСТЬ NULL
	// -- Галфинд_ДомнышеваКР_11_04_2024
	|";
	
	Запрос.УстановитьПараметр("Склад", мСкладов);
	Запрос.УстановитьПараметр("РасходныйОрдер", мОрдеров);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	
	Для каждого Стр Из тзОтмеченных Цикл
		НПП = НПП + 1;
		СтруктураЗаполнения = Новый Структура(
		"НомерПП, РО, Склад, НомерРО, Автор, Контрагент, АдресДоставки, ТК, НомерРТУ, РасходныйОрдер",
		НПП, 
		Стр["НомерРО"],
		Стр["Склад"],
		Стр["НомерРО"], 
		...
		ЗаполнитьЗначенияСвойств(ОбластьСтрока.Параметры, СтруктураЗаполнения);
		Выборка.Сбросить();
		Если Выборка.НайтиСледующий(Стр.РасходныйОрдер, "РасходныйОрдер") Тогда
			Если Не ЗначениеЗаполнено(Стр["Документ"]) Тогда
				ЗаполнитьЗначенияСвойств(ОбластьСтрока.Параметры, Выборка);
			Иначе
				ЗаполнитьЗначенияСвойств(ОбластьСтрока.Параметры, Выборка, "СкладКодКлиента");
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура ВыгрузитьРеестрОтгрузки(Команда)
	ТабДок = Новый ТабличныйДокумент;
	Сформирован = Ложь;
	ВыгрузитьРеестрОтгрузкиНаСервере(ТабДок, Сформирован);
	Если Сформирован Тогда
		...
		ТабДок.Показать("Реестр отгрузки");
		
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ИзменитьРО(Команда)
	
	// ++ Галфинд ВолковЕВ 2024/03/18
	Отказ = Ложь;
	
	Если Не ПроверитьРольДляОтменыРО() Тогда
		Возврат;
	КонецЕсли;	
	...
	Если Не ОтмеченныеСтроки.Количество() Тогда
		ТекстСообщения = НСтр("ru = 'Нет отмеченных строк! '");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	
	Если ОтмеченныеСтроки.Количество() > 1 Тогда
		ТекстСообщения = НСтр("ru = 'Отмечено несколько строк!
		| Для выполнения отмены отгрузки необходимо выбирать только одну строку! '");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	
	МассивДокументовРасходныйОрдер = Новый Массив;
	
	Для Каждого СтрокаМассива Из ОтмеченныеСтроки Цикл
		МассивДокументовРасходныйОрдер.Добавить(СтрокаМассива.РасходныйОрдер);
	КонецЦикла;
	
	ПроверитьНаличиеДокументовРеализации(Отказ, МассивДокументовРасходныйОрдер);
	
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	
	//ОписаниеОповещения = Новый ОписаниеОповещения("ИзменитьРОПродолжение", ЭтотОбъект, ОтмеченныеСтроки);
	//
	////Значение = ПредопределенноеЗначение("Перечисление.СтатусыРасходныхОрдеров.ВПроцессеПроверки");
	////
	//ПоказатьВводЗначения(ОписаниеОповещения, Значение, "Выберите статус РО");
	...
	Оповещение = Новый ОписаниеОповещения("ИзменитьРОПродолжение", ЭтотОбъект, ОтмеченныеСтроки);
	...
	Если ИмяФормы = "ВнешняяОбработка.гф_МониторЛогиста.Форма.Форма" Тогда
		ОткрытьФорму("ВнешняяОбработка.гф_МониторЛогиста.Форма.ФормаВопросаИзменитьРО", ПараметрыОткрытия,
		ЭтотОбъект, , , , Оповещение);
	Иначе
		ОткрытьФорму("Обработка.гф_МониторЛогиста.Форма.ФормаВопросаИзменитьРО", ПараметрыОткрытия,
		ЭтотОбъект, , , , Оповещение);
	КонецЕсли;
	// -- Галфинд ВолковЕВ 2024/03/18
...
Процедура ПроверитьНаличиеДокументовРеализации(Отказ, МассивДокументовРасходныйОрдер)
	...
	|ГДЕ
	|	РеализацияТоваровУслуг.Проведен
	|	И РеализацияТоваровУслуг.гф_РасходныйОрдер В (&гф_РасходныйОрдер)";
	
	Запрос.УстановитьПараметр("гф_РасходныйОрдер", МассивДокументовРасходныйОрдер);
	...
	НаличиеРеализацияТоваровУслуг = Ложь;
	НаличиеСчетФактураВыданный = Ложь;
	
	//Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
	Для Каждого Строка Из ВыборкаДетальныеЗаписи Цикл
		//ДокументРеализация.Добавить(ВыборкаДетальныеЗаписи.РеализацияТоваровУслуг);
		ДокументРеализация.Добавить(Строка.РеализацияТоваровУслуг);
		
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Существует проведенная ""%1"" для ""%2""
		| Необходимо распровести документ перед выполнением операции по отмене отгрузки.'"),
		//ВыборкаДетальныеЗаписи.РеализацияТоваровУслуг, ВыборкаДетальныеЗаписи.РасходныйОрдер);
		Строка.РеализацияТоваровУслуг, Строка.РасходныйОрдер);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		НаличиеРеализацияТоваровУслуг = Истина;
	КонецЦикла;
	...
	|	И СчетФактураВыданный.ДокументОснование В ИЕРАРХИИ(&ДокументОснование)";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Существует проведенная ""%1"" для ""%2""
		| Необходимо распровести документ перед выполнением операции по отмене отгрузки.'"),
		ВыборкаДетальныеЗаписи.СчетФактураВыданный, ВыборкаДетальныеЗаписи.ДокументОснование);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		НаличиеСчетФактураВыданный = Истина;
	КонецЦикла;
	
	Если НаличиеРеализацияТоваровУслуг Или НаличиеСчетФактураВыданный Тогда
		Отказ = Истина;
	КонецЕсли;
	// -- Галфинд ВолковЕВ 2024/03/18
...
&НаКлиенте
Процедура ИзменитьРОПродолжение(СтруктураПереданныхДанных, СтруктураПараметров) Экспорт
	...
	Если Не СтруктураПереданныхДанных = Неопределено Тогда
		...
		Если Действие = "ПродолжитьВыполнение" Тогда
			
			Для Каждого Строка Из СтруктураПараметров Цикл
				ИзменитьРОПродолжениеНаСервере(Строка.РасходныйОрдер);
			КонецЦикла;
...
Процедура ИзменитьРОПродолжениеНаСервере(РасходныйОрдер)
	
	// ++ Галфинд ВолковЕВ 2024/03/18
	ОбъектРасходныйОрдер = РасходныйОрдер.ПолучитьОбъект();
	
	НачатьТранзакцию();
	Перепроведение = Ложь;
	Попытка
		ОбъектРасходныйОрдер.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = '%1 находится в процессе редактирования и не может быть изменен.'"), ОбъектРасходныйОрдер);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецПопытки;
	
	//Для Каждого СтрокаТ Из ОбъектРасходныйОрдер.ОтгружаемыеТовары Цикл
	
	
	ТабЗнач = ОбъектРасходныйОрдер.ОтгружаемыеТовары.Выгрузить();
	ТабЗнач.ЗаполнитьЗначения(Перечисления.ДействияСоСтрокамиОрдеровНаОтгрузку.НеОтгружать, "Действие");
	//ТабЗнач.ЗаполнитьЗначения("", "гф_КодСтрокиДополнительныеСведения");
	ОбъектРасходныйОрдер.ОтгружаемыеТовары.Загрузить(ТабЗнач);
	ОбъектРасходныйОрдер.ТоварыПоРаспоряжениям.Очистить();
	
	ОбъектРасходныйОрдер.Статус = Перечисления.СтатусыРасходныхОрдеров.Отменен;
	...
	Попытка
		...
		Если Не Отладка Тогда
			ОбъектРасходныйОрдер.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
		КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Ордер не изменен ""%1"".'"),
		РасходныйОрдер);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция ПроверитьРольДляОтменыРО()
	
	// ++ Галфинд ВолковЕВ 2024/03/18
	Если (Не ПроверкаРоли("гф_ОтменаНеОтгруженногоОрдера") 
		Или Не ПроверкаРоли("гф_ОтменаОтгруженногоОрдера"))
		И Не ПроверкаРоли("ПолныеПрава") Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'На отмену отгрузки требуюся соответствующие права.'"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат Ложь;
	КонецЕсли;
...
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeb125b67e066b
&НаСервере
Процедура ОграничитьФункционал()
	Элементы.ГруппаСтраницаПоставка.Видимость = Ложь;
	Элементы.ГруппаСтраницаНастройки.Видимость = Ложь;
	Элементы.ГруппаСтраницаПочтаРоссии.Видимость = Ложь;
	Элементы.ФормаВыгрузитьWMS.Видимость = Ложь;
	Элементы.ФормаЗагрузитьWMS.Видимость = Ложь;
	Элементы.ПеремещениеСоздатьПО.Видимость = Ложь;
	Элементы.ИзменитьСтатусыРО.Видимость = Ложь;
	Элементы.ФормаПротоколОбмена.Видимость = Ложь; // добавлено Галфинд_ДомнышеваКР_29_01_2024
КонецПроцедуры
// -- Галфинд СадомцевСА 16.01.2024
...
Процедура ПротоколОбмена(Команда)
	...
	ОтборОтмеченных.Вставить("Флаг", Истина);
	
	МассивОтмеченныхСтрок = Объект["Отгрузка"].НайтиСтроки(ОтборОтмеченных);
	
	Для каждого ОтмеченнаяСтрока ИЗ МассивОтмеченныхСтрок Цикл
			МассивДокументов.Добавить(ОтмеченнаяСтрока.РасходныйОрдер);
	КонецЦикла;
	
	МассивОтмеченныхСтрок = Объект["Перемещение"].НайтиСтроки(ОтборОтмеченных);
	...
	МассивОтмеченныхСтрок = Объект["Поставка"].НайтиСтроки(ОтборОтмеченных);
	...
	Если Не МассивДокументов.Количество() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не найдено отмеченных строк для получения данных");
	...
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Объект", МассивДокументов);
	
	Форма = ПолучитьФорму("РегистрСведений.гф_ОшибкиЗагрузки.ФормаСписка", ПараметрыОткрытия);
	Форма.Открыть();
	// Галфинд_ДомнышеваКР_29_01_2024
