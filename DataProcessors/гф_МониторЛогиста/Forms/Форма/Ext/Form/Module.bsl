...
Функция ПолучитьТекстЗапросаПоставка()
	...
	// ++Галфинд_ДомнышеваКР_01_06_2023
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8ea8b083fed1320811ee0061978c902e
	// Документ ВозвратТоваровОтКлиента заменен на ЗаявкаНаВозвратТоваровОтКлиента,
	// а ТЧ Товары заменена на ТЧ ВозвращаемыеТовары.
	|	ЗаявкаНаВозвратТоваровОтКлиента.Дата КАК Дата,
	|	ЗаявкаНаВозвратТоваровОтКлиента.Номер КАК Номер,
	|	ТИПЗНАЧЕНИЯ(ЗаявкаНаВозвратТоваровОтКлиента.Ссылка) КАК Поле1,
	|	ЗаявкаНаВозвратТоваровОтКлиента.Ссылка КАК Ссылка,
	|	СУММА(ЗаявкаНаВозвратТоваровОтКлиентаТовары.СуммаСНДС) КАК СуммаСНДС,
	...
	|	СУММА(0) КАК Поле2,
	|	СУММА(ЗаявкаНаВозвратТоваровОтКлиентаТовары.Количество) КАК Количество,
	|	МаркировкаТоваровИСМП.Ссылка КАК Ссылка2,
	...
	|	ЗаявкаНаВозвратТоваровОтКлиента.ДатаПоступления КАК ДатаПоступления,
	|	ПОДСТРОКА(ЗаявкаНаВозвратТоваровОтКлиента.Комментарий, 1, 100) КАК Поле3,
	|	ВЫБОР
	|		КОГДА Контрагенты.ОбособленноеПодразделение
	|			ТОГДА ""Обособленное подразделение""
	|		ИНАЧЕ Контрагенты.ЮридическоеФизическоеЛицо
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.гф_ИсторияСтатусовПриходныхОрдеров.СрезПоследних(ДАТАВРЕМЯ(3000, 1, 1), ) КАК гф_ИсторияСтатусовПриходныхОрдеровСрезПоследних
...
Функция ПолучитьТекстЗапросаОтгрузка()
	...
	|ИЗ
	|	ВТ_РТУ КАК ВТ_РТУ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.гф_ПочтаРоссииИсторияОтслеживания.СрезПоследних(ДАТАВРЕМЯ(3000, 1, 1), ) КАК гф_ПочтаРоссииИсторияОтслеживанияСрезПоследних
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.гф_ИсторияСтатусовРасходныхОрдеров.СрезПоследних(ДАТАВРЕМЯ(3000, 1, 1), ) КАК гф_ИсторияСтатусовРасходныхОрдеровСрезПоследних
