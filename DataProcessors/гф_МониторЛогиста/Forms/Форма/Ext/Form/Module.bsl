...
Процедура ВыгрузитьРеестрОтгрузкиНаСервере(ТабДок, Сформирован)
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	тчСклдыДопРекв.Значение КАК КодКлиента,
	...
	Для каждого Стр Из тзОтмеченных Цикл
		...
		"НомерПП, РО, Склад, СкладКодКлиента, НомерРО, Автор, Контрагент, АдресДоставки, ТК, НомерРТУ, РасходныйОрдер",
		НПП, Стр["НомерРО"], Стр["Склад"], 
		СокрЛП(Стр["КодКлиента"]), 
		Стр["НомерРО"], Стр["Ответственный"],
		Стр["Контрагент"], 
		Стр["АдресДоставки"], 
		Стр["ТК"],
		Стр["Номер"],
