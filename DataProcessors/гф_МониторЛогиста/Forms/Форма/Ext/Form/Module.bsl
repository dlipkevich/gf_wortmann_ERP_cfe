...
Функция ПолучитьТекстЗапросаОтгрузка()
	...
	Результат = "ВЫБРАТЬ
	            ...
	            |	КОЛИЧЕСТВО(РеализацияТоваровУслуггф_ТоварыВКоробах1.НомерСтроки) КАК КоличествоКоробов,
	            |	Заказ.Ссылка КАК ЗаказКлиента,
	            |	РеализацияТоваровУслуг.ПеревозчикПартнер КАК ТК,
	            |	ВЫБОР
	            |		КОГДА МАКСИМУМ(ЗаказКлиентаТовары.ДатаОтгрузки) < НАЧАЛОПЕРИОДА(РеализацияТоваровУслуг.Дата, ДЕНЬ)
	            |			ТОГДА РАЗНОСТЬДАТ(МАКСИМУМ(ЗаказКлиентаТовары.ДатаОтгрузки), РеализацияТоваровУслуг.Дата, ДЕНЬ)
	            |		ИНАЧЕ 0
	            ...
	            |	гф_АдресаДоставкиСправочник.Адрес КАК АдресДоставки,
	            ...
	            |			ПО Заказ.АдресДоставки = гф_АдресаДоставкиСправочник.Адрес
	            |		ПО РеализацияТоваровУслуг.ЗаказКлиента = Заказ.Ссылка
	            |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.гф_ТоварыВКоробах КАК РеализацияТоваровУслуггф_ТоварыВКоробах1
	            |		ПО РеализацияТоваровУслуг.Ссылка = РеализацияТоваровУслуггф_ТоварыВКоробах1.Ссылка
	            ...
	            |	гф_АдресаДоставкиСправочник.Адрес,
	            ...
	            |	ЕСТЬNULL(гф_ИсторияСтатусовРасходныхОрдеровСрезПоследних.Загружен, ЛОЖЬ) КАК РОЗагружен,
	            |	ВЫБОР
	            |		КОГДА ВТ_РО.РасходныйОрдер ЕСТЬ NULL
	            |			ТОГДА """"
	            |		КОГДА ЕСТЬNULL(гф_ИсторияСтатусовРасходныхОрдеровСрезПоследних.Загружен, ЛОЖЬ)
	            |			ТОГДА ""Обмен с WMS""
	            |		ИНАЧЕ ""Отгрузил""
	            |	КОНЕЦ КАК Отгрузил
	            |ИЗ
