...
Функция ПолучитьТекстЗапросаОтгрузка()
	...
	|ГДЕ
	// ++ Галфинд ВолковЕВ 2024/01/13
	// |	РеализацияТоваровУслуг.Проведен
	// |	И РеализацияТоваровУслуг.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	(РеализацияТоваровУслуг.Проведен 
	|	Или Не РеализацияТоваровУслуг.гф_РасходныйОрдер = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка))
	// -- Галфинд ВолковЕВ 2024/01/13
	|	И РеализацияТоваровУслуг.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
...
Процедура ВыгрузитьРеестрОтгрузкиНаСервере(ТабДок, Сформирован)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕНЫЕ
