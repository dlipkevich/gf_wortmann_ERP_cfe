...
// не позволянт разбить их на отдельные тексты.
//
...
Функция ПолучитьТекстЗапросаПоставка()
	...
	|ИЗ
	|	ВТ_ПО КАК ВТ_ПО
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.гф_ИсторияСтатусовПриходныхОрдеров.СрезПоследних(
	|				&КонецВремен,
	|				Объект В
	|					(ВЫБРАТЬ
	|						Т.ПриходныйОрдер
	...
	|		ПО ВТ_ПО.ПриходныйОрдер = гф_ИсторияСтатусовПриходныхОрдеровСрезПоследних.Объект";
...
Функция ПолучитьТекстЗапросаОтгрузка()
	...
	|ИЗ
	|	ВТ_РТУ КАК ВТ_РТУ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.гф_ПочтаРоссииИсторияОтслеживания.СрезПоследних(
	|				&КонецВремен,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.гф_ИсторияСтатусовРасходныхОрдеров.СрезПоследних(
	|				&КонецВремен,
	|				Объект В
	|					(ВЫБРАТЬ
	|						Т.РасходныйОрдер
...
Функция ПолучитьТекстЗапросаПеремещение()
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.гф_ПочтаРоссииИсторияОтслеживания.СрезПоследних(
	|		&КонецВремен, 
	|		ПочтовоеОтправление В (ВЫБРАТЬ Т.гф_ПочтовоеОтправление ИЗ ВТ_Перемещение КАК Т)
	|		) КАК гф_ПочтаРоссииИсторияОтслеживанияСрезПоследних
...
Процедура ПроверитьСтатусДокумента(Документ)
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.РасходныйОрдерНаТовары") Тогда
		...
		Если НЕ Документ.Статус = Перечисления.СтатусыРасходныхОрдеров.КОтбору Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("У документа " + Документ + " статус НЕ ""К отбору""");
	...
	Иначе
		...
		Если НЕ Документ.Статус = Перечисления.СтатусыПриходныхОрдеров.КПоступлению Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("У документа " + Документ + " статус НЕ ""К поступлению""");
