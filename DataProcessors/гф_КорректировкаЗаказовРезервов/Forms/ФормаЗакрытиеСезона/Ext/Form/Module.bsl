...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	ПричинаОтмены = Справочники.ПричиныОтменыЗаказовКлиентов.НайтиПоНаименованию("Закрытие сезона");
	ЗаполнитьТаблицаЗаказы();
	ЭтотОбъект.АвтоЗаголовок = Ложь;
	ЭтотОбъект.Заголовок = "Закрытие сезона";
КонецПроцедуры
...
Процедура ЗаполнитьТаблицаЗаказы()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	И ЗаказКлиентаТовары.Ссылка.Договор.гф_Сезон = &гф_Сезон";
	...
	Если Не ЗначениеЗаполнено(Параметры.Склад) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЗаказКлиентаТовары.Ссылка.Склад = &Склад", "ИСТИНА");
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Параметры.Контрагент) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЗаказКлиентаТовары.Ссылка.Контрагент = &Контрагент", "ИСТИНА");
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Параметры.Сезон) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЗаказКлиентаТовары.Ссылка.Договор.гф_Сезон = &гф_Сезон", "ИСТИНА");
	КонецЕсли;
