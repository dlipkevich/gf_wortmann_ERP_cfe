...
Процедура ЗакрытьЗаказ(ЗаказКлиента)
	...
	КонецЦикла;
	Для Каждого СтрокаТЧ Из ДокОбъект.гф_ТоварыВКоробах Цикл
		Если СтрокаТЧ["ВариантОбеспечения"] = Перечисления.ВариантыОбеспечения.Отгрузить Тогда
			Продолжить;
		КонецЕсли;
		СтрокаТЧ["Отменено"] = Истина;
		СтрокаТЧ["ПричинаОтмены"] = ПричинаОтмены;
	КонецЦикла;
	ДокОбъект.ДополнительныеСвойства.Вставить("гф_ОбработкаОтгрузкаПоЗаказам", Истина);
...
Процедура СнятьРезервыПоЗаказу(ЗаказКлиента)
	...
	ОтобратьОстаткиПоЗаказу(ТаблицаОстатки, Назначение);
	ТаблицаОстатки.Свернуть("Склад, Номенклатура, Характеристика", "Доступно");
	ТоварыВКоробах = ВернутьТоварыВКоробах(ЗаказКлиента);
	Если ТоварыВКоробах Тогда
		ЗаполнитьIDкоробаОстаткиПоЗаказу(ТаблицаОстатки, ЗаказКлиента);
	КонецЕсли;
...
Процедура ОсвободитьУпаковочныеЛисты(ЗаказКлиента)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	               ...
	               |ГДЕ
	               |	УпаковочныйЛист.гф_Заказ = &ЗаказКлиента
...
&НаСервереБезКонтекста
Процедура ОтобратьОстаткиПоЗаказу(ТаблицаОстатки, Назначение)
...
&НаСервереБезКонтекста
Процедура ЗаполнитьIDкоробаОстаткиПоЗаказу(ТаблицаОстатки, ЗаказКлиента)
	УпаковочныеЛисты = НайтиУпаковочныеЛисты(ЗаказКлиента);
	...
	Если УпаковочныеЛисты.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	// здесь мы имеем таблицу значений УпаковочныеЛисты
	ТаблицаОстатки.Колонки.Добавить("гф_IDкороба");
	ТаблицаОстаткиУЛ = ТаблицаОстатки.СкопироватьКолонки();
	Для Каждого СтрокаУЛ Из УпаковочныеЛисты Цикл
		УпаковочныйЛист = СтрокаУЛ.УпаковочныйЛист;
		ДобавитьСтрокиУЛ(ТаблицаОстатки, ТаблицаОстаткиУЛ, СтрокаУЛ);
	КонецЦикла;
	// теперь перезаполним таблицу остатков с учетом Упаковочных листов
	ОчиститьТаблицаОстатки(ТаблицаОстатки);
	// здесь в таблице остались только остатки для которых не найден упаковочный лист
	ЗаполнитьТаблицаОстатки(ТаблицаОстатки, ТаблицаОстаткиУЛ, УпаковочныйЛист);
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ЗаполнитьТаблицаОстатки(ТаблицаОстатки, ТаблицаОстаткиУЛ, УпаковочныйЛист)
	Для Каждого СтрокаТЗ Из ТаблицаОстатки Цикл
		СтрокаТЗ.гф_IDкороба = УпаковочныйЛист;
	КонецЦикла;
	Для Каждого СтрокаТЗ Из ТаблицаОстаткиУЛ Цикл
		нс = ТаблицаОстатки.Добавить();
		ЗаполнитьЗначенияСвойств(нс, СтрокаТЗ);
	КонецЦикла;
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ОчиститьТаблицаОстатки(ТаблицаОстатки)
	МассивУдалитьСтроки = Новый Массив;
	Для Каждого СтрокаТЗ Из ТаблицаОстатки Цикл
		Если Не ЗначениеЗаполнено(СтрокаТЗ.Доступно) Тогда
			МассивУдалитьСтроки.Добавить(СтрокаТЗ);
		КонецЕсли;
	КонецЦикла;
	Для Каждого УдалитьСтроку Из МассивУдалитьСтроки Цикл
...
&НаСервереБезКонтекста
Процедура ДобавитьСтрокиУЛ(ТаблицаОстатки, ТаблицаОстаткиУЛ, СтрокаУЛ)
	Для Каждого СтрокаТЧ Из СтрокаУЛ.УпаковочныйЛист.Товары Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если НайденнаяСтрока.Доступно <= 0 Тогда
				Продолжить;
			КонецЕсли;
			// здесь найдена строка с остатками по Номенклатуре/Характеристике из Упаковочного листа
			Доступно = Мин(НайденнаяСтрока.Доступно, СтрокаТЧ.КоличествоУпаковок);
			Если Доступно > 0 Тогда
				нс = ТаблицаОстаткиУЛ.Добавить();
				ЗаполнитьЗначенияСвойств(нс, НайденнаяСтрока);
				нс.Доступно = Доступно;
				нс.гф_IDкороба = СтрокаУЛ.УпаковочныйЛист;
				НайденнаяСтрока.Доступно = НайденнаяСтрока.Доступно - Доступно;
			КонецЕсли;	
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
&НаСервереБезКонтекста
Функция НайтиУпаковочныеЛисты(ЗаказКлиента)
	...
	Если ЗначениеЗаполнено(ЗаказКлиента) Тогда
		...
		Запрос.УстановитьПараметр("гф_Заказ", ЗаказКлиента);
		Результат = Запрос.Выполнить();
