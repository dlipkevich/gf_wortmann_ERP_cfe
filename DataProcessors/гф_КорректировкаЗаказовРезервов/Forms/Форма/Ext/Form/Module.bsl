...
Процедура ЗаполнитьДанныеЗаказаНаСервере()
	...
	Если	РежимРаботы = 1
		...
		Если НомерВерсииЗаказа = Неопределено Тогда
			СообщитьПользователю("Для Заказа:" + мЗаказКлиента + " НЕТ сохраненных ранее версий объекта. Заполнение не возможно!");
			Возврат;
		КонецЕсли;
		
		ВерсияОбъекта  = ВерсионированиеОбъектов.РазборВерсии(мЗаказКлиента, НомерВерсииЗаказа);
		Если ТипЗнч(ВерсияОбъекта) = Тип("Структура") Тогда
			Для Каждого СтрокаТЧ Из ВерсияОбъекта.ТабличныеЧасти["Товары"] Цикл
				нс = Объект.Резервы.Добавить();
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокиЗаказа = Объект.Резервы.НайтиСтроки(СтруктураПоиска);
			Если СтрокиЗаказа.Количество() > 0 Тогда
				СтрокаЗаказа = СтрокиЗаказа[0];
			Иначе
				СтрокаЗаказа = Объект.Резервы.Добавить();
			КонецЕсли;
		...
		Для Каждого СтрокаЗаказа Из Объект.Резервы Цикл
	...
	Иначе	
		//здесь надо получить первоначальные данные заказа из регистра сведений ВерсииОбъектов
		ВерсияОбъекта  = ВерсионированиеОбъектов.РазборВерсии(мЗаказКлиента, 1);
		Если ТипЗнч(ВерсияОбъекта) = Тип("Структура") Тогда
			Для Каждого СтрокаТЧ Из ВерсияОбъекта.ТабличныеЧасти["Товары"] Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если СтрокиЗаказа.Количество() > 0 Тогда
				СтрокаЗаказа = СтрокиЗаказа[0];
			Иначе
				СтрокаЗаказа = Объект.Товары.Добавить();
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(СтрокаЗаказа, Выборка);
			СтрокаЗаказа.КоличествоСИзменениями = Выборка.Количество;
			Если СтрокаЗаказа.Отменено Тогда
		...
		Для Каждого СтрокаЗаказа Из Объект.Товары Цикл
			Если СтрокаЗаказа.КоличествоСИзменениями > СтрокаЗаказа.КоличествоПервоначально Тогда
				СтрокаЗаказа.Добавлено = СтрокаЗаказа.КоличествоСИзменениями - СтрокаЗаказа.КоличествоПервоначально;
			Иначе
				СтрокаЗаказа.Удалено = СтрокаЗаказа.КоличествоПервоначально - СтрокаЗаказа.КоличествоСИзменениями;
...
Процедура ПересчитатьИтогПоСтроке()
	...
	Если ТД <> Неопределено Тогда
		...
		Если ТД.Количество < 0 Тогда
			СообщитьПользователю("Отрицательный предварительный итог в строке заказа:" + ТД.НомерСтроки);
		КонецЕсли;
	...
	Если РежимРаботы = 1 Тогда
		...
		Для Каждого СтрокаТЧ Из Объект.Резервы Цикл
			...
			Если ТекстОшибки <> "" Тогда
				СообщитьПользователю(ТекстОшибки + " в строке:" + СтрокаТЧ["НомерСтроки"]);
				флЕстьОшибкиЗаполнения = Истина;
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли флТоварыВКоробах Тогда
...
Процедура Сохранить(Команда)
	...
	Если флЕстьОшибкиЗаполнения Тогда
		//Если РежимРаботы = 1 Тогда
			Возврат;
		//Иначе
		//	ОписаниеОповещения = Новый ОписаниеОповещения("ВопросСохранитьЗаказ", ЭтотОбъект);
		//	ПоказатьВопрос(ОписаниеОповещения, "Найдены ошибки заполнения. Продолжить сохранение заказа?", РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
		//КонецЕсли;
	Иначе
		ВыполнитьСохранениеЗаказа();
	КонецЕсли;
...
Процедура ЗаписатьЗаказНаСервере()
	...
	Если РежимРаботы = 1 Тогда
		...
		Если Объект.Резервы.Итог("Зарезервировать") > 0 Тогда
			...
			ДокРезервирование.Комментарий = "Создан автоматически обработкой Корректировка заказов и резервов " + ТекущаяДата();
			...
			Если Не ЗначениеЗаполнено(ДокРезервирование.Назначение) Тогда
				...
				ДокРезервирование = Неопределено;
				СообщитьПользователю("Не удалось получить Назначение для Заказа:" + мЗаказКлиента);
			Иначе
		...
		Если Объект.Резервы.Итог("СнятьРезерв") > 0 Тогда
			...
			ДокСнятиеРезерва.Комментарий = "Создан автоматически обработкой Корректировка заказов и резервов " + ТекущаяДата();
	...
	Попытка
		...
		СообщитьПользователю("Проведен:" + ЗаказОбъект.Ссылка);
		...
		ЗаполнитьДанныеЗаказаНаСервере();
	Исключение
		СообщитьПользователю("Ошибка при записи:" + ОписаниеОшибки());
	КонецПопытки;
...
Процедура ЗаполнитьПричинуДобавления(Команда)
	...
	ОткрытьФорму("Справочник.ПричиныОтменыЗаказовКлиентов.Форма.ФормаВыбора",ПараметрыФормы,ЭтаФорма,,,,Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ЗагрузитьИзExcel(Команда)
	Оповещение = Новый ОписаниеОповещения("ЗагрузкаИзExcelЗавершение", ЭтотОбъект);
	ПараметрыФормы = Новый Структура("флТоварыВКоробах", флТоварыВКоробах);
	Если ЭтаФорма.ИмяФормы = "ВнешняяОбработка.гф_КорректировкаЗаказовРезервов.Форма.Форма" Тогда
		ОткрытьФорму("ВнешняяОбработка.гф_КорректировкаЗаказовРезервов.Форма.ФормаЗагрузкиИзЕксель", ПараметрыФормы, ЭтотОбъект,,,,Оповещение);
	Иначе
		ОткрытьФорму("Обработка.гф_КорректировкаЗаказовРезервов.Форма.ФормаЗагрузкиИзЕксель", ПараметрыФормы, ЭтотОбъект,,,,Оповещение);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузкаИзExcelЗавершение(Результат, ДополнительныеПараметры) Экспорт
		...
		Для Каждого Элемент Из Результат Цикл
			Если флТоварыВКоробах Тогда
				...
				Если СтрокиЗаказа.Количество() > 0 Тогда
					...
					НеРаспределено = "";
					Если ОсталосьДобавить > 0 Тогда
						НеРаспределено = НеРаспределено + "Не получилось добавить:" + ОсталосьДобавить + ". ВариантКомплектации:" + Элемент["ВариантКомплектации"];
					КонецЕсли;
					Если ОсталосьУдалить > 0 Тогда
						НеРаспределено = НеРаспределено + "Не получилось удалить:" + ОсталосьДобавить + ". ВариантКомплектации:" + Элемент["ВариантКомплектации"];
					КонецЕсли;
			...
			Иначе
				...
				Если СтрокиЗаказа.Количество() > 0 Тогда
					...
					НеРаспределено = "";
					Если ОсталосьДобавить > 0 Тогда
						НеРаспределено = НеРаспределено + "Не получилось добавить:" + ОсталосьДобавить + ". Номенклатура:" + Элемент["Номенклатура"] + ". Характеристика:" + Элемент["Характеристика"];
					КонецЕсли;
					Если ОсталосьУдалить > 0 Тогда
						НеРаспределено = НеРаспределено + "Не получилось удалить:" + ОсталосьДобавить + ". Номенклатура:" + Элемент["Номенклатура"] + ". Характеристика:" + Элемент["Характеристика"];
					КонецЕсли;
					Если НеРаспределено <> "" Тогда
						СообщитьПользователю(НеРаспределено);
					КонецЕсли;
...
Процедура ДобавитьСтрокуВКоробах(Команда)
	...
	ЭтаФорма.ТекущийЭлемент = Элементы.ТоварыВКоробах;
...
Функция ПолучитьНазначение(Заказ)
	...
	Если Результат.Пустой() Тогда
		...
		Назначение.Записать();
		СообщитьПользователю("Создан Назначение:" + Назначение.Ссылка + " для Заказа" + Заказ);
...
Процедура ПеренестиРезервНаСервере(ПолеКоличество, ПолеНазначение, ИзменитьЗаказ = Ложь)
	...
	Для Каждого СтрокаНазначение Из ТаблицаНазначений Цикл
		...
		ДокКорректировка.Комментарий = "Создан автоматически обработкой Корректировка заказов и резервов " + ТекущаяДата();
		...
		Попытка
			...
			СообщитьПользователю("Проведен:" + ДокКорректировка.Ссылка);
			СтрокаПротокола = ТЗПротокол.Добавить();
		...
		Если ИзменитьЗаказ Тогда
			...
			Попытка
				...
				СообщитьПользователю("Проведен:" + ДокЗаказ.Ссылка);
				СтрокаПротокола = ТЗПротокол.Добавить();
