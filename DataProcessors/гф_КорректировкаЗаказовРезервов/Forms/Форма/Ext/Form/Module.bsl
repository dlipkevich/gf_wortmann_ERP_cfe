...
Процедура ИзменитьЗаказНовоеНазначение(ИзменитьЗаказ, Назначение)
	Если ИзменитьЗаказ Тогда
		...
		Если ЗначениеЗаполнено(Назначение.Заказ) Тогда
			...
			флОшибка = Ложь;
			ЗаписатьДокументВПопытке(ДокЗаказ, "Проведен:", ТЗПротокол, "Изменен и проведен", флОшибка, Истина);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ИзменитьЗаказВКоробахНовоеНазначение(ИзменитьЗаказ, Назначение)
	...
	СтрокаЗаказа.СтавкаНДС = гф_ЗаполнитьСтавкуНДС(СтрокаЗаказа.ВариантКомплектации, СтруктураДействий);
			...
			флОшибка = Ложь;
			ЗаписатьДокументВПопытке(ЗаказОбъект, "Проведен:", ТЗПротокол, "Изменен и проведен", флОшибка, Истина);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
