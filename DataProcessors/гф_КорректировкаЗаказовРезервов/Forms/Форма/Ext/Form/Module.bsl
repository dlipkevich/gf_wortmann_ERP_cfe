...
Процедура РезервыНовоеНазначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	КонецЕсли;
	ПараметрыФормы = Новый Структура("мЗаказКлиента, ВариантКомплектации, Номенклатура, Характеристика", мЗаказКлиента, Неопределено, Номенклатура, Характеристика);
...
Процедура РезервыПеренестиНовоеНазначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Номенклатура = Неопределено;
	Характеристика = Неопределено;
	ПараметрыФормы = Новый Структура("мЗаказКлиента, ВариантКомплектации, Номенклатура, Характеристика",
		мЗаказКлиента, Неопределено, Номенклатура, Характеристика);
	ПараметрыФормы.Вставить("РезервыВКоробах", Ложь);
...
Процедура ПересчитатьТЧТовары(флЗаписатьЗаказ)
	Для Каждого СтрокаТЧ Из тчТовары Цикл
		...
		ТД.гф_ЦенаСоСкидкой = ТД.Цена * (1 - ТД.ПроцентРучнойСкидки/100);
		...
		ТД.СуммаНДСБезВозвратнойТары = ТД.Сумма * (Ставка/100);
...
Процедура ПересчитатьИтогПоСтрокеРезерва()
	ТД = Элементы.Резервы.ТекущиеДанные;
	Если ТД <> Неопределено Тогда
		//ТД.КоличествоУпаковок = ТД.КоличествоСИзменениями + ТД.Зарезервировать - ТД.СнятьРезерв - ТД.ПеренестиВДругойЗаказ;
	...
	Для Каждого СтрокаНазначение Из ТаблицаНазначений Цикл
		...
		Для Каждого СтрокаРезерва Из СтрокиРезерва Цикл
			...
			Пока СнятьРезерв > 0 Цикл
				...
				//ДокУпаковочныйЛист = ВнестиИзмененияВУпаковочныйЛист(УпаковочныйЛист, СтрокаНазначение[ПолеНазначение].Заказ,
				//	СтрокаНазначение[ПолеНазначение]);
				//флОшибка = Ложь;
				//ЗаписатьДокументВПопытке(ДокУпаковочныйЛист, "Проведен:", ТЗПротокол, "Изменен и проведен", флОшибка);
...
Процедура ЗаполнитьТЧВПарах(ИмяТЧ, НомерВерсии = 1)
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(СтрокаЗаказа, Выборка);
		//СтрокаЗаказа.КоличествоСИзменениями = Выборка.КоличествоУпаковок;
		//СтрокаЗаказа.Количество = Выборка.КоличествоУпаковок;
...
Процедура РезервыВКоробахНовоеНазначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	КонецЕсли;
	ПараметрыФормы = Новый Структура("мЗаказКлиента, ВариантКомплектации, Номенклатура, Характеристика", мЗаказКлиента, ВариантКомплектации, Неопределено, Неопределено);
...
Процедура РезервыВКоробахПеренестиНовоеНазначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВариантКомплектации = Неопределено;
	ПараметрыФормы = Новый Структура("мЗаказКлиента, ВариантКомплектации, Номенклатура, Характеристика",
		мЗаказКлиента, ВариантКомплектации, Неопределено, Неопределено);
	ПараметрыФормы.Вставить("РезервыВКоробах", Истина);
	ПараметрыФормы.Вставить("ПеренестиНовоеНазначение", Истина);
	Если ИмяФормы = "ВнешняяОбработка.гф_КорректировкаЗаказовРезервов.Форма.Форма" Тогда
		ОткрытьФорму("ВнешняяОбработка.гф_КорректировкаЗаказовРезервов.Форма.ФормаВыбораНазначения", ПараметрыФормы,
...
Функция РассчитатьСкидку(Заказ)
	...
	МассивВидыСкидок = _омОбщегоНазначенияКлиентСервер.ПолучитьГлобальноеЗначениеМассив("гф_ГлобальныеЗначенияСкидкиДляЗаказаКлиента");
...
//	Элементы.ТаблицаЗаказыКонтрагент.Видимость = ВидимостьКонтрагент;
//	Элементы.ТаблицаЗаказыСклад.Видимость = ВидимостьСклад;
//	Элементы.ТаблицаЗаказыСуммаДокумента.Видимость = ВидимостьСумма;
//	Элементы.ТаблицаЗаказыКомментарий.Видимость = ВидимостьКомментарий;
//КонецПроцедуры
