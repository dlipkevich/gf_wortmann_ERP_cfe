...
Процедура СоздатьКорректировкуСнятиеРезерваНаСервере()
	...
	Для Каждого СтрокаРезерва Из Объект.РезервыВКоробах Цикл
		...
		Пока СнятьРезерв > 0 Цикл
			...
			ДокУпаковочныйЛист = ВнестиИзмененияВУпаковочныйЛист(УпаковочныйЛист, Документы.ЗаказКлиента.ПустаяСсылка(),
...
Процедура ЗаполнитьПометкуПоСтроке(ТД, ПолеДобавить, ПолеУдалить, ПолеПеренестиВДругойЗаказ)
	Если ПолеДобавить <> ""	И ЗначениеЗаполнено(ТД[ПолеДобавить]) Тогда
		ТД["Пометка"] = Истина;
	ИначеЕсли ПолеУдалить <> "" И ЗначениеЗаполнено(ТД[ПолеУдалить]) Тогда
		ТД["Пометка"] = Истина;
	ИначеЕсли ПолеПеренестиВДругойЗаказ <> "" И ЗначениеЗаполнено(ТД[ПолеПеренестиВДругойЗаказ]) Тогда
		ТД["Пометка"] = Истина;
	Иначе
		ТД["Пометка"] = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьРезервыВКоробахНаСервере()
	...
	Для Каждого СтрокаРезерва Из Объект.РезервыВКоробах Цикл
		...
		ОтборПоВариантуКомплектации = Новый Структура("ВариантКомплектации", СтрокаРезерва["ВариантКомплектации"]);
		СтрокиНоменклатуры = ТаблицаНоменклатуры.НайтиСтроки(ОтборПоВариантуКомплектации);
		СписокНоменклатуры = ПолучитьСписокНоменклатуры(СтрокиНоменклатуры);
		...
		ИсправитьДоступноНаСкладе(мДоступноНаСкладе, ТаблицаОстатки, СтрокаРезерва, СтрокаСвободныхОстатков);
...
&НаСервереБезКонтекста
Процедура ИсправитьДоступноНаСкладе(мДоступноНаСкладе, ТаблицаОстатки, СтрокаРезерва, СтрокаСвободныхОстатков)
...
Процедура ЗакрытьСезон(Команда)
	ТекстСообщения = "Процедура не реализована!";
	СообщитьПользователю(ТекстСообщения);
КонецПроцедуры
