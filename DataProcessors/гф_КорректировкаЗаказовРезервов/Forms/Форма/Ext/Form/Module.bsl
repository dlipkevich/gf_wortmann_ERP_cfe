...
Процедура РезервыВКоробахЗарезервироватьПриИзменении(Элемент)
	...
	Если ТД <> Неопределено Тогда
		ТД["Зарезервировать"] = ТД["КоличествоСИзменениями"] - ТД["ТоварВРезерве"];
		Если ТД["Зарезервировать"] > ТД["ДоступноНаСкладе"] Тогда
			ТД["Зарезервировать"] = ТД["ДоступноНаСкладе"];
		КонецЕсли;
	КонецЕсли;
...
Процедура РезервыВКоробахСнятьРезервПриИзменении(Элемент)
	ТД = Элементы["РезервыВКоробах"].ТекущиеДанные;
	Если ТД <> Неопределено Тогда
		Если ТД["СнятьРезерв"] > ТД["ТоварВРезерве"] Тогда
			ТД["СнятьРезерв"] = ТД["ТоварВРезерве"];
		КонецЕсли;
	КонецЕсли;
	ПересчитатьКоличествоПоСтрокеВКоробах("РезервыВКоробах", "", "", "ПеренестиВДругойЗаказ");
КонецПроцедуры
...
Процедура РезервыВКоробахПеренестиВДругойЗаказПриИзменении(Элемент)
	// Галфинд СадомцевСА 03.10.2023 Реализовал "очистку" колонки "Новое назначение" при условии что "Количество" = 0
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee61296d192f16
	ТД = Элементы["РезервыВКоробах"].ТекущиеДанные;
	Если ТД <> Неопределено Тогда
		Если Не ЗначениеЗаполнено(ТД.ПеренестиВДругойЗаказ) Тогда
			ТД.ПеренестиНовоеНазначение = ПредопределенноеЗначение("Справочник.Назначения.ПустаяСсылка");
		КонецЕсли;
	КонецЕсли;
	ПересчитатьКоличествоПоСтрокеВКоробах("РезервыВКоробах", "", "", "ПеренестиВДругойЗаказ");
КонецПроцедуры
...
Процедура ЗаполнитьТЧТоварыНаСервере()
    ...
    мНомерВерсииЗаказа = НомерВерсииЗаказа;
...
Процедура ЗаполнитьДанныеЗаказа(Команда)
	...
	// СадомцевСА 19.02.2024
	ПересчитатьмТоварВРезерве();
	ПересчитатьИтогиПоКолонкам();
КонецПроцедуры
...
Процедура ПересчитатьТЧТовары(флЗаписатьЗаказ)
	...
	Для Каждого СтрокаТЧ Из тчТовары Цикл
		...
		Если ЗначениеЗаполнено(ТД.ВидЦены) И ТД.Назначение <> мНазначение Тогда
			// пересчитываем цену только для "нового" назначения СадомцевСА 22.09.2023
...
Процедура СохранитьРезервы()
	...
	Если Объект.Резервы.Итог("ПеренестиВДругойЗаказ") > 0 Тогда
		тчТовары.Очистить(); // надо здесь
		ЗаполнитьТЧТоварыПоРезервам(); // надо здесь
		флЗаписатьЗаказ = Ложь;
		ПересчитатьТЧТовары(флЗаписатьЗаказ); // надо здесь
...
&НаСервере
Функция ПолучитьУпаковочныеЛисты()
	Запрос = Новый Запрос;
	...
	|				И Склад = &Склад) КАК гф_ДвижениеКодовМаркировкиОрганизацииОстатки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ втОстаткиУпаковок КАК втОстаткиУпаковок
	|		ПО (втОстаткиУпаковок.КМ = УпаковочныйЛист.гф_Агрегация)
	|ГДЕ
	...
	|	И НЕ втОстаткиУпаковок.КМ ЕСТЬ NULL";
...
Процедура ЗаполнитьУпаковочныеЛисты()
	...
	Если ЗначениеЗаполнено(мЗаказКлиента) Тогда
		мУпаковочныеЛисты.Загрузить(ПолучитьУпаковочныеЛисты());
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура СохранитьРезервыВКоробах()
...
Процедура СоздатьКорректировкуСнятиеРезерваНаСервере(ИмяТЧ)
	...
	// СадомцевСА 28.02.2024 Перенес "транзакцию" в цикл по строкам Резерва - Упаковочным листам
	//// СадомцевСА 21.02.2024
	//УЛ = ПолучитьУпаковочныеЛисты();
	//// здесь надо сравнить УЛ и мУпаковочныеЛисты
	//// и вывести предупреждение о том что перечень "УЛ" изменился
	//ТаблицыИдентичны = СравнитьТаблицы(УЛ, мУпаковочныеЛисты, "УпаковочныйЛист");
	//Если Не ТаблицыИдентичны Тогда
	//	ТекстСообщения = "Сведения об упаковочных листах устарели. Сохранение невозможно!" + Символы.ПС
	//	+ "Нажмите кнопку Заполнить данные заказа.";
	//	СообщитьПользователю(ТекстСообщения);
	//	Возврат;
	//КонецЕсли;
	
	НомерВерсииЗаказа = НайтиНомерВерсииЗаказаПоСтатусу(мЗаказКлиента, ВыборСтатуса);
	Если НомерВерсииЗаказа <> мНомерВерсииЗаказа Тогда
		ТекстСообщения = "Сведения о Заказе устарели. Сохранение невозможно!" + Символы.ПС
		+ "Нажмите кнопку Заполнить данные заказа.";
		СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	//	Для Каждого СтрокаУЛ Из мУпаковочныеЛисты Цикл
	//		ЗаблокироватьСсылку(Блокировка, СтрокаУЛ.УпаковочныйЛист, "Документ.УпаковочныйЛист");
	//	КонецЦикла;
	...
	ПустоеНазначение = Справочники.Назначения.ПустаяСсылка();
		...
		Для Каждого СтрокаРезерва Из Объект[ИмяТЧ] Цикл
			...
			Пока СнятьРезерв > 0 Цикл
				...
				// 1) Получаем УЛ по Заказу и Варианту комплектации
				УЛ = ПолучитьУпаковочныеЛисты();
				УпаковочныеЛисты = УЛ.НайтиСтроки(СтруктураПоиска);
				ВсегоУпаковочныхЛистов = УпаковочныеЛисты.Количество();
				Если ВсегоУпаковочныхЛистов = 0 Тогда
					// здесь НЕ найдено ни одного УЛ - надо перейти к следующей строке резерва
					Прервать;
				КонецЕсли;
				
				// 2) Блокируем УЛ и создаем Корректировку
				УпаковочныйЛист = УпаковочныеЛисты[0].УпаковочныйЛист;
				НачатьТранзакцию();
				флОшибка = Ложь;
				Попытка
					// блокируем "УЛ"
					Блокировка = Новый БлокировкаДанных;
					ЗаблокироватьСсылку(Блокировка, УпаковочныйЛист, "Документ.УпаковочныйЛист");
					Блокировка.Заблокировать();
					...
					// заполняем Номенклатуру из Упаковочного листа
					ЗаполнитьТЧТоварыКорректировкаНазначения(ДокСнятиеРезерва, УпаковочныйЛист, мЗаказКлиента, мНазначение);
				
					// 3) Сохраняем Корректировку
					ЗаписатьДокументВПопытке(ДокСнятиеРезерва, "Проведен:", ТЗПротокол, "Создан и проведен", флОшибка);
					Если флОшибка Тогда
						ОтменитьТранзакцию();
						РазблокироватьДанныеДляРедактирования(УпаковочныйЛист);
						Прервать;
					КонецЕсли;
				
					// 4) в Упаковочном листе надо очистить Заказ и Назначение (в тч Товары)
				...
				//Индекс = Индекс + 1;
			КонецЦикла; // Пока СнятьРезерв > 0 Цикл
		КонецЦикла; // Для Каждого СтрокаРезерва Из Объект[ИмяТЧ] Цикл
		//// СадомцевСА 19.02.2024
		//// флОшибка = Ложь;
		//ЗаписатьДокументВПопытке(ДокСнятиеРезерва, "Проведен:", ТЗПротокол, "Создан и проведен", мОшибка);
		//// Если Не флОшибка Тогда Перенес проверку "ошибки" в цикл
		//ПустойЗаказ = Документы.ЗаказКлиента.ПустаяСсылка();
		//ПустоеНазначение = Справочники.Назначения.ПустаяСсылка();
		...
		//	ДокУпаковочныйЛист = ВнестиИзмененияВУпаковочныйЛист(УпаковочныйЛист, ПустойЗаказ, ПустоеНазначение);
		//	// флОшибка = Ложь;
		//	ЗаписатьДокументВПопытке(ДокУпаковочныйЛист, "Проведен:", ТЗПротокол, "Изменен и проведен", мОшибка);
		//	
		//	// СадомцевСА 21.02.2024
		//	РазблокироватьДанныеДляРедактирования(УпаковочныйЛист);
		//КонецЦикла;
	...
	//	ОтменитьТранзакцию();
	//	Для Каждого СтрокаУЛ Из мУпаковочныеЛисты Цикл
	//		РазблокироватьДанныеДляРедактирования(СтрокаУЛ.УпаковочныйЛист);
	//	КонецЦикла;
	//	
	//КонецПопытки;
КонецПроцедуры
...
Процедура СоздатьКорректировкуРезервированиеНаСервере(ИмяТЧ)
	...
	//// СадомцевСА 21.02.2024
	//СвободныеУЛ = ПолучитьСвободныеУЛ();
	//// здесь надо сравнить СвободныеУЛ и мСвободныеУпаковочныеЛисты
	//// и вывести предупреждение о том что перечень "свободных УЛ" изменился
	//ТаблицыИдентичны = СравнитьТаблицы(СвободныеУЛ, мСвободныеУпаковочныеЛисты, "УпаковочныйЛист");
	//Если Не ТаблицыИдентичны Тогда
	//	ТекстСообщения = "Сведения о свободных упаковочных листах устарели. Сохранение невозможно!" + Символы.ПС
	//		+ "Нажмите кнопку Заполнить данные заказа.";
	//	СообщитьПользователю(ТекстСообщения);
	//	Возврат;
	//КонецЕсли;
	...
	//НачатьТранзакцию();
	//Попытка
	//	// блокируем "свободные УЛ"
	//	Блокировка = Новый БлокировкаДанных;
	//	Для Каждого СтрокаУЛ Из мСвободныеУпаковочныеЛисты Цикл
	//		ЗаблокироватьСсылку(Блокировка, СтрокаУЛ.УпаковочныйЛист, "Документ.УпаковочныйЛист");
	//	КонецЦикла;
	//	Блокировка.Заблокировать();
	//	
	...
	//	МассивУпаковочныеЛисты = Новый Массив;
		...
		Для Каждого СтрокаРезерва Из Объект[ИмяТЧ] Цикл
			...
			Пока Зарезервировать > 0 Цикл
				// 1) Получаем свободные УЛ по Варианту комплектации
				СвободныеУЛ = ПолучитьСвободныеУЛ();
				УпаковочныеЛисты = СвободныеУЛ.НайтиСтроки(СтруктураПоиска);
				ВсегоУпаковочныхЛистов = УпаковочныеЛисты.Количество();
				Если ВсегоУпаковочныхЛистов = 0 Тогда
					// здесь НЕ найдено ни одного УЛ - надо перейти к следующей строке резерва
					Прервать;
				...
				Попытка
					...
					// 4) в Упаковочном листе надо записать Заказ и Назначение (в тч Товары)
				...
				КонецПопытки;
				РазблокироватьДанныеДляРедактирования(УпаковочныйЛист);
				Зарезервировать = Зарезервировать - 1;
			КонецЦикла; // Пока Зарезервировать > 0 Цикл
		КонецЦикла; // Для Каждого СтрокаРезерва Из Объект[ИмяТЧ] Цикл
	//	// СадомцевСА 19.02.2024
	//	// флОшибка = Ложь;
	//	ЗаписатьДокументВПопытке(ДокРезервирование, "Проведен:", ТЗПротокол, "Создан и проведен", мОшибка);
	//	// Если Не флОшибка Тогда Перенес проверку "ошибки" в цикл
	//	Для Каждого УпаковочныйЛист Из МассивУпаковочныеЛисты Цикл
	//		// СадомцевСА 19.02.2024
	//		Если мОшибка Тогда
...
Процедура ВыполнитьРаспределениеЗапасовНаСервере()
	// СадомцевСА 28.07.2023 Зависание в обработке вызвано выполнением типовой процедуры РаспределениеЗапасов.ВыполнитьРаспределениеВФоне(),
	// которая в текущей версии конфигурации 1С:ERP Управление предприятием 2 (2.5.11.96) в типовом коде НЕ вызывается (НЕ выполняется).
	// Отменил вызов процедуры ВыполнитьРаспределениеВФоне().
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee2c7ee69a2ab3
...
&НаСервереБезКонтекста
Процедура ЗаписатьДокументВПопытке(ДокОбъект, ТекстСообщения, ТЗПротокол, Комментарий, флОшибка, флПроверитьВидЦены = Ложь)
	//Попытка
	...
	Попытка
		...
		// СадомцевСА 22.09.2023
		// Добавил в протокол "сохранения заказа-приемника" сообщение "Вид цены не указан. Заполните цены!".
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee51757e267b8e (п.4)
		Если флПроверитьВидЦены Тогда
			Если Не ЗначениеЗаполнено(ДокОбъект.гф_ВидЦены) Тогда
				СтрокаПротокола.Комментарий = СтрокаПротокола.Комментарий
					+ ". Вид цены не указан. Заполните цены!";
			КонецЕсли;
		КонецЕсли;
	Исключение
		...
		Если ТипЗнч(ДокОбъект) = Тип("ДокументОбъект.КорректировкаНазначенияТоваров") Тогда
			СтрокаПротокола.Комментарий = "Не удалось провести Корректировку назначения товаров!";
		КонецЕсли;
		флОшибка = Истина;
	КонецПопытки;
КонецПроцедуры
...
Процедура ЗаписатьЗаказНаСервере()
	...
	Попытка
		...
		Блокировка = Новый БлокировкаДанных;
		// блокируем мЗаказ
		ЗаблокироватьСсылку(Блокировка, мЗаказКлиента, "Документ.ЗаказКлиента");
		Блокировка.Заблокировать();
		...
		//Если РежимРаботы = 1 Тогда
		//	// СадомцевСА 21.02.2024 в этом режиме Заказ изменяется в других процедурах
		//	//// Резервы по заказам
		//	//Если Не флТоварыВКоробах Тогда
		...
		ЗаписатьДокументВПопытке(ЗаказОбъект, "Проведен:", ТЗПротокол, "Изменен и проведен", мОшибка);
		//Если Не мОшибка Тогда
		//	флКомментарийСохранен = Истина;
		//КонецЕсли;
		//КонецЕсли;
		
		// СадомцевСА 21.02.2024
		РазблокироватьДанныеДляРедактирования(мЗаказКлиента);
...
Процедура СохранитьКомментарийВЗаказе()
	...
	Попытка
		...
		// флОшибка = Ложь;
		ЗаписатьДокументВПопытке(ЗаказОбъект, "Проведен:", ТЗПротокол, "Изменен комментарий и проведен", мОшибка);
...
Функция ПолучитьЗначениеДопРеквизита(Ссылка, Свойство) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);   // Добавлено  Галфинд \ Sakovich 12.09.2023
...
Процедура ЗаполнитьРезервыНаСервере()
	...
	Для Каждого СтрокаТЧ Из Объект.Резервы Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ["КоличествоСИзменениями"]) Тогда
			СтрокаТЧ["ДоступноНаСкладе"] = 0;
		КонецЕсли;
	КонецЦикла;
...
Процедура СформироватьПротоколРаспределенияРезервов(ТаблицаНоменклатуры, УпаковочныеЛисты, ЭтоРезерв = Истина)
	...
	Для Каждого СтрокаУЛ Из УпаковочныеЛисты Цикл
		...
		Для Каждого СтрокаНоменклатуры Из СтрокиНоменклатуры Цикл
			...
			ОбластьСтрока.Параметры.Артикул = СтрокаНоменклатуры.Номенклатура.Артикул; // 21.02.2024
...
Функция ТекстЗапросаОстаткиКоробов()
	Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			...
			// |	гф_ДвижениеКодовМаркировкиОрганизацииОстатки.Резерв КАК Резерв, // Галфинд \ Sakovich 23.05.2023 измерение удалено
	        |	гф_ДвижениеКодовМаркировкиОрганизацииОстатки.КоличествоОстаток КАК КоличествоОстаток
	        |ПОМЕСТИТЬ втОстаткиУпаковок
	        |ИЗ
	        |	РегистрНакопления.гф_ДвижениеКодовМаркировкиОрганизации.Остатки(
...
Функция ТекстЗапросаСвободныеОстаткиКоробов()
	Возврат "ВЫБРАТЬ РАЗЛИЧНЫЕ РАЗРЕШЕННЫЕ
	        ...
	        |
	        |////////////////////////////////////////////////////////////////////////////////
			// ++ 21.02.2024 СадомцевСА Упаковочный лист считается "свободным"
			// при условии: гф_Заказ - пусто И Назначение в тч Товары - пусто
	        |ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	        |	УпаковочныйЛистТовары.Ссылка КАК УпаковочныйЛист,
	        |	МАКСИМУМ(УпаковочныйЛистТовары.Назначение) КАК Назначение
	        |ПОМЕСТИТЬ втНазначенияУЛ
	        |ИЗ
	        |	Документ.УпаковочныйЛист.Товары КАК УпаковочныйЛистТовары
	        |ГДЕ
	        |	УпаковочныйЛистТовары.Ссылка.гф_Заказ.Номер ЕСТЬ NULL
	        |	И УпаковочныйЛистТовары.Ссылка.Проведен
	        |	И УпаковочныйЛистТовары.Ссылка.гф_СостояниеКороба В(&СостояниеКороба)
	        |СГРУППИРОВАТЬ ПО
			...
			// ++ 21.02.2024 СадомцевСА
			//|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УпаковочныйЛист КАК УпаковочныйЛист
			//|		ПО гф_ДвижениеКодовМаркировкиОрганизацииОстатки.КМ.ЗначениеШтрихкода = УпаковочныйЛист.Код
			//|			И (УпаковочныйЛист.Ссылка.Проведен)
			|		ЛЕВОЕ СОЕДИНЕНИЕ втНазначенияУЛ КАК втНазначенияУЛ
			|		ПО гф_ДвижениеКодовМаркировкиОрганизацииОстатки.КМ.ЗначениеШтрихкода = втНазначенияУЛ.УпаковочныйЛист.Код
	        |ГДЕ
			...
			|	втНазначенияУЛ.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
			|	И втНазначенияУЛ.УпаковочныйЛист.гф_Комплектация В(&ВариантыКомплектации)
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Процедура ЗаполнитьРезервыВКоробахНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
				   ...
				   // СадомцевСА 19.02.2024
				   |ГДЕ
				   |	втОстаткиКоробов.УпаковочныйЛист.гф_ТекущийОрдер = ЗНАЧЕНИЕ(Документ.РасходныйОрдерНаТовары.ПустаяСсылка)";
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ВЫБОР
	               |		КОГДА т.УпаковочныйЛист.гф_СостояниеКороба = ЗНАЧЕНИЕ(Справочник.гф_СостянияКоробов.ПолныйКомплект)
	               |			ТОГДА 1
	               |		КОГДА т.УпаковочныйЛист.гф_СостояниеКороба = ЗНАЧЕНИЕ(Справочник.гф_СостянияКоробов.Пересорт)
	               |			ТОГДА 2
	               |		КОГДА т.УпаковочныйЛист.гф_СостояниеКороба = ЗНАЧЕНИЕ(Справочник.гф_СостянияКоробов.НеПолный)
	               |			ТОГДА 3
	               |		КОГДА т.УпаковочныйЛист.гф_СостояниеКороба = ЗНАЧЕНИЕ(Справочник.гф_СостянияКоробов.НеПолныйИПересорт)
	               |			ТОГДА 4
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК Порядок,
	               ...
	               |	ЛОЖЬ КАК флНеХватаетДоступныхОстатков
	               |ИЗ
	               |	втСвободныеОстаткиКоробов КАК т
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Порядок";
	РезультатУпаковочныеЛисты = Запрос.Выполнить();
	...
	Для Каждого СтрокаРезерва Из Объект.РезервыВКоробах Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаРезерва["КоличествоСИзменениями"]) Тогда
...
Функция ПолучитьНазначение(Заказ)
	...
	Иначе
		...
		// Галфинд СадомцевСА 09.02.2024 Обработал ошибку "НЕ ОПРЕДЕЛЕНО НАЗНАЧЕНИЕ" при активизации строки в списке Заказов
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eec7175a52b056
		Иначе
			ТекстСообщения = "НЕ ОПРЕДЕЛЕНО НАЗНАЧЕНИЕ для Заказа:" + Заказ;
			СообщитьПользователю(ТекстСообщения);
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ИзменитьЗаказНовоеНазначение(ИзменитьЗаказ, Назначение)
	Если Не ИзменитьЗаказ Тогда
		Возврат;
	КонецЕсли;
	// здесь вносим изменения в Заказ (новое назначение)
	Если ЗначениеЗаполнено(Назначение.Заказ) Тогда
		МаксимальныйКодСтроки = МаксимальныйКодСтрокиВЗаказеКлиента(Назначение.Заказ);
		ДокЗаказ = Назначение.Заказ.ПолучитьОбъект();
		СтруктураПоиска = Новый Структура("Назначение", Назначение);
		СтрокиРезерва = тчТовары.НайтиСтроки(СтруктураПоиска);
		// СадомцевСА 22.09.2023 Добавил в комментарий измененных заказов сведения о перенесенном количестве коробов/пар.
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee51757e267b8e (п.4)
		...
		Для Каждого СтрокаТЧ Из СтрокиРезерва Цикл
			...
			КоличествоУпаковок = КоличествоУпаковок + СтрокаЗаказа.КоличествоУпаковок;
			// 11.06.2024
			СтрокаЗаказа.Обособленно = Истина;
		...
		ДобавитьКомментарий(ДокЗаказ, КоличествоУпаковок, мЗаказКлиента, Истина, флТоварыВКоробах, КомментарийИсточника);
		
		ДокЗаказ.МаксимальныйКодСтроки = МаксимальныйКодСтроки;
		//флОшибка = Ложь;
		ЗаписатьДокументВПопытке(ДокЗаказ, "Проведен:", ТЗПротокол, "Изменен и проведен", мОшибка, Истина);
	КонецЕсли;
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ДобавитьКомментарий(ЗаказПриемник = Неопределено, Количество = 0, ЗаказИсточник, флПриемник, флТоварыВКоробах
	, КомментарийИсточника)
	// СадомцевСА 22.09.2023 Добавил в комментарий измененных заказов сведения о перенесенном количестве коробов/пар.
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee51757e267b8e (п.4)
	Если флТоварыВКоробах Тогда
		ЕдИзм = "коробов";
	Иначе
		ЕдИзм = "пар";
	КонецЕсли;
	Если флПриемник Тогда
		Если ЗначениеЗаполнено(ЗаказПриемник.Комментарий) Тогда
			ЗаказПриемник.Комментарий = ЗаказПриемник.Комментарий + Символы.ПС;
		КонецЕсли;
		ЗаказПриемник.Комментарий = ЗаказПриемник.Комментарий +
			Формат(ТекущаяДатаСеанса(), "ДФ=dd.MM.yyyy") +
			" в заказ перенесено:" + Количество + " " + ЕдИзм + 
			" из заказа " + ЗаказИсточник.Номер + " от " + Формат(ЗаказИсточник.Дата, "ДФ=dd.MM.yyyy") +
			" пользователем " + ПараметрыСеанса.ТекущийПользователь;
		// здесь собираем комментарий для Заказа-источника	
		Если ЗначениеЗаполнено(КомментарийИсточника) Тогда
			КомментарийИсточника = КомментарийИсточника + Символы.ПС;
		КонецЕсли;
		КомментарийИсточника = КомментарийИсточника +
			Формат(ТекущаяДатаСеанса(), "ДФ=dd.MM.yyyy") +
			" из заказа перенесено:" + Количество + " " + ЕдИзм + 
			" в заказ " + ЗаказПриемник.Номер + " от " + Формат(ЗаказПриемник.Дата, "ДФ=dd.MM.yyyy") +
			" пользователем " + ПараметрыСеанса.ТекущийПользователь;
	Иначе
		Если ЗначениеЗаполнено(ЗаказИсточник.Комментарий) Тогда
			ЗаказИсточник.Комментарий = ЗаказИсточник.Комментарий + Символы.ПС;
		КонецЕсли;
		ЗаказИсточник.Комментарий = ЗаказИсточник.Комментарий + КомментарийИсточника;
	КонецЕсли;		
КонецПроцедуры
...
Процедура ИзменитьИсходныйЗаказВКоробах(ИзменитьЗаказ)
	...
	// тч Товары перезаполняются перед записью документа
	// ЗаполнитьТоварыНаСервере(ЗаказОбъект);
	ДобавитьКомментарий( , , ЗаказОбъект, Ложь, флТоварыВКоробах, КомментарийИсточника);
	//флОшибка = Ложь;
	ЗаписатьДокументВПопытке(ЗаказОбъект, "Проведен:", ТЗПротокол, "Изменен и проведен", мОшибка);
	Если Не мОшибка Тогда
		флКомментарийСохранен = Истина;
...
&НаСервере
Процедура ИзменитьСтрокиЗаказа(СтрокиЗаказа, ОсталосьОтменить, СтрокаТЧ)
	ЗначениеСто = 100;
	Для Каждого СтрокаЗаказа Из СтрокиЗаказа Цикл
		Если Не ЗначениеЗаполнено(СтрокаЗаказа.КоличествоУпаковок) Тогда
			Продолжить;
		КонецЕсли;
		ТекОтменить = Мин(ОсталосьОтменить, СтрокаЗаказа.КоличествоУпаковок);
		Если ТекОтменить = СтрокаЗаказа.КоличествоУпаковок Тогда
			// 11.03.2024 здесь надо "отменить" все количество в строке заказа
			СтрокаЗаказа.Отменено = Истина;
			СтрокаЗаказа.ПричинаОтмены = СтрокаТЧ.ПричинаОтмены;
		Иначе
			...
			Если ЗначениеЗаполнено(ТД.СтавкаНДС) Тогда
				Ставка = ПолучитьЗначениеРеквизита(ТД.СтавкаНДС, "Ставка");
			КонецЕсли;
			Цена = ТД.гф_ЦенаСоСкидкой;
			Если Не ЗначениеЗаполнено(Цена) Тогда
				Цена = ТД.Цена;
			КонецЕсли;
			ТД.Сумма = ТД.КоличествоУпаковок * Цена;
			// пересчет НДС с учетом скидки
			ТД.СуммаНДС = Окр(ТД.Сумма * (Ставка / ЗначениеСто), 2);
			// пересчет Сумма с НДС с учетом скидки
			ТД.СуммаСНДС = ТД.Сумма + ТД.СуммаНДС;
		КонецЕсли;
...
&НаСервере
Процедура ИзменитьИсходныйЗаказ(ИзменитьЗаказ)
	// СадомцевСА 19.02.2024
	Если мОшибка Тогда
		Возврат;
	КонецЕсли;
	...
	СтрокиРезерва = тчТовары.НайтиСтроки(СтруктураПоиска);
	Для Каждого СтрокаТЧ Из СтрокиРезерва Цикл
		// корректируем исходные строки заказа
		...
		ИзменитьСтрокиЗаказа(СтрокиЗаказа, ОсталосьОтменить, СтрокаТЧ);
		//Для Каждого СтрокаЗаказа Из СтрокиЗаказа Цикл
		//	Если Не ЗначениеЗаполнено(СтрокаЗаказа.КоличествоУпаковок) Тогда
		//		Продолжить;
		//	КонецЕсли;
		...
		//	КонецЕсли;
		//КонецЦикла;
		// 11.03.2024 строку "отменено" НЕ добавляем т.к. отмена/уменьшение количества выполняется в исходной строке заказа
		//// добавляем строку "отменено"
		// СтрокаЗаказа = ДокЗаказ.Товары.Добавить();
		// МаксимальныйКодСтроки = МаксимальныйКодСтроки + 1;
		// ЗаполнитьЗначенияСвойств(СтрокаЗаказа, СтрокаТЧ);
	...
	КонецЦикла;
	СкидкиНаценкиСервер.ЗаполнитьКлючиСвязиВТабличнойЧастиТовары(ДокЗаказ, "Товары");
	ДобавитьКомментарий( , , ДокЗаказ, Ложь, флТоварыВКоробах, КомментарийИсточника);
	
	ДокЗаказ.МаксимальныйКодСтроки = МаксимальныйКодСтроки;
	// флОшибка = Ложь;
	ЗаписатьДокументВПопытке(ДокЗаказ, "Проведен:", ТЗПротокол, "Изменен и проведен", мОшибка);
	Если Не мОшибка Тогда
		флКомментарийСохранен = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ИзменитьЗаказВКоробахНовоеНазначение(ИзменитьЗаказ, Назначение)
	...
	ЗначениеСто = 100;
	// здесь вносим изменения в Заказ (новое назначение)
	Если Не ЗначениеЗаполнено(Назначение.Заказ) Тогда
		Возврат;
	КонецЕсли;
	...
	ДобавитьКомментарий(ЗаказОбъект, Количество, мЗаказКлиента, Истина, флТоварыВКоробах, КомментарийИсточника);
	
	//флОшибка = Ложь;
	ЗаписатьДокументВПопытке(ЗаказОбъект, "Проведен:", ТЗПротокол, "Изменен и проведен", мОшибка, Истина);
КонецПроцедуры
...
&НаСервере
Процедура ДокКорректировкаЗаполнитьТовары(ДокКорректировка, СтрокиРезерва, ПолеКоличество)
	Для Каждого СтрокаТЧ Из СтрокиРезерва Цикл
		Если ЗначениеЗаполнено(СтрокаТЧ[ПолеКоличество]) Тогда
			Количество = Мин(СтрокаТЧ[ПолеКоличество], СтрокаТЧ["ТоварВРезерве"]);
			...
			нс.ИсходноеНазначение = мНазначение;
			нс.ИсходныйЗаказ = мЗаказКлиента;
			нс.ИсходноеНазначениеДвиженияПоСкладскимРегистрам = Истина;
			// Аналитика учета заполняется при проведении документа автоматически...
		КонецЕсли;
...
Процедура ПеренестиРезервНаСервере(ПолеКоличество, ПолеНазначение, ИзменитьЗаказ = Ложь)
	...
	// СадомцевСА 28.02.2024 Перенес "транзакцию" в цикл по Назначениям
	//НачатьТранзакцию();
	//Попытка
		...
		//			Продолжить;
		//		КонецЕсли;
		//		ЗаблокироватьСсылку(Блокировка, СтрокаНазначение[ПолеНазначение].Заказ, "Документ.ЗаказКлиента");
		//	КонецЦикла;
		//	ЗаблокироватьСсылку(Блокировка, мЗаказКлиента, "Документ.ЗаказКлиента");
		//КонецЕсли;
		//Блокировка.Заблокировать();
		...
		Для Каждого СтрокаНазначение Из ТаблицаНазначений Цикл
			...
			Попытка
				...
				// СадомцевСА 27.04.2024 Исправил ошибки СонарКуба. Перенес код в процедуру.
				ДокКорректировкаЗаполнитьТовары(ДокКорректировка, СтрокиРезерва, ПолеКоличество);
				//Для Каждого СтрокаТЧ Из СтрокиРезерва Цикл
				//	Если ЗначениеЗаполнено(СтрокаТЧ[ПолеКоличество]) Тогда
				//		Количество = Мин(СтрокаТЧ[ПолеКоличество], СтрокаТЧ["ТоварВРезерве"]);
				//		Если НЕ ЗначениеЗаполнено(Количество) Тогда
				...
				ЗаписатьДокументВПопытке(ДокКорректировка, "Проведен:", ТЗПротокол, "Создан и проведен", мОшибка);
				Если мОшибка Тогда
					ОтменитьТранзакцию();
					РазблокироватьДанныеДляРедактирования(СтрокаНазначение[ПолеНазначение].Заказ);
					РазблокироватьДанныеДляРедактирования(мЗаказКлиента);
					Продолжить;
				КонецЕсли;
				...
				ИзменитьЗаказНовоеНазначение(ИзменитьЗаказ, СтрокаНазначение[ПолеНазначение]);
				
				// здесь вносим изменения в "исходный" Заказ
				ИзменитьИсходныйЗаказ(ИзменитьЗаказ);
		...
		КонецЦикла; // Для Каждого СтрокаНазначение Из ТаблицаНазначений Цикл
	...
	//		КонецЕсли;
	//		// здесь вносим изменения в Заказ (новое назначение)
	//		ИзменитьЗаказНовоеНазначение(ИзменитьЗаказ, Назначение);
	//		
	//		// СадомцевСА 21.02.2024
	//		РазблокироватьДанныеДляРедактирования(Назначение.Заказ);
	//	КонецЦикла;
	//	
	//	// здесь вносим изменения в "исходный" Заказ
	//	ИзменитьИсходныйЗаказ(ИзменитьЗаказ);
...
&НаСервере
Процедура ДокКорректировкаЗаполнитьТЧТоварыПоСтрокамРезерва(ДокКорректировка, СтрокиРезерва, УЛ,
	ТаблицаУпаковочныеЛисты, ПолеКоличество, ПолеНазначение, СтрокаНазначение)
...
Процедура ПеренестиРезервВКоробахНаСервере(ПолеКоличество, ПолеНазначение, ИзменитьЗаказ = Ложь)
	...
	//	КонецЦикла;
		...
		Для Каждого СтрокаНазначение Из ТаблицаНазначений Цикл
			...
			НачатьТранзакцию();
			мОшибка = Ложь;
			ТаблицаУпаковочныеЛисты.Очистить();
			Попытка
				Блокировка = Новый БлокировкаДанных;
				// Формируем таблицу УЛ в цикле по новым Назначениям
				УЛ = ПолучитьУпаковочныеЛисты();
				Для Каждого СтрокаУЛ Из УЛ Цикл
					// блокируем УЛ
					ЗаблокироватьСсылку(Блокировка, СтрокаУЛ.УпаковочныйЛист, "Документ.УпаковочныйЛист");
				КонецЦикла;
				Если ИзменитьЗаказ Тогда
				...
				СтрокиРезерва = Объект.РезервыВКоробах.НайтиСтроки(СтруктураПоиска);
				// СадомцевСА 27.04.2024 Исправил ошибки СонарКуба. Перенес код в процедуру.
				ДокКорректировкаЗаполнитьТЧТоварыПоСтрокамРезерва(ДокКорректировка, СтрокиРезерва, УЛ,
					ТаблицаУпаковочныеЛисты, ПолеКоличество, ПолеНазначение, СтрокаНазначение);
				//Для Каждого СтрокаРезерва Из СтрокиРезерва Цикл
				//	Если Не ЗначениеЗаполнено(СтрокаРезерва[ПолеКоличество]) Тогда
				//		Продолжить;
				...
				// СадомцевСА 18.09.2023 e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee51757e267b8e (п.3)
				Если ДокКорректировка.Товары.Количество() > 0 Тогда
					// СадомцевСА 19.02.2024
				...
				// здесь вносим изменения в Заказ (новое назначение)
				ИзменитьЗаказВКоробахНовоеНазначение(ИзменитьЗаказ, СтрокаНазначение[ПолеНазначение]);
...
Процедура ЗаполнитьТЧВКоробах(ИмяТЧ, НомерВерсии = 1)
	...
	Если ТипЗнч(ВерсияОбъекта) = Тип("Структура") Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	т.КоличествоПервоначально КАК КоличествоПервоначально,
		               |	т.Сумма КАК Сумма
		               |ПОМЕСТИТЬ втТЧ
		               |ИЗ
		               |	&ТЗ КАК т";
		...
		Запрос.Текст = "ВЫБРАТЬ
		               |	т.ВариантКомплектации КАК ВариантКомплектации,
		               |	МАКСИМУМ(т.Скидка) КАК Скидка,
		               |	МАКСИМУМ(т.ВидЦены) КАК ВидЦены,
		               |	СУММА(т.КоличествоПервоначально) КАК КоличествоПервоначально,
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	МАКСИМУМ(ЗаказКлиентагф_ТоварыВКоробах.ЦенаКороба) КАК ЦенаКороба,
	               |	МАКСИМУМ(ЗаказКлиентагф_ТоварыВКоробах.ЦенаКоробаСоСкидкой) КАК ЦенаКоробаСоСкидкой,
	               |	МАКСИМУМ(ЗаказКлиентагф_ТоварыВКоробах.СтавкаНДС) КАК СтавкаНДС,
	               |	МАКСИМУМ(ЗаказКлиентагф_ТоварыВКоробах.ВидЦены) КАК ВидЦены,
...
Процедура ЗаполнитьТЧВПарах(ИмяТЧ, НомерВерсии = 1)
	...
	Если ТипЗнч(ВерсияОбъекта) = Тип("Структура") Тогда
		Для Каждого СтрокаТЧ Из ВерсияОбъекта.ТабличныеЧасти["Товары"] Цикл
			...
			КонецЕсли;
		КонецЦикла;
		// ++ СадомцевСА 27.02.2024 Реализовал сворачивание данных заказа по Номенклатуре/Характеристике
		ТЗ = Объект[ИмяТЧ].Выгрузить();
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |	т.Цена КАК Цена,
		               |	т.гф_ЦенаСоСкидкой КАК гф_ЦенаСоСкидкой,
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	МАКСИМУМ(т.гф_ЦенаСоСкидкой) КАК гф_ЦенаСоСкидкой,
		               ...
		               |	МАКСИМУМ(т.Отменено) КАК Отменено,
		               |	МАКСИМУМ(т.Склад) КАК Склад
		               |ИЗ
		               |	втТЧ КАК т
		               |
		               |СГРУППИРОВАТЬ ПО
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	МАКСИМУМ(ЗаказКлиентаТовары.гф_ЦенаСоСкидкой) КАК гф_ЦенаСоСкидкой,
	               |	МАКСИМУМ(ЗаказКлиентаТовары.СтавкаНДС) КАК СтавкаНДС,
	               |	МАКСИМУМ(ЗаказКлиентаТовары.ВидЦены) КАК ВидЦены,
	               ...
	               |	МАКСИМУМ(ЗаказКлиентаТовары.Отменено) КАК Отменено,
	               |	МАКСИМУМ(ЗаказКлиентаТовары.Склад) КАК Склад
	               |ИЗ
	...
	ЗаполнитьДобавленоУдалено(ИмяТЧ);
   	// ++ СадомцевСА 27.02.2024
	ТЗ = Объект[ИмяТЧ].Выгрузить();
	ТЗ.Сортировать("Номенклатура, Характеристика");
	Объект[ИмяТЧ].Загрузить(ТЗ);
   	// -- СадомцевСА 27.02.2024
КонецПроцедуры
...
Процедура РезервыПриИзменении(Элемент)
	...
	ПересчитатьИтогиПоКолонкам("Резервы");
КонецПроцедуры
...
&НаКлиенте
Процедура ПересчитатьмТоварВРезерве()
	// СадомцевСА 19.02.2024
	ИмяТЧ = ОпределитьИмяТЧ();
	мКоличествоСИзменениямиПоЗаказу = 0;
	мТоварВРезерве = 0;
	мДоступноНаСкладе = 0;
	Если Не ЗначениеЗаполнено(ИмяТЧ) Тогда
		Возврат;
	КонецЕсли;
	Для Каждого СтрокаТЧ Из Объект[ИмяТЧ] Цикл
		мКоличествоСИзменениямиПоЗаказу = мКоличествоСИзменениямиПоЗаказу + СтрокаТЧ.КоличествоСИзменениями;
		Если ИмяТЧ = "Резервы" ИЛИ ИмяТЧ = "РезервыВКоробах" Тогда
			мТоварВРезерве = мТоварВРезерве + СтрокаТЧ.ТоварВРезерве;
			мДоступноНаСкладе = мДоступноНаСкладе + СтрокаТЧ.ДоступноНаСкладе;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура ПересчитатьИтогиПоКолонкам(ИмяТЧ = "")
	// СадомцевСА 28.02.2024 Реализовал итоги по колонкам с количеством
	Если Не ЗначениеЗаполнено(ИмяТЧ) Тогда
		ИмяТЧ = ОпределитьИмяТЧ();
	КонецЕсли;
	мЗарезервировать = 0;
	мСнятьРезерв = 0;
	мПеренестиРезерв = 0;
	мПеренестиВДругойЗаказ = 0;
	Если Не ЗначениеЗаполнено(ИмяТЧ) Тогда
		Возврат;
	КонецЕсли;
	Для Каждого СтрокаТЧ Из Объект[ИмяТЧ] Цикл
		мЗарезервировать = мЗарезервировать + СтрокаТЧ.Зарезервировать;
		мСнятьРезерв = мСнятьРезерв + СтрокаТЧ.СнятьРезерв;
		мПеренестиРезерв = мПеренестиРезерв + СтрокаТЧ.ПеренестиРезерв;
		мПеренестиВДругойЗаказ = мПеренестиВДругойЗаказ + СтрокаТЧ.ПеренестиВДругойЗаказ;
	КонецЦикла;
КонецПроцедуры
...
Процедура РезервыВКоробахПриИзменении(Элемент)
	...
	ПересчитатьИтогиПоКолонкам("РезервыВКоробах");
КонецПроцедуры
...
Процедура ЗаполнитьДоступными(Команда)
	...
	Для Каждого СтрокаТЗ Из Объект[ИмяТЧ] Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ПересчитатьИтогиПоКолонкам(ИмяТЧ);
КонецПроцедуры
...
Процедура ОбработатьРезультатНаСервере(Результат, ДополнительныеПараметры)
	...
	Если ИмяРеквизита = "ПеренестиНовоеНазначение" Тогда
		Для Каждого СтрокаТЧ Из Объект[ИмяТЧ] Цикл
			...
			// СадомцевСА 18.09.2023 Реализовал "Перенос в другой заказ" "всего количества" коробов/пар из текущего заказа.
			// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee51757e267b8e (п.3)
			// СтрокаТЧ[ИмяРеквизитаКоличество] = СтрокаТЧ.ТоварВРезерве;
			СтрокаТЧ[ИмяРеквизитаКоличество] = СтрокаТЧ.КоличествоСИзменениями;
		КонецЦикла;
		Возврат;
	КонецЕсли;
...
// vvv Галфинд \ Sakovich 12.09.2023
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee514f19220e24
&НаКлиенте
Процедура ОткрытьСписокКорректировок(Команда)
	ОткрытьФорму("Документ.КорректировкаНазначенияТоваров.ФормаСписка");
КонецПроцедуры // ^^^ Галфинд \ Sakovich 12.09.2023

&НаСервере
Функция ПолучитьСвободныеУЛ()
...
&НаСервереБезКонтекста
Функция СравнитьТаблицы(КЗ_1, КЗ_2, Колонка)
	ТаблицыИдентичны = Истина;
	Если ТипЗнч(КЗ_1) = Тип("ДанныеФормыКоллекция") Тогда
		ТЗ_1 = КЗ_1.Выгрузить();
	Иначе
		ТЗ_1 = КЗ_1.Скопировать();
	КонецЕсли;
	Если ТипЗнч(КЗ_2) = Тип("ДанныеФормыКоллекция") Тогда
		ТЗ_2 = КЗ_2.Выгрузить();
	Иначе
		ТЗ_2 = КЗ_2.Скопировать();
	КонецЕсли;
	// сначала ищем строки ТЗ_1 в ТЗ_2
	Для Каждого Стр Из ТЗ_1 Цикл
		НайденнаяСтрока = ТЗ_2.Найти(Стр[Колонка], Колонка);
	...
	Если ТаблицыИдентичны Тогда
		// затем ищем строки ТЗ_2 в ТЗ_2
		Для Каждого Стр Из ТЗ_2 Цикл
			НайденнаяСтрока = ТЗ_1.Найти(Стр[Колонка], Колонка);
			Если НайденнаяСтрока = Неопределено Тогда
				ТаблицыИдентичны = Ложь;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	Возврат ТаблицыИдентичны;
КонецФункции

&НаСервере
Процедура ТЕСТНаСервере(ВариантКомплектации)
...
Процедура ТЕСТ(Команда)
	ТД = Элементы.РезервыВКоробах.ТекущиеДанные;
	Если ТД <> Неопределено Тогда
		ТЕСТНаСервере(ТД["ВариантКомплектации"]);
	КонецЕсли;
КонецПроцедуры
