...
Процедура СоздатьКорректировкуСнятиеРезерваНаСервере()
	...
	Для Каждого СтрокаРезерва Из Объект.РезервыВКоробах Цикл
		Если ЗначениеЗаполнено(СтрокаРезерва["СнятьРезерв"]) Тогда
			...
			УпаковочныеЛисты = мУпаковочныеЛисты.НайтиСтроки(Новый Структура("ВариантКомплектации",
			СтрокаРезерва["ВариантКомплектации"]));
...
Процедура СоздатьКорректировкуРезервированиеНаСервере()
	...
	Для Каждого СтрокаРезерва Из Объект.РезервыВКоробах Цикл
		Если ЗначениеЗаполнено(СтрокаРезерва["Зарезервировать"]) Тогда
			...
			СтрокиУпаковочныхЛистов = мСвободныеУпаковочныеЛисты.НайтиСтроки(Новый Структура("ВариантКомплектации",
				СтрокаРезерва["ВариантКомплектации"]));
			Сч = 0;
...
Процедура ЗаполнитьРезервыВКоробахНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				И Склад = &Склад
	               |				И КМ.ТипУпаковки = ЗНАЧЕНИЕ(Перечисление.ТипыУпаковок.МультитоварнаяУпаковка)
	               |					) КАК гф_ДвижениеКодовМаркировкиОрганизацииОстатки
