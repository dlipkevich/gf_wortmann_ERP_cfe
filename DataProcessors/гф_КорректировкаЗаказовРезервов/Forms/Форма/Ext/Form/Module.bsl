...
&НаКлиенте
Процедура ПересчитатьКоличествоПоСтрокеВКоробах(ИмяТЧ, ПолеДобавить, ПолеУдалить, ПолеПеренестиВДругойЗаказ = "")
	ТД = Элементы[ИмяТЧ].ТекущиеДанные;
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ДобавитьСтрокуПричиныИзменения(ЗаказОбъект, СтрокаЗаказа, мКоличествоДоИзменения = 0, мПричинаУдаления = Неопределено)
...
Процедура ЗаполнитьТЧВКоробах(ИмяТЧ, НомерВерсии = 1)
	...
	Если ТипЗнч(ВерсияОбъекта) = Тип("Структура") Тогда
		...
		ТЗ.Свернуть("ВариантКомплектации, Скидка", "КоличествоПервоначально, Сумма");
...
Процедура ЗаполнитьТЧВПарах(ИмяТЧ, НомерВерсии = 1)
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.Отменено Тогда
			Продолжить;
		КонецЕсли;
		СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, гф_ЦенаСоСкидкой, СтавкаНДС");
...
&НаКлиенте
Процедура ПересчитатьСуммуСНДСвСтрокеТаблицы(ИмяТЧ)
	ТД = Элементы[ИмяТЧ].ТекущиеДанные;
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьСтавкуНДСвСтрокеТаблицы(ИмяТЧ)
	ТД = Элементы[ИмяТЧ].ТекущиеДанные;
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПересчитатьСуммуСНДСвСтрокеТаблицыВКоробах(ИмяТЧ)
	ТД = Элементы[ИмяТЧ].ТекущиеДанные;
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;
