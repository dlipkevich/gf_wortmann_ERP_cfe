...
Процедура ЗаполнитьДоступныйОстатокНаСкладе(ТаблицаНоменклатуры, СтрокаУЛ, ТаблицаОстатки)
	СтуктураПоиска = Новый Структура("ВариантКомплектации, УпаковочныйЛист");
	ЗаполнитьЗначенияСвойств(СтуктураПоиска, СтрокаУЛ);
	СтрокиНоменклатуры = ТаблицаНоменклатуры.НайтиСтроки(СтуктураПоиска);
	СтуктураПоискаОстатков = Новый Структура("Номенклатура, Характеристика");
	Для Каждого СтрокаНоменклатуры Из СтрокиНоменклатуры Цикл
		ЗаполнитьЗначенияСвойств(СтуктураПоискаОстатков, СтрокаНоменклатуры);
		СтрокиОстатков = ТаблицаОстатки.НайтиСтроки(СтуктураПоискаОстатков);
...
Процедура ЗаполнитьРезервыВКоробахНаСервере()
	...
	Для Каждого СтрокаРезерва Из Объект.РезервыВКоробах Цикл
		...
		Если Выборка.НайтиСледующий(СтруктураПоиска) Тогда
			...
			ОтборПоВариантуКомплектации = Новый Структура("ВариантКомплектации, флНеХватаетДоступныхОстатков", СтрокаРезерва["ВариантКомплектации"], Ложь);
