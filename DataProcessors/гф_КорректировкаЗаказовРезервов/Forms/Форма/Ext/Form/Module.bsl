...
Процедура ЗаполнитьТЧТоварыВКоробах(ЗаказОбъект, ТоварыВКоробах)
	Для Каждого СтрокаТЧ Из ТоварыВКоробах Цикл
		Если ЗначениеЗаполнено(СтрокаТЧ["Добавить"]) Тогда
			...
			СтрокаЗаказа["ЦенаКоробаСоСкидкой"] = СтрокаЗаказа.ЦенаКороба * (1 - СтрокаЗаказа.Скидка/100);
			...
			КонецЕсли;
			СтрокаЗаказа["СуммаНДС"] = СтрокаЗаказа["Сумма"] * (Ставка/100);
...
&НаСервереБезКонтекста
Функция гф_ЗаполнитьСтавкуНДС(ВариантКомплектации)
