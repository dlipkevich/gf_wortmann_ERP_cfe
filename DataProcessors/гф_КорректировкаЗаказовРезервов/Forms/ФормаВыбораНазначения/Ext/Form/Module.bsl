...
Процедура ЗаполнитьСписокНазначенийВКоробах(Запрос)
	...
	Для Каждого СтрокаТЗ Из СписокНазначений Цикл
		...
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee51757e267b8e
		// для оптимизации впоследствии нужно вычислять в запросе
		СтрокаТЗ["КоличествоУпаковокЗаказа"]= СтрокаТЗ.Заказ.гф_ТоварыВКоробах.Итог("Количество");
		// ^^^ Галфинд \ Sakovich 13.09.2023
	КонецЦикла;
...
Процедура ЗаполнитьСписокНазначений()
	...
	Если флПеренестиНовоеНазначение Тогда
		Запрос.Текст = ТекстВтНазначения();
		Запрос.Выполнить();
		// втНазначения
	...
	Запрос.Текст = ТекстВтЗаказыТовары();
	...
	Для Каждого СтрокаТЗ Из СписокНазначений Цикл
		...
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee51757e267b8e
		// для оптимизации впоследствии нужно вычислять в запросе
		СтрокаТЗ["КоличествоУпаковокЗаказа"]= СтрокаТЗ.Заказ.Товары.Итог("КоличествоУпаковок");
		// ^^^ Галфинд \ Sakovich 13.09.2023
	КонецЦикла;
