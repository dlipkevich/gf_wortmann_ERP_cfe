...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если флТоварыВКоробах Тогда
		Элементы.ДанныеФайлаАртикул.Видимость = Ложь;
		Элементы.ДанныеФайлаНоменклатура.Видимость = Ложь;
		Элементы.ДанныеФайлаХарактеристика.Видимость = Ложь;
		Элементы.ДанныеФайлаХарактеристикаНаименование.Видимость = Ложь;
	Иначе
		Элементы.ДанныеФайлаВариантКомплектацииНаименование.Видимость = Ложь;
...
Функция ПолучитьОсновнойВариантКомплектации(Номенклатура, Характеристика, флДобавитьСтрокуФайла, j)
	...
	Если Не ЗначениеЗаполнено(ВариантКомплектации) Тогда
		ТекстСообщения = "Не найден вариант комплектации для номенклатуры "
							...
							+ Строка(Характеристика) + ". Данные по строке XLS "
							+ Строка(j) + "файла XLS не загружены.";
		СообщитьПользователю(ТекстСообщения);
		флДобавитьСтрокуФайла = Ложь;
	КонецЕсли;
...
Функция НайтиВариантКомплектации(Наименование, флДобавитьСтрокуФайла, j)
	...
	Иначе
		ТекстСообщения = "Не найден Вариант комплектации по наименованию:"
							+ Строка(Наименование) + ". Данные по строке XLS "
							+ Строка(j) + "файла XLS не загружены.";
		СообщитьПользователю(ТекстСообщения);
		флДобавитьСтрокуФайла = Ложь;
		Возврат Справочники.ВариантыКомплектацииНоменклатуры.ПустаяСсылка();
...
&НаКлиенте
Процедура ПрочитатьФайлВариантКомплектации(Команда)
	...
	Для j = СтрокаЗагрузки По RCount Цикл
		...
		ВариантКомплектацииНаименование = ex.ActiveSheet.Cells(j, 1).Text;
		//Если флТоварыВКоробах Тогда
			ВариантКомплектации = НайтиВариантКомплектации(ВариантКомплектацииНаименование, флДобавитьСтрокуФайла, j);
		//КонецЕсли;
		
		Добавить = 0;
		_Добавить = СокрЛП(ex.ActiveSheet.Cells(j, 2).Value);
		...
		Удалить = 0;
		_Удалить = СокрЛП(ex.ActiveSheet.Cells(j, 3).Value);
		...
		НоваяСтрока.ВариантКомплектацииНаименование = ВариантКомплектацииНаименование;
...
Процедура ПрочитатьФайл(Команда)
	...
	Если флТоварыВКоробах Тогда
		ПрочитатьФайлВариантКомплектации(Команда);
	Иначе
		ПрочитатьФайлНоменклатура(Команда);
	КонецЕсли;
	
КонецПроцедуры  

&НаКлиенте
Процедура ПрочитатьФайлНоменклатура(Команда)
