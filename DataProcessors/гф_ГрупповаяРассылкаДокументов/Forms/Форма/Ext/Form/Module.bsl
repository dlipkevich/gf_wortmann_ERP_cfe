...
Процедура СохранитьПечатныеФормыНаДиск(Команда) 
	...
	Если Не ПроверитьВозможностьФормированияПисем() Тогда
		
		Возврат;	
		
	КонецЕсли;	
	
	ПоместитьПечатныеФормыВХранилище(); 
...
//
&НаКлиенте
Функция ПроверитьВозможностьФормированияПисем() 
	...
	Если МассивВыгружаемыхСтрок.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Нет документов для отправки");
	...
	Если ЕстьОтправленныеПисьма И Не Объект.ОтправлятьПовторно Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("В списке есть отмеченные строки с письмами не в статусе черновик
...
Процедура ВставитьПараметрыВТекстСообщения(Сообщение, МассивСтрок)
	...
	ВставитьПараметрВТекстСообщения(Сообщение, "Наименование документа", НаименованиеДокумента); 

	ВставитьПараметрВТекстСообщения(Сообщение, "Скан подписанного документа", НаименованиеПодписанного); 
	
	ВставитьПараметрВТекстСообщения(Сообщение, "Адрес отправителя", Объект.АдресОтправителя); 
...
//
&НаСервере
Процедура ПоместитьПечатныеФормыВХранилище() 
	...
	Для Каждого ВыгружаемаяСтрока Из МассивВыгружаемыхСтрок Цикл 
		...
		КомандыПечатиОбъекта = УправлениеПечатью.КомандыПечатиОбъектаДоступныеДляВложений(Метаданные.Документы[Объект.ТипДокумента]); 
		
		КомандаПечатиОбъекта = КомандыПечатиОбъекта.Найти(Объект.ПечатнаяФорма, "Идентификатор");

		КомандаПечати = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(КомандаПечатиОбъекта);
		...
		ДанныеФайла = УправлениеПечатью.НапечататьВФайл(КомандаПечати, МассивОбъектов, НастройкиСохранения);
		
		ВыгружаемаяСтрока.Адрес = ПоместитьВоВременноеХранилище(ДанныеФайла[0].ДвоичныеДанные, УникальныйИдентификатор);
...
Процедура СоздатьИлиЗаполнитьПисьмо(ПисьмоСсылка, Сообщение, МассивСтрок)
	...
	Письмо.ОтправительПредставление = Объект.АдресОтправителя;
	Письмо.Дата						= ТекущаяДатаСеанса();
	...
	МассивПолучателейКопий = СтрРазделить(Объект.ПоставитьВКопию, ";", Ложь);
	...
	Для Каждого СтрокаМассива Из МассивСтрок Цикл
		...
		ПрикрепленныйДокумент.Документ = СтрокаМассива.Документ;
		
		СтрокаМассива.Письмо = Письмо.Ссылка;
