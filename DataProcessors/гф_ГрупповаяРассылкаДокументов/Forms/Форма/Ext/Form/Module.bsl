...
Процедура ТипДокументаПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.ТипДокумента) Тогда
		...
		ОтображатьОтправленныеПриИзмененииНаСервере();
...
//
&НаСервере
Процедура ОтображатьОтправленныеПриИзмененииНаСервере()
	
	ПараметрОтображатьОтправленные = 
		КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ОтображатьОтправленные");
	
	ПараметрОтображатьОтправленные.Значение = Объект.ОтображатьОтправленные;
...
//
&НаКлиенте
Процедура ОтображатьОтправленныеПриИзменении(Элемент)
...
Процедура СписокДокументовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	УсловиеНаИмя = Поле.Имя = "СписокДокументовПисьмо"
...
Процедура ПроверитьВозможностьФормированияПисем(Ошибки)
	...
	Если Не МассивВыгружаемыхСтрок.Количество() Тогда
	...
	ПроверитьАдреса(МассивВыгружаемыхСтрок, Ошибки);
...
//
//	Параметры:
//  МассивВыгружаемыхСтрок	- Массив
//  Ошибки 					- Массив
//
&НаКлиенте
Процедура ПроверитьАдреса(МассивВыгружаемыхСтрок, Ошибки)
	...
	Для Каждого ВыгружаемаяСтрока Из МассивВыгружаемыхСтрок Цикл
		...
		Если ЗначениеЗаполнено(ВыгружаемаяСтрока.СтатусПисьма)
			И ВыгружаемаяСтрока.СтатусПисьма 
	...
	Если КонтрагентыБезАдресов.Количество() Тогда
		
		Для Каждого Контрагент Из КонтрагентыБезАдресов Цикл       
			
			СтрокаСообщения = СтрШаблон("Не заполнен Emai контрагента %1", Контрагент);
...
Процедура СформироватьПисьма(Команда)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьОшибкиПользователю(Ошибки, Отказ);
	// Закомментировала Галфинд_ДомнышеваКР_06_02_2024
	//// ++ ЕсиповАВ Галфинд 05.02.24
	//гф_ЗаполнитьЗначениеПолучателяКопии();
	//// -- ЕсиповАВ Галфинд 05.02.24
...
&НаСервере
Процедура гф_ЗаполнитьЗначениеПолучателяКопии()
	
	КолТабл = ЭтотОбъект.СписокДокументов.Количество();
	Если Не КолТабл = 0 Тогда
		Для каждого Стр Из ЭтотОбъект.СписокДокументов Цикл
			Запрос = Новый Запрос;
			Запрос.Текст =
			"ВЫБРАТЬ
			|	Контрагенты.гф_ОтветственныйМенеджер.(
			|		Ссылка КАК Ссылка,
			|		НомерСтроки КАК НомерСтроки,
			|		Организация КАК Организация,
			|		Пользователь КАК Пользователь
			|	) КАК гф_ОтветственныйМенеджер
			|ИЗ
			|	Справочник.Контрагенты КАК Контрагенты
			|ГДЕ
			|	Контрагенты.Ссылка = &Ссылка";
			Запрос.Параметры.Вставить("Ссылка",Стр.Контрагент);
			Таб = Запрос.Выполнить().Выгрузить();
			Если Не Таб.Количество() = 0 Тогда
				Для каждого СтрТаб Из Таб[0].гф_ОтветственныйМенеджер Цикл
					Если СтрТаб.Организация = Объект.Организация  И Не СтрТаб.Пользователь.КонтактнаяИнформация.Количество() = 0 Тогда
					Объект.ПоставитьВКопию = СтрТаб.Пользователь.КонтактнаяИнформация[0].АдресЭП;
					Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Процедура КомандаПоместитьВИсходящиеНаСервере()
	...
	Черновик = ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Черновик");
	
	ОтборСтрок.Вставить("СтатусПисьма", Черновик);
...
Процедура ЗаполнитьСписокВыбораТиповДокументов()
	...
	Для Каждого СтрокаИсточников Из ТаблицаИсточников Цикл
		...
		Условие = Метаданные.Документы.Содержит(Документ)
...
Процедура ЗаполнитьСписокВыбораПечатныхФорм()
	...
	Если ЗначениеЗаполнено(Объект.ТипДокумента) Тогда
		...
		Если Объект.ТипДокумента = "СчетНаОплатуКлиенту" Тогда
			МассивУдаляемых = Новый Массив;
			Для каждого Стр Из Объект.ПечатныеФормы Цикл
				Если Стр.Представление = "Счет на оплату" ИЛИ Стр.Представление = "Счет на оплату с факсимиле" Тогда
					 МассивУдаляемых.Добавить(Стр);
				 КонецЕсли;
			 КонецЦикла;
			 
			 Для каждого Стр Из МассивУдаляемых Цикл 
				  Объект.ПечатныеФормы.Удалить(Стр);
			КонецЦикла;
...
Процедура УстановитьКомпоновку()
	...
	ЭлементГруппировка = 
	СхемаКомпоновки.ВариантыНастроек[0].Настройки.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных"));
	...
	Для Каждого ПолеВыбора Из КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора.Элементы Цикл
		...
		ВыбранноеПоле = 
		СхемаКомпоновки.ВариантыНастроек[0].Настройки.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
	...
	ПолеПорядка = 
	СхемаКомпоновки.ВариантыНастроек[0].Настройки.Порядок.Элементы.Добавить(Тип("ЭлементПорядкаКомпоновкиДанных"));
...
Функция ТекстЗапросаДокументов(ТипДокумента)
	...
	Результат = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	            ...
	            |ПОМЕСТИТЬ ВТ_МаксимальныеДатыДокументов
	            |ИЗ
	            ...
	            |////////////////////////////////////////////////////////////////////////////////
	            |ВЫБРАТЬ
	            |	ВТ_МаксимальныеДатыДокументов.Документ КАК Документ,
	            ...
	            |ПОМЕСТИТЬ ВТ_Письма
	            |ИЗ
	            |	ВТ_МаксимальныеДатыДокументов КАК ВТ_МаксимальныеДатыДокументов
	            |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ВсеПисьма КАК ВТ_Письма
	            |		ПО ВТ_МаксимальныеДатыДокументов.Дата = ВТ_Письма.Дата
	            |			И ВТ_МаксимальныеДатыДокументов.Документ = ВТ_Письма.Документ
	            |;
	            |
	            |////////////////////////////////////////////////////////////////////////////////
	            |ВЫБРАТЬ
	            ...
	            |	ВТ_Документы.Email,
	            |	ВТ_Письма.Письмо,
	            |	ВТ_Письма.СтатусПисьма,
	            |	ВТ_Письма.ДатаОтправления,
	            |	ВТ_Письма.Дата,
	            |	ВТ_Письма.Тема
	            |ИЗ
	            |	ВТ_Документы КАК ВТ_Документы
	            ...
	            |		ПО ВТ_Документы.Документ = ВТ_Письма.Документ
	            |ГДЕ
	            |	ВТ_Письма.СтатусПисьма = ЗНАЧЕНИЕ(Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Черновик)
	            |
	            |ОБЪЕДИНИТЬ ВСЕ
	            |
	            |ВЫБРАТЬ
	            ...
	            |	ВТ_Результат.Email КАК Email,
	            |	ВТ_Результат.Письмо КАК Письмо,
	            |	ВТ_Результат.СтатусПисьма КАК СтатусПисьма,
	            |	ВТ_Результат.ДатаОтправления КАК ДатаОтправления,
	            |	ВТ_Результат.Дата КАК Дата,
	            |	ВТ_Результат.Тема КАК Тема
	            |ИЗ
	            |	ВТ_Результат КАК ВТ_Результат
	            |
	            |СГРУППИРОВАТЬ ПО
	            |	ВТ_Результат.ДатаОтправления,
	            |	ВТ_Результат.Дата,
	            |	ВТ_Результат.Тема,
	            |	ВТ_Результат.СтатусПисьма,
	            |	ВТ_Результат.Email,
	            |	ВТ_Результат.Письмо,
...
Процедура ОтобратьНаСервере()
	...
	Для Каждого СтрокаДокумента Из СписокДокументов Цикл
		...
		СтрокаДокумента.Отметка  = Истина;
		СтрокаДокумента.ИмяФайла = СтрокаДокумента.Документ.Номер 
		+ "_" 
		+ ?(ЗначениеЗаполнено(СтрокаДокумента.Email), СтрокаДокумента.Email, "НетEmail") 
		+ "_" 
...
Функция ИмяПеречисления(ЗначениеПеречисления)
	...
	ИндексЗначенияПеречисления = 
		Перечисления[ИмяПеречисления].Индекс(ЗначениеПеречисления);
	
	ИмяЗначенияПеречисления = 
		Метаданные.Перечисления[ИмяПеречисления].ЗначенияПеречисления[ИндексЗначенияПеречисления].Имя;
...
Процедура ВставитьПараметрыВТекстСообщения(Сообщение, МассивСтрок)
	...
	НаименованиеПодписанного = СклонениеПредставленийОбъектов.ПросклонятьПредставление(НаименованиеПодписанного,
	Падеж,
	Неопределено);
	...
	НаименованиеДокумента = СклонениеПредставленийОбъектов.ПросклонятьПредставление(НаименованиеДОкумента,
	Падеж,
	Неопределено);
	
	гф_РассылкаПечатныхФорм.ВставитьПараметрВТекстСообщения(Сообщение, "Наименование документа", НаименованиеДокумента);
	
	гф_РассылкаПечатныхФорм.ВставитьПараметрВТекстСообщения(Сообщение,
	"Скан подписанного документа",
	НаименованиеПодписанного);
	...
	гф_РассылкаПечатныхФорм.ВставитьПараметрВТекстСообщения( Сообщение, "Адрес офиса", ЮридическийАдрес);
	
	Телефоны = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "Телефоны");
	
	гф_РассылкаПечатныхФорм.ВставитьПараметрВТекстСообщения(Сообщение, "Телефон организации", Телефоны);
...
Процедура ПоместитьПечатныеФормыВХранилище(Ошибки)
	...
	Для Каждого ВыгружаемаяСтрока Из МассивВыгружаемыхСтрок Цикл
		...
		// ++ Галфинд СадомцевСА 19.04.2024
		// Реализовал добавление печ. формы Счета при отправке по почте печ. формы Спецификации Групповой рассылкой документов
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eefcd193bae294
		// здесь надо дополнить Таблицу печатных форм Счетами на оплату для документа Спецификация
		Если ТипЗнч(ВыгружаемаяСтрока.Документ) = Тип("ДокументСсылка.гф_СпецификацияПокупателя") Тогда
			гф_ДополнитьТаблицаПечатныхФорм(ТаблицаПечатныхФорм, ВыгружаемаяСтрока.Документ, ФорматФайла, ВыгружаемаяСтрока.Email);
		КонецЕсли;
		// -- Галфинд СадомцевСА 19.04.2024
...
Процедура СформироватьПисьмо(МассивСтрок)
	...
	ТаблицаФайлов = ПолучитьТаблицуФайлов(МассивСтрок);
	...
	ОтветственныйМенеджер = ПолучитьПочтуОтветственногоМенеджера(Предмет);
	Если ОтветственныйМенеджер <> Неопределено Тогда
		ПочтаОтветственныйМенеджер = СтрСоединить(ОтветственныйМенеджер, "; "); // Галфинд_ДомнышеваКР_14_02_2024
		АдресаКопии = Объект.ПоставитьВКопию + ";" + ПочтаОтветственныйМенеджер;
	Иначе
		АдресаКопии = Объект.ПоставитьВКопию;
	КонецЕсли;
	// Галфинд_ДомнышеваКР_06_02_2024
	
	гф_РассылкаПечатныхФорм.СоздатьИлиЗаполнитьПисьмо(Письмо,
	Сообщение,
	МассивДокументов,
	Объект.УчетнаяЗапись,
	Объект.АдресОтправителя,
	// ++ Галфинд_ДомнышеваКР_06_02_2024
	//Объект.ПоставитьВКопию); 
	АдресаКопии);
	// -- Галфинд_ДомнышеваКР_06_02_2024	
...
//
&НаСервере
Функция ПолучитьТаблицуФайлов(МассивСтрок)
...
// #wortmann {
// Подтягивание данных в копию получателя
// Галфинд ДомнышеваКР_06_02_2024
//
// Параметры:
// Контрагент - СправочникСсылка.Контрагенты 
//
// Возвращаемое значение:
//	- Строка или Неопределено - значение почтового ящика
//
&НаСервере
Функция ПолучитьПочтуОтветственногоМенеджера(Контрагент)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Контрагентыгф_ОтветственныйМенеджер.Ссылка КАК Ссылка,
		|	Контрагентыгф_ОтветственныйМенеджер.Пользователь КАК Пользователь
		|ПОМЕСТИТЬ Пользователи
		|ИЗ
		|	Справочник.Контрагенты.гф_ОтветственныйМенеджер КАК Контрагентыгф_ОтветственныйМенеджер
		|ГДЕ
		|	Контрагентыгф_ОтветственныйМенеджер.Организация = &Организация
		|	И Контрагентыгф_ОтветственныйМенеджер.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		// ++ Галфинд_ДомнышеваКР_14_02_2024
		// по новому решению в ТЧ Ответственный менеджер по одной организации может быть несколько записей Пользователей.
		//Выборка.Следующий();
		//Возврат Выборка.АдресЭП;
		АдресаМенеджеров = Новый Массив;
		Пока Выборка.Следующий() Цикл
			АдресаМенеджеров.Добавить(Выборка.АдресЭП);
		КонецЦикла; 
		Возврат АдресаМенеджеров;
		// -- Галфинд_ДомнышеваКР_14_02_2024
	Иначе
...
&НаСервере
Процедура гф_ДополнитьТаблицаПечатныхФорм(ТаблицаПечатныхФорм, Документ, ФорматФайла, Email)
	мСчетаНаОплату = гф_НайтиСчетаНаОплату(Документ);
	...
	Для Каждого СчетНаОплату Из мСчетаНаОплату Цикл
		СтрокаПечатнойФормы = ТаблицаПечатныхФорм.Добавить();
		СтрокаПечатнойФормы.Документ = СчетНаОплату;
		СтрокаПечатнойФормы.ИдентификаторПечатнойФормы = "СчетНаОплату";
		СтрокаПечатнойФормы.ПредставлениеПечатнойФормы = мПечатнаяФормаСчетаНаОплату;
		СтрокаПечатнойФормы.ФорматФайла = ФорматФайла;
		СтрокаПечатнойФормы.ИмяФайла = СтрЗаменить(мПечатнаяФормаСчетаНаОплату, " ", "_")
			+ "_" + СчетНаОплату.Номер 
			+ "_" 
			+ ?(ЗначениеЗаполнено(Email), Email, "НетEmail") 
			+ "_" 
			+ Формат(СчетНаОплату.Дата, "ДФ=yyyyMMdd")
			+ "." + ИмяПеречисления(Объект.ФорматВложений);
	КонецЦикла;
КонецПроцедуры

&НаСервере
Функция гф_НайтиСчетаНаОплату(Спецификация)
	Запрос = Новый Запрос;
	...
	Запрос.УстановитьПараметр("гф_СпецификацияПокупателя", Спецификация);
