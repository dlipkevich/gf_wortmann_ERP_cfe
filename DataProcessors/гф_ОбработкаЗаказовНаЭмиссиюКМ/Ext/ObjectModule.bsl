...
Процедура СоздатьДокументыЗаказНаЭмиссиюКодовМаркировки(РезультатОбработки) Экспорт
	...
	// vvv Галфинд \ Sakovich 25.03.2024
	...
	Если Пользователи.ЭтоПолноправныйПользователь( , Истина) Тогда
		ОбновитьНумерациюОбъектов(Метаданные.Документы.ЗаказНаЭмиссиюКодовМаркировкиСУЗ);
	КонецЕсли;
	// ^^^ Галфинд \ Sakovich 25.03.2024
	...
	Для каждого Эл Из ИтоговыйМассивДляОбработки Цикл
		...
		Для каждого стрТч Из тчТовары Цикл
			...
			// ++ СадомцевСА 14.03.2024 Добавил условие по Виду номенклатуры при создании Заказов на эмиссию КМ регл. заданием
			// Перечень видов номенклатуры указывается в глобальном значении "Виды номенклатуры для маркировки"
			// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eee14c221f3bcc
			
			//вынесено из цикла  Галфинд \ Sakovich 25.03.2024
			//СписокВидовНоменклатуры = _омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначениеМассив("гф_ВидыНоменклатурыДляМаркировки");
			
			ВидНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(стрТч["Номенклатура"], "ВидНоменклатуры");
			Если СписокВидовНоменклатуры.Найти(ВидНоменклатуры) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			// -- СадомцевСА 14.03.2024
...
Процедура УстановитьСтатусыКМЭмитированыПТУ(флСоздатьЗаполнитьОрдера, МассивПТУ)
	// Галфинд СадомцевСА 27.11.2023 Добавил параметр флСоздатьЗаполнитьОрдера
	// ++ Галфинд СадомцевСА 04.12.2023 Добавил параметр МассивПТУ
	Если МассивПТУ <> Неопределено Тогда
		Если флСоздатьЗаполнитьОрдера Тогда
			СоздатьЗаполнитьОрдераПоПТУ(МассивПТУ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьСтатусыКМЭмитированыЗаказыПоставщикам()
	...
	|	МАКСИМУМ(ДанныеСравнения.КоличествоКМХватает) КАК КоличествоКМХватает
	|ИЗ
	|	ДанныеСравнения КАК ДанныеСравнения
	|
	|СГРУППИРОВАТЬ ПО
	...
	|
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(ДанныеСравнения.КоличествоКМХватает) = ИСТИНА");
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если обЗаказ <> Неопределено Тогда
				...
				// vvv Галфинд \ Sakovich 13.12.2023
				АгрегироватьУлВЗаказеПоставщику(Выборка["Заказ"]);
				
				// после агрегации Ул нужно перечитать объект
				обЗаказ.Прочитать();
				
				УЛПоВариантуСозданыВсе = Истина;
				Менеджер = Документы.ЗаказПоставщику;
				Для каждого стрТЗ Из обЗаказ.гф_ПродукцияВКоробах Цикл
					Если Не Менеджер.УпаковочныеЛистыСозданыВсе(Выборка["Заказ"], стрТз["ВариантКомплектации"], стрТз["КоличествоКоробов"])  Тогда
						УЛПоВариантуСозданыВсе = Ложь;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				
				Если УЛПоВариантуСозданыВсе Тогда
					обЗаказ.гф_КМЭмитированы = Истина;
				КонецЕсли;
				// ^^^ Галфинд \ Sakovich 13.12.2023	
...
// vvv Галфинд \ Sakovich 08.11.2023, 13.12.2023
...
Процедура АгрегироватьУлВЗаказеПоставщику(ЗаказПоставщикуСсылка)
	
	Менеджер = Документы.ЗаказПоставщику;    
	лЗаказ = ЗаказПоставщикуСсылка;
	...
	Для каждого строкаУпЛист Из мСтрокДляСозданияУпЛистов Цикл
		...
		Если РезультатЗапроса.Пустой() Тогда
			...
			// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eef02a9735e5ef
		...
		// Добавлено  Галфинд \ Sakovich 27.03.2024
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeec18a6b213f4
		ДокУпЛист.ДополнительныеСвойства.Вставить("ЗаказПоставщику", ЗаказПоставщикуСсылка);
		...
		Если ЗначениеЗаполнено(СкладЭмиссииРЗ) Тогда
			ДокУпЛист.ДополнительныеСвойства.Вставить("ПараметрСклад", СкладЭмиссииРЗ);
		Иначе
		...
		Исключение  
			Если Не ЗначениеЗаполнено(ДокУпЛист.гф_Агрегация) Тогда
				...
				// vvv Галфинд \ Sakovich 02.04.2024
				// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eef02a9735e5ef
				НомерВМассивеЗадействованных = мЗадействованныхСерийныхНомеров.Найти(Выборка["Наименование"]);
				Если НомерВМассивеЗадействованных <> Неопределено Тогда
					мЗадействованныхСерийныхНомеров.Удалить(НомерВМассивеЗадействованных);
				КонецЕсли;
				// ^^^ Галфинд \ Sakovich 02.04.2024
	...
	тзIDКоробов.Сортировать("ВариантКомплектации, IDКороба"); // Галфинд_ДомнышеваКР_01_04_2024
...
КонецПроцедуры // ^^^ Галфинд \ Sakovich 08.11.2023, 13.12.2023


Процедура ПроверитьУстановитьСтатусыКМЭмитированы(флСоздатьЗаполнитьОрдера = Ложь, МассивПТУ = Неопределено) Экспорт
	// Галфинд СадомцевСА 04.12.2023 Добавил параметр МассивПТУ
	УстановитьСтатусыКМЭмитированыПТУ(флСоздатьЗаполнитьОрдера, МассивПТУ);
	// ++ Галфинд СадомцевСА 27.11.2023 Добавил параметр флСоздатьЗаполнитьОрдера
	Если флСоздатьЗаполнитьОрдера Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДополнительноОбработатьУЛПоДокументамСПризнакомКМЭмитированы() 
	...
	Пока Выборка.Следующий() Цикл
		...
		Если обУл <> Неопределено Тогда
			// vvv Галфинд \ Sakovich 27.03.2024
			// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeec18a6b213f4
			Если ТипЗнч(Выборка["Документ"]) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
				обУл.ДополнительныеСвойства.Вставить("РусскаяЗима", Истина);
				обУл.ДополнительныеСвойства.Вставить("ЗаказПоставщику", Выборка["Документ"]);
				Если ЗначениеЗаполнено(СкладЭмиссииРЗ) Тогда
					обУл.ДополнительныеСвойства.Вставить("ПараметрСклад", СкладЭмиссииРЗ);
				Иначе
					СкладЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка["Документ"], "Склад");
					обУл.ДополнительныеСвойства.Вставить("ПараметрСклад", СкладЗаказа);
				КонецЕсли;
			КонецЕсли;
			// ^^^ Галфинд \ Sakovich 27.03.2024
			Попытка
				обУл.Записать(РежимЗаписиДокумента.Проведение);
			Исключение
				ТекстОшибки = ОписаниеОшибки();
