...
Функция ПолучитьСлужебныйКонтейнер(ПараметрОбъект) Экспорт
	...
	Возврат Новый Структура("ИсходныйОбъект, Объект, ЭтоСтрока, ЭтоДокумент", СтрокаОбъект, ?(ЭтоСтрока, НРег(СтрокаОбъект), СтрокаОбъект), ЭтоСтрока, ЭтоДокумент);
...
Процедура ОбъектыПереводПриИзмененииНаСервере(СтрокаОбъект, Перевод)
	...
	Если Не ПустаяСтрока(Перевод) Тогда
		...
		Попытка
			...
			Объект.Объекты.НайтиСтроки(Отбор).Перевод = Перевод;
