...
Процедура УстановитьЗначения(Команда) 
	...
	Для Каждого стр Из Объект.Блокировка Цикл 
		...
		Если стр.Флаг Тогда
		Если Элементы.Блокировка.ТекущийЭлемент.Имя="БлокировкаКонтрагент" Тогда	
...
Процедура ВыполнитьНаСервере()
	...
	Для Каждого стр Из объект.Блокировка Цикл
		Если стр.Флаг Тогда
			ПроверкаНаУникальностьЗаписейПоПериоду(стр.Контрагент, стр.ДоговорКонтрагента, стр.ВидБлокировки,стр.Организация,стр.Заблокирован,стр.Исключения,стр.ДействуетДо); 
...
&НаСервере
Процедура ПроверкаНаУникальностьЗаписейПоПериоду(Контрагент, ДоговорКонтрагента, ВидБлокировки,Организация,Заблокирован,Исключения,ДействуетДо)
	...
	Запрос.Текст = 
		...
		|	И гф_ПричиныБлокировок.Контрагент = &Контрагент
