...
Процедура ВыполнитьСопоставлениеПрОрдеровИЗаявокНаВозврат(РезультатВыполнения) Экспорт
	...
	НеПринятыеСкладом = тзТовары.Скопировать(СтруНеПринятых); 
	
	Если НеПринятыеСкладом.Количество()>0 Тогда 
		
		СтруНЕОтмененных = Новый Структура("Отменено", Ложь);
		МассивНеОтмененных = НеПринятыеСкладом.НайтиСтроки(СтруНеОтмененных);
		Если МассивНеОтмененных.Количество()>0  Тогда
