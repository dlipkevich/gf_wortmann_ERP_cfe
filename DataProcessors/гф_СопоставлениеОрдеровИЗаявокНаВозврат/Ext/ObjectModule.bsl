...
Процедура ВыполнитьСопоставлениеПрОрдеровИЗаявокНаВозврат(РезультатВыполнения) Экспорт
	...
	Пока Связи.Следующий() Цикл
		...
		ПроверитьЗаявкуНаПринятСкладомИПисьмо(Связи["Заявка"]);
		// --Галфинд_ДомнышеваКР_07_06_2023
...
// e1cib/data/Задача.ЗадачаИсполнителя?ref=8ea8b083fed1320811ee037bc37e8488
// Галфинд_ДомнышеваКР_07_06_2023
Процедура ПроверитьЗаявкуНаПринятСкладомИПисьмо(Заявка) 
	
   Товары = Заявка.ВозвращаемыеТовары.Выгрузить();
   Для Каждого Стр Из Товары Цикл
	   Если Стр["гф_ПринятСкладом"] = Ложь Тогда
		   НаписатьИОтправитьПисьмо(Заявка);
		   Прервать;
	   КонецЕсли;
   КонецЦикла;
...
Процедура НаписатьИОтправитьПисьмо(Заявка) 
	...
	Письмо.Дата						= ТекущаяДатаСеанса();
	Письмо.Текст = "В заявке на возврат " + Заявка.Ссылка + " есть позиции не принятые складом.";
