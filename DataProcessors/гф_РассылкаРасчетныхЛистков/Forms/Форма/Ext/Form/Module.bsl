...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Справочники.ВидыКонтактнойИнформации.НайтиПоРеквизиту("ИдентификаторДляФормул", "гф_РассылкаРасчетныхЛистков");
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыРасчетныеЛистки  
...
Процедура Настройки(Команда) 
	...
	ОткрытьФорму(ИмяФормыНастроек, ПараметрыОткрытия, ЭтотОбъект, , , , 
...
Процедура СформироватьНаСервере()  
	...
	ТипГенератор = Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений");
	
	КомпоновщикМакета 	= Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКомпоновки 	= КомпоновщикМакета.Выполнить(СхемаКомпоновки, НастройкиКомпоновки, , , ТипГенератор);
	...
	Для Каждого СтрокаОрганизации Из ТаблицаОрганизаций Цикл 
		...
		Макет = ПолучитьОбщийМакет("гф_ПФ_MXL_РасчетныйЛистокНастраиваемыйДляРассылки");
		
		ОтчетРасчетныйЛисток.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Вставить(
		"МакетПечатнойФормы", Макет);
		
		ДокументРезультат = ЗарплатаКадрыОтчеты.ДанныеРасчетныхЛистков(
		МассивОбъектов, Организация, Период.ДатаНачала, Период.ДатаОкончания, 
		ОтчетРасчетныйЛисток, ПараметрыСтраницы).ДокументРезультат;  
		
		УстановитьПривилегированныйРежим(Ложь);  
		
		ПечатаемыйДокумент.Автомасштаб = Истина;
...
// В нашем случае нам нужен именно каталог	
...
// BSLLS:TempFilesDir-on	
	...
	Для Каждого Элемент Из МассивПолучателей Цикл
		...
		ЗаписьФайла = Новый ЗаписьZipФайла(ИмяФайлаАрхива, СокрЛП(ПрефиксПароля) + Прав(СНИЛС, 5)); 		
		
		ДобавитьДокументВФайл(Строка, ЗаписьФайла, ИмяКаталога);
		...
		ДанныеФайла = Новый ДвоичныеДанные(ИмяФайлаАрхива);
		
		Письмо.Вложения.Добавить(ДанныеФайла, ФайлАрхива.Имя);
		...
		Попытка
			...
			Запись.НаименованиеФайла = ФайлАрхива.Имя;
		...
		Исключение                                  
			...
			ТекстСообщения = МенеджерОбработкиОшибок.ПредставлениеОшибкиДляПользователя(ИнформацияОбОшибке());
...
&НаСервере
Процедура ДобавитьДокументВФайл(Строка, ЗаписьФайла, ИмяКаталога)
	...
	Для Каждого Область Из ПечатаемыйДокумент.Области Цикл
		...
		Если Сред(Область.Имя, 18, 32) = Строка.УИД Тогда  
			...
			ОбластьДокумент = ПечатаемыйДокумент.ПолучитьОбласть(Область.Имя);  
			
			Для Каждого ОбластьОбласти Из ОбластьДокумент.Области Цикл

				ОбластьОбласти.ЦветФона = WebЦвета.Белый;
			...
			ЗаписьФайла.Добавить(ИмяФайла, РежимСохраненияПутейZip.НеСохранятьПути);
...
Процедура Отправить(Команда)
	...
	Для Каждого Строка Из Строки Цикл
		...
		Если Не ЗначениеЗаполнено(Строка.ЭлектронныйАдрес) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Строка.Наименование 
			+ ": адрес получателя не указан, расчетный листок отправлен не будет");
		...
		Если Не ЗначениеЗаполнено(Строка.СНИЛС) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Строка.Наименование 
			+ ": СНИЛС получателя не указан, расчетный листок отправлен не будет");
			
			Продолжить;
			
		КонецЕсли;	                 
		
		Если Не ЕстьЛисток(Строка, ПечатаемыйДокумент) Тогда
...
&НаКлиенте
Функция ЕстьЛисток(Строка, ПечатаемыйДокумент)
