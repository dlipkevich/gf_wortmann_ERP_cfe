...
Процедура гф_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	// ++ Галфинд_ДомнышеваКР_14_05_2024
	ШаблонОтбора = "(%1.ДатаДокументаИБ >= &НачалоПериода
	|		ИЛИ &НачалоПериода = ДАТАВРЕМЯ(1, 1, 1))
	|		И (%1.ДатаДокументаИБ <= &КонецПериода
	|			ИЛИ &КонецПериода = ДАТАВРЕМЯ(1, 1, 1))"; 
	УсловиеОтбора = СтрШаблон(ШаблонОтбора, ПсевдонимОсновнойТаблицы);
	ОператорыЗапроса.Отбор.Добавить(УсловиеОтбора);
    // -- Галфинд_ДомнышеваКР_14_05_2024
