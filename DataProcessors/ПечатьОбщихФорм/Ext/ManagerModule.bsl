...
Процедура гф_ЗаполнитьТабличныйДокументУПД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, СтруктураТипов) Экспорт
	...
	//(н)Галфинд/Сефербеков 25.04.2024 ( 
	Если ТипЗнч(МассивДок[0].Ссылка) = Тип("ДокументСсылка.РеализацияУслугПрочихАктивов") Тогда
		ПервичныеДокументыВКоробах = Ложь;
	КонецЕсли;
	//(к)Галфинд/Сефербеков 25.04.2024 ) 
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Пока СтрокаТовары.Следующий() Цикл
			...
			//(н)Галфинд/Сефербеков 25.04.2024 ( если ни Артикул, ни Характеристика не заполнены, тогда выводится просто слэш. Например, при печати из документа РеализацияУслугПрочихАктивов 
			//ОбластьСтрока.Параметры.НоменклатураКод = СтрокаТовары.Номенклатура.Артикул + "/" + СтрокаТовары.ХарактеристикаНаименование;
			_НомеклатураКод = СтрокаТовары.Номенклатура.Артикул + ?(ЗначениеЗаполнено(СтрокаТовары.Номенклатура.Артикул) И ЗначениеЗаполнено(СтрокаТовары.ХарактеристикаНаименование), "/", "") + СтрокаТовары.ХарактеристикаНаименование ;
			Если ЗначениеЗаполнено(_НомеклатураКод) Тогда
				ОбластьСтрока.Параметры.НоменклатураКод = _НомеклатураКод  ;
			КонецЕсли;
			//(к)Галфинд/Сефербеков 25.04.2024 ) 
