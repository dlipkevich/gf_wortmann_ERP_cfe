...
Процедура ВыполнитьОбработкуНаСервере(ЗаписыватьВЖурналРегистрации = Ложь) Экспорт
	...
	//Шук 21.02.2024 Создание корректировок по каждому упаковочному листу    
	//{
	Если ПоКоробам Тогда                                                  				
		ШаблонКомментария = "Создан обработкой по %1 упаковочный лист %2";
		ТекущийПользователь = Пользователи.ТекущийПользователь();
		
		Для Каждого СтрокаКОбработке Из ЗаказыКОбработке Цикл			
			...
			НазначениеЗаказа           = Документы.КорректировкаНазначенияТоваров.НазначениеЗаказа(СтрокаКОбработке.ЗаказКлиента);
			ДокументОбъект.Назначение  = НазначениеЗаказа;
			ДокументОбъект.Комментарий = СтрШаблон(ШаблонКомментария, Поставка, СтрокаКОбработке.УпаковочныйЛист);
			ДокументОбъект.Автор         = ТекущийПользователь;
			ДокументОбъект.Ответственный = ТекущийПользователь;	
			ДанныеКОбработке = Данные.Выгрузить(Новый Структура("УпаковочныйЛист", СтрокаКОбработке.УпаковочныйЛист)); 
			...
			ДокументОбъектУЛ = СтрокаКОбработке.УпаковочныйЛист.ПолучитьОбъект();		
			Для Каждого СтрокаТабличнойЧастиУЛ Из ДокументОбъектУЛ.Товары Цикл				
				СтрокаТабличнойЧастиУЛ.Назначение = НазначениеЗаказа;				
			КонецЦикла;
		
			НачатьТранзакцию();
			Попытка    
				ДокументОбъект.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
				ДокументОбъектУЛ.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);							
				ЗафиксироватьТранзакцию();
			Исключение  
				ОтменитьТранзакцию();
				ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстСообщения = "Ошибка создания корректировки по упаковочному листу " + СтрокаКОбработке.УпаковочныйЛист;
				...
				УстановитьДопРеквизитПриобретениеТоваровУслуг(СтрокаКОбработке.ЗаказКлиента);
				Продолжить;
			КонецПопытки;	
			...
			ТекстСообщения = СтрШаблон(ТекстСообщения, ДокументОбъектУЛ.Ссылка);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);		
		КонецЦикла;	
...
Процедура ЗаполнитьНаСервере() Экспорт
	// Шук 22.02.2024 Изменение логики заполнения ТЧ
	//{
	//Запрос       = Новый Запрос;
	//Запрос.Текст =
	//"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|			ИНАЧЕ УпаковочныйЛистТовары.Ссылка.гф_Поставка = &Поставка
	|		КОНЕЦ
	|	И УпаковочныйЛистТовары.Ссылка.гф_СостояниеКороба В (&гф_СостояниеКороба)
	|	И УпаковочныйЛистТовары.Ссылка.гф_Заказ <> ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	втДанные.УпаковочныйЛист КАК УпаковочныйЛист,
	|	МИНИМУМ(ВЫБОР
	|			КОГДА втДанные.Назначение = &ТехническоеНазначение
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ) КАК ТехническийКороб
	|ПОМЕСТИТЬ ОтобранныеКороба
	|ИЗ
	|	втДанные КАК втДанные
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	втДанные КАК втДанные
	|ГДЕ
	|	втДанные.УпаковочныйЛист В
	|			(ВЫБРАТЬ
	|				ОтобранныеКороба.УпаковочныйЛист КАК УпаковочныйЛист
	|			ИЗ
	|				ОтобранныеКороба КАК ОтобранныеКороба
	|			ГДЕ
	|				ОтобранныеКороба.ТехническийКороб)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ТехническоеНазначение",	Справочники.Назначения.гф_Техническое);	
	гф_СостояниеКороба = Новый Массив;
	гф_СостояниеКороба.Добавить(Справочники.гф_СостянияКоробов.ПолныйКомплект);
	гф_СостояниеКороба.Добавить(Справочники.гф_СостянияКоробов.Пересорт);
	гф_СостояниеКороба.Добавить(Справочники.гф_СостянияКоробов.НеПолный);       
	гф_СостояниеКороба.Добавить(Справочники.гф_СостянияКоробов.НеПолныйИПересорт);
	Запрос.УстановитьПараметр( "гф_СостояниеКороба", 	гф_СостояниеКороба);
	
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	
	Данные.Загрузить(РезультатЗапроса[2].Выгрузить());
	ДанныеДляОтображения.Загрузить(РезультатЗапроса[3].Выгрузить());	
	//}
