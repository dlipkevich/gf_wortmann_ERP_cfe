...
Процедура гф_ЗаполнитьТабличныйДокументСчетаНаОплату(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		#Вставка
		Товары.Колонки.Добавить("ЗаказКлиента"); // СадомцевСА 31.01.2024
		Товары.Колонки.Добавить("ЦенаЗаПару"); // СадомцевСА 31.01.2024
		Товары.Колонки.Добавить("КоличествоПарВУпаковке"); // СадомцевСА 31.01.2024
		Товары.Колонки.Добавить("ОбщееКоличествоПар"); // СадомцевСА 31.01.2024
		Товары.Колонки.Добавить("СкидкаНаценка"); // Галфинд_ДомнышеваКР_29_03_2024
        Товары.Колонки.Добавить("СкидкаНаценкаРуб"); // Галфинд_ДомнышеваКР_29_03_2024
		гф_ЗаполнитьТаблицуТоваров(Товары, ДанныеПечати.Ссылка);
		СуммаСкидкаНаценка = Товары.Итог("СкидкаНаценкаРуб"); // Галфинд_ДомнышеваКР_29_03_2024
		#КонецВставки
		...
		Иначе
			...
			#Вставка
			Если ПараметрыПечати.Свойство("ОтображатьПодписи") Тогда
				ОбластьЗаказа	             = Макет.ПолучитьОбласть("СтрокаЗаказа");
			КонецЕсли;
			#КонецВставки
			...
			СоответствиеСтавокНДС = Новый Соответствие;
			#Вставка
			ТекЗаказ = Неопределено; // СадомцевСА 31.01.2024
			#КонецВставки
			Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
				...
				ОбластьЗаказаЗаполнитьПараметрЗаказКлиента(ТабличныйДокумент, ОбластьЗаказа, ПараметрыПечати, СтрокаТовары, ТекЗаказ);
				//Если ПараметрыПечати.Свойство("ОтображатьПодписи") И ЗначениеЗаполнено(СтрокаТовары.ЗаказКлиента) Тогда
				//	Если ТекЗаказ <> СтрокаТовары.ЗаказКлиента Тогда
				//		ОбластьЗаказа.Параметры.ЗаказКлиента = "Заказ № " + СокрЛП(СтрокаТовары.ЗаказКлиента.Номер) + " от " + Формат(СтрокаТовары.ЗаказКлиента.Дата, "ДФ=dd.MM.yyyy");
				//		ТабличныйДокумент.Вывести(ОбластьЗаказа);
				//		ТекЗаказ = СтрокаТовары.ЗаказКлиента;
				//	КонецЕсли;
				//КонецЕсли;
				#КонецВставки
				ТабличныйДокумент.Вывести(ОбластьСтрокаТаблицы);
			...
			СтруктураДанныхВсегоСкидки.Вставить("ВсегоСкН", ФормированиеПечатныхФорм.ФорматСумм(СуммаСкидкаНаценка));// ++ Галфинд_ДомнышеваКР_29_03_2024
			#КонецВставки
...
Процедура ОбластьЗаказаЗаполнитьПараметрЗаказКлиента(ТабличныйДокумент, ОбластьЗаказа, ПараметрыПечати, СтрокаТовары, ТекЗаказ)
	Если ПараметрыПечати.Свойство("ОтображатьПодписи") И ЗначениеЗаполнено(СтрокаТовары.ЗаказКлиента) Тогда
		Если ТекЗаказ <> СтрокаТовары.ЗаказКлиента Тогда
			ОбластьЗаказа.Параметры.ЗаказКлиента = "Заказ № " + СокрЛП(СтрокаТовары.ЗаказКлиента.Номер) + " от " + Формат(СтрокаТовары.ЗаказКлиента.Дата, "ДФ=dd.MM.yyyy");
			ТабличныйДокумент.Вывести(ОбластьЗаказа);
...
Процедура гф_ЗаполнитьТаблицуТоваров(ТаблицаТовары, СчетСсылка)
	...
	|	Товары.ЗаказКлиента                  КАК ЗаказКлиента, // СадомцевСА 31.01.2024
	...
	//|	Товары.ВариантКомплектации					                 КАК Артикул,
	|	ВЫБОР КОГДА НЕ Товары.ВариантКомплектации.Владелец ЕСТЬ NULL ТОГДА Товары.ВариантКомплектации
	|		ИНАЧЕ Товары.Номенклатура.Артикул КОНЕЦ	                 КАК Артикул,
	// -- Галфинд_ДомнышеваКР_13_02_2024
	// СадомцевСА 07.03.2024
	//|	Товары.ВариантКомплектации.Владелец				             КАК Номенклатура,
	|	ВЫБОР КОГДА НЕ Товары.ВариантКомплектации.Владелец ЕСТЬ NULL ТОГДА Товары.ВариантКомплектации.Владелец
	|		ИНАЧЕ Товары.Номенклатура КОНЕЦ				             КАК Номенклатура,
	// СадомцевСА 07.03.2024
	//|	Товары.ВариантКомплектации.КоличествоУпаковок				 КАК КоличествоПарВУпаковке,
	|	ЕСТЬNULL(Товары.ВариантКомплектации.КоличествоУпаковок, 0)	 КАК КоличествоПарВУпаковке,
	// СадомцевСА 07.03.2024
	//|	""""                                                         КАК НаименованиеПолное,
	|	ВЫБОР КОГДА НЕ Товары.ВариантКомплектации.Владелец ЕСТЬ NULL ТОГДА Товары.ВариантКомплектации.Владелец
	|		ИНАЧЕ Товары.Номенклатура КОНЕЦ				             КАК НаименованиеПолное,
	// СадомцевСА 07.03.2024
	//|	Товары.ВариантКомплектации.Упаковка                          КАК Упаковка,
	|	ВЫБОР КОГДА НЕ Товары.ВариантКомплектации.Упаковка ЕСТЬ NULL ТОГДА Товары.ВариантКомплектации.Упаковка
	|		ИНАЧЕ Товары.Номенклатура.ЕдиницаИзмерения КОНЕЦ         КАК Упаковка,
	...
	|	ВЫБОР КОГДА НЕ Товары.ВариантКомплектации.Упаковка ЕСТЬ NULL ТОГДА Товары.ВариантКомплектации.Упаковка
	|		ИНАЧЕ Товары.Номенклатура.ЕдиницаИзмерения КОНЕЦ         КАК ЕдиницаИзмерения,
	|
	...
	|	Товары.Цена*Товары.Количество*(Товары.СкидкаНаценка/100)     КАК СкидкаНаценкаРуб,
	// -- Галфинд_ДомнышеваКР_29_03_2024
	|
	...
	|УПОРЯДОЧИТЬ ПО ЗаказКлиента"; // СадомцевСА 31.01.2024
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	МАКСИМУМ(ЗаказКлиентаТовары.гф_ЦенаСоСкидкой) КАК ЦенаЗаПару
	               |ИЗ
	...
	Запрос.УстановитьПараметр("СписокЗаказов", Результат.Выгрузить().ВыгрузитьКолонку("ЗаказКлиента"));
	РезультатЦеныНоменклатуры = Запрос.Выполнить();
	ВыборкаЦеныНоменклатуры = РезультатЦеныНоменклатуры.Выбрать();
	СтруктураПоиска = Новый Структура("ЗаказКлиента, Номенклатура");
	//-- СадомцевСА 31.01.2024
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВыборкаЦеныНоменклатуры.НайтиСледующий(СтруктураПоиска) Тогда
			нс.ЦенаЗаПару = ВыборкаЦеныНоменклатуры.ЦенаЗаПару;
			нс.ОбщееКоличествоПар = нс.КоличествоПарВУпаковке * нс.Количество;
		КонецЕсли;
		//-- СадомцевСА 31.01.2024
	КонецЦикла;
...
Процедура гф_ЗаполнитьРеквизитыШапкиСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, ТаблицаТовары)
	...
	#Вставка
	// ++ Галфинд_ДомнышеваКР_13_02_2024
	Если Найти(ЗначениеВСтрокуВнутр(ОбластьМакета), "Поставка") <> 0 Тогда  
	ОбластьМакета.Параметры.Поставка = ДанныеПечати.Ссылка.гф_НомерПоставки.Номер;
	КонецЕсли;
	// -- Галфинд_ДомнышеваКР_13_02_2024
	#КонецВставки
	ТабличныйДокумент.Вывести(ОбластьМакета);
