...
Процедура КомандаЗаполнитьНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |			ИЛИ НЕ ДоговорыКонтрагентовСвойствоПредоплата.Значение ЕСТЬ NULL
	               ...
	               |ГДЕ
	               |	НЕ ЕСТЬNULL(гф_СпецификацияПокупателя.ПометкаУдаления, ЛОЖЬ)
	               |
	               |ОБЪЕДИНИТЬ ВСЕ
	               |
	               |ВЫБРАТЬ
	               ...
	               |	ВТ_Заказы.Договор,
	               |	ЗНАЧЕНИЕ(Документ.гф_СпецификацияПокупателя.ПустаяСсылка),
	               |	ДАТАВРЕМЯ(1, 1, 1)
	               |ИЗ
	               |	ВТ_Заказы КАК ВТ_Заказы
	               |;
...
// Параметры:
//
// СпецификацияОбъект - ДокументОбъект
// СтрокаСпецификации - ДокументТабличнаяЧастьСтрока
//
&НаСервере
