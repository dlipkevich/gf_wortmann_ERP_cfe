...
Функция НайтиЗаказПоНомеруДате(Заказ, стрНомер, стрДата)
	Если стрНомер > "" Тогда
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	ЗаказКлиента.Дата >= &НачалоИнтервалаПоиска
			|	И ЗаказКлиента.Номер = &Номер
			|	И НЕ ЗаказКлиента.ПометкаУдаления
			|";
		
		Запрос.УстановитьПараметр("НачалоИнтервалаПоиска", НачалоДня(?(ЗначениеЗаполнено(стрДата), стрДата, ТекущаяДатаСеанса()) - Объект.бф_ИнтервалПоиска * 24*60*60));
		...
		Если ВыборкаДетальныеЗаписи.Количество() > 1 Тогда
			СтрокаПротокола = ТЗПротокол.Добавить();
			СтрокаПротокола.Комментарий = "В указанном периоде найдено несколько заказов с номером:" + стрНомер;
			Возврат Истина;
		ИначеЕсли ВыборкаДетальныеЗаписи.Следующий() Тогда
			Нашли = ВыборкаДетальныеЗаписи.Ссылка;
		Иначе
			Нашли = Документы.ЗаказКлиента.ПустаяСсылка();
		КонецЕсли;
		//--- БФ Бобнев К.С. 22.03.24
