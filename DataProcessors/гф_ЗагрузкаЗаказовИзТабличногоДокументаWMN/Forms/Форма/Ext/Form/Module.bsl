...
Процедура ЗагрузитьДанныеСтроки(стрНомер, данные)
	...
	Если Не ЗначениеЗаполнено(стрНомер.Номер)
		ИЛИ Не ЗначениеЗаполнено(стрНомер.Дата) Тогда
		СтрокаПротокола = ТЗПротокол.Добавить();
		СтрокаПротокола.Документ = "Пустые поля Номер:" + стрНомер.Номер + " Дата:" + стрНомер.Дата;
		СтрокаПротокола.Комментарий = "Строки файла пропущены";
		Возврат;
	КонецЕсли;
		
	Заказ = Неопределено;
	Если стрНомер.Номер > "" Тогда
		Нашли = Документы.ЗаказКлиента.НайтиПоНомеру(стрНомер.Номер, стрНомер.Дата);
	...
	Если Заказ = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(стрНомер.Дата) Тогда
			Заказ.Дата = стрНомер.Дата;
		Иначе
			Заказ.Дата = ТекущаяДата();
		КонецЕсли;
		
		Если стрНомер.Номер > "" Тогда
			Заказ.Номер = стрНомер.Номер;
		Иначе
