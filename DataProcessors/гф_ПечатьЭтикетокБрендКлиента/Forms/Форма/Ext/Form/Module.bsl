...
Процедура ТаблицаНоменклатурыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.ВариантыКомплектацииНоменклатуры.ФормаОбъекта", 
	ПараметрыОткрытия, ЭтотОбъект, ТекущиеДанные.ВариантКомплектации);
...
Функция ПечатьНаСервере()
	...
	Запрос.Параметры.Вставить("Color_Name", 
	ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "гф_НоменклатураColor_Name"));
	Запрос.Параметры.Вставить("Wideness", 
	ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "гф_НоменклатураWideness"));
	...
	МассивДокументов = ПечатьНаСервереДалее(Результат, МассивДокументов, Макет);
...
&НаСервере
Функция ПечатьНаСервереДалее(Результат, МассивДокументов, Макет)
	...
	ТабДокумент.ПолеСверху	= 14;
	ТабДокумент.ПолеСлева	= 40;
	ТабДокумент.ПолеСправа	= 40;
	...
	Для Каждого СтрокаАртикула Из ТаблицаАртикулов Цикл
		...
		ПечатьСтрокЭтикеток(СтрокиЭтикеток, Макет, ТабДокумент, ЭтикетокНаСтранице);
		
		Если ВариантПечати = Два Тогда
...
&НаСервере
Процедура ПечатьСтрокЭтикеток(СтрокиЭтикеток, Макет, ТабДокумент, ЭтикетокНаСтранице) 
	...
	Для Каждого СтрокаЭтикетки Из СтрокиЭтикеток Цикл
		...
		ВывестиКартинку(СтрокаЭтикетки, ОбластьЭтикетка);
		...
		ВывестиЭтикетки(СтрокаЭтикетки, ОбластьЭтикетка, ТабДокумент, ЭтикетокНаСтранице);
...
&НаСервере
Процедура ВывестиКартинку(СтрокаЭтикетки, ОбластьЭтикетка)
...
&НаСервере
Процедура ВывестиЭтикетки(СтрокаЭтикетки, ОбластьЭтикетка, ТабДокумент, ЭтикетокНаСтранице)
	...
	Для Н = 1 По СтрокаЭтикетки.КоличествоЭтикеток Цикл 
		...
		Если ЭтикетокНаСтранице >= Шесть Тогда
...
Процедура ПолучитьДокументыНаСервере()
	...
	Если Объект.Контрагент.Пустая() Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Контрагент", "ИСТИНА");		
		
	Иначе
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Контрагент", "Выборка.Контрагент = &Контрагент");		
		
	КонецЕсли;	

	Если Объект.ДоговорКонтрагента.Пустая() Тогда
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Договор", "ИСТИНА");		
		
	Иначе
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Договор", "Выборка.Договор = &ДоговорКонтрагента");		
	...
	ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "гф_СкладыТоварыВКоробах"));
...
Процедура ЗаполнитьПоЗаказамНаСервере()
	...
	Запрос = Новый запрос("ВЫБРАТЬ
	                      |	Подзапрос.КоличествоЭтикеток КАК КоличествоЭтикеток,
	                      |	Подзапрос.ВариантКомплектации КАК ВариантКомплектации,
	                      |	Подзапрос.Номенклатура КАК Номенклатура,
	                      |	Подзапрос.КоличествоВКомплекте КАК КоличествоВКомплекте
	                      |ИЗ
	                      |	(ВЫБРАТЬ
	                      ...
	                      |		И ВариантыКомплектацииНоменклатуры.Количество <> 0
	                      |	
	                      |	СГРУППИРОВАТЬ ПО
	                      |		ЗаказКлиентагф_ТоварыВКоробах.ВариантКомплектации,
	                      |		ВариантыКомплектацииНоменклатуры.Владелец,
	                      |		ВариантыКомплектацииНоменклатуры.Количество) КАК Подзапрос
	                      |ГДЕ
	                      |	Подзапрос.КоличествоЭтикеток <> 0");  
