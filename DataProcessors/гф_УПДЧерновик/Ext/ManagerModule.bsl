...
Процедура ЗаполнитьТабличныйДокументУПД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, СтруктураТипов) Экспорт
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ДанныеПечати.Ссылка.Договор.ЗакупкаПодДеятельность)
			И (ДанныеПечати.Ссылка.Договор.ЗакупкаПодДеятельность = Перечисления.ТипыНалогообложенияНДС.НалоговыйАгентПоНДС
			ИЛИ ДанныеПечати.Ссылка.Договор.ЗакупкаПодДеятельность = Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт) Тогда
		...
		Если ПервичныеДокументыВКоробах Тогда
			...
			Для Каждого СтрТов Из Таб[0].гф_ТоварыВКоробах Цикл //Короба
				...
				Если ЗначениеЗаполнено(СтрТов.Ссылка.Договор.НалогообложениеНДС)
		...
		Иначе
			...
			Пока СтрокаТовары.Следующий() Цикл
				...
				ОбластьСтрокаКомплектующие.Параметры.ВОдномМесте               = 1;
				ОбластьСтрокаКомплектующие.Параметры.МестШтук                  = СтрокаТовары.Количество;
				// -- Галфинд ВолковЕВ 2024/01/30
