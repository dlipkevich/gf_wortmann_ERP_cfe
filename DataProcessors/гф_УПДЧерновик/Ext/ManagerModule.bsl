...
Процедура ЗаполнитьТабличныйДокументУПД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, СтруктураТипов) Экспорт
	...
	Если ПервичныеДокументыВКоробах Тогда
		...
		МакетУПД = УправлениеПечатью.МакетПечатнойФормы("Обработка.гф_УПДЧерновик.ПФ_MXL_УПД_Свернутый_Вкоробах");
		МакетУПД_625 = УправлениеПечатью.МакетПечатнойФормы("Обработка.гф_УПДЧерновик.ПФ_MXL_УПД_Свернутый_Вкоробах");
		МакетУПД_981 = УправлениеПечатью.МакетПечатнойФормы("Обработка.гф_УПДЧерновик.ПФ_MXL_УПД_Свернутый_Вкоробах");
		МакетУПД_534 = УправлениеПечатью.МакетПечатнойФормы(
		"Обработка.гф_УПДЧерновик.ПФ_MXL_УПД_Свернутый_Вкоробах");
