		...
		Иначе
			...
			Пока СтрокаТовары.Следующий() Цикл
				...
				Если ДанныеПечати.Валюта <> ВалютаРегламентированногоУчета(ДанныеПечати) ТОгда
				   ...
				   СтрокаДокумента = Запрос.Выполнить().Выбрать();	
					
				   Если СтрокаДокумента.Следующий() ТОгда
					   //ТаблицаДок = МассивДок[0].Ссылка.Товары.Выгрузить(); 
					   //СтрокаДокумента =  ТаблицаДок.НайтиСтроки(Новый Структура("Номенклатура, Характеристика",СтрокаТовары.Номенклатура,СтрокаТовары.Характеристика));
					   //МассивДок[0].Ссылка.Товары[СтрокаТовары.НомерСтроки-1]; 
					   ...
					   ОбластьСтрокаКомплектующие.Параметры.СуммаСНДС  =  СтрокаДокумента.СуммаСНДС;
				   КонецЕсли;
			   КонецЕсли;
			// КОНЕЦ ИЗМЕНЕНИЙ
