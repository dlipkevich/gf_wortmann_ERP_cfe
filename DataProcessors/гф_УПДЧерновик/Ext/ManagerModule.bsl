...
Процедура ЗаполнитьТабличныйДокументУПД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, СтруктураТипов) Экспорт
	...
	Если ПервичныеДокументыВКоробах = Неопределено Тогда
		КодЯзыка = "";
		Результат = УправлениеСвойствами.ЗначенияСвойств(МассивДок[0].Ссылка.Договор, Истина, Истина , "гф_ДоговорыКонтрагентовПервичныеДокументыВКоробах", КодЯзыка);
		Если Не Результат.Количество() = 0 Тогда
			ПервичныеДокументыВКоробах = Результат[0].Значение;
		Иначе
			ПервичныеДокументыВКоробах = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		// ++ Галфинд ВолковЕВ 2024/01/30
		
		//(н)Галфинд/Сефербеков 16.02.2024 ( заремарил, оставляем содержание ПредставлениеГрузополучателя в типовом варианте,  e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eeccbaf93f4fdd
		
		// НАЧАЛО ИЗМЕНЕНИЙ, МАСТЕР СОФТ, РЫБАКОВА, 19.02.2024, Задача:
		СведенияОГрузополучателе = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Ссылка.Контрагент, ДанныеПечати.Дата);
		ПредставлениеГрузополучателя = ДанныеПечати.Ссылка.ДокументОснование.АдресДоставки;
		ПредставлениеГрузополучателя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1; %2';
		|en = '%1; %2'"),
		...
		// КОНЕЦ ИЗМЕНЕНИЙ
		//(к)Галфинд/Сефербеков 16.02.2024 ) 
				
		Если ЗначениеЗаполнено(ДанныеПечати.Ссылка.Договор.НалогообложениеНДС)
			И (ДанныеПечати.Ссылка.Договор.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НалоговыйАгентПоНДС
			ИЛИ ДанныеПечати.Ссылка.Договор.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт) Тогда
			
			ОбластьШапка.Параметры.ИННПокупателя = ДанныеПечати.Ссылка.Контрагент.НалоговыйНомер;
			
		КонецЕсли;
		Если ЗначениеЗаполнено(ДанныеПечати.Ссылка.Договор.ВалютаВзаиморасчетов) Тогда
			
			ВалютаНаименованиеПолное 	= ДанныеПечати.Ссылка.Договор.ВалютаВзаиморасчетов.НаименованиеПолное;
			ВалютаКод 					= ДанныеПечати.Ссылка.Договор.ВалютаВзаиморасчетов.Код;
			
			ОбластьШапка.Параметры.Валюта = СтрШаблон(НСтр("ru = '%1 %2'"), ВалютаНаименованиеПолное, ВалютаКод);
		...
		// ++ Галфинд ВолковЕВ 2024/01/30
		ОбластьПодвал.Параметры.Основание = "Договор № " + ДанныеПечати.Ссылка.ДокументОснование.Договор.Номер
		+ " " + Формат(ДанныеПечати.Ссылка.ДокументОснование.Договор.Дата, "ДЛФ=ДД");
		// -- Галфинд ВолковЕВ 2024/01/30
		...
		// НАЧАЛО ИЗМЕНЕНИЙ, МАСТЕР СОФТ, РЫБАКОВА, 06.03.2024, Задача:
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СкладыДополнительныеРеквизиты.Свойство КАК Свойство,
		|	СкладыДополнительныеРеквизиты.Значение КАК ТоварыВКоробах
		|ИЗ
		...
		Запрос.УстановитьПараметр("Склад",МассивДок[0].Ссылка.Склад );
		
		ТоварыВКоробах = Ложь;
		Выборка = Запрос.Выполнить().Выбрать();
		
		Если Выборка.Следующий() Тогда
			ТоварыВКоробах = Выборка.ТоварыВКоробах;
		КонецЕсли;
					
		СтрутураИтого = Новый Структура;
		
		Если ПервичныеДокументыВКоробах и ТоварыВКоробах Тогда
		// КОНЕЦ ИЗМЕНЕНИЙ	
			...
			Для Каждого СтрТов Из Таб[0].гф_ТоварыВКоробах Цикл //Короба
				...
				// НАЧАЛО ИЗМЕНЕНИЙ, МАСТЕР СОФТ, РЫБАКОВА, 21.02.2024, Задача:
				...
				ОбластьСтрока.Параметры.ПредставлениеНоменклатуры    = СтрТов.УпаковочныйЛист.гф_Комплектация.Владелец.НаименованиеПолное;
				...
				//++Галфинд ЕсиповАВ 19.01.24
				ОбластьСтрока.Параметры.ВОдномМесте                  = СтрТов.КоличествоПар;
				ОбластьСтрока.Параметры.МестШтук                     = Мест;
				ОбластьСтрока.Параметры.Количество                   = СтрТов.КоличествоПар * Мест;
				...
				Если ЗначениеЗаполнено(СтрТов.Ссылка.Договор.ЗакупкаПодДеятельность)
					И (СтрТов.Ссылка.Договор.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НалоговыйАгентПоНДС
					ИЛИ СтрТов.Ссылка.Договор.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт) Тогда
					ОбластьСтрока.Параметры.СтавкаНДС = "0%";
				Иначе	
					ОбластьСтрока.Параметры.СтавкаНДС = СтрТов.УпаковочныйЛист.гф_Комплектация.Владелец.СтавкаНДС.ПеречислениеСтавкаНДС;
				КонецЕсли;
				ОбластьСтрока.Параметры.КодВидаТовара = СтрТов.УпаковочныйЛист.гф_Комплектация.Владелец.КодТНВЭД;
				// -- Галфинд ВолковЕВ 2024/01/30
				...
				ИтогоКоличествоИтого = ИтогоКоличествоИтого + СтрТов.КоличествоПар * Мест;
		...
		Иначе
			...
			Пока СтрокаТовары.Следующий() Цикл
				...
				// НАЧАЛО ИЗМЕНЕНИЙ, МАСТЕР СОФТ, РЫБАКОВА, 06.03.2024, Задача:
				НоменклатураКодПредставление =  СтрокаТовары.Номенклатура.Артикул + ?(СтрокаТовары.Характеристика.Наименование = "", "","/" + СтрокаТовары.Характеристика.Наименование);
				ОбластьСтрокаКомплектующие.Параметры.НоменклатураКод           =   НоменклатураКодПредставление;
				// КОНЕЦ ИЗМЕНЕНИЙ
				//--Галфинд ЕсиповАВ 19.01.24
				...
				//--Галфинд ЕсиповАВ 19.01.24
				ОбластьСтрокаКомплектующие.Параметры.ВОдномМесте               = СтрокаТовары.Количество;
				ОбластьСтрокаКомплектующие.Параметры.МестШтук                  = Формат(1, "ЧДЦ=3");
				ОбластьСтрокаКомплектующие.Параметры.Количество                = СтрокаТовары.Количество;
				ОбластьСтрокаКомплектующие.Параметры.СтранаПроисхожденияКод    = ?(ЗначениеЗаполнено(СтрокаТовары.СтранаПроисхождения),СтрокаТовары.СтранаПроисхождения.Код,"");
				ОбластьСтрокаКомплектующие.Параметры.СтранаПроисхождения       = ?(ЗначениеЗаполнено(СтрокаТовары.СтранаПроисхождения),СтрокаТовары.СтранаПроисхождения.Наименование,"");
				ОбластьСтрокаКомплектующие.Параметры.НомерГТД                  = СтрокаТовары.НомерГТДСсылка.Код;
				//++Галфинд ЕсиповАВ 19.01.24
				ОбластьСтрокаКомплектующие.Параметры.НомерСтроки                   = НомерСтроки;
				ОбластьСтрокаКомплектующие.Параметры.Упак                          = "упак";
				...
				// НАЧАЛО ИЗМЕНЕНИЙ, МАСТЕР СОФТ, РЫБАКОВА, 08.04.2024, Задача: неверно выводится сумма если одна и та же ном. с разным гтд
				ОбластьСтрокаКомплектующие.Параметры.СуммаБезНДС                   =  СтрокаТовары.Цена*СтрокаТовары.Количество;//СтрокаТовары.СуммаБезНДС;
				// КОНЕЦ ИЗМЕНЕНИЙ
				...
				Если ЗначениеЗаполнено(СтрокаТовары.Ссылка.Договор.НалогообложениеНДС)
					И (СтрокаТовары.Ссылка.Договор.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НалоговыйАгентПоНДС
					ИЛИ СтрокаТовары.Ссылка.Договор.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт) Тогда
					ОбластьСтрокаКомплектующие.Параметры.СтавкаНДС = "0%";
				Иначе	
					ОбластьСтрокаКомплектующие.Параметры.СтавкаНДС = СтрокаТовары.СтавкаНДС.Наименование;
				КонецЕсли;
				ОбластьСтрокаКомплектующие.Параметры.КодВидаТовара = СтрокаТовары.Номенклатура.КодТНВЭД;
				// Волк1
				...
				// НАЧАЛО ИЗМЕНЕНИЙ, МАСТЕР СОФТ, РЫБАКОВА, 14.03.2024, Задача:
				Если ДанныеПечати.Валюта <> ВалютаРегламентированногоУчета(ДанныеПечати) ТОгда
					...
					Запрос.УстановитьПараметр("Ссылка",МассивДок[0].Ссылка);
				   ...
				   // НАЧАЛО ИЗМЕНЕНИЙ, МАСТЕР СОФТ, РЫБАКОВА, 08.04.2024, Задача: неверно выводится сумма если одна и та же ном. с разным гтд
					...
					СтрокиДокументам =  Запрос.Выполнить().Выгрузить();	
					
				   Если СтрокиДокументам.Количество() <> 0  ТОгда
					   СтрокаДокумента = СтрокиДокументам[0];
					   ОбластьСтрокаКомплектующие.Параметры.Цена  = СтрокаДокумента.Цена;
					   ОбластьСтрокаКомплектующие.Параметры.СуммаБезНДС  = СтрокаДокумента.Сумма;
					   ОбластьСтрокаКомплектующие.Параметры.СуммаНДС  =  СтрокаДокумента.СуммаНДС;  
					  	СуммаСНДС = СтрокаТовары.Количество*СтрокаДокумента.Цена; 
					   СтрокаДокумента.СуммаСНДС = СуммаСНДС;
					   ОбластьСтрокаКомплектующие.Параметры.СуммаСНДС  = СуммаСНДС;// СтрокаДокумента.СуммаСНДС;
					   // КОНЕЦ ИЗМЕНЕНИЙ
				   КонецЕсли;
			   КонецЕсли;
			// КОНЕЦ ИЗМЕНЕНИЙ
				...
				ИтогоКоличествоМестШтук = ИтогоКоличествоМестШтук + 1;
				...
				Если ДанныеПечати.Валюта <> ВалютаРегламентированногоУчета(ДанныеПечати) ТОгда
					Если Не НаборыСервер.ИспользоватьОбластьКомплектующие(СтрокаТовары, ИспользоватьНаборы) Тогда
						Если ИспользоватьПечатныеФормыСФ2_5 Тогда
							РассчитатьИтоговыеСуммы2_5(ИтоговыеСуммы, СтрокаДокумента);
						Иначе
							РассчитатьИтоговыеСуммы(ИтоговыеСуммы, СтрокаДокумента);
						КонецЕсли;
					КонецЕсли;
				Иначе
					Если Не НаборыСервер.ИспользоватьОбластьКомплектующие(СтрокаТовары, ИспользоватьНаборы) Тогда
		...
		////ЕсповАВ
			...
			// НАЧАЛО ИЗМЕНЕНИЙ, МАСТЕР СОФТ, РЫБАКОВА, 15.03.2024, Задача:
			Если ПервичныеДокументыВКоробах  и ТоварыВКоробах Тогда 
			// КОНЕЦ ИЗМЕНЕНИЙ
