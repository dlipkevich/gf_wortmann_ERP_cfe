	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ПервичныеДокументыВКоробах Тогда
			...
			Для Каждого СтрТов Из Таб[0].гф_ТоварыВКоробах Цикл //Короба
				...
				Если ЗначениеЗаполнено(СтрТов.Ссылка.Договор.ЗакупкаПодДеятельность)
					И СтрТов.Ссылка.Договор.ЗакупкаПодДеятельность = Перечисления.ТипыНалогообложенияНДС.НалоговыйАгентПоНДС Тогда
		...
		Иначе
			...
			Пока СтрокаТовары.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(СтрокаТовары.Ссылка.Договор.ЗакупкаПодДеятельность)
					И СтрокаТовары.Ссылка.Договор.ЗакупкаПодДеятельность = Перечисления.ТипыНалогообложенияНДС.НалоговыйАгентПоНДС Тогда
