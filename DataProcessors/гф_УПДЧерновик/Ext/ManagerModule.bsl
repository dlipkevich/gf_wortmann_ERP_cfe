...
Процедура ЗаполнитьТабличныйДокументУПД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, СтруктураТипов) Экспорт
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ПервичныеДокументыВКоробах Тогда
			...
			Запрос.Текст =
			"ВЫБРАТЬ
			|	РеализацияТоварыВКоробах.НомерСтроки КАК НомерСтроки,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист КАК УпаковочныйЛист,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.гф_Комплектация КАК ВариантКомплектации,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.гф_Комплектация.Наименование КАК НоменклатураКод,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.гф_Комплектация.Упаковка.Наименование КАК Упак,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.гф_Комплектация.Владелец.НаименованиеПолное КАК ПредставлениеНоменклатуры,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.гф_Комплектация.Владелец.ЕдиницаИзмерения.Код КАК ЕдиницаИзмеренияКод,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.гф_Комплектация.Владелец.ЕдиницаИзмерения.Наименование КАК ЕдиницаИзмеренияНаименование,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.гф_Комплектация.Владелец.СтранаПроисхождения.Код КАК СтранаПроисхожденияКод,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.гф_Комплектация.Владелец.СтранаПроисхождения.Наименование КАК СтранаПроисхождения,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.гф_Комплектация.Владелец.СтавкаНДС КАК СтавкаНДС,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.гф_Комплектация.Владелец.СтавкаНДС.ПеречислениеСтавкаНДС КАК ПеречислениеСтавкаНДС, 
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.ВсегоМест КАК ВОдномМесте,
			|	РеализацияТоварыВКоробах.Коэффициент КАК МестШтук,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.ВсегоМест * РеализацияТоварыВКоробах.Коэффициент КАК Количество,
			|	РеализацияТоварыВКоробах.ЦенаКороба КАК Цена,
			|	РеализацияТоварыВКоробах.ЦенаКороба * РеализацияТоварыВКоробах.Коэффициент КАК СуммаБезНДС,
			|	РеализацияТоварыВКоробах.НДС * РеализацияТоварыВКоробах.Коэффициент КАК СуммаНДС,
			|	РеализацияТоварыВКоробах.ЦенаКороба * РеализацияТоварыВКоробах.Коэффициент + РеализацияТоварыВКоробах.НДС * РеализацияТоварыВКоробах.Коэффициент КАК СуммаСНДС,
			|	РеализацияТоварыВКоробах.УпаковочныйЛист.гф_Агрегация.гф_НомерГТД КАК НомерГТД,
			|	РеализацияТоварыВКоробах.Ссылка КАК Ссылка
			|ИЗ
			|	Документ.РеализацияТоваровУслуг.гф_ТоварыВКоробах КАК РеализацияТоварыВКоробах
			|
			|ГДЕ
            |	РеализацияТоварыВКоробах.Ссылка = &Ссылка
			|
			|УПОРЯДОЧИТЬ ПО
			|	НомерСтроки
			|ИТОГИ
			...
			Запрос.УстановитьПараметр("Ссылка", МассивДок[0]);
			...
			Если ВыборкаОбщийИтог.Следующий() Тогда
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
					...
					Если ВыборкаДетальныеЗаписи["ПеречислениеСтавкаНДС"] <> ПредопределенноеЗначение("Перечисление.СтавкиНДС.БезНДС") Тогда
						ТолькоСтавкаБезНДС = Ложь;	
					КонецЕсли;
					
				КонецЦикла;
				
				ИтогоКоличествоМестШтук = ВыборкаОбщийИтог["МестШтук"];
				ИтогоКоличествоИтого    = ВыборкаОбщийИтог["Количество"];
				
				ИтоговыеСуммы.ИтогоСуммаБезНДС    = ВыборкаОбщийИтог["СуммаБезНДС"];
				ИтоговыеСуммы.ИтогоСуммаНДС       = ВыборкаОбщийИтог["СуммаНДС"];
				ИтоговыеСуммы.ИтогоСуммаСНДС      = ВыборкаОбщийИтог["СуммаСНДС"];
			...
			//	//ОбластьСтрока.Параметры.НоменклатураКод              = СтрТов["ВариантКомплектацииНаименование"];
			//	//
			//	////--Галфинд ЕсиповАВ 19.01.24
