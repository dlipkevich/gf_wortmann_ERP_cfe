...
Процедура НайденныеОшибкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СтрокаОшибок.Данные) Или СтрокаОшибок.РежимПроверки = ВсеРежимыПроверок().Формы Тогда
	
		СтандартнаяОбработка = Ложь;
		
		Если СтрокаОшибок.РежимПроверки = ВсеРежимыПроверок().Формы И ЗначениеЗаполнено(СтрокаОшибок.ПолноеИмяОбъекта)  Тогда
...
Процедура ЗаполнитьПредставлениеНастроекПроверки()
	...
	КоличествоОбъектов = ?(Объект.СколькоПроверятьОднотипныхДокументов = 0, 1, Объект.СколькоПроверятьОднотипныхДокументов);
	ШаблонПредставления = "Проведение / запись документов (сколько документов одного типа проверять: %1)";
	ПредставлениеНастроекДокументов = СтрШаблон(ШаблонПредставления, КоличествоОбъектов);
	
	КоличествоОбъектов = ?(Объект.СколькоПроверятьОднотипныхОбъектов = 0, 1, Объект.СколькоПроверятьОднотипныхОбъектов);
	ШаблонПредставления = "Запись справочников (сколько элементов справочника одного типа проверять: %1)";
	ПредставлениеНастроекСправочников = СтрШаблон(ШаблонПредставления, КоличествоОбъектов);
...
Процедура ДополнитьДеревоОбщимиФормами(Дерево)
	...
	ГруппаДерева.Синоним = "Общие формы";
...
Процедура СобратьПроверки(РезультатыПроверок)
	...
	Для Каждого ЭлементСписка Из РежимыПроверок Цикл
		...
		Если ЭлементСписка.Значение = ВсеРежимы.Документы Тогда
			...
			Если Объект.СколькоПроверятьОднотипныхДокументов > 1 Тогда
				ВесПроверки = ВесПроверки * Объект.СколькоПроверятьОднотипныхДокументов;	
			КонецЕсли;
		...
		Если ЭлементСписка.Значение = ВсеРежимы.Справочники Тогда 
			...
			Если Объект.СколькоПроверятьОднотипныхОбъектов > 1 Тогда
				ВесПроверки = ВесПроверки * Объект.СколькоПроверятьОднотипныхОбъектов;	
			КонецЕсли;
	...
	Для Каждого СтрокаКласса Из ФормыОбъектов.ПолучитьЭлементы() Цикл
		...
		Для Каждого СтрокаОбъекта Из СтрокаКласса.ПолучитьЭлементы() Цикл
			...
			Если СтрокаКласса.Имя = "ОбщиеФормы" Тогда
				...
				ПроверитьФорму(СтрокаКласса.ЕстьСсылки, "", СтрокаОбъекта, ВесПроверки, КартинкаРежима, РезультатыПроверок);
			...
			Для Каждого СтрокаФормы Из СтрокаОбъекта.ПолучитьЭлементы() Цикл
				...
				ПроверитьФорму(СтрокаКласса.ЕстьСсылки, СтрокаОбъекта.ПолноеИмя, СтрокаФормы, ВесПроверки, КартинкаРежима, РезультатыПроверок);
...
&НаКлиенте
Процедура ПроверитьФорму(ЕстьСсылкиВКлассе, ПолноеИмяОбъекта, СтрокаФормы, ВесПроверки, КартинкаРежима, РезультатыПроверок)
...
Процедура ЗаполнитьНастройкиОбработкиВПротоколе(СтрокиПротокола)
	...
	Для Каждого ЭлементСписка Из РежимыПроверок Цикл
		...
		Если ЭлементСписка.Значение = ВсеРежимы.Документы  Тогда
			...
			КоличествоОбъектов = ?(Объект.СколькоПроверятьОднотипныхДокументов = 0, 1, Объект.СколькоПроверятьОднотипныхДокументов); 
			СтрокиПротокола.Добавить("	Сколько документов одного типа проверять: " + КоличествоОбъектов);
		КонецЕсли; 
		
		Если ЭлементСписка.Значение = ВсеРежимы.Справочники  Тогда
			СтрокиПротокола.Добавить("Настройки проверки справочников:"); 
			КоличествоОбъектов = ?(Объект.СколькоПроверятьОднотипныхОбъектов = 0, 1, Объект.СколькоПроверятьОднотипныхОбъектов);
			СтрокиПротокола.Добавить("	Сколько элементов справочника одного типа проверять: " + КоличествоОбъектов);
		КонецЕсли;
