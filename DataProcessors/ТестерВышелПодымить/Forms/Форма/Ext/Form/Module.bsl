...
Процедура ВыполнитьПроверки(Команда)
	...
	Если ПроверятьСправочники Тогда
		ПроверитьСправочники();
	КонецЕсли;
	
	ЗаполнитьРезультатыТестов();
	СоздатьОтчетАллюр();
...
#Область ОбработчикиКомандФормы_РезультатыТестов

&НаКлиенте
Процедура Результаты_РазвернутьВсеГруппы(Команда)
	
	Для Каждого СтрокаДерева Из РезультатыТестов.ПолучитьЭлементы() Цикл
		Элементы.РезультатыТестов.Развернуть(СтрокаДерева.ПолучитьИдентификатор());
	КонецЦикла;
...
Процедура Результаты_СвернутьВсеГруппы(Команда)
	...
	СвернутьВеткиДерева(Элементы.РезультатыТестов, РезультатыТестов);
...
&НаКлиенте
Процедура ПечатныеФормыОбъектПечатиПриИзменении(Элемент)
	
	ТекущаяСтрокаДерева = Элементы.ПечатныеФормы.ТекущиеДанные;
...
#Область ОбработчикиСобытийЭлементовТаблицыРезультатыТестов

&НаКлиенте
Процедура РезультатыТестовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле <> Элементы.РезультатыТестовПроверяемыйОбъект Тогда
		Возврат;
	КонецЕсли;
	
	ТекущиеДанные = Элементы.РезультатыТестов.ТекущиеДанные;
...
Функция ДеревоМетаданныхКонфигурации(Отбор = Неопределено, ПолучатьСамОбъектМетаданных = Ложь) Экспорт 
	...
	ДобавитьСтрокуВКоллекциюОбъектовМетаданных("Справочники",  "Справочники",
		БиблиотекаКартинок.Справочник, БиблиотекаКартинок.Справочник, КоллекцииОбъектовМетаданных);
	
	ДобавитьСтрокуВКоллекциюОбъектовМетаданных("Документы", "Документы",
		БиблиотекаКартинок.Документ, БиблиотекаКартинок.ДокументОбъект, КоллекцииОбъектовМетаданных);
		
	ДобавитьСтрокуВКоллекциюОбъектовМетаданных("ПланыВидовХарактеристик", "Планы видов характеристик",
		БиблиотекаКартинок.ПланВидовХарактеристик, БиблиотекаКартинок.ПланВидовХарактеристик, КоллекцииОбъектовМетаданных);
	
	ДобавитьСтрокуВКоллекциюОбъектовМетаданных("ПланыВидовРасчета", "Планы видов расчета",
		БиблиотекаКартинок.ПланВидовРасчета, БиблиотекаКартинок.ПланВидовРасчетаОбъект, КоллекцииОбъектовМетаданных);
		
	ДобавитьСтрокуВКоллекциюОбъектовМетаданных("Обработки", "Обработки",
		БиблиотекаКартинок.Обработка, БиблиотекаКартинок.Обработка, КоллекцииОбъектовМетаданных);
		
	Если ДобавитьБизнесПроцессы Тогда
		ДобавитьСтрокуВКоллекциюОбъектовМетаданных("БизнесПроцессы", "Бизнес-процессы",
			БиблиотекаКартинок.БизнесПроцесс, БиблиотекаКартинок.БизнесПроцессОбъект, КоллекцииОбъектовМетаданных);
	КонецЕсли;
		
	Если ДобавитьЗадачи Тогда
		ДобавитьСтрокуВКоллекциюОбъектовМетаданных("Задачи", "Задачи",
			БиблиотекаКартинок.Задача, БиблиотекаКартинок.ЗадачаОбъект, КоллекцииОбъектовМетаданных);
	КонецЕсли;
	...
	Для Каждого СтрокаКоллекции Из КоллекцииОбъектовМетаданных Цикл
		...
		Для Каждого ОбъектМетаданных Из Метаданные[СтрокаКоллекции.Имя] Цикл
			...
			СтрокаДереваОбъект.Картинка  = СтрокаКоллекции.КартинкаОбъекта;
...
&НаСервереБезКонтекста
Процедура ДобавитьСтрокуВКоллекциюОбъектовМетаданных(Имя, Синоним, Картинка, КартинкаОбъекта, Таб)
...
Процедура ЗаполнитьФормыОбъектов()
	...
	ОбщегоНазначения.ЗаполнитьКоллекциюЭлементовДереваДанныхФормы(ФормыОбъектов.ПолучитьЭлементы(), НовоеДерево);
...
Процедура Печать_ПодобратьСсылкиОбъектовПечати()
	...
	Для Каждого СтрокаТипов Из ПечатныеФормы.ПолучитьЭлементы() Цикл
		Для Каждого СтрокаОбъектов Из СтрокаТипов.ПолучитьЭлементы() Цикл
			...
			Попытка
				...
				Иначе
					...
					Иначе
						СтрокаОбъектов.ОбъектПечати = Неопределено;
					КонецЕсли;
...
Процедура ЗаполнитьДокументыКонфигурации()
	...
	ОбщегоНазначения.ЗаполнитьКоллекциюЭлементовДереваДанныхФормы(ДокументыКонфигурации.ПолучитьЭлементы(), НовоеДерево);
...
Процедура ЗаполнитьСправочникиКонфигурации()
	...
	ОбщегоНазначения.ЗаполнитьКоллекциюЭлементовДереваДанныхФормы(СправочникиКонфигурации.ПолучитьЭлементы(), НовоеДерево);
...
Процедура Формы_ВыполнитьВыборкуФорм(ФормыДляВыполненияТестирования)
	...
	ДеревоЗначений = РеквизитФормыВЗначение("ФормыОбъектов");
...
Процедура Формы_ПопытатьсяОткрытьФорму(РежимПроверки, ТекФорма, Ключ)
	...
	Попытка
		...
		ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ТекФорма.ИмяФормы, Ключ, ОписаниеПроверки);
		
	Исключение 
		
		ИнформацияОбИсключении = ИнформацияОбИсключении();
		КраткаяОшибка   = ИнформацияОбИсключении.КраткаяОшибка;
		ПодробнаяОшибка = ИнформацияОбИсключении.ПодробнаяОшибка;
		
		СтатусАллюр = СтатусыАллюр().Ошибка;
		
		ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, ТекФорма.ИмяФормы, 
			Ключ, КраткаяОшибка, ПодробнаяОшибка, СтатусАллюр);
...
Процедура Отчеты_СформироватьФайлОтчета(Параметры)
	...
	Попытка
		...
		ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ПроверяемыйОбъект, , "Создание отчета");
	...
	Исключение
		...
		Описание = "Ошибка при попытке создать объект отчета" + ПодробнаяОшибка; 
			
		СтатусАллюр	= СтатусыАллюр().Ошибка;
				
		ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, ПроверяемыйОбъект,, 
			КраткаяОшибка, Описание, СтатусАллюр);
	...
	Попытка
		...
		ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ПроверяемыйОбъект, , "Компоновка отчета");
	...
	Исключение
		...
		ПредставлениеНавигационнойСсылки = Строка(ПолучитьНавигационнуюСсылку(Параметры.ОтчетВариант));
		
		Описание = "Ошибка при попытке скомпоновать отчет "
			+ Символы.ПС + ПредставлениеНавигационнойСсылки 
	...
	Попытка
		...
		Иначе
			...
			ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ПроверяемыйОбъект, , "Получение настроек варианта отчета");
	...
	Исключение
		...
		ПредставлениеСсылки = Строка(ПолучитьНавигационнуюСсылку(Параметры.ОтчетВариант));
		
		Описание = "Ошибка при попытке загрузкить настройки " + ПодробнаяОшибка;
	...
	Попытка
		...
		ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ПроверяемыйОбъект, , "Скомпоновать результат");
...
&НаКлиенте
Процедура ПроверитьПечатныеФормы()
	...
	Для Каждого СтрокаТипаМетаданных Из ПечатныеФормы.ПолучитьЭлементы() Цикл
		...
		Для Каждого СтрокаГруппы Из СтрокаТипаМетаданных.ПолучитьЭлементы() Цикл
			...
			Для Каждого СтрокаЭлемента Из СтрокаГруппы.ПолучитьЭлементы() Цикл
				...
				Иначе
					...
					ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, СтрокаЭлемента.ПолноеИмя,, 
...
Процедура Печать_СформироватьФайлПечатнойФормы(Параметры, ПрефиксФайла = "")
	...
	Попытка
		...
		ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ПолноеИмя, ОбъектПечати, "Печатная форма");
	...
	Исключение
		...
		ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, ПолноеИмя, ОбъектПечати, 
			КраткаяОшибка, ПодробнаяОшибка, СтатусАллюр);
...
Процедура ПроверитьДокументы()
	...
	Исключение
		...
		Описание = "Ошибка при выполнении тестирования" + Символы.ПС + ПодробнаяОшибка;
			
		СтатусАллюр		= СтатусыАллюр().Ошибка;
				
		ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, РежимПроверки,, 
			КраткаяОшибка, Описание, СтатусАллюр);
...
Процедура Документы_ВыполнитьТестНаКлиенте()
	...
	Для Каждого ТекКласс Из ДокументыКонфигурации.ПолучитьЭлементы() Цикл
...
Процедура Документы_ПротестироватьОбъектПоСсылкеНаКлиенте(ИмяОбъектаМетаданных, Ссылка)
	...
	Попытка
		...
		ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ИмяФормыТестирования, Ссылка, "Открытие формы объекта");
	...
	Исключение
		...
		Описание = "Ошибка открытия формы " + ИмяФормыТестирования
			+ Символы.ПС + "Открыть форму объекта по ссылке : " + НавигационнаяСсылка
			+ Символы.ПС + ПодробнаяОшибка;
			
		СтатусАллюр = СтатусыАллюр().Ошибка;
		
		ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, ИмяФормыТестирования, Ссылка, 
			КраткаяОшибка, Описание, СтатусАллюр);
	...
	Попытка
		...
		ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ИмяФормыТестирования, Ссылка, "Запись объекта");
	...
	Исключение
		...
		Описание =  "Ошибка проверки заполнения формы " + ИмяФормыТестирования
			+ Символы.ПС + "Проверить заполнение (записать) объект по ссылке: " + НавигационнаяСсылка
			+ Символы.ПС + ПодробнаяОшибка;
	...
	Для Каждого ТекКласс Из ДокументыКонфигурации.ПолучитьЭлементы() Цикл
		...
		Для Каждого ТекДокумент Из ТекКласс.ПолучитьЭлементы() Цикл
			...
			Если НЕ ЗначениеЗаполнено(МассивРегистраторов) Тогда
				...
				ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, ТекДокумент.ПолноеИмя,, 
					КраткоеОписание, КраткоеОписание, СтатусАллюр);			
	...
	Для Каждого ТекДокумент Из МассивРегистраторов Цикл
		...
		Если ЕстьОшибкиЗаполнения Тогда
			...
			Для Каждого СтрокаСообщения Из СообщенияПользователю Цикл
				...
				Описание = "Ошибка проверки заполнения " + ИмяМетаданного + ПолучитьНавигационнуюСсылку(Ключ) + ПодробнаяОшибка;	
						
				СтатусАллюр		= СтатусыАллюр().Ошибка;

				ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, ИмяМетаданного, 
					Ключ, КраткаяОшибка, Описание, СтатусАллюр);
		...
		Иначе
			...
			ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ИмяМетаданного, ТекДокумент, "Проверка заполнения");
		...
		Попытка
			...
			ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ИмяМетаданного, ТекДокумент, "Проведение документа");
		...
		Исключение
			...
			ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, ИмяМетаданного, 
				ТекДокумент, КраткаяОшибка, Описание, СтатусАллюр);
...
Процедура Документы_ЗаполнитьПроверяемыеСсылки()
	...
	НовоеДерево = РеквизитФормыВЗначение("ДокументыКонфигурации", Тип("ДеревоЗначений"));
	Документы_ЗаполнитьСпискиПроверяемыхСсылок(НовоеДерево);
	ЗначениеВРеквизитФормы(НовоеДерево, "ДокументыКонфигурации");
...
Процедура Документы_ЗаполнитьСпискиПроверяемыхСсылок(Дерево)
	...
	Для Каждого ТекДокумент Из Дерево.Строки[0].Строки Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(Документы_КоличествоОбъектов) Тогда
		...
		Если ЗначениеЗаполнено(Количество) Тогда
			ПервыеКоличествоСтрокой = "0";	
		Иначе
			ПервыеКоличествоСтрокой = Формат(Количество, "ЧГ=");	
		КонецЕсли;
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПЕРВЫЕ 1", "ПЕРВЫЕ " + ПервыеКоличествоСтрокой);
...
&НаКлиенте
Процедура ПроверитьСправочники()
	...
	Исключение
		...
		Описание = "При тестировании объектов не должно возникать ошибок обработки " + ПодробнаяОшибка;
...
Процедура Справочники_ПротестироватьПоТиповымНастройкам()
	...
	ОбъектыКТиповомуТестированию = Справочники_ПолучитьОбъектыКТиповомуТестированию(СправочникиКонфигурации);
	...
	Для Каждого ТестируемыйОбъект Из ОбъектыКТиповомуТестированию Цикл
		...
		Если НЕ ЗначениеЗаполнено(ТестируемыеСсылки) Тогда
			...
			ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, ТестируемыйОбъект,, 
				КраткоеОписание, КраткоеОписание, СтатусАллюр);	
...
Функция Справочники_ПротестироватьОбъектПоСсылкеНаКлиенте(ИмяОбъектаМетаданных, Ссылка)
	...
	Попытка
		...
		ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ПроверяемыйОбъект, Ссылка, "Открытие формы объекта по ссылке");
	...
	Исключение
		...
		ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, ПроверяемыйОбъект, 
			Ссылка, КраткаяОшибка, Описание, СтатусАллюр);
	...
	Попытка
		...
		ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, Форма.ИмяФормы, Ссылка, "Заполнение формы");
	...
	Исключение
		...
		ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, Форма.ИмяФормы, 
			Ссылка, КраткаяОшибка, Описание, СтатусАллюр);
...
Процедура Справочники_ПротестироватьЗаписьОбъектаНаСервере(СсылкаНаОбъект)
	...
	Попытка
		...
		ДобавитьИнформациюПройденнойПроверки(ВыполненныеПроверки, РежимПроверки, ПроверяемыйОбъект, СсылкаНаОбъект, "Запись объекта");
	...
	Исключение
		...
		Описание = "Ошибка при перезаписи объекта на сервере " + ПодробнаяОшибка;

		СтатусАллюр	= СтатусыАллюр().Ошибка;
		
		ДобавитьИнформациюПроверки(ВыполненныеПроверки, РежимПроверки, ПроверяемыйОбъект, 
			СсылкаНаОбъект, КраткаяОшибка, Описание, СтатусАллюр);
...
&НаСервере
Процедура ЗаполнитьРезультатыТестов()
	
	КорневыеУзлы = РезультатыТестов.ПолучитьЭлементы();
	КорневыеУзлы.Очистить();
	
	ВыполненныеПроверки.Сортировать("РежимПроверки");
	
	СтатусАллюр		= СтатусыАллюр().Ошибка;
	СтрокиОшибок	= ВыполненныеПроверки.НайтиСтроки(Новый Структура("СтатусАллюр", СтатусАллюр));
...
Процедура ДобавитьИнформациюПроверки(ТаблицаИнформации, РежимПроверки, ПроверяемыйОбъект, 
	...
	НоваяСтрокаВыполненныеПроверки						= ТаблицаИнформации.Добавить();
	НоваяСтрокаВыполненныеПроверки.РежимПроверки		= РежимПроверки;
	НоваяСтрокаВыполненныеПроверки.ПроверяемыйОбъект	= ПроверяемыйОбъект;
	НоваяСтрокаВыполненныеПроверки.Данные				= Данные;
	НоваяСтрокаВыполненныеПроверки.КраткоеОписание		= КраткоеОписание;
	НоваяСтрокаВыполненныеПроверки.Описание				= Описание;
	НоваяСтрокаВыполненныеПроверки.Картинка				= КартинкаРежимаПроверки(РежимПроверки);
	НоваяСтрокаВыполненныеПроверки.СтатусАллюр			= Статус;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ИнформацияОбИсключении()
...
Функция СоздатьОтчетАллюрНаСервере()
	...
	Для Каждого СтрокаИнформация Из ВыполненныеПроверки Цикл
