...
Процедура ПриОткрытии(Отказ)
	...
	Если не РольДоступна("ПолныеПрава") Тогда
		Объект.ПерезаполнятьСвойства.Видимость = Ложь;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АдресВоВремХран = ПоместитьВоВременноеХранилище(СКД_Отбор, УникальныйИдентификатор);
...
Процедура СоздатьДоговора(Команда)
	ПоказатьВопрос(Новый ОписаниеОповещения("ВопросСозданиеДоговора", ЭтаФорма), "Создать договоры?", 
					РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
КонецПроцедуры

&НаКлиенте
Процедура УстановитьДоговорОсновным(Команда)
	ПоказатьВопрос(новый ОписаниеОповещения("ВопросУстановитьДоговорОсновным", ЭтаФорма), 
					"Установить договоры основными?", РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
КонецПроцедуры
...
Процедура ВыполнитьСозданиеДоговораНаСервере(ПараметрыСезона, Контрагент)
	...
	Если нашли_договор = Неопределено Тогда
		...
		спр.Код = сокрЛП(сред(Контрагент.Код, 3)) + "_" + сокрЛП(ПараметрыСезона.гф_Номер) + прав(ПараметрыСезона.гф_Год, 2);
		...
		//НовыйКонтрагент =  ПроверкаДоговоровКонтрагента(Контрагент);
		ДоговорПредидущегоСезона = НайтиДоговорПредыдущегоСезона(Контрагент);
		Если ДоговорПредидущегоСезона <> Неопределено Тогда
			спр.Номер = ДоговорПредидущегоСезона.гф_Номер; 
		Иначе 	
		...
		Если ДоговорПредидущегоСезона <> Неопределено Тогда
			...
			ПерезаписатьСвойства(ДоговорПредидущегоСезона, спр); 
	...
	Иначе
		...
		Если Объект.ПерезаполнятьСвойства И ДоговорПредидущегоСезона <> Неопределено  Тогда
			ПерезаписатьСвойства(ДоговорПредидущегоСезона, нашли_договор);
		КонецЕсли;
...
Функция ПолучениеКонтрагентовЧерезПостроитель()
	...
	МакетКомпоновки = КомпоновщикМакета.Выполнить(СКД_Отбор, КомпоновщикНастроек.ПолучитьНастройки(), , , Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"));
...
&НаСервере
Функция ПроверкаБесрочныхПроцентов(Номеклатура)
	...
	Запрос.Текст = 
		...
		|	И гф_ПроцентыПредоплаты.Номеклатура = &Номеклатура";
	
	Запрос.УстановитьПараметр("Номеклатура", Номеклатура);
...
&НаСервере
Процедура ПерезаписатьСвойства(ДоговорПредидущегоСезона, спр)
	
	ИзменяемыйДоговор = спр.ПолучитьОбъект();
	
	ОтгрузкаКодовМаркировки = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Отгрузка кодов маркировки парами "); 
	ОтгрузкаКодовМаркировкиЗначение = ДоговорПредидущегоСезона.ДополнительныеРеквизиты.Найти(ОтгрузкаКодовМаркировки);
	
	ПервичныеДокументы = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Первичные документы в парах коробах"); 
	ПервичныеДокументыЗначение = ДоговорПредидущегоСезона.ДополнительныеРеквизиты.Найти(ПервичныеДокументы); 
	...
	НовыйРеквизит1.Значение = ОтгрузкаКодовМаркировкиЗначение;            
	...
	НовыйРеквизит2.Значение = ПервичныеДокументыЗначение; 
	
	СтраховойДепозит = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Страховой депозит"); 
	СтраховойДепозитЗначение = ДоговорПредидущегоСезона.ДополнительныеРеквизиты.Найти(СтраховойДепозит); 
	
	Предоплата = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Предоплата"); 
	ПредоплатаЗначение = ДоговорПредидущегоСезона.ДополнительныеРеквизиты.Найти(Предоплата);
	
	Если ПроверкаБесрочныхПроцентов(СтраховойДепозитЗначение.Значение) = Истина Тогда
		...
		НовыйРеквизит3.Значение = СтраховойДепозитЗначение; 
	КонецЕсли;
	
	Если ПроверкаБесрочныхПроцентов(ПредоплатаЗначение.Значение) = Истина Тогда
		...
		НовыйРеквизит4.Значение = ПредоплатаЗначение; 
	КонецЕсли;
