...
Процедура ВыполнитьСозданиеДоговораНаСервере(ПараметрыСезона, Контрагент)
	...
	Если нашли_договор = Неопределено Тогда
		...
		Если ДоговорПредыдущегоСезона <> Неопределено Тогда
			спр.Номер = ДоговорПредыдущегоСезона.гф_Номер; 
		Иначе 	
	...
	Иначе
		...
		Если Объект.ПерезаполнятьСвойства И ДоговорПредыдущегоСезона <> Неопределено  Тогда
			ПерезаписатьСвойства(ДоговорПредыдущегоСезона, нашли_договор);
		КонецЕсли;
...
&НаСервере
Процедура ПерезаписатьСвойства(ДоговорПредыдущегоСезона, спр)
	...
	Если ПроверкаБессрочныхПроцентов(СтраховойДепозитЗначение.Значение) = Истина Тогда
	...
	Если ПроверкаБессрочныхПроцентов(ПредоплатаЗначение.Значение) = Истина Тогда
