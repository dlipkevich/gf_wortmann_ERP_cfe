...
Процедура ВыполнитьОсновныеДействия() Экспорт
	...
	ВидДвиженияВыводИзОборота = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаДистанционнаяПродажа;
...
Процедура ОбработатьВыборкуВыводИзОборота(Выборка, ВидДвижения);
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаВнешняяСистема.Следующий() Цикл
			...
			Если ТекВнешняяСистема = Перечисления.гф_ВнешниеСистемы.Wildberries Тогда
				НаименованиеПервичногоДокумента = "Чек ККМ";
				СтрокаКомментарияДокумент = " по чеку ККМ №";
			Иначе
				НаименованиеПервичногоДокумента = "Заказ покупателя";
				СтрокаКомментарияДокумент = " по заказу №";
			КонецЕсли;
			...
			Пока ВыборкаДатаОперации.Следующий() Цикл
				...
				СтруктураШапки.Вставить("НаименованиеПервичногоДокумента", НаименованиеПервичногоДокумента);
					...
					Пока ВыборкаТовары.Следующий() Цикл
						Если НЕ ЗначениеЗаполнено(текДатаЗаказа) Тогда
							...
							Если НЕ ГруппироватьДокументыПоДате Тогда
								...
								ДокВывода.Комментарий = текКомментарий + ТекВнешняяСистема + СтрокаКомментарияДокумент + текНомерЗаказа + " от " + Формат(текДатаЗаказа, "ДЛФ=Д");
							КонецЕсли;
						КонецЕсли;
				...
				Пока ВыборкаНомерЗаказа.Следующий() Цикл;
					...
					Пока ВыборкаТовары.Следующий() Цикл
						Если НЕ ЗначениеЗаполнено(текДатаЗаказа) Тогда
							текДатаЗаказа = ВыборкаТовары.ДатаЗаказа;
							Если НЕ ГруппироватьДокументыПоДате Тогда
								ДокВозвата.Комментарий = текКомментарий + ТекВнешняяСистема + СтрокаКомментарияДокумент + текНомерЗаказа + " от " + Формат(текДатаЗаказа, "ДЛФ=Д");
							КонецЕсли;
						КонецЕсли;	
						//Делаем запись в ТЗ для заполнения данных В РС
						нСтрокаРС = тзДляЗаписиВРС.Добавить();
						...
						стрТовары.НаименованиеПервичногоДокумента = НаименованиеПервичногоДокумента;
