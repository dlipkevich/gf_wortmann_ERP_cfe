...
Процедура РассчитатьЛимиты()
	...
	ТаблицаКонтрагенты.Колонки.Добавить("Контрагент");
    ТаблицаКонтрагенты.Колонки.Добавить("Организация");
	...
	Для каждого Строка Из ДеревоЗначений.Строки Цикл
		Для каждого Строка_Н Из Строка.Строки Цикл
						Для каждого Строка_Т Из Строка_Н.Строки Цикл
							...
							Стр = ТаблицаКонтрагенты.Добавить();
							Стр.Контрагент = Строка_Т.Клиент;
							Стр.Организация = Строка_Т.Организация;
							//Стр.Статус = "Кислый";
							//Если Строка_Т.Отгрузить Тогда
							//	Объект.СтоимостьОтмеченныхКОтгрузкеТоваров = Объект.СтоимостьОтмеченныхКОтгрузкеТоваров + Строка_Т.Стоимость;
							//	
	...
	//
	//Для каждого Элемент Из СписокКонтрагенты Цикл
	...
	ТаблицаКонтрагенты.Свернуть("Контрагент, Организация");
	
	
	ТаблицаВидЛимита = Новый ТаблицаЗначений;
	//ТаблицаВидЛимита.Колонки.Добавить("Контрагент");
	//ТаблицаВидЛимита.Колонки.Добавить("Организация");
	//ТаблицаВидЛимита.Колонки.Добавить("ВидЛимита");
	//ТаблицаВидЛимита.Колонки.Добавить("Статус");
	//ТаблицаВидЛимита.Колонки.Добавить("Сумма");
	...
	ТаблицаВидЛимита = РезультатЗапроса.Выгрузить();
	 
	
	ЛимитТЗ = Новый ТаблицаЗначений;
	ЛимитТЗ.Колонки.Добавить("Период");
	ЛимитТЗ.Колонки.Добавить("Партнер");
	ЛимитТЗ.Колонки.Добавить("Организация");
	ЛимитТЗ.Колонки.Добавить("ВидЛимита");
	ЛимитТЗ.Колонки.Добавить("Статус");
	ЛимитТЗ.Колонки.Добавить("Сумма");
	//Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		
		Для каждого Строка Из ТаблицаВидЛимита Цикл
			
			//СтрокаНайдена = ТаблицаВидЛимита.Найти(Строка.Клиент, Строка.Организация, Строка.ВидЛимита, );
			
			Если ЛимитТЗ.Количество() = 0 Тогда
				Стр = ЛимитТЗ.Добавить();
			...
			Иначе
				...
				Для каждого Строка2 Из ЛимитТЗ Цикл
					
					Если Строка2.Партнер = Строка.Партнер
						И Строка2.Организация = Строка.Организация
						И Строка2.ВидЛимита = Строка.ВидЛимита Тогда
						
						Если Строка2.Период < Строка.Период Тогда
							Строка2.Период = Строка.Период;
							Строка2.Статус = Строка.Статус;
							Строка2.Сумма = Строка.Сумма;
							НайденЛимит = Истина;
							Прервать;
						КонецЕсли;
