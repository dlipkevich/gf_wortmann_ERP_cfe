...
Процедура РассчитатьЛимиты(ДеревоЗначений)
	...
	ТЗЛимит.Колонки.Добавить("ДолгОрганизицииКонтрагенту");
	ТЗЛимит.ЗаполнитьЗначения(Ложь, "НайденВТаблицеСЛимитами");
	ТЗЛимит.ЗаполнитьЗначения(0, "АвансОтКонтрагента");
	ТЗЛимит.ЗаполнитьЗначения(0, "ДолгОрганизицииКонтрагенту");
			...
			Если Не СтрВыборка.АналитикаУчетаПоПартнерам = NULL 
				...
				Если ЗначениеЗаполнено(СтрВыборка.гф_Сезон) Тогда
					...
					Если Не СтрВыборка.СуммаОстаток = 0 И СтрВыборка.СуммаОстаток > 0 Тогда
						Строка_ЛимитыТЗ.АвансОтКонтрагента = Строка_ЛимитыТЗ.АвансОтКонтрагента + СтрВыборка.СуммаОстаток;
					Иначе
						Строка_ЛимитыТЗ.ДолгОрганизицииКонтрагенту = Строка_ЛимитыТЗ.ДолгОрганизицииКонтрагенту + СтрВыборка.СуммаОстаток * (-1);
					КонецЕсли;
			...
			Если Не СтрВыборка.АналитикаУчетаПоПартнерам = NULL Тогда
				...
				Если ЗначениеЗаполнено(СтрВыборка.гф_Сезон) Тогда
					...
					Если НайденныеСтрокиТЗЛимит.Количество() = 1 Тогда
						...
						Если Не СтрВыборка.СуммаОстаток = 0 И СтрВыборка.СуммаОстаток > 0 Тогда
							НайденныеСтрокиТЗЛимит[0].ДолгОрганизицииКонтрагенту = НайденныеСтрокиТЗЛимит[0].ДолгОрганизицииКонтрагенту + СтрВыборка.СуммаОстаток;
						Иначе
					...
					ИначеЕсли НайденныеСтрокиТЗЛимит.Количество() = 0 Тогда	
						...
						Стр.ДолгОрганизицииКонтрагенту = 0;
						//Стр.СуммаВзаиморасчетов = СтрВыборка.СуммаОстаток * (-1);
						Стр.НайденВТаблицеСЛимитами = Ложь;
						
						Если Не СтрВыборка.СуммаОстаток = 0 И СтрВыборка.СуммаОстаток > 0 Тогда
							Стр.ДолгОрганизицииКонтрагенту = СтрВыборка.СуммаОстаток;
						Иначе
	...
	ТЗКлиентЗаказ.Колонки.Добавить("ДолгОрганизицииКонтрагенту");
	ТЗКлиентЗаказ.ЗаполнитьЗначения(0, "АвансОтКонтрагента");
	ТЗКлиентЗаказ.ЗаполнитьЗначения(0, "ДолгОрганизицииКонтрагенту");
		...
		Для Каждого Строка_Заказ Из ТЗКлиентЗаказ Цикл
			...
			Если Строка_Заказ.Заказ = ВыборкаДетальныеЗаписи.ЗаказКлиента Тогда
				...
				//Если Не ВыборкаДетальныеЗаписи.СуммаДокумента = 0 И ВыборкаДетальныеЗаписи.СуммаДокумента > 0 Тогда
					Строка_Заказ.ДолгОрганизицииКонтрагенту = Строка_Заказ.ДолгОрганизицииКонтрагенту + ВыборкаДетальныеЗаписи.СуммаДокумента;
				//Иначе
	...
	Для Каждого Строка_ЛимитыТЗ Из ТЗЛимит Цикл
		Для Каждого Строка_Заказ Из ТЗКлиентЗаказ Цикл
			...
			Если Строка_ЛимитыТЗ.Партнер = Строка_Заказ.Клиент
				...
				Строка_ЛимитыТЗ.ДолгОрганизицииКонтрагенту = Строка_ЛимитыТЗ.ДолгОрганизицииКонтрагенту + Строка_Заказ.ДолгОрганизицииКонтрагенту;
	...
	Для Каждого Строка_Лимиты Из ТЗЛимит Цикл
		...
		Если Строка_Лимиты.Статус = Справочники.гф_СтатусыЛимитов.Действующий Тогда
			...
			Если ТЗЛимитыКонтрагентов.Количество() = 0 Тогда
				...
				Стр.Сумма = Строка_Лимиты.Сумма + Строка_Лимиты.АвансОтКонтрагента - Строка_Лимиты.ДолгОрганизицииКонтрагенту;
			Иначе
		...
		ИначеЕсли Строка_Лимиты.Статус = Неопределено И (Строка_Лимиты.АвансОтКонтрагента > 0 Или Строка_Лимиты.АвансОтКонтрагента > 0) Тогда
			...
			Иначе
				Для Каждого Строка_ЛимитыТЗ Из ТЗЛимитыКонтрагентов Цикл
					...
					Если Строка_ЛимитыТЗ.Партнер = Строка_Лимиты.Партнер И Строка_Лимиты.Статус = Неопределено И Строка_ЛимитыТЗ.Организация = Строка_Лимиты.Организация Тогда
						//Строка_ЛимитыТЗ.Сумма = Строка_ЛимитыТЗ.Сумма + Строка_Лимиты.СуммаВзаиморасчетов;
						Строка_ЛимитыТЗ.Сумма = Строка_ЛимитыТЗ.Сумма + Строка_Лимиты.АвансОтКонтрагента - Строка_Лимиты.ДолгОрганизицииКонтрагенту;
						НайденЛимит = Истина;
					КонецЕсли;
		...
		Если СтрокаЗаказы.ВозможнаЗаписьЗаказа И СтрокаЗаказы.НовыеДанныеДляТаблицыПоЗаказу.Количество() > 0 Тогда
			...
			Если СтрокаЗаказы.Перепроведен Тогда
				...
				Попытка
					...
					СтруктураЗадания = ОформитьРасходныеОрдераНаСервере(ОбъектыДляОрдера, Отказ);
...
//Процедура РазбитьСтрокуЗавершениеДобавленоПоПричине(НоваяСтрока, ДополнительныеПараметры, ТекущаяСтрока)
//	
...
//Функция ПолучитьРазницуТаблицЗначений(СтрокаУпаковочныйЛист)
//	
...
&НаСервере
Функция ОформитьРасходныеОрдераНаСервере(ОбъектыДляОрдера, Отказ)
...
//&НаСервере
//Функция ПолучитьОтмеченныеДанныеДереваЗначенийТоварыВКоробках()
//	
//	ДеревоЗначений = РеквизитФормыВЗначение("ДеревоДокументов");
//	
//	// Таблица для записи каждого заказа с конкретным списком упаковочных листов по этому заказу
