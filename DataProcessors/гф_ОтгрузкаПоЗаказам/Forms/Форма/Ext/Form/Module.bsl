...
&НаКлиенте
Процедура бф_ЗагрузкаСпискаАртикулов(Команда)
//+++ БФ Бобнев К.С. 05.04.24
	Если ЗначениеЗаполнено(Объект.Склад) И НЕ Объект.ТоварыВКоробках Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("бф_ПослеЗакрытияТабДока", ЭтотОбъект);
		ОткрытьФорму("Обработка.гф_ОтгрузкаПоЗаказам.Форма.бф_ФормаЗагрузкиИзТабличногоДокумента",, ЭтотОбъект,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		ТекстСообщения = НСтр("ru = 'Заполнение возможно только если у склада НЕ установлен флаг ""Товары в коробах""!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;	
КонецПроцедуры
...
Процедура РассчитатьОбщуюСтоимостьОтмеченныхКОтгрузкеТоваровКлиент()
	...
	//Если Объект.ТоварыВКоробках Тогда //--- БФ Бобнев К.С. 11.04.24
			...
			Для Каждого Строка_Н Из Строка.ПолучитьЭлементы() Цикл
				...
				Для Каждого Строка_ТТ Из Строка_Н.ПолучитьЭлементы() Цикл
					...
					Для Каждого Строка_Т Из Строка_ТТ.ПолучитьЭлементы() Цикл
						...
						РассчитатьПоСтрокеДерева(Строка_Т);
...
Процедура РассчитатьЛимиты(ДеревоЗначений)
	...
	// ++ Галфинд ВолковЕВ 2024/05/07
	|	И РеализацияТоваровУслуг.Договор.гф_Сезон = Значение(Справочник.КоллекцииНоменклатуры.ПустаяСсылка)
	// -- Галфинд ВолковЕВ 2024/05/07
...
Процедура ОтгрузитьПродолжитьСоСкидками(СтруктураПараметров)
	...
	АктивныеЛимитыКлиента = ПолучитьЛимиты(СтруктураПараметров);
...
Функция ПолучитьЛимиты(СтруктураПараметров)
	...
	МассивСуммаОтгрузки = СтруктураПараметров.гф_СуммаДляОрдера;
	
	СуммаОтгрузки = 0; 
	
	Для Каждого СтрокаМассива Из МассивСуммаОтгрузки Цикл
		
		СуммаОтгрузки = СуммаОтгрузки + СтрокаМассива.Сумма;
		
	КонецЦикла;
	
	Для Каждого СтрокаКлиент Из ТабСуммаОтгрузки Цикл
		
		Если СуммаОтгрузки > Число(СтрокаКлиент.ДействующийЛимит) Тогда
	...
	Для Каждого СтрокаКопия Из КопияТаб Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			Попытка
				...
				СтруктураПереданныхДанных.Вставить("ЗаказАдрес", СтрокаКопия.НомерАдреса);
				// -- Галфинд ВолковЕВ 2024/04/25
				...
				СтруктураЗадания = ОформитьРасходныеОрдераНаСервере(ОбъектыДляОрдера, Отказ, ОбъектыЗаказы, СтруктураПереданныхДанных);
...
Процедура ДобавитьСтрокуРазбиениемУбратьКоличествоПересорт(ПараметрыОбработки, ТекущаяСтрока, ТаблицаИсточник, ПроверкаОтказ)
	...
	Если ТекущаяСтрока.КоличествоУпаковок > Количество Тогда
		...
		//ПолучитьМаксКодСтроки(ТаблицаИсточник, ТаблицаИсточник, НоваяСтрока);
		НоваяСтрока.КодСтроки = ПолучитьМаксНомерСтрокиПлюсОдин(ТаблицаИсточник, ТаблицаИсточник, "КодСтроки");
...
Процедура ДобавитьСтрокуРазбиениемНеПолный(ПараметрыОбработки, НоваяТаблица, ТекущаяСтрока, ТаблицаИсточник, ПроверкаОтказ,
	...
	Если Не ТипЗнч(ТекущаяСтрока) = Тип("Массив") Тогда
		...
		Если ТекущаяСтрока[ПараметрыРазбиенияСтроки.ИмяПоляКоличество] > Количество Тогда
			...
			//ПолучитьМаксКодСтроки(НоваяТаблица, ТаблицаИсточник, НоваяСтрока);
			НоваяСтрока.КодСтроки = ПолучитьМаксНомерСтрокиПлюсОдин(НоваяТаблица, ТаблицаИсточник, "КодСтроки");
...
&НаСервере
Функция ОформитьРасходныеОрдераНаСервере(ОбъектыДляОрдера, Отказ, ОбъектЗаказ, СтруктураПереданныхДанных)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Склад") Цикл
		...
		Пока Выборка.СледующийПоЗначениюПоля("Получатель") Цикл
			...
			ПараметрыПереоформленияРасходныхОрдеров.Вставить("гф_СуммаДляОрдера");
			ПараметрыПереоформленияРасходныхОрдеров.гф_СуммаДляОрдера = СтруктураПереданныхДанных.гф_СуммаДляОрдера;
			// -- Галфинд ВолковЕВ 2024/04/05
			
			// ++ Галфинд ВолковЕВ 2024/04/25
			ПараметрыПереоформленияРасходныхОрдеров.Вставить("гф_ЗаказАдрес");
			ПараметрыПереоформленияРасходныхОрдеров.гф_ЗаказАдрес = СтруктураПереданныхДанных.ЗаказАдрес;
			// -- Галфинд ВолковЕВ 2024/04/25
...
Функция ПолучитьОтмеченныеДанныеДереваЗначений()
	...
	//							СтрокаСписокУпаковочныйЛистИлиНоменклатура.Номенклатура 		= Строка_Т.Номенклатура;
	//							СтрокаСписокУпаковочныйЛистИлиНоменклатура.Характеристика 		= Строка_Т.Характеристика;
	//							// СадомцевСА 15.03.2024
	//							//СтрокаСписокУпаковочныйЛистИлиНоменклатура.Количество 			= Строка_Т.КоличествоНоменклатуры;
	//							СтрокаСписокУпаковочныйЛистИлиНоменклатура.Количество 			= Строка_Т.КоличествоОтгрузить;
	//						КонецЕсли;
	//						
	//					КонецЕсли;
	//					
...
Функция ЗаполнитьДеревоЗначенийДляНеКоробногоСклада()
	...
	Запрос.Текст = 
		...
		|	ВТ.Заказ КАК Заказ,
		//+++ БФ Бобнев К.С. 05.04.24
		|	ВТ.Заказ.Номер КАК бф_НомерЗаказа,
		//--- БФ Бобнев К.С. 05.04.24
	...
	Для Каждого СтрокаРазница Из ТЗСНоменклатуройИКоличествомДляКорректировки Цикл
		...
		Если Не НайтиСтроки_ВариантОбеспечения = Неопределено И НайтиСтроки_ВариантОбеспечения.Количество() > 0 Тогда
			...
			Если СтрокаРазница.ДобавитьКоличество > 0 Тогда
				...
				//ПолучитьМаксКодСтроки(ТЗ_Заказ_Товары, ТЗ_Заказ_Товары, НоваяСтрока);
				НоваяСтрока.КодСтроки = ПолучитьМаксНомерСтрокиПлюсОдин(ТЗ_Заказ_Товары, ТЗ_Заказ_Товары, "КодСтроки");
		...
		Иначе
			...
			Если СтрокаРазница.ДобавитьКоличество > 0 Тогда
				...
				Если НайтиСтроки_ВариантОбеспечения.Количество() > 0 Тогда
					...
					// ++ Галфинд ВолковЕВ 2024/03/18 Заполнием из найденной строки однотипные данные из номенклатуры комплектации				
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ВременнаяТЗ_Сумма[0], ,
					"НомерСтроки, КодСтроки, ИдентификаторСтроки, Характеристика, Количество, КоличествоУпаковок, Сумма, СуммаНДС, СуммаСНДС, гф_ДобавленоПоПричине, гф_ПричинаДобавления, ДатаОтгрузки");
											
					НоваяСтрока.ИдентификаторСтроки = Новый УникальныйИдентификатор();
					НоваяСтрока.НомерСтроки = ПолучитьМаксНомерСтрокиПлюсОдин(ТЗ_Заказ_Товары, ТЗ_Заказ_Товары, "НомерСтроки");
					НоваяСтрока.КодСтроки = ПолучитьМаксНомерСтрокиПлюсОдин(ТЗ_Заказ_Товары, ТЗ_Заказ_Товары, "КодСтроки");
	...
	Для Каждого СтрокаКомплектацияРазбиение Из ТабКомплектация Цикл
		...
		Если Не ТипЗнч(СтрокаТовары) = Тип("Массив") Тогда
			...
			Если СтрокаТовары.КоличествоУпаковок > Количество Тогда
				...
				//ПолучитьМаксКодСтроки(ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, НоваяСтрока);
				НоваяСтрока.КодСтроки = ПолучитьМаксНомерСтрокиПлюсОдин(ТЗ_Новая_Заказ_Товары, ТЗ_Заказ_Товары, "КодСтроки");
...
Функция ПолучитьПараметрыФормы()
	...
	Для Каждого СтрокаТЗ Из ТаблицаЗначений Цикл
		...
		Если флДобавитьЗаказ Тогда
			...
			Если СтрокаТЗ.КоличествоНоменклатуры Тогда // Товары в коробах 
				...
				СтрокаЗаказа.Вставить("ЦенаУпаковочногоЛиста",			СтрокаТЗ.Цена);
				СтрокаЗаказа.Вставить("СтоимостьУпаковочногоЛиста",		СтрокаТЗ.Стоимость); 
	...
	// Волк4
	Если Объект.ТоварыВКоробках Тогда
		ПараметрыФормы.Вставить("ТоварыВКоробках", Истина);
	КонецЕсли;
	// Волк4
...
Процедура ПрименитьСкидкиКСтроке(ОбщаяСкидка, Скидки, МассивСтрокКомментария, ЭтоНаценки = Ложь)
	...
	// ++ Галфинд ВолковЕВ 2024/05/03 Суммирование всех скидок в общую скидку
	...
	Если Скидки.Количество() > 0 Тогда
		...
		ОбщаяСкидка = (ЗначениеСто - СуммаЗначенийСкидок) / ЗначениеСто;
...
Функция ПолучитьРезультатКомпоновкиДанных2(Склад, Клиент, СхемаКомпоновкиДанных, ОтборЗаказ)
	...
	Запрос.Текст = 
		...
		// ++ Галфинд ВолковЕВ 2024/04/03
		|	ЕСТЬNULL(ЗаказКлиентагф_ТоварыВКоробах.СуммаНДС, 0) КАК СуммаНДСУпаковочногоЛиста,
		// -- Галфинд ВолковЕВ 2024/04/03
		|	УпаковочныйЛистДанные.УпаковочныйЛистКомплектация КАК УпаковочныйЛистКомплектация,
		|	УпаковочныйЛистДанные.ОсобенностьУчета КАК ОсобенностьУчета
		...
		|	ВЫБОР
		|	КОГДА УпаковочныйЛистДанныеЗаказов.ЦенаВключаетНДС = Истина
		|		ТОГДА 
		|		УпаковочныйЛистДанныеЗаказов.ЦенаУпаковочногоЛиста
		|			* УпаковочныйЛистДанныеЗаказов.КоличествоОстаток
		|	* (1 - УпаковочныйЛистДанныеЗаказов.Скидка / 100)
		|	ИНАЧЕ
		|		(УпаковочныйЛистДанныеЗаказов.ЦенаУпаковочногоЛиста
		|			* УпаковочныйЛистДанныеЗаказов.КоличествоОстаток
		|		)
		|	* (1 + ЗаказНоменклатураХарактеристика.СтавкаНДС / 100)
		|	* (1 - УпаковочныйЛистДанныеЗаказов.Скидка / 100)
		|	КОНЕЦ КАК Стоимость,
		|	УпаковочныйЛистДанныеЗаказов.ЦенаВключаетНДС КАК  ЦенаВключаетНДС,
        // -- Галфинд ВолковЕВ 2024/04/03
		...
		// ++ Галфинд ВолковЕВ 2024/04/03
		|	УпаковочныйЛистДанныеЗаказов.ЦенаВключаетНДС,
		|	УпаковочныйЛистДанныеЗаказов.СуммаНДСУпаковочногоЛиста,
		// ++ Галфинд ВолковЕВ 2024/04/03
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблица.Отгрузить КАК Отгрузить,
		|	ВременнаяТаблица.Клиент КАК Клиент,
		|	ВременнаяТаблица.АдресДоставки КАК АдресДоставки,
		|	ВременнаяТаблица.Склад КАК Склад,
		|	ВременнаяТаблица.ДокументПоступления КАК ДокументПоступления,
		|	ВременнаяТаблица.ДокументПоступленияНомер КАК ДокументПоступленияНомер,
		|	ВременнаяТаблица.Заказ КАК Заказ,
		|	ВременнаяТаблица.ОтгрузкаПеремещением КАК ОтгрузкаПеремещением,
		|	ВременнаяТаблица.УпаковочныйЛистСсылка КАК УпаковочныйЛистСсылка,
		|	ВременнаяТаблица.УпаковочныйЛистНомер КАК УпаковочныйЛистНомер,
		|	ВременнаяТаблица.СостояниеКороба КАК СостояниеКороба,
		|	ВременнаяТаблица.КоличествоОстаток КАК КоличествоОстаток,
		|	ВременнаяТаблица.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	ВременнаяТаблица.КоличествоДнейХранения КАК КоличествоДнейХранения,
		|	ВременнаяТаблица.СтатусКМ КАК СтатусКМ,
		|	ВременнаяТаблица.БлокировкиОтгрузок КАК БлокировкиОтгрузок,
		|	ВременнаяТаблица.Организация КАК Организация,
		|	ВременнаяТаблица.Артикул КАК Артикул,
		|	ВременнаяТаблица.Договор КАК Договор,
		|	ВременнаяТаблица.НомерАдреса КАК НомерАдреса,
		|	ВременнаяТаблица.ДействующийЛимит КАК ДействующийЛимит,
		|	ВременнаяТаблица.Блокировка КАК Блокировка,
		|	ВременнаяТаблица.УИСтрокиДереваЗначений КАК УИСтрокиДереваЗначений,
		|	ВременнаяТаблица.ТоварыВКоробках КАК ТоварыВКоробках,
		|	ВременнаяТаблица.Проверка КАК Проверка,
		|	ВЫБОР
		|		КОГДА НЕ ВременнаяТаблица.СтатусКМ ЕСТЬ NULL
		|			ТОГДА ВременнаяТаблица.СтавкаНДС
		|		ИНАЧЕ СправочникСтавкиНДС.Ставка
		|	КОНЕЦ КАК СтавкаНДС,
	    |	ВременнаяТаблица.КоличествоНоменклатуры КАК КоличествоНоменклатуры,
		|	ВЫБОР
		|		КОГДА НЕ ВременнаяТаблица.СтатусКМ ЕСТЬ NULL
		|			ТОГДА ВременнаяТаблица.Стоимость
		|			ИНАЧЕ 
		
		|			ВЫБОР
		|				КОГДА ВременнаяТаблица.ЦенаВключаетНДС = Истина
		|					ТОГДА 
		|						ЗаказКлиентагф_ТоварыВКоробах.ЦенаКороба
		|						* (1 - ЗаказКлиентагф_ТоварыВКоробах.Скидка / 100)
		|			ИНАЧЕ
		|				(ЗаказКлиентагф_ТоварыВКоробах.ЦенаКороба
		|				)
		|				* (1 + СправочникСтавкиНДС.Ставка / 100)
		|				* (1 - ЗаказКлиентагф_ТоварыВКоробах.Скидка / 100)
		|			КОНЕЦ
		|	КОНЕЦ КАК Стоимость,
		|	НЕ ДоговорыКонтрагентовДополнительныеРеквизиты.Значение ЕСТЬ NULL КАК ПредоплатныйДоговор,
		...
		|	ВременнаяТаблица.УпаковочныйЛистКомплектация КАК УпаковочныйЛистКомплектация,
		|	ВременнаяТаблица.ОсобенностьУчета КАК ОсобенностьУчета,
	    |	ВЫБОР
		|		КОГДА НЕ ВременнаяТаблица.СтатусКМ ЕСТЬ NULL
		|			ТОГДА ВременнаяТаблица.СтавкаНДССсылка
		|		ИНАЧЕ ЗаказКлиентагф_ТоварыВКоробах.СтавкаНДС
		|	КОНЕЦ КАК СтавкаНДССсылка
		|ИЗ
		|	ВременнаяТаблица КАК ВременнаяТаблица
		|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.гф_ТоварыВКоробах КАК ЗаказКлиентагф_ТоварыВКоробах
		|	ПО (ВременнаяТаблица.Заказ = ЗаказКлиентагф_ТоварыВКоробах.Ссылка)
		|		И (ВременнаяТаблица.Артикул = ЗаказКлиентагф_ТоварыВКоробах.ВариантКомплектации)
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтавкиНДС КАК СправочникСтавкиНДС
		|	ПО (СправочникСтавкиНДС.Ссылка = ЗаказКлиентагф_ТоварыВКоробах.СтавкаНДС)
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов.ДополнительныеРеквизиты КАК ДоговорыКонтрагентовДополнительныеРеквизиты
		|	ПО ВременнаяТаблица.Договор = ДоговорыКонтрагентовДополнительныеРеквизиты.Ссылка
		|		И (ДоговорыКонтрагентовДополнительныеРеквизиты.Свойство = &Предоплата)
		|ГДЕ
		|	&ПредоплатныйДоговор
		|СГРУППИРОВАТЬ ПО
		|	ВременнаяТаблица.Отгрузить,
		|	ВременнаяТаблица.Клиент,
		|	ВременнаяТаблица.АдресДоставки,
		|	ВременнаяТаблица.Склад,
		|	ВременнаяТаблица.ДокументПоступления,
		|	ВременнаяТаблица.ДокументПоступленияНомер,
		|	ВременнаяТаблица.Заказ,
		|	ВременнаяТаблица.ОтгрузкаПеремещением,
		|	ВременнаяТаблица.УпаковочныйЛистСсылка,
		|	ВременнаяТаблица.УпаковочныйЛистНомер,
		|	ВременнаяТаблица.СостояниеКороба,
		|	ВременнаяТаблица.КоличествоОстаток,
		|	ВременнаяТаблица.ЕдиницаИзмерения,
		|	ВременнаяТаблица.КоличествоДнейХранения,
		|	ВременнаяТаблица.СтатусКМ,
		|	ВременнаяТаблица.БлокировкиОтгрузок,
		|	ВременнаяТаблица.Организация,
		|	ВременнаяТаблица.Артикул,
		|	ВременнаяТаблица.Договор,
		|	ВременнаяТаблица.НомерАдреса,
		|	ВременнаяТаблица.ДействующийЛимит,
		|	ВременнаяТаблица.Блокировка,
		|	ВременнаяТаблица.УИСтрокиДереваЗначений,
		|	ВременнаяТаблица.ТоварыВКоробках,
		|	ВременнаяТаблица.Проверка,
		|	ВЫБОР
		|	КОГДА НЕ ВременнаяТаблица.СтатусКМ ЕСТЬ NULL
		|		ТОГДА ВременнаяТаблица.СтавкаНДС
		|	ИНАЧЕ СправочникСтавкиНДС.Ставка
		|	КОНЕЦ,
		|	ВременнаяТаблица.КоличествоНоменклатуры,
		|	ВЫБОР
		|	КОГДА НЕ ВременнаяТаблица.СтатусКМ ЕСТЬ NULL
		|		ТОГДА ВременнаяТаблица.Стоимость
		|	ИНАЧЕ ВЫБОР
		|			КОГДА ВременнаяТаблица.ЦенаВключаетНДС = ИСТИНА
		|				ТОГДА ЗаказКлиентагф_ТоварыВКоробах.ЦенаКороба * (1 - ЗаказКлиентагф_ТоварыВКоробах.Скидка / 100)
		|			ИНАЧЕ ЗаказКлиентагф_ТоварыВКоробах.ЦенаКороба * (1 + СправочникСтавкиНДС.Ставка / 100) * (1 - ЗаказКлиентагф_ТоварыВКоробах.Скидка / 100)
		|		КОНЕЦ
		|	КОНЕЦ,
		|	НЕ ДоговорыКонтрагентовДополнительныеРеквизиты.Значение ЕСТЬ NULL,
		|	ВременнаяТаблица.Скидка,
		|	ВременнаяТаблица.Цена,
		|	ВременнаяТаблица.УпаковочныйЛистКомплектация,
		|	ВременнаяТаблица.ОсобенностьУчета,
		|	ВЫБОР
		|		КОГДА НЕ ВременнаяТаблица.СтатусКМ ЕСТЬ NULL
		|			ТОГДА ВременнаяТаблица.СтавкаНДССсылка
		|		ИНАЧЕ ЗаказКлиентагф_ТоварыВКоробах.СтавкаНДС
...
Процедура РассчитатьЛимиты2(ТЗДляДерева)
	...
	// Волк3
	// |	РеализацияТоваровУслуг.ЗаказКлиента В(&ЗаказКлиента)
	|	РеализацияТоваровУслуг.Партнер в (&Партнер)
	// Волк3
	|	И РеализацияТоваровУслуг.Проведен = ЛОЖЬ
	|	И РеализацияТоваровУслуг.ПометкаУдаления = ЛОЖЬ
	// ++ Галфинд ВолковЕВ 2024/05/07
	...
	// Волк3
	Для Каждого Строка_ЛимитыТЗ Из ТЗЛимит Цикл
		Для Каждого Строка_2 Из ВыборкаДетальныеЗаписи Цикл
			
			Если Строка_ЛимитыТЗ.Партнер = Строка_2.Партнер
				И Строка_ЛимитыТЗ.Организация = Строка_2.Организация Тогда
				
				
				//Строка_Заказ.Сумма = Строка_Заказ.Сумма - Строка_2.СуммаДокумента;
				//Строка_ЛимитыТЗ.ДолгКонтрагентаПередОрганизацией = Строка_ЛимитыТЗ.ДолгКонтрагентаПередОрганизацией + Строка_2.СуммаДокумента;
				

				
				//Строка_ЛимитыТЗ.АвансОтКонтрагента = Строка_ЛимитыТЗ.АвансОтКонтрагента + Строка_Заказ.АвансОтКонтрагента;
				Строка_ЛимитыТЗ.ДолгКонтрагентаПередОрганизацией = Строка_ЛимитыТЗ.ДолгКонтрагентаПередОрганизацией 
					+ Строка_2.СуммаДокумента;
	...
	Для Каждого Строка Из ДеревоЗначений.Строки Цикл
		Для Каждого Строка_Н Из Строка.Строки Цикл
			//+++ БФ Бобнев К.С. 11.04.24
			//Для Каждого Строка_Т Из Строка_Н.Строки Цикл
			Для Каждого Строка_ТT Из Строка_Н.Строки Цикл
			Для Каждого Строка_Т Из Строка_ТT.Строки Цикл
			...
			КонецЦикла;
		КонецЦикла;
		КонецЦикла; //+++ БФ Бобнев К.С. 11.04.24
	КонецЦикла;
...
// Процедура устарела, необходимо использовать альтернативную Функцию "ПолучитьМаксНомерСтрокиПлюсОдин". 
Процедура ПолучитьМаксКодСтроки(Таблица0, Таблица1, НоваяСтрока)
...
// Униварсальное получение максимального значения номера по наименованию колонки.
...
Процедура ВыставитьСчета(Команда)
	...
	ПараметрыФормы.Вставить("НеУчитыватьВыстСчета", НеУчитыватьВыстСчета); // Галфинд_ДомнышеваКР_08_04_2024
...
//+++ БФ Бобнев К.С. 05.04.24
	...
	бф_УстановитьОтметки(Результат);
...
&НаСервере
Процедура бф_УстановитьОтметки(ТабДок)
//+++ БФ Бобнев К.С. 05.04.24
	Таблица = Новый ТаблицаЗначений;
	Таблица.Колонки.Добавить("Артикул", Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(200)));
	Таблица.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(10,0)));  
	Таблица.Колонки.Добавить("НомерАдреса", Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(200)));
	Таблица.Колонки.Добавить("НомерЗаказа", Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(200)));
	
	ЗаполненНомерАдреса = Ложь;
	ЗаполненНомерЗаказа = Ложь;
	Отказ = Ложь;
	
	Для ъ = 2 По ТабДок.ВысотаТаблицы Цикл
		Артикул = ТабДок.Область(ъ, 1, ъ, 1).Текст;
		Количество = Число(ТабДок.Область(ъ, 2, ъ, 2).Текст);
		НомерАдреса = ТабДок.Область(ъ, 3, ъ, 3).Текст;
		НомерЗаказа = ТабДок.Область(ъ, 4, ъ, 4).Текст;
		
		Если ъ = 2 Тогда
			ЗаполненНомерАдреса = ЗначениеЗаполнено(НомерАдреса);
			ЗаполненНомерЗаказа = ЗначениеЗаполнено(НомерЗаказа);
		Иначе	
			Если ЗаполненНомерАдреса <> ЗначениеЗаполнено(НомерАдреса) Тогда
				ОбщегоНазначения.СообщитьПользователю("В строке № " + ъ + ?(ЗаполненНомерАдреса, " не", "") + " заполнен номер адреса");
				Отказ = Истина;
			КонецЕсли;
			Если ЗаполненНомерЗаказа <> ЗначениеЗаполнено(НомерЗаказа) Тогда
				ОбщегоНазначения.СообщитьПользователю("В строке № " + ъ + ?(ЗаполненНомерЗаказа, " не", "") + " заполнен номер заказа");
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(Артикул) Тогда 
			ОбщегоНазначения.СообщитьПользователю("В строке № " + ъ + " не заполнен артикул");
			Отказ = Истина;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(Количество) Тогда
			ОбщегоНазначения.СообщитьПользователю("В строке № " + ъ + " не заполнено количество");
			Отказ = Истина;
		КонецЕсли;         
		...
		Если НЕ Отказ Тогда
			...
			НоваяСтрока.Количество = Количество;
			НоваяСтрока.НомерАдреса = НомерАдреса;
	...
	ПараметрыОтбора.Вставить("Артикул");
	Если ЗаполненНомерАдреса Тогда
		ПараметрыОтбора.Вставить("НомерАдреса");
	КонецЕсли;
	Если ЗаполненНомерЗаказа Тогда
		ПараметрыОтбора.Вставить("НомерЗаказа");
	КонецЕсли;
	
	
	ДеревоЗначений = РеквизитФормыВЗначение("ДеревоДокументов");
	Для каждого СтрокаКонтрагент Из ДеревоЗначений.Строки Цикл
		Для каждого СтрокаАдрес Из СтрокаКонтрагент.Строки Цикл
			Для каждого СтрокаЗаказ Из СтрокаАдрес.Строки Цикл
				Для каждого СтрокаНоменклатура Из СтрокаЗаказ.Строки Цикл 
					СтрокаНоменклатура.КоличествоОтгрузить = 0;
					ЗаполнитьЗначенияСвойств(ПараметрыОтбора, СтрокаНоменклатура);
					НайденныеСтроки = Таблица.НайтиСтроки(ПараметрыОтбора);
					Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
						КоличествоОтгрузить = 0;
						Если (СтрокаНоменклатура.КоличествоДоступно - СтрокаНоменклатура.КоличествоОтгрузить) >= НайденнаяСтрока.Количество Тогда
							КоличествоОтгрузить = НайденнаяСтрока.Количество;
						Иначе	
							КоличествоОтгрузить = (СтрокаНоменклатура.КоличествоДоступно - СтрокаНоменклатура.КоличествоОтгрузить);
						КонецЕсли;
						СтрокаНоменклатура.КоличествоОтгрузить = СтрокаНоменклатура.КоличествоОтгрузить + КоличествоОтгрузить;
						НайденнаяСтрока.Количество = НайденнаяСтрока.Количество - КоличествоОтгрузить;
					КонецЦикла;
				    Если СтрокаНоменклатура.КоличествоОтгрузить > 0 Тогда
						СтрокаНоменклатура.Отгрузить = Истина;
						СтрокаЗаказ.Отгрузить 		 = Истина;
						СтрокаАдрес.Отгрузить		 = Истина;
						СтрокаКонтрагент.Отгрузить	 = Истина;
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
		КонецЦикла;
	...
	Для каждого СтрокаТЗ Из Таблица Цикл
		Если СтрокаТЗ.Количество > 0 Тогда
			ОбщегоНазначения.СообщитьПользователю("Артикул " + СтрокаТЗ.Артикул + " не распределено для отгрузки " + СтрокаТЗ.Количество + " шт");
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура РассчитатьПоСтрокеДерева(Строка_Т)
	Если Строка_Т.Отгрузить
		И (ЗначениеЗаполнено(Строка_Т.УпаковочныйЛистСсылка)
		Или ЗначениеЗаполнено(Строка_Т.Номенклатура)) Тогда
