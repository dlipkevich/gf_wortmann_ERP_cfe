...
Процедура ЗаполнитьПоОстаткам(СхемаКомпоновки, ОтчетОбъект)
	...
	// формирование списка упаковочных листовне прошедших проверку
	Для каждого Строка Из Дерево.Строки Цикл
		Для каждого Строка_Н Из Строка.Строки Цикл
			Для каждого Строка_Т Из Строка_Н.Строки Цикл
				...
				|	гф_АдресаДоставки.Адрес КАК Адрес,
				|	гф_АдресаДоставки.Представление КАК Представление
				|ИЗ
				|	Справочник.гф_АдресаДоставки КАК гф_АдресаДоставки
				|ГДЕ
				|	гф_АдресаДоставки.Адрес = &Адрес
				|	И гф_АдресаДоставки.Владелец = &Владелец";
				
				Запрос.УстановитьПараметр("Владелец", Строка_Т.Заказ.Контрагент);
				Запрос.УстановитьПараметр("Адрес", Строка_Т.АдресДоставки);
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					Строка_Т.АдресДоставкиНомер = ВыборкаДетальныеЗаписи.НомерАдреса;
					Адрес = ВыборкаДетальныеЗаписи.НомерАдреса
				КонецЦикла;
				
				Строка_Т.АдресДоставкиНомер = Адрес;
				
				Строка_Т.ДокументПоступленияНомер = Строка_Т.ДокументПоступления.Номер; 
				ПоступленияНомер = Строка_Т.ДокументПоступления.Номер;
				
			КонецЦикла;
			Строка_Н.АдресДоставкиНомер = Адрес;
			Строка_Н.ДокументПоступленияНомер = ПоступленияНомер;
		КонецЦикла;
		Строка.АдресДоставкиНомер = ВыборкаДетальныеЗаписи.НомерАдреса;
	КонецЦикла;
...
Процедура РассчитатьЛимиты()
	...
	Запрос.Текст = 
		...
		|		ПО ВременнаяТаблица1.Ссылка = РасчетыСКлиентамиОстатки.АналитикаУчетаПоПартнерам";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Для Каждого Строка_ЛимитыТЗ Из ТЗЛимит Цикл
			...
			Если Строка_ЛимитыТЗ.Партнер = ВыборкаДетальныеЗаписи.АналитикаУчетаПоПартнерам.Партнер
				И Строка_ЛимитыТЗ.Организация = ВыборкаДетальныеЗаписи.Организация Тогда
				
				Строка_ЛимитыТЗ.СуммаВзаиморасчетов = Строка_ЛимитыТЗ.СуммаВзаиморасчетов + ВыборкаДетальныеЗаписи.КОплатеОстаток;
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Для Каждого Строка_Заказ Из ТЗКлиентЗаказ Цикл
				...
				Если Строка_Заказ.Заказ = ВыборкаДетальныеЗаписи.ЗаказКлиента Тогда
					//И Строка_Заказ.Организация = ВыборкаДетальныеЗаписи.Организация 
	...
	//ТЗЛимитыКонтрагентов = ТЗЛимит.Скопировать();
	//ЗначениеВРеквизитФормы(ТЗЛимитыКонтрагентов, "ЛимитыКонтрагентов");
	...
	//БлокировокиКонтрагентовТЗ = РезультатЗапроса.Выгрузить();
	//ЗначениеВРеквизитФормы(БлокировокиКонтрагентовТЗ, "БлокировокиКонтрагентов");
			...
			Для каждого Строка_Т Из Строка_Н.Строки Цикл
				...
				Для Каждого Строка_Блокировки Из БлокировокиКонтрагентовТЗ Цикл
					...
					ИначеЕсли Строка_Т.Клиент = Строка_Блокировки.Партнер 
						...
						//И Не Строка_Блокировки.Организация = Справочники.Организации.ПустаяСсылка()
...
Процедура ОтгрузитьНаСервере()
	...
	Для Каждого Строка Из Таб Цикл
		...
		СписокКодовСтроки.Колонки.Добавить("УпаковочныйЛист");
		СписокКодовСтроки.Колонки.Добавить("КодСтроки");
		СписокКодовСтроки.Колонки.Добавить("Номенклатура");
		СписокКодовСтроки.Колонки.Добавить("Характеристика");
		СписокКодовСтроки.Колонки.Добавить("Количество");
			...
			Иначе
				...
				Для Каждого СтрокаКомплектацияРазбиение Из ТабКомплектация Цикл
					Если СтрокаКомплектацияРазбиение.Номенклатура = СтрокаТовары.Номенклатура
						...
						ВвестиКоличествоДляРазбиения(ПараметрыОбработки, ТЗ, СтрокаТовары, ОбъектЗаказ.Товары, Отказ, СписокКодовСтроки, Строка.УпаковочныйЛист);
		...
		Если Не ОтказЗапись И ТабКомплектацияЗаказ.Количество() > 0 Тогда
			...
			// Запись нового максимального значения кода строки
			ТЗАльфа = ОбъектЗаказ.Товары.Выгрузить();
			
			Альфа = Новый СравнениеЗначений;
			ТЗАльфа.Сортировать("КодСтроки Убыв", Альфа);
			ОбъектЗаказ.МаксимальныйКодСтроки = ТЗАльфа[0].КодСтроки;
			...
			Попытка
				...
				//Стр.СписокКодовСтроки = СписокКодовСтроки;
				
				УпаковочныйЛистНоменклатураКодовСтроки = Строка.УпаковочныйЛист.ПолучитьОбъект();
				
				
				Для Каждого СтрокаУпаковочныйЛистНоменклатура Из УпаковочныйЛистНоменклатураКодовСтроки.Товары Цикл
					
					Для Каждого СтрокаСписокКодовСтроки Из СписокКодовСтроки Цикл
				...
				УпаковочныйЛистНоменклатураКодовСтроки.Записать();
...
Процедура ДобавитьСтрокуРазбиением(Количество, ПараметрыОбработки, ТЗ, ТекущаяСтрока, Товары, Отказ, СписокКодовСтроки, УпаковочныйЛист)
	...
	Если ТЗ.Получить(Товары.Индекс(ТекущаяСтрока))[ПараметрыРазбиенияСтроки.ИмяПоляКоличество] > Количество Тогда
		...
		ТЗАльфа = ТЗ.Скопировать();
		
		Альфа = Новый СравнениеЗначений;
		ТЗАльфа.Сортировать("КодСтроки Убыв", Альфа);
		МаксимальныйКодСтроки = ТЗАльфа[0].КодСтроки;
	...
	ИначеЕсли ТЗ.Получить(Товары.Индекс(ТекущаяСтрока))[ПараметрыРазбиенияСтроки.ИмяПоляКоличество] = Количество Тогда
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрокаСписокКодовСтроки, СтрокаТЗ);
		НоваяСтрокаСписокКодовСтроки.УпаковочныйЛист = УпаковочныйЛист;
...
Процедура ВвестиКоличествоДляРазбиения(ПараметрыОбработки, ТЗ, СтрокаТовары, Товары, Отказ, СписокКодовСтроки, УпаковочныйЛист)
	
	Количество = ПараметрыОбработки.Количество;
	
	ВвестиКоличествоДляРазбиенияПослеВвода(Количество, ПараметрыОбработки, ТЗ, СтрокаТовары, Товары, Отказ, СписокКодовСтроки, УпаковочныйЛист);
	
КонецПроцедуры

Процедура ВвестиКоличествоДляРазбиенияПослеВвода(Количество, ПараметрыОбработки, ТЗ, СтрокаТовары,  Товары, Отказ, СписокКодовСтроки, УпаковочныйЛист)
	
	ДобавитьСтрокуРазбиением(Количество, ПараметрыОбработки, ТЗ, СтрокаТовары, Товары, Отказ, СписокКодовСтроки, УпаковочныйЛист);
