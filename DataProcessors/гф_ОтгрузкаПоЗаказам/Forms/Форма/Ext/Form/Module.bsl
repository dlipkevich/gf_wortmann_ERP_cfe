...
Процедура ДеревоДокументовПриИзменении(Элемент)
	...
	Если Не Объект.ТоварыВКоробках Тогда
		...
		Если Строка.КоличествоДоступно >= Строка.КоличествоОтгрузить Тогда
			Строка.Стоимость = Строка.КоличествоОтгрузить * Строка.Цена;
		Иначе 
			
			Строка.КоличествоОтгрузить = Строка.КоличествоДоступно;
	...
	УстановитьФлажкиВыше(Строка, Строка.Отгрузить);
...
Процедура ПослеВыбораДействия(СтруктураПереданныхДанных, СтруктураПараметров) Экспорт
	...
	Если Не СтруктураПереданныхДанных = Неопределено Тогда
		...
		ИначеЕсли Действие = "ЗаписатьСкидкиПродолжить" Тогда
			...
			ОтгрузитьПродолжить(СтруктураПереданныхДанных);
...
&НаКлиенте
Процедура ОтгрузитьУпаковочныеЛистыСПересортом()
	
	    СтруктураПараметров = Неопределено;
		//Оповещение = Новый ОписаниеОповещения("ПослеВыбораДействияУстановитьСкидки", ЭтотОбъект, СтруктураПараметров);
...
Процедура ИсключитьКоробСРасхождениями()
	...
	Для Каждого Строка Из ДеревоЗначений.Строки Цикл
		Для Каждого Строка_Н Из Строка.Строки Цикл
			Для Каждого Строка_Т Из Строка_Н.Строки Цикл
				...
				Если Строка_Т.Отгрузить И ЗначениеЗаполнено(Строка_Т.УпаковочныйЛистСсылка) И Строка_Т.СостояниеКороба = "Пересорт" Тогда
...
//&НаКлиенте
//Процедура ПослеВыбораДействияУстановитьСкидки(СтруктураПереданныхДанных, СтруктураПараметров) Экспорт	
	
	//Если Не СтруктураПереданныхДанных = Неопределено Тогда
	//	
	//	Действие = СтруктураПереданныхДанных.Действие;
	//	
	//	Если Действие = "ЗаписатьПродолжить" Тогда
	//		
	//		ОтгрузитьПродолжить();
	//		
	//	ИначеЕсли Действие = "Отменить" Тогда
...
&НаКлиенте
Процедура ОтгрузитьПродолжить(СтруктураПереданныхДанных)
...
Процедура ВыполнитьДействиеПоОтгрузкеСУчетомСклада(Отказ, СтруктураПереданныхДанных)
	...
	Для Каждого Строка Из ДеревоЗначений.Строки Цикл
		Для Каждого Строка_Н Из Строка.Строки Цикл
			Для Каждого Строка_Т Из Строка_Н.Строки Цикл
				...
				Если Объект.ИсключитьКоробСРасхождениями И Строка_Т.Отгрузить И ЗначениеЗаполнено(Строка_Т.УпаковочныйЛистСсылка) И Строка_Т.СостояниеКороба = "Пересорт" Тогда
	...
	Для Каждого СтрокаЗаказы Из ТЗ_Заказы Цикл
		...
		Для Каждого СтрокаУпаковочныйЛист Из СтрокаЗаказы.УпаковочныеЛисты Цикл
			...
			//// Сортировка
			//
			//Т_2 = Новый ТаблицаЗначений;
			//Т_2 = ТЗ_Товары.Скопировать();
			//Т_2.Сортировать("ПроцентРучнойСкидки Убыв, Номенклатура Убыв, Характеристика Возр");
			//
			//Т_1 = Новый ТаблицаЗначений;
			//
			//Т_1 = Т_2.СкопироватьКолонки("Номенклатура, ПроцентРучнойСкидки, ВариантОбеспечения");
			//
			////Структура1 = Новый Структура("ВариантОбеспечения", "Перечисления.ВариантыОбеспечения.Отгрузить");
			//
			//МассивНайденныхСтрок = Т_1.НайтиСтроки(Новый Структура("ВариантОбеспечения", "Перечисления.ВариантыОбеспечения.Отгрузить"));
			//
			//Для Каждого ЭлементМассива Из МассивНайденныхСтрок Цикл
			//	
			//	НайденнаяСтрока = Т_1.Найти(ЭлементМассива);
			//	Т_1.Удалить(НайденнаяСтрока);
			//	
			//КонецЦикла;	
			//	
			//Т_1.Свернуть("Номенклатура, ПроцентРучнойСкидки");
			//
			//Для Каждого СтрокаТовары Из Т_2 Цикл
			//	
			//	Если СтрокаТовары.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить Тогда
			//		Продолжить;
			//	Иначе
			...
			Для Каждого СтрокаТоварыВКоробах Из ТЗ_ТоварыВКоробах Цикл
				...
				ИначеЕсли СтрокаТоварыВКоробах.ВариантКомплектации = СтрокаУпаковочныйЛист.УпаковочныйЛист.гф_Комплектация Тогда
						...
						ПараметрыОбработки = Новый Структура;
						ПараметрыОбработки.Вставить("ТабличнаяЧасть",           	ТЗКВТ);
						ПараметрыОбработки.Вставить("ЭлементФормы",             	ЭтаФорма);
						ПараметрыОбработки.Вставить("ОповещениеПослеРазбиения", 	"");
						ПараметрыОбработки.Вставить("ПараметрыРазбиенияСтроки", 	ПараметрыРазбиенияСтроки);
						ПараметрыОбработки.Вставить("ДополнительныеПараметры", 		ДополнительныеПараметры);
						...
						Если ТекущаяСтрока[ПараметрыРазбиенияСтроки.ИмяПоляКоличество] > Количество Тогда
							...
							НоваяСтрока.КомментарийРТУ = "";
							НоваяСтрока.СкидкаДляРТУ = (1 - НоваяСтрока.Скидка/100);
							
							Для Каждого СтрокаСтруктураПереданныхДанных Из СтруктураПереданныхДанных.Результат Цикл
								НоваяСтрока.КомментарийРТУ = НоваяСтрока.КомментарийРТУ + " " +СтрокаСтруктураПереданныхДанных.КомментарийРТУ;
								НоваяСтрока.ДатаДляРТУ = СтрокаСтруктураПереданныхДанных.ДатаДляРТУ;
								//НоваяСтрока.СкидкаДляРТУ = НоваяСтрока.СкидкаДляРТУ + СтрокаСтруктураПереданныхДанных.СкидкаДляРТУ;
								НоваяСтрока.СкидкаДляРТУ = НоваяСтрока.СкидкаДляРТУ * (1 - СтрокаСтруктураПереданныхДанных.СкидкаДляРТУ/100);
							КонецЦикла;
						...
						ИначеЕсли ТекущаяСтрока[ПараметрыРазбиенияСтроки.ИмяПоляКоличество] = Количество Тогда
							...
							// Если строка не разбивается, то "НоваяСтрока" очищаем, иначе данные остаются в ней,
							// когда отгружается несколько упаковоных листов. "НоваяСтрока" проверяются в "РазбитьСтрокуЗавершение()"
							...
							ТекущаяСтрока.КомментарийРТУ = "";
							ТекущаяСтрока.СкидкаДляРТУ = (1 - ТекущаяСтрока.Скидка/100);
							
							Для Каждого СтрокаСтруктураПереданныхДанных Из СтруктураПереданныхДанных.Результат Цикл
								ТекущаяСтрока.КомментарийРТУ = ТекущаяСтрока.КомментарийРТУ + " " + СтрокаСтруктураПереданныхДанных.КомментарийРТУ;
								ТекущаяСтрока.ДатаДляРТУ = СтрокаСтруктураПереданныхДанных.ДатаДляРТУ;
								ТекущаяСтрока.СкидкаДляРТУ = ТекущаяСтрока.СкидкаДляРТУ * (1 - СтрокаСтруктураПереданныхДанных.СкидкаДляРТУ/100);
							КонецЦикла;
			...
			Если НеПолный Тогда
				ТЗтоЧтоЕсть = Новый ТаблицаЗначений;
				ТЗтоЧтоЕсть.Колонки.Добавить("Номенклатура");
				ТЗтоЧтоЕсть.Колонки.Добавить("Характеристика");
				ТЗтоЧтоЕсть.Колонки.Добавить("Количество");
			КонецЕсли;
			...
			Если Пересорт Тогда
				...
				НайденныйЭлемент = Справочники.ВариантыКомплектацииНоменклатуры.НайтиПоНаименованию(СтрокаУпаковочныйЛист.УпаковочныйЛист.гф_Комплектация.Наименование);
					...
					Если Не НайденныйЭлемент = Неопределено Тогда
						...
						ТЗ_Эталон = Новый ТаблицаЗначений;
						ТЗ_Эталон = НайденныйЭлемент.Товары.Выгрузить();
						ТЗ_Эталон.Свернуть("Номенклатура, Характеристика, КоличествоУпаковок");
				...
				РазницаТаблиц = РазницаТаблицЗначений(ТЗ_Эталон, ТЗ_УпаковочныйЛист);
				...
				Для Каждого СтрокаРазницаВНоменклатуре Из НайтиСтрокиРазницаТаблиц Цикл
					...
					НайтиСтрокиЭталон = ТЗ_Эталон.НайтиСтроки(Новый Структура("Номенклатура, Характеристика", СтрокаРазницаВНоменклатуре.Номенклатура, СтрокаРазницаВНоменклатуре.Характеристика));
				...
				Для Каждого СтрокаРазница Из ТЗСНоменклатуройИКоличествомДляКорректировки Цикл
					...
					Найти_Строки_123 = ТЗ_Товары.НайтиСтроки(Новый Структура("Номенклатура, Характеристика, ПроцентРучнойСкидки, ВариантОбеспечения",
						СтрокаРазница.Номенклатура, СтрокаРазница.Характеристика, СкидкаВКоробах, Перечисления.ВариантыОбеспечения.КОбеспечению));
						
						Если Не Найти_Строки_123 = Неопределено И Найти_Строки_123.Количество() > 0 Тогда
							
							Если СтрокаРазница.ДобавитьКоличество > 0 Тогда
								
								
								СтрокаТовары = Найти_Строки_123[0];
								...
								ДобавитьСтрокуРазбиением3(ПараметрыОбработки, ТЗ, СтрокаТовары, ТЗ_Товары, ОтказЗаписьУпаковочногоЛиста,
						...
						ИначеЕсли СтрокаРазница.УбратьКоличество > 0 Тогда
							...
							СтрокаТовары = Найти_Строки_123[0];
								...
								ДобавитьСтрокуРазбиением2(ПараметрыОбработки, ТЗ, СтрокаТовары, ТЗ_Товары, ОтказЗаписьУпаковочногоЛиста,
								СписокКодовСтрокиПоУпаковочнымЛистам, СтрокаУпаковочныйЛист.УпаковочныйЛист, СтруктураПереданныхДанных, СкидкаВКоробах);
					...
					Для Каждого СтрокаКомплектацияРазбиение Из ТабКомплектация Цикл
						...
						Найти_еее = ТЗ_Товары.НайтиСтроки(Новый Структура("Номенклатура, Характеристика, ПроцентРучнойСкидки, ВариантОбеспечения",
						СтрокаКомплектацияРазбиение.Номенклатура, СтрокаКомплектацияРазбиение.Характеристика,СкидкаВКоробах, Перечисления.ВариантыОбеспечения.КОбеспечению));
						
						
						Если НеПолный Тогда
							СтрокаТЗтоЧтоЕсть = ТЗтоЧтоЕсть.Добавить();
							СтрокаТЗтоЧтоЕсть.Номенклатура = СтрокаКомплектацияРазбиение.Номенклатура;
							СтрокаТЗтоЧтоЕсть.Характеристика = СтрокаКомплектацияРазбиение.Характеристика;
							СтрокаТЗтоЧтоЕсть.Количество = СтрокаКомплектацияРазбиение.Количество;
						КонецЕсли;
						
						Если Найти_еее.Количество() > 1 Тогда
							
							ИспользоватьДляОтгрузкиНоменклатуруИзРазныхСтрок = Истина;
							
							СтрокаТовары = Найти_еее;
						Иначе
							СтрокаТовары = Найти_еее[0];
						КонецЕсли;
...
Процедура ДобавитьСтрокуРазбиением2(ПараметрыОбработки, ТЗ, ТекущаяСтрока, Товары, ПроверкаОтказ, СписокКодовСтроки, УпаковочныйЛист, СтруктураПереданныхДанных, СкидкаВКоробах)
	...
	Если ТекущаяСтрока[ПараметрыРазбиенияСтроки.ИмяПоляКоличество] > Количество Тогда
		...
		ТЗМакс = Товары.Скопировать(,"КодСтроки");
		ТоварыМакс = Товары.Скопировать(,"КодСтроки");
...
Процедура ДобавитьСтрокуРазбиением3(ПараметрыОбработки, ТЗ, ТекущаяСтрока, Товары, ПроверкаОтказ, СписокКодовСтроки, УпаковочныйЛист, СтруктураПереданныхДанных, СкидкаВКоробах)
	...
	Иначе
		...
		ИначеЕсли КоличествоОбщееНоменклатуры = Количество Тогда
			...
			Для Каждого ЭлементМассива Из ТекущаяСтрока Цикл
				...
				ЭлементМассива.гф_КомментарийРТУ = "";
				ЭлементМассива.гф_СкидкаДляРТУ = (1 - ЭлементМассива.ПроцентРучнойСкидки/100);
				
				Для Каждого СтрокаСтруктураПереданныхДанных Из СтруктураПереданныхДанных.Результат Цикл
					ЭлементМассива.гф_КомментарийРТУ = ЭлементМассива.гф_КомментарийРТУ + " " +СтрокаСтруктураПереданныхДанных.КомментарийРТУ;
					ЭлементМассива.гф_ДатаДляРТУ = СтрокаСтруктураПереданныхДанных.ДатаДляРТУ;
					ЭлементМассива.гф_СкидкаДляРТУ = ЭлементМассива.гф_СкидкаДляРТУ * (1 - СтрокаСтруктураПереданныхДанных.СкидкаДляРТУ/100);
				КонецЦикла;
				
				Если Объект.ТоварыВКоробках Тогда
					НоваяСтрокаСписокКодовСтроки = СписокКодовСтроки.Добавить();
					
					ЗаполнитьЗначенияСвойств(НоваяСтрокаСписокКодовСтроки, ЭлементМассива);
					НоваяСтрокаСписокКодовСтроки.УпаковочныйЛист = УпаковочныйЛист;
				КонецЕсли;
...
Процедура ЗаполнитьРезервыНаСервере(Заказ, УпаковочныйЛист)
	...
	НаличиеПроверкаЗаказ = НаличиеПроверка.НайтиСтроки(Новый Структура("ЗаказНаОтгрузку", гф_Заказ));
	...
	Если НайденнаяСтрока = Неопределено Тогда
		...
		Если Проверка Тогда
			...
			ОбъектНаличиеЗаказ = Объект.Наличие.НайтиСтроки(Новый Структура("ЗаказНаОтгрузку", гф_Заказ));
			
			Для Каждого СтрокаУпковочногоЛиста Из УпаковочныйЛист.Товары Цикл
				
				Для Каждого СтрокаНаличие Из ОбъектНаличиеЗаказ Цикл
					
					Если гф_Заказ = СтрокаНаличие.ЗаказНаОтгрузку
...
&НаСервере
Процедура ОформитьРасходныеОрдераНаСервере(УпаковочныйЛистДляОрдера, Отказ)
...
Функция ЗаполнитьДеревоЗначенийДляНеКоробногоСклада()
	...
	Для Каждого СтрокаОстатки Из ТаблицаОстатки Цикл
		...
		Если Не ТЗВыборкаНоменклатураСтрока = Неопределено Тогда
			...
			СтрокаОстатки.КоличествоОтгрузить 	= ТЗВыборкаНоменклатураСтрока.КоличествоУпаковок;
