...
Функция ОтгрузитьНаСервере(Отказ)
	...
	Если СписокКонтрагентов.Количество() > 1 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Необходимо отметить для отгрузки упаковочные листы только по одному клиенту.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	...
	Если СписокДоговоров.Количество() > 1 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Отмечены для отгрузки упаковочные листы по разным договорам! Необходимо отметить только по одному договору.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	...
	Если СписокЗаказовСезонность.Количество() > 1 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Отмечены заказы с разной сезонностью!
		|Необходимо отметить упаковочные листы по заказу с одинаковой сезонностью'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	...
	Если Лимиты Или Блокировки Тогда
		АктивныеЛимитыблокировки = Истина;
	КонецЕсли;
	
	Возврат АктивныеЛимитыблокировки;	
...
Процедура Отгрузить(Команда)
	...
	АктивныеЛимитыблокировки = ОтгрузитьНаСервере(Отказ);
	
	Если Не АктивныеЛимитыблокировки = Неопределено И АктивныеЛимитыблокировки Тогда
...
Функция ЗаполнитьДеревоЗначенийДляНеКоробногоСклада()
	...
	Запрос.Текст = 
		...
		|	ТаблицаОстатки2.Доступно КАК КоличествоНоменклатуры
		|ПОМЕСТИТЬ ВТ
		|ИЗ
