...
Процедура ЗаполнитьТаблицуОстатков(Дерево, Склад, СхемаКомпоновкиДанных)
	...
	//СхемаКомпоновкиДанных = Отчеты.гф_ОстаткиИДоступностьТоваров.ПолучитьМакет("СхемаКомпоновкиНоменклатуры");
	...
	Для каждого Строка Из Дерево.Строки Цикл
		Для каждого Строка_Н Из Строка.Строки Цикл
			...
			Для каждого Строка_Т Из Строка_Н.Строки Цикл
				...
				//НомерАдреса = ПолучитьНомерАдреса(Строка_Т.Заказ, Строка_Т.АдресДоставки);
				//Строка_Т.АдресДоставкиНомер = НомерАдреса;
			...
			КонецЦикла;
			//Строка_Н.АдресДоставкиНомер = НомерАдреса;
			Строка_Н.ДокументПоступленияНомер = ПоступленияНомер;
			
		КонецЦикла;
		//Строка.АдресДоставкиНомер = НомерАдреса;
	КонецЦикла;
	...
	Для каждого Строка Из ДеревоЗначений.Строки Цикл
		...
		Для каждого Строка_Н Из Строка.Строки Цикл
			...
			Если Не ИндексСтроки = Неопределено Тогда
				...
				Строка.Строки.Удалить(ИндексСтроки);
		...
		Если Не ИндексСтроки = Неопределено Тогда
			...
			ДеревоЗначений.Строки.Удалить(ИндексСтроки);
...
Процедура ОтгрузитьНаСервере(Отказ)
	...
	Для Каждого Строка Из Таб Цикл
		...
		Для Каждого СтрокаТовары Из ОбъектЗаказ.Товары Цикл
			...
			Иначе
				...
				Для Каждого СтрокаКомплектацияРазбиение Из ТабКомплектация Цикл
					Если СтрокаКомплектацияРазбиение.Номенклатура = СтрокаТовары.Номенклатура
						...
						ДобавитьСтрокуРазбиением(ПараметрыОбработки, ТЗ, СтрокаТовары, ОбъектЗаказ.Товары, ПроверкаОтказ, СписокКодовСтроки, Строка.УпаковочныйЛист);
			...
			Если ПроверкаОтказ Тогда
				Прервать;	
			КонецЕсли;
			
		КонецЦикла;
		
		Если Не ПроверкаОтказ Тогда
...
Процедура ПослеЗапоненияСпискаУпаковочныхЛистов(СтруктураПереденныхДанных, Строка) Экспорт
	...
	Если НЕ СтруктураПереденныхДанных = Неопределено Тогда
		...
		АктуализироватьДанныеДереваДокументов(СтруктураПереденныхДанных);
		
		//Оповещение = Новый ОписаниеОповещения("ПослеВыбораДействияВопросаОДатеЦенИСкидкеКлиента", ЭтотОбъект, Строка);
		//	Отбор = Новый Структура();
		//	Отбор.Вставить("Дата", Объект.ДатаПолученияЦен);
		//	Отбор.Вставить("Скидка", Объект.ПроцентСкидки);
		//	ПараметрыОткрытия = Новый Структура("Ключ", Отбор);
...
Процедура АктуализироватьДанныеДереваДокументов(СтруктураПереденныхДанных)
	
	ДеревоЗначений = РеквизитФормыВЗначение("ДеревоДокументов");
	
	ТЗ = Новый ТаблицаЗначений;
	ТЗ.Колонки.Добавить("УпаковочныйЛист");
	ТЗ.Колонки.Добавить("Проверка");
	...
	ТЗ.Свернуть("УпаковочныйЛист, Проверка");
			...
			Для каждого Строка_Т Из Строка_Н.Строки Цикл
				...
				Для Каждого ЭлементМассива Из СтруктураПереденныхДанных Цикл
					...
					Если ЗначениеЗаполнено(Строка_Т.УпаковочныйЛистСсылка)
						...
						НайденнаяСтрока.Проверка = Истина;
...
&НаКлиенте
Процедура СписокКоробов(Команда)
	...
	ОткрытьФорму("Обработка.гф_ОтгрузкаПоЗаказам.Форма.ФормаЗагрузкиКодовМаркировкиУпаковочныхЛистов",ПараметрыФормы,,,,,Оповещение,);
