...
Процедура ПослеВыбораДействия(СтруктураПереданныхДанных, СтруктураПараметров) Экспорт
	...
	Если Не СтруктураПереданныхДанных = Неопределено Тогда
		...
		Если Действие = "НеУчитыватьЗадолженность" Тогда
			
			ДеревоДокументовУстановитьФлажки(СтруктураПараметров, "НеУчитыватьЗадолженность");
...
&НаКлиенте
Процедура ДеревоДокументовУстановитьФлажки(СтруктураПараметров, ДействиеИсточник)
...
Процедура Отгрузить(Команда)
	...
	АктивныеЛимиты = ПолучитьЛимиты(Отказ);
	
	Активныеблокировки = ПолучитьАктивныеБлокировки(Отказ);
	
	Если Не Активныеблокировки = Неопределено И Активныеблокировки Тогда
...
Функция ПолучитьАктивныеБлокировки(Отказ)
...
Функция ПроверкаРеквиитовВыбранныхОбъектовДляОтгрузки(Отказ)
	...
	Если СписокДоговоров.Количество() > 1 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Отмечены для отгрузки объекты по разным договорам! Необходимо отметить только по одному договору.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ОтгрузитьНаСервере(Отказ)
	...
	Иначе
		...
		//Отказ = Истина;
		//ТекстСообщения = НСтр("ru = 'Разработка даного фунционала для не коробного склада не завершена!'");
		//ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		//Возврат;
