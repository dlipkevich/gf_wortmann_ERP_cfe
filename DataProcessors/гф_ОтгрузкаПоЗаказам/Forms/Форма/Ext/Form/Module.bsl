...
Процедура ДеревоДокументовПриИзменении(Элемент)
	...
	РассчитатьОбщуюСтоимостьОтмеченныхКОтгрузкеТоваров(Неопределено);
	РазвернутьДеревоДокументов();
...
Процедура ЗаполнитьПоОстаткам(ОтчетОбъект, СхемаКомпоновки, СхемаКомпоновкиНоменклатуры)
	...
	//ПроверкаДоступностиНоменклатуры(Дерево, ТЗ_СТоварами);
	...
	РассчитатьОбщуюСтоимостьОтмеченныхКОтгрузкеТоваров(Дерево);
...
Функция ЗаполнитьТаблицуОстатков(Дерево, Склад, СхемаКомпоновкиДанных)
	...
	//|	ТаблицаСДанными.Склад КАК Склад
	|ПОМЕСТИТЬ ВТ
	|ИЗ
...
//Процедура ПроверкаДоступностиНоменклатуры(Дерево, ТЗ_СТоварами)
//	
//	НаличиеПроверка2 = Объект.Наличие.Выгрузить();
//	
//	Для Каждого Строка Из Дерево.Строки Цикл
//		Для Каждого Строка_Н Из Строка.Строки Цикл
//			
//			МассивНоменклатурыПоЗаказу = НаличиеПроверка2.НайтиСтроки(Новый Структура("ЗаказНаОтгрузку", Строка_Н.Заказ));
//			
//			Для Каждого Строка_Т Из Строка_Н.Строки Цикл
//				
//				ЗаполнитьРезервыНаСервере(Строка_Т.Заказ, Строка_Т.УпаковочныйЛистСсылка, ТЗ_СТоварами, МассивНоменклатурыПоЗаказу);
//				
...
//				
//				//Строка_Т.УпаковочныйЛистНомер = Строка_Т.УпаковочныйЛистКод;
//				
//			КонецЦикла;
			...
			Для Каждого Строка_Н Из Строка.Строки Цикл
				...
				Для Каждого СтрокаТЗ Из ТЗ Цикл
					...
					Если НайденныйОбъект.Количество() > 0 Тогда
						//
						//НОУпаковочныйЛистСсылка = НайденныйОбъект[0].УпаковочныйЛистСсылка;
...
Процедура РассчитатьОбщуюСтоимостьОтмеченныхКОтгрузкеТоваров(ДеревоЗначений)
...
Процедура ПослеВыбораДействия(СтруктураПереданныхДанных, СтруктураПараметров) Экспорт
	...
	Если Не СтруктураПереданныхДанных = Неопределено Тогда
		...
		Если Действие = "НеУчитыватьБлокировки" Тогда
			...
			ОтгрузитьПродолжить();
			//ОтгрузитьУпаковочныеЛистыСПересортом();
		...
		ИначеЕсли Действие = "Отменить" Тогда
			...
			ТекстСообщения = НСтр("ru = 'Действие по отгрузке отменено! '");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
&НаКлиенте
Процедура ОтгрузитьПродолжитьСоСкидками(СтруктураПереданныхДанных)
	...
	Если Не АктивныеЛимитыКлиента = Неопределено Тогда
		...
		Если ПроверкаРолиОбходаБлокировок Тогда
			...
			// Передаем структуру скидки
			СтруктураПараметров = СтруктураПереданныхДанных;
	...
	Иначе
		...
		ОтгрузитьНаСервере(Ложь, СтруктураПереданныхДанных);
...
Процедура Отгрузить(Команда)
	...
	ПроверкаРеквизитовВыбранныхОбъектовДляОтгрузки(Отказ);
		...
		Для Каждого Строка_Н Из строка.Строки Цикл
			Для Каждого Строка_Т Из Строка_Н.Строки Цикл
				...
				Если Строка_Т.Отгрузить И Строка_Т.БлокировкиОтгрузок Тогда
					...
					//Если Не ПроверкаРолиОбходаБлокировок Тогда
...
Процедура ПроверкаРеквизитовВыбранныхОбъектовДляОтгрузки(Отказ)
...
Процедура ВыполнитьДействиеПоОтгрузке(Отказ, СтруктураПереданныхДанных)
    ...
    ТЗ_Заказы.Колонки.Добавить("СписокУпаковочныйЛистНоменклатура");
			...
			Для Каждого Строка_Т Из Строка_Н.Строки Цикл
				...
				Если Строка_Т.Отгрузить И (ЗначениеЗаполнено(Строка_Т.УпаковочныйЛистСсылка) Или (Не Объект.ТоварыВКоробках И ЗначениеЗаполнено(Строка_Т.Номенклатура))) Тогда
					...
					Если НайтиЗаказ = Неопределено Тогда
						...
						ТЗ_УпаковочныеЛисты.Колонки.Добавить("ЭлементУпаковочныйЛистНоменклатура");
						...
						ТЗ_УпаковочныеЛисты.Колонки.Добавить("Характеристика");

						СтрокаУпаковочныеЛистыНоменклатура = ТЗ_УпаковочныеЛисты.Добавить();
						...
						СтрокаЗаказы.СписокУпаковочныйЛистНоменклатура = ТЗ_УпаковочныеЛисты;
					Иначе
						
						СтрокаУпаковочныеЛистыНоменклатура = НайтиЗаказ.СписокУпаковочныйЛистНоменклатура.Добавить();
	...
	Для Каждого СтрокаЗаказы Из ТЗ_Заказы Цикл
		...
		Для Каждого СтрокаУпаковочныйЛистНоменклатура Из СтрокаЗаказы.СписокУпаковочныйЛистНоменклатура Цикл
						...
						Если Объект.ТоварыВКоробках Тогда
							Для Каждого СтрокаКомплектацияРазбиение Из ТабКомплектация Цикл
								Если СтрокаКомплектацияРазбиение.Номенклатура = СтрокаТовары.Номенклатура
									...
									СписокКодовСтрокиПоУпаковочнымЛистам, СтрокаУпаковочныйЛистНоменклатура.ЭлементУпаковочныйЛистНоменклатура, СтруктураПереданныхДанных);
			...
			//		ДополнительныеПараметры.Вставить("СтруктураПересчетаСуммы", СтруктураПересчетаСуммы);
			//		
			//		ОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьСтруктуруПересчетаСуммы(СтруктураПересчетаСуммы, СтрокаУпаковочныйЛистНоменклатура.ЭлементУпаковочныйЛистНоменклатура);
			//		Для Каждого СтрокаПересчета Из СтруктураПересчетаСуммы.Поля Цикл
			//			Если СтрокаПересчета.Значение = Неопределено Тогда
			//				
			//				СтруктураПересчетаСуммы.Поля[СтрокаПересчета.Ключ] = 0;
			...
			//					
			//					ДобавитьСтрокуРазбиением(ПараметрыОбработки, ТЗ, СтрокаТовары, ТЗ_Товары, ОтказЗаписьУпаковочногоЛиста,
			//					СписокКодовСтрокиПоУпаковочнымЛистам, СтрокаУпаковочныйЛистНоменклатура.ЭлементУпаковочныйЛистНоменклатура);
			//					
			//					Прервать;
			//					
			//				КонецЕсли;
			...
			//			
			//			ДобавитьСтрокуРазбиением(ПараметрыОбработки, ТЗ, СтрокаТовары, ТЗ_Товары, ОтказЗаписьУпаковочногоЛиста,
			//			Неопределено, СтрокаУпаковочныйЛистНоменклатура.ЭлементУпаковочныйЛистНоменклатура);
			//			
			//			//Прервать;
			//			
			//			//КонецЕсли;			
			...
			Если СтрокаЗаказы.Перепроведен Тогда
				...
				Попытка
					...
					Если Объект.ТоварыВКоробках Тогда
						...
						ОформитьРасходныеОрдераНаСервере(УпаковочныйЛистДляОрдера, Отказ);
					Иначе
						ОформитьРасходныеОрдераНаСервере(Неопределено, Отказ);
					КонецЕсли;
...
Процедура ВыполнитьДействиеПоОтгрузкеСУчетомСклада(Отказ, СтруктураПереданныхДанных)
	...
	// Таблица для записи каждого заказа с конкретным списком упаковочных листов по этому заказу,
	// Добавляем Таблицу "НовыеДанныеДляТаблицыПоЗаказу" с новыми строками и количеством для отгрузки получившиеся после учета
		...
		Для Каждого СтрокаУпаковочныйЛист Из СтрокаЗаказы.УпаковочныеЛисты Цикл
			...
			Для Каждого СтрокаТоварыВКоробах Из ТЗ_ТоварыВКоробах Цикл
				...
				Если СтрокаТоварыВКоробах.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить Тогда
					Продолжить;
				ИначеЕсли СтрокаТоварыВКоробах.ВариантКомплектации = СтрокаУпаковочныйЛист.УпаковочныйЛист.гф_Комплектация Тогда
						...
						// В цикле перебираются упаковочные листы последовательно, поэтому передаем количестово один для разбиения строки
						Количество = 1;
			...
			Пересорт = СтрокаУпаковочныйЛист.УпаковочныйЛист.гф_СостояниеКороба = Справочники.гф_СостянияКоробов.Пересорт;
			НеПолный = СтрокаУпаковочныйЛист.УпаковочныйЛист.гф_СостояниеКороба = Справочники.гф_СостянияКоробов.НеПолный;
			
			Если Пересорт Тогда
				
				//ТЗРезульт = ПолучитьНоменклатуруТабличнойЧастиЗаказа(ТЗ_Товары, СкидкаВКоробах, Перечисления.ВариантыОбеспечения.КОбеспечению);
				
				НайденныйЭлемент = Справочники.ВариантыКомплектацииНоменклатуры.НайтиПоНаименованию(СтрокаУпаковочныйЛист.УпаковочныйЛист.гф_Комплектация.Наименование);
				...
				Иначе
					...
					ТекстСообщения = СтрШаблон(НСтр("ru = 'Не возможно сравнить пересорт с эталоном комплектации, комплектация не найдена ""%1"" по ""%2"" для""%3""'"), СтрокаУпаковочныйЛист.УпаковочныйЛист.гф_Комплектация.Наименование, ОбъектЗаказ, СтрокаУпаковочныйЛист.УпаковочныйЛист);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
					Продолжить;				
				КонецЕсли;
				...
				ТЗ_УпаковочныйЛист = СтрокаУпаковочныйЛист.УпаковочныйЛист.Товары.Выгрузить();
				...
				//РазницаТаблиц = Получить(СтрокаУпаковочныйЛист);
					...
					Если Не НайтиСтроки_ВариантОбеспечения = Неопределено И НайтиСтроки_ВариантОбеспечения.Количество() > 0 Тогда
						...
						Если СтрокаРазница.ДобавитьКоличество > 0 Тогда
							...
							ДобавитьСтрокуРазбиениемДобавитьКоличество(ПараметрыОбработки, ТЗ, СтрокаТовары, ТЗ_Товары, ОтказЗаписьУпаковочногоЛиста,
								СписокКодовСтрокиПоУпаковочнымЛистам, СтрокаУпаковочныйЛист.УпаковочныйЛист, СтруктураПереданныхДанных, СкидкаВКоробах);
				...
				Для Каждого СтрокаКомплектацияРазбиение Из ТабКомплектация Цикл
					...
					ДобавитьСтрокуРазбиением4(ПараметрыОбработки, ТЗ, СтрокаТовары, ТЗ_Товары, ОтказЗаписьУпаковочногоЛиста,
						СписокКодовСтрокиПоУпаковочнымЛистам, СтрокаУпаковочныйЛист.УпаковочныйЛист, СтруктураПереданныхДанных);
			...
			ИначеЕсли НеПолный Тогда
				...
				Для Каждого СтрокаКомплектацияРазбиение Из ТабКомплектация Цикл
					...
					ДобавитьСтрокуРазбиением5(ПараметрыОбработки, ТЗ, СтрокаТовары, ТЗ_Товары, ОтказЗаписьУпаковочногоЛиста,
						СписокКодовСтрокиПоУпаковочнымЛистам, СтрокаУпаковочныйЛист.УпаковочныйЛист, СтруктураПереданныхДанных);
					...
					Иначе
						...
						Для Каждого СтрокаКомплектацияРазбиение Из ТабКомплектация Цикл
							...
							Если СтрокаКомплектацияРазбиение.Номенклатура = СтрокаТовары.Номенклатура
								...
								И СкидкаВКоробах = СтрокаТовары.ПроцентРучнойСкидки Тогда
			...
			Если Не ОтказЗаписьУпаковочногоЛиста Тогда
				...
				// Получить список из ростовки и сравнить с этой таблицей ТЗтоЧтоЕсть
				// отметить что найдено, что не найдено удалить из ТЗ_Товары
				//Если НеПолный Тогда
				//	НайденныйЭлемент = Справочники.ВариантыКомплектацииНоменклатуры.НайтиПоНаименованию(СтрокаУпаковочныйЛист.УпаковочныйЛист.гф_Комплектация.Наименование);
				//	
				//	Если Не НайденныйЭлемент = Неопределено Тогда
				//		
				//		ТЗ_123 = Новый ТаблицаЗначений;
				//		ТЗ_123 = НайденныйЭлемент.Товары.Выгрузить();
				//		ТЗ_123.Колонки.Добавить("Найдено");
				//		ТЗ_123.Колонки.Добавить("КоличествоРазличие");
				//		Для Каждого Стр Из ТЗ_123 Цикл
				//			Стр.Найдено = Ложь;
				//			Стр.КоличествоРазличие = 0;
				//		КонецЦикла;
				//		
				//		Для Каждого Стр Из ТЗтоЧтоЕсть Цикл
				//			
				//			Найти_123 = ТЗ_123.Найти(Стр.Характеристика, "Характеристика");
				//			Если Стр.Количество = Найти_123.КоличествоУпаковок Тогда
				//				Найти_123.Найдено = Истина;
				//				Найти_123.КоличествоРазличие = 0;
				//			Иначе
				//				Найти_123.Найдено = Истина;
				//				Найти_123.КоличествоРазличие = Найти_123.КоличествоУпаковок - Стр.Количество;
				//			КонецЕсли;
				//		КонецЦикла;
				//		
				//		СтрокиНайти_123 = ТЗ_123.НайтиСтроки(Новый Структура("Найдено", Ложь));
				//		
				//		Для Каждого Элем Из СтрокиНайти_123 Цикл
				//			
				//			Для Каждого Стр Из ТЗ_Товары Цикл
				//				
				//				Если Элем.Номенклатура = Стр.Номенклатура
				//					И Элем.Характеристика = Стр.Характеристика
				//					И СкидкаВКоробах = Стр.ПроцентРучнойСкидки Тогда
				//					
				//					ИндексСтроки = ТЗ_Товары.Индекс(Стр);
				//					ТЗ_Товары.Удалить(ИндексСтроки);
				//					
				//					Прервать;
				//				КонецЕсли;
				//				
		...
		Если СтрокаЗаказы.ВозможнаЗаписьЗаказа И СтрокаЗаказы.НовыеДанныеДляТаблицыПоЗаказу.Количество() > 0 Тогда
			...
			Если СтрокаЗаказы.Перепроведен Тогда
				...
				Попытка
					...
					// Проведение документов расходный ордер и упаковочный лист, изменение статуса агрегации (справочник кодов маркировки)
					СтруктураЗадания = ОформитьРасходныеОрдераНаСервере(УпаковочныйЛистДляОрдера, Отказ);
...
Процедура РазбитьСтрокуЗавершениеДобавленоПоПричине(НоваяСтрока, ДополнительныеПараметры, СтрокаТЗ)
	...
	Если НоваяСтрока <> Неопределено Тогда
		...
		ПересчитатьСуммы(ДополнительныеПараметры.СтруктураПересчетаСуммы);
		//ПересчитатьСуммы(СтруктураПересчетаСуммы);
...
Функция Получить(СтрокаУпаковочныйЛист)
...
Процедура ПолучитьМаксНомерСтроки(Таблица1, Таблица2, НоваяСтрока)
	
	ТЗМакс = Таблица1.Скопировать(,"КодСтроки");
	ТоварыМакс = Таблица2.Скопировать(,"КодСтроки");
...
Процедура ДобавитьСтрокуРазбиениемДляУпаковочногоЛиста(ПараметрыОбработки, ТабличнаяЧасть, ТекущаяСтрока, СтруктураПереданныхДанных)
	...
	ИначеЕсли ТекущаяСтрока[ПараметрыРазбиенияСтроки.ИмяПоляКоличество] = Количество Тогда
		...
		// Если строка не разбивается, то "НоваяСтрока" очищаем от данных,
		// Когда отгружается несколько упаковочных листов, то "НоваяСтрока" проверяются на Неопределено в "РазбитьСтрокуЗавершение()"
		...
		ПолучитьКомментарийИСкидкиДляРТУ(ТекущаяСтрока.КомментарийРТУ, ТекущаяСтрока.СкидкаДляРТУ, ТекущаяСтрока.ДатаДляРТУ, ТекущаяСтрока.Скидка, СтруктураПереданныхДанных);
...
Процедура ДобавитьСтрокуРазбиением(ПараметрыОбработки, ТЗ, ТекущаяСтрока, Товары, ПроверкаОтказ, СписокКодовСтроки, УпаковочныйЛист, СтруктураПереданныхДанных)
	...
	Если ТекущаяСтрока[ПараметрыРазбиенияСтроки.ИмяПоляКоличество] > Количество Тогда
		...
		//ТЗМакс = ТЗ.Скопировать(,"КодСтроки");
		//ТоварыМакс = Товары.Скопировать(,"КодСтроки");
		//
		//Если ТЗМакс.Количество() > 0 Тогда
		//	МаксКодСтроки = Новый СравнениеЗначений;
		//	ТЗМакс.Сортировать("КодСтроки Убыв", МаксКодСтроки);
		...
		ПолучитьМаксНомерСтроки(ТЗ, Товары, НоваяСтрока);
		
		//НоваяСтрока.гф_КомментарийРТУ = "";
		//НоваяСтрока.гф_СкидкаДляРТУ = 1 - НоваяСтрока.ПроцентРучнойСкидки/100;
		//
		//Для Каждого СтрокаСтруктураПереданныхДанных Из СтруктураПереданныхДанных.Результат Цикл
		//	
		//	Если Не ЗначениеЗаполнено(НоваяСтрока.гф_КомментарийРТУ) Тогда
		//		НоваяСтрока.гф_КомментарийРТУ = СтрокаСтруктураПереданныхДанных.КомментарийРТУ;
		//	Иначе
		//		НоваяСтрока.гф_КомментарийРТУ = НоваяСтрока.гф_КомментарийРТУ + " " + СтрокаСтруктураПереданныхДанных.КомментарийРТУ;
		//	КонецЕсли;
		//	НоваяСтрока.гф_ДатаДляРТУ = СтрокаСтруктураПереданныхДанных.ДатаДляРТУ;
		//	НоваяСтрока.гф_СкидкаДляРТУ = НоваяСтрока.гф_СкидкаДляРТУ * (1 - СтрокаСтруктураПереданныхДанных.СкидкаДляРТУ / 100);
		//КонецЦикла;
		//
		//НоваяСтрока.гф_СкидкаДляРТУ = (1 - НоваяСтрока.гф_СкидкаДляРТУ) * 100;
		//
		//Если ЗначениеЗаполнено(НоваяСтрока.ПроцентРучнойСкидки) И НоваяСтрока.ПроцентРучнойСкидки > 0 И ЗначениеЗаполнено(НоваяСтрока.гф_КомментарийРТУ) Тогда
		//	НоваяСтрока.гф_КомментарийРТУ = СтрШаблон("Дата получения цен %1, %2 %3, %4", НоваяСтрока.гф_ДатаДляРТУ, "%Скидки", НоваяСтрока.ПроцентРучнойСкидки, НоваяСтрока.гф_КомментарийРТУ);
		//ИначеЕсли ЗначениеЗаполнено(НоваяСтрока.ПроцентРучнойСкидки) И НоваяСтрока.ПроцентРучнойСкидки > 0 И Не ЗначениеЗаполнено(НоваяСтрока.гф_КомментарийРТУ) Тогда
		//	НоваяСтрока.гф_КомментарийРТУ = СтрШаблон("Дата получения цен %1, %2 %3", НоваяСтрока.гф_ДатаДляРТУ, "%Скидки", НоваяСтрока.ПроцентРучнойСкидки);
		//Иначе
		//	НоваяСтрока.гф_КомментарийРТУ = СтрШаблон("Дата получения цен %1", НоваяСтрока.гф_ДатаДляРТУ);
		//КонецЕсли;
	...
	ИначеЕсли ТекущаяСтрока[ПараметрыРазбиенияСтроки.ИмяПоляКоличество] = Количество Тогда
		...
		//ТекущаяСтрока.гф_КомментарийРТУ = "";
		//ТекущаяСтрока.гф_СкидкаДляРТУ = 1 - ТекущаяСтрока.ПроцентРучнойСкидки/100;
		//
		//Для Каждого СтрокаСтруктураПереданныхДанных Из СтруктураПереданныхДанных.Результат Цикл
		//	
		//	Если Не ЗначениеЗаполнено(ТекущаяСтрока.гф_КомментарийРТУ) Тогда
		//		ТекущаяСтрока.гф_КомментарийРТУ = СтрокаСтруктураПереданныхДанных.КомментарийРТУ;
		//	Иначе
		//		ТекущаяСтрока.гф_КомментарийРТУ = ТекущаяСтрока.гф_КомментарийРТУ + " " + СтрокаСтруктураПереданныхДанных.КомментарийРТУ;
		//	КонецЕсли;
		//	ТекущаяСтрока.гф_ДатаДляРТУ = СтрокаСтруктураПереданныхДанных.ДатаДляРТУ;
		//	ТекущаяСтрока.гф_СкидкаДляРТУ = ТекущаяСтрока.гф_СкидкаДляРТУ * (1 - СтрокаСтруктураПереданныхДанных.СкидкаДляРТУ / 100);
		//КонецЦикла;
		//
		//ТекущаяСтрока.гф_СкидкаДляРТУ = (1 - ТекущаяСтрока.гф_СкидкаДляРТУ) * 100;
		//
		//Если ЗначениеЗаполнено(ТекущаяСтрока.ПроцентРучнойСкидки) И ТекущаяСтрока.ПроцентРучнойСкидки > 0 И ЗначениеЗаполнено(ТекущаяСтрока.гф_КомментарийРТУ) Тогда
		//	ТекущаяСтрока.гф_КомментарийРТУ = СтрШаблон("Дата получения цен %1, %2 %3, %4", ТекущаяСтрока.гф_ДатаДляРТУ, "%Скидки", ТекущаяСтрока.ПроцентРучнойСкидки, ТекущаяСтрока.гф_КомментарийРТУ);
		//ИначеЕсли ЗначениеЗаполнено(ТекущаяСтрока.ПроцентРучнойСкидки) И ТекущаяСтрока.ПроцентРучнойСкидки > 0 И Не ЗначениеЗаполнено(ТекущаяСтрока.гф_КомментарийРТУ) Тогда
		//	ТекущаяСтрока.гф_КомментарийРТУ = СтрШаблон("Дата получения цен %1, %2 %3", ТекущаяСтрока.гф_ДатаДляРТУ, "%Скидки", ТекущаяСтрока.ПроцентРучнойСкидки);
		//Иначе
		//	ТекущаяСтрока.гф_КомментарийРТУ = СтрШаблон("Дата получения цен %1", ТекущаяСтрока.гф_ДатаДляРТУ);
		//КонецЕсли;
...
Процедура ДобавитьСтрокуРазбиениемДобавитьКоличество(ПараметрыОбработки, ТЗ, ТекущаяСтрока, Товары, ПроверкаОтказ, СписокКодовСтроки, УпаковочныйЛист, СтруктураПереданныхДанных, СкидкаВКоробах)
	...
	РазбитьСтрокуЗавершениеДобавленоПоПричине(НоваяСтрока, ДополнительныеПараметры, Неопределено);
	
КонецПроцедуры

Процедура ДобавитьСтрокуРазбиением4(ПараметрыОбработки, ТЗ, ТекущаяСтрока, Товары, ПроверкаОтказ, СписокКодовСтроки, УпаковочныйЛист, СтруктураПереданныхДанных)
	...
	Иначе
		...
		Если КоличествоОбщееНоменклатуры > Количество Тогда
			ЕщеТребуетсяОтгрузитьКоличество = 0;
			...
			Для Каждого ЭлементМассива Из ТекущаяСтрока Цикл
				...
				Если ЭлементМассива.гф_ДобавленоПоПричине Тогда
					...
					Если ЭлементМассива.КоличествоУпаковок = Количество Тогда
						...
						// Дописать все остальное 
						
						Прервать;
					ИначеЕсли ЭлементМассива.КоличествоУпаковок > Количество Тогда
						
						// Данная ситуация исключена, так как пожобная номенклатура добавляемая по причине, добавляется в том количестве,
					...
					ИначеЕсли Не ЭлементМассива.КоличествоУпаковок = 0 И ЭлементМассива.КоличествоУпаковок < Количество Тогда
						...
						ЕщеТребуетсяОтгрузитьКоличество = Количество - ЭлементМассива.КоличествоУпаковок;
					Иначе
						// Отказ вывести	 
						ПроверкаОтказ = Истина;
						Возврат;
			...
			Если ЕщеТребуетсяОтгрузитьКоличество Тогда
				
				Для Каждого ЭлементМассива Из ТекущаяСтрока Цикл
					
					Количество = ЕщеТребуетсяОтгрузитьКоличество;
...
Процедура ДобавитьСтрокуРазбиением5(ПараметрыОбработки, ТЗ, ТекущаяСтрока, Товары, ПроверкаОтказ, СписокКодовСтроки, УпаковочныйЛист, СтруктураПереданныхДанных)
...
//Функция ПараметрыРазбиенияСтрокиДляУпаковочногоЛиста() Экспорт
//	
//	ПараметрыРазбиенияСтроки = Новый Структура;
...
//&НаСервере
//Процедура ЗаполнитьРезервыНаСервере(Заказ, УпаковочныйЛист, ТЗ_СТоварами, Массив_СтрокиНаличиеПроверка)
//	
//		//выполняем последовательное копирование
//	ТЗ_НаличиеПроверкаЗаказ = Новый Массив;
//	Для Инд = 0 По Массив_СтрокиНаличиеПроверка.ВГраница() Цикл
//		ТЗ_НаличиеПроверкаЗаказ.Добавить(Массив_СтрокиНаличиеПроверка[Инд]);
//	КонецЦикла;
//		
//	// Выгрузка актуальных данных из таблицы "Наличие"
//	//ТЗ_НаличиеПроверка = Новый ТаблицаЗначений;
//	//ТЗ_НаличиеПроверка = Объект.Наличие.Выгрузить();
//	Проверка = Истина;
//	
//	// Выгрузка актуальных данных из упаковочного листа
...
//	
//	//
//	НайдСтр = ТЗ_СТоварами.Найти(УпаковочныйЛист);
...
//	//Иначе
//	//	ТЗДляПроверкиТоваровИзУпаковочногоЛиста = УпаковочныйЛист.Товары.Выгрузить();
//	//КонецЕсли;
//	
...
//	
//	//ТЗ_НаличиеПроверкаЗаказ = ТЗ_НаличиеПроверка.НайтиСтроки(Новый Структура("ЗаказНаОтгрузку", гф_Заказ));
//	
//	//Предварительное сравнение номенклатуры упаковочноголиста с номенклатурой в заказе (есть ли она вообще в заказе) 
//	//Необходима, что бы не проходить впустую сравнение на доступное количество
//	//Для Каждого СтрокаДляПроверки Из ТЗДляПроверкиТоваровИзУпаковочногоЛиста Цикл
//	//    Для Каждого СтрокаНаличие Из ТЗ_НаличиеПроверкаЗаказ Цикл
//	//		
...
//	Для Каждого СтрокаНаличие Из ТЗ_НаличиеПроверкаЗаказ Цикл
//		
//		НайтиСтрокиПроверкаЗаказ = ТЗДляПроверкиТоваровИзУпаковочногоЛиста.НайтиСтроки(Новый Структура("Номенклатура, Характеристика", СтрокаНаличие.Номенклатура, СтрокаНаличие.Характеристика));
//		
//		Если НайтиСтрокиПроверкаЗаказ.Количество() > 0 Тогда
//			Для Каждого ЭлементМассива Из НайтиСтрокиПроверкаЗаказ Цикл
//				ЭлементМассива.Проверка = Истина;
//				//ЭлементМассива.УпаковочныйЛист = УпаковочныйЛист;
//			КонецЦикла;
//		//Иначе
//		//	СтрокаДляПроверки.Проверка = Ложь;
//		//	СтрокаДляПроверки.УпаковочныйЛист = УпаковочныйЛист;
...
//	НайденнаяСтрока = ТЗДляПроверкиТоваровИзУпаковочногоЛиста.Найти(Ложь, "Проверка");
//	
...
//		
//		//гф_Заказ = УпаковочныйЛист.гф_Заказ;
//		
//		//ТЗ_НаличиеПроверкаЗаказ = ТЗ_НаличиеПроверка.НайтиСтроки(Новый Структура("ЗаказНаОтгрузку", гф_Заказ));
//	
...
//		//Для Каждого СтрокаУпаковочногоЛиста Из УпаковочныйЛист.Товары Цикл
//		Для Каждого СтрокаУпаковочногоЛиста Из ТЗДляПроверкиТоваровИзУпаковочногоЛиста Цикл
//				
//			Для Каждого СтрокаНаличие Из ТЗ_НаличиеПроверкаЗаказ Цикл
//				
//				Если СтрокаУпаковочногоЛиста.Номенклатура = СтрокаНаличие.Номенклатура
//					И СтрокаУпаковочногоЛиста.Характеристика = СтрокаНаличие.Характеристика Тогда
//					
//					Если СтрокаНаличие.Доступно - СтрокаУпаковочногоЛиста.КоличествоУпаковок < 0 Тогда
//						Проверка = Ложь;
//						Прервать;
//					Иначе
//						СтрокаНаличие.Доступно = СтрокаНаличие.Доступно - СтрокаУпаковочногоЛиста.КоличествоУпаковок;
//						Проверка = Истина;
//					КонецЕсли;
//					
//				КонецЕсли;
...
//		Если Проверка Тогда
//			
//			//ОбъектНаличиеЗаказ = ТЗ_НаличиеПроверка.НайтиСтроки(Новый Структура("ЗаказНаОтгрузку", гф_Заказ));
//			
//			
//			Для Каждого СтрокаНаличие Из Массив_СтрокиНаличиеПроверка Цикл
//					
//					ОбъектНаличиеЗаказ = ТЗДляПроверкиТоваровИзУпаковочногоЛиста.НайтиСтроки(Новый Структура("Номенклатура, Характеристика", СтрокаНаличие.Номенклатура, СтрокаНаличие.Характеристика));
//					
//					
//					Если ОбъектНаличиеЗаказ.Количество() > 0 Тогда
//						
//						Для Каждого СтрокаУпаковочногоЛиста Из ОбъектНаличиеЗаказ Цикл
//							Если Не СтрокаНаличие.Доступно - СтрокаУпаковочногоЛиста.КоличествоУпаковок < 0 Тогда 
//								СтрокаНаличие.Доступно = СтрокаНаличие.Доступно - СтрокаУпаковочногоЛиста.КоличествоУпаковок;
...
//		//				И СтрокаУпаковочногоЛиста.Номенклатура = СтрокаНаличие.Номенклатура
//		//				И СтрокаУпаковочногоЛиста.Характеристика = СтрокаНаличие.Характеристика Тогда
//		//				
//		//				Если Не СтрокаНаличие.Доступно - СтрокаУпаковочногоЛиста.КоличествоУпаковок < 0 Тогда 
//		//					СтрокаНаличие.Доступно = СтрокаНаличие.Доступно - СтрокаУпаковочногоЛиста.КоличествоУпаковок;
...
Процедура ЗаполнитьРезервыНаСервере(Заказ, УпаковочныйЛист, УпаковочныеЛистыСТаблицейТовары, МассивНоменклатурыПоЗаказу)
	...
	//НаличиеПроверкаЗаказ = НаличиеПроверка.НайтиСтроки(Новый Структура("ЗаказНаОтгрузку", Заказ));
...
&НаСервере
Функция ОформитьРасходныеОрдераНаСервере(УпаковочныйЛистДляОрдера, Отказ)
...
Процедура СнятьРезервОбработкаДанныхТовары()
				...
				Если Строка_Т.Отгрузить И ЗначениеЗаполнено(Строка_Т.Номенклатура) Тогда
					...
					Если НайтиЗаказ = Неопределено Тогда
						...
						Иначе
							...
							СтрокаЗаказы.СписокУпаковочныйЛистНоменклатура = ТЗ_Номенклатура;
	...
	Для Каждого СтрокаЗаказы Из ТЗ_Заказы Цикл
		...
		Для Каждого СтрокаУпаковочныйЛист Из СтрокаЗаказы.СписокУпаковочныйЛистНоменклатура Цикл
...
Процедура УстановитьЗначениеТоварыВКоробах()
	...
	Если Не Объект.ТоварыВКоробках Тогда
		...
		//Элементы.ДеревоДокументовЦенаПрайса.Видимость 					= Ложь;
		//Элементы.ДеревоДокументовСкидка.Видимость 						= Ложь;
		Элементы.ДеревоДокументовСостояниеКороба.Видимость 				= Ложь;
		Элементы.ДеревоДокументовКоличествоВНаличии.Видимость 			= Ложь;
		//Элементы.ДеревоДокументовДатаПрайсЛиста.Видимость 				= Ложь;
	...
	Иначе
		...
		//Элементы.ДеревоДокументовЦенаПрайса.Видимость 					= Истина;
		//Элементы.ДеревоДокументовСкидка.Видимость 						= Истина;
		Элементы.ДеревоДокументовСостояниеКороба.Видимость 				= Истина;
		Элементы.ДеревоДокументовКоличествоВНаличии.Видимость 			= Истина;
		//Элементы.ДеревоДокументовДатаПрайсЛиста.Видимость 				= Истина;
...
Функция ЗаполнитьДеревоЗначенийДляНеКоробногоСклада()
	...
	//	Если Не ЗначениеЗаполнено(Строка.ЗаказНаОтгрузку) Тогда
	//		МассивУдалитьСтроки.Добавить(Строка);
	//	КонецЕсли;
	//КонецЦикла;
	//Для Каждого УдалитьСтроку Из МассивУдалитьСтроки Цикл
	...
	Для Каждого СтрокаОстатки Из ТаблицаОстатки Цикл
		...
		Если Не ТЗВыборкаНоменклатураСтрока = Неопределено Тогда
			...
			СтрокаОстатки.Цена = ТЗВыборкаНоменклатураСтрока.Цена;
			
			СтрокаОстатки.Стоимость = СтрокаОстатки.Доступно * ТЗВыборкаНоменклатураСтрока.Цена;
			
			СтрокаОстатки.КоличествоЗаказано 	= ТЗВыборкаНоменклатураСтрока.КоличествоУпаковок;
...
Функция ПолучитьНоменклатуруТабличнойЧастиЗаказа(ТЗ_Товары, СкидкаВКоробах, ВариантыОбеспечения)
	...
	//|	ВТ.ВариантОбеспечения КАК ВариантОбеспечения,
