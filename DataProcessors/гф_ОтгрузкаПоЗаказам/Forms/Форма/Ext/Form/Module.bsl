...
Процедура ПриОткрытии(Отказ)
	...
	// ++ Галфинд ВолковЕВ 03.10.2023 Очищаем общую сумму в шапке формы
	Объект.СтоимостьОтмеченныхКОтгрузкеТоваров = 0;
	// -- Галфинд ВолковЕВ 03.10.2023
...
Функция ЗаполнитьТаблицуОстатков(Дерево, Склад, СхемаКомпоновкиДанных)
	...
	// ++ Галфинд Волков 2023/12/15 Условие добавлено в основной запрос
...
Функция ПолучитьРезультатКомпоновкиДанных(Склад, Клиент, СхемаКомпоновкиДанных)
	...
	Если ОтборПредоплатныхКлиентов = 1 Тогда
		...
		СхемаКомпоновкиДанных.НаборыДанных.НаборДанных1.Запрос = СтрЗаменить(СхемаКомпоновкиДанных.НаборыДанных.НаборДанных1.Запрос, "&ПредоплатныйДоговор", "ЗаказНоменклатураХарактеристика.ПредоплатныйДоговор");
		
	ИначеЕсли ОтборПредоплатныхКлиентов = 2 Тогда
		
		СхемаКомпоновкиДанных.НаборыДанных.НаборДанных1.Запрос = СтрЗаменить(СхемаКомпоновкиДанных.НаборыДанных.НаборДанных1.Запрос, "&ПредоплатныйДоговор", "Не ЗаказНоменклатураХарактеристика.ПредоплатныйДоговор");
		
	Иначе
		
		СхемаКомпоновкиДанных.НаборыДанных.НаборДанных1.Запрос = СтрЗаменить(СхемаКомпоновкиДанных.НаборыДанных.НаборДанных1.Запрос, "&ПредоплатныйДоговор", "ИСТИНА");
...
Процедура РассчитатьЛимиты(ДеревоЗначений)
	...
	//	|ГДЕ
	//	|	гф_ЛимитыКонтрагентовСрезПоследних.Статус = ЗНАЧЕНИЕ(Справочник.гф_СтатусыЛимитов.Действующий)";
	
	// Получаем все возможные лимиты для этого списка контрагентов из регистра. Лимиты должны быть активные.
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	гф_ЛимитыКонтрагентовСрезПоследних.Период КАК Период,
	...
	Для Каждого СтрВыборка Из Выборка Цикл
		...
		Для Каждого Строка_ЛимитыТЗ Из ТЗЛимит Цикл
			...
			Если Строка_ЛимитыТЗ.Партнер = СтрВыборка.АналитикаУчетаПоПартнерам.Партнер  Тогда
				...
				//Если СтрВыборка.Валюта = Константы.БазоваяВалютаПоУмолчанию.Получить() Тогда
				//	КоэфВалюты = 1;
				//Иначе	
				//	КоэфВалюты = ПолучитьКурсВалюты(СтрВыборка.Валюта);	
				//КонецЕсли;
				//
				...
				Если Не СтрВыборка.СуммаОстаток = 0 И СтрВыборка.СуммаОстаток > 0 Тогда
					...
					//Строка_ЛимитыТЗ.ДолгКонтрагентаПередОрганизацией = Строка_ЛимитыТЗ.ДолгКонтрагентаПередОрганизацией + СтрВыборка.СуммаОстаток * КоэфВалюты;
				Иначе
					Строка_ЛимитыТЗ.АвансОтКонтрагента = Строка_ЛимитыТЗ.АвансОтКонтрагента + СтрВыборка.СуммаОстаток * (-1);
					//Строка_ЛимитыТЗ.АвансОтКонтрагента = Строка_ЛимитыТЗ.АвансОтКонтрагента + СтрВыборка.СуммаОстаток * (-1) * КоэфВалюты;
				КонецЕсли;
	...
	Для Каждого СтрВыборка Из Выборка Цикл
		...
		Если НайденныеСтрокиТЗЛимит.Количество() = 1 Тогда
			...
			Если Не СтрВыборка.СуммаОстаток = 0 И СтрВыборка.СуммаОстаток > 0 Тогда
				...
				//НайденныеСтрокиТЗЛимит[0].ДолгКонтрагентаПередОрганизацией = НайденныеСтрокиТЗЛимит[0].ДолгКонтрагентаПередОрганизацией + СтрВыборка.СуммаОстаток * КоэфВалюты;
			Иначе
				НайденныеСтрокиТЗЛимит[0].АвансОтКонтрагента = НайденныеСтрокиТЗЛимит[0].АвансОтКонтрагента + СтрВыборка.СуммаОстаток * (-1);
				//НайденныеСтрокиТЗЛимит[0].АвансОтКонтрагента = НайденныеСтрокиТЗЛимит[0].АвансОтКонтрагента + СтрВыборка.СуммаОстаток * (-1) * КоэфВалюты;
			КонецЕсли;
		...
		ИначеЕсли НайденныеСтрокиТЗЛимит.Количество() = 0 Тогда
			...
			Если НайденныеСтрокиТЗЛимит.Количество() = 0 Тогда
				...
				Если Не СтрВыборка.СуммаОстаток = 0 И СтрВыборка.СуммаОстаток > 0 Тогда
					...
					//Стр.ДолгКонтрагентаПередОрганизацией = СтрВыборка.СуммаОстаток * КоэфВалюты;
				Иначе
					Стр.АвансОтКонтрагента = СтрВыборка.СуммаОстаток * (-1);
					//Стр.АвансОтКонтрагента = СтрВыборка.СуммаОстаток * (-1) * КоэфВалюты;
				КонецЕсли;
			КонецЕсли;
		Иначе
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			Для Каждого СтрокаЗаказы Из НайденныеСтроки Цикл
				...
				Если Не (СтрокаЗаказы.ВозможнаЗаписьЗаказа И СтрокаЗаказы.НовыеДанныеДляТаблицыПоЗаказу.Количество() > 0) Тогда
					Проверка = Ложь;
					СообщЗаказ = "" + СтрокаЗаказы.Заказ;
				КонецЕсли;
				
			КонецЦикла;
			
			Если Не Проверка Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось выполнить запись заказа/ов %1'"), СообщЗаказ);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,);
				Продолжить;
			КонецЕсли;
			...
			Для Каждого СтрокаЗаказы Из НайденныеСтроки Цикл
				...
				Если Не СтрокаЗаказы.Перепроведен Тогда
					Проверка = Ложь;
					СообщЗаказ = "" + СтрокаЗаказы.Заказ;
				КонецЕсли;
			...
			Если Не Проверка Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось выполнить перепроведение заказа/ов %1'"), СообщЗаказ);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,);
				Продолжить;
			КонецЕсли;
...
Процедура ПолучитьКомментарийИСкидкиДляРТУ(КомментарийРТУ, СкидкаДляРТУ, ДатаДляРТУ, 
	Знач ПроцентРучнойСкидки, СтруктураПереданныхДанных, ПрайсЛистДляРТУ)
	...
	Если Не ДанныеДляРТУ.Количество() Тогда
		
		Возврат;
		
	КонецЕсли;	 
	
	Заказ = СтруктураПереданныхДанных.Заказ;
	
	Если РасчетЦен = 2 Тогда
		
		ВидыСкидкок = Новый Массив;
		
		ВидыСкидкок.Добавить(ПредопределенноеЗначение("Перечисление.гф_ВидыСкидок.РекламационныйБонус"));
		ВидыСкидкок.Добавить(ПредопределенноеЗначение("Перечисление.гф_ВидыСкидок.СкидкаЗаОбъем"));
		...
		Запрос.УстановитьПараметр("ВидыСкидкок",	ВидыСкидкок);
		Запрос.УстановитьПараметр("Дата",			ДатаДляРТУ);
		
		Запрос.Текст = "ВЫБРАТЬ
		|	СУММА(ЕСТЬNULL(гф_ИсторияИзмененияСкидокСрезПоследних.Скидка, 0)) КАК Скидка
		|ИЗ
		...
		|ГДЕ
		|	гф_ИсторияИзмененияСкидокСрезПоследних.ВидСкидки В(&ВидыСкидкок)";
		...
		Иначе	
			...
			ПроцентРучнойСкидки = Выборка.Скидка;
	...
	Для Каждого СтрокаСтруктураПереданныхДанных Из ДанныеДляРТУ Цикл
		...
		мСкидкаДляРТУ = Мин(СтрокаСтруктураПереданныхДанных.СкидкаДляРТУ, ЗначениеСто);
		
		РучнаяСкидкаКоэффициент = (ЗначениеСто - ПроцентРучнойСкидки) / ЗначениеСто; 
		СкидкаКоэффициент = (ЗначениеСто - мСкидкаДляРТУ) / ЗначениеСто; 
		
		СкидкаДляРТУ = ЗначениеСто - (РучнаяСкидкаКоэффициент * СкидкаКоэффициент * ЗначениеСто) 
		- СтрокаСтруктураПереданныхДанных.НаценкаДляРТУ;
		
		ДатаДляРТУ = СтрокаСтруктураПереданныхДанных.ДатаДляРТУ;
		
		Если РасчетЦен = 2 Тогда
			
			КомментарийРТУ = СтрШаблон("Дата получения цен %1. ",Формат(ДатаДляРТУ, "ДФ=dd.MM.yyyy")); 
		...
		Если СкидкаДляРТУ > 0 Тогда
			...
			КомментарийРТУ = КомментарийРТУ + "Скидка " + СкидкаДляРТУ + "%";
			
		ИначеЕсли СкидкаДляРТУ < 0 Тогда 
			
			КомментарийРТУ = КомментарийРТУ + "Наценка " + (-СкидкаДляРТУ) + "%";
			
		Иначе
			
			КомментарийРТУ = КомментарийРТУ + "Скидки отстутствуют";
...
Процедура ЗаполнитьРезервыНаСервере(Заказ, УпаковочныйЛист, УпаковочныеЛистыСТаблицейТовары, МассивНоменклатурыПоЗаказу, ЗаказУпаковочныеЛисты)
...
Функция ЗаполнитьДеревоЗначенийДляНеКоробногоСклада()
	...
	Если ТаблицаКомпоновкаДанных.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Данные для отгрузки, соответствующие условию отбора.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	...
	Для Каждого СтрокаОстатки Из ТаблицаОстатки Цикл
		...
		//КонецЕсли;
			...
			ИначеЕсли МассивСтрокТоварыЗаказаОтмененоЛожь.Количество() > 0 Тогда	
				...
				// Стоимость с НДС
				СтрокаОстатки.СтавкаНДС = МассивСтрокТоварыЗаказаОтмененоЛожь[0].СтавкаНДС;
