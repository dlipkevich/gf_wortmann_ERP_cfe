			...
			Для каждого Строка2 Из Строка1.Строки Цикл
				Для каждого Строка3 Из Строка2.Строки Цикл
					Для каждого Строка4 Из Строка3.Строки Цикл
						...
						ТЗ_22.Колонки.Добавить("УпаковочныйЛистСсылка");
						//ТЗ_22.Колонки.Добавить("Проверка");
						
						Для каждого Строка5 Из Строка4.Строки Цикл
							//ЗаполнитьРезервыНаСервере(Строка5.Заказ, Строка5.УпаковочныйЛистСсылка);	
							НоваяСтрока = ТЗ_22.Добавить();
							НоваяСтрока.УпаковочныйЛистСсылка = Строка5.УпаковочныйЛистСсылка;
							//НоваяСтрока.Проверка = Строка5.Проверка;							
							
						КонецЦикла;
						
						Для каждого Строка_22 Из ТЗ_22 Цикл
							
							Нашли = Строка4.Строки.НайтиСтроки(Новый Структура("УпаковочныйЛистСсылка", Строка_22.УпаковочныйЛистСсылка));
							
							Для каждого Стр Из Объект.ДоступностьУпаковочныхЛистов Цикл
								
								Если Стр.УпаковочныйЛист =Нашли[0].УпаковочныйЛистСсылка И Не Стр.Проверка Тогда
									Строка4.Строки.Удалить(Нашли[0]);
...
Процедура ЗаполнитьРезервыНаСервере(Заказ, УпаковочныйЛист)
	...
	//НаличиеВсегоПеречняНоменклатурыВОстатках = Истина; 
	Т1 = Новый ТаблицаЗначений;
	//Т2 = Новый ТаблицаЗначений;
	Т1 = УпаковочныйЛист.Товары.Выгрузить();
	//Т2 = НаличиеПроверка.Скопировать();
	Т1.Колонки.Добавить("Проверка");
	Т1.Колонки.Добавить("Строка1");
	
	Для Каждого Строка1 Из Т1 Цикл
	    Для Каждого СтрокаНаличие Из НаличиеПроверка Цикл
			
			 Если УпаковочныйЛист.гф_Заказ = СтрокаНаличие.ЗаказНаОтгрузку
				И Строка1.Номенклатура = СтрокаНаличие.Номенклатура
				И Строка1.Характеристика = СтрокаНаличие.Характеристика Тогда
				Строка1.Проверка = Истина;
				Строка1.УпаковочныйЛист = УпаковочныйЛист;
				Прервать;
			Иначе
				Строка1.Проверка = Ложь;
				Строка1.УпаковочныйЛист = УпаковочныйЛист;
			КонецЕсли;
	...
	НайденнаяСтрока = Т1.Найти(Ложь, "Проверка");
