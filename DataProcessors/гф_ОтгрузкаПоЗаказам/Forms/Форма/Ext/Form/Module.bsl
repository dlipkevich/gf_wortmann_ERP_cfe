...
Функция ЗаполнитьДеревоЗначенийДляНеКоробногоСклада()
	...
	Для Каждого СтрокаОстатки Из ТаблицаОстатки Цикл
		...
		Если Не ТЗВыборкаНоменклатураСтрока = Неопределено Тогда
			...
			// Стоимость с НДС
			СтрокаОстатки.Стоимость = СтрокаОстатки.КоличествоОтгрузить * ТЗВыборкаНоменклатураСтрока.Цена * (1 + ТЗВыборкаНоменклатураСтрока.СтавкаНДС.Ставка/100);
...
Процедура РазвернутьСтрокиДереваПомеченных(Команда)
	СтандартныеПодсистемыКлиент.РазвернутьУзлыДерева(ЭтотОбъект, "Элементы.ДеревоДокументов");
КонецПроцедуры

&НаКлиенте
Процедура СвернутьСтрокиДереваВыбораПомеченных(Команда)
	ВсеСтроки = Элементы.ДеревоДокументов;
	Для Каждого ДанныеСтроки Из Элементы.ДеревоДокументов.ПолучитьЭлементы() Цикл 
