...
Процедура РассчитатьЛимиты(ДеревоЗначений)
	...
	Для Каждого Строка_Лимиты Из ТЗЛимит Цикл
		...
		Если Строка_Лимиты.Статус = Справочники.гф_СтатусыЛимитов.Действующий Тогда
			...
			КонецЕсли;
		ИначеЕсли Не Строка_Лимиты.Статус = Справочники.гф_СтатусыЛимитов.Действующий И Строка_Лимиты.СуммаВзаиморасчетов > 0 Тогда
			...
			Для Каждого Строка_Т Из Строка_Н.Строки Цикл
				...
				Если Строка_Т.Отгрузить И ЗначениеЗаполнено(Строка_Т.УпаковочныйЛистСсылка) Тогда
					...
					Если НайтиЗаказ = Неопределено Тогда
						...
						ТЗ_УпаковочныеЛисты.Колонки.Добавить("УпаковочныйЛист");
						ТЗ_УпаковочныеЛисты.Колонки.Добавить("КоличествоОстаток");
						ТЗ_УпаковочныеЛисты.Колонки.Добавить("ЦенаПары");
						ТЗ_УпаковочныеЛисты.Колонки.Добавить("СписокКодовСтроки");
						ТЗ_УпаковочныеЛисты.Колонки.Добавить("Перепроведен");
						ТЗ_УпаковочныеЛисты.Колонки.Добавить("КоличествоПарОбуви");
						ТЗ_УпаковочныеЛисты.Колонки.Добавить("Скидка");

						СтрокаУпаковочныеЛисты = ТЗ_УпаковочныеЛисты.Добавить();
						
						СтрокаУпаковочныеЛисты.УпаковочныйЛист 		= Строка_Т.УпаковочныйЛистСсылка;
						СтрокаУпаковочныеЛисты.КоличествоОстаток 	= Строка_Т.КоличествоОстаток;
						СтрокаУпаковочныеЛисты.ЦенаПары 			= Строка_Т.Цена;
						СтрокаУпаковочныеЛисты.КоличествоПарОбуви 	= Строка_Т.УпаковочныйЛистСсылка.Товары.Количество();	
		...
		Для Каждого СтрокаУпаковочныйЛист Из СтрокаЗаказы.УпаковочныеЛисты Цикл
			...
			Для Каждого СтрокаТоварыВКоробах Из ТЗ_ТоварыВКоробах Цикл
				...
				ИначеЕсли СтрокаТоварыВКоробах.ВариантКомплектации = СтрокаУпаковочныйЛист.УпаковочныйЛист.гф_Комплектация Тогда
						...
						ДобавитьСтрокуРазбиениемДляУпаковочногоЛиста(ПараметрыОбработки, ТЗКВТ, СтрокаТоварыВКоробах, СтруктураПереданныхДанных, ОтказЗаписьУпаковочногоЛиста, СтрокаУпаковочныйЛист.Скидка);
						
						Если ОтказЗаписьУпаковочногоЛиста Тогда
							ТекстСообщения = СтрШаблон(НСтр("ru = 'Не достаточно остатков в ""%1"" по ""%2"" на вкладке ""Товары в коробах""'"), ОбъектЗаказ, СтрокаУпаковочныйЛист.УпаковочныйЛист);
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
							Прервать;	
						КонецЕсли;
						...
						СтрокаПричиныИзменения.Причина = Справочники.ПричиныОтменыЗаказовКлиентов.НайтиПоНаименованию("Отгружено");
...
Процедура ДобавитьСтрокуРазбиениемДляУпаковочногоЛиста(ПараметрыОбработки, ТабличнаяЧасть, ТекущаяСтрока, СтруктураПереданныхДанных, ПроверкаОтказ, СкидкаДляРТУДляЗаписьВОрдер)
