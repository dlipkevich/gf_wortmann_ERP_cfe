...
Процедура РассчитатьЛимиты(ДеревоЗначений)
	...
	Запрос.Текст = 
		...
		|	И (РасходныйОрдерНаТовары.Статус = &СтатусКОтбору Или РасходныйОрдерНаТовары.Статус = &СтатусПроверен)
	...
	ВыборкаДет.Свернуть("Получатель, СкладОрганизация", "СуммаНоменклатуры");
	...
	Для Каждого Строка_ЛимитыТЗ Из ТЗЛимитыКонтрагентов Цикл
		...
		Для Каждого СтрокаВыборка Из ВыборкаДет Цикл
			...
			Если Строка_ЛимитыТЗ.Партнер = СтрокаВыборка.Получатель
...
Процедура ПроверкаРеквизитовВыбранныхОбъектовДляОтгрузки(Отказ, ЗаказСПеремещением)
	...
	Для Каждого МассивСтрока Из МассивУпаковочныхЛистов Цикл
		...
		Расхождение = ПроверитьДокумент(МассивСтрока.УпаковочныйЛистСсылка, СостояниеКороба, ФактическоеСостояниеКороба);
