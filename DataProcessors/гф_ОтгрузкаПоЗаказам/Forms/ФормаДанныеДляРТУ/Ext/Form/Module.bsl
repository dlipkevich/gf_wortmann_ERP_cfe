...
Процедура РассчитатьСкидкуДляРТУ()
	...
	Для Каждого СтрокаТЗ Из Объект.Скидки Цикл
		Если Не ЗначениеЗаполнено(СтрокаТЗ.СкидкаДляРТУ) Тогда
			Продолжить;
		КонецЕсли;
		СкидкаДляРТУ = СкидкаДляРТУ * (1 - СтрокаТЗ.СкидкаДляРТУ / ЗначениеСто);
	КонецЦикла;
	// с учетом %Скидки из заказа
	СкидкаДляРТУ = СкидкаДляРТУ * (1 - ПроцентСкидкиМаксимум / ЗначениеСто);
	мСкидкаДляРТУ = (1 - СкидкаДляРТУ) * ЗначениеСто;
