...
Процедура ЗаполнитьТЧСкидкиПоЗаказам()
	...
	Для Каждого ЭлементМассива Из Параметры.МассивЗаказы Цикл
		...
		Если СтрокиПоЗаказу.Количество() = 0 Тогда
			...
			нс.РучнаяСкидка = ПолучитьПроцентСкидкиПоЗаказу(ЭлементМассива);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаТЧ Из Объект.СкидкиПоЗаказам Цикл 
		...
		Если СтрокаТЧ.СезонныйЗаказ Тогда 
			...
			Если СкидкаНаПредоплату <> 0 Тогда
				...
				мСкидкаДляРТУ = СкидкаНаПредоплату;
				
			Иначе	
				
				мСкидкаДляРТУ = СкидкаНаКурс;
		...
		мСкидкаДляРТУ = Мин(мСкидкаДляРТУ, ЗначениеСто);
		
		СтрокаТЧ.СкидкаДляРТУ = мСкидкаДляРТУ;
		СтрокаТЧ.НаценкаДляРТУ = НаценкаНаКурс; 
...
Функция ЗаполнитьРезультат()
	...
	Для Каждого СтрокаТЧ Из Объект.СкидкиПоЗаказам Цикл
		...
		Если СтрокаТЧ.ИтоговаяСкидкаДляРТУ = 0 Тогда
			
			КомментарийРТУ = "";
			
		ИначеЕсли СтрокаТЧ.ИтоговаяСкидкаДляРТУ > 0 Тогда
			
			КомментарийРТУ = "Скидка " + СтрокаТЧ.ИтоговаяСкидкаДляРТУ + "%";
			
		Иначе
			
			КомментарийРТУ = "Наценка " + (-СтрокаТЧ.ИтоговаяСкидкаДляРТУ) + "%";
			
		КонецЕсли;
		
		СкидкаПоЗаказу = Новый Структура("Заказ, СкидкаДляРТУ, НаценкаДляРТУ,РучнаяСкидка");
