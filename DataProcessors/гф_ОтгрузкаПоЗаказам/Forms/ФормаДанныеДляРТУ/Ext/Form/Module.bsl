...
Процедура ЗаполнитьТЧСкидкиПоЗаказам()
	Для Каждого ЭлементМассива Из Параметры.МассивЗаказы Цикл
		...
		Иначе	
			СтрокиПоЗаказу[0].РучнаяСкидка = ПолучитьПроцентСкидкиПоЗаказу(ЭлементМассива);
		КонецЕсли;
	КонецЦикла;
...
Процедура РассчитатьСкидкуДляРТУ()  
	...
	Для Каждого СтрокаТЧ Из Объект.СкидкиПоЗаказам Цикл 
		...
		НаценкаКоэффициент = (ЗначениеСто + НаценкаНаКурс) / ЗначениеСто; 
	...
	Для Каждого СтрокаТЧ Из Объект.СкидкиПоЗаказам Цикл  
		...
		Если СтрокаТЧ.СезонныйЗаказ Тогда 
			...
			Если СкидкаНаПредоплату <> 0 Тогда
				...
				Скидки.Добавить(СкидкаНаПредоплату, ПредопределенноеЗначение("Перечисление.гф_ВидыСкидок.СкидкаЗаПредоплату"));
				
			Иначе	
				
				Скидки.Добавить(СкидкаНаКурс, ПредопределенноеЗначение("Перечисление.гф_ВидыСкидок.СкидкаЗаКурс"));
