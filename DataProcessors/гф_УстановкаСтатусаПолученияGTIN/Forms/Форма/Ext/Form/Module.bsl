...
Процедура ЗагрузитьИзEcxel(Команда)
	...
	ОткрытьФорму("ВнешняяОбработка.гф_УстановкаСтатусаПолученияGTIN.Форма.ФормаЗагрузки", СтруктураПараметров, ЭтотОбъект, , , ,
...
Процедура ЗаполнитьНаСервере()
	...
	Запрос.Текст = 
		...
		|	ВТ_НоменклатураВся.Номенклатура.Артикул КАК Артикул,
		...
		|		ПО ВТ_НоменклатураВся.Номенклатура = ШтрихкодыНоменклатуры.Номенклатура
		|ГДЕ
		|	ВЫБОР
		|			КОГДА &Штрихкод <> """"
		|				ТОГДА ШтрихкодыНоменклатуры.Штрихкод = &Штрихкод
