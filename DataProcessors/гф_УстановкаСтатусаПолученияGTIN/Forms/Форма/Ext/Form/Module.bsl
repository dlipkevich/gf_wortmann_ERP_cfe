...
Процедура ЗаполнитьНаСервере()
	...
	// Галфинд_ДомнышеваКР_22_09_2023
	// Переделала запрос по новым замечаниям
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ИЗ
		|	ВТ_ПТУ КАК ВТ_ПТУ
		|		ПОЛНОЕ СОЕДИНЕНИЕ ВТ_ОграниченияДоп2 КАК ВТ_ОграниченияДоп2
		|		ПО ВТ_ПТУ.Номенклатура = ВТ_ОграниченияДоп2.Номенклатура
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Если ЗначениеЗаполнено(ДокументЗагрузки) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПОЛНОЕ СОЕДИНЕНИЕ ВТ_ОграниченияДоп2", "ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ОграниченияДоп2");
	...
	Если Штрихкод <> "" ИЛИ (Список.Количество() > 0 И ВыборGTIN = "1") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПОЛНОЕ СОЕДИНЕНИЕ ВТ_НоменклатураШтрих", "ПОЛНОЕ СОЕДИНЕНИЕ ВТ_НоменклатураШтрих");
	КонецЕсли;
...
Процедура УстановитьСтатусGTINНаСервере(ТолькоНовые = Ложь)
	...
	Для каждого Строка Из ТаблицаОбработки Цикл
		...
		// ++ Галфинд_ДомнышеваКР_25_09_2023
        // необходимо проверять Организацию к заполнению для корректной отработки последующих рег.заданий 
