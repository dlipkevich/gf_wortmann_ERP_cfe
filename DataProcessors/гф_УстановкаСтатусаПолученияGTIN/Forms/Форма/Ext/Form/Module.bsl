...
Процедура ЗаполнитьНаСервере()
	...
	Если Штрихкод <> "" ИЛИ (Список.Количество() > 0 И ВыборGTIN = "1") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПОЛНОЕ СОЕДИНЕНИЕ ВТ_НоменклатураШтрих", "ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_НоменклатураШтрих");
	КонецЕсли;
...
Функция ТекстЗапросаСПТУ()
	Текст = 
		...
		|ИЗ
		|	ВТ_ПТУ КАК ВТ_ПТУ
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ОграниченияДоп2 КАК ВТ_ОграниченияДоп2
		|		ПО ВТ_ПТУ.Номенклатура = ВТ_ОграниченияДоп2.Номенклатура
		|			И ВТ_ПТУ.Характеристика = ВТ_ОграниченияДоп2.Характеристика
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
