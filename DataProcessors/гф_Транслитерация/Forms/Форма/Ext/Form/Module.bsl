...
Процедура Перевести(Команда)
	...
	Для Строка=1 По Источник.КоличествоСтрок() Цикл
		ОбработкаПрерыванияПользователя();
		Текст = Источник.ПолучитьСтроку(Строка);
		Если Текст > "" Тогда
	...
	//Для Индекс = 1 По Источник.КоличествоСтрок() Цикл
	//	ОбработкаПрерыванияПользователя();
	//	
	//    Сообщить(Источник.ПолучитьСтроку(Индекс));
	//КонецЦикла;
	//
	...
	Если ИспользоватьAPI Тогда
		...
		Для каждого строка Из ДляПеревода Цикл
			...
			Результат.ДобавитьСтроку(ПереводОнлайн(строка.Текст, НаправлениеПеревода, ИмяФайла));
	...
	ИначеЕсли ИспользоватьСловарь Тогда
		...
		Для каждого строка Из ДляПеревода Цикл
			...
			Результат.ДобавитьСтроку(Транслитерация(строка.Текст));
		КонецЦикла;	
	Иначе
		Для каждого строка Из ДляПеревода Цикл
			ОбработкаПрерыванияПользователя();
						
			Результат.ДобавитьСтроку(Транслитерация(строка.Текст, Истина));
		КонецЦикла;
	КонецЕсли;
