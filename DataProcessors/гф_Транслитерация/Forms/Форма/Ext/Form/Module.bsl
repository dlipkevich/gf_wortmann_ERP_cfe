...
Процедура Перевести(Команда)
	...
	ГраницаИндикатора = КоличествоСтрок * знчМножитель;
	...
	Для Строка = 1 По КоличествоСтрок Цикл
		...
		процент = Окр(знчПроцент * Индикатор / ГраницаИндикатора, 0);
	...
	ПеревестиТекст(знчПроцент, ГраницаИндикатора, Индикатор);
...
&НаКлиенте
Процедура ПеревестиТекст(знчПроцент, ГраницаИндикатора, Индикатор)
...
Функция ПереводОнлайн(Данные, НаправлениеПеревода, ИмяФайла)
	...
	знчСертификатКлиента = Новый СертификатКлиентаWindows(СпособВыбораСертификатаWindows.Выбирать);
	
	SSL = Новый ЗащищенноеСоединениеOpenSSL(знчСертификатКлиента, Новый СертификатыУдостоверяющихЦентровWindows());
