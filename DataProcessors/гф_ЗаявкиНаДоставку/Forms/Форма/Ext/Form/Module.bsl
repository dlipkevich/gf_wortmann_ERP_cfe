...
Функция СформироватьДокументПР(СтрокиПоТК)
	...
	Для Каждого СтрокаДанных Из СтрокиПоТК Цикл
		...
		Если СтрокаДанных.ЮрФизЛицо = Перечисления.КомпанияЧастноеЛицо.ЧастноеЛицо Тогда
			ОбластьСтрока.Параметры.ADRESAT = СтрокаДанных.КонтрагентПредставление + ";" + СтрокаДанных.КодКлиентаСклад + СтрокаДанных.ДокументОтгрузкиНомер;
		Иначе	
			ОбластьСтрока.Параметры.ADRESAT = СтрокаДанных.Партнер.НаименованиеПолное + ";" + СтрокаДанных.КодКлиентаСклад + СтрокаДанных.ДокументОтгрузкиНомер;
		КонецЕсли;
		// -- Галфинд Волков 04.12.2023
