...
Функция ПолучитьДанныеДляФормированияДокументов()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Склады.гф_КодКлиентаТК КАК КодКлиентаТК,
...
Функция СформироватьДокументПР(СтрокиПоТК)
	...
	Для Каждого СтрокаДанных Из СтрокиПоТК Цикл    
		...
		КодСклада = "";//ПолучитьКодКлиента(СтрокаДанных.Склад);  //код клиента Hellman  
		
		ОбластьСтрока.Параметры.ADDRESSLINE   		 = СтрокаДанных.АдресДоставки;
		ОбластьСтрока.Параметры.ADRESAT       	 	 = СтрокаДанных.Контрагент + ";" + КодСклада + СтрокаДанных.Номер;
		...
		ОбластьСтрока.Параметры.COMMENT      		 = КодСклада + СтрокаДанных.Номер; 
		
		КонтактныеДанные = ПолучитьДанныеКонтактногоЛица(СтрокаДанных.КонтактноеЛицо);
		
		ОбластьСтрока.Параметры.TELADDRESS    		 = КонтактныеДанные.Телефон;
		ОбластьСтрока.Параметры.MAILTYPE      		 = w_Настройки.MAILTYPE.Значение;
		ОбластьСтрока.Параметры.INDEXFROM     		 = w_Настройки.INDEXFROM.Значение;
		ОбластьСтрока.Параметры.VSD           		 = w_Настройки.VSDДляЗаявокВТК.Значение;
		ОбластьСтрока.Параметры.TRANSPORTMODE 		 = w_Настройки.TRANSPORTMODE.Значение; 
		ОбластьСтрока.Параметры.COSTCENTER    		 = СтрокаДанных.КодКлиентаТК;
		ОбластьСтрока.Параметры.PREPOSTALPREPARATION = w_Настройки.PREPOSTALPREPARATION.Значение;
