...
|			ТОГДА СУММА(ЗаказКлиентаТовары.СуммаСНДС)
|		ИНАЧЕ СУММА(ЗаказКлиентаТовары.СуммаСНДС) / СУММА(ЗаказКлиентаТовары.Количество) * Подзапрос.Количество
|	КОНЕЦ КАК Сумма,
...
&НаСервере
Процедура КомандаПередатьЧерезAPIНаСервере() 
	...
	Для Каждого СтрокаТК Из ТаблицаТК Цикл 
		...
		Если СтрокиПоТК.Количество() Тогда
			...
			СформироватьЗаказыПР(СтрокиПоТК); 
...
&НаСервере 
Процедура СформироватьЗаказыПР(СтрокиПоТК)
	
КонецПроцедуры

&НаКлиенте
Процедура КомандаПередатьЧерезAPI(Команда)
	КомандаПередатьЧерезAPIНаСервере();
КонецПроцедуры
