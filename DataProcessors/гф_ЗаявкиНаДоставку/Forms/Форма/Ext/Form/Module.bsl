...
// для теста	
	...
	Элементы.ФормаКомандаСоздатьЗаданияНаПеревозку.Видимость = Ложь;
...
Процедура КомандаСохранить(Команда)
	...
	Элементы.ФормаКомандаСоздатьЗаданияНаПеревозку.Видимость = Истина;
	...
	Для Каждого СтрокаСкладТК Из ТаблицаСкладТК Цикл
		...
		Для Каждого СтрокаДокумента Из СтрокиДокумента Цикл
			...
			Если ПерваяСтрока Тогда
				...
				ОтборДокументов = Новый Структура("Документ", СтрокаДокумента.Документ);
				
				НайденныеДокументы = Объект.ДокументыКОтправке.НайтиСтроки(ОтборДокументов);
				
				Если Не НайденныеДокументы.Количество() Тогда
				...
				Если ЗначениеЗаполнено(НайденныеДокументы[0].ЗаданиеНаПеревозку) Тогда     
					...
					Если НайденныеДокументы[0].СтатусЗаданияНаПеревозку <> 
				...
				Иначе     
					...
					ЗаданиеНаПеревозку.Дата		= ТекущаяДатаСеанса();  
					ЗаданиеНаПеревозку.Статус	= ПредопределенноеЗначение("Перечисление.СтатусыЗаданийНаПеревозку.Формируется");
				...
				ЗаданиеНаПеревозку.Контрагент			= СтрокаДокумента.Контрагент;			
				...
				ЗаданиеНаПеревозку.ТранспортноеСредство	= Автотранспорт;
