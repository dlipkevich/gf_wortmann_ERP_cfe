...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("МассивОрдеров", МассивОрдеров) Тогда
		...
		Для Каждого Ордер Из МассивОрдеров Цикл
			...
			// ++ Галфинд Волков 04.12.2023
			//+++ БФ Бобнев К.С. 09.04.24
			//СтрокаДокумента.ДокументОтгрузки = Ордер.РеализацияТоваровИУслуг;
			Если ТипЗнч(Ордер.РеализацияТоваровИУслуг) = Тип("ДокументСсылка.ВнутреннееПотребление") Тогда
				ДокументОтгрузки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ордер.РеализацияТоваровИУслуг, "ЗаказНаВнутреннееПотребление");
			Иначе	
			    ДокументОтгрузки = Ордер.РеализацияТоваровИУслуг;
			КонецЕсли;
			СтрокаДокумента.ДокументОтгрузки = ДокументОтгрузки;
			//--- БФ Бобнев К.С. 09.04.24
			// -- Галфинд Волков 04.12.2023
...
// Параметры:
// Элемент				- ПолеФормы
// ДанныеВыбора         - СписокЗначений - Описано в синтакс-помощнике
// СтандартнаяОбработка	- Булево
//
&НаКлиенте
...
// Параметры:
// Результат				- Массив
// ДополнительныеПараметры	- Структура - Описано в синтакс-помощнике.
//
&НаКлиенте
...
Процедура СписокДокументовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьФормуПисьма = Поле.Имя = "СписокДокументовПисьмо"
	...
	ИЛИ Поле.Имя = "СписокДокументовТема";
	...
	ИначеЕсли ОткрытьФормуПисьма Тогда
...
Процедура КомандаСоздатьЗаданияНаПеревозкуНаСервере()
	...
	СтатусОтправлено = ПредопределенноеЗначение("Перечисление.СтатусыЗаданийНаПеревозку.Отправлено"); // ГФ_Домнышева
	...
	Для Каждого СтрокаСкладТК Из ТаблицаСкладТК Цикл
		...
		Иначе
			...
			// ЗаданиеНаПеревозку.Статус = СтатусФормируется; // Галфинд_ДомнышеваКР_04_04_2024
			
			ЗаданиеНаПеревозку.ОбменДанными.Загрузка = Истина;
			
		КонецЕсли;
		
		ЗаданиеНаПеревозку.Статус = СтатусОтправлено; // Галфинд_ДомнышеваКР_04_04_2024
		ЗаданиеНаПеревозку.Дата             = ТекущаяДатаСеанса();
		...
		ЗаполнитьМаршрутВЗаданииНаПеревозку(СтрокиДокумента, ЗаданиеНаПеревозку);
		
		ЗаданиеНаПеревозку.ОбменДанными.Загрузка = Ложь;
		
		ЗаданиеНаПеревозку.Записать();
		
		ЗаполнитьЗаданиеВОрдерах(СтрокиДокумента, ЗаданиеНаПеревозку);
...
//
// 	Параметры:
//  СтрокиДокумента		- Массив
//  ЗаданиеНаПеревозку	- ДокументОбъект - ЗаданиеНаПеревозку.
//
&НаСервере
Процедура ЗаполнитьМаршрутВЗаданииНаПеревозку(СтрокиДокумента, ЗаданиеНаПеревозку)
...
//
&НаСервере
Процедура ЗаполнитьЗаданиеВОрдерах(СтрокиДокумента, ЗаданиеНаПеревозку)
...
Процедура УстановитьЗначенияПоУмолчанию()
	...
	БезналичнаяОплата = ПредопределенноеЗначение("Перечисление.ФормыОплаты.Безналичная");
	...
	ЗначенияПоУмолчанию.Вставить("СтранаДляТК",            Россия);
	ЗначенияПоУмолчанию.Вставить("ФормаОплатыУслугиДляТК", БезналичнаяОплата);
	...
	ЗначенияПоУмолчанию.Вставить("АдресЗабораГрузаДляТК",
	"143980, Россия, обл Московская, мкр Жeлезнодорожный, ул Южная, дом 21, стр. 3");
...
//	- Строка - Тест запроса
&НаСервере
Функция ТекстЗапросаДанныеДляФормированияДокументов()
	
	Результат = "ВЫБРАТЬ
	|	ТаблицаДокументов.Документ КАК Документ,
	// ++ Галфинд Волков 04.12.2023
	|	ТаблицаДокументов.ДокументОтгрузки КАК ДокументОтгрузки,
	// -- Галфинд Волков 04.12.2023
	...
	|ПОМЕСТИТЬ ВТ_ТаблицаДокументовКонтрагенты
	|ИЗ
	...
	|ИЗ
	|	ВТ_ТаблицаДокументовКонтрагенты КАК ТаблицаДокументов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.КонтактнаяИнформация КАК КонтрагентыКонтактнаяИнформация
	|		ПО (КонтрагентыКонтактнаяИнформация.Ссылка = ТаблицаДокументов.Контрагент)
	...
	|	СкладыДополнительныеРеквизиты.Свойство КАК Свойство,
	|	СкладыДополнительныеРеквизиты.Значение КАК Значение
	|ПОМЕСТИТЬ ВТ_КодКлиентаСклад
	|ИЗ
	|	Справочник.Склады.ДополнительныеРеквизиты КАК СкладыДополнительныеРеквизиты
	|ГДЕ
	|	СкладыДополнительныеРеквизиты.Свойство.Имя = ""гф_СкладыКодКлиентаСклад""
	|;
	// -- Галфинд Волков 04.12.2023
	...
	// ++ Галфинд Волков 04.12.2023
	|	ВТ_ТаблицаДокументов.ДокументОтгрузки КАК ДокументОтгрузки,
	|	ЕСТЬNULL(ВТ_КодКлиентаСклад.Значение, """") КАК КодКлиентаСклад,
	// -- Галфинд Волков 04.12.2023
	...
	// ++ Галфинд_ДомнышеваКР_05_04_2024
	//|	РасходныйОрдерНаТовары.Получатель КАК Партнер,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ВТ_ТаблицаДокументов.ДокументОтгрузки) = ТИП(Документ.ЗаказНаВнутреннееПотребление)
	|			ТОГДА ВЫРАЗИТЬ(ВТ_ТаблицаДокументов.ДокументОтгрузки КАК Документ.ЗаказНаВнутреннееПотребление).гф_Партнер
	|		ИНАЧЕ РасходныйОрдерНаТовары.Получатель
	|	КОНЕЦ КАК Партнер,
	// -- Галфинд_ДомнышеваКР_05_04_2024 
	|	0 КАК Сумма,
	...
	// ++ Галфинд Волков 04.12.2023
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_КодКлиентаСклад КАК ВТ_КодКлиентаСклад
	|			ПО РасходныйОрдерНаТовары.Склад = ВТ_КодКлиентаСклад.Ссылка
	// -- Галфинд Волков 04.12.2023
	...
	// ++ Галфинд Волков 04.12.2023
	|	ВТ_Номенклатура.ДокументОтгрузки КАК ДокументОтгрузки,
	|	ВТ_Номенклатура.КодКлиентаСклад КАК КодКлиентаСклад,
	// -- Галфинд Волков 04.12.2023
	...
	// ++ Галфинд Волков 04.12.2023
	|	ВТ_Номенклатура.ДокументОтгрузки,
	|	ВТ_Номенклатура.КодКлиентаСклад,
	// -- Галфинд Волков 04.12.2023
	...
	|;
	// ++ Галфинд_Домнышева_19_03_2024
	// по УЛ по парным складам не происходит сворачивания из-за отличающейся номенклатуры
	// в результирующем запросе изменена таблица для выбора Сумм
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_Суммы.Документ КАК Документ,
	|	ВТ_Суммы.УпаковочныйЛист КАК УпаковочныйЛист,
	|	ВТ_Суммы.гф_Заказ КАК гф_Заказ,
	|	СУММА(ВТ_Суммы.Количество) КАК Количество,
	|	СУММА(ВТ_Суммы.Сумма) КАК Сумма,
	|	МАКСИМУМ(ВТ_Суммы.КоличествоВЗаказе) КАК КоличествоВЗаказе
	|ПОМЕСТИТЬ ВТ_СуммыСвернуто
	|ИЗ
	|	ВТ_Суммы КАК ВТ_Суммы
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_Суммы.Документ,
	|	ВТ_Суммы.УпаковочныйЛист,
	|	ВТ_Суммы.гф_Заказ  
	|;
	// -- Галфинд_Домнышева_19_03_2024
	// ++ Галфинд_Домнышева_05_04_2024	
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_УпаковочныеЛисты.Документ КАК Документ,
	|	ЗаказКлиентаДокумент.Ссылка КАК ЗаказКлиента,
	|	ЗаказКлиентаДокумент.гф_АдресДоставки КАК гф_АдресДоставки
	|ПОМЕСТИТЬ ЗАКАЗЫ
	|ИЗ
	|	ВТ_УпаковочныеЛисты КАК ВТ_УпаковочныеЛисты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УпаковочныйЛист КАК УпаковочныйЛистДокумент
	...
	|		ПО ВТ_УпаковочныеЛисты.УпаковочныйЛист = УпаковочныйЛистДокумент.Ссылка
	|ГДЕ
	|	НЕ ЗаказКлиентаДокумент.Ссылка ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВТ_УпаковочныеЛисты.Документ,
	|	ЗаказКлиентаДокумент.Ссылка,
	|	ЗаказКлиентаДокумент.гф_АдресДоставки
	|ИЗ
	|	ВТ_УпаковочныеЛисты КАК ВТ_УпаковочныеЛисты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УпаковочныйЛист КАК УпаковочныйЛистДокумент
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаВнутреннееПотребление КАК ЗаказКлиентаДокумент
	|			ПО УпаковочныйЛистДокумент.гф_Заказ = ЗаказКлиентаДокумент.Ссылка
	|		ПО ВТ_УпаковочныеЛисты.УпаковочныйЛист = УпаковочныйЛистДокумент.Ссылка
	|ГДЕ
	|	НЕ ЗаказКлиентаДокумент.Ссылка ЕСТЬ NULL
	...
	//Добавлено  Галфинд \ Sakovich 08.04.2024
	|Различные
	
	|	ВТ_УпаковочныеЛисты.Документ КАК Документ,
	// ++ Галфинд Волков 04.12.2023
	|	ВТ_УпаковочныеЛисты.ДокументОтгрузки КАК ДокументОтгрузки,
	// ++ Галфинд_Домнышева_05_04_2024
	//|	РеализацияТоваровУслуг.Номер КАК ДокументОтгрузкиНомер,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ВТ_УпаковочныеЛисты.ДокументОтгрузки) = ТИП(Документ.ЗаказНаВнутреннееПотребление)
	//+++ БФ Бобнев К.С. 10.04.24
	//|			ТОГДА ВЫРАЗИТЬ(ВТ_УпаковочныеЛисты.ДокументОтгрузки КАК Документ.ЗаказНаВнутреннееПотребление).Номер
	|			ТОГДА ВЫРАЗИТЬ(ВТ_УпаковочныеЛисты.Документ КАК Документ.РасходныйОрдерНаТовары).Номер
	//--- БФ Бобнев К.С. 10.04.24
	|		ИНАЧЕ РеализацияТоваровУслуг.Номер
	|	КОНЕЦ КАК ДокументОтгрузкиНомер,
	// -- Галфинд_Домнышева_05_04_2024	
	|	ВТ_УпаковочныеЛисты.КодКлиентаСклад КАК КодКлиентаСклад,
	|	ЕСТЬNULL(СправочникАдресаДоставкиРТУ.НомерАдреса, СправочникАдресаДоставки.НомерАдреса) КАК АдресДоставкиНомерАдреса,
	...
	//|	0 КАК Ширина,
	//|	0 КАК Высота,
	|	ЕСТЬNULL(ВариантыКомплектацииНоменклатуры.гф_ДлинаКороба, 0) КАК Длина,
	|	ЕСТЬNULL(ВариантыКомплектацииНоменклатуры.гф_ШиринаКороба, 0) КАК Ширина,
	|	ЕСТЬNULL(ВариантыКомплектацииНоменклатуры.гф_ВысотаКороба, 0) КАК Высота,
	
	//рем  Галфинд \ Sakovich 08.04.2024
	//|	КонтактныеЛицаПартнеров.КонтактнаяИнформация КАК КонтактнаяИнформация, 
	...
	// ++ Галфинд_ДомнышеваКР_05_04_2024
	//|	ЗаказКлиентаДокумент.Ссылка КАК ЗаказКлиента,
	//|	ЕСТЬNULL(РеализацияТоваровУслуг.гф_АдресДоставки, ЗаказКлиентаДокумент.гф_АдресДоставки) КАК АдресДоставкиСсылка,
	|	ЗАКАЗЫ.ЗаказКлиента КАК ЗаказКлиента,
	|	ЕСТЬNULL(РеализацияТоваровУслуг.гф_АдресДоставки, ЗАКАЗЫ.гф_АдресДоставки) КАК АдресДоставкиСсылка,
	// -- Галфинд_ДомнышеваКР_05_04_2024
	|	ЕСТЬNULL(СправочникАдресаДоставкиРТУ.Представление, СправочникАдресаДоставки.Представление) КАК АдресДоставки
	|ИЗ
	|	ВТ_УпаковочныеЛисты КАК ВТ_УпаковочныеЛисты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УпаковочныйЛист КАК УпаковочныйЛистДокумент
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СуммыСвернуто КАК ВТ_Суммы
	|			ПО УпаковочныйЛистДокумент.Ссылка = ВТ_Суммы.УпаковочныйЛист
	// ++ Галфинд_ДомнышеваКР_05_04_2024
	//|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента КАК ЗаказКлиентаДокумент
	//|			ПО УпаковочныйЛистДокумент.гф_Заказ = ЗаказКлиентаДокумент.Ссылка
	|			ЛЕВОЕ СОЕДИНЕНИЕ ЗАКАЗЫ КАК ЗАКАЗЫ
	|			ПО УпаковочныйЛистДокумент.гф_Заказ = ЗАКАЗЫ.ЗаказКлиента
	// ++ Галфинд_ДомнышеваКР_05_04_2024
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.гф_АдресаДоставки КАК СправочникАдресаДоставки
	|			ПО (ЗАКАЗЫ.гф_АдресДоставки = СправочникАдресаДоставки.Ссылка)
	// ++ Галфинд Волков 04.12.2023
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВариантыКомплектацииНоменклатуры КАК ВариантыКомплектацииНоменклатуры
	|			ПО (УпаковочныйЛистДокумент.гф_Комплектация = ВариантыКомплектацииНоменклатуры.Ссылка)
	// -- Галфинд Волков 04.12.2023
	...
	//|		ПО СправочникАдресаДоставки.КонтактноеЛицо = КонтактныеЛицаПартнеров.Ссылка
	
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
	|		ПО (ВТ_УпаковочныеЛисты.Склад = Склады.Ссылка)
	// -- Галфинд Волков 04.12.2023
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
	...
	// Галфинд_ДомнышеваКР_09_04_2024
	|ГДЕ
...
//
// Возвращаемое значение:
//	- ТаблицаЗначений - список документов к отправке.
//
&НаСервере
Функция ПолучитьДанныеДляФормированияДокументов()
	...
	Запрос.Текст = ТекстЗапросаДанныеДляФормированияДокументов();
	
	ТаблицаДокументов = Объект.ДокументыКОтправке.Выгрузить();
...
Процедура ПоместитьДокументыВХранилище()
	...
	Для Каждого СтрокаТК Из ТаблицаТК Цикл
		...
		СформироватьИПоместитьВХранилищеПечатныеФормы(НастройкаТК, СтрокаТК, СтрокиПоТК);
...
// Параметры:
// НастройкаТК	- Массив
// СтрокаТК		- СтрокаТаблицы
// СтрокиПоТК	- Массив
//
&НаСервере
Процедура СформироватьИПоместитьВХранилищеПечатныеФормы(НастройкаТК, СтрокаТК, СтрокиПоТК)
	
	Если СтрокиПоТК.Количество() Тогда
		СоответствиеАдрес_Телефон = ПолучитьНомераТелефонов(СтрокиПоТК);
		Если НастройкаТК.ПР Тогда
			
			ДокументПР = СформироватьДокументПР(СтрокиПоТК, СоответствиеАдрес_Телефон);
		...
		Если НастройкаТК.DPD Тогда
			...
			ДокументDPD = СформироватьДокументDPD(СтрокиПоТК, СоответствиеАдрес_Телефон);
		...
		Если НастройкаТК.Консолидированный Тогда
			...
			ДокументКонсолидированный = СформироватьДокументКонсолидированный(СтрокиПоТК, СоответствиеАдрес_Телефон);
...
//
// Возвращаемое значение:
//	- ТабличныйДокумент - ДокументПР.
//
&НаСервере
Функция СформироватьДокументПР(СтрокиПоТК, СоответствиеАдрес_Телефон)
	...
	Для Каждого СтрокаДанных Из СтрокиПоТК Цикл
		...
		ОбластьСтрока.Параметры.ADRESAT = СтрокаДанных.НаименованиеПолное + ";" + СтрокаДанных.КодКлиентаСклад
			+ СтрокаДанных.ДокументОтгрузкиНомер;
		// -- Галфинд Волков 04.12.2023
		...
		// ++ Галфинд Волков 18.12.2023
		//ОбластьСтрока.Параметры.COMMENT = СтрокаДанных.КодКлиентаТК + СтрокаДанных.Номер;
		ОбластьСтрока.Параметры.COMMENT = СтрокаДанных.КодКлиентаСклад + СтрокаДанных.ДокументОтгрузкиНомер;
		// -- Галфинд Волков 18.12.2023
		...
		ОбластьСтрока.Параметры.TELADDRESS           = СоответствиеАдрес_Телефон[СтрокаДанных.АдресДоставкиСсылка];
...
//
// Возвращаемое значение:
//	- ТабличныйДокумент - ДокументDPD.
//
&НаСервере
Функция СформироватьДокументDPD(СтрокиПоТК, СоответствиеАдрес_Телефон)
	...
	Для Каждого СтрокаДанных Из СтрокиПоТК Цикл
		...
		ОбластьСтрока.Параметры.НомерТТН 			= СтрокаДанных.КодКлиентаСклад + СтрокаДанных.ДокументОтгрузкиНомер;
		// -- Галфинд Волков 04.12.2023
		...
		ОбластьСтрока.Параметры.НомерАдреса 		= СтрокаДанных.АдресДоставкиНомерАдреса;
		ОбластьСтрока.Параметры.НазваниеКлиента		= СтрокаДанных.НаименованиеПолное;
		КоэфДляПересчетаРазмеров = 1000;
		// -- Галфинд Волков 04.12.2023
		ОбластьСтрока.Параметры.АдресКлиента      = СтрокаДанных.АдресДоставки;
		ОбластьСтрока.Параметры.Вес  = СтрокаДанных.Вес;;
		ОбластьСтрока.Параметры.Длина 				= СтрокаДанных.Длина/КоэфДляПересчетаРазмеров;
		ОбластьСтрока.Параметры.Ширина 				= СтрокаДанных.Ширина/КоэфДляПересчетаРазмеров;
		ОбластьСтрока.Параметры.Высота 				= СтрокаДанных.Высота/КоэфДляПересчетаРазмеров;
		// ++ Галфинд Волков 04.12.2023
		// ОбластьСтрока.Параметры.Цена = СтрокаДанных.Сумма;
		ОбластьСтрока.Параметры.Цена = Формат(СтрокаДанных.Сумма, "ЧДЦ=2");
		// -- Галфинд Волков 04.12.2023
		...
		СкладТоварыВКоробах = ПолучитьЗначениеРеквизитаСкладаТоварыВКоробках(СтрокаДанных.Склад);
		КоэфДляВидаНоменклатуры = 1000000000;
		Если СтрокаДанных.ВидНоменклатуры = Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Обувь")
			И СкладТоварыВКоробах Тогда
			ОбластьСтрока.Параметры.Объем = СтрокаДанных.Длина * СтрокаДанных.Ширина * СтрокаДанных.Высота
				/ КоэфДляВидаНоменклатуры;
		Иначе
			ОбластьСтрока.Параметры.Объем = Объект.ОбъемКоробаДляТК;
		КонецЕсли;
		// -- Галфинд Волков 04.12.2023
		...
		ОбластьСтрока.Параметры.Содержимое        = ?(СтрокаДанных.Содержимое = "Accessories",
		"Аксессуары",
		СтрокаДанных.Содержимое);
		ОбластьСтрока.Параметры.ТелефонКонтактногоЛица = СоответствиеАдрес_Телефон[СтрокаДанных.АдресДоставкиСсылка];
		// ++ Галфинд Волков 04.12.2023
		...
		// vvv Галфинд \ Sakovich 08.04.2024
		//Для каждого Строка Из СтрокаДанных.КонтактнаяИнформация Цикл
		Если ЗначениеЗаполнено(СтрокаДанных.КонтактноеЛицо) Тогда
			тзКонтактнаяИнформация = СтрокаДанных.КонтактноеЛицо.КонтактнаяИнформация.Выгрузить();
			Для каждого Строка Из тзКонтактнаяИнформация Цикл
				// ^^^ Галфинд \ Sakovich 08.04.202
				
				Если Строка.Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты Тогда
					Если ЭлектроннаяПочта = "" Тогда
						ЭлектроннаяПочта = ЭлектроннаяПочта + Строка.Представление;
					Иначе
						ЭлектроннаяПочта = ЭлектроннаяПочта + "; " + Строка.Представление;
					КонецЕсли;
				КонецЕсли;
			...
			КонецЦикла;
		КонецЕсли;   //Добавлено  Галфинд \ Sakovich 08.04.2024
		
		ОбластьСтрока.Параметры.ЭлектроннаяПочта = ЭлектроннаяПочта;
		Если СтрокаДанных.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент Тогда
			ОбластьСтрока.Параметры.Опции = "ПРД/ВДО_ДСД";
		Иначе
			ОбластьСтрока.Параметры.Опции = "ПРР_ДСД";
		КонецЕсли;
		// -- Галфинд Волков 04.12.2023
...
//
// Возвращаемое значение:
//	- ТабличныйДокумент - ДокументКонсолидированный.
//
&НаСервере
Функция СформироватьДокументКонсолидированный(СтрокиПоТК, СоответствиеАдрес_Телефон);
	...
	Для Каждого СтрокаДанных Из СтрокиПоТК Цикл
		...
		Если СтрокаДанных.Документ <> Документ Тогда
			...
			Если СтрокаДанных.ВидНоменклатуры = Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Обувь")
				И СкладТоварыВКоробах Тогда
				ОбъемГруза = СтрокаДанных.Длина * СтрокаДанных.Ширина * СтрокаДанных.Высота / КоэфДляВидаНоменклатуры;
			Иначе
				ОбъемГруза = Объект.ОбъемКоробаДляТК;
			КонецЕсли;
		    // -- Галфинд Волков 04.12.2023
			КоличествоМест = 0;
			Сумма          = 0;
			...
			ОбластьСтрока.Параметры.Грузополучатель 	= СтрокаДанных.НаименованиеПолное;
			// -- Галфинд Волков 04.12.2023
			...
			ОбластьСтрока.Параметры.НомерЗаказа 		= СтрокаДанных.КодКлиентаСклад + СтрокаДанных.ДокументОтгрузкиНомер;
			ОбластьСтрока.Параметры.КодАдресаДоставки 	= СтрокаДанных.АдресДоставкиНомерАдреса;
			// -- Галфинд Волков 04.12.2023
			...
			// ++ Галфинд Волков 04.12.2023
			// ОбластьСтрока.Параметры.Страна            = СтрокаДанных.Страна;
			Если ЗначениеЗаполнено(СтрокаДанных.Страна) Тогда
				ОбластьСтрока.Параметры.Страна = СтрокаДанных.Страна;
			Иначе
				ОбластьСтрока.Параметры.Страна = Справочники.СтраныМира.НайтиПоКоду("643");
			КонецЕсли;
			// -- Галфинд Волков 04.12.2023
			ОбластьСтрока.Параметры.ФормаОплатыУслуги = Объект.ФормаОплатыУслугиДляТК;
...
Процедура ВставитьПараметрыВТекстСообщения(Сообщение, МассивСтрок)
	...
	ЮридическийАдрес	= ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "ЮридическийАдрес");
	Телефоны			= ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "Телефоны");
	
	ВставитьПараметрВТекстСообщения(Сообщение, "Адрес офиса", ЮридическийАдрес);
	ВставитьПараметрВТекстСообщения(Сообщение, "Телефон организации", Телефоны);
...
//
// Параметры:
// Сообщение			- Структура - см. ШаблоныСообщений.СформироватьСообщение.
// ИмяПараметра			- Строка
...
Процедура СформироватьПисьмо(МассивСтрок)
	...
	Сообщение = ШаблоныСообщений.СформироватьСообщение(Объект.Шаблон,
	Предмет,
...
//
//	Параметры:
//  ПисьмоСсылка    - ДокументСсылка
//  Сообщение       - ДокументОбъект
//  МассивСтрок		- Массив
//
&НаСервере
Процедура СоздатьИлиЗаполнитьПисьмо(ПисьмоСсылка, Сообщение, МассивСтрок)
	...
	Для Каждого ПолучательПисьма Из Сообщение.Получатель Цикл
		...
		Если СписокПолучателейСпискомЗначений Тогда
			...
			НоваяСтрокаАдрес         = ПолучательПисьма.Значение;
			НоваяСтрокаПредставление = ПолучательПисьма.Представление;
			
		Иначе
			
			НоваяСтрокаАдрес         = ПолучательПисьма.Адрес;
			НоваяСтрокаПредставление = ПолучательПисьма.Представление;
			НоваяСтрокаКонтакт       = ПолучательПисьма.ИсточникКонтактнойИнформации;
	...
	ОбработатьВложения(Письмо, Сообщение, РазмерыВложений, ПисьмоХТМЛ);
...
//
//	Параметры:
//  Письмо          - ДокументОбъект
//  Сообщение       - ДокументОбъект
//  РазмерыВложений	- Соответствие
//  ПисьмоХТМЛ      - Булево
//
&НаСервере
Процедура ОбработатьВложения(Письмо, Сообщение, РазмерыВложений, ПисьмоХТМЛ)
...
//
// Параметры:
// Письмо - ДокументСсылка - Электронное письмо входящее.
//
&НаСервере
...
// Параметры:
//
// ДанныеДокументов	- ТаблицаЗначений
// Ошибки			- Массив
//
&НаСервере
Процедура СформироватьЗаказыПР(ДанныеДокументов, Ошибки)
	...
	Для Каждого СтрокаУчетныхЗаписей Из ТаблицаУчетныхЗаписей Цикл
		...
		Если СтрокиПоУчетнойЗаписи.Количество() Тогда
			...
			ВыгрузитьЗаказыПоАПИ(СтрокиПоУчетнойЗаписи, СтрокаУчетныхЗаписей.УчетнаяЗаписьПР, Ошибки);
...
// Параметры:
//
// СтрокиЗаказов	- ТаблицаЗначений
// УчетнаяЗапись	- СправочникСсылка
// Ошибки			- Массив
//
&НаСервере
Процедура ВыгрузитьЗаказыПоАПИ(СтрокиЗаказов, УчетнаяЗапись, Ошибки = Неопределено)
	...
	Для Каждого СтрокаЗаказа Из СтрокиЗаказов Цикл
		...
		ГраммВКилограмме = 1000;
		
		гф_ПочтаРоссии.ЗаписатьСвойствоИЗначениеВJSON(ЗаписьЖСОН, "postoffice-code", Объект.INDEXFROM);
		гф_ПочтаРоссии.ЗаписатьСвойствоИЗначениеВJSON(ЗаписьЖСОН, "mail-type", "EMS_RT");
		гф_ПочтаРоссии.ЗаписатьСвойствоИЗначениеВJSON(ЗаписьЖСОН, "mail-category", "WITH_DECLARED_VALUE");
		гф_ПочтаРоссии.ЗаписатьСвойствоИЗначениеВJSON(ЗаписьЖСОН, "mail-direct", "643");
		гф_ПочтаРоссии.ЗаписатьСвойствоИЗначениеВJSON(ЗаписьЖСОН, "order-num", СтрокаЗаказа.IDКороба);
		
		гф_ПочтаРоссии.ЗаписатьСвойствоИЗначениеВJSON(ЗаписьЖСОН, "comment", Комментарий);
		гф_ПочтаРоссии.ЗаписатьСвойствоИЗначениеВJSON(ЗаписьЖСОН, "mass", СтрокаЗаказа.Вес * ГраммВКилограмме);
		гф_ПочтаРоссии.ЗаписатьСвойствоИЗначениеВJSON(ЗаписьЖСОН, "insr-value", СтрокаЗаказа.Сумма);
		гф_ПочтаРоссии.ЗаписатьСвойствоИЗначениеВJSON(ЗаписьЖСОН, "given-name", Адресат);
		
		гф_ПочтаРоссии.ЗаписатьАдресПолучателя(ЗаписьЖСОН, НормализованныйАдрес);
		
		гф_ПочтаРоссии.ЗаписатьТелефонПолучателя(ЗаписьЖСОН, НормализованныйТелефон);
		...
		ЧтениеЖСОН.УстановитьСтроку(ОтветОтСервера);
		
		Ответ = ПрочитатьJSON(ЧтениеЖСОН, Истина);
...
Процедура СоздатьПартиюЗаказов(МассивЗаказов, УчетнаяЗапись, Ошибки = Неопределено)
	...
	Для Каждого НовыйЗаказ Из МассивЗаказов Цикл
		...
		ЗаписьЖСОН.ЗаписатьЗначение(НовыйЗаказ.ИдентификаторПР);
	...
	ОбработатьОшибкиВПартии(ОшибкиВПартии, Ошибки);
...
// #wortmann {
// #Заявки на доставку
// Галфинд Окунев 2023/06/27
//
// Параметры:
//
// ОшибкиВПартии	- Массив
// Ошибки			- Массив
//
&НаСервере
Процедура ОбработатьОшибкиВПартии(ОшибкиВПартии, Ошибки)
...
&НаСервере
Функция ПолучитьНомераТелефонов(СтрокиПоТК)
	МассивАдресов = Новый Массив;
	Для Каждого СтрокаДанных Из СтрокиПоТК Цикл
		ТекАдрес = СтрокаДанных.АдресДоставкиСсылка; 
		Если МассивАдресов.Найти(ТекАдрес) = Неопределено Тогда
			МассивАдресов.Добавить(ТекАдрес);
		КонецЕсли;
	КонецЦикла;
	
	СоответствиеАдрес_Телефон = Новый Соответствие;
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	гф_АдресаДоставки.Ссылка КАК Адрес,
	|	гф_АдресаДоставки.КонтактноеЛицо КАК КонтактноеЛицо
	|ПОМЕСТИТЬ Адрес_КЛ
	|ИЗ
	|	Справочник.гф_АдресаДоставки КАК гф_АдресаДоставки
	|ГДЕ
	|	гф_АдресаДоставки.Ссылка В(&МассивАдресов)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	КонтактныеЛицаПартнеровКонтактнаяИнформация.Тип КАК Тип,
	...
	|	КонтактныеЛицаПартнеровКонтактнаяИнформация.ВидДляСписка КАК ВидДляСписка,
	...
	|			(ВЫБРАТЬ
	|				Адрес_КЛ.КонтактноеЛицо КАК КонтактноеЛицо
	|			ИЗ
	|				Адрес_КЛ КАК Адрес_КЛ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Адрес_КЛ.Адрес КАК Адрес,
	|	Адрес_КЛ.КонтактноеЛицо КАК КонтактноеЛицо,
	|	ЕСТЬNULL(КИ_КЛ_моб.Представление, КИ_КЛ_тел.Представление) КАК Телефон
	|ИЗ
	|	Адрес_КЛ КАК Адрес_КЛ
	|		ЛЕВОЕ СОЕДИНЕНИЕ КИ_КЛ КАК КИ_КЛ_моб
	|		ПО Адрес_КЛ.КонтактноеЛицо = КИ_КЛ_моб.КонтактноеЛицо
	|			И (КИ_КЛ_моб.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.МобильныйТелефонКонтактногоЛица))
	|		ЛЕВОЕ СОЕДИНЕНИЕ КИ_КЛ КАК КИ_КЛ_тел
	|		ПО Адрес_КЛ.КонтактноеЛицо = КИ_КЛ_тел.КонтактноеЛицо
	|			И (КИ_КЛ_тел.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.ТелефонКонтактногоЛица))";	
	Запрос.УстановитьПараметр("МассивАдресов", МассивАдресов);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СоответствиеАдрес_Телефон.Вставить(Выборка.Адрес, Выборка.Телефон);
	КонецЦикла;
	Возврат СоответствиеАдрес_Телефон;
КонецФункции // ()
