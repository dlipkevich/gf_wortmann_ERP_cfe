...
Процедура ЗагрузитьДанныеНаСервере(Инвойс, КаталогАрхива, КаталогЛог, МассивОбработанныхФайлов, МассивИмен) 
	...
	Если НайденныйДокумент <> Документы.ПриобретениеТоваровУслуг.ПустаяСсылка() Тогда 
		...
		НоменклатураРоялти = ПланыВидовХарактеристик.гф_ГлобальныеЗначения.гф_ГлобальныеЗначенияНоменклатураДляРоялти.Значение;
...
// Галфинд_Окунев 2023.11.22 {		
		...
		Число(СтрокаИнвойса.License_amount)/?(Число(СтрокаИнвойса.Invoiced_quantity)=0, 1, Число(СтрокаИнвойса.Invoiced_quantity));
// Галфинд_Окунев 2023.11.22 }		
