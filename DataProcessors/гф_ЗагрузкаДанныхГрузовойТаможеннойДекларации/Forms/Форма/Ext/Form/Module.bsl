	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого Вложенный Из ОбъектШтрихкодУТ.ВложенныеШтрихкоды Цикл
			...
			Если СтрокиШтрих.Количество() > 0 Тогда
				гф_НомерГТД = НайтиНомерГТД((СокрП(НомерГТДСсылка.Код) + "/" + СтрокиШтрих[0].Раздел), СтрокиШтрих[0].СтранаПроисхождения, Истина);
			Иначе
				гф_НомерГТД = НомерГТДСсылка;
			КонецЕсли;
...
Функция НайтиНомерГТД(ЗначениеХДТО, СтранаПроисхождения, Родитель = Ложь)
	...
	Если РезультатЗапроса.Пустой() Тогда  
		...
		Если Родитель Тогда
			НовыйНомерГТД.Родитель = НомерГТДСсылка;
		КонецЕсли;
