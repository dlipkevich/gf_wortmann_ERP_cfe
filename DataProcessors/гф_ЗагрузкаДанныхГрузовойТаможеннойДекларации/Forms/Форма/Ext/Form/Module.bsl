...
Процедура СоздатьДокументыНаСервере(стрАдресФайлаОбмена, НомерГТД, стрИмяФайла, ВыводОкна, РежимЗаписи)
	...
	Попытка
		...
		Если Объект.ЗагрузкаОбразцов Тогда
			СоздатьДокументОбразцы(ДанныеЗаполнения, ТаблицаЗначений, РежимЗаписиДокумента.Проведение);
		Иначе
...
&НаСервере
Процедура СоздатьДокументОбразцы(ДанныеЗаполнения, ТаблицаЗначенийДокументы, РежимЗаписи) Экспорт
...
Функция ГТДИмпортПолучитьДанныеШапкиНаСервере(ДанныеЗаполнения)
	...
	НомерГТД = НайтиНомерГТД(ДанныеЗаполнения.Получить("НомерГТД"), ДанныеЗаполнения.Получить("СтранаПроисхождения"), ДанныеЗаполнения.Получить("НомерГТД"));
...
Функция ГТДИмпортПолучитьДанныеРазделыНаСервере(ДанныеЗаполнения)
	...
	Если ЭтоТаблицаЗначений(ТекЗначение) Тогда
		...
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eec5846f2f5bf8
		Если НЕ Объект.ЗагрузкаОбразцов Тогда
...
&НаСервере
Функция ГТДИмпортПолучитьДанныеТоварыОбразцыНаСервере(ДанныеЗаполнения, ДокументПартии, ДанныеРазделы)
	...
	Если ЭтоТаблицаЗначений(ТекЗначение) Тогда
		...
		// ++ Галфинд_ДомнышеваКР_07_02_2024
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eec5846f2f5bf8
		// Заполняем ставкуНДС из данных по разделу
		Для каждого Стр Из ДанныеРазделы Цикл
			Поиск = Новый Структура;
			Поиск.Вставить("НомерРаздела", Стр.НомерРаздела);
			СтрокиТовараРаздела = ТаблицаТоваров.НайтиСтроки(Поиск);	
			Для каждого Строка Из СтрокиТовараРаздела Цикл
	...
	Пока Выборка.Следующий() Цикл 
		...
		Для Каждого Вложенный Из ОбъектШтрихкодУТ.ВложенныеШтрихкоды Цикл
			...
			Если СтрокиШтрих.Количество() > 0 Тогда
				гф_НомерГТД = НайтиНомерГТД((СокрЛП(ОсновнойГТД) + "/" + СтрокиШтрих[0].Раздел), СтрокиШтрих[0].СтранаПроисхождения, ОсновнойГТД, Истина);
				Прервать;
			КонецЕсли;
...
Функция СоздатьДокументГТДИмпортНаСервере(ДанныеЗаполнения, ПоступлениеТоваровУслуг)
	...
	Иначе 
	// Добавила передачу в проц ДанныеРаздела для заполнения в Товарах ставкиНДС Галфинд_ДомнышеваКР_07_02_2024
	ДанныеТовары	= ГТДИмпортПолучитьДанныеТоварыОбразцыНаСервере(ДанныеЗаполнения, ПоступлениеТоваровУслуг, ДанныеРазделы);
	КонецЕсли;
...
&НаСервере
Функция НайтиНомерГТД(ЗначениеХДТО, СтранаПроисхождения, ОсновнойГТД, Родитель = Ложь)
	...
	Если РезультатЗапроса.Пустой() Тогда  
		...
		НомерГТДСсылка = СоздатьНомерГТД(ЗначениеХДТО, СтранаПроисхождения, ОсновнойГТД);
...
Функция СоздатьНомерГТД(НомерГТД, СтранаПроисхождения, ОсновнойГТД)
	...
	ПараметрыДляЗаполнения.РегистрационныйНомер = ОсновнойГТД;
...
Процедура ЗагрузитьНомераГТД(Команда)
	...
	Если Объект.ЗагрузкаОбразцов Тогда
		Сообщение = "Для загрузки КМ снимите флаг ""Загрузка образцов""."; 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
	    Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗагрузкаОбразцовПриИзменении(Элемент)
	Если Объект.ЗагрузкаОбразцов Тогда
		Элементы.ПроверятьКМ.Доступность = Ложь;
		Объект.ПроверятьКМ = ЛОЖЬ; 
	Иначе 
		Элементы.ПроверятьКМ.Доступность = Истина;
	КонецЕсли;
КонецПроцедуры
