...
Функция ВыполнитьОбменДанными(МассивЗагружаемыхФайлов) Экспорт
	...
	Если МассивЗагружаемыхФайлов.Количество()>0 Тогда
		Для каждого ФайлОбмена Из МассивЗагружаемыхФайлов Цикл
			Сообщение = Строка(ТекущаяДата()) + " Начало загрузки файла " + ФайлОбмена.ИмяФайла;
	...
	Для Каждого СозданныйДокумент из СозданныеДокументы Цикл
		...
		Попытка
			...
			ИначеЕсли ДокументДанные.Интерфейс = Перечисления.гф_Интерфейсы.ORDRSP
				или ДокументДанные.Интерфейс = Перечисления.гф_Интерфейсы.INVOICE 
				Или ДокументДанные.Интерфейс = Перечисления.гф_Интерфейсы.IС_INVOICE 
				Или ДокументДанные.Интерфейс = Перечисления.гф_Интерфейсы.SHIPPING_LIST
				Или ДокументДанные.Интерфейс = Перечисления.гф_Интерфейсы.PRICAT_SORT Тогда			
		...
		Исключение	
			...
			"Произошли ошибки при проведении документа " + СозданныйДокумент + ".|" + 
...
Процедура ЗагрузитьФайл(ДанныеДляЗагрузки)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Иначе
				Сообщение = Строка(ТекущаяДата()) + " Не удалось определить тип интерфейса файла " + ДанныеДляЗагрузки.ИмяФайла+". Настройки обмена в табличную часть не загружены!";
			...
			Если ЗначениеЗаполнено(ДокументДанные.Интерфейс)<>"" Тогда
				...
				Запрос.УстановитьПараметр("Интерфейс", ДокументДанные.Интерфейс);
	...
	Исключение
	  ...
	  Сообщение = Строка(ТекущаяДата()) + " Не удалось загрузить файл " + ДанныеДляЗагрузки.ИмяФайла+" - ошибка записи в регистр сведений ""СтрокиДокументаДанныеЗагрузки""!";
	  ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
	...
	Если ДокументДанные.Интерфейс = Перечисления.гф_Интерфейсы.INVOICE Тогда
		...
		Иначе	
			Если НЕ (НаборЗаписей[0].Значение = "380" ИЛИ НаборЗаписей[0].Значение = "325") Тогда
				Сообщение = Строка(ТекущаяДата()) + " Не удалось загрузить файл " + ДанныеДляЗагрузки.ИмяФайла+" - Document_type для инвойса отличен от ""325"" и ""380"" "+СокрЛП(ДокументДанные.Ссылка);
				Возврат;	
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;	
	...
	//Сообщение = Строка(ТекущаяДата()) + " Загружен файл " + ДанныеДляЗагрузки.ИмяФайла;
	 //ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
...
Процедура УпорядочитьПоИнтерфейсам(МассивДокументовДЗ) 
	...
	//Соответствие определяет мэппинг интерфейсов. Интерфейс для ранжировки последовательности проведения (Перечисление.RC_Интерфейс) - Ключ
	//и значения реквизита Интерфейс у документов ДанныеЗагрузки (Перечисление.Интерфейсы)  - Значение
...
Функция ОтсортироватьМассивПоРанжиру(МассивДокументовДЗ, Интерфейсы) 
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекущийИнтерфейсДокументаПоПриоритету	=  ПолучитьЗначениеПеречисленияИнтерфейсыПоИмени(Интерфейсы.Получить(СокрЛП(Выборка.Интерфейс)));
		//На всякий случай, если окажется что для такого интерфейса не было задано соответствие или не нашлось значение перечисления
		Если ТекущийИнтерфейсДокументаПоПриоритету	= Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если Выборка.Интерфейс = Перечисления.гф_Интерфейсы.PRICAT ИЛИ Выборка.Интерфейс = Перечисления.гф_Интерфейсы.PRICAT_SORT Тогда
		...
		Иначе		// Все остальные кроме  PRICAT'ов
			МассивДокументовПоТекущемуИнтерфейсу	= ОтобратьДокументыПоИнтерфейсу(МассивДокументовДЗ, ТекущийИнтерфейсДокументаПоПриоритету);
		КонецЕсли;
