	...
	Для Каждого СозданныйДокумент из СозданныеДокументы Цикл
		...
		Попытка
			...
			Если ДокументДанные.ДополнительныеСвойства.Свойство("ТекстыОшибок") Тогда
				...
				ЗаписатьОшибку(ТекстыОшибок, ДокументДанные);
			КонецЕсли;
		Исключение	
			Если ДокументДанные.ДополнительныеСвойства.Свойство("ТекстыОшибок") Тогда
		    	ТекстыОшибок = ДокументДанные.ДополнительныеСвойства.ТекстыОшибок;
...
Процедура ЗагрузитьФайл(ДанныеДляЗагрузки)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Иначе
				Сообщение = Строка(ТекущаяДатаСеанса()) + " Не удалось определить тип интерфейса файла " + ДанныеДляЗагрузки.ИмяФайла
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Не удалось удалить врмееный файл '"), УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
//
// Параметры:
// ТекстыОшибок - <ТаблицаЗначений> - ТЗ с колонками "Дата,ПутьФайла,Номер,ТекстОшибки" заполняемые в док.гф_ДанныеЗагрузки.   
// ДокументДанные - <ДокументСсылка.гф_ДанныеЗагрузки>.
//
Процедура ЗаписатьОшибку(ТекстыОшибок, ДокументДанные)
	Попытка	
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("Ошибка", УровеньЖурналаРегистрации.Ошибка, ДокументДанные.Ссылка, ДокументДанные.Ссылка, СообщениеОбОшибке + ОписаниеОшибки());		
	КонецПопытки;
