...
//
	...
	Если ОкончаниеСлужебногоСообщения > 0 И НачалоСлужебногоСообщения > 0 Тогда
		ТекстСообщения = Лев(ТекстСообщения, (НачалоСлужебногоСообщения - 1)) +
		                 Сред(ТекстСообщения, (ОкончаниеСлужебногоСообщения + Число2));
	КонецЕсли;
...
//
	...
	ИначеЕсли МассивЧастей.Количество() = Число3 или МассивЧастей.Количество() = Число6 Тогда
		
		Если МассивЧастей[0] >= Число1000 Тогда
			Временно = МассивЧастей[0];
			МассивЧастей[0] = МассивЧастей[2];
			МассивЧастей[2] = Временно;
		КонецЕсли;
		
		Если МассивЧастей[2] < Чиcло100 Тогда
			МассивЧастей[2] = МассивЧастей[2] + ?(МассивЧастей[2] < Число30, 2000,1900);
		КонецЕсли;
...
//
	...
	Если Объект.СсылкаИсточника = Неопределено Тогда
		Возврат;
	КонецЕсли;
	// ++ СадомцевСА 13.03.2024 Для загрузки Заказа клиента т.ч. всегда "Товары в коробах"
	Если ТипЗнч(Объект.СсылкаИсточника) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		СписокВыбора.Добавить("гф_ТоварыВКоробах", "Товары в коробах");
		Объект.ТабличнаяЧастьИсточника = "гф_ТоварыВКоробах";
		Элементы.ТабличнаяЧастьИсточника.Доступность = Ложь;
		Возврат;
	КонецЕсли;
	// -- СадомцевСА 13.03.2024
...
//
	...
	ИначеЕсли Объект.РежимЗагрузки = 1 Тогда
		...
		КонецЕсли; 
	ИначеЕсли Объект.РежимЗагрузки = Число2 Тогда
...
Функция ЗагрузитьДанныеСервер()
	...
	Если Объект.РежимЗагрузки = 1  Тогда
		...
		// -- Галфинд СадомцевСА 26.01.2023
		Если ЗаписыватьОбъект
			И ЗаписатьОбъект(ОбъектИсточника, "", Объект.ПередЗаписьюОбъекта, Объект.ПриЗаписиОбъекта) Тогда
...
//
	...
	ОписаниеОшибки = "";
	Отказ = Ложь;
	// Галфинд СадомцевСА 14.08.2023 Исправил "ошибки" СонарКуба: перенес НачатьТранзакцию()
	// в процедуру ВыполнитьВПопыткеПередЗаписьюОбъекта()
	// НачатьТранзакцию();
...
//
	...
	// ++ Галфинд СадомцевСА 04.08.2023 Исправил "ошибку" СонарКуба
	// СадомцевСА 14.11.2023 Добавил параметр Примечание. Устранил замечания по задаче
	// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee81fb436c865c.
	//Результат = ПодготовитьРезультат(Область, Колонка, ТекущиеДанные, ТекстыЯчеек, Представление);
	Результат = ПодготовитьРезультат(Область, Колонка, ТекущиеДанные, ТекстыЯчеек, Представление, Примечание);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьТаблицуЗагружаемыхРеквизитов();
	...
	//	// СадомцевСА 13.03.2024 Удаляю строки с пустым полем ИмяРеквизита
	...
	//	Для Каждого СтрокаТЗ Из мТаблицаЗагружаемыхРеквизитов Цикл
	//		Если Не ЗначениеЗаполнено(СтрокаТЗ.ИмяРеквизита) Тогда
	//			МассивУдалитьСтроки.Добавить(СтрокаТЗ);
	//		КонецЕсли;
	//	КонецЦикла;
	//	Для Каждого УдалитьСтроку Из МассивУдалитьСтроки Цикл
	//		мТаблицаЗагружаемыхРеквизитов.Удалить(УдалитьСтроку);
	//	КонецЦикла;
	//	ТаблицаЗагружаемыхРеквизитов.Загрузить(мТаблицаЗагружаемыхРеквизитов);
	//КонецЕсли;
	// --
...
&НаСервере
Процедура ЗаполнитьТаблицуЗагружаемыхРеквизитов()
	мТаблицаЗагружаемыхРеквизитов = мВосстановитьЗначение("ТаблицаЗагружаемыхРеквизитов");
	Если мТаблицаЗагружаемыхРеквизитов <> Неопределено Тогда
		// СадомцевСА 13.03.2024 Удаляю строки с пустым полем ИмяРеквизита
		//..ТаблицаЗагружаемыхРеквизитов.Загрузить(мВосстановитьЗначение("ТаблицаЗагружаемыхРеквизитов"));
...
// Копия функции из Формы документа Перемещение товаров. Добавил параметр Объект
	...
	Запрос.Текст = 
		...
		// } #wortmann
		// СадомцевСА 04.12.2023 Заменил строку
...
&НаСервере
Функция ПодготовитьРезультат(Область, Колонка, ТекущиеДанные, ТекстыЯчеек, Представление, Примечание)
	// Галфинд СадомцевСА 07.08.2023 Исправил "ошибки" СонарКуба: добавил Примечание = "";
	//Примечание = "";  // СадомцевСА 14.11.2023 Заремерил строку. Добавил параметр Примечание
	Если Колонка.РежимЗагрузки = "Вычислять" Тогда
...
&НаСервере
Процедура ОчиститьНастройкуНаСервере()
	мСохранитьЗначение("ТаблицаЗагружаемыхРеквизитов", Неопределено);
...
&НаКлиенте
Процедура ОчиститьНастройку(Команда)
	ОчиститьНастройкуНаСервере();
КонецПроцедуры
