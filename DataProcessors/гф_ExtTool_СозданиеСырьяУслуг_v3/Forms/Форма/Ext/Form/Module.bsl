...
Процедура СоздатьУслугиСырьеНаСервере()
	...
	ТекстСообщения = "Начало работы:" + ТекущаяДатаСеанса();
	...
	Пока Строка <= ТабДок.ВысотаТаблицы Цикл
		...
		Если НайденаяНоменклатура = Справочники.Номенклатура.ПустаяСсылка() Тогда
			ТекстСообщения = "Строка:" + Строка + " Не найдена номенклатура с артикулом:" + нОбласть.Текст + " и Сезоном:" +
				ОбластьСезон.Текст;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Строка = Строка + 1;
			Продолжить;
		...
		// В номенклатуре "головного" артикула заполнять значения соответствующих доп.реквизитов "Сырье для производства" и
		// "Услуга для производства" (соответствие по артикулу+сезон(если заполнен)).
	...
	ТекстСообщения = "Окончание работы:" + ТекущаяДатаСеанса();
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	ТекстСообщения = "ВсегоОбработаноСтрок:" + ВсегоОбработаноСтрок;
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция СоздатьНоменклатуру(Строка, ОсобенностьУчета);
	...
	Если ОсобенностьУчета = "Сырье" Тогда
		...
		// Галфинд СадомцевСА 06.12.2023
		// Реализовал заполнение реквизита "Группа финансового учета" при создании Номенклатуры (Сырье/Услуга)
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee92b6f79cef44
		Номенклатура.ГруппаФинансовогоУчета = Объект.сВидНоменклатуры.ГруппаФинансовогоУчета;
	...
	ИначеЕсли ОсобенностьУчета = "Услуга" Тогда
		...
		// Реализовал заполнение реквизита "Группа финансового учета" при создании Номенклатуры (Сырье/Услуга)
		// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811ee92b6f79cef44
		Номенклатура.ГруппаФинансовогоУчета = Объект.уВидНоменклатуры.ГруппаФинансовогоУчета;
	...
	Если Не флТест Тогда
		...
		ТекстСообщения = "Создана новая номенклатура. Артикул: " + Номенклатура.Артикул + " Наименование: " +
...
Процедура ЗаполнитьДопРеквизиты(НайденаяНоменклатура, НайденоеСырье, НайденаяУслуга)
	// В номенклатуре "головного" артикула заполнять значения соответствующих доп.реквизитов "Сырье для производства"
	// и "Услуга для производства" (соответствие по артикулу+сезон(если заполнен)).
	
	ДопРеквизитСырьеДляПроизводства = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту(
		"ИдентификаторДляФормул", "гф_НоменклатураСырьеДляПроизводства");
	ДопРеквизитУслугаДляПроизводства = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту(
		"ИдентификаторДляФормул", "гф_НоменклатураУслугаДляПроизводства");
