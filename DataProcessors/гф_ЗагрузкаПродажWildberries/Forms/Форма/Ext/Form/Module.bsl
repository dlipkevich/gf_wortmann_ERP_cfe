...
&НаСервере
Процедура ВыполнитьЗапросAPIКМНаСервере()
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ВыполнитьЗагрузкуДвиженийКМВБНаСервере(); 
	
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьЗапросAPIКМ(Команда)
	
	Период = Объект.Период;
	
	Если Не ЗначениеЗаполнено(Период.ДатаНачала) Тогда
		Ответ = Неопределено;
		ПоказатьВопрос(Новый ОписаниеОповещения("ВыполнитьЗапросAPIКМЗавершение1", ЭтаФорма, Новый Структура("Период", Период)), "Не указана дата начала получения изменений. Установить дату равную предыдущему дню?", РежимДиалогаВопрос.ДаНет);
        Возврат;
	Иначе
		//Скорректируем введенный период 
		Период.ДатаНачала = НачалоДня(НачалоДня(Период.ДатаНачала)-1);
	КонецЕсли;
	
	ВыполнитьЗапросAPIКМПродолжитьВызов();
	
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьЗапросAPIКМЗавершение1(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ВыполнитьЗапросAPIКМПродолжитьВызов()
	
	Если Не ЗначениеЗаполнено(Объект.Организация) Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ВыполнитьЗапросAPIКМЗавершение", ЭтаФорма), "Не указана организация. Выполнить запрос по всем организациям, для которых есть настройки?", РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;
	
	ВыполнитьЗапросAPIКМФрагмент();

КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьЗапросAPIКМЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ВыполнитьЗапросAPIКМФрагмент()
	
	Если НЕ ОтказВыполнения Тогда
		ВыполнитьЗапросAPIКМНаСервере();
	КонецЕсли;
