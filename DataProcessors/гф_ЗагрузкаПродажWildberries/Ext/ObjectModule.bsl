...
Функция ЗаполнитьНастройкиОбработки(Орг = Неопределено)
	...
	КолонкиТЗ.Добавить("ПолеЦена", ОбщегоНазначения.ОписаниеТипаСтрока(150)); //++ 29.03.2023 Перминов А.С.
	КолонкиТЗ.Добавить("XLS_НомерКолонкиПолеЦена", ОбщегоНазначения.ОписаниеТипаЧисло(4));    //++ 29.03.2023 Перминов А.С.
	...
	Пока Выборка.Следующий() Цикл
		...
		//<< 29.03.2023 Если не удалось заполнить поле "ПолеЦена", то устанавливаем значение по-умолчанию.
		Если Не ЗначениеЗаполнено(Нстрока.ПолеЦена) Тогда
			нСтрока.ПолеЦена = "retail_amount";
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(нСтрока.XLS_НомерКолонкиПолеЦена) Тогда
			нСтрока.XLS_НомерКолонкиПолеЦена = 16;
		КонецЕсли;
		//>>
	КонецЦикла;
...
Процедура ВыполнитьЗапросВБСоздатьДокументы(Настройки, Интервал) Экспорт
	...
	Сервер = ПолучитьАдресСервераСтатистики(); 
	...
	Пока ПродолжитьВыполнениеЗапроса Цикл
		...
		РезультатЗапроса = ВыполнитьHTTPЗапросВБ(Сервер, СтрокаЗапроса, Настройки.key, ЕстьОшибка, ТекстОшибки);
	...
	//<< 29.03.2023 Перминов А.С.
	//тзВхДанные = ОбработатьВходящиеДанные(МассивВходящихДанных);         
	ПолеЦена = Настройки.ПолеЦена;
	тзВхДанные = ОбработатьВходящиеДанные(МассивВходящихДанных, ПолеЦена);
	//>> 29.03.2023 Перминов А.С.
...
Функция ВыполнитьHTTPЗапросВБ(Сервер, Запрос, Ключ, ЕстьОшибка, ТекстОшибки)
	...
	Попытка
		...
		Заголовки.Вставить("Authorization", Ключ);
...
//Функция ОбработатьВходящиеДанные(Массив)          //-- 29.03.2023 Перминов А.С.
Функция ОбработатьВходящиеДанные(Массив, ПолеЦена)  //++ 29.03.2023 Перминов А.С.
	...
	Для каждого элементМассива из Массив Цикл
		...
		ЭтоПродажа = ОпределитьТипДокумента(Основание);
		...
		Если элементМассива.Свойство("quantity") Тогда
			Количество = ?(ЭтоПродажа,элементМассива.quantity,элементМассива.quantity*-1);
		...
		Если элементМассива.Свойство(ПолеЦена) Тогда
			ЦенаПродажи = элементМассива[ПолеЦена];
		//>> 29.03.2023 Перминов А.С. 
...
Процедура ВыполнитьЗагрузкуДвиженийКМВБНаСервере() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Период.ДатаНачала) Тогда
		...
		Период.ДатаНачала = НачалоДня(НачалоДня(ТекущаяДатаСеанса())-1);
	КонецЕсли;
	
	Для Каждого Настройки из тзНастройкиОбработки Цикл

		ОтражатьДвиженияКМ = Ложь;
		
		//Проверка опции "ОтражатьДвиженияКМ" 
		НастройкиЗначения = Настройки.ПредопределенныеЗначения;
		Если НЕ НастройкиЗначения.Свойство("ОтражатьДвиженияКМ") Тогда
			ОбщегоНазначения.СообщитьПользователю("Для органзации "+Настройки.Организация+" не настроена опция ОтражатьДвиженияКМ. Загрузка движений КМ не осуществляется");
		Иначе
			Если ТипЗнч(НастройкиЗначения.ОтражатьДвиженияКМ) <> тип("Булево") Тогда
				ОбщегоНазначения.СообщитьПользователю("Для органзации "+Настройки.Организация+" опция ОтражатьДвиженияКМ имеет неверный тип данных. Загрузка движений КМ не осуществляется");
			Иначе
				ОтражатьДвиженияКМ = НастройкиЗначения.ОтражатьДвиженияКМ;
			КонецЕсли;
		КонецЕсли;
	
		Если ОтражатьДвиженияКМ Тогда
			ВыполнитьЗапросВБСоздатьДокументыДвиженийКМ(Настройки, Период.ДатаНачала);
		КонецЕсли;
...
Процедура ВыполнитьЗапросВБСоздатьДокументыДвиженийКМ(Настройки, ДатаНачала)
	...
	структураПараметрыЗапроса = Новый Структура;
	структураПараметрыЗапроса.Вставить("dateFrom", XMLСтрока(ДатаНачала));
	структураПараметрыЗапроса.Вставить("key", Настройки.key);

	Сервер = ПолучитьАдресСервераСтатистики(); 
	
	Запрос = "api/v1/supplier/excise-goods";
	...
	Параметры = Лев(Параметры, СтрДлина(Параметры)-1);
	СтрокаЗапроса = Запрос+Параметры;
	...
	Если ЕстьОшибка и ЗначениеЗаполнено(ТекстОшибки) Тогда
		ЗаписатьОшибкуВЛогОшибокОбменаСВнешнимиСистемами(Настройки.ВнешняяСистема, Настройки.Организация, "Запрос данных движений КМ WB",ТекстОшибки, СтрокаЗапроса);	
	КонецЕсли;
	...
	тзВхДанныеКМ = ОбработатьВходящиеДанныеКМ(РезультатЧтения, Настройки);
	
	ДанныеДвиженийКМ = ОбработатьДвижениеКодовМаркировки(тзВхДанныеКМ, Настройки, ЕстьОшибка);  // обрабатываем тз движений КМ
	
	СоздатьЗаписиРСДвижениеКодовМаркировки(ДанныеДвиженийКМ, Настройки);
	
КонецПроцедуры

Функция ОбработатьВходящиеДанныеКМ(Массив, Настройки)

	тзВхДанныеКМ = СоздатьТзДляДвиженийКМ();
	
	Для каждого элементМассива из Массив Цикл
		
		Если элементМассива.Свойство("country") Тогда
			Если ЗначениеЗаполнено(элементМассива.country) И Нрег(элементМассива.country) <> Нрег("Россия") Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли; 
		...
		Если элементМассива.Свойство("excise") Тогда
			Сч = Сч + 1;
			Если ЗначениеЗаполнено(элементМассива.excise) Тогда
				КодМаркировкиСтрока = элементМассива.excise;
				мДанныеКМ = СтрРазделить(КодМаркировкиСтрока, Символ(29));
				//Берем только первую часть КМ
				GtinSGTIN = мДанныеКМ[0];
				//Проверка1 на корявые КМки
				Проверка1 = Лев(GtinSGTIN,3);
				Если Проверка1 = "010" Тогда 
					SGTIN = Сред(GtinSGTIN, 19);
					GTIN = Сред(GtinSGTIN, 4,13); 
				Иначе
					SGTIN = Сред(GtinSGTIN, 17);
					GTIN = Сред(GtinSGTIN, 2,13);
				КонецЕсли;
				SGTIN = СтрЗаменить(SGTIN, "\", Символ(34));
				//Проверка2 бывает что GTIN есть, а SGTIN нет вообще, пропускаем
				Если НЕ ЗначениеЗаполнено(SGTIN) Тогда
					ЗаписатьОшибкуВЛогОшибокОбменаСВнешнимиСистемами(Настройки.ВнешняяСистема, Настройки.Организация, "Обработка входящих данных о КМ", "Неверный формат КМ", КодМаркировкиСтрока + " не удалось преобразовать к формату 1с", Сч); 
					Продолжить;
				КонецЕсли;
				//Проверка3 если длина SGTIN короче 13 символов, пропускаем
				Если СтрДлина(SGTIN)<13 Тогда
					ЗаписатьОшибкуВЛогОшибокОбменаСВнешнимиСистемами(Настройки.ВнешняяСистема, Настройки.Организация, "Обработка входящих данных о КМ", "Неверный формат КМ", КодМаркировкиСтрока + " не удалось преобразовать к формату 1с", Сч); 
					Продолжить;
				КонецЕсли;
				//Проверка4 если длина SGTIN длинее 13 символов, то, возможно остался "хвост" его надо найти и обрезать
				Если СтрДлина(SGTIN) > 21 Тогда
					//Ищем разделитель
					Индекс91 = СтрНайти(SGTIN, "91",,14);
					Если Индекс91 > 0 Тогда
						//Обрезаем
						SGTIN = Лев(SGTIN, Индекс91-1);
					Иначе
						//Пробуем обрезать до 20 символов
						SGTIN = Лев(SGTIN, 20);
					КонецЕсли;
				КонецЕсли;
				Штрихкод = GTIN;
				КодМаркировки = "(01)0"+GTIN+"(21)"+SGTIN;
				КМBASE64 = ШтрихкодированиеИСКлиентСервер.ШтрихкодВBase64(КодМаркировки);	
			Иначе
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		нСтрока = тзВхДанныеКМ.Добавить();
		
		Если элементМассива.Свойство("fiscalDt") Тогда
			ДатаОперации = XMLЗначение(Тип("Дата"), элементМассива.fiscalDt);
			нСтрока.ДатаЗаказа = ДатаОперации;
			нСтрока.ДатаОперации = ДатаОперации;
		КонецЕсли;
		Если элементМассива.Свойство("docNumber") Тогда
			НомерЧека = элементМассива.docNumber;
			нСтрока.НомерЗаказа = Формат(элементМассива.docNumber, "ЧГ=0");			
		КонецЕсли;
		Если элементМассива.Свойство("operationTypeId") Тогда
			чОперация = элементМассива.operationTypeId;
			Если чОперация = 1 Тогда
				//Операция = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаПродажаПоОбразцамДистанционнаяПродажа;
				Операция = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаДистанционнаяПродажа	
			Иначе
				Операция = Перечисления.ВидыОперацийИСМП.ВозвратВОборотПриДистанционномСпособеПродажи;
			КонецЕсли;
			нСтрока.Операция = Операция;			
		КонецЕсли;
		нСтрока.КодМаркировкиСтрока = КодМаркировкиСтрока;			
        нСтрока.КодМаркировки = КодМаркировки;
		нСтрока.Штрихкод = Штрихкод;
		нСтрока.КМBASE64 = КМBASE64;
		Если элементМассива.Свойство("finishedPrice") Тогда
			Цена = элементМассива.finishedPrice;
			нСтрока.Цена = Цена;			
		КонецЕсли;
	
	КонецЦикла;
	
	// есть дубли в строках, сворачиваем
	тзВхДанныеКМ.Свернуть("ДатаЗаказа, НомерЗаказа, ДатаОперации, Операция, КодМаркировкиСтрока, КодМаркировки, КМBASE64, Штрихкод, Цена"); 
	
	Возврат тзВхДанныеКМ;
...
Процедура СоздатьДокументыВозврата(Данные, Настройки, Интервал)
	...
	Пока ВыборкаДата.Следующий() Цикл
		...
		ДатаВозврата = ВыборкаДата.НачалоПериода;
		ВыборкаДокумент = ВыборкаДата.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаДокумент.Следующий() Цикл
...
Процедура ОбработатьДанныеНенайденнаяНоменклатура(НенайденнаяНоменклатура, Настройки)
	...
	Пока Выборка.Следующий() Цикл
		...
		//ТЕСТ  
		//Данные = "sa_name:"+Выборка.Артикул+" ts_name:"+Выборка.ХарактеристикаСтрока+" barcode:"+Выборка.Штрихкод;    //--29.03.2023 Перминов А.С.
		Данные = "sa_name:"+Выборка.Артикул+" ts_name:"+Выборка.ХарактеристикаСтрока+" barcode:"+Выборка.Штрихкод+" Дата продажи:"+Выборка.ДатаПродажи;      //++29.03.2023 Перминов А.С.
		ТекстОшибки = "По штрихкоду:"+Выборка.Штрихкод+" не найдена номенклатура. Строка пропущена";
		
		ЗаписатьОшибкуВЛогОшибокОбменаСВнешнимиСистемами(ВнешняяСистема, Организация, Операция, ТекстОшибки, Данные, Сч);
...
Процедура ВыполнитьЗагрузкуДанныхПродажИзФайла(АдресВХ) Экспорт
	...
	//НомКолонкиЦенаПродажи = 20;                              //-- 29.03.2023 Перминов А.С. 
	НомКолонкиЦенаПродажи = Настройки.XLS_НомерКолонкиПолеЦена;   //++ 29.03.2023 Перминов А.С.
	НомКолонкиИндентификаторСтроки = 52;                       //++ 09.10.2023 Перминов А.С.
	...
	Для сч=НомерСтрокиНачалаДанных По ТабДок.ВысотаТаблицы Цикл
        ...
        АдресЯчейки = СтрЗаменить(АдресЯчейки,Символы.НПП,"");
		Количество = ?(ЭтоПродажа,Число(СОКРЛП(ТабДок.Область(АдресЯчейки).Текст)),Число(СОКРЛП(ТабДок.Область(АдресЯчейки).Текст)*-1));
		
		//чтение ЦенаПродажи (тип Число)
		АдресЯчейки = ИмяСтрока + СОКРЛП(сч)+ ИмяКолонкиЦенаПродажи;
        АдресЯчейки = СтрЗаменить(АдресЯчейки,Символы.НПП,"");
		//<< Галфинд, Перминов А.С. 09.10.2023
		Попытка
			ЦенаПродажи = Число(СОКРЛП(ТабДок.Область(АдресЯчейки).Значение));  
		Исключение
			ЦенаПродажи = Число(СтрЗаменить(СтрЗаменить(СОКРЛП(ТабДок.Область(АдресЯчейки).Текст),Символы.НПП,""),".",","));  
		КонецПопытки;
		//>> Галфинд, Перминов А.С. 09.10.2023
...
Функция ОбработатьДвижениеКодовМаркировки(ТЗ, Настройки, ЕстьОшибка = Ложь);
	...
	Пока НенайденныеКМ.Следующий() Цикл
		ТекстСообщения = "КодМаркировки: "+НенайденныеКМ.КодМаркировки+" Операция: "+НенайденныеКМ.Операция+" Дата чека: "+НенайденныеКМ.ДатаЗаказа+", Номер чека: "+НенайденныеКМ.НомерЗаказа;
...
Функция НеобходимоДобавлениеСтроки(Элемент)
	...
	мРазрешенныхОписаний = СтрРазделить("продажа,возврат,сторно продаж,сторно возвратов", ",");
...
Функция ОпределитьТипДокумента(Элемент)

	мРазрешенныхОписаний = СтрРазделить("продажа,сторно возвратов", ",");
...
Функция ПолучитьЗапросПоискНоменклатуры()
	...
	Текст = "ВЫБРАТЬ
	        ...
	        |	ЕСТЬNULL(спрНоменклатура.СтавкаНДС, ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяССылка)) КАК СтавкаНДС
	        ...
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	СУММА(ВТ_НайденныеПродажи.Количество) КАК Количество,
	        ...
	        |			И ВТ_НайденныеПродажи.Количество = ВТ_СвернутыеПоSRID.Количество
	        |
	        |СГРУППИРОВАТЬ ПО
	        |	ВТ_НайденныеПродажи.Характеристика,
	        |	ВТ_НайденныеПродажи.ДатаЗаказа,
	        |	ВТ_НайденныеПродажи.СтавкаНДС,
	        |	ВТ_НайденныеПродажи.ЦенаПродажи,
	        |	ВТ_НайденныеПродажи.ДатаПродажи,
	        |	ВТ_НайденныеПродажи.Основание,
	        |	ВТ_НайденныеПродажи.Количество,
	        |	ВТ_НайденныеПродажи.НачалоПериода,
	        |	ВТ_НайденныеПродажи.ИдентификаторСтроки,
	        |	ВТ_НайденныеПродажи.КонецПериода,
	        |	ВТ_НайденныеПродажи.Артикул,
	        |	ВТ_НайденныеПродажи.Упаковка,
	        |	ВТ_НайденныеПродажи.ТипДокумента,
	        |	ВТ_НайденныеПродажи.Штрихкод,
	        |	ВТ_НайденныеПродажи.Номенклатура,
	        |	ВТ_НайденныеПродажи.ХарактеристикаСтрока
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	        ...
	        |	ВЫБОР
	        |		КОГДА ВТ_ДанныеПродаж.ТипДокумента = ""Продажа""
	        |			ТОГДА ВТ_ДанныеПродаж.Количество
	        |		ИНАЧЕ 0
	        |	КОНЕЦ КАК КоличествоВыданоЗаказ,
	        |	ВЫБОР
	        |		КОГДА ВТ_ДанныеПродаж.ТипДокумента = ""Возврат""
	        |			ТОГДА ВТ_ДанныеПродаж.Количество * -1
	        |		ИНАЧЕ 0
...
Функция ПолучитьДатуЗапрета(Организация, Пользователь) Экспорт
	...
	Если ЗапретВключен Тогда
		...
		Адресаты = ДляПользователей.Адресаты;
		РазделыДляВсехПользователей = Адресаты.Получить(ДляВсехПользователей);
		// 9. Для любого раздела (пустой раздел), любого объекта (объект = раздел) и любого пользователя.
		ПустойРазделВсехПользователей = РазделыДляВсехПользователей.Получить(ПустойРазделСсылка);
		ДатаЗапрета = ПустойРазделВсехПользователей.Получить(ПустойРазделСсылка);
		// 8. Для любого раздела (пустой раздел), любого объекта (объект = раздел) и группы пользователей.
		РазделыДляГруппыПользователя = Адресаты.Получить(ГруппаПользователя);
		Если РазделыДляГруппыПользователя <> Неопределено Тогда
			ПустойРазделДляГруппыПользователя = РазделыДляГруппыПользователя.Получить(ПустойРазделСсылка);
			ДатаЗапрета = ПустойРазделДляГруппыПользователя.Получить(ПустойРазделСсылка);
		КонецЕсли;
		// 7. Для любого раздела (пустой раздел), любого объекта (объект = раздел) и пользователя.
		РазделДляПользователя = Адресаты.Получить(Пользователь);
		Если РазделДляПользователя <> Неопределено Тогда
			ПустойРазделДляПользователя = РазделДляПользователя.Получить(ПустойРазделСсылка);
			ДатаЗапрета = ПустойРазделДляПользователя.Получить(ПустойРазделСсылка);
		КонецЕсли;
		// 6. Для раздела, любого объекта (объект = раздел) и любого пользователя.
		РазделПродажиДляВсехПользователей = РазделыДляВсехПользователей.Получить(ПроверяемыйРаздел);
		Если РазделПродажиДляВсехПользователей <> Неопределено Тогда
			ДатаЗапрета = РазделПродажиДляВсехПользователей.Получить(ПроверяемыйРаздел);
		КонецЕсли;
		// 5. Для раздела, любого объекта (объект = раздел) и группы пользователей.
		Если РазделыДляГруппыПользователя <> Неопределено Тогда
			РазделПродажи = РазделыДляГруппыПользователя.Получить(ПроверяемыйРаздел);
			Если РазделПродажи <> Неопределено Тогда
				ДатаЗапрета = РазделПродажи.Получить(ПроверяемыйРаздел);
			КонецЕсли;
		КонецЕсли;
		//	// 4. Для раздела, любого объекта (объект = раздел) и пользователя.
		Если РазделДляПользователя <> Неопределено Тогда
			РазделПродажи = РазделДляПользователя.Получить(ПроверяемыйРаздел);
			Если РазделПродажи <> Неопределено Тогда
				ДатаЗапрета = РазделПродажи.Получить(ПроверяемыйРаздел);
			КонецЕсли;
		КонецЕсли;
		...
		РазделПродажиДляВсехПользователей = РазделыДляВсехПользователей.Получить(ПроверяемыйРаздел);
		Если РазделПродажиДляВсехПользователей <> Неопределено Тогда
			ДатаПоОбъекту = РазделПродажиДляВсехПользователей.Получить(лОбъект);
			Если ДатаПоОбъекту <> Неопределено Тогда
				ДатаЗапрета = ДатаПоОбъекту;
			КонецЕсли;
		КонецЕсли;
		// 2. Для раздела, объекта и группы пользователей.
		Если РазделыДляГруппыПользователя <> Неопределено Тогда
			РазделПродажи = РазделыДляГруппыПользователя.Получить(ПроверяемыйРаздел);
			Если РазделПродажи <> Неопределено Тогда
				ДатаПоОбъекту = РазделПродажи.Получить(лОбъект);
				Если ДатаПоОбъекту <> Неопределено Тогда
					ДатаЗапрета = ДатаПоОбъекту;
				КонецЕсли;
			КонецЕсли;
...
Функция ПолучитьАдресСервераСтатистики()

	//Возврат "suppliers-stats.wildberries.ru";
