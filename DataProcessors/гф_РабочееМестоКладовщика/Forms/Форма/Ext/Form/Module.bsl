...
Асинх Процедура ПечатьЭтикеткиНаКлиенте(ИмяКоманды, ПисатьВРегистрВремениПечати = Истина)
	...
	Если (СтрНайти(ИмяКоманды, "ПечатьБезNVE") > 0 ИЛИ СтрНайти(ИмяКоманды, "АвтоматическаяПечать") > 0 ) 
		И (Объект.АвтоматическаяПечать ИЛИ РаспечатанныйЭкземплярЕдинственный(NVE_Распечатан, УЛ)) Тогда
		ЗадатьВопросОПовторнойПечати = Ложь;
	КонецЕсли;
	// -- ДомнышеваКР_02_10_2023
	...
	Если ЗадатьВопросОПовторнойПечати Тогда
		...
		Если  Ответ = КодВозвратаДиалога.Нет Тогда
			Объект.NVE = "";
			ДействияПриИзмененииNVE(); 
			ТекущийЭлемент = ?(СтрНайти(ИмяКоманды, "СNVE") > 0, Элементы.NVE, Элементы.Артикул); // ДомнышеваКР_02_10_2023
			Возврат;
