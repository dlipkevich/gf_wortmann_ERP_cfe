...
Процедура ВводGTINПриИзмененииНаСервере()
	...
	Выборка.Следующий();
	стрПоиска = Новый Структура("GTIN", Выборка["GTIN"]);
	...
	Если мСтрок.Количество() > 0 Тогда
		...
		Возврат;
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("В выбранном документе GTIN не найден. Введите другой GTIN."); 
		Возврат;
	КонецЕсли;
