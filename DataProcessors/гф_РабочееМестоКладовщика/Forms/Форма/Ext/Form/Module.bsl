...
Асинх Процедура ПечатьЭтикеткиНаКлиенте(ИмяКоманды, ПисатьВРегистрВремениПечати = Истина)
	...
	Объект.NVE = "";
	ДействияПриИзмененииNVE();
	РезультатУдаления = ВыполнитьДействияБлокировкаАртикуловПоNVE("", "", "Удалить" , Истина);
КонецПроцедуры
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница.Имя = "СтраницаПечатьБезNVE" Тогда
		ПодключитьОбработчикОжидания("УдалитьБлокировкуАртикуловПоПользователю", 60);
	Иначе
		ОтключитьОбработчикОжидания("УдалитьБлокировкуАртикуловПоПользователю");
	КонецЕсли;
...
Процедура ПривестиАтрикулПоМаске()
	// "1-2-34567-89-012/345"  устанавливает "1-2-34567-89-012/345"
	// "123456789012345"       устанавливает "1-2-34567-89-012/345"
	// добавляются знаки "-" и "/" в установленные позиции
...
Функция ВыполнитьДействияБлокировкаАртикуловПоNVE(
	...
	Если Действие = "Найти" Тогда
		Если МенеджерЗаписи.Выбран() Тогда
			СтруктураВозврата = Новый Структура("NVE, Артикул, Пользователь, Дата",
			NVE, Артикул , МенеджерЗаписи["Пользователь"], МенеджерЗаписи["Дата"]);
	...
	Если Действие = "Записать" Тогда
			Если Не(СокрЛП(NVE) = "" ИЛИ СокрЛП(Артикул) = "") Тогда
				МенеджерЗаписи["NVE"] = NVE;
				МенеджерЗаписи["Артикул"] = Артикул;
	...
	Если Действие = "Удалить" И УдалитьВсеЗаписиТекущегоПользователя Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка["Пользователь"] = лПользователь Тогда
				...
				МенеджерЗаписи["Артикул"] = Выборка["Артикул"];
				МенеджерЗаписи.Прочитать();
				Если МенеджерЗаписи.Выбран() Тогда
	...
	Для каждого стрТЗ Из тзВсп Цикл
		Если ЗначениеЗаполнено(стрТЗ["IDКороба"]) Тогда
			...
			NVEНазначен = ЗначенияРеквизитов["гф_NVE_назначен"];
						...
						Если Не (Не NVEРаспечатан И Не NVEНазначен) Тогда
				...
				Если Не NVEНазначен Тогда
					ОбщегоНазначения.СообщитьПользователю("По артикулу <" + Объект.Артикул + "> - NVE: <" +
					NVE + "> - установлен признак ""NVE назначен"". Воспользуйтесь печатью с NVE.");	
				КонецЕсли;
			...
			Иначе	
				...
				// vvv Галфинд \ Sakovich 04.04.2023
				РезультатПоискаБлокировкиПоАртикулу = 
				ВыполнитьДействияБлокировкаАртикуловПоNVE(NVE, Объект.Артикул, "Найти");
				Если ТипЗнч(РезультатПоискаБлокировкиПоАртикулу) = Тип("Структура") Тогда
					ОбщегоНазначения.СообщитьПользователю("Артикул <" + Объект.Артикул + "> - NVE: <" +
					NVE + "> - Обрабатывается пользователем " + 
					РезультатПоискаБлокировкиПоАртикулу["Пользователь"] + " с " + 
					РезультатПоискаБлокировкиПоАртикулу["Дата"]);
					Продолжить;
				Иначе
					РезультатЗаписи = ВыполнитьДействияБлокировкаАртикуловПоNVE(NVE, Объект.Артикул, "Записать");
				КонецЕсли;
				// ^^^ Галфинд \ Sakovich 04.04.2023 
...
КонецПроцедуры 
 ...
 &НаКлиенте
 Процедура  УдалитьБлокировкуАртикуловПоПользователю() Экспорт
	Если СокрЛП(Объект.Артикул) = "" Тогда
		РезультатУдаления = ВыполнитьДействияБлокировкаАртикуловПоNVE("", "", "Удалить" , Истина);
	КонецЕсли;
 КонецПроцедуры
