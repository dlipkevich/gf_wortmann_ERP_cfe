	...
	Для каждого стрТЗ Из тз Цикл
		Если стрТЗ["ТипДопСведения"] = "Реквизит" Тогда 
			...
			Иначе
				...
				Если стрТЗ["РеквизитИлиСвойство"] = "Артикул " Тогда
...
Процедура ДействияПриИзмененииNVE(ОчиститьТовары = Истина)
	...
	ИначеЕсли ПересортицаИСтатус["Статус"] = Перечисления.гф_СтатусыИсключений.ЭтикеткиНаклеены Тогда
		...
		Набор.Прочитать();
		КромеКМ = Набор.ВыгрузитьКолонку("ШтрихкодУпаковки");
		ЗаполнитьТаблицуТоваров(СтруктураШтрихкодУпаковочныйЛист, ,КромеКМ);
...
Функция ОпределитьЧтоПоУлЕстьПересортицаИКодыПолучены(Агрегат)
	...
	Иначе
		...
		Выборка.Следующий();
		СтруктураВозврата = Новый Структура("Пересортица, Статус", Выборка["Пересортица"], Выборка["Статус"]);
...
Асинх Процедура ПечатьЭтикеткиНаКлиенте(ИмяКоманды, ПисатьВРегистрВремениПечати = Истина)
	...
	Если ИмяКоманды = "ПечатьСNVE" Тогда
		СтруктураУЛ_ШК = НайтиШтрикходУпаковкиТоваровИУпаковочныйЛистПоNVE(Объект.NVE);
		ПересортицаСтатус = ОпределитьЧтоПоУлЕстьПересортицаИКодыПолучены(СтруктураУЛ_ШК["ШтрихКодУпаковки"]);
