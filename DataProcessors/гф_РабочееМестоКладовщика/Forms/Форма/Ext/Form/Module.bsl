...
&НаСервере
Процедура ПечатьЭтикетки(МассивКМДляПечати = Неопределено)
...
Асинх Процедура ПечатьЭтикеткиНаКлиенте(ИмяКоманды, ПисатьВРегистрВремениПечати = Истина)
	...
	ПечатьЭтикетки();
	Если ИмяКоманды = "ПечататьДобавленныеКМ" Тогда
...
Процедура АртикулПриИзмененииНаСервере()
	...
	Для каждого стрТЗ Из тзВсп Цикл
		Если ЗначениеЗаполнено(стрТЗ["IDКороба"]) Тогда
			NVEРаспечатан = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(стрТЗ["IDКороба"], "гф_NVE_распечатан");
			NVE = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(стрТЗ["IDКороба"], "Код");
	
			Если  Не NVEРаспечатан Тогда
				// Объект.Товары.Очистить();
				Объект.NVE = NVE;
