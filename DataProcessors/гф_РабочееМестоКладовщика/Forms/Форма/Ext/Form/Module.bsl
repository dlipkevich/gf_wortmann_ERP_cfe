...
Процедура ПечатьСNVE(Команда)
	...
	ТекущийЭлемент = Элементы.NVE;//Галфинд_ДомнышеваКР_10_01_2024
КонецПроцедуры
...
Процедура ПечатьЭтикетки(МассивКМДляПечати = Неопределено, ИмяКоманды = Неопределено)
	...
	ОтмеченныеСтроки = Объект.Товары.НайтиСтроки(Новый Структура("Пометка", Истина));
	тзТовары = Объект.Товары.Выгрузить(ОтмеченныеСтроки);
	...
	// Галфинд \ Shtak  2024/01/31  =>
	// Делаем Группировку по коробам
	ДанныеМакета = Новый Структура;
	ДанныеМакета.Вставить("ДеревоТовары", ДеревоТовары);
	ДанныеМакета.Вставить("КоличествоКолонок", КоличествоКолонок);
	ДанныеМакета.Вставить("ЭтикетокНаЛисте", ЭтикетокНаЛисте);
	ДанныеМакета.Вставить("КоличествоСтрок", КоличествоСтрок);
	//ДанныеМакета.Вставить("ИндексЭтикетки", ИндексЭтикетки);
	//ДанныеМакета.Вставить("ВсегоНужноНапечататьЭтикеток", ВсегоНужноНапечататьЭтикеток);
	
	Если Не РежимПечатиНекоробноеРТУ Тогда   //Добавлено  Галфинд \ Sakovich 05.04.2024
		Если ГруппироватьПоNVE Тогда
			ТабNVE = тзТовары.Скопировать();
			ТабNVE.Свернуть("NVE");
			Для каждого строкаNVE Из ТабNVE Цикл
				СтрокиПоNVE = тзТовары.НайтиСтроки(Новый Структура("NVE", строкаNVE.NVE));
				ТабБлока = тзТовары.Скопировать(СтрокиПоNVE);
				ПараметрыЗаголовка = ПараметрыЗаголовкаПечатьNVE(СтрокиПоNVE[0]);
				ВывестиБлокNVE(ТД, Макет, ПараметрыЗаголовка, ТабБлока, ДанныеМакета);
			КонецЦикла;
		Иначе	
			ВывестиБлокNVE(ТД, Макет, ПараметрыЗаголовка, тзТовары, ДанныеМакета);
		КонецЕсли;
	Иначе
		// vvv Галфинд \ Sakovich 05.04.2024
		табАртикул = тзТовары.Скопировать();
		табАртикул.Свернуть("Артикул");
		Для каждого стрАртикул Из табАртикул Цикл
			СтрокиПоАртикулу = тзТовары.НайтиСтроки(Новый Структура("Артикул", стрАртикул.Артикул));
			табБлока = тзТовары.Скопировать(СтрокиПоАртикулу);
			ПараметрыЗаголовка = ПараметрыЗаголовкаПечатьNVE(СтрокиПоАртикулу[0]);
			ПараметрыЗаголовка["АртикулУпаковки"] = СтрокиПоАртикулу[0]["Артикул"];
			ВывестиБлокNVE(ТД, Макет, ПараметрыЗаголовка, табБлока, ДанныеМакета);
		КонецЦикла;
		// vvv Галфинд \ Sakovich 05.04.2024
	КонецЕсли;
	...
	Если Не(ИмяКоманды = "ПечатьБезNVEСПредварительнымПросмотром" 
		...
		Если Не РежимПечатиНекоробноеРТУ Тогда // Добавлено условие Галфинд \ Sakovich 05.04.2024
...
&НаСервере
Процедура ВывестиБлокNVE(ТД, Макет, ПараметрыЗаголовка, тзТовары, ДанныеМакета)
	ДеревоТовары = ДанныеМакета.ДеревоТовары;
	КоличествоКолонок = ДанныеМакета.КоличествоКолонок;
	ЭтикетокНаЛисте = ДанныеМакета.ЭтикетокНаЛисте;
	КоличествоСтрок = ДанныеМакета.КоличествоСтрок;
	//ВсегоНужноНапечататьЭтикеток = ДанныеМакета.ВсегоНужноНапечататьЭтикеток;
	ВсегоНужноНапечататьЭтикеток = тзТовары.Итог("КоличествоЭтикеток");
	...
	Для Каждого строка Из тзТовары Цикл
		...
		Если Нашли = Неопределено Тогда
			...
			//Продолжить; //Галфинд_ДомнышеваКР_10_01_2024_сообщение выводим но все равно печатаем
			Продолжить; //Вернул  Галфинд \ Sakovich 10.01.2024 13:44:46
		Иначе
			таб = Нашли.Строки;
		КонецЕсли;
		...
		Для а = 1 По строка.КоличествоЭтикеток Цикл
			...
			Если ИндексЭтикетки = ЭтикетокНаЛисте или ИндексСтроки > КоличествоСтрок Тогда
				...
				Иначе
					ПечатьПодвалаНекоробной(ТД, Макет, ПараметрыЗаголовка, ИндексСтроки - 1);
				КонецЕсли;
				ВывелиПодвал = Истина;
	...
	Если НЕ ВывелиПодвал Тогда
		...
		Иначе
			ПечатьПодвалаНекоробной(ТД, Макет, ПараметрыЗаголовка);
		КонецЕсли;
		ВывелиПодвал = Истина;
	КонецЕсли;	
КонецПроцедуры
...
Процедура ДействияПриИзмененииNVE(ОчиститьТовары = Истина)
	...
	Если НЕ НеУчитыватьСклад Тогда // ДомнышеваКР_29_01_2024
	...
	КонецЕсли; // ДомнышеваКР_29_01_2024
	...
	ИначеЕсли ПересортицаИСтатус["Статус"] = Перечисления.гф_СтатусыИсключений.ЭтикеткиНаклеены Тогда
		...
		// vvv Галфинд \ Sakovich 13.01.2024
...
Функция ТекстЗапросаПоДобавленнымКМ()
	...
	// в пуле могут не содержаться данные из-за действий пользователя
	|ГДЕ
...
Процедура NVEПриИзменении(Элемент)
	...
	Если Объект.АвтоматическаяПечать Тогда
		...
		// -- Галфинд_Домнышева_10_10_2023
	КонецЕсли;
	ПодключитьОбработчикОжидания("ВернутьКурсорНаПолеВвода", 0.3, Истина); // Галфинд_Домнышева_11_01_2024
...
Функция ТолькоЦифры(Строка)
    ...
    РазрешенныеСимволы = "0123456789AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz";
    Длинна = СтрДлина(Строка);
    Итог = "";
    Для Инд = 1 По Длинна Цикл
        Символ = Сред(Строка,Инд,1);
		Если СтрНайти(РазрешенныеСимволы, Символ) Тогда
			 Итог = Итог + Символ;
		КонецЕсли;
    КонецЦикла;
...
Процедура ПривестиАтрикулПоМаске()
	...
	// ++ Галфинд_ДомнышеваКР_11_01_2024 Добавила Маску для Артикула вида
	// "1/23456/78 901 234"
	// ++ Галфинд_ДомнышеваКР_11_01_2024
	
	// ++ Галфинд_ДомнышеваКР_12_01_2024 
	// В артикул добавляются латинские символы, в связи с чем преобразуем Артикул к виду цифры-буквы, 
	// потом расставляем знаки через маску // 123456789AAA345 и 12345678AAA345
	Объект.Артикул = ТолькоЦифры(Объект.Артикул);//  Галфинд_ДомнышеваКР_12_01_2024
	// -- Галфинд_ДомнышеваКР_12_01_2024
	
	// vvv Галфинд \ Sakovich 26.01.2024
	// Добавлена  маска для вида: 1-23732-41-8A0/200  
	// идет в параметр запроса: 123732418A0200 (9 цифр, 1 латинск. буква, 4 цифры)
	// ^^^ Галфинд \ Sakovich 26.01.2024
	...
	|	(ВЫРАЗИТЬ(ЛЕВ(СОКРЛП(&П), 1) КАК СТРОКА(1))) + ""-"" +   
	|   (ВЫРАЗИТЬ(ПРАВ(ЛЕВ(СОКРЛП(&П), 7), 5) КАК СТРОКА(5))) + ""-"" +
	|   (ВЫРАЗИТЬ(ПРАВ(ЛЕВ(СОКРЛП(&П), 10), 2) КАК СТРОКА(2))) + ""-"" +
	|   (ВЫРАЗИТЬ(ПРАВ(ЛЕВ(СОКРЛП(&П), 14), 3) КАК СТРОКА(3))) + ""/"" +
	|   (ВЫРАЗИТЬ(ПРАВ(ЛЕВ(СОКРЛП(&П), 18), 3) КАК СТРОКА(3))),
	|	ИСТИНА
	|ГДЕ
	|	СОКРЛП(&П) ПОДОБНО ""[0-9]/[0-9][0-9][0-9][0-9][0-9]/[0-9][0-9] [0-9][0-9][0-9] [0-9][0-9][0-9]""
	// -- Галфинд_ДомнышеваКР_11_01_2024
	...
	|	ИСТИНА
	|ГДЕ
	|	СОКРЛП(&П) ПОДОБНО ""[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][A-Z][A-Z][A-Z][0-9][0-9][0-9]""
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ИСТИНА
	|ГДЕ
	|	СОКРЛП(&П) ПОДОБНО ""[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][A-Z][A-Z][A-Z][0-9][0-9][0-9]""
	// -- Галфинд_ДомнышеваКР_12_01_2024
	
	// vvv Галфинд \ Sakovich 26.01.2024  маска для вида: 1-23732-41-8A0/200   (9 цифр 1 буква 4 цифры) 123732418A0200
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ИСТИНА
	|ГДЕ
	|	СОКРЛП(&П) ПОДОБНО ""[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][A-Z][0-9][0-9][0-9][0-9]""
	// ^^^ Галфинд \ Sakovich 26.01.2024
	|";
...
Процедура ГотовоУдалить(Команда)
	...
	ШрифтСообщения = ПолучитьШрифтСообщения();
	ФорматированноеСообщение = Новый ФорматированнаяСтрока(Текст, ШрифтСообщения);
...
&НаСервереБезКонтекста
Функция ПолучитьШрифтСообщения()
	ШрифтСообщения = Метаданные.ЭлементыСтиля.гф_ШрифтОбычныйШрифт24Полужирный.Значение;
...
Процедура ВопросЕстьНемаркированныйGTINЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ВопросЕстьНемаркированныйGTINЗавершениеНаСервере();   
		//}
...
//{       
&НаСервере
Процедура ВопросЕстьНемаркированныйGTINЗавершениеНаСервере() 
	...
	Попытка
		ЗанестиВРС_ОбработкаИсключений(Объект.NVE,
		Перечисления.гф_СтатусыИсключений.ИсключениеСоздано,
		ТекДата);
		сч=0;
		Для каждого стрКоллекции Из Объект.ТЧ_Убрать Цикл
			сч = сч + 1;
		...
		ТекДата = ТекущаяДата() + 1;
		ЗанестиВРС_ОбработкаИсключений(Объект.NVE,
		Перечисления.гф_СтатусыИсключений.ЭтикеткиНаклеены,
		ТекДата);	
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию(); 
		Описание = ОписаниеОшибки();
		ОбщегоНазначения.СообщитьПользователю(Описание);                            
		Возврат;
	КонецПопытки;                                         
...
Процедура ГотовоДобавить(Команда)
	...
	Если НужноОбработатьСерыеМетки Тогда
		...
		// Шук 13.02.2024 БФ
		//{      
		// Перенесено в ВопросПредупреждениеДобавлениеGTINЗавершение
		...
		// Шук 13.02.2024 БФ
		//{
		ДопПараметры.Вставить("ОбработатьСерыеМетки", Истина); 
		ДопПараметры.Вставить("РезультатПроверки", РезультатПроверки);
		//}
...
Процедура ВопросПредупреждениеДобавлениеGTINЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		//Если  ДополнительныеПараметры.Свойство("Сопоставленные") Тогда	
		//	СтруктураЭмиссии = НовыйДокументЭмиссииНаСервере(ДокументПересортица, ДополнительныеПараметры);
		//Иначе
		//// -- Галфинд_ДомнышеваКР_24_10_2023
		//СтруктураЭмиссии = НовыйДокументЭмиссииНаСервере(ДокументПересортица);
		...
		//КонецЕсли;
		//// ^^^ Галфинд \ Sakovich 14.04.2023     
		ВопросПредупреждениеДобавлениеGTINЗавершениеНаСервере(ДополнительныеПараметры);
		//}
...
// Шук 13.02.2024 {
&НаСервере
Процедура ВопросПредупреждениеДобавлениеGTINЗавершениеНаСервере(ДополнительныеПараметры) 
	
	НачатьТранзакцию();
	
	Попытка
	
		Если ДополнительныеПараметры.Свойство("ОбработатьСерыеМетки") И ДополнительныеПараметры.ОбработатьСерыеМетки Тогда		
			ОбработатьСерыеМетки(ДополнительныеПараметры.РезультатПроверки);				
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтруктураЭмиссии["ЗаказНаЭмиссиюКодовМаркировкиСУЗ"]) 
			...
			ЗанестиВРС_ОбработкаИсключений(пNVE, ПредопределенноеЗначение("Перечисление.гф_СтатусыИсключений.ИсключениеСоздано"));
			ЗанестиВРС_ПересортицаУдаляемыеШтрихкоды(ДокументПересортица);
			// ++ Галфинд СадомцевСА 15.03.2024
			// Записываем новый Заказ на эмиссию КМ в очередь сообщений (рабочее место кладовщика)
			// e1cib/data/Задача.ЗадачаИсполнителя?ref=8eabb083fed1320811eedbbea5bc7e0c
			МассивДокументов = Новый Массив;
			МассивДокументов.Добавить(СтруктураЭмиссии["ЗаказНаЭмиссиюКодовМаркировкиСУЗ"]);
			гф_ЭмиссияКодовМаркировкиВызовСервера.гф_ПодготовитьКПередачеИСМП(МассивДокументов, Истина, Ложь);
			// -- Галфинд СадомцевСА
		КонецЕсли;
...
Процедура УбратьТЧ_СканированиеКМПриИзменении(Элемент)
	...
	// Шук 13.02.2024 БФ
	//{
	Если РезультатСканирования = 5 Тогда
		ПоказатьПредупреждение(, "Данный КМ: <" + УбратьТЧ_СканированиеКМ + "> - уже обрабатывался ранее", 10, "Ошибка ввода!");
		УбратьТЧ_СканированиеКМ = "";	
		ЭтотОбъект.ТекущийЭлемент = Элементы.УбратьТЧ_СканированиеКМ;
		Возврат;
	КонецЕсли;  	
...
Функция УбратьТЧ_СканированиеКМПриИзмененииНаСервере()
	...
	//Шук 13.02.2024 БФ
	//{
	КонтрольДублей = БФД_ОбщегоНазначенияСервер.ЗначениеДополнительнойНастройки("КонтрольДублейПриУдаленииКМ");
	Если КонтрольДублей <> Неопределено И КонтрольДублей.Значение Тогда
		ЗапросПроверкиДублей = Новый Запрос;
		ЗапросПроверкиДублей.Текст = "
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ВЫБОР
		|			КОГДА ТИПЗНАЧЕНИЯ(гф_ПересортицаУдаляемыеШК.ШтрихкодУпаковки) = ТИП(СТРОКА)
		|				ТОГДА гф_ПересортицаУдаляемыеШК.ШтрихкодУпаковки = &ШтрихкодУпаковкиСтрокой
		|			ИНАЧЕ гф_ПересортицаУдаляемыеШК.ШтрихкодУпаковки = &ШтрихкодУпаковки
		|		КОНЕЦ
		|	И ВЫБОР
		|			КОГДА гф_ПересортицаУдаляемыеШК.Пересортица ССЫЛКА Документ.ПересортицаТоваров
		|				ТОГДА ВЫРАЗИТЬ(гф_ПересортицаУдаляемыеШК.Пересортица КАК Документ.ПересортицаТоваров).гф_IDКороба = &УпаковочныйЛист
		|			ИНАЧЕ гф_ПересортицаУдаляемыеШК.Пересортица = &УпаковочныйЛист
		|		КОНЕЦ
		|	И НЕ гф_ПересортицаУдаляемыеШК.Пересортица.ПометкаУдаления
		|";
		ЗапросПроверкиДублей.УстановитьПараметр("УпаковочныйЛист", СтруктураШК_УЛ.УпаковочныйЛист);
		ЗапросПроверкиДублей.УстановитьПараметр("ШтрихкодУпаковки", Выборка["КМ"]);
		ЗапросПроверкиДублей.УстановитьПараметр("ШтрихкодУпаковкиСтрокой", УбратьТЧ_СканированиеКМ);		
		РезультатПроверки = ЗапросПроверкиДублей.Выполнить();	
		
		Если Не РезультатПроверки.Пустой() Тогда
			Возврат 5;	// Код был обработан ранее	
		КонецЕсли;
...
Процедура ПечатьЗаголовка(ТД, Макет, ПараметрыЗаголовка)
	Если Не РежимПечатиНекоробноеРТУ Тогда // Добавлено условие Галфинд \ Sakovich 05.04.2024
		...
		// vvv Галфинд \ Sakovich 05.04.2024
	Иначе
		Область = Макет.ПолучитьОбласть("НомерСерииНеКоробной");
		Область.Параметры.Артикул = ПараметрыЗаголовка["АртикулУпаковки"];
		// ^^^ Галфинд \ Sakovich 05.04.2024
	КонецЕсли;
	ТД.Вывести(Область);
КонецПроцедуры
...
Процедура ПечатьПодвалаНекоробной(ТД, Макет, ПараметрыЗаголовка, ЧислоЛинийQR = 2)
	ПустаяСтрока = Макет.ПолучитьОбласть("ПустаяСтрока");
	ТестоваяСтрока = Макет.ПолучитьОбласть("ТестоваяСтрока");
	
	Область = Макет.ПолучитьОбласть("НомерСерииНеКоробнойПодвал");
	Область.Параметры.Артикул = ПараметрыЗаголовка["АртикулУпаковки"];
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПечатьИзРТУ") Тогда
		ГруппироватьПоNVE = Истина; // Галфинд \ Shtak  2024/01/31  =>
		Для Каждого ЭлементСтраница Из Элементы.ГруппаСтраницы.ПодчиненныеЭлементы Цикл
			
			Если ЭлементСтраница.Имя = "СтраницаПечатьИзРТУ" Тогда
		...
		СсылкаНаРТУ = Параметры["СсылкаНаРТУ"];
		// vvv Галфинд \ Sakovich 05.04.2024
		//ЗаполнитьТаблицуТоваровИзРТУ();
		Склад = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры["СсылкаНаРТУ"], "Склад");
		ЭтоКоробнойСклад = УправлениеСвойствами.ЗначениеСвойства(Склад, "гф_СкладыТоварыВКоробах") = Истина;
		Если Не ЭтоКоробнойСклад Тогда
			Элементы.ГруппироватьПоNVE.Видимость = Ложь;
			Элементы.Товары1NVE1.Видимость = Ложь;
			Элементы.Товары1АртикулУпаковки1.Видимость = Ложь;
			
			РежимПечатиНекоробноеРТУ = Истина;
			ЗаполнитьТаблицуТоваровИзРТУНекоробное();
		Иначе
			ЗаполнитьТаблицуТоваровИзРТУ();
		КонецЕсли;	
		// ^^^ Галфинд \ Sakovich 05.04.2024
		Заголовок = "Печать этикеток из РТУ"; 
...
Процедура ПриОткрытии(Отказ)
	...
	// -- Галфинд ВолковЕВ 22.06.2023
	ДоступностьНеУчитыватьСклад();// ДомнышеваКР_29_01_2024
	
КонецПроцедуры

// #wortmann { 
// Переопределяет Видимость/Доступность для НеУчитыватьСклад 
// Галфинд_Домнышева 2024/01/29
&НаСервере
Процедура  ДоступностьНеУчитыватьСклад()
	
	НеУчитыватьСклад = Ложь;
	Если РольДоступна("ПолныеПрава") Или РольДоступна("гф_ПечатьЭтикетокБезУчетаСклада") Тогда
		
		Элементы.НеУчитыватьСклад.Доступность = Истина;
		Элементы.НеУчитыватьСклад.Видимость = Истина;
...
Процедура ПечатьЭтикеткиПеремаркировки(МассивКМДляПечати = Неопределено, ИмяКоманды = Неопределено)
	...
	Для Каждого строка Из тзТовары Цикл
		...
		Если Нашли = Неопределено Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(Текст);
			//Продолжить; //Галфинд_ДомнышеваКР_10_01_2024_сообщение выводим но все равно печатаем
			Продолжить; // Вернул Галфинд \ Sakovich 10.01.2024 13:46:35
		Иначе
			таб = Нашли.Строки;
		КонецЕсли;
...
// #wortmann {
// Обработчик ожидания возвращает курсор к NVE если значение не заполнено
// Галфинд_Домнышева 2024/01/11
&НаКлиенте
Процедура ВернутьКурсорНаПолеВвода() Экспорт
	
	Если Объект.NVE = "" Тогда
		ТекущийЭлемент = Элементы.NVE;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьТаблицуТоваровИзРТУ()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ Разрешенные
	|	ДокУпаковочныйЛист.Ссылка КАК УпаковочныйЛист,
	|	ДокУпаковочныйЛист.гф_Комплектация КАК АртикулУпаковки,
	|	ДокУпаковочныйЛист.гф_Заказ.Контрагент КАК Клиент,
	|	ДокУпаковочныйЛист.гф_Заказ.Партнер.Код КАК КодКлиента,
	|	ДокУпаковочныйЛист.Код КАК КодУпЛиста,
	|	ДокУпаковочныйЛист.гф_Агрегация КАК Агрегат
	|ПОМЕСТИТЬ УпЛист
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.гф_ТоварыВКоробах КАК РеализацияТоварыВКоробах
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УпаковочныйЛист КАК ДокУпаковочныйЛист
	|		ПО РеализацияТоварыВКоробах.УпаковочныйЛист = ДокУпаковочныйЛист.Ссылка 
	|ГДЕ
	|	РеализацияТоварыВКоробах.Ссылка = &СсылкаНаРТУ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ Разрешенные
	...
	|		ПО ТчВложенныеШтрихкоды.Штрихкод = СпрШтрихкоды.Ссылка
	|ГДЕ
	|	ТчВложенныеШтрихкоды.Ссылка В(ВЫБРАТЬ ВтУпЛист.Агрегат ИЗ УпЛист КАК ВтУпЛист)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ Разрешенные
	...
	Запрос.УстановитьПараметр("СсылкаНаРТУ", СсылкаНаРТУ);
...
// vvv Галфинд \ Sakovich 05.04.2024
&НаСервере
Процедура ЗаполнитьТаблицуТоваровИзРТУНекоробное()
	...
	|	"""" КАК АртикулУпаковки,
	|	спрНоменклатура.Артикул КАК Артикул,
	|	Штрихкоды.ШтрихкодУпаковки.Номенклатура КАК Номенклатура,
	|	Штрихкоды.ШтрихкодУпаковки.Характеристика КАК Характеристика,
	|	1 КАК КоличествоБазЕдиниц,
	|	СпрНоменклатура.ЕдиницаИзмерения КАК ЕдИзмБазовая,
	|	Значение(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка) КАК NVE,
	|	спрНоменклатура.СтранаПроисхождения КАК СтранаПроисхождения,
	...
	|	1 КАК КоличествоЭтикеток,
	|	ВЫБОР
	|		КОГДА Штрихкоды.ШтрихкодУпаковки.гф_ПолныйКодМаркировки <> """"
	|			ТОГДА Штрихкоды.ШтрихкодУпаковки.гф_ПолныйКодМаркировки
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ПулКодов.ПолныйКодМаркировки ЕСТЬ НЕ NULL 
	|					ТОГДА ПулКодов.ПолныйКодМаркировки
	|				ИНАЧЕ Штрихкоды.ШтрихкодУпаковки.ЗначениеШтрихкода
	|			КОНЕЦ
	|	КОНЕЦ КАК ПолныйКодМаркировки
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.ШтрихкодыУпаковок КАК Штрихкоды
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПулКодовМаркировкиСУЗ КАК ПулКодов
	|		ПО Штрихкоды.ШтрихкодУпаковки.ЗначениеШтрихкода = ПулКодов.КодМаркировки
	|		левое соединение Справочник.Номенклатура Как спрНоменклатура
	|		по Штрихкоды.ШтрихкодУпаковки.Номенклатура = спрНоменклатура.Ссылка 
	|		левое соединение Документ.РеализацияТоваровУслуг КАК РТУ 
	|			Левое соединение Справочник.Партнеры КАК Партнеры 
	|			По  РТУ.Партнер = Партнеры.Ссылка
	|		по Штрихкоды.Ссылка = РТУ.Ссылка
	|ГДЕ Штрихкоды.Ссылка = &СсылкаНаРТУ
	|УПОРЯДОЧИТЬ ПО
	|	Штрихкоды.ШтрихкодУпаковки.Характеристика.Наименование";
	
	Запрос.УстановитьПараметр("СсылкаНаРТУ", СсылкаНаРТУ);
	
	Объект.Товары.Загрузить(Запрос.Выполнить().Выгрузить());
	
КонецПроцедуры // ^^^ Галфинд \ Sakovich 05.04.2024

&НаКлиенте
Процедура СсылкаНаРТУПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПечатьИзРТУ(Команда)
	
	ПечатьИзРТУНаКлиенте(Команда.Имя);
	
КонецПроцедуры

&НаКлиенте
Процедура ПечатьИзРТУСПредварительнымПросмотром(Команда)
...
&НаКлиенте
Процедура ПечатьИзРТУНаКлиенте(ИмяКоманды)
	...
	Если СтрЗаканчиваетсяНа(ИмяКоманды, "ПредварительнымПросмотром") Тогда
		ПечатьЭтикеткиИзРТУНаКлиенте(ИмяКоманды, Ложь);
	Иначе
		ПечатьЭтикеткиИзРТУНаКлиенте(ИмяКоманды);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПечатьЭтикеткиИзРТУНаКлиенте(ИмяКоманды, ПисатьВРегистрВремениПечати = Истина)
	...
	Если ПисатьВРегистрВремениПечати 
		И Не РежимПечатиНекоробноеРТУ //Добавлено  Галфинд \ Sakovich 05.04.2024
		Тогда
		ЗанестиВРС_ВремяПечати();
	КонецЕсли;
...
&НаКлиенте
Процедура ПечатьИзРТУ_УстановитьФлажки(Команда)
	
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		ИД = СтрокаТовары.ПолучитьИдентификатор();
		Если Элементы.Товары2.ПроверитьСтроку(ИД) Тогда
...
&НаКлиенте
Процедура ПечатьИзРТУ_СнятьФлажки(Команда)
