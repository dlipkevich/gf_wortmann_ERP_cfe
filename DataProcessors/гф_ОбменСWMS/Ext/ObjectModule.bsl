...
Процедура ВыгрузитьДанные (МассивДокументовРасход = Неопределено, МассивДокументовПриход = Неопределено) Экспорт
	...
	Иначе
		...
		МассивДокументовКорректировки = Новый Массив; //чтобы не возникала ошибка и не утежелять код
	КонецЕсли;	
	...
	Для каждого Документ Из МассивДокументовРасход Цикл
		...
		ЭтоРеализация = (ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЗаказКлиента") И Распоряжение.гф_ОтгрузкаПеремещением = Ложь);
...
Функция ПолучитьПакетРасхода(ИмяФайлаБезПути, ИмяФайла, НомерСообщения, Документ, НастройкиFTPСоединения)
	...
	Иначе	
		Пакет.НомерПакета = ?(СтрДлина(НомерПакетаКорректировки) = 1, "0" + НомерПакетаКорректировки, НомерПакетаКорректировки);
...
Функция ЭлементСпискаТоваров_ПарныеСклады(строка, Документ)
	...
	Иначе   
		// Домнышева - временно - нет данных как читать цену
	КонецЕсли;
	//
...
Функция Шапка(Тип, Документ, Партнер, НомерИнвойса = Ложь)
	...
	ИначеЕсли ЭтоКорректировка Тогда
		// нет комментария у корректировки
	Иначе
...
Функция НомерЗаявки(Документ)
	...
	Если ЭтоРеализация Тогда
		НомерРеализации = ПолучитьНомерРеализацииПоЗаказу(Документ);
		НомерЗаявки = КодКлиента + НомерРеализации;
...
Процедура ОбработатьЗагружаемыйФайл(МассивЗагружаемыхФайлов, МассивИменЗагруженныхФайлов, РасходныйОрдер = Ложь)
	...
	Для Каждого ФайлОбмена Из МассивЗагружаемыхФайлов Цикл
		...
		Если Файл.Размер() = 0 Тогда
			...
			Продолжить;
		КонецЕсли;
		чтение = Новый ЧтениеТекста(ФайлОбмена.Файл);
		текст = чтение.Прочитать();
		чтение.Закрыть();
...
Процедура ПроизвестиЗаполнениеИзменениеТабличныхЧастейДокумента(НайденныйДокументОбъект, ОбъектXDTO, ПутьФайла)
	...
	Если СкладПолучательВПарах Тогда
		...
		ОтгружаемыеТоварыДокумента = НайденныйДокументОбъект.ОтгружаемыеТовары.Скопировать(СтруктураОтбора);
		...
		Если ТипЗнч(СтрокиДокументаXDTO) = Тип("СписокXDTO") Тогда
			...
			Для Каждого СтрокаXDTO Из СтрокиДокументаXDTO Цикл 
				ЗаписатьСтрокуОтгружаемыеТовары(НайденныйДокументОбъект, СтрокаXDTO, ОтгружаемыеТоварыИзФайла, ПутьФайла, Истина);
			КонецЦикла;	
		Иначе	
...
Процедура ПроизвестиПроверкуТЧДокументаПО(НайденныйДокументОбъект, ОбъектXDTO, ПутьФайла)
	...
	Иначе
		...
		ТоварыДокумента = НайденныйДокументОбъект.Товары.Скопировать(СтруктураОтбора);
...
Процедура ОбработатьТЧПОПарныйСклад(СтрокиДокументаXDTO, НайденныйДокументОбъект, ПутьФайла)
	...
	ТоварыДокумента = ПреобразоватьТоварыИзДокументаПО(НайденныйДокументОбъект);
	...
	ОтгружаемыеТоварыИзФайла.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	//ОтгружаемыеТоварыИзФайла.Колонки.Добавить("КоличествоУпаковок", ОписаниеЧисло);
	ОтгружаемыеТоварыИзФайла.Колонки.Добавить("Количество", ОписаниеЧисло);
	ОтгружаемыеТоварыИзФайла.Колонки.Добавить("ШтрихкодУпаковки", Новый ОписаниеТипов("СправочникСсылка.ШтрихкодыУпаковокТоваров"));
...
Функция ПреобразоватьТоварыИзДокументаПО(НайденныйДокументОбъект)
	...
	Запрос.Текст = 
		...
		|	ПриходныйОрдерНаТоварыТовары.Характеристика КАК Характеристика,
		|	ПриходныйОрдерНаТоварыТовары.КоличествоУпаковок КАК Количество
		//|	ПриходныйОрдерНаТоварыТовары.гф_IDКороба КАК гф_IDКороба
		|ИЗ
		|	Документ.ПриходныйОрдерНаТовары.ШтрихкодыУпаковок КАК ШтрихкодыУпаковок
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриходныйОрдерНаТовары.Товары КАК ПриходныйОрдерНаТоварыТовары
...
//  ОтгружаемыеТоварыИзФайла - ТаблицаЗначений - заполненная ранее данными из обрабатываемого файла
//	ПутьФайла - Строка
Процедура ЗаписатьСтрокуОтгружаемыеТовары(ОбъектДокумент, СтрокаXDTO, ОтгружаемыеТоварыИзФайла, ПутьФайла, Расход = Ложь)
