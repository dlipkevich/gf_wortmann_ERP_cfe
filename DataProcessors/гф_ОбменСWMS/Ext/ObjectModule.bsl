...
Функция ПроверитьЗначениеСкладМаркетплейс(Склад)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Свойство = "гф_СкладыСкладМаркетплейс" 
				И Выборка.Значение = Истина Тогда
				Возврат Истина;
			КонецЕсли;
...
Функция ПроверитьЗначениеСкладВКоробах(Склад)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Свойство = "гф_СкладыТоварыВКоробах"
				И Выборка.Значение = Истина Тогда
				Возврат Истина;
			КонецЕсли;
...
Функция СтрокаТовара(строка, Документ)
	...
	Если ЭтоПеремещение ИЛИ ЭтоРеализация Тогда
		// ++ Галфинд_ДомнышеваКР_26_04_2024
		СтрокаТовара.Вариант = строка.Артикул;
		СтрокаТовара.Номенклатура = строка.Номенклатура.Наименование;
...
Функция СтрокаТовараПриход(строка, Документ)
	...
	СтрокаТовара.Цена = ПолучитьЦенуПО(строка, Документ);
	СтрокаТовара.Вариант = строка.Артикул;
	СтрокаТовара.Номенклатура = строка.Номенклатура.Наименование;
	СтрокаТовара.Места = Строка(строка.УпаковочныйЛист.ВсегоМест);
	// -- Галфинд_ДомнышеваКР_26_04_2024
...
Процедура ОтправитьПисьмо(ФайлОбмена, ДокументОрдер, ТекстУведомления)
	...
	АвторРеализации = ДокументОрдер.Ответственный; // Галфинд_ДомнышеваКР_26_04_2024
...
Процедура ОбработатьТЧПОПарныйСклад(СтрокиДокументаXDTO, НайденныйДокументОбъект, ПутьФайла)
	...
	Если ВиртуальныеСклады.Найти(НайденныйДокументОбъект.Отправитель) <> Неопределено Тогда
		СравнитьИзменитьТЧТоварыДокумента(НайденныйДокументОбъект, ТоварыДокумента, ОтгружаемыеТоварыИзФайла, 
											ПутьФайла, ТоварыОбувь, Истина);
	Иначе
		СравнитьИзменитьТЧТоварыДокумента(НайденныйДокументОбъект, ТоварыДокумента, ОтгружаемыеТоварыИзФайла, ПутьФайла, ТоварыОбувь);
	КонецЕсли;
...
//	ПутьФайла - Строка 
//	ТоварыОбувь - Булево - признак вида Номенклатуры
...
Процедура СравнитьИзменитьТЧТоварыДокумента(НайденныйДокументОбъект, ТоварыДокумента, ОтгружаемыеТоварыИзФайла,
											ПутьФайла, ТоварыОбувь, СоздатьНовыйПО = Ложь)
	...
	// ++ Галфинд_ДомнышеваКР_23_04_2024
	Если НЕ ТоварыОбувь Тогда
		Возврат;
	КонецЕсли;
	// -- Галфинд_ДомнышеваКР_23_04_2024
