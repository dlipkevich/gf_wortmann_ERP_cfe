...
Процедура ПродолжитьЗагрузку(ФайлыКЗагрузке, Соединение, НастройкиFTPСоединения, ПапкаДляВременнойЗагрузки, ПапкаСклад = Неопределено)
	...
	Если МассивКОбработке.Количество() > 0 Тогда
		...
		ОбработатьПолученныеФайлы(МассивКОбработке, МассивИменЗагруженныхФайлов);
	Иначе    
...
//
// Возвращаемое значение:
//	Файл - каталог для временной загрузки файлов ЛОГ
Функция ПолучитьПапкуНаСервере(ПапкаСклад, КаталогЗагрузки)
	
	ПапкаЛОГ = "LOG";
	Если ПапкаСклад <> Неопределено ТОгда
		Если Прав(КаталогЗагрузки, 1) <> "/" Тогда
			КаталогЗагрузки = КаталогЗагрузки + "/" + ПапкаСклад;
		Иначе
			КаталогЗагрузки = КаталогЗагрузки + ПапкаСклад;
		КонецЕсли;
	КонецЕсли;
