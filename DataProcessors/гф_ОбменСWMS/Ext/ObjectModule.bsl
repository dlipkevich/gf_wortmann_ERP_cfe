...
Процедура ВыгрузитьНаFTP(ФайлДляВыгрузки, НастройкиFTPСоединения, Отказ, Документ, Соединение = Неопределено, 
	...
	Если Соединение = Неопределено Тогда
		ДобавитьСтрокуВТекстыОшибки("Соединение не установлено, проверьте настройки", Истина, ФайлДляВыгрузки.ПолноеИмя, Документ);
		Возврат;
	КонецЕсли;	
...
Процедура ПоискТекстовыхФайлов(Соединение, НастройкиFTPСоединения)
	...
	КаталогиЛОГ = ПолучитьКаталогиЛОГ();
	ПапкаДляВременнойЗагрузки = 
							_омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначение("гф_ПапкаДляВременнойЗагрузкиСоСкладаFTP");
	КаталогЗагрузки = ПапкаДляВременнойЗагрузки + "\" + "LOG";
	СоздатьКаталог(КаталогЗагрузки);
	
	Каталог = Новый Файл(КаталогЗагрузки);
	Если Не Каталог.Существует() Тогда
		ДобавитьСтрокуВТекстыОшибки("Каталог для временной загрузки LOG файлов задан неверно или не существует...",
		Истина, КаталогЗагрузки);
		Возврат;
	КонецЕсли;
	...
	Для каждого Каталог из КаталогиЛОГ Цикл
		...
		МассивТекстовыхФайлов = Соединение.НайтиФайлы(Каталог.ПапкаЛогWMS, "*.txt");
		Соединение = Неопределено; 
		
		Если МассивТекстовыхФайлов.Количество() > 0 Тогда
			Организация = Каталог.Организация;
...
Процедура НайтиДокументИЗаписатьКомментарий(ФайлОбмена)
	...
	НомерДокумента = Прав(МассивСтроки[1], СтрДлина(МассивСтроки[1]) - 3);
...
Процедура ИзменитьСтатусДокумента(Данные, НайденныйДокументОбъект, КоллекцияДокументовXDTO, ФайлОбмена,
	...
	Исключение
		...
		+ НайденныйДокументОбъект.Ссылка + " документ записан." + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ДобавитьСтрокуВТекстыОшибки(Сообщение, Истина, ФайлОбмена.Файл, НайденныйДокументОбъект.Ссылка);
	КонецПопытки;
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		// ^^^ Галфинд \ Sakovich 30.01.2024
			...
			Если УстанавливатьПеремещениюСтатусПринято Тогда
				ВыборкаПеремещение = ПакетРезультатов[0].Выбрать();
				...
				Если Не обПеремещение.Статус = Перечисления.СтатусыПеремещенийТоваров.Принято Тогда
					...
					Исключение
						...
						ЗаписьЖурналаРегистрации("Ошибка", УровеньЖурналаРегистрации.Ошибка, ЭтотОбъект, 
...
Процедура ПолучитьОбработатьИнформациюПоУЛ(НайденныйДокументОбъект, СтрокаXDTO, ПутьФайла, ТаблицаПоУЛ) 
	...
	Если Номенклатура = Неопределено Тогда
		ШаблонТекста = СтрШаблон("Не найдена номенклатура по артикулу %1.", Артикул);
		ДобавитьСтрокуВТекстыОшибки(ШаблонТекста, Истина, ПутьФайла, НайденныйДокументОбъект.Ссылка);
		Возврат;
	КонецЕсли;
