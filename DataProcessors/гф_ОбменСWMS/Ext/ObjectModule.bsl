...
Функция ПолучитьПакетПрихода(ИмяФайлаБезПути, ИмяФайла, НомерСообщения, Документ, НастройкиFTPСоединения)
	...
	УсловиеНеСогласованногоФормата = ЭтоТаможенныйСклад ИЛИ ЭтоСкладНедостачи 
...
Функция ТоварыРасходКорректировка(Документ, Назначение = Неопределено)
	...
	Для каждого строка Из Артикулы Цикл
		НПП = НПП + 1;			
		Товары.Строка.Добавить(СтрокаТовара2(строка, НПП, Истина, Выборка, Назначение));
	КонецЦикла;
...
Функция СтрокаТовара2(строка, НПП, СерийныйНомер, выборка, Назначение = Неопределено)
