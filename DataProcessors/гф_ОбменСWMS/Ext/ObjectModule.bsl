...
Процедура ВыгрузитьДанные (МассивДокументовРасход = Неопределено, МассивДокументовПриход = Неопределено) Экспорт
	...
	Для каждого Документ Из  МассивДокументовПриход Цикл
		ЭтоВозврат = ТипЗнч(Документ.Распоряжение) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента"); // Перенсла выше для заказов
...
Функция АдресаРасход(КомпанияАдрес, Документ, СвязанныйДокумент)
	...
	Если ЗначениеЗаполнено(СтруктураКИ) Тогда
		...
		СтруктураАдреса.Вставить("Представление", АдресДоставки);
...
Функция Шапка(Тип, Документ, Партнер, НомерИнвойса = Ложь)
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.РасходныйОрдерНаТовары") Тогда
		...
		Если НЕ ЭтоСобственныеНужды Тогда
...
Функция СтрокаТовараПриход(строка, Документ)
	...
	ТекстКомментария = строка.Артикул + "|" + ПолучитьЦенуПО(строка, Документ) + "|" + строка.Номенклатура;
...
// Возвращаемое значение:
//	Номер - Строка
Функция ПолучитьНомерРеализацииПоЗаказу(Документ)
...
Процедура ЗагрузитьДанные() Экспорт
	...
	Иначе    
		...
		НомерОшибки = НомерОшибки + 1;
	КонецЕсли;
	// Временно закоментировано - не знаем пока как обрабатывать
	...
	//КонецЕсли;
	Если МассивИменЗагруженныхФайлов.Количество() > 0 Тогда
	//УдалитьФайлыНаФТП(МассивИменЗагруженныхФайлов, НастройкиFTPСоединения, МассивФайловДляУдаления, Соединение);	
	КонецЕсли; 
...
Процедура НайтиДокументИЗаписатьКомментарий(Файл)
	...
	Три = 3; // 3 - количество цифр в коде клиента
	НомерДокумента = Прав(СтрокаИмени, СтрДлина(СтрокаИмени) - Три);
...
Процедура ИзменитьСтатусДокумента(Данные, НайденныйДокументОбъект, КоллекцияДокументовXDTO, ФайлОбмена,
	...
	Исключение
		...
		Сообщение = "Произошла ошибка при проведении " + ?(РасходныйОрдер, "расходного ордера", "приходного ордера") 
		+ НайденныйДокументОбъект.Ссылка + " документ записан." + Символы.ПС + ОписаниеОшибки();
		ДобавитьСтрокуВТекстыОшибки(Сообщение, Истина, ФайлОбмена.Файл);
	КонецПопытки;
...
//	ФайлОбмена - Файл - обрабатываемый файл.
Процедура ПроизвестиЗаполнениеИзменениеТабличныхЧастейДокумента(НайденныйДокументОбъект, ОбъектXDTO, ФайлОбмена, Возвращено)
	...
	Если СкладПолучательВПарах Тогда
		// Изменено Галфинд_ДомнышеваКР_19_12_2023
		
		ОтгружаемыеТоварыИзФайла = ОбработатьТЧРасходПарныйСклад(СтрокиДокументаXDTO, НайденныйДокументОбъект, 
									ПутьФайла, ТоварыОбувь);
		// ++ Галфинд_ДомнышеваКР_19_01_2024							
		Если ТоварыОбувь Тогда
			МассивШтрихкодов = ОтгружаемыеТоварыИзФайла.ВыгрузитьКолонку("ШтрихкодУпаковки");
		КонецЕсли;
		// -- Галфинд_ДомнышеваКР_19_01_2024
		ОтгружаемыеТоварыИзФайла.Свернуть("Номенклатура, Характеристика", "Количество");
		ОтгружаемыеТоварыДокумента = ПолучитьОтгружаемыеТовары(НайденныйДокументОбъект, ТоварыОбувь);
		ПроанализироватьИДобавитьИсходящиеУЛ(НайденныйДокументОбъект, СтрокиДокументаXDTO, ФайлОбмена);
	Иначе
		...
		Если ТипЗнч(СтрокиДокументаXDTO) = Тип("СписокXDTO") Тогда
			...
			Для Каждого СтрокаXDTO Из СтрокиДокументаXDTO Цикл 
				ЗаписатьСтрокуОтгружаемыеТоварыПоУЛ(НайденныйДокументОбъект, СтрокаXDTO, ОтгружаемыеТоварыИзФайла, ПутьФайла, МассивШтрихкодов);
			КонецЦикла;	
		Иначе	
...
Функция ОбработатьТЧРасходПарныйСклад(СтрокиДокументаXDTO, НайденныйДокументОбъект, ПутьФайла, ТоварыОбувь)
	...
	ОтгружаемыеТоварыИзФайла = Новый ТаблицаЗначений;
	ОтгружаемыеТоварыИзФайла.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	...
	Если ТоварыОбувь Тогда
		...
		ОтгружаемыеТоварыИзФайла.Колонки.Добавить("ШтрихкодУпаковки", 
...
Функция ПолучитьОтгружаемыеТовары(НайденныйДокументОбъект, ТоварыОбувь) 
	...
	//Иначе
		...
		ОтгружаемыеТоварыДокумента.Свернуть("Номенклатура, Характеристика", "Количество");
	//КонецЕсли; 
	
	Возврат ОтгружаемыеТоварыДокумента;
...
Процедура СравнитьИзменитьТЧТоварыДокумента(НайденныйДокументОбъект, ТоварыДокумента, ОтгружаемыеТоварыИзФайла,
	...
	Если МассивСтрокКУдалению.Количество() > 0 Тогда
		Для каждого Индекс Из МассивСтрокКУдалению Цикл
			...
			ДобавитьСтрокуВТекстыОшибки(ШаблонТекста, Ложь, ПутьФайла);
...
//	ФайлОбмена - Файл - обрабатываемый файл.
Процедура ПроанализироватьИДобавитьИсходящиеУЛ(НайденныйДокументОбъект, СтрокиДокументаXDTO, ФайлОбмена)
	
	ОписаниеЧисло =  Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3));
	ОписаниеСтрока = Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(200));
	ТаблицаПоУЛ = Новый ТаблицаЗначений;
	ТаблицаПоУЛ.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаПоУЛ.Колонки.Добавить("Характеристика", 
									Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	...
	Если ТипЗнч(СтрокиДокументаXDTO) = Тип("СписокXDTO") Тогда
		...
		Для Каждого СтрокаXDTO Из СтрокиДокументаXDTO Цикл 
			ПолучитьОбработатьИнформациюПоУЛ(НайденныйДокументОбъект, СтрокаXDTO, ФайлОбмена.Файл, ТаблицаПоУЛ);
		КонецЦикла;	
	Иначе	
		ПолучитьОбработатьИнформациюПоУЛ(НайденныйДокументОбъект, СтрокиДокументаXDTO, ФайлОбмена.Файл, ТаблицаПоУЛ);
	КонецЕсли; 

    ПреобразоватьРасходныйОрдерНайтиСоздатьУЛ(НайденныйДокументОбъект, ФайлОбмена, ТаблицаПоУЛ);
...
Процедура ПолучитьОбработатьИнформациюПоУЛ(НайденныйДокументОбъект, СтрокаXDTO, ПутьФайла, ТаблицаПоУЛ) 
	...
	НоваяСтрока.Количество = КоличествоУпаковок;
	НоваяСтрока.КодУЛ = Строка(СтрокаXDTO.lp_info.lp);
...
//  ФайлОбмена - Файл - обрабатываемый файл.
//	ТаблицаПоУЛ - ТаблицаЗначений - собираемая таблица для создания УЛ
Процедура  ПреобразоватьРасходныйОрдерНайтиСоздатьУЛ(НайденныйДокументОбъект, ФайлОбмена, ТаблицаПоУЛ)
...
Функция ПолучитьДокумент(ОбъектXDTO, ФайлОбмена, РасходныйОрдер)
	...
	Если СтрЗаканчиваетсяНа(НомерИзФайла, "S") ИЛИ СтрЗаканчиваетсяНа(НомерИзФайла, "V") 
		...
		НомерДокумента = СокрЛП(Сред(НомерИзФайла, 4, СтрДлина(НомерИзФайла) - 2));
			// ПОИСКРТУ = Ложь;
...
Процедура УдалитьФайлыНаФТП(МассивИменЗагруженныхФайлов, НастройкиFTPСоединения, МассивФайловДляУдаления, Соединение) 
	...
	Для Каждого Файл Из МассивИменЗагруженныхФайлов Цикл 
		Отбор = Новый Структура;
		Отбор.Вставить("Имя", Файл);
		НашлиФайл = МассивФайловДляУдаления.НайтиСтроки(Отбор).Количество();
		Если НашлиФайл > 0 Тогда
			Соединение.Удалить(НашлиФайл[0].ИмяНаФТП);
		КонецЕсли;
	КонецЦикла;
