...
Процедура ВыгрузитьДанные (МассивДокументовРасход = Неопределено, МассивДокументовПриход = Неопределено) Экспорт
	...
	Иначе
		МассивДокументовРасход = ПолучитьМассивДокументов(Истина, МассивДокументовРасход);
		МассивДокументовПриход = ПолучитьМассивДокументов(Ложь, МассивДокументовПриход);
	КонецЕсли;	
...
// #wortmann { 
// Функция формирует массив документов РасходныйОрдерНаТовары или ПриходныйОрдерНаТовары по срезу последних
// со статусом ГотовоКОтправке или (Отправлено с условием что прошло более 5 дней)
// Галфинд_Домнышева 2022/12/01
//
// Параметры:
...
Функция ПолучитьМассивДокументов(РасходныйОрдер, Документы = Неопределено) 
...
Процедура ЗаписатьОтправитьПакет(Пакет, ИмяФайлаБезПути, ИмяФайла, НомерСообщения, ВидДанных, Документ, Настройки)
	...
	Если Пакет = Неопределено Тогда
		...
		//ЗаписатьСообщениеОшибки("Файл не записан" + ИмяФайла);
		Возврат;
	КонецЕсли;
	Попытка
		Отказ = Ложь;
...
Процедура ВыгрузитьНаFTP(ФайлДляВыгрузки, НастройкиFTPСоединения, Соединение = Неопределено, ПомещатьСразуВАрхив = Истина, НовоеРасширение = "", Отказ, НомерСклада = "") Экспорт
	...
	Если Не ЗначениеЗаполнено(ФайлДляВыгрузки) Тогда
		ДобавитьСтрокуВТекстыОшибки("Не определен файл выгрузки", Истина, ФайлДляВыгрузки.ПолноеИмя);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	Если НастройкиFTPСоединения = Неопределено Тогда
		ДобавитьСтрокуВТекстыОшибки("Не определены настройки ftp-соединения", Истина, ФайлДляВыгрузки.ПолноеИмя);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Попытка                                                                                               
		...
		Если ПомещатьСразуВАрхив Тогда
			...
			ИмяАрхивнойПапки = _омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначение("гф_ПапкаДляВыгрузкиНаСервере") + "Archive";
			//////////
			СоздатьКаталог(ИмяАрхивнойПапки);
			
			КаталогАрхив = Новый Файл(ИмяАрхивнойПапки);
			Если Не КаталогАрхив.Существует() Тогда
				ДобавитьСтрокуВТекстыОшибки("Каталог для архивирования файлов задан неверно или не существует...", Истина, Файл.ПолноеИмя);
				Отказ = Истина;
				Возврат;
			КонецЕсли;
...
Процедура ЗагрузитьДанные() Экспорт
	...
	МассивПапок = _омОбщегоНазначенияВызовСервера.ПолучитьГлобальноеЗначениеМассив("гф_СписокПапокДляЗагрузкиСоСкладаFTP");	
	...
	Для Каждого Папка Из МассивПапок Цикл
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ФайлыКОбработке, ФайлыКОбработкеТекущая, Истина);
	КонецЦикла;		
...
Процедура НайтиИОбработатьДокумент(Данные, ФайлОбмена, ДокументПоОбмену, ДокументНайден, РасходныйОрдер)
	...
	Если ТипЗнч(КоллекцияДокументовXDTO) = Тип("СписокXDTO") Тогда
		Для Каждого ТекущийЭлемент Из КоллекцияДокументовXDTO Цикл 
			...
			Если НайденныйДокументОбъект <> Неопределено Тогда
				ИзменитьСтатусДокумента(Данные, НайденныйДокументОбъект, ТекущийЭлемент, ФайлОбмена, ДокументПоОбмену, РасходныйОрдер);
			Иначе	
				Продолжить;
				ДокументНайден = Ложь;
			КонецЕсли;
	...
	Иначе
		...
		Если НайденныйДокументОбъект <> Неопределено Тогда
			ИзменитьСтатусДокумента(Данные, НайденныйДокументОбъект, КоллекцияДокументовXDTO, ФайлОбмена, ДокументПоОбмену, РасходныйОрдер);
		Иначе
			ДокументНайден = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ИзменитьСтатусДокумента(Данные, НайденныйДокументОбъект, КоллекцияДокументовXDTO, ФайлОбмена, ДокументПоОбмену, РасходныйОрдер)
	...
	Если РасходныйОрдер Тогда
		...
		ИначеЕсли Статус = "PICKED" Тогда
			ПроизвестиЗаполнениеИзменениеТабличныхЧастейДокумента(НайденныйДокументОбъект, КоллекцияДокументовXDTO, ФайлОбмена.Файл);
...
Процедура ПроизвестиЗаполнениеИзменениеТЧДокументаПО(НайденныйДокументОбъект, ОбъектXDTO)
	...
	//Иначе
	//	// По заявкам на возврат должно быть не удаление а Действие - НеОтгрузить
	//	ПерезаполнитьТЧОтгружаемыеТовары(НайденныйДокументОбъект, ОтгружаемыеТоварыИзФайла);
	//КонецЕсли;
