...
Процедура ЗаполнитьНаПоставкуНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.НомерПоставки) Тогда 
		СтрокаУсловий = "Где ПриобретениеТоваровУслуг.Ссылка = &ПТУ";
		Запрос.УстановитьПараметр("ПТУ", Объект.НомерПоставки);
	КонецЕсли;
	
	Если ОтборПоСписку И СписокВыбранныхКонтрагентов.Количество() > 0 Тогда
		
		Если ПустаяСтрока(СтрокаУсловий) Тогда 
			СтрокаУсловий = "Где ЗаказКлиента.Контрагент В (&СписокКонтрагентов)";
		Иначе
	...
	ИначеЕсли ЗначениеЗаполнено(КонтрагентПоставки) Тогда
		...
		Если ПустаяСтрока(СтрокаУсловий) Тогда 
			СтрокаУсловий = "Где ЗаказКлиента.Контрагент = &ОтборПоКонтрагенту";
		Иначе
	...
	Если ЗначениеЗаполнено(СезонПоставки) Тогда
		...
		Если ПустаяСтрока(СтрокаУсловий) Тогда 
			СтрокаУсловий = "Где ЗаказКлиента.Договор.гф_Сезон = &гф_Сезон";
		Иначе
	...
	Если ПустаяСтрока(СтрокаУсловий) Тогда
		СтрокаУсловий = "Где ПриобретениеТоваровУслуг.Проведен И ЗаказКлиента.Проведен И УпаковочныйЛист.Проведен"; 
	Иначе
		СтрокаУсловий = СтрокаУсловий + " И ПриобретениеТоваровУслуг.Проведен И ЗаказКлиента.Проведен И УпаковочныйЛист.Проведен"; 
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "//ГДЕ", СтрокаУсловий);
...
Процедура ПолучитьДанныеПоПТУ()
	...
	|СГРУППИРОВАТЬ ПО
	|	втРез.Сезон";	
	Запрос.УстановитьПараметр("гф_Поставка", Объект.НомерПоставки);
