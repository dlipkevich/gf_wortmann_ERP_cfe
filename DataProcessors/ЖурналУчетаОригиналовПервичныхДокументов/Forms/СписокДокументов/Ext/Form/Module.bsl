...
Процедура гф_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	ДобавитьУсловноеФорматирование(); // Галфинд_ДомнышеваКР_25_04_2024
...
Процедура гф_КомандаПечать(Команда)
	...
	ОтправленЭДО = ПолучитьСтатусОтправленПоЭДО(); // Галфинд_ДомнышеваКР_25_04_2024
	...
	Для Каждого ВыделеннаяСтрока Из Элементы.СписокДокументов.ВыделенныеСтроки Цикл
		...
		Если ДанныеСтроки <> Неопределено Тогда
			...
			// ++ Галфинд_ДомнышеваКР_25_04_2024
			// необходимо заблокировать возможность печати документа со статусом "Отправлен по ЭДО"
			Если ДанныеСтроки.СостояниеОригиналаПервичногоДокумента = ОтправленЭДО Тогда
				
				 ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Документ со статусом ""Отправлен по ЭДО"" не подлежит печати.");
				 Продолжить;
...
//	- ТаблицаЗначений - Таблица статусов.
//
&НаСервере
Функция ПолучитьТаблицуСтатусов(ТабличныйДокумент) Экспорт
	...
	Для НомерСтроки = НачальнаяСтрока По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		НоваяСтрока.Номер = ТабличныйДокумент.Область(НомерСтроки
		, КолонкаНомер
		, НомерСтроки
		, КолонкаНомер).Текст;     
		
		ОбластьДляКуба = ТабличныйДокумент.Область(НомерСтроки
			, КолонкаДата
			, НомерСтроки
			, КолонкаДата);
		
		НоваяСтрока.Дата = СтроковыеФункцииКлиентСервер.СтрокаВДату(ОбластьДляКуба.Текст);
		
		НоваяСтрока.ВидДокумента = ТабличныйДокумент.Область(НомерСтроки
			, КолонкаВидДокумента
			, НомерСтроки
			, КолонкаВидДокумента).Текст;
		
		НоваяСтрока.ПервичныйДокумент = ТабличныйДокумент.Область(НомерСтроки
			, КолонкаПервичныйДокумент
			, НомерСтроки
			, КолонкаПервичныйДокумент).Текст;
		
		НоваяСтрока.СостояниеОригинала = ТабличныйДокумент.Область(НомерСтроки
			, КолонкаСостояниеОригинала
			, НомерСтроки
			, КолонкаСостояниеОригинала).Текст;
...
// #wortmann {
// Галфинд_ДомнышеваКР_25_04_2024
// Доработка условного оформления - выделение документов со статусом "Отправлен по ЭДО"
//
&НаСервере
Процедура ДобавитьУсловноеФорматирование()
	...
	ЭлементОтбора.ПравоеЗначение = Справочники.СостоянияОригиналовПервичныхДокументов.НайтиПоНаименованию("Отправлен по ЭДО");
	ЭлементОтбора.Использование = Истина;
	
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветФона", ЦветаСтиля.ЦветФонаУдачнойОтправки);

КонецПроцедуры// } #wortmann

// #wortmann {
// Галфинд_ДомнышеваКР_25_04_2024
// Возвращает значение "Отправлен по ЭДО"
//
//	Возвращаемое значение:
//	- СправочникСсылка.СостоянияОригиналовПервичныхДокументов - значение "Отправлен по ЭДО"
//
&НаСервере
Функция ПолучитьСтатусОтправленПоЭДО() 
	
	Возврат Справочники.СостоянияОригиналовПервичныхДокументов.НайтиПоНаименованию("Отправлен по ЭДО");
